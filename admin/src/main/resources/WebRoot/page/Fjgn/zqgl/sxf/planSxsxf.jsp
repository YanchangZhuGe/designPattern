...
<script type="text/javascript">
   ...
   /* var WF_STATUS = getQueryParam("WF_STATUS");//当前状态
    ...
    var adLevel = adCode.length() == 2?"2":"3";
    //全局变量
    ...
    var Fxsxf_json_common = {
            ...
            '001': [
                {
                    ...
                    handler: function (btn) {
                        ...
                        for (var i in records) {
                            if (adLevel == '2') {
                                ids.push(records[i].get("ZQ_ID"));
                            } else {
                                ids.push(records[i].get("ZD_DETAIL_ID"));
                            }
                        }
                        $.post("/createMssxf.action", {
                            adLevel: adLevel,
        ...
        store: {
            WF_STATUS: DebtEleStore(json_debt_zt5)
        }
    };
    /**
     * 页面初始化
    ...
    function initContent() {
        var items;
        if (adLevel == '2') {
            items = [initContentGrid(), initContentDetilGrid()];
        } else if (adLevel == '3') {
            items = [initContentGridMs(), initContentDetilGrid()];
        }
    ...
    function initContentGrid() {
        ...
        return DSYGrid.createGrid({
            ...
            dataUrl: 'getMssxfMainGridData.action',
            checkBox: true,
            border: false,
            autoLoad: false,
            height: '100%',
            ...
            listeners: {
                itemclick: function (self, record) {
                    //刷新明细表
                    DSYGrid.getGrid('contentGrid_detail').getStore().getProxy().extraParams['ZQ_ID'] = record.get('ZQ_ID');
                    DSYGrid.getGrid('contentGrid_detail').getStore().getProxy().extraParams['adLevel'] = adLevel;
                    DSYGrid.getGrid('contentGrid_detail').getStore().getProxy().extraParams['WF_STATUS'] = WF_STATUS;
    ...
    function initContentGridMs() {
        ...
        return DSYGrid.createGrid({
            ...
            dataUrl: 'getQxsxfMainGridData.action',
            checkBox: true,
            border: false,
            autoLoad: false,
            height: '100%',
            ...
            listeners: {
                itemclick: function (self, record) {
                    //刷新明细表
                    DSYGrid.getGrid('contentGrid_detail').getStore().getProxy().extraParams['ZD_DETAIL_ID'] = record.get('ZD_DETAIL_ID');
                    DSYGrid.getGrid('contentGrid_detail').getStore().getProxy().extraParams['adLevel'] = adLevel;
                    DSYGrid.getGrid('contentGrid_detail').getStore().getProxy().extraParams['WF_STATUS'] = WF_STATUS;
                    DSYGrid.getGrid('contentGrid_detail').getStore().loadPage(1);
                }
    ...
    function initContentDetilGrid(callback) {
        ...
        var config = {
            ...
            dataUrl: 'getMssxfDtlGridData.action'
        };
        if (Fxsxf_json_common.item_content_detailgrid_config) {
            config = $.extend(false, config, Fxsxf_json_common.item_content_detailgrid_config);
        }
