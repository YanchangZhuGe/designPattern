...
*/
                    ...
                    '001': [//未送审
                        ...
                        },
                        {
                            ...
                            handler: function (btn) {
                                ...
                                button_name = btn.text;
                                //发送ajax请求，查询主表和明细表数据
                                $.post("/getSjZdhkxxGridData.action", {
    ...
    $(document).ready(function () {
        ...
        if (df_plan_id != null && df_plan_id != "") {
            $.post("/getId.action", function(dataId) {
                ...
                //发送ajax请求，查询主表和明细表数据
                $.post("/getSjZdhkDfdataByDfPlanId.action", {
    ...
    function initContentGrid() {
        ...
        return DSYGrid.createGrid({
            ...
            dataUrl: 'getSjZdHkMainGridData.action',
            checkBox: true,
            border: false,
            height: '100%',
            tbar: [
    ...
    function initContentDetilGrid(callback) {
        var config = {
            ...
            flex: 1,
            dataUrl: '/getSjZdhkxxGridData.action',
            checkBox: true,
            border: false,
            autoLoad: false,
            height: '100%',
    ...
    function initWindow_select() {
        var items ;
        //解决下拉选框绘制两次问题
        if ((userAdCode.length == 4 && !(userAdCode.lastIndexOf("00") == 2)) || userAdCode.length == 2) {
            items = [initWindow_select_grid(), initWindow_select_grid_detail()]
        }else {
            items=[initWindow_select_grid()];
        }
    ...
    function initWindow_select_grid() {
        var headerJson = [
            {xtype: 'rownumberer', width: 45},
            {text: "债券ID", dataIndex: "ZQ_ID", type: "string", hidden: true},
            {text:"区划名称",dataIndex: "AD_NAME",type:"string"},
        ...
        return DSYGrid.createGrid({
            ...
            width: '100%',
            flex: 1,
            dataUrl: 'getSjZdHkjhGridData.action',  //转贷还款计划 视图：DEBT_V_ZQGL_ZDHKJH
            pageConfig: {
                enablePage: false
            },
            dockedItems: [
    ...
    function initWindow_select_grid_detail() {
        ...
        return DSYGrid.createGrid({
            ...
            dataUrl: 'getSjZdHkXjzdGridData.action',
            features: [{
                ftype: 'summary'
            }]
        });
    ...
    function doWorkFlow(btn) {
        ...
        if (button_name == '送审') {
            Ext.Msg.confirm('提示', '请确认是否' + button_name + '!', function (btn_confirm) {
                if (btn_confirm === 'yes') {
                    //发送ajax请求，修改节点信息
                    $.post("/updateSjZdhkxxNode.action", {
