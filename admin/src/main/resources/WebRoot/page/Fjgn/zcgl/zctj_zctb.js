...
//资产类别基础数据
var ZCLB_ID_Store = DebtEleTreeStoreDB("DEBT_ZCLB");
//处置类型
...
function initContentRightPanel() {
    return Ext.create('Ext.form.Panel', {
                ...
                items: [
                    {
                        ...
                        fieldLabel: '状态',
                        name: 'SB_STATUS',
                        store: DebtEleStore(zcgl_json_common['zhuangtai']),
                        allowBlank: false,
                        ...
                        listeners: {
                            change: function (self, newValue) {
                                var SB_STATUS = newValue;
                                var toolbar = Ext.ComponentQuery.query('toolbar#contentPanel_toolbar')[0];
                                toolbar.removeAll();
                                toolbar.add(zcgl_json_common['zctb'].items[SB_STATUS]);
                                //刷新当前表格
                                reloadGrid();
                                if(newValue==002){
                                    var grid = DSYGrid.getGrid('zctbGrid');
                                    for (var i = 0; i < grid.columns.length; i++) {
                                        if (grid.columns[i].dataIndex == 'SBQJ_NAME' ) {//上报期间显示
                                            grid.columns[i].setHidden(false);
                                            break;
                                        }
                                    }
                                }else{
        ...
        items: zcgl_json_common['zctb'].items_content_rightPanel_items ? zcgl_json_common['zctb'].items_content_rightPanel_items() : [
            initXMZCTBGrid(),
            initXMZCTBMXGrid()
        ]
    });
}
$.extend(zcgl_json_common['zctb'], {
        ...
        '001': [
            {
                ...
                handler: function (btn) {
                    ...
                    //放开增加时对资产的校验
                    /*$.post('/isCanZCTB.action', {AG_ID: AG_ID}, function (data) {
                        if (data.success) {
                            if (!data.exists) {
                                ...
                                Ext.MessageBox.alert('提示', '该单位数据已经上报，不能再增加!');
                            }
                        }
                    }, 'json');*/
                    chooseProjectWindow.show();
                ...
                handler: function (btn) {
                    ...
                    AG_ID = treeArray[1].getSelection()[0].get('id');
                    AG_NAME = treeArray[1].getSelection()[0].get('text');
                    if (DSYGrid.getGrid('zctbGrid').getStore().data.items.length == 0) {
                        Ext.MessageBox.alert('提示', '该单位下没有未上报项目，不能进行上报操作！')
                        return;
                    }
                    sbDW_name = AG_NAME;
                    var grid=WeiXingzichan();
                    grid.getStore().load(function () {
                        ...
                        ConfirmWindow({height:document.body.clientHeight*0.8,title:'提示'},'本单位以下项目未形成资产,且上报后不能再新增或修改数据，是否确认上报?',grid,function (self) {
                            if (IS_INPUT_WXCZC_REASON) {//需要填写未形成资产原因，则校验必录项
                                var flag = false;
                                var records = grid.getStore().getData().items;
                                Ext.Array.forEach(records, function (record) {
                                    var map = {};
                                    map.XM_ID = record.get("XM_ID");
                                    map.WXCZC_REASON  = record.get("WXCZC_REASON");
                                    if (map.WXCZC_REASON == null || map.WXCZC_REASON == undefined || map.WXCZC_REASON == '') {
                                        flag = true;
                                        return;
                                    }
                                    reasonList.push(map);
                                ...
                                if (flag) {
                                    Ext.MessageBox.alert('提示', button_name + '失败！' + '" 未形成资产原因 "为必填项！');
                                    return;
                                }
                            }
        ...
        '002': [
            {
                ...
                handler: function (btn) {
                    var treeArray = btn.up('panel').down('panel#treePanel_left').query('treepanel');
                    var selected_ad = treeArray[0].getSelection()[0]; //获取区划树的地区code
                    var selected_ag = treeArray[1].getSelection()[0];//获取区划树的单位code
                    var records=DSYGrid.getGrid('zctbGrid').getSelection();
                    var ids=[];
                    var sbqj=[];
                    var json={};//创建一个空对象
                    /* if (!selected_ad && !selected_ag) {
                     ...
                     AG_ID = treeArray[1].getSelection()[0].get('id');*/
                    if(records.length<=0){
                        Ext.Msg.alert('提示','请选择至少一条后再进行操作');
                        return;
                    }
                    for(var i in records){//把取得的每一个数组元素作为对象的属性进行保存 并赋值为1 来达到去重的效果
                        if(!json[records[i].get("AG_ID")]){
                            ids.push(records[i].get("XMZC_ID"));
                            json[records[i].get("AG_ID")]=1;
                        }
                    }
                    Ext.Ajax.request({
                        url: 'canCancelSB.action',
                        params: {
                            ids: ids
                        },
                        method: 'post',
                        success: function (data) {
                            ...
                            if (respText.success) {
                                if (respText.status) {
                                    Ext.MessageBox.confirm('提示', '是否撤消已上报资产(提示:本单位下的所有资产将被同时撤销)？', function (button) { //' + treeArray[1].getSelection()[0].get('text') + '
                                        if (button == 'yes') {
                                            Ext.Ajax.request({
                                                url: 'cancelZCSB.action',
                                                params: {
                                                    ids: ids,
                                                    AD_CODE: AD_CODE
                                                },
...
//重新加载项目资产填报表格
function reloadGrid() {
    var zctbGrid = DSYGrid.getGrid("zctbGrid");
    var zctbStore = zctbGrid.getStore();
    var mhcx = Ext.ComponentQuery.query('textfield[name="mhcx"]')[0].value;
    var SB_STATUS = Ext.ComponentQuery.query('combobox[name="SB_STATUS"]')[0].value;
    zctbStore.getProxy().extraParams['mhcx'] = mhcx;
    zctbStore.getProxy().extraParams['AD_CODE'] = AD_CODE;
    zctbStore.getProxy().extraParams['AG_CODE'] = AG_CODE;
    zctbStore.getProxy().extraParams['SB_STATUS'] = SB_STATUS;
    zctbStore.loadPage(1);
        ...
        buttons: [
            {
                ...
                handler: function (btn) {
                    ...
                    var qt=accAdd(formValues.QT_AMT,formValues.QITA_AMT);
                    ...
                    Ext.Array.forEach(data, function (value) {
                        ...
                        if (  value.data.ZCXZ_ID.indexOf('01') == 0  && !(value.data.SNLJSY_AMT > 0)) {
                            flag = true;
                            message = '资产性质为01时,本期收益大于0！';
                            return;
                        }
                        ...
                        if (value.data.LJSY_AMT < value.data.SNLJSY_AMT ) {
                            flag = true;
                            message = '表格中累计收益应大于等于本期收益';
                            return;
                        }
    ...
    var headerJson = [
        ...
        },
        {
            ...
            'columns': [{
                ...
                "type": "float",
                "text": "累计收益",
                'editor': {hideTrigger: true, xtype: 'numberFieldFormat', allowBlank: false, decimalPrecision: 2, maxValue: 9999999999.99},
                summaryType: 'sum',
                summaryRenderer: function (value) {
                    return Ext.util.Format.number(value, '0,000.00');
                }
                ...
                "type": "float",
                "text": "每年平均经营性现金流收入",
                'editor': {hideTrigger: true, xtype: 'numberFieldFormat', allowBlank: false, decimalPrecision: 2},
                summaryType: 'sum',
                summaryRenderer: function (value) {
                    return Ext.util.Format.number(value, '0,000.######');
                }
...
var sbWindow = {
    window: null,
    show: function (isHaveProject) {
        if (!this.window) {
            this.window = initSbwindow(isHaveProject);
        }
        this.window.show();
    }
};
function initSbwindow(isHaveProject) {//isHaveProject是否有未形成资产的项目
    return Ext.create('Ext.window.Window', {
        title: '选择'+ sbDW_name+'上报期间',
        width: 375,
        height: 200,
        itemId: 'sbWindow',
        closeAction: 'destroy',
        ...
        buttons: [
            {
                ...
                handler: function (btn) {
                    ...
                    if (!isHaveProject) {
                        Ext.Msg.confirm("提示", '上报后不能再新增或修改数据，是否确认上报？', function (botton) {

                            if (botton == 'yes') {
                                sbRequest(btn,form,sbqj);
                            } else {
                                btn.up('window').close();
                            }
                        });
 ...
 * 上报请求方法
 */
function sbRequest(btn,form,sbqj){
    Ext.Ajax.request({
        url: 'isCanZCSB.action',
        params: {
            AG_ID: AG_ID,
            AD_CODE: AD_CODE,
            SBQJ_ID: sbqj
        ...
        method: 'post',
        success: function (data) {
        	var xmzc_store = DSYGrid.getGrid('zctbGrid').getStore();
        	var xmzc_ids = [];
        	for (var i=0;i<xmzc_store.getCount();i++) {
        		xmzc_ids.push(xmzc_store.getAt(i).get('XMZC_ID'));
        	}
            var responseText = Ext.util.JSON.decode(data.responseText);
            if (responseText.success) {
                if (responseText.canZCSB) {
                    Ext.Ajax.request({
                        url: 'sbXMZC.action',
                        params: {
                            AD_CODE: AD_CODE,
                            AG_ID: AG_ID,
                            SBQJ_ID : form.getValues().SBQJ,
                            XMZC_IDS : xmzc_ids
                        },
                        method: 'post',
                        success: function (data) {
                            ...
                            if (respText.success) {
                                if (respText.success) {
                                    if (IS_INPUT_WXCZC_REASON && reasonList.length > 0) {//需要将填写未形成资产原因保存到项目信息表
                                        $.post("/saveReasonToProjectInfo.action", {
                                            reasonList: Ext.util.JSON.encode(reasonList)
                                        }, function (data) {
                                            if (data.success) {
                                                Ext.toast({
                                                    html: '上报成功！',
