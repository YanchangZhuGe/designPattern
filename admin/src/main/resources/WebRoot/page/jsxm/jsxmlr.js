...
var tab_items_load = {'jbqk':{},'tzjh':{},'szys':{},/*'clzw':{},'xmzc':{},*/'ztb':{},'jsjd':{},'sjsy':{},'sdgc':{}};
...
function getHbfxDataList() {
    ...
    var XMCJSJ = Ext.ComponentQuery.query('combobox#XMCJSJ_SEARCH')[0].getValue();
    //初始化表格Store参数
...
function initContentTabGrid() {
    return Ext.create('Ext.tab.Panel', {
        ...
        items: [
            ...
            {title: '全部项目', opstatus: 5, items:[initContentGrid("contentGrid5") ]},
    ...
    if(is_zxzq=='1'){
        ...
        if (isOld_szysGrid == '1') {
            ...
            if (szysGrid.getStore().data.length > 0 &&( ((xm_used_date == null) || (xm_used_date == "") || xm_used_date == undefined) || ((xm_used_limit == null) || (xm_used_limit == "") || xm_used_limit==undefined))){
                message_error = "收支预算：项目投入使用日期或项目期限不能为空！";
                Ext.Msg.alert('提示', message_error);
                var i=  findTabName('收支预算');
                zqxxtbTab(i);
                return false;
            ...
            if (szysGrid.getStore().data.length <= 0 &&(((xm_used_date != null) && (xm_used_date != "") && xm_used_date != undefined) || ((xm_used_limit != null) && (xm_used_limit != "") && xm_used_limit!=undefined))){
                message_error = "收支预算：项目投入使用日期或项目期限不为空时，必须有收入！";
                Ext.Msg.alert('提示', message_error);
                var i=  findTabName('收支预算');
                zqxxtbTab(i);
                return false;
            ...
            szysGrid.getStore().each(function (record) {
                if(!((parseInt(record.get('INCOME_YEAR'))>=XM_USED_DATE_int)&&(parseInt(record.get('INCOME_YEAR'))<=(XM_USED_DATE_int+XM_USED_LIMIT_int)))){
                    message_error="收支预算:表中年度不能大于项目投入使用日期与项目期限之和，不能小于项目投入使用日期所在年度";
            ...
            if(is_zxzq=='1'){
                if(is_lxcs!=1){
                    message_error = "收支预算信息存在变更操作，请重新进行利息测算！";
                    var i=  findTabName('收支预算');
                    zqxxtbTab(i);
                }
            }
            ...
            //     message_error = "收支预算：项目对应的政府性基金科目不为空时，必须录入项目预计总收入！";
            //     Ext.Msg.alert('提示', message_error);
            //     var i=  findTabName('收支预算');
            //     zqxxtbTab(i);
            //     return false;
            ...
            if (xmztr_amt > 0 && ((zfxjjkm_id == null) || (zfxjjkm_id == "") || zfxjjkm_id == undefined)) {
                message_error = "收支预算：项目预计总收入不为0时，项目对应的政府性基金科目不可为空！";
                Ext.Msg.alert('提示', message_error);
                var i=  findTabName('收支预算');
                zqxxtbTab(i);
                return false;
            ...
            if (xmztr_amt <= 0 && ((xm_used_date != null && xm_used_date != ""&& xm_used_date != undefined) || (xm_used_limit != null && xm_used_limit != "" && xm_used_limit != undefined))) {
                message_error = "收支预算：开始日期或预算年限不为空时，必须录入项目预计总收入！";
                Ext.Msg.alert('提示', message_error);
                var i=  findTabName('收支预算');
                zqxxtbTab(i);
                return false;
            ...
            if (xmztr_amt > 0 && ( ((xm_used_date == null) || (xm_used_date == "") || xm_used_date == undefined) || ((xm_used_limit == null) || (xm_used_limit == "") || xm_used_limit == undefined)  )) {
                message_error = "收支预算：项目预计总收入不为0时，开始日期和项目期限不能为空！";
                Ext.Msg.alert('提示', message_error);
                var i=  findTabName('收支预算');
                zqxxtbTab(i);
                return false;
            ...
            if(is_tdcb){
                ...
                // “评估土地使用权出让收入”之和大于等于“土地储备项目资金支出”之和，否则提示：土储项目应当实现总体收支预算 01 >= 03
                // 当年“土地储备项目资金”必须大于等于“土地储备项目资金支出”，否则提示：土储项目应当实现年度收支预算 02 >= 03
                ...
                var record_01 = szysGrid.getStore().getAt(gs_relation_guid["01"]);// 四、项目运营支出
                ...
                if(Math.abs((parseFloat(schrzhb_amt).toFixed(6)-parseFloat(schrz_amt).toFixed(6))) > 0.000001){
        ...
        var zqxxtbTab = Ext.create('Ext.tab.Panel', {
            ...
            items: [
                ...
                },
                {
                    ...
                    scrollable: true,
                    hidden: is_zxzq == '1' ? true : false,//专项隐藏,一般放开
                ...
                },
                {
                    ...
                    btnsItemId: 'szys',
                    scrollable: true,
                    hidden: is_zxzq == '1' ? false : true,//一般隐藏,专项放开
            ...
            listeners: {
                tabchange: function (tabPanel, newCard, oldCard) {
                    if (isOld_szysGrid == '0' && newCard.btnsItemId == 'szys') {
                        ...
                        Ext.ComponentQuery.query('button#lxcs')[0].setHidden(false);//20211025liyue同步利息测算按钮
                        ...
                        Ext.ComponentQuery.query('button#lxcs')[0].setHidden(true);//20211025liyue同步利息测算按钮
                    }
                    if(is_tdcb==true){//20201110_wangyl_土地储备隐藏导入按钮
                    	 Ext.ComponentQuery.query('button#import')[0].setHidden(true);
                    	 Ext.ComponentQuery.query('button#mbxz')[0].setHidden(true);
                        Ext.ComponentQuery.query('button#lxcs')[0].setHidden(true);
            ...
            items: [
                '->',
                {
                    ...
                    handler: function (btn) {
                        ...
                        if(lenght_temp==0){
                            btn.up('grid').insertData(lenght_temp,{INCOME_YEAR:used_date_data.substring(0,4)
                            });
                        }
                        if(lenght_temp>0){
