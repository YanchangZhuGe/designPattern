...
var zbwj_store=DebtEleTreeStoreDBTable("dsy_v_ele_zbwjxd"); //指标文件下达
Ext.define('ZQModel', {
...
$(document).ready(function () {
    ...
    $.post("getWfNodeName.action?wf_id=" + wf_id + "&node_code=" + node_code, function (data) {
        ...
        // 20210713 guoyf 审核岗去除被退回、曾经办、状态
        //根据节点名称初始化状态下拉框store
        /*if (nodeType == "END" || node_type == 'zcsh') {
            json_zt = json_debt_ztlc;
        } else {
        ...
        buttons: [
            {
                ...
                handler: function (btn) {
                    ...
                    //弹出新增债券填报页面
                    var window_zhmx = initWindow_select_xzzq({ZCD_YEAR: data_zdmx.ZCD_YEAR, ZQ_ID: data_zdmx.ZQ_ID});
                    window_zhmx.show();
...
function initWindow_select_zdmx_grid() {
    var headerJson = [
        //20210722 fzd 转贷债券选择错行问题，加上序号，数据量大时会错行
        //{xtype: 'rownumberer', width: 45},
        {
...
function initWindow_select_xzzq_grid(param) {
    var headerJson = [
        ...
        {text: "建设性质", dataIndex: "JSXZ_NAME", type: "string", hidden: true},
        {text: "项目类型", dataIndex: "XMLX_NAME", type: "string", width: 180},
        {text: "项目性质", dataIndex: "XMXZ_NAME", type: "string", width: 180, hidden: true},
        {text: "立项审批级次", dataIndex: "SPJC_NAME", type: "string", hidden: true},
        {text: "计划开工日期", dataIndex: "START_DATE_PLAN", type: "string", hidden: true},
        {text: "计划竣工日期", dataIndex: "END_DATE_PLAN", type: "string", hidden: true},
        {text: "开工日期", dataIndex: "START_DATE_ACTUAL", type: "string"},
        {text: "竣工日期", dataIndex: "END_DATE_ACTUAL", type: "string"},
...
function initWindow_save_zcxx_grid(config) {
    var headerJson = [
       ...
       {
            text: "指标文号", dataIndex: "ZBWJ_CODE", type: "string",width: 200, tdCls: 'grid-cell',  allowblank: true,
           editor: {
                ...
                // lines: false,
               listeners: {
                    change: function (self, newValue, ZBWJXD_AMT) {
                        if(isNull(self.value)){
                            Ext.ComponentQuery.query('grid[itemId="window_save_zcxx_grid"]')[0].getSelectionModel().getSelection()[0].set("ZBWJXD_AMT", 0);
                        }
                        var record = zbwj_store.findNode('id', newValue, true, true, true);

                        if(!isNull(newValue)){
                            $.post("/getIsZBWJById.action", {
                                change_code: newValue
                            }, function (data) {
                                if (!data.success) {
                                    Ext.MessageBox.alert('提示', data.message);
                                    return;
                                } else {
                                    var ZBWJXD_AMT_VAL = data.list[0].ZBWJXD_AMT;
                                    var SYYZC_AMT = data.list[0].SYYZC_AMT;
                                    Ext.ComponentQuery.query('grid[itemId="window_save_zcxx_grid"]')[0].getSelectionModel().getSelection()[0].set("ZBWJXD_AMT", ZBWJXD_AMT_VAL);
                                    Ext.ComponentQuery.query('grid[itemId="window_save_zcxx_grid"]')[0].getSelectionModel().getSelection()[0].set("SYYZC_AMT", SYYZC_AMT);
                                }
            ...
            renderer: function (value, metaData, record) {
                var record1 = zbwj_store.findNode('id', value, true, true, true);
                return record1 != null ? record1.get('name') : "";
            }
        },
        {
            text: '指标文件下达额度(元)', dataIndex: 'ZBWJXD_AMT', type: 'float', tdCls: 'grid-cell-unedit', width: 160, hidden: sysAdcode != '21',
            renderer: function (value) {
                return Ext.util.Format.number(value, '0,000.######');
            }
        },
        {
            text: '剩余已下达金额(元)', dataIndex: 'SYYZC_AMT', type: 'float', tdCls: 'grid-cell-unedit', width: 160, hidden: sysAdcode != '21',
            renderer: function (value) {
                return Ext.util.Format.number(value, '0,000.######');
            }
        },

        {fieldLabel: "BF_LEVEL", name: "BF_LEVEL", readOnly: true, hidden :true, fieldCls: 'form-unedit'},
        ...
        {
            text: "本次支出金额(元)", dataIndex: "BCZC_AMT", type: "float", width: 160, tdCls: 'grid-cell', //PAY_AMT
        ...
        },
        {
            ...
            text: "工程类别", dataIndex: "GCLB_ID", type: "string",width: 200, tdCls: 'grid-cell',hidden:sysAdcode=='42'?false:true,allowblank:sysAdcode=='42'?false:true,
            editor: {
                xtype: 'treecombobox',
                displayField: 'text',
                valueField: 'id',
...
function checkEditorGrid(grid) {
    ...
    for (var i = 0; i < grid.getStore().getCount(); i++) {
        ...
        if (typeof (tzjhNd[record.get('ZBWJ_CODE')]) != 'undefined') {
            return "指标文号不能相同";
        } else {
            tzjhNd[record.get('ZBWJ_CODE')] = '' ;
        }
        ...
        if (!record.get('BCZC_AMT') || record.get('BCZC_AMT') == null || record.get('BCZC_AMT') <= 0 ||record.get('BCZC_AMT') > record.get('ZBWJXD_AMT') ) {
             return '支出金额必须大于0！并不得大于指标文件下达额度';
        }
        if (record.get('ZBWJXD_AMT') < record.get('PAY_XZ_AMT_TOTAL')) {
            return '指标文下达额度不能大于新增债券金额！';
        }
        if (record.get('BCZC_AMT') < record.get('ZX_ZBJ_AMT')) {
            return '专项债用作资本金不能大于支出金额！';
        }
        if (record.get('BCZC_AMT') < record.get('XZCZAP_AMT')) {
            return '其中:新增赤字金额不能大于支出金额！';
        }
...
function initZcxxData_update(config) {
    ...
    $.post("/getFxdfZqsyZqzcGridById.action", {
        ...
        zbwj_store=DebtEleTreeStoreDBTable("dsy_v_ele_zbwjxd",{condition: " AND ZQ_ID = '"+zq_id+"' "});
        if (data.success) {
...
function initZcxxData_confirm(btn, ids, is_show) {
    ...
    $.post('/getFxdfZqsyXzzqGrid_select.action', {ids: ids, ZCD_YEAR: btn.ZCD_YEAR, ZQ_ID: btn.ZQ_ID}, function (data) {
        if (!data.success) {
...
function initZcxx_data_zcmx_ADD(data_zcmx, window_zc, is_show) {
    ...
    for (var i = 0; i < data_zcmx.length; i++) {
        ...
        if (!xm) {
            ...
            data_zcmx[i].BCZC_AMT = data_zcmx[i].BCZC_AMT;// 取值为该笔债券该个项目的发行金额
            data_zcmx[i].ZX_ZBJ_AMT = data_zcmx[i].BCZC_ZX_ZBJ;// 取值为该笔债券该个项目的专项债用作资本金金额
            data_zcmx[i].XZCZAP_AMT = data_zcmx[i].XZCZAP_AMT;// 取值为该笔债券该个项目的专项债用作资本金金额
...
function initZcxx_data_zcmx_UPDATE(data_zcmx, window_zc, is_show) {
    ...
    for (var i = 0; i < data_zcmx.length; i++) {
        ...
        xm.YZC_AMT_BDCS = xm.YZC_AMT_BDCS + data_zcmx[i].BCZC_AMT;//累加本单初始
        xm.YZC_AMT_BD = xm.YZC_AMT_BDCS;
        xm.WBF_AMT = xm.YS_AMT - xm.YZC_AMT_CS - xm.YZC_AMT_BD + xm.YZC_AMT_BDCS;
        xm.WBF_AMT_CS = xm.WBF_AMT;//仅作为显示用
    }
