...
public class GwmsCommonUtil {
    ...
    public static String getStaffNo(String tempStaffNo) {
        String staffNo = null;
        Profile profile = GwmsCommonUtil.getProfile();
        if (profile != null) {
    ...
    public static Date transDate(Date date) {
        if (date != null) {
            try {
                ...
                log.error("----------GWMS.CommonUtil.transDate异常----------");
                e.printStackTrace();
            }
        }
        //未正常格式化的情况下返回原来的时间
...
//                System.out.println(f.get(object));
    ...
    public static Integer diffDays(Date date1, Date date2) {
        ...
        date1 = GwmsCommonUtil.transDate(date1);
        date2 = GwmsCommonUtil.transDate(date2);
     ...
     * @return
     */
    public static Object copyPageCommon(Object response, Page pageView) {
        if (response != null && pageView != null) {
            Integer page = 0;
            Integer limit = 0;
            Integer total = 0;
            int size = 0;
            ...
            //response = BeanUtil.toBean(map, response.getClass());//此方式有时候赋值成功
            BeanUtil.copyProperties(map, response);
        }
        return response;
    }
