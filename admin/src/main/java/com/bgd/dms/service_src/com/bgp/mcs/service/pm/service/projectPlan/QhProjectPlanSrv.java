 ...
 */
public class QhProjectPlanSrv extends BaseService {
	private ILog log;
	private RADJdbcDao radDao = (RADJdbcDao)BeanFactory.getBean("radJdbcDao");
	private ProjectMCSBean projectMCSBean;
	
	public QhProjectPlanSrv(){
		log = LogFactory.getLogger(QhProjectPlanSrv.class);
	...
	public ISrvMsg getProjectPlan(ISrvMsg reqDTO) throws Exception{
		...
		//String sql = "select  p.*,temp.plan_num from bgp_qh_project_plan p join ( select nvl(max(p1.base_plan_id),0)+1 as plan_num from bgp_qh_project_plan p1 where p1.bsflag = '0' and p1.project_info_no = '"+project_info_no+"') temp on 1=1 where p.project_info_no = '"+project_info_no+"' and p.bsflag = '0' and p.base_plan_id is null ";
		...
		//			 "from bgp_qh_project_plan p join (select nvl(max(p1.base_plan_id), 0) + 1 as plan_num from bgp_qh_project_plan p1 "+
...
//					 "and p.bsflag = '0' and p.base_plan_id is null ";
	...
	public ISrvMsg getEditProjectPlan(ISrvMsg reqDTO) throws Exception{
		...
		String sql = "select * from bgp_qh_project_plan p1 where p1.project_info_no = '"+project_info_no+"' and p1.bsflag = '0'";
		//String sql = "select p1.*,p2.object_id as PROJECT_PLAN_OBJECT_ID,p2.baseline_project_id as BASELINE_PLAN_OBJECT_ID from bgp_qh_project_plan p1 "+
	...
	public ISrvMsg getProjectPlanList(ISrvMsg reqDTO) throws Exception{
		...
		String sql = "select  * from bgp_qh_project_plan where 1=1 and bsflag = '0' and base_plan_id is null";
	...
	public ISrvMsg saveProjectPlan(ISrvMsg reqDTO) throws Exception{
		...
		if(object_id == null || object_id == ""){
			//如果已经存在初始计划,则不能再保存
			String checkSql = "select t.object_id from bgp_qh_project_plan t where t.bsflag = '0' and t.base_plan_id is null and t.project_info_no = '"+projectInfoNo+"'";
			...
			if(projectPlanMap != null){
				...
				BeanFactory.getPureJdbcDAO().saveOrUpdateEntity(map,"bgp_qh_project_plan");
	...
	public ISrvMsg getTeamInfo(ISrvMsg reqDTO) throws Exception{
		...
		String sql = "select  * from bgp_qh_project_plan where project_info_no = '"+project_info_no+"' and bsflag = '0' ";
		log.debug("查询sql:"+sql);
		page = radDao.queryRecordsBySQL(sql, page);
	...
	public ISrvMsg getAllPlanList(ISrvMsg reqDTO) throws Exception{
		...
		if(projectInfoNo != null && projectInfoNo != ""){
			//单项目
			String checkPlanSaved = "select t.object_id from bgp_qh_project_plan t where t.bsflag = '0' and t.base_plan_id is null and t.project_info_no = '"+projectInfoNo+"'";
			//判断该项目的项目计划是否被保存
			if(radDao.queryRecordBySQL(checkPlanSaved) != null){
					getAllPlanListSql = "select p.object_id,p.project_info_no,p.project_name,p.baseline_plan_object_id, " +
				                        ...
				                        " p.team_id,p.create_date,p.base_plan_id from bgp_qh_project_plan p left outer join common_busi_wf_middle t on p.object_id = t.business_id and t.bsflag = '0' where p.bsflag = '0' "+
			...
			//多项目
					getAllPlanListSql = "select p.object_id,p.project_info_no,p.project_name,p.baseline_plan_object_id, " +
									    " t.proc_status as proc_status_value,decode(t.proc_status, '1', '待审批', '3', '审批通过','4','审批不通过','未提交') proc_status, "+
						                " (case nvl(p.base_plan_id, 0) when '0' then '初始计划' else '变更计划'||p.base_plan_id end) as plan_name,"+
						                " p.team_id,p.create_date,p.base_plan_id from bgp_qh_project_plan p left outer join common_busi_wf_middle t on p.object_id = t.business_id and t.bsflag = '0' where p.bsflag = '0' order by p.modifi_date desc";			
		}
	...
	public ISrvMsg saveProjectBackUpPlan(ISrvMsg reqDTO) throws Exception{
		...
		if (object_id == null || "".equals(object_id)) {
			...
			if(map.get("plan_num_value") != null && map.get("plan_num_value") != ""){
				...
				String checkProjectExist = "select w.object_id from bgp_qh_project_plan w where w.bsflag = '0' and w.project_info_no = '"+project_info_no+"' and w.base_plan_id = '"+map.get("plan_num_value")+"'";
	...
	public ISrvMsg deleteProjectPlan(ISrvMsg reqDTO) throws Exception{
		...
		String delProjectPlanSql = "update bgp_qh_project_plan p set p.bsflag = '1' where p.object_id = '"+object_id+"'";
