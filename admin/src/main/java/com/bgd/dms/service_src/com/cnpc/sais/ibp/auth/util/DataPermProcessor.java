...
import com.cnpc.sais.ibp.auth.pojo.PAuthFilterItem;
import com.cnpc.sais.ibp.auth.pojo.PAuthFilterScope;
import com.cnpc.sais.ibp.auth.pojo.PAuthFuncFilter;
...
public class DataPermProcessor implements
		...
		for (int i = 0; i < params.size(); i++)
			try {
				...
				if (key != null && value != null)
					JavaBeanUtils.updatePojoField(AuthInitializor.param, key, value);
	 ...
	 */
	private List<String> processDataItem(Map<String, String> funcFilter, UserToken user) {
	 ...
	 * 数据范围权限设置
	 */
	private String processScope(String sql, Map<String, String> funcFilter, UserToken user) {
		...
		if ("1".equals(funcFilter.get("filter_type"))) {// 按项目过滤
			querySql = "SELECT fs.FILTER_TYPE,fs.USER_ID,fs.ROLE_ID,fs.DEPT_ID"
			...
			querySql = "SELECT fs.FILTER_TYPE,fs.USER_ID,fs.ROLE_ID,fs.DEPT_ID,o."
					+ AuthInitializor.param.getOrgCodeColumn()
					+ " as ORG_CODE FROM p_auth_filter_scope_dms   fs," + AuthInitializor.param.getOrgTable()
					+ " o WHERE FILTER_ID=':filterId' AND fs.ORG_ID=o."
					+ AuthInitializor.param.getOrgIdColumn();
		}
		Map<String, String> params = new HashMap<String, String>();
		params.put("filterId", funcFilter.get("entity_id"));
		List<Map<String, String>> scopes = jdbcDao.queryRecordsByParamSQL(querySql, params).getData();
		...
		filterColumnName = (fcn == null || fcn.length() == 0) ? filterColumnName : fcn;

		StringBuffer ftSb = new StringBuffer("SELECT distinct dataAuthView.* FROM ("
				+ sql);
		...
		if ("1".equals(funcFilter.get("filter_type"))) {// 按项目过滤
			...
			if (scopes.isEmpty()) {
				ftSb.append("," + rtable + " dataAuthOrg");
				ftSb.append(" WHERE dataAuthView." + filterColumnName
						+ "=dataAuthOrg." + rrc + " AND "+ rfc + "='" + user.getUserId()
						+ "' ");				
			}
		} else if ("2".equals(funcFilter.get("filter_type"))) {// 按用户过滤
			...
			ftSb.append(")dataAuthView," + AuthInitializor.param.getOrgTable()
							+ " dataAuthOrg");
			ftSb.append(" WHERE dataAuthView." + filterColumnName
					+ "=dataAuthOrg." + AuthInitializor.param.getOrgIdColumn() + " AND (");
			if (scopes == null || scopes.isEmpty()) {// 按用户组织机构权限过滤
				ftSb.append(" dataAuthOrg." + AuthInitializor.param.getOrgCodeColumn()
						+ " LIKE '" + user.getOrgCode() + "%' ");
			}
			else {// 按p_auth_filter_scope表的条件过滤
				for (int i = 0; i < scopes.size(); i++) {
					if (i > 0)
						ftSb.append(" OR ");
					ftSb.append(" dataAuthOrg." + AuthInitializor.param.getOrgCodeColumn()
									+ " LIKE '" + scopes.get(i).get("org_code")
									+ "%' ");
				}
			}
			ftSb.append(")");
...
//		System.out.println("**************************************");
	...
	public static void main(String[] args) throws Exception {
		String ret = replaceOrderStr("ORDER BY RWBI.MODIFY_TIME DESC");
