...
<%
	...
	String laborCategory = respMsg.getValue("laborCategory");
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
 ...
 <div id="list_table">
	<div id="table_box" >
	  <table width="99%" border="0" cellspacing="0" cellpadding="0"  id="queryRetTable">		
	     <tr>
	      ...
	      <td class="bt_info_odd" width="6%">劳动合同编号</td>
    ...
    <tr>
   	 ...
   	 <input type="hidden" name="fy<%=i%>laborId" id="fy<%=i%>laborId" value="<%=record.get("labor_id") != null ? record.get("labor_id"):""%>"/>		
   	 <input type="hidden" name="fy<%=i%>laborDeployId" id="fy<%=i%>laborDeployId" value="<%=record.get("labor_deploy_id")%>"/>		
   	 <input type="hidden" name="fy<%=i%>deployDetailId" id="fy<%=i%>deployDetailId" value="<%=record.get("deploy_detail_id")%>"/>	 
   	 <input type="hidden" name="fy<%=i%>projectInfoNo" id="fy<%=i%>projectInfoNo" value="<%=record.get("project_info_no")%>"/></td>			
	 <td class="<%=className%>even" ><%= record.get("employee_name") != null ? record.get("employee_name"):"" %>&nbsp;</td>	
	 <td class="<%=className%>odd" ><%= record.get("employee_id_code_no") != null ? record.get("employee_id_code_no"):"" %>&nbsp;</td>	
	 <td class="<%=className%>even" ><%= record.get("cont_num") != null ? record.get("cont_num"):"" %>&nbsp;</td>	 
	 <td class="<%=className%>odd"><%= record.get("apply_team_name") != null ? record.get("apply_team_name"):"" %><input type="hidden" name="fy<%=i%>team" id="fy<%=i%>team" value="<%=record.get("apply_team")%>"/>&nbsp;</td>
	   <td class="<%=className%>even" ><%= record.get("post_name") != null ? record.get("post_name"):"" %><input type="hidden" name="fy<%=i%>post" id="fy<%=i%>post" value="<%=record.get("post")%>"/>&nbsp;</td>
	 <td class="<%=className%>odd"><%= record.get("start_date") != null ? record.get("start_date"):"" %>&nbsp;</td>
	 <td class="<%=className%>even" >
	 <input type="text" name="fy<%=i%>actualStartDate" id="fy<%=i%>actualStartDate" value="<%=(record.get("actual_start_date") != null && !"".equals(record.get("actual_start_date"))) ? record.get("actual_start_date"):record.get("start_date") != null ? record.get("start_date"):""%>" readonly="readonly"/>
	...
	<table id="taskTable" width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:2px;">
    	<tr class="bt_info">
            ...
            <input type="hidden" id="lineNum" name="lineNumJ" value="0"/>
            </td>			
        </tr>
    </table>
    </div>
...
function save(){
	if(checkForm()){
		...
		form.action = "<%=contextPath%>/rm/em/toLaborAcceptEditNew.srq";
		form.submit();
		newClose();
	}
}
