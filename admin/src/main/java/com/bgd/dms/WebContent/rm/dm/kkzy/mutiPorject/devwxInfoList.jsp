...
.input_width2 {
 ...
 <body style="background:#cdddef" onload="searchDevData()">
	 	<!-- <fieldset style="margin-left: 2px">
			<legend>待处理故障设备</legend>
			<div id="show_bygz_list" style="line-height:20px;height: 20px;overflow-x:hidden;overflow-y:scroll"></div>
		</fieldset> -->
      	<div id="list_table">
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     ...
			     <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{bywx_id}' id='rdo_entity_id_{bywx_id}'  onclick='chooseOne(this);loadDataDetail();'/>" >选择</td>
					...
					<!-- <td class="bt_info_odd" exp="{maintenance_level}">维修级别</td> -->
...
<script type="text/javascript">
    ...
    function refreshData(v_self_num,s_wz_id){
		...
		var str = "";
		str+="";
		str+=" select    (case when l.bywx_type='0' then '更换' when l.bywx_type='1' then '维修' end) as bywx_type, (case when d.coding_name is null then '无' when d.coding_name is not null then d.coding_name end)  as zcj_type, l.bywx_id,l.bywx_date,l.work_hours,l.falut_desc,l.falut_case,l.maintenance_level,l.maintenance_desc, (case  when l.performance_desc = '0' then    '良好'    when l.performance_desc = '1' then '待修'  when l.performance_desc = '2' then '待查' end) as performance_desc,l.falut_reason, l.legacy,l.repair_unit,l.repair_men,l.bak, dui.self_num, dui.dev_acc_id,dui.dev_sign  from gms_device_zy_bywx l  left join gms_device_account dui on dui.dev_acc_id = l.dev_acc_id  left join comm_coding_sort_detail d on d.coding_code_id=l.zcj_type      where (l.bywx_type!='2' or l.bywx_type is null) and l.project_info_id is null and l.bsflag='0' and l.ORG_SUBJECTION_ID like 'C105006003%'";
	...
	//打开新增界面
	 function toAdd(){   
	 	popWindow("<%=contextPath%>/rm/dm/kkzy/mutiPorject/acceptLedgerwxAdd.jsp",'1024:680'); 
	 }
	 ...
	 function toModifyXgll(){
			...
			popWindow('<%=contextPath%>/rm/dm/kkzy/mutiPorject/acceptLedgerwxAdd.jsp?wx_ids='+wz_ids,'1350:680');
	...
	*/
	function yzjl(shuaId){
        ...
        var querySql = "select info.wz_sequence,mat.wz_id,info.actual_price,info.stock_num,i.wz_name,i.wz_prickie,mat.use_num,d.coding_name,mat.wz_sequences from gms_device_zy_wxbymat mat   left join gms_device_zy_bywx l on  l.usemat_id=mat.usemat_id ";
            ...
            querySql +=" and i.bsflag = '0' and c.bsflag = '0') on mat.wz_id=i.wz_id   where mat.bsflag='0' and l.bywx_id='"+shuaId+"'	 ";
		...
		function gzxx(shuaId){
			if (shuaId != null) {
				var baseData;
				baseData = jcdpCallService("DevInsSrv", "getGzInfoByBywxidForWx", "bywx_id="+shuaId);
	...
	/* function showBygzList(){
		var baseData = jcdpCallService("DevInsSrv", "getBygzList", "");
		if(baseData.bygzList!=undefined){
			var innerhtml ="";
			for (var i = 0; i < baseData.bygzList.length; i++) {
				//动态新增表格
				if(i+1==baseData.bygzList.length){
					innerhtml +="<a href='#' onclick='showGzAdd(\""+baseData.bygzList[i].dev_acc_id+"\")'><font color='red'>"+baseData.bygzList[i].self_num+"("+baseData.bygzList[i].counts+"条故障)</font></a>";
				}else{
					innerhtml +="<a href='#' onclick='showGzAdd(\""+baseData.bygzList[i].dev_acc_id+"\")'><font color='red'>"+baseData.bygzList[i].self_num+"("+baseData.bygzList[i].counts+"条故障)</font></a>\、";
				}
			}
			innerhtml +='';
			$("#show_bygz_list").append(innerhtml);
		}
	} */
	function showGzAdd(dev_acc_id){
		popWindow('<%=contextPath%>/rm/dm/kkzy/mutiPorject/acceptLedgerwxAdd.jsp?dev_acc_id='+dev_acc_id,'1350:680');
	}
</script>
</html>