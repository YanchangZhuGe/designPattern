...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <title>多项目-设备转移-设备分中心设备转移</title> 
 </head> 
 ...
 <body style="background:#cdddef" onload="refreshData()">
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			   <td class="ali_cdn_name">转移单名称</td>
			    <td class="ali_cdn_input">
			    	<input id="s_dev_mov_name" name="s_dev_mov_name" type="text" class="input_width" />
			    </td>
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_appdet_id_{dev_mov_id}' name='dev_mov_id' idinfo='{dev_mov_id}'>
					...
					<td class="bt_info_odd" exp="{in_project_name}">转入项目名称</td>
					<td class="bt_info_even" exp="{opertor}">经办人</td>
					<td class="bt_info_odd" exp="{apply_date}">申请时间</td>
					<td class="bt_info_even" exp="{move_status_desc}">状态</td>
			     </tr> 
			  </table>
			</div>
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				//先进行查询
				var str = "select dui.dev_coding,t.actual_out_time,dui.dev_name,dui.dev_model,dui.self_num,dui.license_num,dui.dev_sign,dui.asset_coding from gms_device_move_detail t left join gms_device_account_wells dui on t.dev_acc_id=dui.dev_acc_id  where t.dev_mov_id='"+currentid+"' ";				
...
<script type="text/javascript"><!--
	...
	function searchDevData(){
		var v_dev_mov_name = document.getElementById("s_dev_mov_name").value;
		...
		refreshData(v_dev_mov_name,v_in_project_name,v_move_status);
	}
	function getdate() { 
		var   now=new   Date() 
		y=now.getFullYear() 
    ...
    function refreshData(v_dev_mov_name,v_in_project_name,v_move_status){
		var str = "select t.dev_mov_id,t.dev_mov_no,t.dev_mov_name,outgp.project_name as out_progect_name,";
			...
			str += "where t.dev_type='wcdev' and t.bsflag='0' and t.org_subjection_id like '%"+suborgId+"%' ";		    
		    
		    if(v_dev_mov_name!=undefined && v_dev_mov_name!=''){
				str += "and t.dev_mov_name like '%"+v_dev_mov_name+"%' ";
			}
    ...
    function loadDataDetail(dev_mov_id){
    	...
    	var retObj;
		if(dev_mov_id!=null){
			retObj = jcdpCallService("DevCommInfoSrv", "getWellsDevMovBaseInfo", "devMovId="+dev_mov_id+"&move_flag=1");
		    ...
		    retObj = jcdpCallService("DevCommInfoSrv", "getWellsDevMovBaseInfo", "devMovId="+ids+"&move_flag=1");
		}
		//取消其他选中的 
		...
		$("#out_progect_name","#projectMap").val(retObj.deviceappMap.out_progect_name);
	...
	function toAddDevInfo(){
		...
		popWindow('<%=contextPath%>/rm/dm/wellsDevCenterPlan/wellsCenDevMoveApply.jsp?projectInfoNo=<%=projectInfoNo%>','950:680');
	...
	function toModifyMainPlanPage(){
		...
		if(unitRet.datas[0].move_status == '0' ){
			...
			popWindow('<%=contextPath%>/rm/dm/wellsDevCenterPlan/devWellsCenMoveModify.jsp?projectInfoNo=<%=projectInfoNo%>&devMovId='+devMovId,'950:680');
		}else{
			alert("本单据已转移，不能修改!");
			return;
		}
	...
	function toDelPlanPage(){
		...
		if(unitRet.datas[0].move_status == '0' || unitRet.datas[0].move_status == ''){
			if(confirm("是否执行删除操作?")){
				...
				var sql = "update gms_device_account_wells set fk_wells_transfer_id='',usage_org_id='',usage_sub_id='',transfer_state='0' where fk_wells_transfer_id ='"+devMovId+"' ";
				var path = cruConfig.contextPath+"/rad/asyncDelete.srq";
				var params = "deleteSql="+sql;
					params += "&ids=";
				var retObject = syncRequest('Post',path,params);
	...
	function toSumbitDevApp(){
		...
		if(unitRet.datas[0].move_status == null || unitRet.datas[0].move_status == '' || unitRet.datas[0].move_status == '0'){
			...
			var retObj = jcdpCallService("DevCommInfoSrv", "saveWellsCenDevMoveInfowfpa", "dev_mov_id="+devMovId+"&in_project_no="+unitRet.datas[0].in_project_info_id);
