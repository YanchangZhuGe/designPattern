...
<%
    //此处为技术平台中多项目中不需要选择项目的项目使用，同时要在文档模块显示，
    //即需要parentFileId参数，需要选择项目的，暂时不在文档模块显示
...
<script type="text/javascript">
	...
	var orgSubId='<%=orgSubjectionId%>';
	
	function refreshData(){
        var str="";
        if(orgSubId !="C105005001"){
        	str="  and t.org_subjection_id like '<%=orgSubjectionId%>%'  ";
        }
		cruConfig.queryStr = " select t.* from ( select t.file_id,t.file_name,t.ucm_id,to_char(t.upload_date,'yyyy-MM-dd') upload_date,e.employee_name from bgp_doc_gms_file t left join comm_human_employee e on t.creator_id=e.employee_id and e.bsflag='0'  where t.bsflag='0' and t.doc_type='<%=codingCodeId%>'  "+str+"  order by t.upload_date desc ) t where 1=1 ";
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/noProjectDoc/add_tdDocModify.jsp?codingCodeId='+codingCodeId+'&parentFileId='+parentFileId,'800:600');
    }
	...
	function toEdit() {
		...
		popWindow('<%=contextPath%>/td/noProjectDoc/add_tdDocModify.jsp?id='+ids,'800:600');
	}
	function toDelete(){
