...
var sckfType = new Array(
['10301','高层互访'],['10302','市场活动'],['10303','投议标信息'],['10304','专题会议'],['10305','客户沟通'],['10306','行业新闻'],['10307','政经动态']
);

var sckfNat = new Array(
['null',''],['10803001','国内'],['10803002','国际']
);
...
var jcdp_codes = new Array(
  	...
  	function toAdd(){
  		editUrl = "/market/marketSckfPage/addMarketSckfPage.jsp?action=0";
  	...
  	function toEdit(){
	    ...
	    var infomation_id = tempa[0];
	   	var infomation_name = tempa[1];
	    	editUrl = "/market/editSckf.srq?infomation_id="+infomation_id; 
	    	window.location=cruConfig.contextPath+editUrl; 
	}	
	function viewLink(infomation_id){
	    	editUrl = "/market/viewSckf.srq?infomation_id="+infomation_id;
	    	window.location=cruConfig.contextPath+editUrl+"&backUrl="+cruConfig.currentPageUrl;
		}
...
function page_init(){
	...
	cdt_init();
	cruConfig.queryStr = "select infomation_id, infomation_name,two_type_id,three_type_id,release_date,creator, case when times is null then '0' else times end as times  from (select r.*, t.father_code from bgp_infomation_release_info r, bgp_infomation_type_info t where r.two_type_id = t.code) where father_code = '103' and bsflag = '0' order by release_date desc, modify_date desc";
	cruConfig.currentPageUrl = "/market/marketSckfPage/MarketSckfPageList.lpmd";
	queryData(1);
}
...
function toDelete(){
	...
	var title = "在后台发布中市场开发里删除了一条标题为：“"+infomation_name+"”的信息"
	var operationPlace = "市场开发";
...
fields[1] = ['two_type_id','类别选择','TEXT',,,'SEL_OPs',sckfType];
...
<table border="0"  cellspacing="0"  cellpadding="0"  class="form_info"  width="100%" id="queryRetTable">
	...
	<tr class="bt_info">
		...
		<td class="tableHeader" 	 exp="{two_type_id}" func="getOpValue,sckfType">类别</td>
		<td class="tableHeader" 	 exp="{three_type_id}" func="getOpValue,sckfNat">新闻范围</td>
