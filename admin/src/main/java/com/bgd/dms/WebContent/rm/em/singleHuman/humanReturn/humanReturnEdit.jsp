 ...
 <div id="list_table">
	<div id="table_box" >
	  <table width="99%" border="0" cellspacing="0" cellpadding="0"  id="queryRetTable">		
	     <tr>
	      ...
	      <td class="bt_info_odd" width="9%">实际进入项目时间</td>
	      <td class="bt_info_even" width="9%">实际离开项目时间</td>
	      <td class="bt_info_odd" width="9%">人员评价<input type="hidden" id="equipmentSize" name="equipmentSize" value="<%=length%>" />
    ...
    <tr>
   	 ...
   	 <input type="hidden" name="fy<%=i%>szType" id="fy<%=i%>szType" value="<%=record.get("sz_type")%>" />	
   	  	 <input type="hidden" name="fy<%=i%>empType" id="fy<%=i%>empType" value="<%=record.get("emp_type")%>" />	
	 ...
	 <td class="<%=className%>odd" ><code:codeSelect name='<%=("fy"+String.valueOf(i)+"projectEvaluate")%>' option="employeeEvaluateLevel" addAll="true" selectedValue='<%=record.get("project_evaluate") != null ? (String)record.get("project_evaluate"):""%>' cssClass="select_width"/>&nbsp;</td>
...
function save(){
	if(checkForm()){
		...
		form.action = "<%=contextPath%>/rm/em/saveHumanReturn.srq";
		form.submit();
		newClose();
	}
