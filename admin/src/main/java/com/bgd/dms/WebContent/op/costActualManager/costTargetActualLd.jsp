      	...
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <td>&nbsp;</td>
				...
				<%if(proc_status){ %>
			     <auth:ListButton functionId="OP_ACTUAL_LO_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			     <auth:ListButton functionId="OP_ACTUAL_LO_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			     <auth:ListButton functionId="OP_ACTUAL_LO_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			     <auth:ListButton functionId="OP_ACTUAL_LO_EDIT" css="dr" event="onclick='toImport()'" title="JCDP_btn_import"></auth:ListButton>
			     <%} %>
			  </tr>
			</table>
			</td>
			...
			<div id="table_box">
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			            ...
			            <td  class="bt_info_even">实际进队时间</td>
			            <td class="bt_info_odd">实际离开时间</td>
			            <td  class="bt_info_even">使用时间</td>
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getDeviceActualDepreOtherInfo&projectInfoNo=<%=projectInfoNo%>";
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualLdEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualLdEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	ids = ids.replace(/\,/g,"','");
	var sql = "update bgp_op_tartet_device_depre t set t.if_actual_change='1' where t.target_depre_id in('"+ids+"')";
...
function toImport(){
	 var retObj = jcdpCallService("OPCostSrv", "importDeviceLdAtualInfo", "projectInfoNo=<%=projectInfoNo%>");
	 alert("导入成功");
	 refreshData();
}
function getSum(){
	var project_info_no = '<%=projectInfoNo%>';
	var querySql = "select sum(nvl(to_char(td.device_count*td.asset_value*0.97/td.depreciation_value/8*1/30*(nvl(td.actual_end_date, to_date(to_char(sysdate, 'yyyy-MM-dd'),'yyyy-MM-dd'))-td.actual_start_date+1),'9999999999999999.00'),0)) sum_value"+
	...
	" where td.bsflag='0' and  td.project_info_no = '"+project_info_no+"'  and td.record_type='1'  and td.if_delete_change is null and td.if_actual_change is null";
