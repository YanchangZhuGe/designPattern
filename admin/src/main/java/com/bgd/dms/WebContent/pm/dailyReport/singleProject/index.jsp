...
<%
	...
	if("5000100004000000006".equals(projectType)){
		//深海项目
		response.sendRedirect(contextPath+"/pm/dailyReport/singleProject/sh/index.jsp");
	}
	...
	if( user.getProjectType().equals("5000100004000000008")){
		...
		response.sendRedirect(contextPath+"/pm/dailyReport/singleProject/ws/wsIndex.jsp");
	...
	if(org_subjection_id.startsWith("C105007")){
		response.sendRedirect(contextPath+"/pm/dailyReport/singleProject/tqh/index.jsp");//大港项目
	}
...
<form name="form1" action="<%=contextPath%>/pm/dailyReport/saveOrUpdateDailyReport.srq" method="post">
	...
	<tr class="even" id="td1" style="display: none;">
		...
		<td class="inquire_form6">
			...
			<!-- <option value="1">试验</option>
				...
				<option value="4">自然停工</option>   shishengjie -->
				<option value="1">气候</option>
				<option value="2">施工协调</option>
				<option value="3">油公司因素</option>
...
function getProjectTeam(){
	var retObj = jcdpCallService("DailyReportSrv", "getProjectTeamName", "projectInfoNo=<%=projectInfoNo %>");
