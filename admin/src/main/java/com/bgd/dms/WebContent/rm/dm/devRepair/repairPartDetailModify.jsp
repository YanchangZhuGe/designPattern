...
<script type="text/javascript"> 
	...
	function refreshData(){
		var sql = "select t.* from GMS_DEVICE_PART_ITEM t ";
		sql += " where  t.id = '<%=id%>'";
		...
		$("#tree_id").val(repairObj[0].tree_id);
		$("#orderno").val(repairObj[0].orderno);
		$("#supplyname").val(repairObj[0].supplyname);
		$("#factoryname").val(repairObj[0].factoryname);
		$("#currery").val(repairObj[0].currery);
		$("#unit").val(repairObj[0].unit);
		$("#perprice").val(repairObj[0].perprice);
		$("#brand").val(repairObj[0].brand);
		$("#useflag").val(repairObj[0].useflag);
		$("#item_name").val(repairObj[0].item_name);
		$("#part_model").val(repairObj[0].part_model);
		}
	/*****
