...
<%
	...
	String taskName = new String(request.getParameter("taskName").getBytes("iso8859-1"),"utf-8");
    ...
    <div id="new_table_box_bg" style="width:95%">
      ...
      <table border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td class="inquire_form4" name="project_name" id="project_name" >
          </td>
          <td class="inquire_item4" >
          	申请单名称:
        ...
        <tr>
          <td class="inquire_item4">工序:</td>
          	<input name="teamid" id="teamid" class="input_width" type="hidden" value="<%=taskId%>"/>
          	<input name="teamtype" id="teamtype" type="hidden" value="<%=teamtype%>" />
          <td class="inquire_form4" name="jobname" id="jobname" >
          </td>
         </tr>
      </table>
      </fieldset>
      <fieldset style="margin-left:2px">
      <legend>设备计划信息</legend>
		...
		<div id="oprdiv0" name="oprdiv" style="float:left;width:70%;overflow:auto;">
	      	<table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
			  	<tr align="right">
      				...
      				<a href="javascript:downloadModel('dev_model','自有设备配置计划模板')">自有设备配置计划模板</a>
			  		</td>
		  ...
		  <div id="resultdiv1" name="resultdiv" style="float:left;height:210px;overflow:auto;display:none">
			  <table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
					...
					<td class="bt_info_even" width="13%">开始时间</td>
					<td class="bt_info_odd" width="13%">结束时间</td>
					<td class="bt_info_even" width="11%">备注</td>
				</tr>
		  ...
		  <div id="oprdiv2" name="oprdiv" style="float:left;width:70%;overflow:auto;display:none">
	      	<table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
			  	<tr align="right">
			  		<td class="ali_cdn_name" ></td>
			  		<td class="ali_cdn_input" ></td>
			  		<td class="ali_cdn_name" ><a href="javascript:downloadModel('coll_model','采集设备配置计划模板')">采集设备配置计划模板</a></td>
			  		<auth:ListButton functionId="" css="dr" event="onclick='exportCollData()'" title="导入excel"></auth:ListButton>
			    	<auth:ListButton functionId="" css="zj" event="onclick='addDetectorRows()'" title="添加检波器设备"></auth:ListButton>
			    	<auth:ListButton functionId="" css="sc" event="onclick='delDetectorRows()'" title="删除检波器设备"></auth:ListButton>
				</tr>
			  </table>
		  </div>
		  <div id="resultdiv2" name="resultdiv" style="float:left;height:210px;overflow:auto;display:none">
			  <table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
		       		<td class="bt_info_even"><input type="checkbox" name="detectordetinfo" id="detectordetinfo" /></td>
					<td class="bt_info_odd" width="16%">班组</td>
					<td class="bt_info_even" width="17%">设备名称</td>
					<td class="bt_info_odd" width="16%">规格型号</td>
					<td class="bt_info_even" width="7%">计量单位</td>
...
$().ready(function(){
	...
	$("#detectordetinfo").change(function(){
		var checkvalue = this.checked;
		$("input[type='checkbox'][name^='detectoridinfo']").attr('checked',checkvalue);
	});
	...
	//根据项目编码判断项目类型
	var projectTypeSql  = "select t.project_type,t.design_start_date,t.design_end_date,t.acquire_start_time,t.acquire_end_time from gp_task_project t where t.project_info_no='<%=projectInfoNo%>'";
	var ptObj = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+projectTypeSql);
	ptObjd = ptObj.datas;
	var startDateValue ;
	var endDateValue ;
	var projectType = ptObjd[0].project_type; 
	if(projectType=="5000100004000000009"){
		//综合物化探
		if(ptObjd[0].acquire_start_time!=null&&ptObjd[0].acquire_start_time!=''){
			startDateValue = ptObjd[0].acquire_start_time;
		}else{
			startDateValue = "<%=nowdate%>";
		}
		if(ptObjd[0].acquire_end_time!=null&&ptObjd[0].acquire_end_time!=''){
			endDateValue = ptObjd[0].acquire_end_time;
		}else{
			endDateValue = "<%=nowdate%>";
		}
	}else{
		//其它类型项目
		if(ptObjd[0].design_start_date!=null&&ptObjd[0].design_start_date!=''){
			startDateValue = ptObjd[0].design_start_date;
		}else{
			startDateValue = "<%=nowdate%>";
		}
		if(ptObjd[0].design_end_date!=null&&ptObjd[0].design_end_date!=''){
			endDateValue = ptObjd[0].design_end_date;
		}else{
			endDateValue = "<%=nowdate%>";
		}
	}
	...
	function addRows(value){
		...
		if(value!="" && value !=undefined){
			...
			for(var i=0;i<trData.length;i++){
				...
				innerhtml += "<select name='team"+i+"' id='team"+i+"' /></select></td>";				
			...
			innerhtml += "<td width='13%'><input value='"+startDateValue+"' name='startdate"+tr_id+"' id='startdate"+tr_id+"' style='line-height:15px' size='10' type='text'/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton2"+tr_id+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(startdate"+tr_id+",tributton2"+tr_id+");'/></td>";
			innerhtml += "<td width='13%'><input value='"+endDateValue+"' name='enddate"+tr_id+"' id='enddate"+tr_id+"' style='line-height:15px' size='10' type='text'/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton3"+tr_id+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(enddate"+tr_id+",tributton3"+tr_id+");'/></td>";
	...
	function addCollRows(value){
		if(value!="" && value !=undefined){
			...
			for(var i=0;i<trData.length;i++){
				...
				var colltypeObj;
				var colltypeSql = "select t.coding_code_id as value,t.coding_name as label from comm_coding_sort_detail t ";
	...
	function addDetectorRows(value){
		...
		if(value!="" && value !=undefined){
			...
			for(var i=0;i<trData.length;i++){
				...
				teamName = tdData[0];
				unit = tdData[3];
				innerhtml += "<tr id='tr"+i+"' name='tr"+i+"' detectorseq='"+i+"'>";
				innerhtml += "<td><input type='checkbox' name='detectoridinfo' id='"+i+"'/></td><td width='16%'>";
				innerhtml += "<select name='detectorteam"+i+"' id='detectorteam"+i+"' /></select></td>";				
				innerhtml += "<td width='17%'><input name='detectordevicename"+i+"' id='detectordevicename"+i+"' style='line-height:15px' value='"+tdData[1]+"' size='15' type='text' />";
				innerhtml += "<td width='16%'><input name='detectordevicetype"+i+"' id='detectordevicetype"+i+"' value='"+tdData[2]+"' size='16'  type='text' /></td>";
				innerhtml += "<td width='7%'><select name='detectorunit"+i+"' id='detectorunit"+i+"' /></select></td>";
				innerhtml += "<td width='7%'><input name='detectorneednum"+i+"' id='detectorneednum"+i+"' value='"+tdData[4]+"' size='4' type='text' onkeyup='checkAppNum(this,'')'/></td>";
				innerhtml += "<td width='13%'><input name='detectorstartdate"+i+"' id='detectorstartdate"+i+"' style='line-height:15px' size='10' type='text' value='"+tdData[5]+"'/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton2"+i+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(startdate"+i+",tributton2"+i+");'/></td>";
				innerhtml += "<td width='13%'><input name='detectorenddate"+i+"' id='detectorenddate"+i+"' style='line-height:15px' size='10' type='text' value='"+tdData[6]+"'/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton3"+i+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(enddate"+i+",tributton3"+i+");'/></td>";
				innerhtml += "<td width='11%'><input name='detectorpurpose"+i+"' value='"+tdData[7]+"' size='10' type='text'/></td>";				
				innerhtml += "</tr>";
				...
				$("#detectorteam"+i).append(teamoptionhtml);
				//查询公共代码，并且回填到界面的单位中
				var retObj;
				var unitSql = "select sd.coding_code_id,coding_name from comm_coding_sort_detail sd ";
					unitSql += "where coding_sort_id ='5110000038' order by coding_code";
				...
				for(var index=0;index<retObj.length;index++){
					...
					$("#detectorunit"+i).append(optionhtml);
			}
			$("#processtable2>tr:odd>td:odd").addClass("odd_odd");
			$("#processtable2>tr:odd>td:even").addClass("odd_even");
			$("#processtable2>tr:even>td:odd").addClass("even_odd");
			$("#processtable2>tr:even>td:even").addClass("even_even");
		}else{
			tr_id = $("#processtable2>tr:last").attr("id");
			if(tr_id != undefined){
				tr_id = parseInt(tr_id.substr(2,1),10);
			}
			if(tr_id == undefined){
			...
			//动态新增表格
			var innerhtml = "<tr id='tr"+tr_id+"' name='tr"+tr_id+"' detectorseq='"+tr_id+"'>";
			innerhtml += "<td><input type='checkbox' name='detectoridinfo' id='"+tr_id+"'/></td>";
			innerhtml += "<td width='16%'><select name='detectorteam"+tr_id+"' id='detectorteam"+tr_id+"' /></select></td>";			
			innerhtml += "<td width='17%'><input name='detectordevicename"+tr_id+"' id='detectordevicename"+tr_id+"' style='line-height:15px' value='' size='15' type='text' />";
			innerhtml += "</td>";
			innerhtml += "<td width='16%'><input name='detectordevicetype"+tr_id+"' id='detectordevicetype"+tr_id+"' value='' size='16'  type='text' /></td>";
			innerhtml += "<td width='7%'><select name='detectorunit"+tr_id+"' id='detectorunit"+tr_id+"' /></select></td>";
			innerhtml += "<td width='7%'><input name='detectorneednum"+tr_id+"' id='detectorneednum"+tr_id+"' value='' size='4' type='text' onkeyup='checkAppNum(this,\"approvenum"+tr_id+"\")'/>";
			//综合物化探 项目资源配置中 审批数量
			innerhtml += "<input type='hidden' id='detectorapprovenum"+tr_id+"' name='detectorapprovenum"+tr_id+"'></td>";			
			innerhtml += "<td width='13%'><input value='"+startDateValue+"' name='detectorstartdate"+tr_id+"' id='detectorstartdate"+tr_id+"' style='line-height:15px' size='10' type='text'/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton2"+tr_id+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(startdate"+tr_id+",tributton2"+tr_id+");'/></td>";
			innerhtml += "<td width='13%'><input value='"+endDateValue+"' name='detectorenddate"+tr_id+"' id='detectorenddate"+tr_id+"' style='line-height:15px' size='10' type='text'/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton3"+tr_id+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(enddate"+tr_id+",tributton3"+tr_id+");'/></td>";
			innerhtml += "<td width='11%'><input name='detectorpurpose"+tr_id+"' value='' size='10' type='text'/></td>";
			innerhtml += "</tr>";
			...
			$("#detectorteam"+tr_id).append(teamoptionhtml);
			//查询公共代码，并且回填到界面的单位中
			var retObj;
			var unitSql = "select sd.coding_code_id,coding_name from comm_coding_sort_detail sd ";
				unitSql += "where coding_sort_id ='5110000038' order by coding_code";
			...
			$("#detectorunit"+tr_id).append(optionhtml);
	...
	function delDetectorRows(){
		$("input[name='detectoridinfo']").each(function(){
			if(this.checked == true){
				$('#tr'+this.id,"#processtable2").remove();
			}
		});
	...
	function submitInfo(){
		...
		var detectorcount = 0;
		var detectorline_infos = '';
		$("tr","#processtable2").each(function(){
			if(this.detectorseq!=undefined){
				if(detectorcount == 0){
					detectorline_infos = this.detectorseq;
				}else{
					detectorline_infos = detectorline_infos+"~"+this.detectorseq;
				}
				detectorcount++;
			}
		});
		...
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveDevAllAppDetailInfo.srq?detectorcount="+detectorcount+"&detectorline_infos="+detectorline_infos+"&count="+count+"&line_infos="+line_infos+"&collcount="+collcount+"&collline_infos="+collline_infos+"&resourceFlag=<%=resourceFlag%>";
	...
	function refreshData(){
		...
		if('<%=deviceallappid%>'!=null&&'<%=deviceallappid%>'!='null'){
			var proSql = "select project.project_name,devapp.device_allapp_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.remark, ";
		...
		}else if('<%=projectInfoNo%>'!=null&&'<%=projectInfoNo%>'!='null'){
			var proSql = "select project.project_name,devapp.device_allapp_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.remark, ";
				proSql += "to_char(sysdate,'yyyy-mm-dd') as appdate "; 
				proSql += "from gms_device_allapp devapp left join gp_task_project project on devapp.project_info_no=project.project_info_no "; 
				proSql += "where devapp.bsflag='0' and devapp.project_info_no='<%=projectInfoNo%>' ";
			var proqueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+proSql+'&pageSize=1000');
			retObj = proqueryRet.datas;
		}
		if(retObj.length>=1){
			...
			$("#device_allapp_id").val(retObj[0].device_allapp_id);
