...
<%
	...
	String normal_no = "0";
	String wrong_no = "0";
	String sql = "select sum(g.use_no) use_no,sum(g.install_no) install_no,sum(g.normal_no) normal_no,sum(g.wrong_no) wrong_no from bgp_hse_common c join bgp_hse_org ho on c.org_id = ho.org_sub_id  join bgp_hse_week_gps_out g on c.hse_common_id = g.hse_common_id where  c.bsflag = '0' and c.subflag = '1' and ho.father_org_sub_id = 'C105' and c.week_end_date = to_date('"+week_end_date+"','yyyy-MM-dd')";
	Map map = BeanFactory.getQueryJdbcDAO().queryRecordBySQL(sql);
	if(map!=null){
		use_no = (String)map.get("useNo") == "" ? "0" : (String)map.get("useNo");
		install_no = (String)map.get("installNo") == "" ? "0" : (String)map.get("installNo");
