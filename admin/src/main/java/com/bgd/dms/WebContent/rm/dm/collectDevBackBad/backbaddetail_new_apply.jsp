      ...
      <table border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        ...
        <tr>
          ...
          <td class="inquire_item4" colspan="3">
          	<input name="backapp_name" id="backapp_name" class="input_width" style="width:92%" type="text" value="" />
          </td>
        </tr>
        <tr>
		  ...
		  <div style="height:100px;overflow:auto">
			  <table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
					...
					<td class="bt_info_odd" width="8%">返还数量</td>
...
<script type="text/javascript">
	...
	function addLine(retObj){
		...
		for(var index=rows;index<rows+retObj.length;index++){
			...
			innerhtml += "<td><input type='checkbox' name='idinfo' seqinfo='"+index+"' id='"+retObj[index-rows].dev_acc_id+"'/></td>";
			...
			innerhtml += "<td><input name='enddate"+index+"' id='enddate"+index+"' devaccid='"+retObj[index-rows].dev_acc_id+"' style='line-height:15px' value='"+actual_out+"' size='10' type='text'/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton3"+index+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(enddate"+index+",tributton3"+index+");'/></td>";
	...
	function submitInfo(){
		var backapp_name = document.getElementById("backapp_name").value;
		if(backapp_name=="" || backapp_name.length>50){
			alert("申请单不能为空或申请单名称太长！");
			return;
			}
		...
		var sql = "update gms_device_collbackapp set back_org_id='"+back_org_id+"',backdate=to_date('"+backdate+"','yyyy-MM-dd'),"+
			...
			" org_subjection_id='<%=org_subjection_id%>',state='0',backapp_name='"+backappname+"',receive_org_id='"+receive_org_id+"' "+
		...
		if(devicebackappid==null || devicebackappid==''){
			...
			"'0',sysdate,'<%=empId%>',sysdate,'<%=empId%>','<%=org_id%>','<%=org_subjection_id%>','0',(lower(sys_guid())),'"+backappname+"','"+backdevtype+"','','2','"+receive_org_id+"');";
		}
...
// 		if(backdevtype!=null && backdevtype!='S0000'){
...
// 			" bsflag,create_date,creator_id,modifi_date,updator_id,org_id,org_subjection_id,state,search_backapp_id,backapp_name,backdevtype,backmix_username,receive_org_id) "+
...
<%-- 			"'0',sysdate,'<%=empId%>',sysdate,'<%=empId%>','<%=org_id%>','<%=org_subjection_id%>','0',(lower(sys_guid())),'"+backappname+"','"+backdevtype+"','"+backmix_username+"','"+receive_org_id+"');"; --%>
// 		}
// 		var retObj = jcdpCallService("QualityItemsSrv", "saveQuality", "sql="+sql);
// 		if(retObj==null || !retObj.returnCode=='0'){
// 			alert('保存失败!');
...
// 			if(count == 0){
// 				idinfos = this.id;
// 				enddateinfo = $("input[id^='enddate2'][devaccid='"+this.id+"']").val();
// 			}else{
// 				idinfos = idinfos+"~"+this.id;
// 				enddateinfo = enddateinfo+"~"+$("input[id^='enddate2'][devaccid='"+this.id+"']").val();
// 			}
// 			count++;
// 		});
// 		if(count == 0){
...
// 		document.getElementById("form1").submit();
	...
	function refreshData(){
		...
		if(devicebackappid!=null && devicebackappid!=''){
			...
			proSql += "backapp.remark,pro.project_info_no,pro.project_name,backapp.receive_org_id,i.org_abbreviation receive_org_name,";
			...
			proSql += "left join comm_org_information i on i.org_id=backapp.receive_org_id ";
	...
	function toAddAddedDetailInfos(){
		...
		var conditionStr = "";
			...
			conditionStr += " and (substr(account.dev_type,1,9)!='<%=DevConstants.MIXTYPE_JIANBOQI%>' or 'S'||substr(account.dev_type,0,8)!='<%=DevConstants.MIXTYPE_JIANBOQI%>')) ";
	...
	function addLine2(retObj){
		...
		for(var index=rows;index<rows+retObj.length;index++){
			...
			innerhtml += "<td width='4%'><input type='checkbox' name='idinfo2' id='"+retObj[index-rows].dev_acc_id+"'/></td>";
			...
			innerhtml += "<td width='12%'><input name='enddate2"+index+"' id='enddate2"+index+"' devaccid='"+retObj[index-rows].dev_acc_id+"' style='line-height:15px' value='"+retObj[index-rows].planning_out_time+"' size='10' type='text'/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton33"+index+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(enddate2"+index+",tributton33"+index+");'/></td>";
