...
<table border="0"  width="100%"  cellspacing="0" cellpadding="0" class="tab_info"  id="equipmentTableInfo">
	<tr>  
		...
		<TD  class="bt_info_odd"  width="20%"><font color=black>基层单位</font></TD>
		<TD class="bt_info_even"  width="15%"><font color=black>下属单位</font></TD>
		<TD  class="bt_info_odd"   width="15%"><font color=black>隐患名称</font></TD>
		<TD class="bt_info_even"  width="6%" >上报日期
		<input type="hidden" id="lineNum" value="0"/>
		</TD>	
		<TD  class="bt_info_odd"   width="10%"><font color=black>整改状态</font></TD>
		<TD class="bt_info_even"  width="10%"><font color=black>风险级别</font></TD>
	</tr>		 
</table>	</div>
...
cruConfig.contextPath='<%=contextPath %>';
 ...
 if(hidden_no !=null  || hidden_no !="'null'"){		 
		...
		hidden_no=id;				
		 ...
		 querySql1 = " select tr.hidden_no, dil.cdetail_no,  ions.CORRECTIVE_NO,  ion.org_name,  oi1.org_abbreviation as second_org_name, oi2.org_abbreviation as third_org_name,   tr.hidden_name,  tr.report_date,    decode(ions.rectification_state,'1','已整改','2','未整改','3','正在整改')rectification_state,   dil.crisk_levels   from BGP_CORRECTINVE_DETAIL dil   left join BGP_CORRECTINVE_INFORMATION ions     on ions.CORRECTIVE_NO = dil.CORRECTIVE_NO    and dil.bsflag = '0'   join BGP_HIDDEN_INFORMATION tr     on dil.hidden_no = tr.hidden_no    and tr.bsflag = '0'   join comm_org_subjection os1     on tr.second_org = os1.org_subjection_id    and os1.bsflag = '0'   join comm_org_information oi1     on oi1.org_id = os1.org_id    and oi1.bsflag = '0'   join comm_org_subjection os2     on tr.third_org = os2.org_subjection_id    and os2.bsflag = '0'   join comm_org_information oi2     on oi2.org_id = os2.org_id    and oi2.bsflag = '0'   join comm_org_information ion     on ion.org_id = os1.org_id  where tr.bsflag = '0'    and ions.bsflag = '0'  and tr.hidden_no in(" +hidden_no+ ")";
			  ...
			  datas1 = queryRet1.datas;	
					...
					if(datas1 != null && datas1 != ''){							 
							...
							for(var i = 0; i<datas1.length; i++){	 	
								...
								tr.insertCell().innerHTML = '<input type="checkbox"    id="hidden_no'+ rowNum + '"   name="hidden_no" value="'+datas1[i].hidden_no+','+datas1[i].hidden_name+','+datas1[i].report_date+','+datas1[i].rectification_state+','+datas1[i].crisk_levels+'" />';
								...
								tr.insertCell().innerHTML =	datas1[i].rectification_state;
								tr.insertCell().innerHTML =	datas1[i].crisk_levels;									
		...
		if(reward_no !="null"){
			for(var i=0;i<certificate.length;i++){
				if(certificate[i].checked==true){
					...
					var rreport_dates= params.split(',')[2];
					var rrisk_levelss = params.split(',')[3];
					var reward_states = params.split(',')[4]; 
					 window.parent.opener.top.frames('list').addLine3('',reward_no,hidden_nos,hidden_names,rreport_dates,rrisk_levelss,reward_states,'','','','','');
			...
			for(var i=0;i<certificate.length;i++){
				if(certificate[i].checked==true){
					...
					var rrisk_levelss = params.split(',')[3];
					var reward_states = params.split(',')[4]; 
					window.top.opener.addLine1('','',hidden_nos,hidden_names,rreport_dates,rrisk_levelss,reward_states,'','','','','');
