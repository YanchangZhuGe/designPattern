...
function getFusionChart(){
	//设置默认时间
      ...
      var self_num = document.getElementById("s_dev_coding").value;
      var project_info_id = document.getElementById("project_info_id").value;
	var retObj1 = jcdpCallServiceCache("EarthquakeTeamStatistics","getZyAllProjectStatistics","project_info_id="+project_info_id+"&start_date="+start_date+"&end_date="+end_date+"&self_num="+self_num+"&change=<%=change%>");
	...
	//整合进来的
	var retObj2 = jcdpCallServiceCache("EarthquakeTeamStatistics","getZyMStatistics","projectInfoNo="+project_info_id+"&start_date="+begin_date+"&end_date="+end_date);
...
function simpleSearch(){
	...
	//整合进来的
	var retObj2 = jcdpCallServiceCache("EarthquakeTeamStatistics","getZyMStatistics","change=<%=change%>"+"&start_date="+start_date+"&end_date="+end_date+"&dev_coding="+self_num+"&project_info_id="+project_info_id);
	var dataXml2 = retObj2.dataXML;
	var myChart2 = new FusionCharts("${applicationScope.fusionChartsURL}/Charts/MSColumn3D.swf", "myChartId1", "100%", "250", "0", "0" );    
	myChart2.setXMLData(dataXml2);
	myChart2.render("chartContainer2");
...
function exportDataDoc(exportFlag){
    ...
    var project_info_id = document.getElementById("project_info_id").value;
    submitStr = "exportFlag="+exportFlag+"&startDate="+startDate+"&endDate="+endDate+"&dev_coding="+dev_coding+"&project_info_id="+project_info_id;
...
function popSingleProjectBjWxByCount(cb){
	var str=cb.split("~");
	var project_info_id=str[0];
	var self_num=str[1];
	var start_date=str[2];
	var end_date=str[3];
	popWindow('<%=contextPath%>/rm/dm/kkzy/singleProjectBjWxByCount.jsp?self_num='
						+ self_num + "&project_info_id=" + project_info_id
...
function exportDataDoc2(exportFlag){
	//调用导出方法
	var path = cruConfig.contextPath+"/rm/dm/common/DmZhfxToExcel.srq";
	var submitStr="";
	var startDate=$("#start_date").val();
	...
	function popZySingleWzXhM(cb){
		
		//projectCode+"~"+dev_coding+"~"+startDate+"~"+endDate
		var str=cb.split("~");
		var project_info_id=str[0];
		var self_num=str[1];
		var start_date=str[2];
		var end_date=str[3];
		popWindow('<%=contextPath %>/rm/dm/kkzy/popZySingleWzXhM.jsp?self_num='+self_num+"&project_info_id="+project_info_id+"&start_date="+start_date+"&end_date="+end_date,'800:600');
									...
									<div class="tongyong_box_title">
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												...
												<td class="ali_cdn_input" align="right"><input
													id="start_date" name="start_date" class="input_width"
													type="text" readonly /></td>
												<td style="width: 10px;" align="left"><img
													src='<%=contextPath%>/images/calendar.gif'
													id='tributton_start_date' width='16' height='16'
													style='cursor: hand;'
													onmouseover='calDateSelector(start_date,tributton_start_date);' />
												</td>
												<td class="ali_cdn_name">结束时间:</td>
												<td class="ali_cdn_input" align="right"><input
													id="end_date" name="end_date" class="input_width"
													type="text" readonly /></td>
												<td style="width: 10px;" align="left"><img
													src='<%=contextPath%>/images/calendar.gif'
													id='tributton_end_date' width='16' height='16'
													style='cursor: hand;'
													onmouseover='calDateSelector(end_date,tributton_end_date);' />
												</td>
												<td class="ali_cdn_name">震源编号:</td>
												<td>
													<table>
														<tr>
															<td><input id="s_dev_coding" name="s_dev_coding"
												...
												<td align="right"><select id="dev_project_name"
													name="dev_project_name" style="width: 200px">
														<option value=""></option>
												</select> <input type="hidden" id="project_info_id" /></td>
												<td align="left"><img
													src="<%=contextPath%>/images/magnifier.gif" width="16"
													height="16" style="cursor: hand;"
													onclick="toChoseProject()" /></td>
												...
												<td class="ali_query"><span class="qc"><a
														href="#" onclick="clearQueryText()" title="JCDP_btn_clear"></a></span>
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
									<tr>
										<td width="49%">
											<div class="tongyong_box">
												<div class="tongyong_box_title">
													<span class="kb"><a href="#"></a></span><a href="#">震源备件消耗数量统计</a><span
														class="gd"><a href="#"></a></span> <span class="dc"
														style="float: right; margin-top: -4px;"> <a
														href="#" onclick="exportDataDoc('lpbjcount')"
														title="导出excel"></a>
													</span>
												</div>
										...
										<td width="49%">
											<div class="tongyong_box">
												<div class="tongyong_box_title">
													<span class="kb"><a href="#"></a></span><a href="#">震源备件消耗金额统计</a><span
														class="gd"><a href="#"></a></span> <span class="dc"
														style="float: right; margin-top: -4px;"> <a
														href="#" onclick="exportDataDoc2('aptm')" title="导出excel"></a>
													</span>
												</div>
												<div class="tongyong_box_content_left" id="chartContainer2"
													style="height: 400px;"></div>
											</div>
										</td>
