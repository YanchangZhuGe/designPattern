        ...
        if(isWbs == "true"){
  			...
  			if(isRoot == "true"){
  				...
  				if('<%=rootBackUrl %>'=='null'){
  					...
  					parent.mainRightframe.location.href = "<%= contextPath%><%=taskBackUrl %>?teamtype=1&taskObjectId="+taskObjectId+"&taskId="+taskId+"&projectInfoNo=<%=projectInfoNo %>&taskName="+encodeURI(encodeURI(taskName,'UTF-8'),'UTF-8')+"<%=customString %>";
  				}
  			} else {
  				//wbs
				if('<%=wbsBackUrl %>'=='null'){
  					...
  					parent.mainRightframe.location.href = "<%= contextPath%><%=taskBackUrl %>?teamtype=2&taskObjectId="+taskObjectId+"&taskId="+taskId+"&projectInfoNo=<%=projectInfoNo %>&taskName="+encodeURI(encodeURI(taskName,'UTF-8'),'UTF-8')+"<%=customString %>";
  				}
  			}
  		} else {
  			//任务
  			if('<%=taskBackUrl %>'=='null'){
  				...
  				parent.mainRightframe.location.href = "<%= contextPath%><%=taskBackUrl %>?teamtype=3&taskObjectId="+taskObjectId+"&taskId="+taskId+"&projectInfoNo=<%=projectInfoNo %>&taskName="+encodeURI(encodeURI(taskName,'UTF-8'),'UTF-8')+"<%=customString %>";
			}
  		}
    }
...
function fun(){
	...
	parent.mainRightframe.location.href = "<%= contextPath%><%=taskBackUrl %>?taskObjectId="+CheckOther1s+"&taskId="+TaskIds+"&projectInfoNo=<%=projectInfoNo %>&taskName="+Names+"<%=customString %>";
