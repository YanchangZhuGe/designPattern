...
<%
    ...
    String surfaceProgressId = respMsg.getValue("surfaceProgressId")!=null?respMsg.getValue("surfaceProgressId"):"";
...
<table  border="0" cellpadding="0" cellspacing="0" class="tab_line_height">  
	<tr class="even">
		<td colspan="4" align = "left">
		    ...
		    &nbsp;&nbsp;
			<a href="<%=contextPath%>/pm/dailyreport/querySurfaceProgress.srq?projectInfoNo=<%=projectInfoNo%>&projectName=<%=projectName%>&orgName=<%=orgName%>"><font color="red">表层进度</font></a>						
			&nbsp;&nbsp;
...
<input id="surfaceProgressId" name="surfaceProgressId" value="<%=surfaceProgressId %>" type="hidden"/>
	...
	<tr class="odd">
		...
		%>		
			<td class="inquire_item6" id="surfaceInfo">&nbsp;</td>
		<% 		
			}else{
		%>	
			<td class="inquire_item6" id="surfaceInfo">&nbsp;<font color='red'>今天数据还未录入</font></td>
		<%
			}
		%>
		...
		<td class="inquire_form6">
			<input name="surfaceFile" id="surfaceFile" type="file"/>&nbsp;
		</td>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			    	...
			    	<auth:ListButton functionId="" css="xz" event="onclick='toDownload()'" title="下载表层模板"></auth:ListButton>	
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
				...
				<tr class="even">
					...
					<td class=""><%=surfaceProgress.getValue("surfaceMethod") %></td>
...
<script type="text/javascript">
	...
	function simpleSearch(){
		...
		window.location = "<%=contextPath%>/pm/dailyreport/querySurfaceProgress.srq?projectInfoNo="+projectNo+"&consDate="+queryDate;
	}
	...
	function validate(){
		...
		if (flag){
		    ...
		    var retObj = jcdpCallService("InputDailyProgressSrv", "validateSurfaceProgress", "projectInfoNo=<%=projectInfoNo%>&consDate="+consDate);
		    if(retObj.updateFlag == "1"){
		    	//有数据,更新
		    	var surfaceProgressId = retObj.daily_progress_no;
			  	...
			  	{
					document.form1.action="<%=contextPath%>/pm/dailyreport/updateSurfaceProgress.srq?projectInfoNo=<%=projectInfoNo %>&dailyProgressNo="+surfaceProgressId;
		    	...
		    	//无数据,插入
	    		document.form1.action="<%=contextPath%>/pm/dailyreport/insertSurfaceProgress.srq?projectInfoNo=<%=projectInfoNo %>&consDate="+consDate;
	    		document.form1.submit();
		    }	  	
		}else{
			return false;
	...
	{
		var consDate=document.form1.consDate.value;
		var surfaceFile = document.form1.surfaceFile.value;
		if (consDate=="")
		{
	...
	function toDelete(){
		var dailyProgressNo = '<%=surfaceProgressId%>';
		if(dailyProgressNo != "" && dailyProgressNo != undefined){
			if(confirm('确定要删除吗?')){  
				var retObj = jcdpCallService("InputDailyProgressSrv", "deleteSurfaceProgress", "surfaceProgressId="+dailyProgressNo);
				var queryDate = document.getElementById("queryDate").value;
				
				cruConfig.queryService = "InputDailyProgressSrv";
				cruConfig.queryOp = "querySurfaceProgress";
	...
	function toEdit(){
		var retObj = jcdpCallService("InputDailyProgressSrv", "validateSurfaceProgress", "projectInfoNo=<%=projectInfoNo%>&consDate=<%=queryDate%>");
		//有数据,可以修改
		if(retObj.updateFlag == "1"){
			var daily_progress_no = retObj.daily_progress_no;
			document.getElementById("downloadFile").href="<%=contextPath %>/pm/dailyreport/downloadSurfaceFile.srq?dailyProgressNo="+daily_progress_no;
	...
	function exportData(){
		var dailyProgressNo = '<%=surfaceProgressId%>';
		if(dailyProgressNo != "" && dailyProgressNo != undefined){
			window.location = "<%=contextPath%>/pm/dailyreport/downloadSurfaceFile.srq?dailyProgressNo=<%=surfaceProgressId%>";	
		}else{
			alert("请选择一条记录!");
			return;
		}
	...
	function toDownload(){
		window.location.href="<%=contextPath%>/pm/dailyReport/singleProject/download.jsp?path=/pm/dailyReport/singleProject/surface.txt&filename=表层进度模版.txt";
	}
