...
<script type="text/javascript">
	...
	cruConfig.queryOp = "getWtAllPlanList";
	...
	function toAdd(){
		...
		var add_flag = document.getElementsByName("addflag")[0].value;
		if(add_flag == "0"){
			alert("初始计划没有保存,不能创建变更计划!");
			return;
		}else{
			//popWindow('<%=contextPath%>/pm/plan/planchangelist/index.jsp?projectInfoNo=<%=projectInfoNo %>');
			location.href = "<%=contextPath%>/wt/pm/planManager/singlePlan/progress/index.jsp?projectInfoNo=<%=projectInfoNo %>";
		}
	}
	...
	function toDelete(){
		...
		if(base_plan_id == ""){
			...
			if(proc_status_value == "1"){
				...
				if(confirm('确定要删除吗?')){  
					var retObj = jcdpCallService("ProjectPlanSrv", "deleteProjectPlan", "objectId="+object_id+"&projectInfoNo="+project_info_no+"&basePlanId="+base_plan_id+"&baselinePlanObjectId="+baseline_plan_object_id);
	...
	function dbclickRow(ids){
		...
		if(proc_status_value == "" || proc_status_value == "4"){
			//未提交和审批不通过状态,可以修改
			//alert("checkPlanList 未提交和审批不通过状态,可以修改");
			if(base_plan_id == ""){
				location.href = "<%=contextPath%>/wt/pm/planManager/singlePlan/progress/planchangelist/editindex.jsp?projectInfoNo="+project_info_no;
			}else{
				location.href = "<%=contextPath%>/wt/pm/planManager/singlePlan/progress/planchangelist/editindex.jsp?projectInfoNo="+project_info_no+"&basePlanId="+base_plan_id;
			}
		}else if(proc_status_value == "1" || proc_status_value == "3"){
			//alert("待审批和审批通过状态,不可修改");
			//待审批和审批通过状态,不可修改
			if(base_plan_id == ""){
				location.href = "<%=contextPath%>/wt/pm/planManager/singlePlan/progress/planchangelist/planListView.jsp?projectInfoNo="+project_info_no;
			}else{
				location.href = "<%=contextPath%>/wt/pm/planManager/singlePlan/progress/planchangelist/planListView.jsp?projectInfoNo="+project_info_no+"&basePlanId="+base_plan_id;
			}
		}else if(proc_status_value == "0"){
			//alert("未保存计划");
			location.href = "<%=contextPath%>/wt/pm/planManager/singlePlan/progress/planchangelist/editindex.jsp?projectInfoNo="+project_info_no+"&notSaved=1";
		}
	}
