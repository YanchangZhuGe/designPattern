...
<script type="text/javascript"> 
	...
	function loadDataDetail(){
		...
		if(device_backdet_id!=null){
		    var querySql="select backdet.device_backdet_id,backdet.device_backapp_id,backdet.dev_acc_id,backdet.dev_name,backdet.dev_model,";
		    	querySql+="backdet.back_num,backdet.planning_out_time,backdet.is_leaving,backdet.bsflag,backdet.devremark,t.back_num as mixnum,";
			    querySql+="t.device_coll_mixinfo_id,unitsd.coding_name as dev_unit_name,acc.device_id,acc.type_id,";
		    	...
		    	querySql+="left join gms_device_coll_account acc on accdui.fk_dev_acc_id=acc.dev_acc_id  ";
	...
	function submitInfo(){
		...
		var newunusingnum = document.getElementById("new_unusing_num").value;//现闲置数量
		var newothernum = document.getElementById("new_other_num").value;//现其他数量
		var newusingnum = document.getElementById("new_using_num").value;//现在用数量
		var usenewtotalnum = document.getElementById("use_new_total_num").value;//现总数量
		
		if(!re.test(newunusingnum) || !re.test(newothernum) || !re.test(newusingnum) || !re.test(usenewtotalnum)){
			alert("数量必须为有效数字!");
        	return;
		}
		var device_backdet_id = '<%=request.getParameter("id")%>';
		if(confirm('确定提交吗?')){
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSubmitJbqStock.srq?device_backdet_id="+device_backdet_id+"&devaccId="+devaccId+"&device_backapp_id="+device_backapp_id+"&projectInfoNo="+projectInfoNo;
