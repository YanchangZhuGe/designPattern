...
<%
	...
	String materials_no="";
	if(request.getParameter("materials_no") != null){
		materials_no=request.getParameter("materials_no");	
	...
	function checkJudge(){
 	...
 	function submitButton(){   
 		if(checkJudge()){
			...
			var submitStr = 'JCDP_TABLE_NAME=BGP_MATERIALS_ACCEPTANCE&JCDP_TABLE_ID=&org_sub_id='+org_sub_id
			+'&second_org='+second_org+'&third_org='+third_org+'&acceptance_numbers='+acceptance_numbers+'&acceptance_time='+acceptance_time
			...
			var materials_no=retObject.entity_id; //获得 主表id 			
 			top.frames('list').frames[1].refreshData();	 			 
 			 window.open("<%=contextPath%>/hse/hseOptionPage/preparednessAndResponse/homeFrame.jsp?addTypes=1&isProject=<%=isProject%>&materials_no="+materials_no,'homeMain','height=500,width=1000px,left=100px,top=100px,menubar=no,status=no,toolbar=no ', '');
...
querySql1 = "select count(*)-(-1) as num_p from BGP_MATERIALS_ACCEPTANCE   where bsflag='0'";
...
if(queryRet1.returnCode=='0'){
  datas1 = queryRet1.datas;	 
	if(datas1 != null && datas1 != ''){	 
		...
		document.getElementsByName("acceptance_numbers")[0].value=numDate;			
	}
}	
