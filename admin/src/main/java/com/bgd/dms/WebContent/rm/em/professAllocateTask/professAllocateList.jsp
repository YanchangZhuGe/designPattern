      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <td class="ali_cdn_name">项目名称</td>
			    <td width="20%" ><input class="input_width" id="s_project_info_no" name="s_project_info_no" type="text" onkeypress="simpleRefreshData()"/>
			   ...
			   <td class="ali_cdn_name">分配状态</td>
			    ...
			    <td width="25%">&nbsp;</td>
  				...
  				<auth:ListButton functionId="F_HUMAN_006" css="dc" event="onclick='exportData()'" title="JCDP_btn_export"></auth:ListButton>		  				
			  </tr>
			</table>
			</td>
			...
			<div id="table_box" style="height:100%">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{profess_no}-{project_type}' id='rdo_entity_id_{profess_no}'  onclick='chooseOne(this);'   />" >选择</td>
			      <td class="bt_info_even" autoOrder="1">序号</td>
			      <td class="bt_info_odd" exp="{project_name}">项目名称</td>
			      <td class="bt_info_even" exp="{apply_no}">单号</td>
			      <td class="bt_info_odd" exp="{z_rw}">任务状态</td>
			      <td class="bt_info_even" exp="{prepare_status_name}">调配状态</td>
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">
			    <li class="selectTag" id="tag3_0" ><a href="#" onclick="getTab3(0)">申请单据</a></li>
			    <li id="tag3_1"><a href="#" onclick="getTab3(1)">审核调配</a></li>
			    <li id="tag3_2"><a href="#" onclick="getTab3(2)">任务分配</a></li>
			...
			<div id="tab_box" class="tab_box">
				...
				<div id="tab_box_content2" class="tab_box_content" style="display:none;">
					<table id="allocateDetailList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
				    	<tr class="bt_info">
				    	    <td>序号</td>
				            <td>班组</td>
				            <td>岗位</td>		
...
//		ids = getSelIds('rdo_entity_id');
		...
		popWindow('<%=contextPath%>/rm/em/toProfessionAllocateEdit.srq?id='+ids.split("-")[0],'1000:800');
		
	}
	
	function refreshData(){

		cruConfig.queryStr = " select p.*  from (select  distinct  nvl(pr.prepare_status,0) prepare_status,decode(pr.prepare_status,     '0', '待调配',    '1',    '调配中',   '2',   '已调配',     '待调配') prepare_status_name,decode(te.proc_status,   '1',   '待审批',  '3',  '审批通过', '4', '审批不通过',te.proc_status) proc_status_name, t.project_name, t.project_type,p.profess_no,p.project_info_no, p.apply_no,p.apply_state,p.applicant_id,e.employee_name, p.apply_company,i.org_name, p.apply_date ,ht.post_detail_no  ,  case   when ht.post_detail_no  is null then     '未分配' else   '已分配'  end    z_rw  from bgp_project_human_profess p   left join bgp_human_prepare pr on p.profess_no = pr.profess_no and pr.prepare_org_id=p.apply_company and pr.bsflag='0'    left join gp_task_project t on t.project_info_no=p.project_info_no left join common_busi_wf_middle te on    te.business_id=p.profess_no   and te.bsflag='0' and business_type  in ('5110000004100000023','5110000004100001054')    left join comm_org_information i on p.apply_company=i.org_id and i.bsflag='0' left join comm_human_employee e on p.applicant_id=e.employee_id and e.bsflag='0'  left join  (select pr.profess_no,t.post_detail_no    from bgp_comm_human_allocate_task_a t  inner join bgp_project_human_profess_deta d  on t.post_detail_no = d.post_detail_no  inner join bgp_project_human_profess_post p  on d.post_no = p.post_no  and p.bsflag = '0'  inner join bgp_project_human_profess pr  on p.profess_no = pr.profess_no  left join comm_coding_sort_detail d1  on t.apply_team = d1.coding_code_id  left join comm_coding_sort_detail d2  on t.post = d2.coding_code_id  left join comm_human_employee d3  on t.employee_id = d3.employee_id  where    pr.bsflag = '0')ht on ht.profess_no= p.profess_no    where  p.bsflag='0' and p.profess_no in (select p.profess_no from bgp_project_human_profess_deta d inner join bgp_project_human_profess_post d1 on d.post_no=d1.post_no and d1.bsflag='0' inner join bgp_project_human_profess p on d1.profess_no=p.profess_no and p.bsflag='0' where d.deploy_org in (select s.org_id from comm_org_subjection s where s.org_subjection_id like 'C105006002%') ) )p    order by  p.prepare_status asc ,p.apply_date desc  ";
	...
	function simpleSearch(){ 
		...
		if(s_proc_status != ''){
			if(s_proc_status=='0'){
				str+=" and 	post_detail_no is null  "
				//str+="  and profess_no not  in (select pr.profess_no   from bgp_comm_human_allocate_task_a t  inner join bgp_project_human_profess_deta d  on t.post_detail_no = d.post_detail_no  inner join bgp_project_human_profess_post p  on d.post_no = p.post_no  and p.bsflag = '0'  inner join bgp_project_human_profess pr  on p.profess_no = pr.profess_no  left join comm_coding_sort_detail d1  on t.apply_team = d1.coding_code_id  left join comm_coding_sort_detail d2  on t.post = d2.coding_code_id  left join comm_human_employee d3  on t.employee_id = d3.employee_id  where    pr.bsflag = '0') ";	
			}else if(s_proc_status=='1'){
				str+=" and	post_detail_no is not null   "
				//str+="  and profess_no   in (select pr.profess_no   from bgp_comm_human_allocate_task_a t  inner join bgp_project_human_profess_deta d  on t.post_detail_no = d.post_detail_no  inner join bgp_project_human_profess_post p  on d.post_no = p.post_no  and p.bsflag = '0'  inner join bgp_project_human_profess pr  on p.profess_no = pr.profess_no  left join comm_coding_sort_detail d1  on t.apply_team = d1.coding_code_id  left join comm_coding_sort_detail d2  on t.post = d2.coding_code_id  left join comm_human_employee d3  on t.employee_id = d3.employee_id  where    pr.bsflag = '0') ";
    ...
    function loadDataDetail(ids){
    	...
    	if(project_type =='5000100004000000008'){
    	  ...
    	  processNecessaryInfo={         
  	    		...
  	    		businessInfo:"多项目专业化调配任务分配流程",        //用于待审批界面展示业务信息
		...
		if(datas2 != null){
			for (var n = 0; n< queryRet2.datas.length; n++) {
				
				var tr = document.getElementById("allocateDetailList").insertRow();		
				...
				var td = tr.insertCell(0);
				td.innerHTML = datas2[n].rownum;
				
				var td = tr.insertCell(1);
				td.innerHTML = datas2[n].apply_teamname;
				
				var td = tr.insertCell(2);
				td.innerHTML = datas2[n].postname;

				var td = tr.insertCell(3);
				td.innerHTML = datas2[n].employee_name;
				
				var td = tr.insertCell(4);
				td.innerHTML = datas2[n].notes;
