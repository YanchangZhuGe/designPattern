...
$().ready(function(){
	...
	function submitInfo(){
		...
		****/
		//alert(count);
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/devRepair/repairSendRtnNewApply.srq?count="+count+"&line_infos="+line_infos;
	...
	$().ready(function(){
		$("#addProcess").click(function(){
			...
			var id = $("#repid").val();
			var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/devRepair/selectDevSendDetailForBack.jsp?id="+id+"&projectinfono=projectInfoNo&backdevtype=backdevtype",paramobj,"dialogWidth=820px;dialogHeight=450px");
			...
			var condition ="(";
				...
				var str = "SELECT SEND.REPAIR_FORM_NO AS REPAIR_FORM_NO,SEND.REPAIR_FORM_NAME AS REPAIR_FORM_NAME,ACC.DEV_NAME AS DEV_NAME, ACC.DEV_MODEL AS DEV_MODEL,ACC.DEV_TYPE AS DEV_TYPE\n" +
	...
	$(
			function refreshData(){
				var sql = "SELECT T.*,SEND.REPAIR_FORM_NO AS REPAIR_FORM_CODE,SEND.ID AS REPID FROM GMS_DEVICE_COL_REP_FORM T,GMS_DEVICE_COLL_REPAIR_SEND SEND WHERE 1 = 1 AND T.BACKAPP_NO = SEND.ID(+) ";
				sql += " and t.ID = '<%=id%>'";
