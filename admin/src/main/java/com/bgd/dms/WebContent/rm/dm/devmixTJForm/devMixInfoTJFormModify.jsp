...
<title>(自有设备)单修改界面</title>
</head>
<body class="bgColor_f3f3f3" onload="refreshData();">
<form name="form1" id="form1" method="post" action="">
<div id="new_table_box" style="width:98%">
  <div id="new_table_box_content" style="width:100%">
    <div id="new_table_box_bg" style="width:97%">
      ...
      <fieldset style="margin-left:2px"><legend>调配台账明细</legend>
		<div id="tab_box" class="tab_box" style="overflow:auto;">
			<table style="width:97.9%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
		       		<td class="bt_info_even" width="10%">班组</td>
...
<script type="text/javascript"> 
	function submitInfo(state){
		...
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveSelfMixFormAllInfo.srq?mixform_type=3&state="+state+"&count="+count+"&line_infos="+line_infos+"&idinfos="+idinfos+"&detailcount="+detailcount;
		...
		$("input[type='checkbox'][name='detinfo']").each(function(i){
			if(this.checked){
				...
				for(var j = 0;j<parseInt(mixnuminfo);j++){
					...
					innerhtml += "<td width='11%'><input name='detdevicetype"+seqinfo+"' id='detdevicetype"+seqinfo+"' value='"+devicemodel+"' style='line-height:18px;' size='9' type='text' readonly/><img src='<%=contextPath%>/images/magnifier.gif' width='16' height='16' style='cursor:hand;' onclick=showDevPage("+seqinfo+",'"+devcicode+"','"+isdevicecode+"') /></td>";
					...
					innerhtml += "<td width='10%'><input name='asset_coding"+seqinfo+"' id='asset_coding"+seqinfo+"' style='line-height:18px;' size='10' value='' readonly/></td>";
	...
	function showDevPage(trid,devcicode,isdevicecode){
		...
		//var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/tree/selectAccountForMix.jsp?dev_ci_code="+devcicode+"&out_org_id="+out_org_id , obj ,"dialogWidth=820px;dialogHeight=380px");
		var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/tree/selectAccountForMix.jsp?isdevicecode="+isdevicecode+"&dev_ci_code="+devcicode+"&out_org_id="+out_org_id , obj ,"dialogWidth=950px;dialogHeight=480px");
	...
	function refreshData(){
		...
		if('<%=devappid%>'!=null){
			//回填基本信息
			var str = "select devapp.device_mixapp_id,devapp.device_mixapp_no,to_char(mif.create_date,'yyyy-mm-dd') as appdate,"
			+"apporg.org_name as app_org_name,devapp.app_org_id as in_org_id,devapp.project_info_no,"
			+"tp.project_name,mixorg.org_abbreviation as mix_org_name,outorg.org_abbreviation as out_org_name,"
			+"devapp.mix_org_id,devapp.mix_type_id,mif.out_org_id,mif.mixinfo_no "
			+"from gms_device_dismixapp devapp "
			+"left join gms_device_mixinfo_form mif on mif.device_app_id=devapp.device_mixapp_id "
			...
			+"where devapp.state='9' and devapp.bsflag='0' and devapp.device_mixapp_id='<%=devappid%>'";
			...
			if(retObj!=undefined && retObj.length>0){
				...
				$("#device_app_no").val(retObj[0].device_mixapp_no);
			...
			//回填明细信息
			var prosql = "select appdet.device_mixapp_detid,appdet.team,appdet.apply_num,appdet.purpose,appdet.isdevicecode,";
				prosql += "appdet.plan_start_date,appdet.plan_end_date,appdet.teamid,appdet.unitinfo,nvl(tmp.mixed_num,0) mixed_num,";
				prosql += "appdet.dev_name as dev_ci_name,appdet.dev_type as dev_ci_model,appdet.dev_ci_code,unitsd.coding_name as unit_name,";
				prosql += "teamsd.coding_name as team_name,amm.assign_num,amm.device_mix_subid ";
				prosql += "from gms_device_dismixapp_detail appdet ";
				...
				prosql += "where amm.bsflag='0' group by device_app_detid) tmp on tmp.device_app_detid = appdet.device_mixapp_detid ";
				prosql += "left join gms_device_appmix_main amm on appdet.device_mixapp_detid = amm.device_app_detid ";
				...
				prosql += "where appdet.device_mixapp_id='<%=devappid%>' and amm.device_mixinfo_id='<%=mixInfoId%>' ";
		...
		if(retObj!=undefined){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td width='5%'><input type='checkbox' name='detinfo' id='"+index+"' value='"+retObj[index].device_mixapp_detid+"' ";
				...
				innerhtml += "<td width='6%'><input name='unitname"+index+"' id='unitname"+index+"' value='"+retObj[index].unit_name+"' size='3' type='text' readonly/>";
				...
				innerhtml += "<input name='checkmixednum"+index+"' id='checkmixednum"+index+"' value='"+(parseInt(retObj[index].mixed_num)-parseInt(retObj[index].assign_num))+"' type='hidden' readonly/></td>";
		...
		var prosql = "select amd.device_mix_detid,amd.dev_ci_code,amd.dev_acc_id,amd.asset_coding,amd.self_num,amd.dev_sign,amd.license_num,'N' as isdevicecode,";
			prosql += "amd.dev_plan_start_date,amd.dev_plan_end_date,amm.device_app_detid,amm.device_mix_subid,ci.dev_ci_name,ci.dev_ci_model,";
		...
		if(retObj!=undefined){
			...
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td width='10%'><input name='dev_sign"+seqinfo+"' id='dev_sign"+seqinfo+"' value='"+retObj[index].dev_sign+"' style='line-height:18px;' type='text' size='11' readonly/></td>";
				...
				innerhtml += "<td width='10%'><input name='asset_coding"+seqinfo+"' id='asset_coding"+seqinfo+"' style='line-height:18px;' size='10' value='"+retObj[index].asset_coding+"' readonly/></td>";
				innerhtml += "<td width='9%'><input name='detownorgname"+seqinfo+"' id='detownorgname"+seqinfo+"' style='line-height:18px;' size='10' value='"+retObj[index].org_abbreviation+"' readonly/></td>";
