...
<%
	...
	if(user != null){
		...
		//设备物资处震源中心 仪器中心  测量中心 和八个作业部 单独处理成可以查看全部项目
		if(orgSubId.startsWith("C105006")){//&&!orgSubId.startsWith("C105006003")){
			orgSubId="C105";
...
if(orgSubId != "C105"){
	var org_in_sql ;
	if(orgSubId=='C105001002'||orgSubId=='C105001003'||orgSubId=='C105001004'||orgSubId=='C105001005'){
		org_in_sql='C6000000000002'
	}else{
		org_in_sql='C6000000000001'
	}
	querySql = "select eps.eps_name as name,eps.object_id,eps.object_id || ',' || eps.org_id || ',' || os.org_subjection_id as id from bgp_eps_code eps " +
	  		   ...
	  		   " join bgp_eps_code eps1 on eps.parent_object_id = eps1.object_id and eps1.bsflag = '0'  and eps1.org_id = '"+org_in_sql+"'"+                                                          
...
function clickNode(selectedNode){
	...
	if("井中地震"==selectedNode.text && "新兴物探开发处"==selectedNode.parentNode.text ){
	...
	if("大港物探处"==selectedNode.text||"C105007"==idss[2]){
		projectType="5000100004000000001,5000100004000000002,5000100004000000010";
	}  else 
	if("深海物探处"==selectedNode.text){
		projectType="5000100004000000006";
	}else{//原来三期地震项目
	    projectType="5000100004000000001";
	}
	if(""!=projectType){
		parent.mainRightframe.location.href = "<%=contextPath %><%=backUrl %>?flg_pss="+flg+"&projectType="+projectType+"&action=<%=action %>&orgSubjectionId="+idss[2]+"&orgId="+idss[1]+"&epsObjectId="+idss[0]+"&clickNode=1&forwardJsp=/rm/dm/devdis/disMainInfoList.jsp";
	}
}
