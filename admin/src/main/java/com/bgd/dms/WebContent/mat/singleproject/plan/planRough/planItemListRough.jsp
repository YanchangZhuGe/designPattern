...
<script type="text/javascript">
	...
	function refreshData(){
		...
		var sql ='';
		sql +="select t.plan_id,t.plan_no,oi.org_abbreviation,t.apply_date,t.apply_person,t.apply_phone,t.erp_no,decode(t.if_submit,'0','未提交','1','已提交') if_submit from gms_mat_demand_plan_rough t left join comm_org_information oi on t.org_id = oi.org_id and oi.bsflag='0' where t.bsflag='0' and t.if_repeat is null and t.project_info_no='<%=projectInfoNo%>' and t.task_object_id='"+taskObjectId+"'";
...
*/				var retObj = jcdpCallService("MatItemSrv", "queryRoughPlan", "plan_id="+ids);
				var submiteIf = retObj.map.ifSubmit;
			 	if(submiteIf =='1'){ alert("申请单已被提交!");
					...
					popWindow('<%=contextPath%>/mat/singleproject/plan/planRough/planEditRough.jsp?taskObjectId='+taskObjectId+'&plan_id='+ids,'1024:800');
				}
		    }
	 }
	 function toAdd(){ 
