			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_even" exp="{is_risk}<input type='hidden' name='is_risk_{risk_identify_id}' id='is_risk_{risk_identify_id}' value='{risk_confirm}'/>">风险确认</td>
			    </tr>
			  </table>
			</div>
					  ...
					  <tr>
					   ...
					   <tr>
					    <td class="inquire_item6">风险描述：</td>
					   	<td class="inquire_form6" id="item0_8">
...
<script type="text/javascript">
	...
	function simpleSearch(){
		...
		if(query_type == "0"|| query_type == "1"){
			...
			if(query_type == "0"){
				...
				if(project_name != ""){
					if(is_risk_confirm == "0"|| is_risk_confirm == "1"){	
						cruConfig.submitStr = "riskConfirm="+is_risk_confirm+"&isMulti=1&projectName="+project_name;	
						queryData(1);
			...
			//项目编号
				...
				if(project_number != ""){
					if(is_risk_confirm == "0"|| is_risk_confirm == "1"){	
						cruConfig.submitStr = "riskConfirm="+is_risk_confirm+"&isMulti=1&projectNumber="+project_number;	
						queryData(1);
					}else if(is_risk_confirm == "2"){
		...
		//没有选择按项目查询,
			...
			if(is_risk_confirm == "0"|| is_risk_confirm == "1"){		
				cruConfig.submitStr = "riskConfirm="+is_risk_confirm+"&isMulti=1";	
				queryData(1);
			}else if(is_risk_confirm == "2"){
				refreshData();
			}
	...
	function toAdd(){
		popWindow('<%=contextPath %>/risk/multiproject/mRiskIdentify/medit_risk_identify.jsp?pageAction=Add');		
	}
	...
	function toEdit(){
		...
		popWindow('<%=contextPath %>/risk/multiproject/mRiskIdentify/medit_risk_identify.jsp?pageAction=Edit&riskIdentifyId='+ids);
	...
	function addMeasure(){
		if(risk_id_value == ""){
			...
			location.href="<%=contextPath %>/risk/addMulRiskMeasure.srq?risk_identify_id="+risk_id_value+"&risk_measure_value="+document.getElementById("risk_measure").value;
		}
	}
