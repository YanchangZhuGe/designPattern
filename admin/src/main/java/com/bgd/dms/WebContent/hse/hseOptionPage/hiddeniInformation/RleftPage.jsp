...
var datas1 =null;
	   ...
	   querySql = "select tr.hidden_no, dil.cdetail_no,  ions.CORRECTIVE_NO,  ion.org_name,  oi1.org_abbreviation as second_org_name, oi2.org_abbreviation as third_org_name,   tr.hidden_name,  tr.report_date,    decode(ions.rectification_state,'1','已整改','2','未整改','3','正在整改')rectification_state,   dil.crisk_levels   from BGP_CORRECTINVE_DETAIL dil   left join BGP_CORRECTINVE_INFORMATION ions     on ions.CORRECTIVE_NO = dil.CORRECTIVE_NO    and dil.bsflag = '0'   join BGP_HIDDEN_INFORMATION tr     on dil.hidden_no = tr.hidden_no    and tr.bsflag = '0'   join comm_org_subjection os1     on tr.second_org = os1.org_subjection_id    and os1.bsflag = '0'   join comm_org_information oi1     on oi1.org_id = os1.org_id    and oi1.bsflag = '0'   join comm_org_subjection os2     on tr.third_org = os2.org_subjection_id    and os2.bsflag = '0'   join comm_org_information oi2     on oi2.org_id = os2.org_id    and oi2.bsflag = '0'   join comm_org_information ion     on ion.org_id = os1.org_id  where tr.bsflag = '0'    and ions.bsflag = '0'  and  tr.hidden_no not in (select dil.hidden_no  from BGP_REWARD_DETAIL dil left join  BGP_REWARD_INFORMATION  ion on ion.REWARD_NO=dil.REWARD_NO and dil.bsflag='0' where ion.bsflag='0')";
		for(var key in arrObj) { 
			...
			if(arrObj[key].value!=undefined && arrObj[key].value!='')
				...
				}else if(arrObj[key].label =='crisk_levels'){
					querySql += " and dil."+arrObj[key].label+" = '"+arrObj[key].value+"' ";					
				} else if(arrObj[key].label =='rectification_state'){
 ...
 function deleteTableTr(tableID){
	   ...
	   function add(){
        ...
        self.parent.frames["rightframe"].location="<%=contextPath %>/hse/hseOptionPage/hiddeniInformation/RrightPage.jsp?hidden_no="+certificate_no+"&reward_no=<%=reward_no%>";
