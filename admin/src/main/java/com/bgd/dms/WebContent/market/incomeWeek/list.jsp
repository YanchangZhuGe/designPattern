...
var jcdp_codes = new Array(
  	...
  	function toEdit(){
	    ...
	    if(subflag == '0'){
	    	...
	    	window.location = "<%=contextPath%>/market/week/editValueReport.srq?week_date="+week_date+"&week_end_date="+week_end_date+"&action=edit";
	    	}else{
	    	window.location = "<%=contextPath%>/market/incomeWeek/orgInsert.jsp?orgId=<%=orgId%>&week_date="+week_date+"&week_end_date="+week_end_date+"&action=edit";
	    	}
	    }else{
	    	alert("该记录已经提交，不能修改!"); 
	    	return; 
  	...
  	function toDelete(){
	    ...
	    if(subflag == '0'){
	    	if(orgId=="C6000000000025"){
	    	var sql = "update bgp_wr_income_money set bsflag='1' where org_type='1' and type='1' and to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
	    	}else{
		    var sql = "update bgp_wr_income_money set bsflag='1' where org_type='0' and type='1' and org_id='<%=orgId%>' and to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
	    	}
	    	deleteEntities(sql);
	    	
	    	var title = "在周市场落实价值工作量中删除了一条周报开始日期为“"+week_date+"”的信息"
	...
	function JcdpButton3OnClick(){
	    ...
	    if(subflag == '0'){
	    	if(orgId=="C6000000000025"){
	    	var sql = "update bgp_wr_income_money set subflag='1' where org_type='1' and type='1' and  to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
	    	}else{
		    	var sql = "update bgp_wr_income_money set subflag='1' where org_type='0' and type='1' and bsflag='0' and org_id='<%=orgId%>' and to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
	    	}
...
function JcdpButton4OnClick(){
	    ...
	    if(subflag == '1'){
	    	if(orgId=="C6000000000025"){
	    	var sql = "update bgp_wr_income_money set subflag='0' where org_type='1' and bsflag='0' and type='1' and  to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
	    	}else{
		    	var sql = "update bgp_wr_income_money set subflag='0' where org_type='0' and type='1' and bsflag='0' and org_id='<%=orgId%>' and to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
	    	}
...
function page_init(){
	...
	cdt_init();
	if(orgId=="C6000000000025"){
		cruConfig.queryStr = " 	select distinct t.week_date,t.subflag,t.week_end_date  from bgp_wr_income_money t  	 where t.bsflag = '0' and t.org_type='1' and t.type='1' order by week_date desc    ";
	}else{
		cruConfig.queryStr = " 	select distinct t.week_date,t.subflag,t.week_end_date  from bgp_wr_income_money t  	 where t.bsflag = '0' and t.org_type='0' and t.type='1' and t.org_id='<%=orgId%>' order by week_date desc    ";
	}
...
function toAdd(){
	if(orgId=="C6000000000025"){
	window.location='<%=contextPath%>/market/incomeWeek/martSelectWeek.jsp';
	
	}else{
	window.location='<%=contextPath%>/market/incomeWeek/orgInsert.jsp?orgId=<%=orgId%>&action=add';
	}
}
...
function linkThirdList(orgId){
	window.location.href="<%=contextPath%>/market/income/incomeReport.srq?orgId="+orgId;
}
</script>
...
<table border="0"  cellspacing="0"  cellpadding="0"  class="form_info"  width="100%" id="queryRetTable">
	...
	<tr class="bt_info">
		...
		<td class="tableHeader" 	 exp="<a href=javascript:link2self('<%=contextPath%>/market/week/viewValueReport.srq?week_date={week_date}&week_end_date={week_end_date}&action=view')><font color='3366ff'>{week_date}</font></a>">周报开始日期</td>
		<%}else{%>
		<td class="tableHeader" 	 exp="<a href=javascript:link2self('<%=contextPath%>/market/incomeWeek/orgInsert.jsp?orgId=<%=orgId%>&week_date={week_date}&week_end_date={week_end_date}&action=view')><font color='3366ff'>{week_date}</font></a>">周报开始日期</td>
		<%} %>
		<td class="tableHeader" 	 exp="{week_end_date}">周报结束日期</td>
