...
<%
	...
	if(orgCode.indexOf("C105007")>-1){
		response.sendRedirect(request.getContextPath()+"/mat/singleproject/plansub/planSubListDg.jsp");
		return;
	}
	    ...
	    //查询函数
	     function  testQuery(selectThis){
			...
			if(selectThis.options[selectThis.selectedIndex].value=='4'){
				sql += "select t.total_money,t.compile_date,u.user_name ,t.plan_invoice_id,t.submite_number,(case when comm.proc_status = '1'then'待审核' when comm.proc_status = '3' then '审批通过' when comm.proc_status = '4' then '审批不通过' else '未提交' end) if_submit,t.plan_name from gms_mat_demand_plan_invoice t left join common_busi_wf_middle comm on t.plan_invoice_id=comm.business_id and comm.bsflag='0' inner join p_auth_user u on t.creator_id=u.user_id and u.bsflag='0'  where t.bsflag = '0'and t.project_info_no='<%=projectInfoNo%>' and t.submite_if is not null order by t.compile_date desc,t.modifi_date desc"; 
//	sql +="select t.compile_date,u.user_name ,t.plan_invoice_id,t.submite_number, decode(t.submite_if,'0','未提交','1','待审批','2','审批未通过','3','审批通过','4','调剂未通过','5','调剂通过') submite_if from gms_mat_demand_plan_invoice t inner join p_auth_user u on t.creator_id=u.user_id and u.bsflag='0'  where t.bsflag = '0'and t.project_info_no='' order by t.compile_date desc";
			}else{
			 	sql += "select t.total_money,t.compile_date,u.user_name ,t.plan_invoice_id,t.submite_number,(case when comm.proc_status = '1'then'待审核' when comm.proc_status = '3' then '审批通过' when comm.proc_status = '4' then '审批不通过' else '未提交' end) if_submit,t.plan_name from gms_mat_demand_plan_invoice t left join common_busi_wf_middle comm on t.plan_invoice_id=comm.business_id and comm.bsflag='0' inner join p_auth_user u on t.creator_id=u.user_id and u.bsflag='0'  where t.bsflag = '0' and comm.proc_status = '"+selectThis.options[selectThis.selectedIndex].value+"' and t.project_info_no='<%=projectInfoNo%>' and t.submite_if is not null order by t.compile_date desc,t.modifi_date desc";
//				sql += "select t.compile_date,u.user_name ,t.plan_invoice_id,t.submite_number, decode(t.submite_if,'0','未提交','1','待审批','2','审批未通过','3','审批通过','4','调剂未通过','5','调剂通过') submite_if from gms_mat_demand_plan_invoice t inner join p_auth_user u on t.creator_id=u.user_id and u.bsflag='0'  where t.bsflag = '0'and t.project_info_no='' and t.submite_if='"+selectThis.options[selectThis.selectedIndex].value+"' order by t.compile_date desc";
