			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_appdet_id_{device_allapp_id}' name='device_allapp_id'  idinfo='{device_allapp_id}'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' value='{device_allapp_id}' allapp_type='{allapp_type}' id='selectedbox_{device_allapp_id}' />" >选择</td>
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				str += "where devapp.device_allapp_id = '"+currentid+"' and alldet.bsflag='0' ";
				str += "union all "
				str += "select alldet.device_allapp_detid,alldet.project_info_no,sd.coding_name as unitname,teamsd.coding_name as teamname, ";
...
<script type="text/javascript">
    ...
    function refreshData(v_devapp_name,v_org_name,v_state_desc){
		...
		var str = "select wfmiddle.create_date as approve_date,pro.project_name,devapp.allapp_type,devapp.device_allapp_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.project_info_no,";
			...
			str += "where devapp.allapp_type = 'S9999' and devapp.bsflag = '0' and devapp.project_info_no='"+projectInfoNos+"' ";		
    ...
    function loadDataDetail(device_allapp_id){
        ...
        processNecessaryInfo={        							//流程引擎关键信息
        			...
        			businessType:"5110000004100001032",    //业务类型 即为之前设置的业务大类
        			businessId:device_allapp_id,           				//业务主表主键值
        			businessInfo:"大港外租设备配置计划审批列表信息<配置计划单名称:"+device_allapp_name+">",
		...
		processAppendInfo={ 
			...
			deviceallappid:device_allapp_id
		};
	...
	function toAddMainPlanPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devPlan/devallplan_new_dg.jsp?projectInfoNo=<%=projectInfoNo%>&allapp_type=S9999');
	}
	function toModifyMainPlanPage(){
		...
		var str = "select devapp.device_allapp_id,nvl(wfmiddle.proc_status,'') as proc_status ";
			...
			str += "where devapp.allapp_type = 'S9999' and devapp.bsflag = '0' and devapp.project_info_no='"+projectInfoNos+"' ";
			str += "and devapp.device_allapp_id='"+deviceallappid+"' ";
		var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		//只要不在流程中，都可以修改
		if(unitRet.datas[0].proc_status == '' ){
			popWindow('<%=contextPath%>/rm/dm/devPlan/devallplan_modify_dg.jsp?projectInfoNo=<%=projectInfoNo%>&deviceallappid='+deviceallappid+'&allapp_type=S9999');
	...
	function toDelPlanPage(){
		...
		var str = "select devapp.device_allapp_id,nvl(wfmiddle.proc_status,'') as proc_status ";
			str += "from gms_device_allapp devapp  ";
			str += "left join common_busi_wf_middle wfmiddle on wfmiddle.business_id = devapp.device_allapp_id  ";
			str += "where devapp.bsflag = '0' and devapp.allapp_type = 'S9999' and devapp.project_info_no='"+projectInfoNos+"' ";
			str += "and devapp.device_allapp_id='"+deviceallappid+"' ";
		var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		//什么状态不能删除
		if(unitRet.datas[0].proc_status == null||unitRet.datas[0].proc_status == ''||unitRet.datas[0].proc_status == '4'){
			if(confirm("是否执行删除操作?")){
				var sql = "update gms_device_allapp set bsflag='1' where device_allapp_id ='"+deviceallappid+"' ";
	...
	function toSumbitDevApp(){
		...
		if(unitRet.datas[0].proc_status == '' || unitRet.datas[0].proc_status == '4'){
			...
			querySql += "from (select device_allapp_detid from gms_device_allapp_detail appdet where appdet.device_allapp_id ='"+deviceallappid+"' union all "+
				"select device_allapp_detid from gms_device_allapp_colldetail collappdet where collappdet.device_allapp_id ='"+deviceallappid+"' ) ";
			...
			var retObj="";
			if(allappType=='S9999'){
				retObj = jcdpCallService("DevCommInfoSrv", "createDgHireappInfo", "device_allapp_id="+deviceallappid);
			}else if(allappType=='S0623'){
				retObj = jcdpCallService("DevCommInfoSrv", "createAppGetAllappSS", "device_allapp_id="+deviceallappid);
	...
	function toModifyDetail(obj){
		...
		window.location.href='<%=contextPath%>/rm/dm/devPlan/devplan_app_frame.jsp?projectInfoNo=<%=projectInfoNo%>&allappType=S9999&deviceallappid='+idinfo+'&dgFlag='+dgFlag;
	}
	
	function dbclickRow(shuaId){

		window.location.href='<%=contextPath%>/rm/dm/devPlan/devplan_app_frame.jsp?projectInfoNo=<%=projectInfoNo%>&allappType=S9999&deviceallappid='+shuaId+'&dgFlag='+dgFlag;		
	} 
