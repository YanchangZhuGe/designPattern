...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <title>多项目-设备出库-设备出库(装备按台)</title> 
 </head> 
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_outinfo_id_{device_outinfo_id}' name='device_outinfo_id'>
			     	<td class="bt_info_even" exp="<input type='checkbox' name='selectedbox' mixstate='{opr_state}' value='{device_mixinfo_id}' id='selectedbox_{device_mixinfo_id}' />" >选择</td>
				...
				<div id="tab_box_content1" name="tab_box_content1" idinfo="" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							...
							<td class="bt_info_even" width="25%">调配类型</td>
							<td class="bt_info_odd" width="25%">设备名称</td>
							<td class="bt_info_even" width="25%">规格型号</td>
							<td class="bt_info_odd" width="20%">调配数量</td>
				        </tr>
...
<script type="text/javascript">
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && currentid == idinfo){
				...
				if(currentid!=null){
					//先进行查询
					var prosql = "select '调配明细' as devtype,amm.device_mix_subid,appdet.dev_name as dev_ci_name,appdet.dev_type as dev_ci_model,amm.assign_num ";
					...
					prosql += "where amm.bsflag = '0' and amm.assign_num is not null  and amm.device_mixinfo_id='"+currentid+"' ";
					prosql += "union all ";
					prosql += "select '补充明细' as devtype,amm.device_mix_subid,amm.dev_name as dev_ci_name,amm.dev_model as dev_ci_model,amm.assign_num ";
					prosql += "from gms_device_appmix_added amm ";
					prosql += "where amm.bsflag = '0' and amm.device_mixinfo_id='"+currentid+"' ";
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			var innerHTML = "<tr>";
			innerHTML += "<td>"+(i+1)+"</td><td>"+datas[i].devtype+"</td><td>"+datas[i].dev_ci_name+"</td>";
			innerHTML += "<td>"+datas[i].dev_ci_model+"</td><td>"+datas[i].assign_num+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
	...
	function refreshData(v_appinfo_name, v_mixinfo_no, v_opr_state_desc, v_project_name){
		var str = "select pro.project_info_no,mix.device_mixinfo_id,mix.mixinfo_no,pro.project_name,mix.create_date,devapp.device_app_no,devapp.device_app_name,"+
		...
		str += "where mix.state='9' and mix.bsflag='0' and mix.mix_type_id!='S0000' and mix.mix_type_id!='S14059999' ";
		//加机构权限
		...
		if(v_opr_state_desc!=undefined && v_opr_state_desc!=''){
			if(v_opr_state_desc == '1'){//处理中
				str += "and mix.opr_state = '1' ";
			}else if(v_opr_state_desc == '9'){//已处理
				str += "and mix.opr_state = '9' ";
			}else if(v_opr_state_desc == '4'){//已关闭
				str += "and mix.opr_state = '4' ";				
			}else if(v_opr_state_desc == '0'){//未处理
				str += "and mix.opr_state = '0' ";				
			}					
		}
		if(v_project_name!=undefined && v_project_name!=''){
			str += "and pro.project_name like '%"+v_project_name+"%' ";
		}
		str += "order by mix.opr_state nulls first,mix.modifi_date desc,mix.modifi_date desc,mix.project_info_no ";
	...
	function toAddDetailPage(){
		...
		var info = shuaId.split("~" , -1);
		if(shuaId!=''){
			popWindowAuto('<%=contextPath%>/rm/dm/EqDevOutForm/eqDevOutInfoNew.jsp?mixInfoId='+shuaId+'&oprstate='+mixstate,'1200:780');
		}
	}
	...
	function toDelRecord(){
		...
		if(confirm("是否执行关闭操作?")){
			var sql = "update gms_device_mixinfo_form set opr_state='4' where device_mixinfo_id = '"+curvalue+"' ";
    ...
    function loadDataDetail(shuaId){
		...
		if(shuaId!=null){
			...
			var str = "select pro.project_info_no,mix.device_mixinfo_id,mix.mixinfo_no,pro.project_name,mix.create_date,devapp.device_app_no,devapp.device_app_name,"+
				...
				"left join comm_org_information inorg on mix.in_org_id=inorg.org_id "+
				"left join comm_org_information outorg on mix.out_org_id=outorg.org_id "+
				"where mix.bsflag='0' and mix.device_mixinfo_id='"+shuaId+"'";
			...
			$("#dev_oprstate_desc").val(retObj[0].opr_state_desc);
		//}
		//重新加载当前标签页信息
    ...
    function viewMixedStore(id,state){
		if(state !='1' && state !='9' && state !='4'){
			...
			if(id != ''){
				...
				popWindow('<%=contextPath%>/rm/dm/EqDevOutForm/EqDevOutedInfoList.jsp?devMixId='+id,'1050:680');
			}			
		}			
	}
