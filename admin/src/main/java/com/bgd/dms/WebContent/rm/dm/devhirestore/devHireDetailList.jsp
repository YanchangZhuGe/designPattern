      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td class="ali_cdn_name">入库状态</td>
			    <td class="ali_cdn_input">
			    	<select id="s_store_state_desc" name="s_store_state_desc" class="select_width" >
			    	    <option value="" selected="selected">--请选择--</option>
						<option value="2">全部入库</option>
						<option value="1">入库处理中</option>
						<option value="0">全部未入库</option>
			    	</select>
			    </td>
			    <td class="ali_query">
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_hireapp_id_{device_hireapp_id}' name='device_hireapp_id' ondblclick='toModifyDetail(this)' idinfo='{device_hireapp_id}'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' value='{device_hireapp_id}' id='selectedbox_{device_hireapp_id}' />" >选择</td>
					...
					<td class="bt_info_even" exp="{store_desc}">入库状态</td>
			     </tr> 
			  </table>
			</div>
...
<script type="text/javascript">
	...
	function searchDevData(){
		...
		var v_store_state_desc = document.getElementById("s_store_state_desc").value;
		refreshData(v_device_app_no, v_device_app_name,v_store_state_desc);
	}
	function clearQueryText(){
		document.getElementById("s_device_app_no").value = "";
		document.getElementById("s_device_app_name").value = "";
	}
    function refreshData(v_device_app_no, v_device_app_name,v_store_state_desc){
		var str = "select aa.*,case aa.state when '0' then '未提交' when '1' then '待审批' when '3' then '审批通过' when '4' then '审批不通过' else '' end as state_desc, ";
		    str += "case aa.state_tmp when '0' then '全部未入库' when '1' then '入库处理中' when '2' then '全部入库' else '' end as store_desc ";
			str += "from (select pro.project_name,devapp.device_hireapp_id,devapp.device_hireapp_no,devapp.device_hireapp_name,devapp.project_info_no,";
			...
			str += "case when bb.ruku_state = cc.sum_state then '0' when bb.ruku_state < cc.sum_state then '1' else '2' end as state_tmp ";
			...
			str += "left join (select appdet.device_hireapp_id, count(1) as ruku_state "
			str += "from gms_device_hireapp_detail appdet where appdet.bsflag = '0' ";
            str += "and (appdet.state is null or appdet.state != '1') ";
            str += "group by appdet.device_hireapp_id) bb on bb.device_hireapp_id = devapp.device_hireapp_id ";            
            str += "left join (select appdet.device_hireapp_id,count(1) as sum_state "
    		str += "from gms_device_hireapp_detail appdet where appdet.bsflag = '0' ";
            str += "group by appdet.device_hireapp_id) cc on cc.device_hireapp_id = devapp.device_hireapp_id ";
			str += "where devapp.bsflag = '0' and devapp.org_subjection_id like '<%=orgSubId%>%' and wfmiddle.proc_status='3' ";
		...
		str +=")aa ";
		if(v_store_state_desc!=undefined && v_store_state_desc!=''){
			str += "where aa.state_tmp = '"+v_store_state_desc+"' ";
		}
		str += "order by aa.state asc,aa.appdate desc";
    ...
    function loadDataDetail(device_hireapp_id){
		...
		if(device_hireapp_id!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getDevHireBaseInfo", "devicehireappid="+device_hireapp_id);
		    ...
		    retObj = jcdpCallService("DevCommInfoSrv", "getDevHireBaseInfo", "devicehireappid="+ids);
		}
		//选中这一条checkbox
    	...
    	processNecessaryInfo={        							//流程引擎关键信息
			...
			businessInfo:"外租申请审批流程<外租申请单名称:"+device_hireapp_name+";外租申请单号:"+device_hireapp_no+">",
   ...
   // }   
    ...
    function toDetailPage(){
		...
		window.location.href='<%=contextPath%>/rm/dm/devhirestore/hireAppPassedDetailList.jsp?mixId='+shuaId;
    }
</script>
</html>