...
<%
	...
	if(orgSubjectionId.length()>6){
		sub_id = orgSubjectionId.substring(0,7);
		if(orgSubjectionId.startsWith("C105005")||orgSubjectionId.startsWith("C105001")){
			if(orgSubjectionId.length()>9){
				sub_id = orgSubjectionId.substring(0,10);
			}
		}
	}
			...
			<table id="hourTable" width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="black" class="tab_line_height" style="margin-top: 10px;">
				<tr style="height: 50px;">
					...
					<%
						...
						Map mapLast = BeanFactory.getQueryJdbcDAO().queryRecordBySQL(sqlLast);
						if(list.size()>3){
...
function linkThirdList(sub_id){
	window.location="<%=contextPath%>/hse/objAndTarget/hseWorkHour/workHour_fourthOrg.jsp?sub_id="+sub_id;
}
