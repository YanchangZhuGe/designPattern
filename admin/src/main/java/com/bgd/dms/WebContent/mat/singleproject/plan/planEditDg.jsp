...
//	    	var applyTeamList=jcdpCallService("HumanCommInfoSrv","queryApplyTeam","");	
       ...
       function toAdd(trid){
			...
			var vReturnValue = window.showModalDialog('<%=contextPath%>/mat/singleproject/plan/selectMatList.jsp?ids='+ids+'&submiteNumber='+submiteNumber,obj,'dialogWidth=1024px;dialogHigth=400px');
       ...
       function save(){	
   		...
   		ids = getSelIds('rdo_entity_id');
   			if (ids == '') {
   				...
   				loadDataDetail();
		  		document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/plan/updatePlan.srq?laborId="+ids;
	     		...
	     		if(ctt.frames.length == 0){
	     			...
	     			ctt.frames[1].location.href = '<%=contextPath %>/mat/singleproject/plan/repMatPlan/planItemListDg.jsp' ;
		 ...
		 function getTimes(){
				...
				for(var i=1;i<row.length;i++){
					...
					if(cell_7!=undefined){
							if(cell_7=='&nbsp;'){
								cell_7=0;
								row[i].cells[7].firstChild.value = cell_7*times;
								}
							else{
								row[i].cells[7].firstChild.value = cell_7*times;
								}
