...
<script type="text/javascript">
	...
	var deviceaddappid = '<%=deviceaddappid%>';
	...
	function refreshData(){
		...
		str += "alldet.apply_num,alldet.teamid,alldet.team,alldet.approve_num, ";
		...
		str += "where devapp.project_info_no = '"+projectInfoNos+"' and devapp.bsflag='0' and alldet.bsflag='0' and (alldet.device_addapp_id='"+deviceaddappid+"' or alldet.device_allapp_id ='"+deviceaddappid+"') ";
		...
		if(retObj && retObj.length>0){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td><input id='applynum"+index+"' name='applynum"+index+"' value='"+approve_num+"' size='3' type='text' readonly/></td>";
		...
		str = "select alldet.device_allapp_detid,alldet.project_info_no,aad.device_addapp_name,sd.coding_name as unitname,teamsd.coding_name as teamname, ";
		...
		str += "from gms_device_allapp_colldetail alldet left join gms_device_allapp_add aad on alldet.device_addapp_id=aad.device_addapp_id ";
		str += "left join gms_device_allapp devapp on alldet.device_allapp_id = devapp.device_allapp_id ";
		str += "left join bgp_p6_activity p6 on alldet.teamid = p6.object_id ";
		...
		if(retObj && retObj.length>0){
			for(var index=0;index<retObj.length;index++){
				if(index == 0 ){
					$("#project_name").val(retObj[index].project_name);
					$("#device_allapp_name").val(retObj[index].device_addapp_name);
				}
				...
				var collapply_num = retObj[index].approve_num;
