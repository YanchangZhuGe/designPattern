...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <title>多项目项目-计划审批-调剂设备申请-出库</title> 
 </head> 
      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <auth:ListButton functionId="" css="jl" event="onclick='toModifyDetail()'" title="调剂出库"></auth:ListButton>
			  </tr>
			</table>
			</td>
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_appdet_id_{device_allapp_id}' name='device_allapp_id' idinfo='{device_allapp_id}'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' value='{device_allapp_id}' id='selectedbox_{device_allapp_id}' />" >选择</td>
					...
					<td class="bt_info_even" exp="{assign_desc}">状态</td>
					<td class="bt_info_odd" exp="<a onclick=viewStore('{device_allapp_id}','{assign_state}')>查看</a>">处理进度</td>					
			     </tr> 
			     <tr>
				     <td  class="inquire_item6">处理进度：</td>
...
<script type="text/javascript">
    ...
    function refreshData(v_project_name,v_org_name,v_assign_state){
		...
		var str = "select pro.project_name,devapp.device_allapp_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.project_info_no,";
			...
			str += "case devapp.assign_state when '0' then '未处理' when '1' then '已处理' else '未处理' end as assign_desc,";
			...
			str += "and devapp.allapp_type = 'S9998' ";
		...
		str +=" order by devapp.assign_state nulls first,devapp.modifi_date desc "
			
		cruConfig.queryStr = str;
		queryData(cruConfig.currentPage);;
	}

	 function viewStore(id,state){
		if(state=='0' || state==''){
			alert("该计划没有指定调剂单位,暂不能查看详细调剂信息!");
		}else{
			var str = "select device_mixapp_id from gms_device_dismixapp where device_app_id = '"+id+"' and bsflag = '0' ";
			var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
			var devicemixappid = unitRet.datas[0].device_mixapp_id;
		
			if(devicemixappid != '')
				popWindow('<%=contextPath%>/rm/dm/devPlanMulti/outMixDevStoreView_dg.jsp?devMixAppId='+devicemixappid,'900:680');
		}			
	}
	...
	function toModifyDetail(obj){
		var v_assign_desc = $("#assign_desc").val();
		...
		popWindow('<%=contextPath%>/rm/dm/devPlanMulti/mixDevOutStoreDet_dg.jsp?deviceallappid='+idinfo,'900:680');
	}
	...
	function dbclickRow(shuaId){
		...
		popWindow('<%=contextPath%>/rm/dm/devPlanMulti/mixDevOutStoreDet_dg.jsp?deviceallappid='+shuaId,'900:680');
	}
