...
function toAdd(){
	...
	tr.insertCell().innerHTML = '<input type="text" id="line_id_'+rowNum+'" size="5" value=""/>';//线束号
...
function initData(){
	...
	if(retObj.datas != null){
		for(var i=1;i<=retObj.datas.length;i++){
			...
			tr.insertCell().innerHTML = '<input type="text" id="line_id_'+record.order_num+'" size="5" value="'+record.line_id+'"/>';//线束号
...
function toSave(){
	...
	for(var i=1;i<rowNum;i++){
		...
		var line_id = document.getElementById("line_id_"+i);//线束号
		if(line_id!=null){
...
function exportExcel(){
	...
	var fromPage = "sh2dlineConsruction";
...
function downloadTemplate(){
	window.location.href="<%=contextPath%>/pm/dailyReport/singleProject/download.jsp?path=/pm/lineConstruction/sh2dlineConsruction.xls&filename=sh2dlineConsruction.xls";
}
...
function importData(){
	...
	if(checkFile(filename)){
		document.getElementById("fileForm").action = "<%=contextPath%>/pm/gpe/importLineConstruction.srq?workMethod=2&isSea=y&teamName="+team_name;
