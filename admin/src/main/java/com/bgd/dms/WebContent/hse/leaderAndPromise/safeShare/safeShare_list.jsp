      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td class="ali_cdn_input"><input id="safeName" name="safeName" type="text" /></td>
			    <td class="ali_query">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{hse_safeshare_id}' id='rdo_entity_id_{hse_safeshare_id}' onclick='loadDataDetail();'/>" >选择</td>
			      ...
			      <td class="bt_info_even" exp="{safe_name}">标题</td>
			      <td class="bt_info_odd" exp="{coding_name}">安全经验类别</td>
			      <td class="bt_info_even" exp="{safe_date}">发表日期</td>
			    </tr>
			  </table>
			</div>
			...
			<input type="hidden" id="hse_safeshare_id" name="hse_safeshare_id"></input>
				...
				<div id="tab_box_content0" class="tab_box_content">
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
						 <tr>
							...
							<td class="inquire_form6" colspan="5"><input type="text" id="safe_name" name="safe_name" class="input_width" value=""></input></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td class="inquire_form6"><input type="text" id="safe_date" name="safe_date" class="input_width" value="" readonly="readonly"/>
						...
						<tr>
							...
							<td class="inquire_form6"><input type="text" id="share_person" name="share_person" value="" class="input_width"></input></td>
							<td class="inquire_item6">分享场合：</td>
							<td class="inquire_form6"><input type="text" id="share_occasion" name="share_occasion" value="" class="input_width"></input></td>
							<td class="inquire_item6"><font color="red">*</font>上传人：</td>
							<td class="inquire_form6"><input type="text" id="upload_person" name="upload_person" value="" class="input_width"></input></td>
						</tr>	
						<tr>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.queryStr = "select t.hse_safeshare_id,t.second_org,t.third_org,t.safe_name,safe_date,d.coding_name,oi1.org_abbreviation as second_org_name,oi2.org_abbreviation as third_org_name,oi3.org_abbreviation as fourth_org_name,t.create_date,t.modifi_date from bgp_hse_safeshare t left join comm_org_subjection os1 on t.second_org = os1.org_subjection_id and os1.bsflag = '0' left join comm_org_information oi1 on oi1.org_id = os1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on t.third_org = os2.org_subjection_id and os2.bsflag = '0' left join comm_org_information oi2 on oi2.org_id = os2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on t.fourth_org = os3.org_subjection_id and os3.bsflag = '0' left join comm_org_information oi3 on oi3.org_id = os3.org_id and oi3.bsflag = '0' left join comm_coding_sort_detail d on d.coding_code = t.safe_type and d.bsflag='0' where t.bsflag = '0' "+querySqlAdd+" order by modifi_date desc";
		cruConfig.currentPageUrl = "/hse/leaderAndPromise/safeShare/safeShare_list.jsp";
		queryData(1);
	}
	...
	function simpleSearch(){
			var safeName = document.getElementById("safeName").value;
			var isProject = "<%=isProject%>";
			var querySqlAdd = "";
			if(isProject=="1"){
				querySqlAdd = getMultipleSql();
			...
			if(safeName!=''&&safeName!=null){
				cruConfig.cdtType = 'form';
				cruConfig.queryStr = "select t.hse_safeshare_id,t.second_org,t.third_org,t.safe_name,safe_date,d.coding_name,oi1.org_abbreviation as second_org_name,oi2.org_abbreviation as third_org_name,oi3.org_abbreviation as fourth_org_name,t.create_date,t.modifi_date from bgp_hse_safeshare t left join comm_org_subjection os1 on t.second_org = os1.org_subjection_id and os1.bsflag = '0' left join comm_org_information oi1 on oi1.org_id = os1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on t.third_org = os2.org_subjection_id and os2.bsflag = '0' left join comm_org_information oi2 on oi2.org_id = os2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on t.fourth_org = os3.org_subjection_id and os3.bsflag = '0' left join comm_org_information oi3 on oi3.org_id = os3.org_id and oi3.bsflag = '0' left join comm_coding_sort_detail d on d.coding_code = t.safe_type and d.bsflag='0'  where t.bsflag = '0' "+querySqlAdd+" and t.safe_name like '%"+safeName+"%' order by modifi_date desc";
				cruConfig.currentPageUrl = "/hse/leaderAndPromise/safeShare/safeShare_list.jsp";
				queryData(1);
			}else{
				refreshData();
	...
	function loadDataDetail(shuaId){
		var retObj;
		if(shuaId!=null){
			 retObj = jcdpCallService("HseSrv", "querySafeShare", "hse_safeshare_id="+shuaId);
		    ...
		    retObj = jcdpCallService("HseSrv", "querySafeShare", "hse_safeshare_id="+ids);
		    document.getElementById("attachement").src = "<%=contextPath%>/hse/docCommon/hseCommDocList.jsp?relation_id="+ids;
		}
		document.getElementById("hse_safeshare_id").value =retObj.map.hseSafeshareId;
		...
		document.getElementById("safe_name").value =retObj.map.safeName;
		document.getElementById("safe_type").value = retObj.map.safeType;
		document.getElementById("safe_date").value = retObj.map.safeDate;
		document.getElementById("info_type").value = retObj.map.infoType;
		document.getElementById("upload_person").value = retObj.map.uploadPerson;
		document.getElementById("share_person").value = retObj.map.sharePerson;
		document.getElementById("share_occasion").value = retObj.map.shareOccasion;
	...
	function toAdd(){
		popWindow("<%=contextPath%>/hse/leaderAndPromise/safeShare/addSafeShare.jsp?isProject=<%=isProject%>");
		
	}
	
	function toEdit(){  
		var hse_safeshare_id = document.getElementById("hse_safeshare_id").value;
	  	if(hse_safeshare_id==''||hse_safeshare_id==null){  
	  		alert("请选择一条信息!");  
	  		return;  
	  	}  
	  	popWindow("<%=contextPath%>/hse/safe/querySafeShare.srq?isProject=<%=isProject%>&hse_safeshare_id="+hse_safeshare_id);
	} 
	...
	function toUpdate(){  
		...
		form.action="<%=contextPath%>/hse/safe/saveSafeShare.srq";
		form.submit();
	} 
	...
	function toDelete(){
		...
		if(confirm('确定要删除吗?')){  
			var retObj = jcdpCallService("HseSrv", "deleteSafeShare", "hse_safeshare_id="+ids);
			queryData(cruConfig.currentPage);
		}
	}
	
	function toUploadFile(){
		popWindow("<%=contextPath%>/hse/leaderAndPromise/safeShare/importFile.jsp?project=<%=isProject%>");
     }

	function toSearch(){
		popWindow("<%=contextPath%>/hse/leaderAndPromise/safeShare/safeShare_search.jsp?isProject=<%=isProject%>");
	}
