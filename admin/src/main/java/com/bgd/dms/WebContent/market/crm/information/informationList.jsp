...
<body style="background:#fff" >
	<div id="inq_tool_box">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td class="ali_cdn_name" align="right">类别:</td>
							<td class="ali_cdn_input" align="left"><select id="type" name="type" onchange="changeType();" class="select_width">
	...
	<div id="tab_box" class="tab_box" style="overflow:hidden;">
		<div id="tab_box_content1" class="tab_box_content" >
			...
			<table width="100%"  border="0" cellspacing="0" cellpadding="0" class="tab_line_height" id="team">
	    		<tr>
			    	<td class="inquire_item4">队伍数量:</td>
			    	<td class="inquire_form4"><input name="team_num" id="team_num" type="text" class="input_width" value="" /></td>
			    	<td class="inquire_item4">采集设备道数:</td>
			    	<td class="inquire_form4"><input name="equipment" id="equipment" type="text" class="input_width" value="" /></td>
			    </tr>
			    <tr>
			    	<td class="inquire_item4">仪器型号:</td>
			    	<td class="inquire_form4"><input name="equipment_mode" id="equipment_mode" type="text" class="input_width" value="" /></td>
			    </tr>
			</table>
		</div>
		<div id="tab_box_content2" class="tab_box_content" style="display:none;">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr align="right" >
				  	<td>&nbsp;</td>
				    <auth:ListButton functionId="" css="tj" event="onclick='saveKey()'" title="JCDP_btn_submit"></auth:ListButton>
			  	</tr>
			</table>
			<table  width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
			    <tr>
		...
		<div id="tab_box_content5" class="tab_box_content" style="display:none;">
			<iframe width="100%" height="100%" name="workload" id="workload" frameborder="0" src="" marginheight="0" marginwidth="0" ></iframe>
		</div>
	</div>
<script type="text/javascript">
	...
	function refreshData(typeId ,typeName ,divisory_type){
		if(typeId==null || typeId =='' || typeId =='null'){
			typeId = type_id ;
			typeName = type_name;
			divisory_type = divisory;
		}
		type_id = typeId;
		type_name = typeName;
		divisory = divisory_type;
		...
		var company_type = document.getElementById("type").value;
		if(company_type==null || company_type==''){
			company_type ='1';
		}
		if(company_type !=null && company_type=='2'){
			document.getElementById("year_workload").style.display = 'block';
			document.getElementById("team").style.display = 'block';
		}else{
			document.getElementById("year_workload").style.display = 'none';
		...
		if(typeId==null || typeId==''){
	    	...
	    	" left join bgp_market_company_type t1 on t.company_place = t1.type_id and t1.bsflag='0' left join bgp_market_company_type t2 on t.parent_company_id = t2.type_id and t2.bsflag='0' where t.bsflag='0' and t.company_type ='"+
	    	company_type+"' order by t.create_date desc";
		}else{
			cruConfig.queryStr = "select t.company_id ,t.company_name ,t.company_short_name ,t.company_place ,t1.type_short_name company_place_name ,t.parent_company_id ,t2.type_short_name parent_company_name ,t.company_stork ,"+
			" t.key_name ,t.key_position ,t.key_telephone ,t.key_email ,c1.contract_num ,c2.contact_num ,c3.bid_total ,c4.bid_num ,c4.bid_num/c3.bid_total bid_pecent from bgp_market_oil_company t " +
			" left join (select distinct c.company_id ,count(c.company_id) contract_num "+
			...
			" where bid.bsflag='0' and bid.eva_result ='5' group by bid.owner) c4 on t.company_id = c4.owner"+
			" left join bgp_market_company_type t1 on t.company_place = t1.type_id and t1.bsflag='0' left join bgp_market_company_type t2 on t.parent_company_id = t2.type_id and t2.bsflag='0' where t.bsflag='0' and t.company_type ='"+
			company_type+"' and (t.company_place ='"+typeId+"' or t.parent_company_id ='"+typeId+"') order by t.create_date desc ";
		}
	...
	function loadDataDetail(id){
		...
		" t.key_name ,t.key_position ,t.key_telephone ,t.key_email ,t.company_type ,t.team_num ,t.equipment ,t.equipment_mode from bgp_market_oil_company t " +
		...
		if(retObj.returnCode=='0'){
			if(retObj.datas[0] != null){
				...
				document.getElementById("team_num").value =datas.team_num;
				document.getElementById("equipment").value =datas.equipment;
				document.getElementById("equipment_mode").value =datas.equipment_mode;
    	...
    	document.getElementById("contract").src = "<%=contextPath%>/market/crm/information/contractList.jsp?company_id="+id+"&company_name="+company_name;
    	document.getElementById("contact").src = "<%=contextPath%>/market/crm/information/contactList.jsp?company_id="+id+"&company_name="+company_name;
    	document.getElementById("workload").src = "<%=contextPath%>/market/crm/information/workloadList.jsp?company_id="+id+"&company_name="+company_name;
	}
	/* 修改 */
	function newSubmit() {
		if(checkValue() == false){
		...
		if(company_id!=null && company_id!=''){
			var substr = "update bgp_market_oil_company t set t.company_id ='"+company_id+"' ,t.updator_id='"+user_id+"' ,t.modifi_date = sysdate ," +
			" t.company_name ='"+company_name+"', t.company_short_name ='"+company_short_name+"' ,t.company_place ='"+company_place+"',"+
			" t.company_type='"+company_type+"' ,t.parent_company_id ='"+parent_company_id+"' ,t.company_stork ='"+company_stork+"' "+
			...
			if(substr!=''){
				...
				if(retObj.returnCode =='0'){
					...
					ctt.frames[1].refresh(type_id,type_name);
					parent.mainTopframe.refreshTree(type_id);
				}
			}
		}
	...
	function checkValue(){
		...
		obj = document.getElementById("company_place") ;
		value = obj.value ;
		if(obj ==null || value==''){
			alert("所在国家(地区)不能为空!");
			return false;
	...
	function toAdd() { 
		...
		popWindow("<%=contextPath%>/market/crm/information/informationEdit.jsp?type_id="+type_id+"&type_name="+type_name+"&divisory_type="+divisory_type);
	}
	function toEdit() {
		var company_id = document.getElementById("company_id").value;
      	var text = '你确定要修改?';
		if(company_id!=null && window.confirm(text)){
			...
			var type_name = document.getElementById("name").innerHTML;
			var divisory_type = document.getElementById("divisory_type").value;
			popWindow("<%=contextPath%>/market/crm/information/informationEdit.jsp?company_id="+company_id+"&type_id="+type_id+"&type_name="+type_name+"&divisory_type="+divisory_type);
			return;
		}
		alert("请选择修改的记录!");
	}
