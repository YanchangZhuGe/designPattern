...
<script type="text/javascript">
	...
	function refreshData(taskId,projectInfoNo){
		...
		cruConfig.queryStr = "select d.plan_detail_id,d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,nvl(d.people_number,0) people_number,nvl(d.profess_number,0) profess_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1 ) nums from bgp_comm_human_plan_detail d  left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0'  and s1.coding_mnemonic_id='<%=projectType%>' left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' and s2.coding_mnemonic_id='<%=projectType%>'  where d.task_id='"+taskId+"' and d.spare1 is null  and d.project_info_no='"+projectInfoNo+"' and d.bsflag='0' ";
		cruConfig.currentPageUrl = "/rm/em/singleHuman/humanPlan/taskPlanList.jsp";
		queryData(1);
	...
	function toAdd(){
		...
		popWindow('<%=contextPath%>/rm/em/singleHuman/humanPlan/add_taskPlanModify.jsp?taskId=<%=taskObjectId%>&projectInfoNo=<%=projectInfoNo%>','1000:800');
	}
	...
	//	ids = getSelIds('rdo_entity_id');
		...
		popWindow('<%=contextPath%>/rm/em/singleHuman/humanPlan/add_taskPlanModify.jsp?taskId=<%=taskObjectId%>&projectInfoNo=<%=projectInfoNo%>&id='+ids,'1000:800');
	}
