      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{hse_plan_id},{flag}' {disasss}  id='rdo_entity_id_{hse_plan_id}' onclick='loadDataDetail();'/>" >选择</td>
			      ...
			      <td class="bt_info_odd" exp="{lead_type}">是否领导宣贯</td>
...
function noEdit(event){
	...
	function simpleSearch(){
			...
			if(projectName!=''&&projectName!=null){
				cruConfig.cdtType = 'form';
				cruConfig.queryStr = "select * from ((select tp.hse_plan_id,decode(tp.lead_type,'1','是','0','否') lead_type,tp.train_object,tp.train_address,oi.org_abbreviation second_org_name,oi1.org_abbreviation third_org_name,tp.project_name,tp.train_time,tp.second_org,tp.third_org,tp.bsflag,tp.modifi_date,1 flag from bgp_hse_train_plan tp left join comm_org_subjection os on tp.second_org=os.org_subjection_id and os.bsflag='0' left join comm_org_information oi on os.org_id=oi.org_id and oi.bsflag='0' left join comm_org_subjection os1 on tp.third_org=os1.org_subjection_id and os1.bsflag='0' left join comm_org_information oi1 on os1.org_id=oi1.org_id and oi1.bsflag='0' where tp.bsflag='0') union all (select pn.train_plan_no,'否' lead_type,pn.train_object,train_address,oi1.org_abbreviation second_org_name,i.org_abbreviation,p.project_name,pn.train_cycle,os1.org_subjection_id second_org,os.org_subjection_id third_org,dl.bsflag,dl.modifi_date,2 flag  from BGP_COMM_HUMAN_TRAINING_DETAIL dl  inner join BGP_COMM_HUMAN_TRAINING_PLAN pn on dl.train_plan_no=pn.train_plan_no  and pn.bsflag='0'left join gp_task_project p on pn.project_info_no = p.project_info_no and p.bsflag = '0' left join comm_org_information i on pn.spare1 = i.org_id and i.bsflag = '0' left join comm_org_subjection os on os.org_id = i.org_id and os.bsflag='0' left join comm_org_subjection os1 on os.father_org_id=os1.org_subjection_id and os1.bsflag='0' left join comm_org_information oi1 on oi1.org_id=os1.org_id and oi1.bsflag='0'  where dl.bsflag='0' and dl.classification='2' )) t where t.bsflag='0' "+sql+" and t.lead_type ='是' and t.project_name like '%"+projectName+"%' order by t.modifi_date desc";
				cruConfig.currentPageUrl = "/hse/accidentNews/accident_list.jsp";
				queryData(1);
			}else{
				refreshData();
	...
	function loadDataDetail(shuaId){
		...
		if(shuaId!=null){
			 ...
			 retObj = jcdpCallService("HseSrv", "viewHseTraining", "hse_plan_id="+id);
			 document.getElementById("attachement").src = "<%=contextPath%>/hse/docCommon/hseCommDocList.jsp?relation_id="+id;
			 document.getElementById("remark").src = "<%=contextPath%>/common/remark/relatedOperation.srq?foreignKeyId="+id;
	...
	function toSearch(){
		popWindow("<%=contextPath%>/hse/orgAndResource/hseTrainingMultiple/training_search.jsp?lead_type=1");
	}
