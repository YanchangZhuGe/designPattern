...
<%
	...
	String projectTypeView = user.getProjectType();	
      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr> 
			    ...
			    <td>&nbsp;</td>
			    <%if(!"5000100004000000009".equals(projectTypeView)){ %>
				...
				<%} else {%>
			    	...
			    	<auth:ListButton functionId="" css="xg" event="onclick='toEditZh()'" title="JCDP_btn_edit"></auth:ListButton>
				  <%}%>
			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      <td class="bt_info_even" exp="<input type='checkbox' name='rdo_entity_id' value='{project_info_no}-{plan_id}-{proc_status}-{mid}-{project_name}' id='rdo_entity_id_{project_info_no}' onclick='chooseOne(this);doCheck(this);' />" >选择</td>
			      <td class="bt_info_odd" autoOrder="1">序号</td>
			      <td class="bt_info_even"  exp="<a href='#' onclick=view_page('{plan_id}')><font color='blue'>{plan_no}</font></a>" >单号</td>
			      
			      <td class="bt_info_odd" exp="{spare3}">申请理由</td>
			      <td class="bt_info_even"  exp="{employee_name}">提交人</td>
			      <td class="bt_info_odd" exp="{apply_date}">提交日期</td>
				...
				<div id="tag-container_3">
				  <ul id="tags" class="tags">
				    ...
				    <li id="tag3_1" ><a href="#" onclick="getTab3(1)">岗位需求</a></li>
					  <%if(!"5000100004000000008".equals(projectTypeView)){ %>
...
<script type="text/javascript">
	...
	var projectTypeView ="<%=projectTypeView%>";
//	cruConfig.funcCode = "F_HUMAN_003";
	...
	function view_page(plan_id){		
		if(projectTypeView =='5000100004000000009'){  //综合 
			if(plan_id != ""){
				popWindow("<%=contextPath%>/rm/em/singleHuman/supplyHumanPlan/sz_add_planning.jsp?splan_id="+plan_id,"1000:800");
			}
		}else{
			if(plan_id != ""){
				window.location= "<%=contextPath%>/rm/em/singleHuman/supplyHumanPlan/humanRequestFrame.jsp?show_p=0&splan_id="+plan_id;
			}
		}
	}
	var businessType="";
	//生产管理项目补充配置计划 流程编码
	var retObj = jcdpCallService("WtProjectSrv", "getProjectResourcesInfo","projectInfoNo=<%=project_Id%>");
	if(retObj.map!=null){
	...
	function toAdd(){
		  popWindow("<%=contextPath%>/rm/em/singleHuman/supplyHumanPlan/addMainPage.jsp");
		//var planId = jcdpCallService("HumanCommInfoSrv","submitSupplyHumanPlan","projectInfoNo=<%=project_Id%>");	
 
		//window.location= "<%=contextPath%>/rm/em/singleHuman/supplyHumanPlan/humanRequestFrame.jsp?splan_id="+planId.planId;
	...
	function toEdit(){
		...
		window.location= "<%=contextPath%>/rm/em/singleHuman/supplyHumanPlan/humanRequestFrame.jsp?splan_id="+ids.split("-")[1];
	...
	function toEditZh(){
		...
		popWindow("<%=contextPath%>/rm/em/singleHuman/supplyHumanPlan/sz_add_planning.jsp?splan_id="+ids.split("-")[1],"1000:800");
...
function submitProcessInfo(){	
    ...
    function loadDataDetail(ids){
    	//{project_info_no}-{plan_id}-{proc_status}-{mid}-{project_name
		if(projectTypeView =='5000100004000000009'){  //综合 
			processNecessaryInfo={        //流程引擎关键信息
					businessTableName:"gp_middle_resources",    //置入流程管控的业务表的主表表明
					businessType:businessType,        //业务类型 即为之前设置的业务大类 5110000004100000096
					businessId:ids.split("-")[3],           // mid
	    	...
	    	var querySql = "select rownum,p.* from (select p.apply_team_name, p.post_name,p.apply_team, p.post,sum(nvl(p.people_number,0)) people_number , sum(nvl(p.profess_number,0)) profess_number,min(p.plan_start_date) plan_start_date, max(p.plan_end_date) plan_end_date,(max(p.plan_end_date)-min(p.plan_start_date) + 1 ) nums from ( select d.plan_detail_id,d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,d.people_number,d.profess_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1) nums from bgp_comm_human_plan_detail d  left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0'  and s1.coding_mnemonic_id='<%=projectType%>'  left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' and s2.coding_mnemonic_id='<%=projectType%>'   where d.spare1='"+ids.split("-")[1]+"'  and d.project_info_no='<%=project_Id%>'  and d.bsflag='0' ) p group by p.apply_team_name,p.post_name ,p.apply_team, p.post  order by p.apply_team_name asc  ) p ";
			var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=10000&querySql='+querySql);
			var datas = queryRet.datas;
			
			var querySql1 = "select rownum,p.* from (select d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,nvl(d.people_number,0) people_number,nvl(d.profess_number,0) profess_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1) nums,a.name,a.planned_start_date,a.planned_finish_date,a.planned_duration from bgp_comm_human_plan_detail d  left join bgp_p6_activity a on d.task_id=a.object_id || '' and a.bsflag='0' left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0'  and s1.coding_mnemonic_id='<%=projectType%>'  left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' and s2.coding_mnemonic_id='<%=projectType%>'   where d.spare1='"+ids.split("-")[1]+"' and d.project_info_no='<%=project_Id%>'  and d.bsflag='0' ) p ";
...
//		}
		...
		var sql = "update bgp_comm_human_plan t set t.bsflag='1' ,t.spare1='1'  where t.plan_id ='"+ids.split("-")[1]+"' ";
		var path = cruConfig.contextPath+"/rad/asyncDelete.srq";
		var params = "deleteSql="+sql;
		params += "&ids="+ids;
		var retObject = syncRequest('Post',path,params);
		//把根据plan_id主键把子表信息也删除  
		var querySql1 = " select dl.plan_detail_id,dl.spare1 ,dl. bsflag  from  bgp_comm_human_plan_detail  dl left join  bgp_comm_human_plan bp on dl.spare1=bp.plan_id   where dl.spare1='"+ids.split("-")[1]+"' and dl.bsflag='0' ";
		var queryRet1 = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=10000&querySql='+querySql1);
		var datas1 = queryRet1.datas;
		if(datas1 != null){	 
		     for (var i = 0; i< queryRet1.datas.length; i++) {    
 				var paths = '<%=request.getContextPath()%>/rad/addOrUpdateEntity.srq';	       	
   				var submitStr = 'JCDP_TABLE_NAME=BGP_COMM_HUMAN_PLAN_DETAIL&JCDP_TABLE_ID='+datas1[i].plan_detail_id +'&bsflag=1&modifi_date=<%=appDate%>';
