...
<%
	...
	List<BgpProjectHumanReliefDeta> subBeanList=new ArrayList<BgpProjectHumanReliefDeta>(0);
	if(subList!=null){
		subBeanList = new ArrayList<BgpProjectHumanReliefDeta>(subList.size());	
		for (int i = 0; i < subList.size(); i++) {
			subBeanList.add((BgpProjectHumanReliefDeta) subList.get(i).toPojo(
					BgpProjectHumanReliefDeta.class));
		}
	}
...
function save(){
	if(checkForm()){
		...
		form.action = "<%=contextPath%>/rm/em/toSaveReliefRequestPrepare.srq";
		form.submit();
		newClose();
	}
}
...
function openSearch(rowid){
	...
	popWindow('<%=contextPath%>/rm/em/professPrepare/searchHumanList.jsp?project_type=<%=project_type%>&rowid='+rowid,'800:700'); 		
...
function addEmployee(applyTeam,applyTeamname,post,postname,employid,employname,planStartDate,planEndDate){
	...
	tr.insertCell().innerHTML = '<input type="hidden" readonly="readonly" maxlength="32" id="hu'+deviceCount+'team" name="hu'+deviceCount+'team" value="'+applyTeam+'" class="input_width" />'+'<input type="text" readonly="readonly" maxlength="32" id="hu'+deviceCount+'teamname" name="hu'+deviceCount+'teamname" value="'+applyTeamname+'" class="input_width" />';
...
<div> 			
	<table id="lineTable" width="99%" border="0" cellspacing="0" cellpadding="0"  style="margin-top:2px;" >
    	<tr>
            ...
            <td class="bt_info_even" width="6%">备注<input type="hidden"
	  ...
	  %>
		...
		<td class="<%=className%>even"><input type="text" id="fy<%=i%>peopleNum" name="fy<%=i%>peopleNum" value="<%=applyDetailInfo.getPeopleNumber()==null?"":applyDetailInfo.getPeopleNumber()%>" class="input_width" readonly="readonly"/></td>
		...
		<td class="<%=className%>even">
			<input type="text" maxlength="32" id="fy<%=i%>notes" name="fy<%=i%>notes"  value="<%=applyDetailInfo.getNotes()==null?"":applyDetailInfo.getNotes()%>" class="input_width" />			
		</td>	
	...
	<div <% if(update != null){ %>  style="display:none" <%} %>> 			
	<table width="99%" border="0" cellspacing="0" cellpadding="0" style="margin-top:2px;">
		...
		for (int i = 0; i < subBeanList.size(); i++) {
			...
			BgpProjectHumanReliefDeta applyDetailInfo = subBeanList.get(i);
			String flag = "false";
		%>	
		...
		<%if(subOrgIDofAffordOrg!=null &&applyDetailInfo.getDeploySubId().startsWith(subOrgIDofAffordOrg)){
			flag = "true";
		%><a href="javascript:openSearch('em<%=i%>')">选择</a><%
		 }else{%>
				 <a href="javascript:openSearch('em<%=i%>')"
					disabled="disabled" onclick="return false;">选择</a>
		 <%} %>
			    <input type="hidden" name="em<%=i%>flag" id="em<%=i%>flag"
					value="<%=flag%>" />
					</td>
		</tr>	
		<%
	...
	<table width="99%" border="0" cellspacing="0" cellpadding="0" style="margin-top:2px;" id="employeeTable">
    	<tr class="bt_info">
            ...
            <td width="3%">操作<input type="hidden"
					...
					type="hidden" id="deleteRowFlag" name="deleteRowFlag" value="" /></td>
        </tr>       
   <%
	...
	%>
			<tr class="<%=className%>" id="hu<%=j%>trflag">
				...
				<td><input type="hidden" maxlength="32" name="hu<%=j%>team"
					id="hu<%=j%>team"
				...
				<td><input type="text" name="hu<%=j%>planStartDate"
					...
					width="16" height="16" style="cursor: hand;"
					onmouseover="dateSelector('hu<%=j%>planStartDate',tributton2<%=j%>);" />
				</td>
				<td><input type="text" name="hu<%=j%>planEndDate"
					...
					width="16" height="16" style="cursor: hand;"
					onmouseover="dateSelector('hu<%=j%>planEndDate',tributton3<%=j%>);"  />
				</td>
