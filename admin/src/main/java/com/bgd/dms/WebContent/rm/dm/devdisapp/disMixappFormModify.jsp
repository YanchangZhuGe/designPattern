...
<%
	...
	String desmixid = request.getParameter("disMixId");
  ...
  <div id="new_table_box_content" style="width:100%">
    <div id="new_table_box_bg" style="width:95%">
      ...
      <table border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          <td class="inquire_item4" >调配计划单号:</td>
          ...
          <td class="inquire_form4" >
          	...
          	<input name="device_mixapp_id" id="device_mixapp_id" type="hidden" value="<%=desmixid%>" />
		  ...
		  <div style="height:90px;overflow:auto">
			  <table width="95%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
					...
					<td class="bt_info_even" width="4%">调剂计划数量</td>
					<td class="bt_info_odd" width="4%">已申请数量</td>
					<td class="bt_info_even" width="4%">申请数量</td>
...
<script type="text/javascript"> 
	...
	function checkAssignNum(obj){
		...
		var mixednumVal = parseInt($("#checkmixappednum"+index).val(),10);
	...
	function refreshData(){
		...
		if('<%=devappid%>'!=null){
			//调剂计划基本信息
			var str = "select device_app_no,devplan.project_info_no,devplan.mix_type_id,";
			str += "tp.project_name,outorg.org_abbreviation as out_org_name,mif.device_mixapp_no,mif.mix_org_id ";
			str += "from gms_device_disapp devplan join gms_device_dismixapp mif on mif.device_app_id=devplan.device_app_id and mif.bsflag='0' "
			+"join gp_task_project tp on devplan.project_info_no=tp.project_info_no " 
			+"join comm_org_information outorg on outorg.org_id=mif.mix_org_id " 
			+"where devplan.device_app_id='<%=devappid%>' and mif.device_mixapp_id = '<%=desmixid%>'";
			...
			if(retObj!=undefined && retObj.length>0){
				...
				//$("#app_date").val(retObj[0].app_date);
			...
			var str = "select appdet.device_app_detid,appdet.project_info_no,sd.coding_name as unitname,teamsd.coding_name as teamname, ";
				str += "appdet.unitinfo,appdet.dev_ci_code,appdet.isdevicecode, ";
				...
				str += "appdet.apply_num,appdet.teamid,appdet.team,nvl(tmp.mixappednum,0) as mixappednum, ";
				str += "appdet.purpose,appdet.employee_id,mixdt.apply_num as mixappnum,";
				str += "appdet.plan_start_date,appdet.plan_end_date ";
				str += "from gms_device_disapp_detail appdet ";
				str += "left join gms_device_dismixapp_detail mixdt on mixdt.device_app_detid = appdet.device_app_detid ";
				...
				str += "left join (select device_app_detid,sum(apply_num) as mixappednum from gms_device_dismixapp_detail mixdt ";
				str += "where exists(select 1 from gms_device_dismixapp mif where mif.bsflag='0' and mif.device_mixapp_id=mixdt.device_mixapp_id) ";
				str += "group by device_app_detid) tmp ";
				str += "on tmp.device_app_detid=appdet.device_app_detid "
				str += "where appdet.device_app_id='<%=devappid%>' and appdet.bsflag='0' and mixdt.device_mixapp_id='<%=desmixid%>'";
				str += "order by appdet.dev_ci_code ";
			var proqueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
			retObj = proqueryRet.datas;
		...
		if(retObj!=undefined){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td><input type='checkbox' name='detinfo' id='"+index+"' value='"+retObj[index].device_app_detid+"' checked/></td>";
				...
				innerhtml += "<td><input name='mixappednum"+index+"' id='mixappednum"+index+"' value='"+retObj[index].mixappednum+"' size='6'  type='text' readonly/>";
				innerhtml += "<input name='checkmixappednum"+index+"' id='checkmixappednum"+index+"' value='"+(parseInt(retObj[index].mixappednum)-parseInt(retObj[index].mixappnum))+"' type='hidden' /></td>";
				innerhtml += "<td><input name='mixappnum"+index+"' id='mixappnum"+index+"' detindex='"+index+"' value='"+retObj[index].mixappnum+"' onkeyup='checkAssignNum(this)' size='6' type='text' />";
