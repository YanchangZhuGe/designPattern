...
<%
	...
	String mix_num = request.getParameter("mixnum");
	String check_num = request.getParameter("checknum");
%>
...
<script type="text/javascript">
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model,v_dev_sign,v_self_num,v_license_num,v_dev_type){
		var str = "select tmp1.dev_acc_id,asset_coding,dev_coding,self_num,dev_sign,license_num,dev_name,dev_model,dev_type,org_abbreviation,";
		str += "case when (select count(1) from gms_device_account tmp2 ";
        str += "where tmp2.dev_acc_id in ("+obj.pageselectedstr+") and tmp2.dev_acc_id = tmp1.dev_acc_id ";
        str += "group by tmp2.dev_acc_id)>=1 then 'disabled' else '' end as selectflag ";
		str += "from ";
		str += "(select account.dev_acc_id,account.asset_coding,account.dev_coding,account.self_num,";
		str += "account.dev_sign,account.license_num,account.dev_name,account.dev_model,account.dev_type,org.org_abbreviation ";
		str += "from gms_device_account account "
		str += "left join comm_org_information org on account.owning_org_id=org.org_id "
		var str= "select * from gms_device_account account ";
		
// 		if(isdevicecode=='N'){
// 			str += "where (search_id is null ";
<%-- 			if('<%=devicemixinfoid%>'!=''){ --%>
...
// 		}
	...
	function submitInfo(){
		...
		if((checknum == 0) && (count>mixnum) ){
           	alert("超出了调配数量 "+mixnum+" ,请重新选择！");
            return;
		}else if(count>(mixnum-checknum)){
           	alert("超出了剩余调配数量 "+(mixnum-checknum)+" ,请重新选择！");
            return;
		}
