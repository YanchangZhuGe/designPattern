...
<%
	...
	String wtcOrgId=request.getParameter("orgid");
	if(null==wtcOrgId){
		wtcOrgId="";
	}
...
$(function(){
	...
	$("#main_grid").datagrid({ 
		...
		url:"${pageContext.request.contextPath}/rm/dm/getListDataBySrvAndMethod.srq?JCDP_SRV_NAME=AssessPlatInfoSrv&JCDP_OP_NAME=queryProjectMonitorStateInfo"
	});
	$(".datagrid-header-rownumber").html('序号');//datagrid 添加序列
});
//显示红绿灯
function showState(value,row,index){
	var projectinfono = row.projectinfono;
	if(value==0){
		return '<a href="####" onclick="popState(\'' + projectinfono+ '\')"><img src="<%=contextPath%>/pm/projectHealthInfo/headgreen.jpg" style="cursor: pointer;" width="14px" height="14px"/></a>';
	}else{
		return '<a href="####" onclick="popState(\'' + projectinfono+ '\')"><img src="<%=contextPath%>/pm/projectHealthInfo/headred.jpg" style="cursor: pointer;" width="14px" height="14px"/></a>';
	}
}
//钻取展示小队项目状态信息
function popState(projectinfono){
	window.top.$.JDialog.open('${pageContext.request.contextPath}/dms_info/indexScoreMonitor.jsp?projectinfono='+projectinfono,{
        win:window,
        width:980,
        height:480,
        title:"红绿灯显示",
