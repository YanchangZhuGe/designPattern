      	...
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <td align="right" style="padding-right: 20px;"><font color="red"><span id="sum_value"></span></font></td>
				<%if(proc_status){ %>
			     <auth:ListButton functionId="OP_ADJUST_DEO_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			     <auth:ListButton functionId="OP_ADJUST_DEO_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			     <auth:ListButton functionId="OP_ADJUST_DEO_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			     <%} %>
			  </tr>
			</table>
			</td>
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getDeviceAdjustDepreOtherInfo&projectInfoNo=<%=projectInfoNo%>";
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetAdjust/costTargetAdjustDeviceLdEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	popWindow(cruConfig.contextPath+"/op/costTargetAdjust/costTargetAdjustDeviceLdEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
...
function getSum(){
	...
	" where td.bsflag='0' and  td.project_info_no = '"+project_info_no+"'  and td.record_type='1' and (td.if_change!='2' and  td.if_delete_change is null)";
