...
<script type="text/javascript">
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				var str = "select dui.dev_name,dui.dev_model,dui.self_num,dui.license_num,dui.dev_sign,t.dev_plan_start_date,t.dev_plan_end_date,t.receive_date from gms_device_hirefill_detail t left join gms_device_account dui on t.dev_acc_id=dui.dev_acc_id where t.device_app_detid='"+currentid+"'";
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<datas.length;i++){
			...
			innerHTML += "<td>"+(i+1)+"</td><td>"+datas[i].dev_name+"</td><td>"+datas[i].dev_model+"</td><td>"+datas[i].self_num+"</td>";
			innerHTML += "<td>"+datas[i].license_num+"</td><td>"+datas[i].dev_sign+"</td><td>"+datas[i].dev_plan_start_date+"</td>";
			innerHTML += "<td>"+datas[i].dev_plan_end_date+"</td>";
			innerHTML += "<td>"+datas[i].receive_date+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model){
		...
		str += "p6.name as jobname,pro.project_name,appdet.state,case appdet.state when '1' then '已验收' else '未验收' end as statedesc, ";
...
<script type="text/javascript">	
	...
	function toFillDetailPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devhirestore/hiredev_detail_fill.jsp?deviceappdetid='+id+"&projectInfoNo=<%=projectInfoNo%>","900:680");
	}
	function dbclickRow(shuaId){
		...
		popWindow('<%=contextPath%>/rm/dm/devhirestore/hiredev_detail_fill.jsp?deviceappdetid='+shuaId+"&projectInfoNo=<%=projectInfoNo%>","900:680");
	}
</script>
</html>