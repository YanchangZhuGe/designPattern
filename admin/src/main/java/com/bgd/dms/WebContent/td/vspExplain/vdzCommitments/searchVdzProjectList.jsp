...
<%
	...
	String str_sql="    and t.project_father_no is null ";
	if(orgSubjectionId.startsWith("C105005001")){
		str_sql=" and t.project_type ='5000100004000000008'    and t.project_father_no is null  ";
	}
...
%>
 ...
 <%}else{ %>
 <body style="background:#fff" onload="refreshData();getTab1();">
<%} %>
...
%>
<li class="selectTag" id="tag3_0" name="tag3_0"><a href="#" onclick="getTab()">系统项目</a></li>
<li id="tag3_1" name="tag3_1"><a href="#" onclick="getTab1()">已维护项目</a></li>

<%}else{ %>
<li   class="selectTag" id="tag3_1" name="tag3_1"><a href="#" onclick="getTab1()">已维护项目</a></li>
...
<div id="tab_box_content0" class="tab_box_content" style="background:#fff;height:500px;overflow-x:hidden; overflow-y: scroll; "  > 
      	...
      	<div id="list_table">
			...
			<div id="table_box" style="height:100%;display:block">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">			    
			     <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{project_info_no},{project_name},{project_type},{optioning_type}' id='rdo_entity_id_{project_info_no}'  />" >选择</td>
 ...
 <div id="tab_box_content1" class="tab_box_content" style="display:none;background:#fff;height:500px;overflow-x:hidden;overflow-y:hidden;" > 
	<iframe width="100%" height="100%" name="selectLabor" id="selectLabor" frameborder="0" src="<%=contextPath%>/td/vspExplain/vdzCommitments/optionProjectList.jsp?types=<%=types%>" marginheight="0" marginwidth="0" >
	</iframe>	
 </div>  
...
<script type="text/javascript">
	...
	function refreshData(){
		if(types =="vsp"){
			cruConfig.queryStr = "select t.* from (select distinct 'wt' optioning_type, pd.org_id,t.project_type,t.project_info_no,t.project_name,t.project_year,c.coding_name as project_typename from gp_task_project t,comm_coding_sort_detail c,gp_task_project_dynamic pd where t.bsflag = '0' and pd.bsflag = '0'   and t.project_type = c.coding_code_id and pd.project_info_no = t.project_info_no     and t.project_info_no in (select   project_info_no  from bgp_ws_view_type  t where  t.view_type_code not in ('5110000053000000006','5110000053000000007') and t.bsflag='0' )   ) t where  t.project_info_no not in (select project_info_no from GP_WS_VSP_COMMITMENTS   where  doc_type = '0110000061200000001'  and bsflag='0' )  order by t.project_year desc    ";
		}else{ 
			cruConfig.queryStr = "select t.* from (select distinct  'wt' optioning_type,pd.org_id,t.project_type,t.project_info_no,t.project_name,t.project_year,c.coding_name as project_typename from gp_task_project t,comm_coding_sort_detail c,gp_task_project_dynamic pd where t.bsflag = '0' and pd.bsflag = '0'   and t.project_type = c.coding_code_id and pd.project_info_no = t.project_info_no     and t.project_info_no in (select   project_info_no  from bgp_ws_view_type  t where  t.view_type_code in ('5110000053000000006','5110000053000000007') and t.bsflag='0' )   ) t where  t.project_info_no not in (select project_info_no from GP_WS_VSP_COMMITMENTS   where  doc_type = '0110000061200000006'  and bsflag='0' )  order by t.project_year desc    ";
		}
		...
		cruConfig.currentPageUrl = "/td/vspExplain/vdzCommitments/searchVdzProjectList.jsp";
		queryData(1);	
