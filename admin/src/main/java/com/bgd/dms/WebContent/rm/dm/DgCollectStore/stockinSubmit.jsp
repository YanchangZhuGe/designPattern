      ...
      <table id="table1" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
         <tr>               
				...
				<td class="inquire_form6">
					...
					<input name="fk_dev_acc_id" id = "fk_dev_acc_id" class="input_width" type="hidden"/>
...
<script type="text/javascript"> 
	...
	function loadDataDetail(){
		...
		if(device_backdet_id!=null){
		   /** var querySql="select backdet.*,t.back_num as mixnum,t.device_coll_mixinfo_id,unitsd.coding_name as dev_unit_name,outacc.device_id,outacc.type_id,";
		    	...
		    	querySql+="left join gms_device_coll_account acc on accdui.dev_name=acc.dev_name and acc.ifcountry != '国外' and accdui.dev_model=acc.dev_model and backfor.receive_org_id=acc.usage_org_id ";//公司级台账使用状态
		    	querySql+="left join gms_device_coll_account outacc on accdui.dev_name=outacc.dev_name and outacc.ifcountry != '国外' and accdui.dev_model=outacc.dev_model and accdui.out_org_id=outacc.usage_org_id ";//公司级台账使用状态
			...
			var querySql = "select t.device_backdet_id,t.device_backapp_id,t.dev_acc_id,t.dev_name,t.dev_model,t.planning_out_time,t.is_leaving ,t.back_num as mixnum,unitsd.coding_name as dev_unit_name,acc.device_id,";
				...
				querySql += "revorg.org_abbreviation as revorg_name,app.project_info_id as project_info_no, ";
				querySql += "usesub.org_subjection_id as usage_sub_id,outacc.owning_org_id as owning_org_id,ownorg.org_abbreviation as owning_org_name,ownsub.org_subjection_id as owning_sub_id, ";
				querySql += "accdui.out_org_id,oldorg.org_abbreviation as oldorg_name,";
				...
				querySql += "left join gms_device_coll_account acc on accdui.device_id = acc.device_id and accdui.out_org_id = acc.usage_org_id ";
				querySql += "left join gms_device_coll_account outacc on accdui.fk_dev_acc_id = outacc.dev_acc_id and outacc.ifcountry != '国外' ";
				...
				querySql += "left join comm_org_information ownorg on outacc.owning_org_id = ownorg.org_id and ownorg.bsflag='0' left join comm_org_subjection ownsub on outacc.owning_org_id = ownsub.org_id and ownsub.bsflag='0' ";
				querySql += "left join comm_org_subjection usesub on usesub.org_id=accdui.out_org_id ";
		...
		document.getElementById("receive_org_id").value =retObj[0].receive_org_id;
		...
		//新加account_dui表 和 account表关联字段
		document.getElementById("fk_dev_acc_id").value =retObj[0].fk_dev_acc_id;
	...
	function submitInfo(){
		...
		if(window.confirm("确认验收?")){
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSubmitDgCollectStock.srq?device_backdet_id="+device_backdet_id+"&devaccId="+devaccId+"&device_backapp_id="+device_backapp_id+"&projectInfoNo="+projectInfoNo;
