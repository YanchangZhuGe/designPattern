      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			    <auth:ListButton functionId="" css="jh" event="onclick='toSumbitDevApp()'" title="提交"></auth:ListButton>
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_backdet_id{device_backdet_id}' name='device_backdet_id'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' value='{device_coll_backdet_id}' id='selectedbox_{device_coll_backdet_id}' leavinginfo='{isleaving}' onclick='chooseOne(this)'/>" >选择</td>
			...
			<div id="tag-container_3">
			 <ul id="tags" class="tags">
			    ...
			    <li id="tag3_1"><a href="#" onclick="getContentTab(this,1)">验收信息</a></li>
			  	<li id="tag3_5"><a href="#" onclick="getContentTab(this,5)">验收记录</a></li>
				...
				<div id="tab_box_content1" name="tab_box_content1" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							...
							<td class="bt_info_even" width="11%">待修数量</td>
							<td class="bt_info_odd" width="11%">毁损数量</td>
							<td class="bt_info_even" width="11%">盘亏数量</td>
							<td class="bt_info_even" width="11%">未交回数量</td>
				        </tr>
					...
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
				        ...
				        <tbody id="checkdetailMaps" name="checkdetailMaps" ></tbody>
					</table>
				</div>
				<div id="tab_box_content2" class="tab_box_content" style="display:none;"></div>
				<div id="tab_box_content3" name="tab_box_content2" class="tab_box_content" style="display:none"></div>
				<div id="tab_box_content4" name="tab_box_content3" class="tab_box_content" style="display:none"></div>
		 </div>
</div>
</body>
<script type="text/javascript">
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			var idinfo = $(filterobj).attr("idinfo");
				//先进行查询
				var str = "select firm.good_num,firm.torepair_num,firm.tocheck_num,firm.destroy_num,firm.noreturn_num,firm.create_date as check_date ";
				str += "from gms_device_coll_account_firm firm ";
				str += "where firm.noreturn_num=0 and  firm.device_backdet_id='"+currentid+"' order by create_date desc";
		...
		if(index == 5){
			...
			var idinfo = $(filterobj).attr("idinfo");
				...
				if(basedatas!=undefined && basedatas.length>=1){
					//先清空
					var filtermapid = "#checkdetailMaps";
					$(filtermapid).empty();
					appendDataToDetailTab(filtermapid,basedatas);
					//设置当前标签页显示的主键
					$(filterobj).attr("idinfo",currentid);
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			...
			innerHTML += "<td>"+datas[i].destroy_num+"</td><td>"+datas[i].tocheck_num+"</td><td>"+datas[i].noreturn_num+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model){
		...
		var str = "select t.device_coll_backdet_id,t.back_num as mix_num,account.dev_name,account.dev_model, case t.is_leaving when '0' then '未验收' when '1' then '已验收' when '2' then '验收中' end as isleaving, account.dev_unit,account.total_num,account.unuse_num,account.use_num,account.actual_in_time,unitsd.coding_name as dev_unit_name,t.in_date as alter_date from gms_device_coll_back_detail t  left join gms_device_coll_account_dui account on t.dev_acc_id2 = account.dev_acc_id left join comm_coding_sort_detail unitsd on account.dev_unit = unitsd.coding_code_id  where t.device_coll_mixinfo_id ='<%=devicebackappid%>' ";
...
<script type="text/javascript">	
	...
	function toSumbitDevApp(){
	  ...
	  if(leavinginfo=="验收中"){
			  editUrl = "<%=contextPath%>/rm/dm/collectStore/stockinSubmitUpdate.jsp?id="+selId;  
			  popWindow(editUrl,'950:760');	  
		  	  return;
		  }
	  editUrl = "<%=contextPath%>/rm/dm/collectStore/stockinSubmit.jsp?id="+selId;  
	  popWindow(editUrl,'950:760');	  
	}
	function toBack(){
		window.location.href='<%=contextPath%>/rm/dm/collectStore/stockinMainInfoList.jsp';
	}
