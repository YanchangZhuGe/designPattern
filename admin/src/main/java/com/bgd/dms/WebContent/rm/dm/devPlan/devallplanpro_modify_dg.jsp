...
<%
	...
	String[] protypeids = rb.getString("BackTypeID").split("~", -1);
	String[] protypenames = rb.getString("BackTypeName").split("~", -1);
	String[] protypeusernames = rb.getString("BackTypeUserName").split("~", -1);
  ...
  <div id="new_table_box_content">
    <div id="new_table_box_bg">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        ...
        <tr>
          <td class="inquire_item4" >申请设备类别:</td>
          <td class="inquire_form4">
...
<script type="text/javascript">
	...
	function saveInfo(){
		var allapptype = document.getElementById("mixownership").value;
		
		//大计划保存
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveDevAllAppBaseInfo.srq?state=0&allapp_type="+allapptype;
	...
	function refreshData(){
		...
		if('<%=deviceallappid%>'!=null){
			//查询基本信息
			var querySql = "select devapp.ALLAPP_TYPE,devapp.project_info_no,pro.project_name,devapp.appdate,devapp.device_allapp_no,devapp.remark,";
		...
		var innerHTML = "";
		<%
			for(int j=1;j<protypeids.length;j++){
				String protypeid = protypeids[j];
				String protypename = protypenames[j];
				String protypeusername = protypeusernames[j];
		%>
			var tmpstr = "<option id='<%=protypeid%>' value='<%=protypeid%>' protypeusername='<%=protypeusername%>'>"+'<%=protypename%>'+"</option>";
			innerHTML += tmpstr;
		<%
			}
		%>
		$("#mixownership").append(innerHTML);
		$("#mixownership").val(basedatas[0].allapp_type);
	}
</script>
</html>
