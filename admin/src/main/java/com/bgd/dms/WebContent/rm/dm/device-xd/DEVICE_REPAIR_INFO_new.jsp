    ...
    <div id="new_table_box_bg" style="width: 95%">
	  ...
	  <table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		  ...
		  <tr>
			...
			<td class="inquire_form6">
			<select cssClass="select_width" name='repairType' id="repairType" value=""  >
	...
	<div style="overflow:auto">
      	<table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		  	<tr align="right">
		  		...
		  		<td class="ali_cdn_input" ></td>
		  		<td>&nbsp;</td>
		    	<auth:ListButton functionId="" css="zj" event="onclick='add()'" title="增加"></auth:ListButton>
	  ...
	  <fieldset><legend>维修保养明细</legend>
	  	...
	  	<table id="educationMap" width="250%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;">
			  <tr>  
				  ...
				  <td class="bt_info_even" width="13%">修理项目</td>
				  <td class="bt_info_odd" width="13%">计划单号</td>
				  <td class="bt_info_even" width="16%">材料名称</td>
				  <td class="bt_info_odd" width="10%">材料编号</td>
				  <td class="bt_info_even" width="10%">单价</td>
				  <td class="bt_info_odd" width="10%">出库数量</td>
				  <td class="bt_info_even" width="10%">消耗数量</td>
				  <td class="bt_info_odd" width="10%">总价</td>
			  </tr>
...
<script type="text/javascript"> 
	...
	function checks(){
		...
		if($("#HUMAN_COST")[0].value!=''&&!reg.test($("#HUMAN_COST")[0].value)){
			alert("工时费的格式不正确！ 工时费应为正浮点数");
			$("#HUMAN_COST")[0].value="";
			$("#HUMAN_COST")[0].focus();
			return false;
		}
				
		if($("#HUMAN_COST")[0].value!=''&&!reg.test($("#MATERIAL_COST")[0].value)){
			alert("材料费的格式不正确！ 材料费应为正浮点数");
		...
		if(objssss.length>0){			
			var repair_item_row;//修理项目
			var wz_name_row;//材料名称
			var wz_price_row;//单价
			var asign_num_row;//消耗数量
			for(var i=0;i<objssss.length;i++){
				
				repair_item_row=document.getElementById("repairItem"+objssss[i].value);
				if(repair_item_row.value==""){
					repair_item_row.focus();
					alert("修理项目不能为空!");
					return false;
				}
				alert(repair_item_row.value);
				//repair_info_row=document.getElementById("repair_info"+objssss[i].value);
	...
	function add(){
		...
		if(repair_type == '0110000037000000003'){//委外维修
			...
			var newTr=assign_body.insertRow();
				...
				newTr.insertCell().innerHTML="<td><select name='repairItem"+rownum+"' id='repairItem"+rownum+"' /></select></td>";
				newTr.insertCell().innerHTML="<td><input type=text id=teammat_out_id"+rownum+" name=teammat_out_id"+rownum+" value='' size='6' readonly><input type=hidden id=use_info_detail"+rownum+" name=use_info_detail"+rownum+" value='' ></td>";//计划单号
				newTr.insertCell().innerHTML="<td><input type=text id=wz_name"+rownum+" name=wz_name"+rownum+" value='' size='16'></td>";//材料名称
				newTr.insertCell().innerHTML="<td><input type=text id=wz_id"+rownum+" name=wz_id"+rownum+" value='' size='6' readonly></td>";//材料编号
				newTr.insertCell().innerHTML="<td><input type=text id=wz_price"+rownum+" name=wz_price"+rownum+" value='0' size='8' onkeyup='AutoCal(this,"+rownum+")'></td>";//单价
				newTr.insertCell().innerHTML="<td><input type=text id=use_num"+rownum+" name=use_num"+rownum+" value='' size='8' readonly></td>";//出库数量
				newTr.insertCell().innerHTML="<font color=red>*</font>&nbsp;<input type='text' id='asign_num"+rownum+"' name='asign_num"+rownum+"' out_num='"+11+"' value='' size='5' onkeyup='AutoCal(this,"+rownum+")'>";//消耗数量
				newTr.insertCell().innerHTML="<td><input type='text' size='5' readonly='readonly' id='total_charge"+rownum+"' name='total_charge"+rownum+"' value='0' size='6' readonly/></td>";//总价
				...
				if(usingdatas != null){
					for (var i = 0; i< usingdatas.length; i++) {
						document.getElementById("repairItem"+rownum).options.add(new Option(usingdatas[i].coding_name,usingdatas[i].coding_code_id)); 
					}
				}
	...
	function edit(basedatas){
		...
		for(var i=0;i<basedatas.length;i++){
			...
			newTr.insertCell().innerHTML="<font color=red>*</font>&nbsp;<input type='text' id='asign_num"+rownum+"' name='asign_num"+rownum+"' out_num='"+basedatas[i].out_num+"' size='5' value='"+basedatas[i].material_amout+"' onkeyup='AutoCal(this,"+rownum+")'>";//消耗数量
