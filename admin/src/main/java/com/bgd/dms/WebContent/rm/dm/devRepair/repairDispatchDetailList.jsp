	...
	<div id="list_table">
					...
					<td background="<%=contextPath%>/images/list_15.png"><table
							...
							<tr>
								...
								<td class="ali_cdn_input"><input
									...
									type="text" class="input_width" /></td>
									<td class="ali_cdn_name">全部派工</td>
								<td class="ali_cdn_input">
								<select name="s_all_dispatch" id="s_all_dispatch" />
								...
								<auth:ListButton functionId="" css="zj"
									event="onclick='toAddDevRepairPage()'" title="新增"></auth:ListButton>
									 -->
								<auth:ListButton functionId="" css="wxpg"
									event="onclick='toModifyDevRepairPage()'" title="派工"></auth:ListButton>
								<!-- 
		...
		<div id="table_box">
			<table style="width: 98.5%" border="0" cellspacing="0"
				...
				<tr id='rec_id_{rec_id}' name='rec_id' idinfo='{rec_id}'>
					<td class="bt_info_odd"
						exp="<input type='checkbox' name='selectedbox' value='{rec_id}' recid='{rec_id}'  repid='{id}'  id='selectedbox_{rec_id}' onclick='chooseOne(this)'/>">选择</td>
					...
					<td class="bt_info_even" exp="{all_dispatch}">全部派工</td>
					<!-- 
				...
				<table id="projectMap" name="projectMap" border="0" cellpadding="0"
					...
					<tr>
						...
						<td class="inquire_form6"><input id="create_date"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
						<td class="inquire_item6">全部派工：</td>
						<td class="inquire_form6"><input id="all_dispatch"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
					</tr>
				</table>
			</div>
...
<script type="text/javascript">
	...
	function searchDevData(){
		...
		var v_all_dispatch = document.getElementById("s_all_dispatch").value;
		refreshData(v_device_repair_app_no, v_device_repair_app_name,v_all_dispatch);
	}
	function clearQueryText(){
		document.getElementById("s_device_repair_app_no").value = "";
		document.getElementById("s_device_repair_app_name").value = "";
	}
    function refreshData(v_device_repair_app_no, v_device_repair_app_name,v_all_dispatch){
		var str = 
			"SELECT REP.*,\n" +
		...
		var str2 = "SELECT M.COUNT1 AS COUNT1, M.FORMID AS FORMID\n" +   //查找已经派工完成的记录
			...
			"   AND M.COUNT1 = N.COUNT2";

		var sql = "select a.*,decode(nvl(b.count1,0),0,'否','是') as all_dispatch from ("+str+") a,("+str2+") b " + 
		"where a.id = b.formid(+) ";
		if(v_all_dispatch!=undefined && v_all_dispatch!=''){
			if(v_all_dispatch == 0){
				sql += " and  nvl(b.count1,0) = 0 ";
			}else{
				sql += " and  nvl(b.count1,0) <> 0 ";
			}
		}
		sql += " order by  decode(nvl(b.count1,0),'0','否','是') ";
    ...
    function loadDataDetail(id){
    	var retObj;
		if(id!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getDevRepairBaseInfo", "id="+id);
		    ...
		    retObj = jcdpCallService("DevCommInfoSrv", "getDevRepairBaseInfo", "id="+ids);
		}
		//选中这一条checkbox
		$("#selectedbox_"+retObj.deviceRepairMap.rec_id).attr("checked","checked");
		//取消其他选中的
		$("input[type='checkbox'][name='selectedbox'][id!='selectedbox_"+retObj.deviceRepairMap.rec_id	+"']").removeAttr("checked");
		//给数据回填
		$("#repair_form_code","#projectMap").val(retObj.deviceRepairMap.repair_form_code);
		$("#repair_form_name","#projectMap").val(retObj.deviceRepairMap.repair_form_name);
		$("#project_name","#projectMap").val(retObj.deviceRepairMap.project_name);
		$("#req_comp_name","#projectMap").val(retObj.deviceRepairMap.req_comp_name);
		$("#todo_comp_name","#projectMap").val(retObj.deviceRepairMap.todo_comp_name);
		$("#creator_name","#projectMap").val(retObj.deviceRepairMap.creator_name);
		$("#create_date","#projectMap").val(retObj.deviceRepairMap.create_date);
		$("#status_desc","#projectMap").val(retObj.deviceRepairMap.status_desc);
		$("#all_dispatch","#projectMap").val(retObj.deviceRepairMap.all_dispatch);
		var device_repair_name = retObj.deviceRepairMap.repair_form_name;
		var device_repair_no = retObj.deviceRepairMap.repair_form_code;
		//重新加载当前标签页信息
	...
	function toModifyDevRepairPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devRepair/repairDispatchDetail.jsp?id='+id,'800:680');
	}
