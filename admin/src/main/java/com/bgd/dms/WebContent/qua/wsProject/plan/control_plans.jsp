...
function toadd(){
  	popWindow('<%=contextPath%>/qua/wsProject/plan/addOrModifyDoc.jsp?quaPlanId=""');
}
function toEdit(){
    ids = getSelIds('chk_entity_id');
    if(ids==''){ 
    ...
    var quaPlanId = ids.split(":")[0];
    var pro = ids.split(":")[3];
    if(pro=='待审批'||pro=='审批通过'){
		alert("已提交流程不可修改！");
    }else{
      	popWindow('<%=contextPath%>/qua/wsProject/plan/addOrModifyDoc.jsp?quaPlanId='+quaPlanId);         
    }
}
...
function refreshData(v_project_name, v_filename){
	var str="SELECT doc.UCM_ID as ucmid,plan.*,case wf.proc_status when '1' then '待审批' when '3' then '审批通过' when '4' then '审批不通过' else '未提交' end as proc_status "
		+"FROM BGP_QUA_CHECK_PLAN_WS plan "
		+"left join COMMON_BUSI_WF_MIDDLE wf on plan.QUA_PLAN_ID=wf.BUSINESS_ID and wf.BSFLAG='0'  "
		+"left join BGP_DOC_GMS_FILE doc on doc.FILE_ID=plan.FILE_ID and doc.BSFLAG='0'"
		+"where  plan.BSFLAG='0' and wf.proc_status in ('1','3','4')  "
		if(v_project_name!=undefined && v_project_name!=''){
			str += "and plan.project_name like '%"+v_project_name+"%' ";
		}
		if(v_filename!=undefined && v_filename!=''){
			str += "and plan.filename like '%"+v_filename+"%' ";
		}
		str += "order by wf.proc_status desc, plan.create_date desc ";
...
function loadDataDetail(qua_plan_id){
	processNecessaryInfo={        							//流程引擎关键信息
		businessTableName:"bgp_qua_check_plan_ws",    				//置入流程管控的业务表的主表表明
		businessType:'5110000004100001036',    					//业务类型 即为之前设置的业务大类 
		businessId:qua_plan_id.split(":")[0],           				//业务主表主键值
		businessInfo:'井中质量检查计划',
		applicantDate:'<%=appDate%>'       						//流程发起时间
	};
	processAppendInfo={ 	
		projectInfoNo:'<%=project_info_no%>',
		quaCheckPlanid:qua_plan_id.split(":")[0]
	};
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
			    ...
			    <td class="ali_query"><span class="qc"><a href="#" onclick="clearQueryText()" title="清除"></a></span></td>
						 	...
						 	<auth:ListButton functionId="F_QUA_PLAN_002" css="xg" event="onclick='toEdit()'" title="JCDP_btn_edit"></auth:ListButton>
							<auth:ListButton functionId="F_QUA_PLAN_002" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
						 	<auth:ListButton functionId="" css="tj" event="onclick='tosubmit()'" title="JCDP_btn_submit"></auth:ListButton>
						 	--><auth:ListButton functionId="F_QUA_PLAN_002" css="xz" event="onclick='toDownload()'" title="JCDP_btn_download"></auth:ListButton>
...
<td class="bt_info_odd" exp="<input type='checkbox' name='chk_entity_id' value='{qua_plan_id}:{file_id}:{ucmid}:{proc_status}' id='chk_entity_id' onclick='chooseOne(this)'/>" >选择</td>
			  	...
			  	<td class="bt_info_even" exp="{planstartdate}">计划开始</td>
			  	<td class="bt_info_even" exp="{planenddate}">计划结束</td>			  	
			  	<td class="bt_info_odd" exp="{responsibility}">责任人</td>
			  	<td class="bt_info_even" exp="{checkperson}">检查人</td>
