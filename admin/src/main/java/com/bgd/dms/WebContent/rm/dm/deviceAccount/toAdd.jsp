				...
				<td>
					<div style="position:relative;">
				    	<span style="margin-left:105px;width:18px;overflow:hidden;">
						    <select style="width:108px;margin-left:-100px" id="cont_num_tmp" name="cont_num_tmp" onchange="this.parentNode.nextSibling.value=this.value" readonly>
...
<script type="text/javascript"> 
	...
	function pageInit(){
		...
		var querySql="select * from comm_coding_sort_detail where coding_sort_id='0110000007' and bsflag='0' order by coding_show_id ";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		...
		//使用状况默认闲置
		document.getElementById("using_stat").options[1].selected=true;
		$("select option[value='0110000007000000001']").remove();//移除在用
		//技术状况默认完好
	...
	function submitInfo(){
		...
		if(accountstat == '0110000013000000006'){
			...
			querySql = "select * from gms_device_account where dev_sign='"+document.getElementById("dev_sign").value+"' "
				+ "and cont_num='"+document.getElementById("cont_num").value+"' and bsflag='0' "
		...
		datas = queryRet.datas;
		if(datas==""||datas==null){
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSubmitAccount.srq";
