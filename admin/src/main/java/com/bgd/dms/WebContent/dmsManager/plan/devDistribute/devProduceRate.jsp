...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		//调整页面高度
		function frameSize() {
			var height=$(window).height()-$("#tongyong_box_title").height()-$("#lineTable").height()-160;
		...
		function getFieldCaptialFusionChart(){
			...
			chart1.setXMLUrl("<%=contextPath%>/dms/plan/devDistribute/getDeviceProduceRate.srq");
		...
		function getList(){	
			var retObj = jcdpCallService("DeviceDistributeSrv","getDeviceProduceRate","");
			var j=0;
			var t = document.getElementById('lineTable');
			if(retObj.list != null){	
				for(var i=0;i<retObj.list.length/2;i++){
					var tr = t.insertRow(i);
					var td = tr.insertCell(0);
					td.innerHTML = retObj.list[j].label;
				    td = tr.insertCell(1);
					td.innerHTML = retObj.list[j].toolText;
					td = tr.insertCell(2);
					td.innerHTML = retObj.list[j+1].label;
					td = tr.insertCell(3);
					td.innerHTML = retObj.list[j+1].toolText;
					j=j+2; 
				}    
			}
		}
