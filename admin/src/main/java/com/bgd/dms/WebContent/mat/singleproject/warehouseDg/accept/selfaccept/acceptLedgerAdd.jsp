...
<%
	...
	String plan_flat_id  =request.getParameter("ids");
...
<table border="0" cellpadding="0" cellspacing="0"
	...
	<input type="hidden" name="plan_flat_id" id="plan_flat_id" value="<%=plan_flat_id %>"/>
	...
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" id="queryRetTable">
			<tr>
				<td class="bt_info_even" exp="<input name = 'rdo_entity_id'  type='checkbox'  value='{wz_id}' checked = 'true' onclick='loadDataDetail()'/>" ><input type='checkbox' name='rdo_entity_id' value='' onclick='check()'/></td>
				...
				<td class="bt_info_even" exp="{plan_price}<input type='text' style='display:none' name='actual_price_{wz_id}' id='mat_num_{wz_id}' value='{plan_price}'  style='width:40px' />">入库单价</td>
				<td class="bt_info_odd"	exp="{plan_num}<input type='text' style='display:none' name='mat_num_{wz_id}' id='mat_num_{wz_id}' value='{plan_num}'  style='width:40px' onkeyup='keyup()'/>">入库数量</td>
				<td class="bt_info_even"exp="{total_money}<input type='text' style='display:none' name='total_money_{wz_id}' id='mat_num_{wz_id}' value='{total_money}'  style='width:40px'/>">入库金额</td>
				<td class="bt_info_odd"	exp="<input type='text' name='warehouse_number_{wz_id}' id='mat_num_{wz_id}' value=''  style='width:40px'/>">收料库</td>
				<td class="bt_info_even" exp="<input type='text' name='goods_allocation_{wz_id}' id='mat_num_{wz_id}' value=''  style='width:40px'/>">货位</td>
				<td class="bt_info_odd" exp="<select style='display:none' id='is_recyclemat' name='is_recyclemat' value='{wz_type}'><option value='{wz_type}'>{wz_type}</option></select>">备注</td> 
			</tr>
		</table>
	</div>
...
<script type="text/javascript"><!--
	...
	function refreshData(){
		var sql =" select i.wz_id,i.wz_name,i.wz_prickie,t.plan_num,t.plan_price,decode(t.wz_type,'2', '物资供应','1','专业化') wz_type,(t.plan_num *  t.plan_price) total_money from gms_mat_demand_plan_flat t inner join gms_mat_infomation i on t.wz_id=i.wz_id  where plan_flat_id='<%=plan_flat_id%>'";
			sql+=" union all select i.wz_id,i.wz_name,i.wz_prickie,t.demand_num plan_num,i.wz_price plan_price,case when t.bsflag is not null then '物资供应' else '物资供应' end wz_type,(t.demand_num * i.wz_price) total_money  from GMS_MAT_DEMAND_PLAN_DETAIL t inner join gms_mat_infomation i on t.wz_id=i.wz_id where plan_detail_id='<%=plan_flat_id%>'";
			sql+="  union all select i.wz_id, i.wz_name, i.wz_prickie, info.out_num plan_num,  i.wz_price plan_price, case when info.bsflag is not null then '转出' else  '转出' end wz_type, (info.out_num* info.out_price) total_money  from  gms_mat_out_info_detail info inner join gms_mat_infomation i on info.wz_id = i.wz_id where info.out_info_detail_id = '<%=plan_flat_id%>' ";
		cruConfig.cdtType = 'form';
