			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_even" exp="<input name=''  type='text' value='{total_price}' readonly/>">预计金额</td>
...
//		var obj = event.srcElement;
		for(var i=1;i<row.length;i++){
			...
			row[i].cells[9].childNodes[4].name = "main_part_"+i;
			row[i].cells[9].childNodes[8].name = "main_part_"+i;
			row[i].cells[9].childNodes[12].name = "main_part_"+i;
			row[i].cells[9].childNodes[16].name = "main_part_value_"+i;
			
			debugger;
			//修改页面主板区域字段，对应复选框
			if(row[i].cells[9].childNodes[16].value!=""&&shuaId=="1"){
				var checkedValue = 	row[i].cells[9].childNodes[16].value.split(",");
			...
			row[i].cells[9].childNodes[16].value = ids;            //选中的项目部
			...
			if(row[i].cells[0].firstChild.checked==true){
				...
				if(cell_5!=undefined&&cell_6!=undefined){
					...
					cell_7 = row[i].cells[7].firstChild.value = Math.round(cell_5*cell_6*1000)/1000;
				}
