...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		...
		function getFusionChart(){
			//新度系数图表
			getSecondLevelNewExtentFusionChart();
		}
		//获取新度系数
		function getSecondLevelNewExtentFusionChart(){
			var chart1 = new FusionCharts( "${applicationScope.fusionChartsURL}/Charts/Column2D.swf", "chart1", "95%", "95%", "0", "0" ); 
			chart1.setXMLUrl("<%=contextPath%>/dms/device/getNewExtent.srq?level=<%=level%>&devTreeId=<%=devTreeId%>&orgSubId=<%=orgSubId%>&country=<%=country%>&ifproduction=<%=ifproduction%>");
			chart1.render("chartContainer1");
		}
		//弹出子层级新度系数
		function popNextLevelNewExtent(level,devTreeId,orgSubId,country,ifproduction){
			popWindow('<%=contextPath%>/dmsManager/device/thirdLevelInsNewExtent.jsp?level='+level+'&devTreeId='+devTreeId+'&orgSubId='+orgSubId+'&country='+country+'&ifproduction='+ifproduction,'800:572');
		}
		//弹出设备信息
