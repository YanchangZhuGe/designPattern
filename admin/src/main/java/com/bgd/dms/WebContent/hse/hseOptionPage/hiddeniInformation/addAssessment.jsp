    ...
    <div id="new_table_box_bg">
				    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height" >					
					  ...
					  <tr>	
					    ...
					    <td class="inquire_form6"><input type="text" id="evaluation_date" name="evaluation_date"  style="color:gray;" value="系统当前日期" class="input_width"   readonly="readonly"  /></td>	 
					  </tr>		
					  </table>
				</div>
			<div id="oper_div">
...
//键盘上只有删除键，和左右键好用
 function noEdit(event){
 	...
 	function submitButton(){  
 			...
 			var evaluation_state="已评价";
 			var path = '<%=request.getContextPath()%>/rad/addOrUpdateEntity.srq';
			var submitStr = 'JCDP_TABLE_NAME=BGP_ASSESSMENT_INFORMATION&JCDP_TABLE_ID=&org_sub_id='+org_sub_id
			+'&second_org='+second_org+'&third_org='+third_org+'&evaluation_numbers='+evaluation_numbers+'&evaluation_state='+evaluation_state+'&evaluation_date=<%=curDate%>'
			...
			var assessment_no=retObject.entity_id; //获得 主表id 			
 			top.frames('list').frames[1].refreshData();	
 			 
 			 window.open("<%=contextPath%>/hse/hseOptionPage/hiddeniInformation/homeFrame.jsp?assessment_no="+assessment_no,'homeMain','height=500,width=1000px,left=100px,top=100px,menubar=no,status=no,toolbar=no ', '');
 			//newClose();
 			//alert('保存成功！');
	...
	var rowParams = new Array(); 
	  ...
	  if(assessment_no !=null && assessment_no !=''){
		...
		top.frames('list').getTab3('1');	
		 ...
		 window.open("<%=contextPath%>/hse/hseOptionPage/hiddeniInformation/homeFrame.jsp?assessment_no=",'homeMain','height=500,width=1000px,left=100px,top=100px,menubar=no,status=no,toolbar=no ', '');
		newClose();
		alert('保存成功！');
...
querySql1 = "select count(*)-(-1) as num_p from BGP_ASSESSMENT_INFORMATION   where bsflag='0'";
...
if(queryRet1.returnCode=='0'){
  datas1 = queryRet1.datas;	 
	if(datas1 != null && datas1 != ''){	 
		...
		document.getElementsByName("evaluation_numbers")[0].value=numDate;			
	}
}	
