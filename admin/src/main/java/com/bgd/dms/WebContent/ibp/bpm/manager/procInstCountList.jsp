...
function page_init(){
	...
	var path = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=WFMgr&JCDP_OP_NAME=queryProcInstCount";
	cruConfig.queryListAction = path;
	cdt_init();
	cruConfig.queryStr = "";
	queryData(1);
}
...
<tr>
	...
	<td exp="{procName}">流程名称</td>
	<td exp="{total}">运行实例数量</td>
	<td exp="&lt;a target='_self' href='${pageContext.request.contextPath}/wf/test/getExamineInfo.srq?examineinstID={entityId}&procinstID={procinstId}'&gt;查看运行实例&lt;/a&gt;">查看操作</td>
    <td exp="&lt;a target='_blank' href='${pageContext.request.contextPath}/ibp/bpm/carapply/coginUsers.jsp?examineinstID={entityId}'&gt;删除模板&lt;/a&gt;">删除操作</td>
</tr>
</thead>
</table>
</div>
