...
<body>
<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="<%=contextPath%>/market/month/saveMonthReport.srq">
<table border="0" cellpadding="0" cellspacing="0" class="form_info">
	 <input name="orgId" type="hidden" value="<%=orgId%>"/>
	<tr>
	  	<td class="rtCRUFdName"><font color=red>*</font>&nbsp;月报开始日期：</td>
	    <td class="rtCRUFdValue">
	    	<input type="text" name="week_date" id="week_date" value="<%=weekDate %>" class="input_width" readonly/>	    
	    </td>
	    <td class="rtCRUFdName"><font color=red>*</font>&nbsp;月报结束日期：</td>
...
<table border="0" cellpadding="0" cellspacing="0" id="lineTable" class="form_info" width="100%" style="margin-top:-1px;">
    ...
    <tr>
	  	<td >&nbsp;本月中标及新签价值工作量情况说明</td>
	    <td colspan="3" >
        <textarea id="typeOne" name="typeOne" style="height: 100px" max=2000 msg="本月市场落实情况不超过2000个汉字"  <%if(action.equals("view")){%>readonly<%} %>><%
        if(list1!=null){
				...
				String corpId=(String)map1.get("orgId");
...
<%=orgAbbreviation %> : <%=content %><%} }%>    
	...
	<tr>
	  	<td>&nbsp;投标项目跟踪</td>
	    <td colspan="3" >
        <textarea id="typeTwo" name="typeTwo" style="height: 100px" max=2000 msg="本月市场落实情况不超过2000个汉字"  <%if(action.equals("view")){%>readonly<%} %>><%
        if(list2!=null){
...
<%=orgAbbreviation %> : <%=content %><%} }%>   
	 ...
	 <tr >
	  	<td >&nbsp;市场开发动态</td>
	    <td colspan="3" >
         <textarea id="typeThree" name="typeThree" style="height: 100px" max=2000 msg="本月市场落实情况不超过2000个汉字"  <%if(action.equals("view")){%>readonly<%} %>><%
         if(list3!=null){
...
<%=orgAbbreviation %> : <%=content %><%} }%>    
	...
	<tr>
	  	<td >&nbsp;油田公司动态</td>
	    <td colspan="3" >
         <textarea id="typeFour" name="typeFour" style="height: 100px" max=2000 msg="本月市场落实情况不超过2000个汉字"  <%if(action.equals("view")){%>readonly<%} %>><%
         if(list4!=null){
...
<%=orgAbbreviation %> : <%=content %><%} }%>   
	...
	<tr>
	  	<td >&nbsp;物探公司动态</td>
	    <td colspan="3" >
        <textarea id="typeFive" name="typeFive" style="height: 100px" max=2000 msg="本月市场落实情况不超过2000个汉字"  <%if(action.equals("view")){%>readonly<%} %>><%
        if(list5!=null){
