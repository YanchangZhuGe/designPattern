					...
					<tr>
						<td width="99%">
							<div class="tongyong_box">
								...
								href="#"></a></span><a href="#">单项目备件大类消耗明细(<%=project_name %>)</a>
								<span class="gd"><a href="#"></a></span>
								<span class="dc" style="float:right;margin-top:-4px;">
									<a href="#" onclick="exportDataDoc('TORPM')" title="导出excel"></a>
								</span>
								</div>
...
<script type="text/javascript">
	...
	function getFusionChart(){
		var retObj = jcdpCallServiceCache("EarthquakeTeamStatistics","getZySingleWzXhM","project_info_id=<%=project_info_id%>&self_num=<%=self_num%>&start_date=<%=start_date%>&end_date=<%=end_date%>");
	...
	function exportDataDoc(exportFlag){
		...
		var submitStr = "project_info_id=<%=project_info_id%>&self_num=<%=self_num%>&start_date=<%=start_date%>&end_date=<%=end_date%>&exportFlag="+exportFlag;
	...
	function popZySingleWzM(cb){
		...
		popWindow('<%=contextPath %>/rm/dm/kkzy/popZySingleWzM.jsp?project_info_id='+project_info_id+"&self_num="+self_num+"&start_date="+start_date+"&end_date="+end_date+"&coding_code_id="+coding_code_id,'800:600');
	}
</script>
</html>
