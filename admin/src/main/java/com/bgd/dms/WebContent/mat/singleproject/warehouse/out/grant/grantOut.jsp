...
<table border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<td class="inquire_item6">班组名称：</td>
		<td class="inquire_item6"><input type="text"
			name="selectText" id="selectText" class="input_width"
			value="" readonly />
			<input type='hidden' name='s_apply_team' id='s_apply_team' value=''/>
			</td>
	...
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" id="queryRetTable">
			<tr>
				...
				<td class="bt_info_odd" exp="{approve_num}">需求数量</td>
				<td class="bt_info_even" exp="{use_num}">已出库数量</td>
				<td class="bt_info_odd" exp="<input type='text' name='mat_num_{wz_id}' id='mat_num_{wz_id}' value='{approve_num}'  style='width:40px'/>">出库数量</td>
...
<script type="text/javascript"><!--
	...
	var selectText=getQueryString("selectText");
	document.getElementById("s_apply_team").value = selectId;
	document.getElementById("selectText").value = selectText;
	function refreshData(value){
		
		var sql ="select i.*,d.approve_num from GMS_MAT_DEMAND_PLAN_BZ t inner join (GMS_MAT_DEMAND_PLAN_DETAIL d inner join gms_mat_infomation i on d.wz_id=i.wz_id and i.bsflag='0') on t.submite_number=d.submite_number and d.bsflag='0' where t.submite_number='"+value+"'and t.s_apply_team='"+selectId+"'and t.project_info_no='<%=projectInfoNo%>' and t.bsflag='0' order by i.coding_code_id asc,i.wz_id asc";
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getOutLeaf";
		cruConfig.submitStr ="value="+value+"&selectId="+selectId;
		queryData(1);
	...
	function showDevPage(){
		var obj = new Object();
		var vReturnValue = window.showModalDialog("<%=contextPath%>/mat/select/template/selectPlanList.jsp?teamId="+selectId,obj,'dialogWidth=1024px;dialogHigth=400px');
