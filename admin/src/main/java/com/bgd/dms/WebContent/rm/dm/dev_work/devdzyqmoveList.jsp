      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			    <td id="add" name="add" class='ali_btn'><span class='zj'><a href='#' onclick='toAddMixPlanPage()'  title='新增'></a></span></td>
 
			    <td id="modify" name="modify" class='ali_btn'><span class='xg'><a href='#' onclick='toModifyMixPlanPage()'  title='修改'></a></span></td>
 
			    <td id="delete" name="delete"  class='ali_btn'><span class='sc'><a href='#' onclick='toDelMixPlanPage()'  title='删除'></a></span></td>
 				<td id="submit" name="submit"  class='ali_btn'><span class='tj'><a href='#' onclick='toSumbitMixApp()'  title='提交'></a></span></td>
			     <td id="jieshou" name="jieshou" style="display:none;" class='ali_btn'><span class='jh'><a href='####' onclick='toDetailPage()'  title='验收'></a></span></td>
			...
			<div id="tab_box" class="tab_box">
			  ...
			  <tr>
				...
				<td class="inquire_form6"><input id="moveapp_no" name="" class="input_width" type="text" /></td>
				<td class="inquire_item6">变更单名称</td>
				<td class="inquire_form6"><input id="moveapp_name" name="" class="input_width" type="text" /></td>
				<td class="inquire_item6">开据人</td>
				<td class="inquire_form6"><input id="opertor_id" name="" class="input_width" type="text" /></td>
			  </tr>
				<tr>
				<td class="inquire_item6">转入单位</td>
				<td class="inquire_form6"><input id="in_org_name" name="" class="input_width" type="text" /></td>
				...
				<td class="inquire_form6"><input id="state" name="" class="input_width" type="text" /></td>
			  </tr>
			</table>
				</div>
				<div id="tab_box_content1" name="tab_box_content1" idinfo="" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							...
							<td class="bt_info_even" width="10%">设备型号</td>
							<td class="bt_info_odd" width="10%">转出完好</td>
							<td class="bt_info_odd" width="10%">转出待报废</td>
							<td class="bt_info_odd" width="10%">转出待修</td>
							<td class="bt_info_odd" width="10%">转出在修</td>
							<td class="bt_info_odd" width="10%">转出毁损</td>
				        </tr>
				...
				<div id="tab_box_content2" name="tab_box_content2" idinfo="" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
				        	...
				        	<td class="bt_info_odd" width="10%">设备名称</td>
							<td class="bt_info_even" width="10%">设备型号</td>
							<td class="bt_info_odd" width="10%">接收完好</td>
							<td class="bt_info_odd" width="10%">接收待报废</td>
							<td class="bt_info_odd" width="10%">接收待修</td>
							<td class="bt_info_odd" width="10%">接收在修</td>
							<td class="bt_info_odd" width="10%">接收毁损</td>
							<td class="bt_info_odd" width="10%">接收盘亏</td>
				        </tr>
...
$(function(){
	...
	function getContentTab(obj,index) { 
		...
		var str = " select t.dev_name,t.dev_model,d.wanhao_num,d.touseless_num,d.weixiu_num,d.huisun_num,  d.zaixiu_num from gms_device_moveapp_detail d left join gms_device_coll_account t on t.dev_acc_id=d.out_dev_id  where d.moveapp_id='"+currentid+"'  ";
		...
		if(index == 2){
			//动态查询明细
			var str = " select t.dev_name,t.dev_model,d.rwanhao_num,d.rtouseless_num,d.rweixiu_num,d.rhuisun_num,  d.rzaixiu_num, d.rpankui_num from gms_device_moveapp_detail d left join gms_device_coll_account t on t.dev_acc_id=d.out_dev_id  where d.rwanhao_num is not null   and d.moveapp_id='"+currentid+"'  ";
					var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str+'&pageSize=10000');
					basedatas = queryRet.datas;
					if(basedatas!=undefined && basedatas.length>=1){
						//先清空
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			...
			innerHTML += "<td>"+datas[i].wanhao_num+"</td><td>"+datas[i].touseless_num+"</td><td>"+datas[i].weixiu_num+"</td>";
			innerHTML += "<td>"+datas[i].zaixiu_num+"</td><td>"+datas[i].huisun_num+"</td>";
			innerHTML += "</tr>";
		...
		function appendDataToDetailTab1(filterobj,datas){
			for(var i=0;i<basedatas.length;i++){
				var innerHTML = "<tr>";
				innerHTML += "<td>"+(i+1)+"</td><td>"+datas[i].dev_name+"</td><td>"+datas[i].dev_model+"</td>";
				innerHTML += "<td>"+datas[i].rwanhao_num+"</td><td>"+datas[i].rtouseless_num+"</td><td>"+datas[i].rweixiu_num+"</td>";
				innerHTML += "<td>"+datas[i].rzaixiu_num+"</td><td>"+datas[i].rhuisun_num+"</td><td>"+datas[i].rpankui_num+"</td>";
				innerHTML += "</tr>";
				$(filterobj).append(innerHTML);
			}
			$(filterobj+">tr:odd>td:odd").addClass("odd_odd");
...
<script type="text/javascript">
	...
	function refreshData(v_device_app_name,v_mixinfo_no,v_opr_state_desc,state_role){
		...
		var str = " select f.moveapp_id, f.moveapp_no,  f.moveapp_name, inorg.org_abbreviation as inorgname, outorg.org_abbreviation as outorgname,f.apply_date, ";
			...
			str+= " where   f.bsflag='0' and f.move_type='2' ";
    ...
    {
    	popWindow('<%=contextPath%>/rm/dm/dev_work/devmovejNewApply.jsp','950:680');
    ...
    $("input[type='checkbox'][name='selectedbox']").each(function(){
    	...
    	popWindow('<%=contextPath%>/rm/dm/dev_work/devmovejNewApply.jsp?mixId='+ids,'950:680');
    }
    ...
    var result='0';
		...
		if(confirm("是否执行删除操作?")){
			var sql = "update gms_device_moveapp  set bsflag='1' where moveapp_id ='"+ids+"'";
   ...
   var orgSubId='<%=userSubId%>';
		...
		var state='';//0未提交 1审核通过 2待审核 3审核失败
			...
			var sql = "update gms_device_moveapp  set state='"+1+"' where moveapp_id ='"+ids+"'";
			var path = cruConfig.contextPath+"/rad/asyncDelete.srq";
			var params = "deleteSql="+sql;
			params += "&ids=";
			var retObject = syncRequest('Post',path,params);
			jcdpCallService("DevProSrv", "updateColAccount", "moveapp_id="+ids);
			refreshData();
		}
   }
   ...
   var ids = getSelIds('selectedbox');
   	...
   	popWindow('<%=contextPath%>/rm/dm/dev_work/devmovejNewApplys.jsp?mixId='+ids,'950:680');
   }
