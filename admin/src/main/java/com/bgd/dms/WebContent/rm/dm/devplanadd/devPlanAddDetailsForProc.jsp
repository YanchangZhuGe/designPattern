 		...
 		<div id="list_table"  >
			<div id="table_box" style="overflow-y:srcoll;height:550px">
			      ...
			      <table border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
			        <tr>
			          ...
			          <td class="inquire_form4" >
			          	...
			          	<input name="deviceaddappid" id="deviceaddappid" type="hidden" value="<%=deviceaddappid%>" />
			          </td>
			          <td class="inquire_item4" >配置补充计划单名称:</td>
			          <td class="inquire_form4" >
			          	<input name="device_addapp_name" id="device_addapp_name" class="input_width" type="text"  value="" readonly/>
			          </td>
			        </tr>
			       </table>
		        </fieldset>
		     <div id="dantaidiv" style="display:none">
			  ...
			  <table style="width:95%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable1">		
			 ...
			 <div id="colldiv" style="display:none">
				 <fieldset style="margin-left:2px;width:98%"><legend>批量管理设备</legend>
				  <table style="width:95%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable2">		
				     <tr id='device_appdet_id_{device_allapp_detid}' name='device_allapp_detid'>
				     	<td class="bt_info_odd" >序号</td>
						<td class="bt_info_even">班组</td>
						<td class="bt_info_odd">设备名称</td>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		var str = "select alldet.device_allapp_detid,alldet.project_info_no,sd.coding_name as unitname,teamsd.coding_name as teamname, ";
		str += "p6.name as jobname,pro.project_name,alldet.dev_ci_code,devapp.device_addapp_name, ";
		...
		str += "left join gms_device_allapp_add devapp on alldet.device_addapp_id = devapp.device_addapp_id ";
		...
		str += "where devapp.project_info_no = '"+projectInfoNos+"' and alldet.bsflag !='1' and devapp.bsflag='0' and alldet.device_addapp_id='<%=deviceaddappid%>'";
		...
		if(retObj && retObj.length>0){
			for(var index=0;index<retObj.length;index++){
				if(index == 0 ){
					if(retObj[index].project_name !=""){
						$("#project_name").val(retObj[index].project_name);
						$("#device_addapp_name").val(retObj[index].device_addapp_name);
					}
				}
				...
				innerhtml += "<td><input name='teamname"+index+"' id='teamname"+index+"' style='line-height:15px' value='"+teamname+"' size='12' type='text' readonly/></td>";
				innerhtml += "<td><input name='devicename"+index+"' id='devicename"+index+"' style='line-height:15px' value='"+dev_ci_name+"' size='12' type='text' readonly/></td>";
				innerhtml += "<td><input id='devicetype"+index+"' name='devicetype"+index+"' value='"+dev_ci_model+"' size='12'  type='text' readonly/></td>";
				innerhtml += "<td><input id='unitname"+index+"' name='unitname"+index+"' class='input_width' value='"+unitname+"' size='8' type='text' readonly/></td>";
				innerhtml += "<td><input id='applynum"+index+"' name='applynum"+index+"' class='input_width' value='"+apply_num+"' size='8' type='text'/></td>";
				...
				innerhtml += "<td><input id='purpose"+index+"' name='purpose"+index+"' value='"+purpose+"' style='line-height:15px' size='12' type='text' readonly/></td>";
				innerhtml += "</tr>";
		...
		str = "select alldet.device_allapp_detid,devapp.device_addapp_name,alldet.project_info_no,sd.coding_name as unitname,teamsd.coding_name as teamname, ";
		...
		str += "left join gp_task_project pro on alldet.project_info_no = pro.project_info_no ";
		str += "left join comm_human_employee emp on alldet.employee_id = emp.employee_id ";
		str += "where devapp.project_info_no = '"+projectInfoNos+"' and devapp.bsflag='0' and alldet.bsflag !='1' and alldet.device_addapp_id='<%=deviceaddappid%>'";
		var detailRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str+'&pageSize=10000');
		var retObj = detailRet.datas;
		if(retObj && retObj.length>0){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td><input name='collteamname"+index+"' id='collteamname"+index+"' style='line-height:15px' value='"+collteamname+"' size='12' type='text' readonly/></td>";
				innerhtml += "<td><input name='colldevicename"+index+"' id='colldevicename"+index+"' style='line-height:15px' value='"+colldev_ci_name+"' size='15' type='text' readonly/></td>";
				innerhtml += "<td><input id='colldevicetype"+index+"' name='colldevicetype"+index+"' value='"+colldev_ci_model+"' size='14'  type='text' readonly/></td>";
				innerhtml += "<td><input id='collunitname"+index+"' name='collunitname"+index+"' class='input_width' value='"+collunitname+"' size='8' type='text' readonly/></td>";
				innerhtml += "<td><input id='collapplynum"+index+"' name='collapplynum"+index+"' class='input_width' value='"+collapply_num+"' size='8' type='text'/></td>";
				...
				innerhtml += "<td><input id='collpurpose"+index+"' name='collpurpose"+index+"' class='input_width' value='"+collpurpose+"' size='8' type='text' readonly/></td>";
				innerhtml += "</tr>";
	...
	function submitInfo(oprinfo){
		...
		var count = $("input[type='text'][name^='applynum']").size();
		var collcount = $("input[type='text'][name^='collapplynum']").size();
		...
		if(confirm("确认审批？")){
			...
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveDevAllAddAppAuditInfowfpa.srq?count="+count+"&collcount="+collcount+"&oprstate="+oprstate;
