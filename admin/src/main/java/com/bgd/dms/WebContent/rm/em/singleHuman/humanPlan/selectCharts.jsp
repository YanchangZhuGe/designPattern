...
var querySql = "    select   sum(p.people_number) num  from Comm_Coding_Sort_Detail sd1  left join bgp_comm_human_plan_detail p    on p.apply_team = sd1.coding_code_id    and p.project_info_no ='<%=projectInfoNo%>'   and p.bsflag = '0'  and p.spare1 is null   where sd1.coding_sort_id = '0110000001'   and sd1.bsflag = '0'   and sd1.spare1 = '0'  and sd1.coding_mnemonic_id='<%=projectType%>'  and length(sd1.coding_code) <= 2 " ;				 	 
...
function submit(){
	...
	myChart2.setXMLUrl("<%=contextPath%>/rm/em/chart/queryBanzu.srq?project_id=<%=projectInfoNo%>");   
	myChart2.render("chartContainer");	

}
function getHumanPost(team){
	//debugger;
	popWindow('<%=contextPath%>/rm/em/singleHuman/humanPlan/humanPostChart.jsp?team='+team,'900:680');
