...
function dailyChange(projectInfoNo,produceDate){
	var projectInfoNo = projectInfoNo;
	var produceDate = produceDate;
	window.location.href('<%=contextPath%>/pm/dailyReport/singleProject/wt/wtReportList.jsp?project_info_no='+projectInfoNo+'&produce_date='+produceDate);
	
}
function dailyChange1(projectInfoNo,produceDate){
	var projectInfoNo = projectInfoNo;
	var produceDate = produceDate;
	window.location.href('<%=contextPath%>/pm/dailyReport/singleProject/wt/wtReportListDrill.jsp?project_info_no='+projectInfoNo+'&produce_date='+produceDate);	
}
function toCollExcle(){
  	popWindow('<%=contextPath%>/pm/dailyReport/singleProject/wt/reportListAuditDrillCollExcle.jsp','1280:700');
}
</script>
      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <td>
			   ...
			   <input type="button" name="daily_mian1" id="daily_mian1" value="面积、长度/公里数" onclick="dailyChange();"></input>
			     <input type="button" name="daily_mian" id="daily_mian" value="坐标点" onclick="dailyChange1();"></input></td>

			    <td>&nbsp;</td>
			    <auth:ListButton functionId="" css="ck" event="onclick='toCollExcle()'" title="查看批量上传日报信息"></auth:ListButton>
			...
			<div id="table_box">
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id'  value='{date1},{audit_status},{if_build},{weather},<%=projectInfoNo %>,{daily_no_wt}' id='rdo_entity_id' onclick=doCheck(this)/>" >选择</td>
			      <td class="bt_info_even" autoOrder="1">序号</td>
			      <td class="bt_info_odd" exp="{date1}<input type='hidden' name='audit_status_{daily_no}' id='audit_status_' value='{audit_status}'/>" >施工日期</td>
			  ...
			  <% if(list!=null) {
				  for(int i=0;i<list.size();i++){
					  Map map=(Map)list.get(i);
						  ...
						  %>
						  <td id="info_zhong" class="bt_info_even" exp="{<%=mapFields.get("exploration_method"+i) %>}"><%=map.get("exploration_method_name"+i) %>公里数</td>
					       <td class="bt_info_even" exp="{<%=mapFields.get("exploration_method_name"+i) %>}%" ><%=map.get("exploration_method_name"+i) %>比</td>
...
debugger;
	...
	cruConfig.queryOp = "queryDailyReportLength";
...
*/		
	...
	function toDelete(){
	   ...
	   var ids = getSelIds('rdo_entity_id');
	    ...
	    while(flag){
	    	...
	    	var audit_status = document.getElementById("audit_status_").value;
		    if(audit_status == "3"){
		    	alert("有已经审批通过的，不能删除！");
		    	return;
		    }
		...
		if(confirm('确定要删除吗?')){  
			...
			var retObj = jcdpCallService("WtDailyReportSrv", "deleteDailyReport", "projectInfoNo=<%=projectInfoNo %>&produceDate="+ids);
	...
	function dbclickRow(ids){
	   ...
	   var ids1 = idss[1];
	    ...
	    popWindow('<%=contextPath%>/pm/dailyReport/singleProject/wt/viewDailyReport.jsp?produce_date='+ids+'&status='+ids1+'&build='+ids2+'&weather='+ids3+'&daily_no_wt='+ids5,'1280:800');
	...
	function toSubmit(){
		...
		//    var retObj = jcdpCallService("WtDailyReportSrv", "getDailyReportInfo", "dailyNo="+daily_no+"&projectInfoNo="+project_info_no+"&produceDate="+produce_date);
			...
			if( audit_status!=""&&audit_status!=null ){
				if( audit_status=="0"){		/* 审批状态 处于“未提交”和“审批不通过”的前提下，才能提交 */
					var retObj = jcdpCallService("WtDailyReportSrv", "submitDailyReport", "dailyNo="+daily_no+"&projectInfoNo="+project_info_no+"&produceDate="+produce_date);
