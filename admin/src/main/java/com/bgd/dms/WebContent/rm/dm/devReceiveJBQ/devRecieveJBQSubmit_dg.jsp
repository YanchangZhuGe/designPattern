...
<script type="text/javascript"> 
	...
	function loadDataDetail(){
		var retObj;
	    var querySql = "select t.dev_ci_code as device_id,dui.dev_acc_id,t.device_mix_subid,sub.org_subjection_id as out_sub_id,";
	        querySql += "info.dev_name as dev_name, info.dev_model as dev_type,det.coding_name,d.apply_num,t.assign_num,";
	        querySql += "d.plan_start_date,d.plan_end_date,d.team as team_id,csd.coding_name as team_name,i.org_id as in_org_id,";
	        querySql += "o.org_id as out_org_id,i.org_abbreviation as in_org_name,o.org_abbreviation as out_org_name,";
	        querySql += "dui.total_num,dui.use_num,dui.unuse_num  from gms_device_appmix_main t ";
	        querySql += "left join gms_device_app_detail d on t.device_app_detid=d.device_app_detid left join comm_coding_sort_detail det on d.unitinfo=det.coding_code_id ";
	        querySql += "left join gms_device_coll_account_dui dui on t.dev_ci_code=dui.device_id and dui.project_info_id='<%=projectInfoNo%>' left join comm_coding_sort_detail csd on d.team=csd.coding_code_id ";
	        querySql += "left join gms_device_mixinfo_form f on t.device_mixinfo_id=f.device_mixinfo_id left join comm_org_information i on f.in_org_id=i.org_id ";
	        querySql += "left join comm_org_information o on f.out_org_id=o.org_id left join gms_device_collectinfo info on t.dev_ci_code=info.device_id ";
	        querySql += "left join comm_org_subjection sub on f.out_org_id = sub.org_id ";
	        querySql += "where t.device_mixinfo_id='<%=outInfoId%>'";
