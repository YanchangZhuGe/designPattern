    ...
    <div id="new_table_box_bg">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        ...
        <tr>
          ...
          <td class="inquire_form4">
          	<input name="out_org_name" id="out_org_name" class="input_width" type="text" /><img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor:hand;" onclick="showOutOrgPage()"  />
...
<script type="text/javascript"> 
	...
	$().ready(function(){
		$("#addProcess").click(function(){
			...
			//拼condition,排除其他单子已经添加了的明细信息
			var condition ="and not exists(select 1 from gms_device_mixinfo_detail mid where mid.device_mix_id!=appmix.device_mix_id)";
			var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/tree/selectAccountForMixForm.jsp?out_org_id="+out_org_id+"&projectinfono="+project_info_no+"&condition="+condition,paramobj,"dialogWidth=820px;dialogHeight=380px");
...
<script type="text/javascript"> 
	...
	function showOutOrgPage(){
		...
		var obj = new Object();
		var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/devmixdetail/selectOutOrgId.jsp?devappid="+deviceappid,obj,"dialogWidth=480px;dialogHeight=250px");
		if(vReturnValue!=undefined){
			var returnvalues = vReturnValue.split('~');
	...
	function refreshData(){
		var proSql = "select distinct amm.project_info_no,amm.in_org_id,pro.project_name,org.org_name as in_org_name ";
			...
			retObj = proqueryRet.datas;
		if(retObj.length>=1){
			var innerHtml = "<option value='' inorg='~' >请选择项目</option>";
			for(var index=0;index<retObj.length;index++){
