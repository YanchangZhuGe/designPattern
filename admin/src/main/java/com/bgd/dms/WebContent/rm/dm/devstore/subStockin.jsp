...
<%
    ...
    String devicemixinfoid = request.getParameter("id");
    String back_dev_type = request.getParameter("backdevtype");
  //用户机构对应的orgsubid信息
...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <title>多项目-收工验收-收工验收(单台)-验收子页面</title> 
 </head> 
      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_appdet_id_{device_backapp_id}' name='device_backapp_id'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' instate='{state}' value='{device_backdet_id}' id='selectedbox_{device_backdet_id}' />" >选择</td>
					...
					<td class="bt_info_even" exp="{dev_coding}">ERP设备编码</td>
			...
			<div id="tab_box" class="tab_box">
				<div id="tab_box_content0" name="tab_box_content0" class="tab_box_content">
					...
					<tr>
				      <td   class="inquire_item6">ERP设备编码：</td>
...
var back_dev_type_tmp="<%=back_dev_type%>";
function frameSize(){
...
<script type="text/javascript">
    ...
    function loadDataDetail(devaccid){
		...
		if(devaccid!=null){
		    ...
		    retObj = jcdpCallService("DevCommInfoSrv", "getStockDetailInfo", "devbackid="+ids);
		}
		//取消选中框--------------------------------------------------------------------------
    	//var obj = document.getElementsByName("selectedbox");  
	   //     for (i=0; i<obj.length; i++){   
	...
	function refreshData(v_dev_name,v_dev_model){
		var str="";
		if(orgType=="Y"){
			str += "select backdet.*,oper.operator_name,dui.dev_name,dui.dev_model,dui.check_time,sd.coding_name as stat_desc, ";
			...
			str += "where backdet.device_mixinfo_id='<%=devicemixinfoid%>' and dui.owning_sub_id like '<%=orgsubid%>%' ";
			//补充查询条件
			...
			str += "select backdet.*,dui.dev_name,dui.dev_model,dui.check_time,sd.coding_name as stat_desc, ";
			str += "case backdet.state when '0' then '未验收' when '1' then '已验收' end as isleaving ";
			str += "from gms_device_backapp_detail backdet ";
			str += "left join gms_device_account_dui dui on backdet.dev_acc_id=dui.dev_acc_id ";
			str += "left join comm_coding_sort_detail sd on dui.account_stat=sd.coding_code_id ";
			if(back_dev_type_tmp == 'S9998'){//大港调剂设备
				str += "where backdet.device_backapp_id='<%=devicemixinfoid%>' ";
			}else{
				str += "where backdet.device_mixinfo_id='<%=devicemixinfoid%>' ";
			}
	...
	function toSumbitDevApp(){
	  ...
	  var ids = getSelIds('selectedbox');  
		...
		if(basedatas.length == 0){
			popWindow('<%=contextPath%>/rm/dm/devstore/stockinSubmit.jsp?id='+selId+'&back_dev_type='+back_dev_type_tmp,'800:680');
		}else{
			alert("存在已验收的设备！");
			return;
		}	  
