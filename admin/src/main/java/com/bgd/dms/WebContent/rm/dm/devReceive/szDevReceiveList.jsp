...
<script type="text/javascript">
	...
	function refreshData(v_dev_name,v_dev_model){
		var str = "select case dad.state when '1' then '已接收' else '未接收' end as state_desc,teamid.coding_name as team_name,"+
			...
			"where dm.device_mixinfo_id='"+mixId+"' and dad.project_no is null ";
		  ...
		  var flag=0;
		    ...
		    if(flag)return;
		 	//ids = getSelIds('rdo_entity_id');  
		  	...
		  	if(ids==''){  alert("请选择记录!");  return;  }
		  	document.getElementById("form1").action="<%=contextPath%>/rm/dm/devReceive/szDevReceiveList.srq?mixId="+mixId+"&listvalue="+ids;
