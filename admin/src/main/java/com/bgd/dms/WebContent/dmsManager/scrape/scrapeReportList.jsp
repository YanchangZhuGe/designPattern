			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='scrape_report_id_{scrape_report_id}' name='scrape_report_id'  idinfo='{scrape_report_id}'>
					...
					<td class="bt_info_odd" exp="{scrape_report_no}">报废批复单号</td>
					<td class="bt_info_odd" exp="{scrape_report_name}">报废批复单名称</td>
					<td class="bt_info_even" exp="{employee_name}">批复人</td>
					<td class="bt_info_odd" exp="{org_name}">批复单位</td>
					<td class="bt_info_even" exp="{report_date}">批复时间</td>
		...
		if(index == 1){
		  ...
		  //设备信息显示
			$("#dev_grid").datagrid({
				...
				queryParams:{'scrape_report_id':ids}
			});
		}
		else if(index == 3){
...
<script type="text/javascript">
    ...
    function refreshData(scrape_report_no, scrape_report_name){
      	var temp = "";
    ...
    function loadDataDetail(scrape_report_id){
		...
		var curbusinesstype="5110000181000000028";
    	processNecessaryInfo={        							//流程引擎关键信息
			...
			businessInfo:"报废批复审批流程>报废批复单名称:"+scrape_report_name+";报废批复单号:"+scrape_report_no+">",
...
//上报数据导出
 function exportDevData(){
 	...
 	var submitStr="scrape_report_id="+ids+"&exportFlag="+exportFlag;
 ...
 function toSeeScrapePage(){
	...
	popWindow('<%=contextPath%>/dmsManager/scrape/scrapeReportSee.jsp?scrape_report_id='+ids,'','报废上报');
 } --%>
