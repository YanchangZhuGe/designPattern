			      		...
			      		<table id="table1" width="100%" border="0" cellspacing="2" cellpadding="0" class="tab_line_height">
			         		<tr>
						  	...
						  	<tr>
								...
								<td class="inquire_form6"><input id="other_num" name="other_num" class="input_width" type="text" readonly/></td>
							...
							<tr>
								...
								<td class="inquire_form6"><input id="new_using_num" name="new_using_num" class="input_width" type="text" readonly="readonly" /></td>
...
<script type="text/javascript"> 
	...
	function loadDataDetail(){
		...
		if(device_backdet_id!=null){
			var querySql="select backdet.device_backdet_id,backdet.device_backapp_id,backdet.dev_acc_id,backdet.dev_name,backdet.dev_model,";
		        ...
		        querySql+="unitsd.coding_name as dev_unit_name,outacc.device_id,outacc.type_id,";
		    	querySql+="accdui.dev_unit,apporg.org_id as apporg_id,apporg.org_abbreviation as apporg_name, ";
		    	querySql+="revorg.org_id as revorg_id,revorg.org_abbreviation as revorg_name,app.project_info_id as project_info_no, ";
		    	...
		    	querySql+="from gms_device_collbackapp_detail backdet ";
		    	...
		    	querySql+="left join comm_org_information revorg on app.receive_org_id = revorg.org_id ";//接收单位
		    	...
		    	querySql+="left join gms_device_coll_account acc on accdui.dev_name=acc.dev_name and accdui.dev_model=acc.dev_model and acc.ifcountry != '国外' and app.receive_org_id=acc.usage_org_id ";//公司级台账使用状态
		    	...
		    	querySql+="left join gms_device_coll_account_tech tech on tech.dev_acc_id=acc.dev_acc_id ";//公司级台账技术状况
				querySql+="where backdet.device_backdet_id='"+device_backdet_id+"'";
		...
		document.getElementById("back_num").value =retObj[0].back_num;
		...
		document.getElementById("other_num").value =retObj[0].back_num;
		var oldothernum = retObj[0].old_other_num;
		if(oldothernum==null ||oldothernum==""){
			oldothernum = 0;
		}
		var newothernum = parseInt(oldothernum,10)+parseInt(retObj[0].back_num,10);
		document.getElementById("new_other_num").value =newothernum;
		document.getElementById("new_unusing_num").value =retObj[0].old_unusing_num;
		
		document.getElementById("old_good_num").value =retObj[0].good_num;
		document.getElementById("new_good_num").value =retObj[0].good_num;
		//给毁损的留着
		...
		if(document.getElementById("old_unusing_num").value==""){
			...
			document.getElementById("new_unusing_num").value ="0";
		}
		...
		if(document.getElementById("old_good_num").value==""){
			...
			document.getElementById("new_good_num").value ="0";
		}
	...
	function submitInfo(){
		...
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSubmitCollectStock.srq?device_backdet_id="+device_backdet_id+"&devaccId="+devaccId+"&device_backapp_id="+device_backapp_id+"&projectInfoNo="+projectInfoNo;
