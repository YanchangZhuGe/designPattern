...
<body style="background:#fff" onload="refreshData()">
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <%if(proc_status){ %>
			   <auth:ListButton functionId="OP_TARGET_PER_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_PER_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_PER_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			   <%} %>
			  </tr>
			</table>
			</td>
			...
			<div id="table_box">
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			            ...
			            <td  class="bt_info_even">起始地点</td>
			            <td class="bt_info_odd">到达地点</td>
			             <td class="bt_info_even">单价（元）</td>		
			             <td class="bt_info_odd">其他费用（元）</td>	
			             <td class="bt_info_even" onclick="getSum()">总计(元)</td>			
			        </tr>            
			   </table>
			</div>
			<div id="fenye_box">
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getTransportHShareInfo&projectInfoNo=<%=projectInfoNo%>";
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareTransportHEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	if (ids == '') {
		...
		popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareTransportHEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	ids = ids.replace(/\,/g,"','");
	var sql = "update bgp_op_cost_tartet_transport_h t set t.bsflag='1' where t.cost_transport_id in('"+ids+"')";
 ...
 function toShare(){
  ...
  function getSum(){
		...
		var querySql = "select sum(nvl(to_char(t.person_num*t.person_money-(-other_money),'9999999999999999.00'),0)) sum_value"+
			" from bgp_op_cost_tartet_transport_h t where t.project_info_no='<%=projectInfoNo%>'"+
