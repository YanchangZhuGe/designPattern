...
var orgId="<%=orgId%>";
  ...
  function toEdit(){
	    ...
	    var subflag = tempa[3];
	   	...
	   	if(subflag == '0'){
	    	window.location = "<%=contextPath%>/market/week/editWeekReport.srq?orgId="+org_id+"&week_date="+week_date+"&week_end_date="+week_end_date+"&action=edit&isSecond=false";
	    }else{
	    	alert("该记录已经提交，不能修改!"); 
	    	return; 
	    }
	...
	function toDelete(){
	    ...
	    if(subflag == '0'){
	    	var sql = "update bgp_wr_martandproject_info set bsflag='1' where type='1' and org_id='" + org_id + "' and to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
			deleteEntities(sql);
			var title = "在市场信息周报中删除了一条周报开始日期为“"+week_date+"”的信息"
			var operationPlace = "市场信息周报";
	...
	function JcdpButton3OnClick(){
		...
		if(ids==""){
		    ...
		    if(subflag == '0'){
		    	var sql = "update bgp_wr_martandproject_info set subflag='1' where type='1' and org_id='" + org_id + "' and to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
...
function JcdpButton4OnClick(){
	...
	if(ids==""){
	    ...
	    if(subflag == '1'){
	    	...
	    	var sql = "update bgp_wr_martandproject_info set subflag='0' where  bsflag='0' and type='1' and  to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
	    	}else{
		    	var sql = "update bgp_wr_martandproject_info set subflag='0' where  type='1' and bsflag='0' and org_id='<%=orgId%>' and to_char(week_date,'yyyy-MM-dd')='" + week_date + "'";
	    	}
...
function page_init(){
	...
	cdt_init();
	cruConfig.queryStr = "select distinct i.week_date,i.week_end_date,i.org_id,i.subflag,oi.org_name from bgp_wr_martandproject_info i join comm_org_information oi on oi.org_id = i.org_id where i.bsflag='0' and i.type='1' and i.org_id = '<%=orgId%>' order by i.week_date desc";
	cruConfig.currentPageUrl = "/pm/wr/wrBudGet/wrMartandprojectInfo.lpmd";
	queryData(1);
}

function toAdd(){
	window.location="<%=contextPath%>/market/budgetWeek/martSelectWeek.jsp?orgId=<%=orgId%>&pagerAction=edit2Add&backUrl="+cruConfig.currentPageUrl;
}
...
function linkThirdList(orgId){
	window.location.href="<%=contextPath%>/market/week/weekReport.srq?orgId="+orgId;
}
...
<table border="0"  cellspacing="0"  cellpadding="0"  class="form_info"  width="100%" id="queryRetTable">
	...
	<tr class="bt_info">
		...
		<td class="tableHeader" 	 exp="<a href=javascript:link2self('<%=contextPath%>/market/week/editWeekReport.srq?orgId={org_id}&week_date={week_date}&week_end_date={week_end_date}&action=view&isSecond=false')><font color='3366ff'>{week_date}</font></a>">周报开始日期</td>
