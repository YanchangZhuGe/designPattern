				...
				<tr>
					...
					<td background="<%=contextPath%>/images/list_15.png"><table
							...
							<tr>
								...
								<td align="right"><a
									href="<%=contextPath%>/mat/singleproject/mattemplate/download.jsp?path=/rm/dm/kkzy/mutiPorject/repMat.xlsx">下载模板</a>
								</td>
...
<script type="text/javascript">
	...
	function refreshData(){
		var sql='';
		 	sql +="select  distinct  i.* ,c.code_name,tt.postion,tt.recyclemat_info, tt.stock_num,tt.actual_price,tt.wz_sequence from (select t.recyclemat_info, t.wz_id,t.wz_sequence,t.stock_num,t.actual_price ,t.postion from gms_mat_recyclemat_info t where t.bsflag = '0' and t.wz_type = '3'   and t.project_info_id  is null    and t.org_subjection_id like '"+orgSubjectionId+"%' ) tt left  join (gms_mat_infomation i inner join gms_mat_coding_code c on i.coding_code_id=c.coding_code_id and i.bsflag='0' and c.bsflag='0') on tt.wz_id=i.wz_id order by tt.postion asc ,  i.coding_code_id asc ,i.wz_id asc";
	...
	function loadDataDetail(shuaId){
		var retObj;
		if(shuaId!= undefined){
			 retObj = jcdpCallService("DevInsSrv", "getMatReps", "laborId="+shuaId);
		    ...
		    if(ids==''){ 
		     ...
		     retObj = jcdpCallService("DevInsSrv", "getMatReps", "laborId="+ids);
		}
		//取消选中框--------------------------------------------------------------------------
	...
	function chooseOne(cb){
 	...
 	function toAdd(){
 	 	popWindow("<%=contextPath%>/rm/dm/kkzy/mutiPorject/repMatLedgerEdit.jsp?orgSubjectionId="+orgSubjectionId+"&orgId="+orgId,'1024:800');
 	 	}
	...
	function toUpdate(){
	    ...
	    if(ids==''){ alert("请先选中一条记录!");
			...
			form.action="<%=contextPath%>/mat/multiproject/matLed/repmat/repMatLedgerUpdateDetail.srq?ids="
					+ ids;
			form.submit();
		}
	}
