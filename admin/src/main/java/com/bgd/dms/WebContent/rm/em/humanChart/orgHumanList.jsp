      	...
      	<div id="list_table">
			...
			<div id="table_box" style="height:175px">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{projectInfoNo},{projectType}' id='rdo_entity_id_{projectInfoNo}'  />" >选择</td>
			      <td class="bt_info_even" autoOrder="1">序号</td>
			      <td class="bt_info_even" exp="{projectName}">项目名称</td>
			      <td class="bt_info_odd" exp="{orgName}">施工队伍</td>
		          <td class="bt_info_even" exp="{manageOrgName}">甲方单位</td>
			      <td class="bt_info_odd" exp="{marketClassifyName}">市场范围</td>
			    <!--   <td class="bt_info_even" exp="{nn}">用工总数</td> -->
			      <%
			      if(paramS.equals("0110000019000000001")){
			      %> 
			      <td class="bt_info_odd" exp="{n1}">合同化在岗人数</td>
			      <%}else if(paramS.equals("0110000019000000002")){ %>
			      <td class="bt_info_even" exp="{n2}">市场化在岗人数</td>
			      <%
			      }
			      %> 
<!-- 			      <td class="bt_info_odd" exp="{n3}">临时季节性</td>
			      <td class="bt_info_even" exp="{n4}">劳务用工</td>
			      <td class="bt_info_odd" exp="{n5}">再就业</td> -->
			     </tr> 			        
			  </table>
			</div>
				...
				<div id="div1" style="width:100%; height:290px; overflow:scroll">
					<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
					  ...
					  <tr>		
					    ...
					    <td class="ali_cdn_input">   <input type='hidden' id="ptype_value" name="ptype_value" value="5000100004000000001"/><select class="select_width" id="s_team" name="s_team"   onchange="changeTeam()"></select></td>	
			...
			<div id="div2" style="display:none;width:100%; height:290px;overflow:scroll" >
		    	...
		    	<tr class="bt_info">
		            ...
		            <td>离开项目时间</td>	
	           		...
	           		<div id="div3" style="display:none;width:100%; height:290px; overflow:scroll">
						<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
						  <tr>
						    <td width="6"><img src="<%=contextPath%>/images/list_13.png" width="6" height="36" /></td>
						    <td background="<%=contextPath%>/images/list_15.png"><table width="100%" border="0" cellspacing="0" cellpadding="0">
...
<script type="text/javascript">
	...
	cruConfig.queryOp = "queryOrgProjectHuman";
	
	function refreshData(arrObj){
			
		cruConfig.submitStr = "orgSub=<%=orgSub%>&employeeGz=<%=paramS%>";	
		cruConfig.currentPageUrl = "/rm/em/humanChart/orgHumanList.jsp";
		queryData(1);
    ...
    function loadDataDetail(ids){ 
      ...
      //  var s_employee_gz = document.getElementsByName("s_employee_gz")[0].value;
    	...
    	myChart2.setXMLUrl("<%=contextPath%>/rm/em/getChart18.srq?projectInfoNo="+ids.split(",")[0]+"&employeeGz=<%=paramS%>");   
    ...
    function getPostList(team){  
    	ids = getSelectedValue();
    	var myChart3 = new FusionCharts( "${applicationScope.fusionChartsURL}/Charts/Column3D.swf", "myChartId3", "100%", "250", "0", "0" );  
    	myChart3.setXMLUrl("<%=contextPath%>/rm/em/getChartPost18.srq?projectInfoNo="+ids.split(",")[0]+"&team="+team+"&employeeGz=<%=paramS%>");   
	...
	function getHumanList(post){
	 ...
	 //   var s_employee_gz = document.getElementsByName("s_employee_gz")[0].value;
		...
		var querySql = "select rownum ,p.* from ( select p.EMPLOYEE_ID, p.EMPLOYEE_NAME,p.employee_gz, d1.coding_name team_name,d2.coding_name work_post_name,p.ACTUAL_START_DATE,p.ACTUAL_END_DATE   from view_human_project_relation p  left join comm_coding_sort_detail d1 on p.TEAM = d1.coding_code_id   left join comm_coding_sort_detail d2 on p.WORK_POST = d2.coding_code_id  where p.project_info_no = '"+ids.split(",")[0]+"'    and p.actual_start_date is not null and p.actual_end_date is null   and p.WORK_POST ='"+post+"' and  p.EMPLOYEE_GZ='<%=paramS%>' ) p ";
