      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td class="inquire_form4">
          	<select name="device_backapp_id" id="device_backapp_id" class="select_width" disabled></select>
          	<input name="device_mixinfo_id" id="device_mixinfo_id" type="hidden" value="<%=devicemixinfoid%>">
          </td>
        ...
        <tr>
          ...
          <td class="inquire_form4">
          	<input name="own_org_name" id="own_org_name" class="input_width" type="text" readonly/>
...
<script type="text/javascript"> 
	...
	$().ready(function(){
		var devicemixinfoid = '<%=devicemixinfoid%>';
		$("#addProcess").click(function(){
			...
			var condition  = "and (bad.device_mixinfo_id is null or bad.device_mixinfo_id = '"+devicemixinfoid+"') ";
			alert("<%=contextPath%>/rm/dm/tree/selectAccountForBackMixForm.jsp?own_org_id="+own_org_id+"&projectinfono="+project_info_no+"&condition="+condition);
			var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/tree/selectAccountForBackMixForm.jsp?own_org_id="+own_org_id+"&projectinfono="+project_info_no+"&condition="+condition,paramobj,"dialogWidth=820px;dialogHeight=380px");
...
<script type="text/javascript"> 
	...
	function refreshData(){
		...
		//查询调配单信息和明细信息，进行回填
		var dataSql = "select detail.device_backdet_id,detail.dev_coding,dui.dev_name,dui.dev_model,detail.self_num,";
		dataSql += "detail.dev_sign,detail.license_num,detail.actual_in_time,detail.planning_out_time,";
		dataSql += "bif.device_backapp_id,bif.project_info_no,pro.project_name,bif.usage_org_id,bif.own_org_id,";
		dataSql += "bif.backmixinfo_no,org.org_name as usage_org_name,ownorg.org_name as own_org_name ";
		dataSql += "from gms_device_backapp_detail detail ";
		dataSql += "left join gms_device_backinfo_form bif on bif.device_mixinfo_id=detail.device_mixinfo_id ";
		dataSql += "left join gp_task_project pro on bif.project_info_no=pro.project_info_no ";
		dataSql += "left join gms_device_account_dui dui on detail.dev_acc_id = dui.dev_acc_id ";
		dataSql += "left join comm_org_information org on org.org_id=bif.usage_org_id ";
		dataSql += "left join comm_org_information ownorg on ownorg.org_id=bif.own_org_id ";
		dataSql += "where detail.device_mixinfo_id='<%=devicemixinfoid%>' and bif.bsflag='0' ";
		
		var dataRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+dataSql);
		retObj = dataRet.datas;
		
		if(retObj!=undefined && retObj.length>=1){
			for(var i=0;i<retObj.length;i++){
				if(i==0){
					//回填基本信息
					$("#device_backapp_id").val(retObj[0].device_backapp_id);
				...
				//回填申请明细信息
				var innerhtml = "<tr id='tr"+retObj[0].device_backdet_id+"' name='tr'>";
				innerhtml += "<td><input type='checkbox' name='idinfo' id='"+retObj[0].device_backdet_id+"'/></td>";
				innerhtml += "<td>"+(i+1)+"<input type='hidden' id='backdetid'"+i+" name='backdetid' value='"+retObj[0].device_backdet_id+"'></td>";
				innerhtml += "<td>"+retObj[0].dev_coding+"</td>";
				innerhtml += "<td>"+retObj[0].dev_name+"</td>";
				innerhtml += "<td>"+retObj[0].dev_model+"</td>";
				innerhtml += "<td>"+retObj[0].self_num+"</td>";
				innerhtml += "<td>"+retObj[0].dev_sign+"</td>";
				innerhtml += "<td>"+retObj[0].license_num+"</td>";
				innerhtml += "<td>"+retObj[0].actual_in_time+"</td>";
				innerhtml += "<td>"+retObj[0].planning_out_time+"</td>";
				innerhtml += "</tr>";
				$("#detailtable").append(innerhtml);
			}
			$("#detailtable>tr:odd>td:odd").addClass("odd_odd");
