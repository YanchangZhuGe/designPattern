...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.queryStr = " select t.* from ( select t.file_id,t.file_name,t.ucm_id,to_char(t.upload_date,'yyyy-MM-dd') upload_date,e.employee_name,t.doc_file_type,d.coding_name from bgp_doc_gms_file t left join comm_human_employee e on t.creator_id=e.employee_id and e.bsflag='0' left join comm_coding_sort_detail d on t.doc_file_type=d.coding_code_id and d.bsflag='0'   where t.bsflag='0' and t.doc_type='<%=codingCodeId%>' order by t.upload_date desc ) t where 1=1 ";
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/newStandardDoc/add_tdDocModify.jsp?codingCodeId='+codingCodeId,'800:600');
    }
	...
	function toEdit() {
		...
		popWindow('<%=contextPath%>/td/newStandardDoc/add_tdDocModify.jsp?id='+ids,'800:600');
	}
