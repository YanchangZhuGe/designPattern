...
function page_init(){
	...
	cruConfig.queryStr = "select p.duty, p.PERSON_ID, p.NAME, p.OFFICE_PHONE, p.order_no from ci_person p,sm_org sm where p.corp_id = sm.org_id and sm.bgp_infomation_type_id = '${param.typeId}' order by to_number(p.order_no)";
	cruConfig.currentPageUrl = "/market/showModel/showCompet.lpmd";
	queryData(1);
}
...
<table border="0"  cellspacing="0"  cellpadding="0"  class="form_info"  width="100%" id="queryRetTable">
	...
	<tr class="bt_info">
		<td class="notNews" 	 exp="{name}">姓名</td>
		<td class="notNews" 	 exp="{duty}">职务</td>
		<td class="notNews" 	 exp="{office_phone}">办公室电话</td>
	</tr>
	</thead>
	<tbody>
