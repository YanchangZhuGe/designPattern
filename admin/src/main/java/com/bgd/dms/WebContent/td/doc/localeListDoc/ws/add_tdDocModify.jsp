...
function save(){
	...
	form.action = "<%=contextPath%>/td/ws/toSavelocaleListDoc.srq?businessType=<%=businessType%>";
	form.submit();
	newClose();
}
...
function checkForm(){
	...
	if (!isLimitB50("written_unit","接收单位")) return false;  
	if (!isTextPropertyNotNull("auditor", "接收人")) return false;
	if (!isLimitB20("auditor","接收人")) return false; 

	if(fileId!='null'){ 
		if (!isTextPropertyNotNull("pk_5000100106000000003_0", "上交资料明细")) return false;
	}else{
		if (!isTextPropertyNotNull("5000100106000000003_0", "上交资料明细")) return false;	
		...
		if(datas!=null && datas.length>0){
			...
			for(var i=0;i<datas.length;i++){
				if(""!=datas[i].ucm_id){
					...
					document.getElementById("pk_5000100106000000003_"+i).value =ucmId;
...
function addRow(){
	...
	var tr=document.all.tableDoc.insertRow();
  	tr.insertCell(0).innerHTML="<input type='hidden' id='pk_5000100106000000003_"+num+"' name='pk_5000100106000000003_"+num+"' value=''/>";
  	
	var td = tr.insertCell(1);
	td.setAttribute("colspan",3);
	td.innerHTML="<input type='file' name='5000100106000000003_"+num+"' id='5000100106000000003_"+num+"' class='input_width'/><div id='down_5000100106000000003_"+num+"'></div>";
...
function delRow(){
	//获得当前点击标签的附件ucm_id值 
    var oldUcmId=$("#pk_5000100106000000003_"+event.srcElement.name).val();
    //如果存在旧文件，则先删除
							...
							<table id="tableDoc" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
								...
								<tr>
									...
									<td class="inquire_form6">
										<input type="text" id="written_time" name="written_time" value="<%=curDate%>" class="input_width" readonly="readonly"/>
								...
								<tr>
									<td class="inquire_item6"><span class="red_star">*</span>上交资料明细：</td>
									<td colspan="3">
										<input type="hidden" id="pk_5000100106000000003_0" name="pk_5000100106000000003_0" value=""/>
										<input type="file" name="5000100106000000003_0" id="5000100106000000003_0" class="input_width"/>
