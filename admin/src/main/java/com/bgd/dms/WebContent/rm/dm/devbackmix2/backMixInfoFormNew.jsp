...
<script type="text/javascript"> 
	function submitInfo(state){		
		...
		//给调配单号设置成空
		$("#mixinfo_no").val("");
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveBackMDFInfoNew.srq?state="+state+"&detailcount="+detailcount;
	...
	function toMixDetailInfos(){
		...
		var str = "select dui.dev_name,dui.dev_model,dui.dev_type,detail.device_backdet_id,detail.dev_acc_id,detail.dev_position,";
		...
		if(retObj!=undefined){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td><input name='devicename"+index+"' id='devicename"+index+"' style='line-height:15px' value='"+retObj[index].dev_name+"' size='9' type='text' readonly/>";
				innerhtml += "<input name='devicebackdetid"+index+"' id='devicebackdetid"+index+"' value='"+retObj[index].device_backdet_id+"' type='hidden' />";
				innerhtml += "<input name='devcicode"+index+"' id='devcicode"+index+"' value='"+retObj[index].dev_type+"' type='hidden' /></td>";
				innerhtml += "<td><input name='devicemodel"+index+"' id='devicemodel"+index+"' value='"+retObj[index].dev_model+"' size='10' type='text' readonly/></td>";				
