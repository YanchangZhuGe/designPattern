      ...
      <table border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        ...
        <td class="inquire_item4">转移申请单号:</td>
          <td class="inquire_form4">
          	...
          	<input name="dev_mov_id" id="dev_mov_id" class="input_width" type="hidden" value="" />
          </td>
...
<script type="text/javascript">
	$().ready(function(){
		$("#addProcess").click(function(){	
			...
			var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/collMove/selectAccountForBack.jsp?projectinfono=<%=projectInfoNo%>&backdevtype="+backdevtype,paramobj,"dialogWidth=820px;dialogHeight=380px");
			...
			var proqueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+devdetSql);
				...
				<%-- for(var index=0;index<retObj.length;index++){
					...
					innerhtml += "<td width='4%'><input type='checkbox' name='idinfo' id='"+retObj[index].dev_acc_id+"'/></td>";
					...
					innerhtml += "<td width='15%'>"+retObj[index].asset_coding+"</td>";
	...
	function refreshData(){
		...
		var proSql = "select t.dev_mov_id,t.dev_mov_no,t.out_project_info_id,t.in_project_info_id,t.dev_mov_name,";
			proSql += "outgp.project_name as out_progect_name,ingp.project_name as in_project_name,";
			proSql += "u.user_name as opertor,t.opertor_id,t.apply_date,";
			proSql += "case when t.out_project_info_id='"+projectInfoNos+"' then '转出' else '转入' end as mov_state,";
			proSql += "case wfmiddle.proc_status when '1' then '待审批' when '3' then '审批通过' when '4' then '审批不通过' else '未提交' end as state_desc ";
			proSql += "from gms_device_move t left join gp_task_project outgp on t.out_project_info_id=outgp.project_info_no left join gp_task_project ingp on t.in_project_info_id=ingp.project_info_no left join p_auth_user u on t.opertor_id=u.user_id left join common_busi_wf_middle wfmiddle on t.dev_mov_id=wfmiddle.business_id where (t.out_project_info_id='"+projectInfoNos+"' or t.in_project_info_id='"+projectInfoNos+"') and t.bsflag='0' and t.dev_mov_id='"+devMovId+"'";		
		...
		if(proqueryRet!=null && proqueryRet.returnCode =='0'){
			...
			$("#project_name").val(retObj[0].out_progect_name);
			...
			$("#dev_mov_id").val(retObj[0].dev_mov_id);
		}
		//查询转移设备明细
		if(devMovId!=null && devMovId!=''){
		
           var devdetSql = "select dui.dev_acc_id,dui.dev_name,dui.dev_model,d.coding_name as unit_name,dui.total_num,dui.unuse_num,";
               devdetSql += "nvl(dui.use_num,0) as use_num,t.mov_num as back_num,dui.actual_in_time,t.actual_out_time ";
               devdetSql += "from gms_device_move_detail t left join gms_device_coll_account_dui dui on t.dev_acc_id=dui.dev_acc_id ";
               devdetSql += "left join comm_coding_sort_detail d on dui.dev_unit=d.coding_code_id ";       	   
        	   devdetSql +=	"where t.dev_mov_id = '"+devMovId+"' ";
