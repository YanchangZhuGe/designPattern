...
<%
	...
	String outprojectinfono = request.getParameter("outprojectinfono");
...
<script type="text/javascript">
	...
	var outprojectInfoNos = '<%=outprojectinfono%>';
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model,license_num,self_num,dev_sign){
		var str = "select account.dev_acc_id,account.asset_coding,account.dev_coding,account.self_num,account.dev_sign, ";
		str += "account.license_num,account.actual_in_time,account.planning_out_time,account.dev_name,account.dev_model ";
		str += "from gms_device_account_dui account ";
		str += "left join gms_device_move out_ve on account.fk_wells_transfer_id = out_ve.dev_mov_id ";
		str += "left join gp_task_project out_pro on out_pro.project_info_no = out_ve.out_project_info_id ";
		str += "where account.bsflag = '0' and account.transfer_state = '0' and account.project_info_id='"+projectInfoNos+"' and out_ve.out_project_info_id='"+outprojectInfoNos+"' and account.actual_out_time is null ";

		if(obj.selectStr != null){
			str += "and account.dev_acc_id not in ("+obj.selectStr+")";
		}
		str += "and account.account_stat!='0110000013000000005' ";
		str += "and (account.repair_state != '1' or account.repair_state is null) and account.is_leaving='0' ";
