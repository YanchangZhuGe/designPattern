			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      ...
			      <%} %>
			      <td class="bt_info_odd" exp="<a style='color:blue;' href=<%=contextPath%>/doc/downloadDocByUcmId.srq?docId={ucm_id}&emflag=0>{file_name}</a>">文档名称</td>
...
<script type="text/javascript">
	...
	function refreshData(str){
		...
		cruConfig.queryStr = "select t.* from (select t.project_info_no,t.tecnical_id,t.title,t.written_time,t.test_start_date,t.writer,t.auditor,t.memo,f.ucm_id,f.file_name,p.project_name "+
			...
			"left join bgp_doc_gms_file f on t.tecnical_id = f.relation_id and f.bsflag = '0' "+
			"where t.bsflag = '0' "+projStr+" and t.business_type = '<%=businessType%>' and dy.org_subjection_id like '"+orgCode+"%' "+str+" order by t.modifi_date desc) t";
		queryData(1);
	}
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/doc/memorandumDoc/wt/add_tdDocModify.jsp?businessType='+businessType+'&fileAbbr='+fileAbbr+'&isSingle='+isSingle,'800:680');
    }
	...
	function toEdit() {
		...
		popWindow('<%=contextPath%>/td/doc/memorandumDoc/wt/add_tdDocModify.jsp?businessType='+businessType+'&id='+id+'&isSingle='+isSingle,'800:680');
	}
