...
function savejsp(){
	...
	for(var i=0;i<rowNum;i++){
		...
		var org_id = document.getElementsByName("org_id_"+i)[0].value;			
		...
		var practice_date = document.getElementsByName("practice_date_"+i)[0].value;
		...
		rowParam['practice_num'] = practice_num;
		rowParam['practice_date'] = practice_date;
...
function addLine(certificate_nos,employee_ids,employee_names,work_units,org_names,certificate_nums,training_institutionss,issuing_agencys,issuing_dates,validitys,practice_nums,practice_dates,qualification_levels,document_ids,qualification_names){
	...
	if(practice_dates != null && practice_dates != ""){
		practice_date=practice_dates;
	}
	...
	var issuingDates = "issuingDate_"+rowNum;
	var validitys = "validity_"+rowNum;
	var practiceDates = "practiceDate_"+rowNum;
	...
	td.className=classCss+"even";
	td.innerHTML = '<input type="text"  readonly="readonly"  name="issuingDate' + '_' + rowNum + '"  id="issuingDate' + '_' + rowNum + '" value="'+issuing_date+'" onFocus="this.select()"/>'+'<img src="<%=contextPath%>/images/calendar.gif" id="tributton1'+rowNum+'" width="16" height="16" style="cursor: hand;"'+ 'onmouseover="calDateSelector('+issuingDates+',tributton1'+rowNum+');" />';
	
	var td = tr.insertCell(7);
	td.className=classCss+"odd";
	td.innerHTML = '<input type="text" readonly="readonly"  name="validity' + '_' + rowNum + '"  id="validity' + '_' + rowNum + '" value="'+validity+'" onFocus="this.select()"/>'+'<img src="<%=contextPath%>/images/calendar.gif" id="tributton2'+rowNum+'" width="16" height="16" style="cursor: hand;"'+ 'onmouseover="calDateSelector('+validitys+',tributton2'+rowNum+');" />'+'<input type="hidden"  class="input_width" name="bsflag' + '_' + rowNum + '" id="bsflag' + '_' + rowNum + '" value="0"/>';

	var td = tr.insertCell(8);
	td.className=classCss+"even";
	td.innerHTML = '<input type="text"   name="practiceNum' + '_' + rowNum + '"  id="practiceNum' + '_' + rowNum + '" value="'+practice_num+'" onFocus="this.select()"/>';
	
	var td = tr.insertCell(9);
	td.className=classCss+"odd";
	td.innerHTML = '<input type="text" readonly="readonly"   name="practiceDate' + '_' + rowNum + '" id="practiceDate' + '_' + rowNum + '" value="'+practice_date+'" onFocus="this.select()"/>'+'<img src="<%=contextPath%>/images/calendar.gif" id="tributton3'+rowNum+'" width="16" height="16" style="cursor: hand;"'+ 'onmouseover="calDateSelector('+practiceDates+',tributton3'+rowNum+');" />'
	...
	td.className=classCss+"even";
	td.innerHTML = '<input type="hidden" name="order" value="' + rowNum + '"/>'+'<span class="sc"><a href="#" onclick="deleteLine(\'' + tr.id + '\')"></a></span>';
...
//	var action = '<%=request.getParameter("action")%>';
	if(certificate_no!='null'){
		var querySql = "select s.certificate_no,e.employee_id,e.employee_name,s.work_unit,i.org_name,s.certificate_num,s.qualification_name,s.training_institutions,s.issuing_agency,s.issuing_date, s.validity,s.practice_num,s.practice_date ,s.qualification_level,s.document_id from bgp_comm_human_certificate s left join comm_human_employee e on s.employee_id = e.employee_id and e.bsflag = '0' left join comm_org_information i on s.work_unit = i.org_id  and i.bsflag = '0' where s.bsflag = '0' and s.certificate_no='<%=request.getParameter("id")%>'  order by s.modifi_date";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var datas = queryRet.datas;
		for (var i = 0; datas && queryRet.datas.length; i++) {			
			addLine(datas[i].certificate_no,datas[i].employee_id,datas[i].employee_name,datas[i].work_unit,datas[i].org_name,datas[i].certificate_num,datas[i].training_institutions,datas[i].issuing_agency,datas[i].issuing_date,datas[i].validity, datas[i].practice_num,datas[i].practice_date,datas[i].qualification_level,datas[i].document_id,datas[i].qualification_name);
		}			
	}
...
function importData(){
	var obj=window.showModalDialog('<%=contextPath%>/rm/em/commCertificate/certImportFile.jsp',"","dialogHeight:500px;dialogWidth:600px");

	if(obj!="" && obj!=undefined ){		
		var checkStr = obj.split(",");
		for(var i=0;i<checkStr.length-1;i++){
			var check = checkStr[i].split("@");
			addLine("",check[11],check[12],check[13],check[14],check[3],check[4],check[5],check[6],check[7],check[8],check[9],check[10],"",check[2]);
		}
...
<div style="width:1000;overflow-x:scroll;overflow-y:scroll;">			
	<table id="lineTable" width="1500" border="0" cellspacing="0" cellpadding="0" style="margin-top:2px;" >
    	<tr>
            ...
            <td class="bt_info_odd">工作单位</td>
            <td class="bt_info_even">资格证名称</td>
            ...
            <td class="bt_info_even">执业证号</td>
            <td class="bt_info_odd">执业证发证日期</td>
            <td class="bt_info_even">资格级别</td>
            <td class="bt_info_odd">文档上传</td>
            <td class="bt_info_even" style="display:<%=addButtonDisplay%>">操作
