			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_appdet_id_{device_allapp_id}' name='device_allapp_id' idinfo='{device_allapp_id}'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' value='{device_addapp_id}~{allapp_type}~{business_type}~{project_info_no}~{mid}' id='selectedbox_{device_addapp_id}' />" >选择</td>
					<td class="bt_info_even" autoOrder="1">序号</td>
					<td class="bt_info_odd" exp="<a onclick=viewProject('{project_info_no}')>{project_name}</a>">项目名称</td>
					<td class="bt_info_even" exp="{device_addapp_no}">计划单号</td>
					<td class="bt_info_odd" exp="{device_addapp_name}">计划名称</td>
					...
					<td class="bt_info_odd" exp="<a onclick=viewStore('{device_addapp_id}','{assign_state}','{allapp_type}')>查看</a>">调配进度</td>					
			     </tr> 
			     <tr>
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(info[0] != undefined && idinfo == currentid){
				...
				str += "where devapp.device_addapp_id = '"+info[0]+"' and alldet.bsflag='0' ";
				str += "union all "
				str += "select alldet.device_allapp_detid,alldet.project_info_no,sd.coding_name as unitname,teamsd.coding_name as teamname, ";
...
//	if(pro_dep=="C6000000005605"){businessType = "5110000004100001021";}//塔里木项目部
    ...
    function refreshData(v_project_name,v_org_name,v_assign_state){
		...
		//
		var str = "select r.mid,wf.business_type,pro.project_name,devapp.device_addapp_id,devapp.device_addapp_no,devapp.device_addapp_name,devapp.project_info_no,";
			...
			str += "left join gp_middle_resources r on r.dev_id = devapp.device_addapp_id and r.bsflag = '0' ";
			str += "left join common_busi_wf_middle wf on r.mid = wf.business_id and wf.bsflag = '0' ";
			str += "and wf.business_type in('5110000004100001015','5110000004100001012','5110000004100001013','5110000004100001020','5110000004100001018','5110000004100001017') ";
			...
			str += "where r.supplyflag = '0' and wf.proc_status = '3' and devapp.bsflag = '0' ";
    ...
    function loadDataDetail(shuaId){
		...
		if(info[0]!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getZhDevAllAppBaseInfo", "deviceaddappid="+info[0]+"&pbflag=N&busitype="+info[2]);			
		}else{
	...
	function toModifyDetail(obj){
		...
		if(info != ''){
	   		popWindow('<%=contextPath%>/rm/dm/devPlanMulti/zhAddDevListOutStoreDet.jsp?projectInfoNo='+info[3]+'&deviceaddappid='+info[0]+'&allapp_type='+info[1],'1050:680');
		}
	}
