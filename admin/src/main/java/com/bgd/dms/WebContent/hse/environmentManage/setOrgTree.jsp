...
if(datas.length>1){
	...
	environmentFlag = queryOrgRet.datas[1].environment_flag;
}else if(datas.length=="1"){
	rootMenuName = queryOrgRet.datas[0].org_abbreviation; 
	rootMenuId  = queryOrgRet.datas[0].org_sub_id; 
	environmentFlag = queryOrgRet.datas[0].environment_flag;
}
...
//	alert(parentNode.id.substring(15));
	...
	Ext.Ajax.request({
		...
		params : {
			currentPage:"1",
			pageSize:"10000",
			querySql:"select ho.org_sub_id,ho.father_org_sub_id,ho.environment_flag,os.org_id,oi.org_abbreviation, case when (select father_org_sub_id from bgp_hse_org where org_sub_id='"+parentNode.id+"') = 'C105' then '1' else '' end is_leaf  from bgp_hse_org ho left join comm_org_subjection os on ho.org_sub_id=os.org_subjection_id and os.bsflag='0' left join comm_org_information oi on os.org_id=oi.org_id and oi.bsflag='0' where ho.father_org_sub_id='"+parentNode.id+"' "
		},
		method : 'Post',
...
//	if(nodeData.is_leaf=="1"){//叶子节点
	...
	if(nodeData.environment_flag=="1"){
...
//   location.href="<%=contextPath%>/hse/yxControl/setEnvironmentOrg.srq?ids="+checkIds+"&&rootMenuId="+rootMenuId;
   jcdpCallService("HseSrv", "setEnvironmentOrg", "ids="+checkIds+"&rootMenuId="+rootMenuId);
   window.close();
}  
...
<table border="0" cellpadding="0" cellspacing="0"  id="FilterLayer" width="100%">
	<tr>
		<td align="left" style="padding-left:20px;padding-top:5px;padding-bottom:5px;">
