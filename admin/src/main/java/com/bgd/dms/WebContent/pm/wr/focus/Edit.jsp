...
<%
	...
	//String[] ins_names={"全站仪","GPS"};
	//String[] ins_ids={"5000300003000000025","5000300003000000026"};
	...
	String sql="select coding_code_id,coding_name from comm_coding_sort_detail where coding_sort_id = '5000300003' and bsflag = '0' and (coding_code like '04%' or coding_code like '05%') order by coding_code,coding_code_id";
	...
	for(int i=0,j=0;it.hasNext();i++){
		...
		if(tmp2.equals("5000300003000000019")){//国内可控震源 继续
			continue;
		}else if(tmp2.equals("5000300003000000024")){//国内测量仪器 跳出
			break;
		}else{
			s1.add(j, tmp1);	
			s2.add(j, tmp2);
	...
	Object[] ins_names=s3.toArray(); 
	Object[] ins_ids=s4.toArray(); 
...
<div id="c_div" style="margin-top:2px;">
  <table border="0" cellpadding="0" cellspacing="0" class="tab_line_height" width="100%" style="margin-top:-1px;">
	...
	<th rowspan="3"  style="text-align: center;background-color: #e8eaeb" >测量仪器</th>
	<%for(int i=0;i<ins_names.length;i++) {%>
	<tr <%if(i%2==0) {%>class="odd" <%}else  {%> class="even" <%} %>  >
	  <td ><%=ins_names[i] %> <input name="<%="focus_id"+i%>" value=""  type="hidden"/><input name="<%= "equipment_type"+i%>" value="<%=ins_ids[i] %>"  type="hidden"/></td>
	  ...
	  <td ><input type="text" name="<%= "safety_num"+i%>" value=""  onkeypress="return key_press_check1(this)" style="ime-mode:disabled" onkeydown="fncKeyStop(event)" onpaste="return false" oncontextmenu = "return false;"></td>
...
<div id="c_div" style="margin-top:2px;">
  <table border="0" cellpadding="0" cellspacing="0" class="tab_line_height" width="100%" style="margin-top:-1px;">
	...
	<th rowspan="5"  style="text-align: center;background-color: #e8eaeb;">可控震源</th>
	<%for(int j=0, i=ins_names.length;j<focus_names.length;j++,i++) { %>
	<tr <%if(j%2==0) {%>class="odd" <%}else  {%> class="even" <%} %>  >
	  <td ><%=focus_names[j] %> <input name="<%="focus_id"+i%>" value=""  type="hidden"/><input name="<%= "equipment_type"+i%>" value="<%=focus_ids[j] %>"  type="hidden"/></td>
	  <td ><input type="text" name="<%= "total_num"+i %>" value="" onkeypress="return key_press_check1(this)" style="ime-mode:disabled" onkeydown="fncKeyStop(event)" onpaste="return false" oncontextmenu = "return false;"></td>
	  <td ><input type="text" name="<%= "use_num"+i%>" value="" onkeypress="return key_press_check1(this)" style="ime-mode:disabled" onkeydown="fncKeyStop(event)" onpaste="return false" oncontextmenu = "return false;"></td>
	  <td ><input type="text" name="<%= "plan_num"+i%>" value=""  onkeypress="return key_press_check1(this)" style="ime-mode:disabled" onkeydown="fncKeyStop(event)" onpaste="return false" oncontextmenu = "return false;"></td>
	  <td ><input type="text" name="<%= "trusfer_num"+i %>" value=""  onkeypress="return key_press_check1(this)" style="ime-mode:disabled" onkeydown="fncKeyStop(event)" onpaste="return false" oncontextmenu = "return false;"></td>
...
<script type="text/javascript">
	...
	var length=<%=ins_names.length+focus_names.length%>;
	var len=<%=ins_names.length%>;
	var code_id="<%=code_id%>";
	...
	function save(){
		...
		for(var i=0;i<length;i++){
			...
			if(i<len){
				rowParam['typeid'] = '2';//测量仪器
			}else{
				rowParam['typeid'] = '1';//可控震源
			}
			rowParam['country'] = '1';//国内
	...
	if(data_week_date!='null'){
		...
		var querySql = "select t.focus_id,t.total_num,t.use_num,t.plan_num,t.trusfer_num,t.safety_num,t.equipment_type,t.notes from BGP_WR_FOCUS_INFO t where t.org_id='"+data_org_id+"' and t.week_date = to_date('"+data_week_date+"','yyyy/MM/dd') and equipment_type in ("+code_id+") and t.bsflag='0' and t.country='1' order by typeid desc,equipment_type ";
		...
		if(queryRet.datas.length!='0'){//view,edit,add三种情况，都是先选取本期数据，如果有，进行填充
			...
			querySql = "select t.total_num,t.use_num,t.plan_num,t.trusfer_num,t.safety_num,t.equipment_type,t.notes from BGP_WR_FOCUS_INFO t where t.org_id='"+data_org_id+"' and t.week_date = (select week_date from (select week_date from BGP_WR_FOCUS_INFO where subflag='1' and bsflag='0' order by week_date desc) where rownum='1' ) and equipment_type in ("+code_id+") and t.bsflag='0' and t.country='1' order by typeid desc,equipment_type ";
