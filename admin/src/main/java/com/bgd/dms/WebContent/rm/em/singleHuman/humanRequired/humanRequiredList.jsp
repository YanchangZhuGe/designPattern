...
function refreshData(){
  ...
  cruConfig.queryStr = "select  te.proc_status,   decode(te.proc_status,   '1',   '待审批',  '3',  '审批通过', '4', '审批不通过',te.proc_status) proc_status_name,r.requirement_no, p.project_name, i.org_id, r.apply_company, i.org_name applicant_org_name, r.applicant_id, t2.employee_name applicant_name, To_char(r.apply_date, 'yyyy-MM-dd') apply_date, To_char(r.create_date, 'yyyy-MM-dd hh24:mi') create_date, r.apply_no from bgp_project_human_requirement r   left join common_busi_wf_middle te on    te.business_id=r.requirement_no   and te.bsflag='0'  left join gp_task_project p on r.project_info_no = p.project_info_no   and p.bsflag = '0' left join comm_org_information i on r.apply_company = i.org_id and i.bsflag = '0' left join comm_human_employee t2 on r.applicant_id = t2.employee_id      and t2.bsflag = '0' where r.bsflag = '0' and r.proflag= '0'  and r.project_info_no='<%=projectInfoNo%>'  order by r.create_date desc"; 
...
function simpleSearch(){ 
	...
	if(s_apply_no!=''){		 
		  cruConfig.queryStr = "select  te.proc_status,   decode(te.proc_status,   '1',   '待审批',  '3',  '审批通过', '4', '审批不通过',te.proc_status) proc_status_name,r.requirement_no, p.project_name, i.org_id, r.apply_company, i.org_name applicant_org_name, r.applicant_id, t2.employee_name applicant_name, To_char(r.apply_date, 'yyyy-MM-dd') apply_date, To_char(r.create_date, 'yyyy-MM-dd hh24:mi') create_date, r.apply_no from bgp_project_human_requirement r   left join common_busi_wf_middle te on    te.business_id=r.requirement_no   and te.bsflag='0'  left join gp_task_project p on r.project_info_no = p.project_info_no   and p.bsflag = '0' left join comm_org_information i on r.apply_company = i.org_id and i.bsflag = '0' left join comm_human_employee t2 on r.applicant_id = t2.employee_id      and t2.bsflag = '0' where r.bsflag = '0' and r.proflag= '0'  and r.project_info_no='<%=projectInfoNo%>'  and  r.apply_no like  '%"+s_apply_no+"%'  order by r.create_date desc"; 
		  cruConfig.currentPageUrl = "/rm/em/singleHuman/humanRequired/humanRequiredList.jsp";
		  queryData(1);
