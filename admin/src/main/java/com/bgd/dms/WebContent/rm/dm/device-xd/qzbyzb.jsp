...
<div id="new_table_box" style="width:98%">
  <div id="new_table_box_content" style="width: 100%">
    <div id="new_table_box_bg" style="width: 95%">
      <fieldset><legend>强制保养记录(装备)</legend>
	  <table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		  <tr>					    
			...
			<td class="inquire_form6">
				...
				<input id="parmeter" name="parmeter" type="hidden" value=""/>
				<input id="colnum" name="colnum" class="input_width" type="hidden" value="" readonly/>
			</td>
	  ...
	  <div style="overflow:auto">
      	<table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		  	<tr align="right">
		  		...
		  		<td class="ali_cdn_input" ></td>
		  		<td>&nbsp;</td>
		    	<auth:ListButton id="zj1" functionId="" css="zj" event="onclick='addRepair()'" title="增加"></auth:ListButton>
		    	<auth:ListButton functionId="" css="sc" event="onclick='removeRepairRow()'" title="删除"></auth:ListButton>
			</tr>
		  </table>
	  </div>
	  <fieldset><legend>保养项目</legend>
	   	<div style="height:105px;overflow:auto">
	  	<table id="repairMap" width="250%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;">
			  <tr style="display:block" >  
			  	  <td class="bt_info_odd" >选择</td>
				  <td class="bt_info_even">检查保养内容</td>
				  <td class="bt_info_odd">检查补充</td>
				  <td class="bt_info_even">问题描述</td>
			  </tr>
			  <tbody id="repair_body"></tbody>
		  </table>
		 </div>
	  </fieldset>
...
<script type="text/javascript"> 
	...
	function submitInfo(){
		...
		if(confirm("提交后数据不能修改，确认提交？")){
			...
			var idinfos ;
			var text1="";
			$("input[type='checkbox'][name='coding_code_id']").each(function(){
				text1=text1+","+this.id;
				if(this.checked){
					if(count == 0){
						line_infos = this.id;
			...
			$("#colnum").val(result-1);"WebContent/rm/dm/device-xd/xcglzb.jsp"
			if (text1.substr(0,1)==',') text1=text1.substr(1);
			$("#parmeter").val(text1);
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/saveDeviceRepairInfozb.srq?state=9&ids="+dev_appdet_id;
	...
	function addRepair()
	{
		...
		var checkstr1 = 0;
		$("input[name^='coding_code_id'][type='hidden']").each(function(i){
		...
		var result = null;
		var checkstr = 0;
		$("input[name^='superior_code_id'][type='hidden']").each(function(i){
			if(this.value!=null&&this.value!=''){
				if(checkstr == 0){
					result = this.value;
				}
		...
		if(result!=null)
		{
			 showpage="<%=contextPath%>/rm/dm/device-xd/selectqzbyZBList.jsp?superior_code_id="+result;
		}
		else
		{
			showpage="<%=contextPath%>/rm/dm/device-xd/selectqzbyZBList.jsp";
		}
		var vReturnValue = window.showModalDialog(showpage,obj,"dialogWidth=800px;dialogHeight=620px");
		var retObjList;
		var bysql = "select coding_name,coding_code_id,d.superior_code_id from comm_coding_sort_detail d where coding_sort_id='5110000190' and  d.coding_code_id in ("+vReturnValue+")  and bsflag = '0' order by coding_show_id";
		...
		if(retObjList!=undefined && retObjList.length>=1){
			for(var index=0;index<retObjList.length;index++){
				i++;
				var table=document.getElementById("repairMap");
				var autoOrder = document.getElementById("repairMap").rows.length;
				var newTR = document.getElementById("repairMap").insertRow(autoOrder);
				var tdClass = 'even';
				if(autoOrder%2==0){
					tdClass = 'odd';
				}
				var td = newTR.insertCell(0);
				var innerhtml = "<input name='superior_code_id"+i+"' id='superior_code_id"+i+"' value='"+retObjList[index].superior_code_id+"'  type='hidden' />";
				 innerhtml += "<input name='coding_code_id"+i+"' id='coding_code_id"+i+"' value='"+retObjList[index].coding_code_id+"'  type='hidden' />";
				innerhtml+="<input type='checkbox' id='"+i+"'  name='coding_code_id' value='" + retObjList[index].coding_code_id + "' checked/>";
				...
				td.innerHTML = "<input type='text' name='check_append"+i+"' id='check_append"+i+"'>";
				td.className =tdClass+'_even'
				   if(autoOrder%2==0){
					td.style.background = "#FFFFFF";
				}else{
				...
				td.innerHTML = "<input type='text' name='bak"+i+"' id='bak"+i+"'>";
				td.className =tdClass+'_even'
				   if(autoOrder%2==0){
					td.style.background = "#FFFFFF";
				}else{
	...
	function removeRepairRow(){
		var cks=$("#repair_body :checked")
	...
	function searchRepairItem(){
		...
		querySql = "select t.bak,t.check_append,d.coding_code_id,d.coding_name,d.superior_code_id from BGP_COMM_DEVICE_REPAIR_TYPE t left join comm_coding_sort_detail d on d.coding_code_id=t.type_id where t.repair_info='"+repair_info+"'";				 	 
		queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=100000&querySql='+encodeURI(encodeURI(querySql)));
 
		retObjList = queryRet.datas;
		if(retObjList != null&&retObjList!=""){		
			for(var index=0;index<retObjList.length;index++){
				var table=document.getElementById("repairMap");
				var autoOrder = document.getElementById("repairMap").rows.length;
				var newTR = document.getElementById("repairMap").insertRow(autoOrder);
				...
				var td = newTR.insertCell(0);
				var innerhtml = "<input name='superior_code_id"+index+"' id='superior_code_id"+index+"' value='"+retObjList[index].superior_code_id+"'  type='hidden' />";
				 innerhtml += "<input name='coding_code_id"+index+"' id='coding_code_id"+index+"' value='"+retObjList[index].coding_code_id+"'  type='hidden' />";
				innerhtml+="<input type='checkbox' id='"+index+"'  name='coding_code_id' value='" + retObjList[index].coding_code_id + "' checked/>";
				td.innerHTML = innerhtml;
				td.className = tdClass+'_odd';
				   if(autoOrder%2==0){
					td.style.background = "#f6f6f6";
				...
				td = newTR.insertCell(2);
				td.innerHTML = "<input type='text' name='check_append"+index+"' id='check_append"+index+"' value='"+retObjList[index].check_append+"'>";
				td.className =tdClass+'_even'
				   if(autoOrder%2==0){
					td.style.background = "#FFFFFF";
				}else{
				...
				td = newTR.insertCell(3);
				td.innerHTML = "<input type='text' name='bak"+index+"' id='bak"+index+"' value='"+retObjList[index].bak+"'>";
				td.className =tdClass+'_even'
				   if(autoOrder%2==0){
					td.style.background = "#FFFFFF";
				}else{
