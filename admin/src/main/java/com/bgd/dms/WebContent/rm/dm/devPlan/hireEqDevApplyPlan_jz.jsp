...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>单项目-井中请示报告-井中请示报告(外租)</title> 
 </head> 
...
<script type="text/javascript">
    ...
    function refreshData(v_devapp_name,v_org_name,v_state_desc){
		...
		var str = "select wfmiddle.create_date as approve_date,pro.project_name,devapp.allapp_type,devapp.device_allapp_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.project_info_no,";
			...
			str += "where devapp.allapp_type = 'S10001' and devapp.bsflag = '0' and devapp.project_info_no='"+projectInfoNos+"' ";		
    ...
    function loadDataDetail(device_allapp_id){
        ...
        processNecessaryInfo={        							//流程引擎关键信息
        			...
        			businessType:"5110000004100001091",    //业务类型 即为之前设置的业务大类
        			businessId:device_allapp_id,           				//业务主表主键值
        			businessInfo:"井中请示报告(外租设备)<申请单名称:"+device_allapp_name+">",
	...
	function toAddMainPlanPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devPlan/devallplan_new_jz.jsp?projectInfoNo=<%=projectInfoNo%>&allapp_type=S10001');
	}
	...
	function toDelPlanPage(){
		...
		var str = "select devapp.device_allapp_id,nvl(wfmiddle.proc_status,'') as proc_status ";
			str += "from gms_device_allapp devapp  ";
			str += "left join common_busi_wf_middle wfmiddle on wfmiddle.business_id = devapp.device_allapp_id  ";
			str += "where devapp.bsflag = '0' and devapp.allapp_type = 'S10001' and devapp.project_info_no='"+projectInfoNos+"' ";
			str += "and devapp.device_allapp_id='"+deviceallappid+"' ";
		var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		//什么状态不能删除
	...
	function toModifyDetail(obj){
		...
		window.location.href='<%=contextPath%>/rm/dm/devPlan/devplan_app_frame.jsp?projectInfoNo=<%=projectInfoNo%>&allappType=S10001&deviceallappid='+idinfo+'&dgFlag='+dgFlag;
	}
