...
<form id="CheckForm" action="" method="post" target="list" enctype="multipart/form-data">
	...
	<table id="lineTable" width="100%" border="0" cellspacing="0" cellpadding="0"  style="margin-top:2px;" >
    	<tr>
            ...
            <td class="bt_info_odd">备注 <input type="hidden" id="equipmentSize" name="equipmentSize" value="<%=beanList.size()%>" /></td>
        </tr>
        
       <%
		int i=0;
		for (int j = 0; j < beanList.size(); j++) {
			BgpDocTdFileCheckData applyDetailInfo = beanList.get(j);

			if(applyDetailInfo.getDataId()!=null){
	  ...
	  %>
		...
		<input type="text" value="<%=applyDetailInfo.getCheckCodeName()==null?"":applyDetailInfo.getCheckCodeName()%>" id="fy<%=i%>checkCodeName" name="fy<%=i%>checkCodeName" class="input_width" readonly="readonly"/>
		</td>
		...
		<option value='1' <%if("1".equals(applyDetailInfo.getIsComplete())){ %>  selected="selected" <% }%>>否</option>
		</select>
		</td>
		<td class="<%=className%>even">
		...
		<option value='1' <%if("1".equals(applyDetailInfo.getIsPass())){ %>  selected="selected" <% }%>>否</option>
		</select>
		</td>
		<td class="<%=className%>odd">
		<input type="text" value="<%=applyDetailInfo.getNotes()==null?"":applyDetailInfo.getNotes()%>" id="fy<%=i%>notes" name="fy<%=i%>notes" class="input_width" type="text" />
		</td>
