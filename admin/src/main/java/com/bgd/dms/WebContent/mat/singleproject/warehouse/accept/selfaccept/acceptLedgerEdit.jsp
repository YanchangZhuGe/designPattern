...
<table border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<td class="inquire_item6"><input type="text"
			...
			value="<gms:msg msgTag="matInfo" key="input_date"/>" readonly />
	...
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" id="queryRetTable">
			<tr>
				<td class="bt_info_even" exp="<input name = 'rdo_entity_id'  type='checkbox' checked='true' value='{wz_id}' onclick='loadDataDetail()'/>" ><input type='checkbox' name='rdo_entity_id' value='' onclick='check()'/></td>
				...
				<td class="bt_info_even" exp="{out_num}">已入库数量</td>
				<td class="bt_info_odd" exp="<input type='text' name='actual_price_{wz_id}' id='actual_price_{wz_id}' value='{actual_price}'  style='width:40px'/>">入库单价</td>
				<td class="bt_info_even"	exp="<input type='text' name='mat_num_{wz_id}' id='mat_num_{wz_id}' value='{mat_num}'  style='width:40px'/>">入库数量</td>
				<td class="bt_info_odd"exp="<input type='text' name='total_money_{wz_id}' id='mat_num_{wz_id}' value='{total_money}'  style='width:40px'/>">入库金额</td>
				<td class="bt_info_even"	exp="<input type='text' name='warehouse_number_{wz_id}' id='mat_num_{wz_id}' value='{warehouse_number}'  style='width:40px'/>">收料库</td>
				<td class="bt_info_odd" exp="<input type='text' name='goods_allocation_{wz_id}' id='mat_num_{wz_id}' value='{goods_allocation}'  style='width:40px'/>">货位</td>
			</tr>
		</table>
	</div>
...
<script type="text/javascript"><!--
	...
	function refreshData(){
		cruConfig.submitStr ="ids="+"<gms:msg msgTag="matInfo" key="invoices_id"/>";
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getAcceptDatas";
		queryData(1);
	...
	function refreshData2(){
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getAcceptDatas2";
		queryData(1);
	}	
	...
	function save(){	
		...
		if(loadDataDetail()){
			openMask();
			document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/warehouse/accept/selfaccept/EditAcceptLedger.srq?id="+ids;
			document.getElementById("form1").submit();
		}
	}
	
	
	var invoices_id = document.getElementById("invoices_id").value;
	function openMatHave(){
		var selectWzId = document.getElementById("selectWzId").value;
		if(projectType=="5000100004000000008"){
			var selected = window.showModalDialog("<%=contextPath%>/mat/singleproject/warehouse/accept/selfaccept/selectAcceptListJZ.jsp?selectWzId="+selectWzId,"","dialogWidth=1240px;dialogHeight=480px");
		}else{
			var selected = window.showModalDialog("<%=contextPath%>/mat/singleproject/warehouse/accept/selfaccept/selectAcceptList.jsp?selectWzId="+selectWzId+"&invoices_id="+invoices_id,"","dialogWidth=1240px;dialogHeight=480px");
		}
		var wz_id = selected;
		document.getElementById("selectWzId").value = wz_id;
		cruConfig.submitStr ="wz_ids="+wz_id+"&ids="+invoices_id;
	...
	function deleteMatHave(){
	   ...
	   // for(var i=0;i<tab.rows.length;i++) {
		...
		if(wz_ids!=null&&wz_ids!=""){
			cruConfig.submitStr ="wz_ids="+wz_ids+"&ids="+invoices_id;;
			refreshData2();
		}
	}
			...
			for(var i=1;i<row.length;i++){
				...
				if(cell_0 == true){
					if(cell_7!=undefined && cell_8!=undefined &&cell_6!=undefined){
						...
						if(Number(cell_5)<Number(cell_6-(-outNum))){
