...
<script type="text/javascript">
	...
	function fillAppDetailNouseNum(orgidvalue){
		...
		var mainsql = "select usage_org_id,device_id,unuse_num from gms_device_coll_account account ";
	...
	function toAddAddedDetailInfos(){
		//获得第一行的team信息
		var teamname = $("input[name^='team_name']")[0].value;
		var team = $("input[name^='team']")[0].value;
		addedseqinfo++;
	...
	function refreshData(){
		...
		if('<%=devappid%>'!=null){
			...
			+"to_char(devapp.appdate,'yyyy-mm-dd') as appdate,tp.project_name,inorg.org_abbreviation as in_org_name,mif.mixinfo_no,"
			+"devapp.mix_org_id,devapp.mix_type_id,devapp.mix_type_name,devapp.mix_user_id, "
			+"mif.out_org_id,outorg.org_abbreviation as out_org_name "
			+"from gms_device_app devapp left join gp_task_project tp on devapp.project_info_no=tp.project_info_no "
			...
			+"where devapp.device_app_id='<%=devappid%>' and mif.device_mixinfo_id='<%=mixInfoId%>'";
			...
			var prosql = "select appdet.device_app_detid,appdet.team,appdet.apply_num,appdet.purpose,appdet.isdevicecode,";
				...
				prosql += "teamsd.coding_name as team_name,amm.assign_num,amm.devremark,amm.device_mix_subid ";
			...
			if(retObj!=undefined){
				for(var index=0;index<retObj.length;index++){
					...
					innerhtml += "<td width='8%'><input name='devremark"+index+"' id='devremark"+index+"' value='"+retObj[index].devremark+"'size='7' type='text' />";
					innerhtml += "</tr>";
			...
			//回填补充明细信息
			var prosql = "select appadded.device_mix_subid,appadded.device_mixinfo_id,appadded.dev_name,";
				prosql += "appadded.dev_model,appadded.assign_num,appadded.dev_unit,appadded.devremark,appadded.team,teamsd.coding_name as team_name ";
				prosql += "from gms_device_appmix_added appadded ";
				prosql += "left join comm_coding_sort_detail teamsd on teamsd.coding_code_id=appadded.team ";
				prosql += "where appadded.bsflag='0' and appadded.device_mixinfo_id='<%=mixInfoId%>' ";
			var proqueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+prosql+'&pageSize=10000');
			var addedretObj = proqueryRet.datas;
			if(retObj!=undefined){
				for(var index=0;index<addedretObj.length;index++){
					...
					innerhtml += "<input type='hidden' name='addeddevicemixsubid"+index+"' id='addeddevicemixsubid"+index+"' value='"+addedretObj[index].device_mix_subid+"'/></td>";
					...
					innerhtml += "<td width='12%'><input name='addeddevicename"+index+"' id='addeddevicename"+index+"' idindex='"+index+"' value='"+addedretObj[index].dev_name+"' style='line-height:18px;width:98%' size='10' type='text' /></td>";
					innerhtml += "<td width='12%'><input name='addeddevicetype"+index+"' id='addeddevicetype"+index+"' value='"+addedretObj[index].dev_model+"' style='line-height:18px;width:98%' size='10' type='text' /></td>";
					innerhtml += "<td width='10%'><input name='addedunit"+index+"' id='addedunit"+index+"' value='"+addedretObj[index].dev_unit+"' style='line-height:18px;width:98%' size='8' type='text' /></td>";
					innerhtml += "<td width='10%'><input name='addedassignnum"+index+"' id='addedassignnum"+index+"' style='line-height:18px;width:98%' value='"+addedretObj[index].assign_num+"' type='text' size='8' onkeyup='checkaddedNum(this)'/></td>";
