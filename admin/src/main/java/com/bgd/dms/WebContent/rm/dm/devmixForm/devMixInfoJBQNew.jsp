...
<title>多项目-调配调剂-调配单管理(检波器)-调配子页面</title>
</head>
	  ...
	  <fieldset style="margin-left:2px"><legend>调配数量明细</legend>
		  <div style="overflow:auto">
			  <table style="width:98%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr><td class="bt_info_odd" width="2%"><input type="checkbox" name="alldetinfo" id="alldetinfo" checked/></td>
					<td class="bt_info_even" width="7%">班组</td>
					<td class="bt_info_odd" width="8%">设备名称</td>
					<td class="bt_info_even" width="10%">规格型号</td>
					<td class="bt_info_odd" width="5%">单位</td>
					<td class="bt_info_even" width="6%">申请数量</td>
					<td class="bt_info_odd" width="7%">已调配数量</td>
					<td class="bt_info_odd" width="7%">库存数量</td>
					<td class="bt_info_even" width="7%">调配数量</td>
					<td class="bt_info_odd" width="8%">计划进场时间</td>
					<td class="bt_info_even" width="8%">计划离场时间</td>
...
var oprstate_tmp="<%=oprstate%>";
	...
	//新加入调配单位可调控数量为空判断
	function fillAppDetailNouseNum(devid){
		//给所有的unusenum设置为空
		...
		var mainsql = "select account.dev_acc_id,usage_org_id,device_id,nvl(total_num,0) as total_num,nvl(unuse_num,0) as unuse_num from gms_device_coll_account account ";
			...
			mainsql += "where (account.usage_sub_id like '"+owingOrgId+"' or account.owning_sub_id like '"+owingOrgId+"') and account.bsflag='0' and account.ifcountry !='国外' ";
		...
		$("input[type='checkbox'][name='detinfo']").each(function(i){
			if(this.checked){
				...
				for(var j = 0;j<parseInt(mixnuminfo);j++){
					...
					innerhtml += "<td width='8%'><input name='detteamname"+seqinfo+"' id='detteamname"+seqinfo+"' value='"+teamname+"' style='line-height:18px;'  type='text' readonly/></td>";
					innerhtml += "<td width='8%'><input name='detdevicename"+seqinfo+"' id='detdevicename"+seqinfo+"' value='"+devicename+"' style='line-height:18px;' type='text' readonly/></td>";
					innerhtml += "<td width='8%'><input name='detdevicetype"+seqinfo+"' id='detdevicetype"+seqinfo+"' value='"+devicemodel+"' style='line-height:18px;' type='text' readonly/><img src='<%=contextPath%>/images/magnifier.gif' width='16' height='16' style='cursor:hand;' onclick=showDevPage("+seqinfo+",'"+devcicode+"','"+isdevicecode+"','"+devicename+"') /></td>";
					innerhtml += "<td width='8%'><input name='dev_acc_id"+seqinfo+"' id='dev_acc_id"+seqinfo+"' type='hidden'/>";
					...
					innerhtml += "<td width='8%'><input name='dev_sign"+seqinfo+"' id='dev_sign"+seqinfo+"' style='line-height:18px;width:99%' type='text' readonly/></td>";
					innerhtml += "<td><input name='unusenum"+index+"' id='unusenum"+index+"' value='' size='7' type='text' readonly/></td>";
					innerhtml += "<td width='8%'><input name='license_num"+seqinfo+"' id='license_num"+seqinfo+"' value='' style='line-height:18px;width:99%' type='text' readonly/></td>";
					innerhtml += "<td width='9%'><input name='asset_coding"+seqinfo+"' id='asset_coding"+seqinfo+"' style='line-height:18px;width:95%' value='' readonly/></td>";
					...
					innerhtml += "<td width='8%'><input name='devstartdate"+seqinfo+"' id='devstartdate"+seqinfo+"' style='line-height:18px;width:99%' value='"+startdate+"' type='text' readonly/></td>";
					innerhtml += "<td width='8%'><input name='devenddate"+seqinfo+"' id='devenddate"+seqinfo+"' style='line-height:18px;width:99%' value='"+enddate+"' type='text' readonly/></td>";
					innerhtml += "</tr>";
	...
	function checkAssignNum(obj){
		...
		var applynumVal = parseInt($("#apply_num"+index).val(),10);//申请数量
		var unusenumVal = parseInt($("#unusenum"+index).val(),10);//库存数量
		...
		if(!re.test(objValue)){
			...
			if(applynumVal>unusenumVal){
				alert("库存数量小于申请数量,不能调配!");
				obj.value = "";
				return false;
			}else if(parseInt(objValue,10)>applynumVal){
	...
	function refreshData(){
		...
		if(retObj!=undefined){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td width='2%'><input type='checkbox' name='detinfo' id='"+index+"' value='"+retObj[index].device_app_detid+"' checked/></td>";
				innerhtml += "<td width='8%'><input name='team_name"+index+"' id='team_name"+index+"' style='line-height:15px' value='"+retObj[index].team_name+"' size='10' type='text' readonly/></td>";
				innerhtml += "<td width='9%'><input name='devicename"+index+"' id='devicename"+index+"' style='line-height:15px' value='"+retObj[index].dev_ci_name+"' size='10' type='text' readonly/>";
				innerhtml += "<input name='device_id"+index+"' id='device_id"+index+"' value='"+retObj[index].dev_ci_code+"' type='hidden' />";
				...
				innerhtml += "<td width='10%'><input name='devicemodel"+index+"' id='devicemodel"+index+"' value='"+retObj[index].dev_ci_model+"' size='12' type='text' readonly/><img id='show-btn' src='<%=contextPath%>/images/magnifier.gif' width='16' height='16' style='cursor:hand;' onclick='showDevCodePage("+index+")' /></td>";
				...
				innerhtml += "<td width='7%'><input name='apply_num"+index+"' id='apply_num"+index+"' style='line-height:15px' value='"+retObj[index].apply_num+"' size='7' type='text' readonly/></td>";
				innerhtml += "<td width='7%'><input name='mixednum"+index+"' id='mixednum"+index+"' value='"+retObj[index].mixed_num+"' size='7'  type='text' readonly/></td>";
				innerhtml += "<td><input name='unusenum"+index+"' id='unusenum"+index+"' value='' size='7' type='text' readonly/></td>";
				innerhtml += "<td width='7%'><input name='mixnum"+index+"' id='mixnum"+index+"' detindex='"+index+"' value='' onkeyup='checkAssignNum(this)' size='7' type='text' /></td>";
	...
	function showDevCodePage(lineobj){
		...
		var obj = new Object();
		var data=window.showModalDialog("<%=contextPath%>/rm/dm/collectTree/collectJBQTreeManager.jsp?ctxmenu=false&inline=true",obj,"dialogWidth=300px;dialogHeight=400px");
		if(data!=undefined){
			...
			fillAppDetailNouseNum(data.device_id);
		}
	}
</script>
</html>
