...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		...
		function getAmountWhlFusionChart(){
			...
			chart1.setXMLUrl("<%=contextPath%>/dms/use/getDeviceUseAndWhole.srq?level=1&devTreeId=<%=devTreeId%>&orgSubId=<%=orgSubId%>&country=<%=country%>&startDate=<%=startDate%>&endDate=<%=endDate%>");
			chart1.render("chartContainer1");
		}
		//弹出子层级利用率
		function popNextLevelUseRate(level,devTreeId,orgSubId,country,startDate,endDate){
			popWindow('<%=contextPath%>/dmsManager/use/useAWh/thirdDeviceUseAWh.jsp?level='+level+'&devTreeId='+devTreeId+'&orgSubId='+orgSubId+'&country='+country+'&startDate='+startDate+'&endDate='+endDate,'800:572');
		}
		//弹出设备信息
