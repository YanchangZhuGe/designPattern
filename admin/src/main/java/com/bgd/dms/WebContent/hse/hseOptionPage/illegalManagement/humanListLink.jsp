...
function page_init(){ 
	...
	cruConfig.queryStr = "select  l.labor_id as employee_id , decode(l.if_engineer, '0110000059000000005', '临时季节性用工', '0110000059000000001', '再就业人员', '0110000059000000003', '劳务派遣人员', '', '其他用工', l.if_engineer) type_s,l.employee_name, l.employee_gender,decode(l.employee_gender, '1', '男', '0', '女')employee_gender_name, l.employee_id_code_no,  d3.coding_name posts,  d2.coding_name post_level_name   from bgp_comm_human_labor l  left join comm_coding_sort_detail d3    on l.post = d3.coding_code_id  left join comm_coding_sort_detail d2    on l.technical_title = d2.coding_code_id    left join bgp_comm_human_labor_list lt on l.labor_id = lt.labor_id and lt.bsflag='0'  where l.bsflag='0' and lt.list_id is null "
		+" union  "
		+" select distinct e.employee_id,case  when hr.employee_cd is not null then  '正式工' else '' end  as type_s ,e.employee_name, e.employee_gender,decode(e.employee_gender, '1', '男', '0', '女','2','女')employee_gender_name,e.employee_id_code_no, hr.post as posts, d1.coding_name post_level_name   from comm_human_employee e  inner join comm_human_employee_hr hr on e.employee_id = hr.employee_id   and hr.bsflag='0'  left join comm_coding_sort_detail d1 on hr.post_level = d1.coding_code_id and d1.bsflag = '0' left join view_human_project_relation r on e.employee_id=r.EMPLOYEE_ID where e.bsflag='0' ";
...
function classicQuery(){
	...
	debugger;
	cruConfig.cdtType = 'form';
	cruConfig.queryStr = "select * from (select  l.labor_id as employee_id , decode(l.if_engineer, '0110000059000000005', '临时季节性用工', '0110000059000000001', '再就业人员', '0110000059000000003', '劳务派遣人员', '', '其他用工', l.if_engineer) type_s,l.employee_name, l.employee_gender,decode(l.employee_gender, '1', '男', '0', '女')employee_gender_name, l.employee_id_code_no,  d3.coding_name posts,  d2.coding_name post_level_name   from bgp_comm_human_labor l  left join comm_coding_sort_detail d3    on l.post = d3.coding_code_id  left join comm_coding_sort_detail d2    on l.technical_title = d2.coding_code_id    left join bgp_comm_human_labor_list lt on l.labor_id = lt.labor_id and lt.bsflag='0'  where l.bsflag='0' and lt.list_id is null "
		+" union  "
		+" select distinct e.employee_id,case  when hr.employee_cd is not null then  '正式工' else '' end  as type_s ,e.employee_name, e.employee_gender,decode(e.employee_gender, '1', '男', '0', '女','2','女')employee_gender_name,e.employee_id_code_no, hr.post as posts, d1.coding_name post_level_name   from comm_human_employee e  inner join comm_human_employee_hr hr on e.employee_id = hr.employee_id   and hr.bsflag='0'  left join comm_coding_sort_detail d1 on hr.post_level = d1.coding_code_id and d1.bsflag = '0' left join view_human_project_relation r on e.employee_id=r.EMPLOYEE_ID where e.bsflag='0') ";
...
<table border="0"  cellspacing="0"  cellpadding="0"  class="form_info"  width="100%" id="queryRetTable">
	...
	<tr class="bt_info">
		...
		<td class="tableHeader" 	 exp="{employee_id_code_no}">身份证号</td>
