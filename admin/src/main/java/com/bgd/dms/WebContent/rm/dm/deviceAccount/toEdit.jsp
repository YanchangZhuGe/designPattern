...
<div id="new_table_box">
  <div id="new_table_box_content">
    <div id="new_table_box_bg">
     <fieldset><legend>基本信息&nbsp;&nbsp;<font color=red>（合同编号选择"ZY"，则标识新增设备为账外设备）</font> </legend>
      <table id="table1" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
         <tr>
			  ...
			  <td class="inquire_item6">使用情况</td>
				...
				<td>
					<div style="position:relative;">
				    	<span style="margin-left:105px;width:18px;overflow:hidden;">
						    <select style="width:108px;margin-left:-100px" id="cont_num_tmp" name="cont_num_tmp" onchange="this.parentNode.nextSibling.value=this.value">
							    <option value="" selected="selected"></option>
							    <option value="ZY">ZY</option>
					    	</select>
					    </span>
					    <input name="cont_num" id="cont_num" style="width:88px;position:absolute;margin-left:-110px;top:3px;"/>
					</div>
			    </td>
			  </tr>
			  <tr>
			  ...
			  <td class="inquire_form6">
					<select type="text" name="ifcountry" id="ifcountry" value="" readonly="readonly" class="input_width">
...
function checkType(){
	...
	function pageInit(){
		...
		var querySql="select case t.ifcountry when '国内' then '0' when '国外' then '1' else '0' end as ifcountry_desc,t.*, "
			+"(select org_abbreviation from comm_org_information org where t.owning_org_id=org.org_id) as owning_org_name_desc, "
			+"(select org_abbreviation from comm_org_information org where t.USAGE_ORG_ID=org.org_id) as usage_org_name_desc "
			+"from gms_device_account t where t.dev_acc_id='"+'<%=devaccid%>'+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		datas = queryRet.datas;
		...
		document.getElementById("usage_org_name").value=datas[0].usage_org_name_desc;
		...
		document.getElementById("ifcountry").value=datas[0].ifcountry_desc;
	...
	**/
	...
	function submitInfo(){
		...
		var devtype = document.getElementById("dev_type").value;
		if(devtype==""){
			alert("请输入设备编码");
			return;
		}
		var owningsubid = $("#owning_sub_id").val();
		...
		var devaccid = document.getElementById("devaccid").value;
		var querySql;
		var accountstat = $("#account_stat").val();
		//不在帐的判断合同编号
		if(accountstat == '0110000013000000006'){
			if(document.getElementById("cont_num").value==""){
				alert("请输入合同编号");
				return;
			}
			querySql = "select * from gms_device_account where dev_sign='"+document.getElementById("dev_sign").value+"' "
				+ "and cont_num='"+document.getElementById("cont_num").value+"' and dev_acc_id != '"+devaccid+"' "
				+ "and owning_sub_id like '"+owningsubid+"#gmslike#' and dev_type = '"+devtype+"' and bsflag='0' ";
		}else{
			querySql = "select * from gms_device_account where dev_sign='"+document.getElementById("dev_sign").value+"' "
				+ "and dev_acc_id != '"+devaccid+"' and bsflag='0' "
				+ "and owning_sub_id like '"+owningsubid+"#gmslike#' and dev_type = '"+devtype+"' ";
		}
