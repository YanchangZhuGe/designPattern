...
function clickNode(selectedNode){ 
	if(selectedNode.leaf){
		...
		var idss = ids.split(",");
		parent.mainRightframe.location.href = "<%=contextPath %><%=backUrl%>?projectInfoNo="+idss[0]+"&projectType="+idss[1]+"&projectName="+encodeURI(encodeURI(selectedNode.text));
	}
	
}
var message = "<%=message%>"; 
if(message != "" && message != 'null'){
	if(message=="1"){  	alert("导入成功"); 	
	var str="projectInfoNo=<%=pInfoNo%>&superiorCodeId=0000000003000000802&planId=<%=pId%>";		
	jcdpCallService("HumanCommInfoSrv","saveCostPlanListCodes",str);	
		parent.mainRightframe.location.href = "<%=contextPath %>/rm/em/singleHuman/humanCostPlan/humanCostPlanList.jsp?projectInfoNo=<%=pInfoNo%>&projectType=<%=pType%>&projectName=<%=pName%>&pId=<%=pId%>";
   
	}
	if(message=="2"){  	alert("补充导入成功"); 	
	var str="projectInfoNo=<%=pInfoNo%>&spare5=1&superiorCodeId=0000000003000000802&planId=<%=pId%>";		
	jcdpCallService("HumanCommInfoSrv","saveCostPlanListCodes",str);	
 
		parent.mainRightframe.location.href = "<%=contextPath %>/rm/em/singleHuman/humanCostPlan/humanCostPlanSupplyList.jsp?projectInfoNo=<%=pInfoNo%>&projectType=<%=pType%>&projectName=<%=pName%>&pId=<%=pId%>";
...
function getSubNodes(parentNode) {
	...
	if(idss[2]=="C105"){
		...
		wtcQuerySql="select 1 as is_leaf,p.project_info_no as id,p.project_name as name,p.project_type as projecttype from gp_task_project p join gp_task_project_dynamic dy on dy.bsflag = '0'  and dy.project_info_no = p.project_info_no and dy.org_subjection_id like '"+idss[2]+"%25"+"' where 1 = 1 and p.bsflag = '0' order by p.modifi_date desc";
