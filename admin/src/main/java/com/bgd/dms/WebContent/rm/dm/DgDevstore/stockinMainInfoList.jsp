			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_backapp_id_{device_backapp_id}' name='device_mixinfo_id'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' backtypemixid='{backdevtype}~{device_mixinfo_id}' value='{device_backapp_id}' id='selectedbox_{device_backapp_id}' onclick='chooseOne(this);loadDataDetail();'/>" >选择</td>
					...
					<td class="bt_info_odd" exp="{back_username}">返还申请人</td>
					<td class="bt_info_even" exp="{backappdate}">返还申请时间</td>
...
<script type="text/javascript">
	...
	function refreshData(v_project_name,v_backapp_name,v_device_backapp_no,v_back_org_name){
		...
		/**var str = "select '' as device_mixinfo_id,backapp.backdevtype,pro.project_name,backapp.device_backapp_id,backapp.device_backapp_no,backapp.backapp_name,";
			str += "backapp.project_info_id as project_info_no,backapp.back_org_id,backorg.org_abbreviation as back_org_name,";
			str += "case backapp.state when '0' then '未提交' when '9' then '已提交' else '异常状态' end as state_desc,";
			...
			str += "where backapp.bsflag = '0' and backapp.state='9' and backapp.backdevtype='S0000' ";
			//加入权限判断
			str += " and exists(select device_backapp_id from gms_device_backapp_detail bad join gms_device_account_dui dui on bad.dev_acc_id=dui.dev_acc_id  ";			
			str += " where dui.owning_sub_id like ";
			str += " (select o.org_code as org_subjection_id from p_auth_user u left join p_auth_org o on o.org_id = u.org_id where ";
			str += " u.user_status<'2' and u.bsflag='0' and u.login_id='qianmengru' and rownum = 1 )||'%' ";
			str + =" and bad.device_backapp_id = backapp.device_backapp_id)";*/
			
			var str = "select distinct '' as device_mixinfo_id,backapp.backdevtype,pro.project_name,backapp.device_backapp_id,backapp.device_backapp_no,backapp.backapp_name,";
			str += " backapp.project_info_id as project_info_no,backapp.back_org_id,backorg.org_abbreviation as back_org_name,";
			str += " case backapp.state when '0' then '未提交' when '9' then '已提交' else '异常状态' end as state_desc,";
			str += " backapp.opr_state,case backapp.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as oprstate_desc,";
			str += " emp.employee_name as back_username,to_char(backapp.modifi_date,'yyyy-mm-dd') as backappdate ";
			...
			str += " left join gp_task_project pro on backapp.project_info_id = pro.project_info_no ";
			str += " left join comm_org_subjection orgsub on backapp.backmix_org_id=orgsub.org_id and orgsub.bsflag='0'";
			str += " left join gms_device_backapp_detail bad on bad.device_backapp_id = backapp.device_backapp_id";
			str += " left join gms_device_account_dui dui on bad.dev_acc_id = dui.dev_acc_id";
			str += " where backapp.bsflag = '0' and backapp.state='9' and backapp.backdevtype='S0000'";
			str += " and dui.owning_sub_id like (select o.org_code as org_subjection_id from p_auth_user u left join p_auth_org o on o.org_id = u.org_id where";
			str += " u.user_status<'2' and u.bsflag='0' and u.user_id='<%=userid%>' and rownum =1)||'%'";
	...
	function toDetailPage(){
		...
		if(info[0] == 'S1405'){
			...
			window.location.href='<%=contextPath%>/rm/dm/DgDevstore/subStockin.jsp?id='+info[1];
		}else{
			//window.location.href='<%=contextPath%>/rm/dm/DgDevstore/subStockin.jsp?id='+shuaId;
