...
<input type='hidden' name='laborId' id='laborId' value=''/>
    ...
    <tr>
    	...
    	<td class="inquire_item4"><font color="red">*</font>项目名称：</td>
      	<td class="inquire_form4"><input type="text" name="project_name" class="input_width" value="<%=projectName %>" /></td>
      	<td class="inquire_item4">申请队伍：</td>
      	<td class="inquire_form4"><input type="text" name="org_name" class="input_width" value="<%=orgName %>" /></td>
    </tr>
    <tr>
    	<td class="inquire_item4">提交人：</td>
      	<td class="inquire_form4"><input type="text" name="creator_id" class="input_width" value="<%=userName %>" /></td>
		<td class="inquire_item4">提交时间：</td>
      	<td class="inquire_form4"><input type="text" name="compile_date" id = 'compile_date'class="input_width" value="<%=subDate %>" />
    ...
    <tr>
	    ...
	    <td class="inquire_form4"><input type='text' name='total_money' id='total_money' value='' class="input_width"></td>
	    <td class="inquire_item4">申请单号：</td>
	    <td class="inquire_form4"><input type='text' name='submite_number' id='submite_number' value='' class="input_width"></td>
    </tr>
    <tr>
	    ...
	    <td class="inquire_item4">计划指向：</td>
	    <td class="inquire_form4"><input type="text" name="plan_invoice_type" id="plan_invoice_type" class="input_width" value="<%=type.equals("WZG")?"物资供应":type.equals("ZYH")?"专业化":"自采购" %>"></td>
...
<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_even" exp="<font name='demand_num{wz_id}'   value='{demand_num}' >{demand_num}</font>">需求数量</td>
			      ...
			      <!-- <td class="bt_info_even" exp="<input name='have_num{wz_id}'  type='text' value='{have_num}' readonly/>">现有数量</td> -->
 		      	  <!-- <td class="bt_info_odd" exp="<input name = 'regulate_num{wz_id}'  type='text' value='{regulate_num}'  readonly/>"  isShow='Hide'>可调剂数量</td> --> 
			     <!--  <td class="bt_info_odd" exp="<input name='apply_num{wz_id}'  type='text' value='' readonly/>">合计申请量</td> -->
			      <td class="bt_info_odd" exp="<font name='plan_money{wz_id}'  value='' /></font>">计划金额</td>
<!--			  <td class="bt_info_even" exp="{create_date}">需求日期</td> 
 		      	<td class="bt_info_odd" exp="{mat_desc}">物资说明</td> -->
			    </tr>
			  </table>
			</div>
...
<script type="text/javascript">
    ...
    var planInvoiceId = getQueryString("id");
    function refreshData(){
    	getPlanNum();
    	cruConfig.queryService = "MatItemSrv";
    	cruConfig.queryOp = "getSubDateDg";
		cruConfig.submitStr ="ids="+planInvoiceId;
		queryData(1);
		document.getElementById("team").src = "<%=contextPath%>/mat/singleproject/plansub/subTeamList.jsp?planInvoiceId="+planInvoiceId;
		loadDataDetail();
	}
	function save(){	
	    ...
	    openMask();
	    document.getElementById("form1").action = "<%=contextPath%>/mat/singleproject/plan/savePlanSum.srq?value="+planInvoiceId;
...
//				var cell_6 = row[i].cells[6].firstChild.value;
				...
				if(cell_4!=undefined && cell_6!=undefined){
						...
						debugger;
					row[i].cells[6].innerHTML=Math.round((cell_3*cell_5)*1000)/1000;
					if(parseInt(row[i].cells[6].innerHTML)<0){
						row[i].cells[6].innerHTML=0;
						}
					//row[i].cells[8].firstChild.value = Math.round((cell_4*(row[i].cells[7].firstChild.value))*1000)/1000;
					totalMoney += Math.round((cell_3*(cell_5))*1000)/1000;
				}
			}
		}
	 ...
	 function getPlanNum(){
			...
			document.getElementById("plan_name").value="计划"+<%=applyDate%>+"-00"+num;
			}	
</script>
</html>