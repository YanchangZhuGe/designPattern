...
var querySql = "select t.mat_menu_id,t.menu_name from gms_mat_buss_menu t where t.bsflag = '0' and t.father_mat_menu_id is null";
var queryOrgRet = syncRequest('Post',cruConfig.contextPath+appConfig.queryListAction,'querySql='+querySql);
var rootMenuId = queryOrgRet.datas[0].mat_menu_id;
var rootMenuName = queryOrgRet.datas[0].menu_name; 
...
function getSubNodes(parentNode) {
	...
	Ext.Ajax.request({
		...
		params : {
			currentPage:"1",
			pageSize:"100",
			querySql:"Select * FROM gms_mat_buss_menu WHERE bsflag='0' and father_mat_menu_id='"+parentNode.id+"' ORDER BY order_num desc"
		},
		method : 'Post',
...
function clickNode(selectedNode){
	...
	if(selectedNode.id != undefined && selectedNode.id != ""){
		var querySql = "select t.*,oi.org_abbreviation from gms_mat_buss_menu t join comm_org_information oi on oi.org_id=t.org_id and oi.bsflag='0' where t.mat_menu_id='"+selectedNode.id+"' and t.bsflag='0'";
		var queryOrgRet = syncRequest('Post',cruConfig.contextPath+appConfig.queryListAction,'querySql='+querySql);
		if(queryOrgRet.datas.length != 0){
			menu_name = queryOrgRet.datas[0].menu_name; 
			org_name = queryOrgRet.datas[0].org_abbreviation; 
			is_leaf = queryOrgRet.datas[0].is_leaf; 
			memo = queryOrgRet.datas[0].memo; 
		}
	}
	
	document.getElementById("menu_name").value=menu_name;
	document.getElementById("org_name").value=org_name;
	document.getElementById("is_leaf").value=is_leaf;
	document.getElementById("memo").value=memo;
}
jcdpTreeCfg.dbClickEvent=true;
function dbClickNode(selectedNode){
	if(selectedNode.id != undefined && selectedNode.id != ""){
		window.location="<%=contextPath %>/mat/multiproject/matLedger/matLedgerMenu.jsp?bussId="+selectedNode.id;
	}
}
...
function toAdd(){
	...
	if(clickedNode == undefined){
		...
		needReload = rootNode;
		popWindow('<%=contextPath %>/mat/common/setMatOrg/addMenu2.jsp?id='+rootMenuId); 
	}else{
		debugger;
		if(clickedNode.leaf){
			alert("叶子节点不能添加!");
		...
		popWindow('<%=contextPath %>/mat/common/setMatOrg/addMenu2.jsp?id='+clickedNode.id); 		
	}
	//新增刷新当前目录即可  	
...
function toModify(){
	...
	popWindow('<%=contextPath %>/mat/common/setMatOrg/addMenu2.jsp?pagerAction=edit&id='+clickedNode.id); 
}
/**
	删除目录
*/
function toDelete(){
	...
	var sql = "update gms_mat_buss_menu set bsflag='1' where mat_menu_id ='{id}'";//selectedNodeId替换{id}
			...
			<div id="tab_box" class="tab_box">
				...
				<div id="tab_box_content0" class="tab_box_content">
					<table width="100%" border="0" cellspacing="0" cellpadding="0" id="folderInfoTable" class="tab_line_height">
					  <tr>
					    <td class="inquire_item8">名称：</td>
					    <td class="inquire_form8" id="item0_1"><input type="text" id="menu_name" name="menu_name" class="input_width" readonly="readonly"/></td>			
					    <td class="inquire_item8">从属单位：</td>
					    <td class="inquire_form8" id="item0_0"><input type="text" id="org_name" name="org_name" class="input_width" readonly="readonly"/></td>		  	  
					  </tr>		
					  <tr>
					    <td class="inquire_item8">是否叶子节点：</td>
					    <td class="inquire_form8" id="item1_0">
					    ...
					    <td class="inquire_form8" id="item1_1"></td>		  
					  </tr>	
					   <tr>
					    <td class="inquire_item8">备注：</td>
					    <td class="inquire_form8" id="item1_0" colspan="3"><textarea class="textarea" id="memo" name="memo"></textarea></td>		  	
					  </tr>	
					</table>
				</div>
			</div>
