			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      ...
			      <td class="bt_info_even" exp="<a style='color:blue;' href=<%=contextPath%>/doc/downloadDocByUcmId.srq?docId={ucm_id}&emflag=0>{file_name}</a>">成果图件名称</td>
			      <td class="bt_info_odd" exp="{line_no}">测线号</td>
			      <td class="bt_info_even" exp="{writer}">绘制人</td>
			     </tr> 			        
			  </table>
			</div>
...
<script type="text/javascript">
	...
	function refreshData(str){
		cruConfig.queryStr ="select t.* from (select t.project_info_no,t.tecnical_id,t.title,t.line_no,t.writer,f.ucm_id as ucm_id,f.file_name as file_name "+
		"from gp_ws_tecnical_basic t left join (select * from bgp_doc_gms_file where ucm_id in(select max(ucm_id) from bgp_doc_gms_file where doc_file_type='0100300002000000038' and bsflag='0' group by relation_id,doc_file_type))f on t.tecnical_id=f.relation_id "+
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/doc/geologicalDoc/ws/add_tdDocModify.jsp?businessType='+businessType+'&fileAbbr='+fileAbbr,'800:680');
    }
	...
	function toEdit() {
		...
		popWindow('<%=contextPath%>/td/doc/geologicalDoc/ws/add_tdDocModify.jsp?businessType='+businessType+'&id='+ids,'800:680');
	}
