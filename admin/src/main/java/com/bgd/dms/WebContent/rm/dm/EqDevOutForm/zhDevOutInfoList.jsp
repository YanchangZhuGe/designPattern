...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>多项目-设备出库-设备出库(综合物化探)-查看明细页面</title> 
 </head> 
      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_mix_detid_{device_mix_detid}' name='device_mix_detid'>
					...
					<!-- <td class="bt_info_even" exp="{operator_name}">操作手</td>
					<td class="bt_info_even" exp="{dev_position}">存放地</td>
					<td class="bt_info_odd" exp="{state_desc}">接收状态</td>
					<td class="bt_info_even" exp="{actual_in_time}">接收时间</td> -->
			     </tr> 
			  </table>
			</div>
			...
			<div id="tab_box" class="tab_box">
			  ...
			  <tr>
				...
				<td class="inquire_form6"><input id="dev_dev_name" name=""  class="input_width" type="text" /><input id="team_dev_acc_id" name="" type="hidden" /></td>
...
<script type="text/javascript">
	...
	function refreshData(v_dev_name,v_dev_model){
		var str = "select case dad.state when '1' then '已接收' else '未接收' end as state_desc,teamid.coding_name as team_name,"+
			"dm.project_info_no,da.dev_name,da.dev_model,unit.coding_name as dev_unit,da.dev_position,dam.assign_num,e.employee_name as operator_name,dad.* "+
			...
			"left join comm_human_employee e on e.employee_id = dad.operator_id "+
    ...
    function loadDataDetail(shuaId){
		...
		if(ids==''){
	  	...
	  	//设备名称
       	...
       	var team =document.getElementById("dev_team").value;
		 	...
		 	editUrl2 = "<%=contextPath%>/rm/dm/EqDevOutForm/zhDevOperatorModify.jsp?id={id}&mixId="+mixId;
		 	editUrl2 = editUrl2.replace('{id}',selId); 
		 	popWindow(editUrl2,'800:680');  
	 }
	 function toModify(){
	 ...
	 function toBack(){
	 	window.location.href='zhDevOutBaseInfoList.jsp';
	 }
</script>
</html>