...
var queryRootIdSql = "select f.file_id from bgp_doc_gms_file f where f.bsflag = '0' and f.parent_file_id is null and f.is_template is null and f.project_info_no is null and f.is_file = '0'";
...
function getSubNodes(parentNode) {
	...
	Ext.Ajax.request({
		...
		params : {
			currentPage:"1",
			pageSize:"10000",
			querySql:"Select f.file_id,f.file_name,f.file_abbr FROM BGP_DOC_GMS_FILE f WHERE f.PARENT_FILE_ID='"+parentNode.id+"' and f.bsflag='0' and f.is_file='0' ORDER BY f.order_num"	
...
function getModuleByFolder(folderId){
		var moduleID;
		var querySql = "Select b.BGP_DOC_FOLDER_MODULE_ID,b.MODULE_ID FROM bgp_doc_folder_module b WHERE b.folder_id = '"+folderId+"' and b.bsflag='0'";
		if(clickNodeId != undefined){
...
function getModuleFolderInfo(){
	...
	if(clickNodeId != undefined){
		var querySql = "Select b.BGP_DOC_FOLDER_MODULE_ID,b.MODULE_ID FROM bgp_doc_folder_module b WHERE b.module_id = '"+clickNodeId+"' and b.bsflag='0'";
		var queryOrgRet = syncRequest('Post',cruConfig.contextPath+appConfig.queryListAction,'querySql='+querySql);
		if(queryOrgRet.datas.length != 0){
			//执行修改操作
			if(ctt.frames.length != 0){
				...
				var module_text = ctt.frames[0].getNodeText();
	 			if(module_id != undefined){
	 				...
	 				if(folder_ids == ""){//
	 					if(confirm("确定取消文件夹?")){
	 						var retObj = jcdpCallService("ucmSrv", "deleteModuleAndFolder", "moduleId="+module_id+"&isMultiProject=1");
	 					...
	 					var retObj = jcdpCallService("ucmSrv", "setNewMenuFolder", "moduleId="+module_id+"&folderids="+folder_ids+"&moduleText="+module_text+"&folderTexts="+folder_texts+"&isMultiProject=1");
		 				var operation_flag = retObj.operationflag;
		 				if(operation_flag == "success"){
		 					alert("操作成功");
		 				}else{
			...
			if(ctt.frames.length != 0){
				...
				if(module_id != undefined){
					...
					var folder_texts = ctt.frames[1].getCheckedNodeText().split(",");
		 			if(folder_ids != ""){
		 				var retObj = jcdpCallService("ucmSrv", "setMenuFolder", "moduleId="+module_id+"&folderids="+folder_ids+"&moduleText="+module_text+"&folderTexts="+folder_texts+"&isMultiProject=1");
		 				var operation_flag = retObj.operationflag;
		 				if(operation_flag == "success"){
		 					alert("操作成功");
		 				}else{
