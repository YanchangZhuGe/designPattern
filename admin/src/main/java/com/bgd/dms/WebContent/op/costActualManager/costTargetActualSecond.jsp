      	...
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <td align="right" style="padding-right: 20px;"><font color="red"><span id="sum_value"></span></font></td>
				<%if(proc_status){ %>
			   <auth:ListButton functionId="OP_ACTUAL_SE_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			   <auth:ListButton functionId="OP_ACTUAL_SE_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			   <auth:ListButton functionId="OP_ACTUAL_SE_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			   <%} %>
			  </tr>
			</table>
			</td>
			...
			<div id="table_box">
			  <table id="queryRetTable" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			    		<td class="bt_info_odd" exp="<input type='checkbox' name='chk_entity_id' value='{target_matare_id}' id='chk_entity_id' onclick=doCheck(this)/>">
			    		<input id="checkbox" name="checkbox" onclick="" name=chk_entity_id  type=checkbox /></td>
			    	    <td class="bt_info_even" autoOrder="1">序号</td>
			            <td class="bt_info_odd" exp="{dev_name}">名称</td>
			            <td class="bt_info_even" exp="{dev_model}">型号</td>
			            <td class="bt_info_odd" exp="{dev_count}">数量</td>		
			            <td class="bt_info_even" exp="{taxi_unit}">修理单价</td>
			            <td class="bt_info_odd" exp="{update_date}">维修日期</td>
			            <td class="bt_info_even" exp="{dev_sum}" onclick="getSum()">修理费</td>
			        </tr>            
			   </table>
			</div>
			<div id="fenye_box">
...
function refreshData(ids){
	...
	cruConfig.queryStr = "select round(t.dev_count*t.taxi_unit*100)/100 dev_sum,t.* from bgp_op_target_device_matare t where t.bsflag ='0' and t.mataxi_type ='1' and t.project_info_no='"+project_info_no+"'";
	queryData(ids);
}
 
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualSecondEdit.jsp");
}
function toModify(){
	...
	if (ids == '') {
		...
		popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualSecondEdit.jsp?target_matare_id="+ids);
}
function toDelete(){
	ids = getSelIds('chk_entity_id');
	if (ids == '') {
 ...
 function getSum(){
		var project_info_no = '<%=projectInfoNo%>';
		var querySql = "select round(sum(nvl(t.dev_count*t.taxi_unit*100,0)))/100 sum_value "+
			" from bgp_op_target_device_matare t where t.project_info_no='<%=projectInfoNo%>'"+
			" and t.bsflag ='0' and t.mataxi_type ='1' ";
