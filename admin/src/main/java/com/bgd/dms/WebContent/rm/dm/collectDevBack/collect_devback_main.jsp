			...
			<div id="table_box">
			  <table border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
					...
					<!-- <td class="bt_info_odd" exp="{receive_org_name}">接收单位</td> -->
				...
				<div id="tab_box_content1" name="tab_box_content1" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							...
							<td class="bt_info_even" width="8%">离队数量</td>
							<td class="bt_info_odd" width="8%">实际进场时间</td>
							<td class="bt_info_even" width="8%">实际离场时间</td>
...
<script type="text/javascript">
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				var str = "select dui.dev_name,dui.dev_model,'' as unit_name,'1'as back_num,''as total_num,''as unuse_num,''as use_num,dui.actual_in_time,det.actual_in_time as planning_out_time,''as devremark from gms_device_backapp_detail det ";
				str += "left join gms_device_account_dui dui on dui.dev_acc_id=det.dev_acc_id ";
				str += "where det.device_backapp_id='"+currentid+"'";
				var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
				basedatas = queryRet.datas;
				if(basedatas!=undefined && basedatas.length>=1){
					//先清空
	...
	function appendDataToDetailTab(filterobj,datas){
		...
		for(var i=0;i<basedatas.length;i++){
			...
			innerHTML += "<td>"+datas[i].back_num+"</td><td>"+datas[i].total_num+"</td><td>"+datas[i].unuse_num+"</td>";
			innerHTML += "<td>"+datas[i].use_num+"</td>";
			innerHTML += "<td>"+datas[i].actual_in_time+"</td><td>"+datas[i].planning_out_time+"</td><td>"+datas[i].devremark+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
    ...
    function refreshData(v_device_backapp_no, v_backapp_name, v_org_name){
		var str = "select colback.device_backapp_id,colback.device_backapp_no,colback.backapp_name,pro.project_name,";
			str += "org.org_abbreviation as back_org_name,recvorg.org_abbreviation as receive_org_name,";
			str += "emp.employee_name as back_employee_name,colback.backdate,";
			str += "case colback.state when '0' then '未提交' when '9' then '已提交' end as state_desc ";
			str += "from gms_device_collbackapp colback ";
			str += "left join comm_org_information org on colback.back_org_id = org.org_id ";
			str += "left join comm_org_information recvorg on colback.receive_org_id = recvorg.org_id ";
			str += "left join comm_human_employee emp on colback.back_employee_id = emp.employee_id ";

			//井中地震 
			if(projectType == "5000100004000000008" && retFatherNo.length >= 1)//子项目
			{
				str += "left join gp_task_project pro on colback.project_info_id = pro.project_info_no and pro.bsflag = '0' and pro.project_type = '5000100004000000008' ";
				str += "where colback.bsflag = '0' and colback.project_info_id = '" +retFatherNo+"' and backapptype='1' and colback.backdevtype='S9000' ";
			}else if(projectType == "5000100004000000008" && retFatherNo.length <= 0){
	        	str += "left join gp_task_project pro on colback.project_info_id = pro.project_info_no and pro.bsflag = '0' and pro.project_father_no is null and pro.project_type = '5000100004000000008' ";
				str += "where colback.bsflag = '0' and colback.project_info_id='"+projectInfoNos+"' and backapptype='1' and colback.backdevtype='S9000' " ;
	         }else{
	        	str += "left join gp_task_project pro on colback.project_info_id = pro.project_info_no and pro.bsflag = '0' ";
				str += "where colback.bsflag = '0' and colback.project_info_id='"+projectInfoNos+"' and backapptype='1' and colback.backdevtype='S9000' ";
		     }
			
			//str += "left join gp_task_project pro on colback.project_info_id = pro.project_info_no ";
			//str += "where colback.bsflag = '0' and colback.project_info_id = '"+projectInfoNos+"' and backapptype='1' and colback.backdevtype='S9000' ";
		if(v_device_backapp_no!=undefined && v_device_backapp_no!=''){
			str += "and colback.device_backapp_no like '%"+v_device_backapp_no+"%' ";
		}
		if(v_backapp_name!=undefined && v_backapp_name!=''){
			str += "and colback.backapp_name like '%"+v_backapp_name+"%' ";
		}
		if(v_org_name!=undefined && v_org_name!=''){
			str += "and recvorg.org_name like '%"+v_org_name+"%' ";
		}
		str += "order by colback.state asc ,colback.backdate desc ";
	...
	function loadDataDetail(devicebackappid){
    	var retObj;
    	var querySql="select colback.device_backapp_id,colback.DEVICE_BACKAPP_NO,colback.BACKAPP_NAME,pro.project_name,"
		...
		if(devicebackappid!=null){
		    ...
		    querySql += " where colback.device_backapp_id='"+ids+"'";
			var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		...
		$("#back_employee_name","#projectMap").val(retObj[0].back_employee_name);
	...
	function toAddBackPlanPage(){
		popWindow('<%=contextPath%>/rm/dm/collectDevBack/backdetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>','950:680');
	}
	//修改
	function toModifyBackPlanPage(){
		...
		//判断状态
		var querySql = "select device_backapp_id,state ";
		querySql += "from gms_device_collbackapp ";
		querySql += "where bsflag='0' and device_backapp_id ='"+devicebackappid+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var basedatas = queryRet.datas;
		
			if(basedatas[0].state == '9' ){
				stateflag = true;
				var alertinfo = "您选择的记录已提交,不能修改!";
				alert(alertinfo);
				return;
			}
		
		popWindow('<%=contextPath%>/rm/dm/collectDevBack/backdetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+devicebackappid,'950:680');
	}
	//删除
	function toDelMixPlanPage(){
		...
		var querySql = "select device_backapp_id,state ";
		querySql += "from gms_device_collbackapp ";
		querySql += "where device_backapp_id ='"+selectedid+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var basedatas = queryRet.datas;
		var alertinfo ;
			if(basedatas[0].state == '9' ){
		...
		//什么状态不能删除，和业务专家确定
		if(confirm("是否执行删除操作?")){
			var sql = "update gms_device_collbackapp set bsflag='1' where device_backapp_id in ('"+selectedid+"')";
			var path = cruConfig.contextPath+"/rad/asyncDelete.srq";
			var params = "deleteSql="+sql;
			params += "&ids=";
			var retObject = syncRequest('Post',path,params);
			
			var sql1 = "update gms_device_backapp set bsflag='1' where device_backapp_id in ('"+selectedid+"')";
			var path1 = cruConfig.contextPath+"/rad/asyncDelete.srq";
			var params1 = "deleteSql="+sql1;
			params1 += "&ids=";
			var retObject1 = syncRequest('Post',path1,params1);
			refreshData();
		}
	}
	//提交
	function toSumbitDevApp(){
		...
		//查询是否有明细，有则才能提交
		var str = "select account.dev_unit,account.total_num,account.unuse_num,account.use_num,account.actual_in_time ";
		str += "from gms_device_collbackapp_detail backdet ";
		str += "left join gms_device_coll_account_dui account on backdet.dev_acc_id = account.dev_acc_id ";
		str += "where backdet.device_backapp_id='"+selectedid+"'";
		str += "union all ";
		str += "select '' as dev_unit,1 as total_num,null as unuse_num,null as use_num,dui.actual_in_time ";
		str += "from gms_device_backapp_detail det left join gms_device_account_dui dui on dui.dev_acc_id = det.dev_acc_id ";
		str += "where det.device_backapp_id='"+selectedid+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		basedatas = queryRet.datas;
		...
		var basedatas = queryRet.datas;
		if(basedatas[0].state == '9' ){
			alert("您选择的单据状态为'已提交'的单据,不能重复提交!")
			return;
		}
		if(confirm("是否提交返还单？")){
			//给申请时添加的实际离场时间，更新到队级台账中 调用一个方法吧，给动态日志在这个时候保存
			jcdpCallService("DevProSrv","saveCollectSubmitInfo","devicebackappid="+selectedid);
			refreshData();
		}
		//popWindow('<%=contextPath%>/rm/dm/devback/backplan_submit.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+id);
	}
	...
	function toModifyDetail(){
		...
		var querySql = "select device_backapp_id,state ";
		querySql += "from gms_device_collbackapp ";
		querySql += "where device_backapp_id ='"+id+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var basedatas = queryRet.datas;
		var stateflag = false;
		var alertinfo ;
		...
		window.location.href = '<%=contextPath%>/rm/dm/collectDevBack/backPlanDetailList.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+id;
	}
	<%-- //双击进入明细编辑
	function dbclickRow(shuaId){
		...
		var querySql = "select device_backapp_id,state ";
		querySql += "from gms_device_collbackapp ";
		querySql += "where device_backapp_id ='"+shuaId+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var basedatas = queryRet.datas;
		var stateflag = false;
		var alertinfo ;
		...
		window.location.href = '<%=contextPath%>/rm/dm/collectDevBack/backPlanDetailList.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+shuaId;
	} --%>
