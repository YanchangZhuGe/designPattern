		  ...
		  <div style="overflow:auto">
			  <table style="width:100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
					...
					<td class="bt_info_odd" width="12%">开始时间</td>
					<td class="bt_info_even" width="12%">结束时间</td>
					<td class="bt_info_odd" width="8%">备注</td>
					<td class="bt_info_even" width="9%">出库单位</td>
				</tr>
...
$().ready(function(){
	...
	function submitInfo(){
		...
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveMixAppDetailInfo.srq?count="+count+"&line_infos="+line_infos+"&idinfos="+idinfos;
	...
	function refreshData(){
		...
		var basedatas;
		//先查询基本信息 
		var basesql = "select pro.project_name,allapp.device_allapp_id,allapp.device_allapp_no,allapp.device_allapp_name,app.mix_type_id from gms_device_app app "
		...
		var mix_type_id = baseObj[0].mix_type_id;
		var device_allapp_id = baseObj[0].device_allapp_id;
		
		if('<%=deviceappid%>'!=null){
			
			var conditionsql = "select mix_type_id from gms_device_app where device_app_id='<%=deviceappid%>'";
			var conditionRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+conditionsql);
			var conditionObj = conditionRet.datas;
			var conditionStr = "";
			if(mix_type_id == '<%=DevConstants.MIXTYPE_ZHENYUAN%>'){
				conditionStr += " and (substr(aad.dev_ci_code,1,5) = '"+conditionObj[0].mix_type_id+"' or 'S'||substr(aad.dev_ci_code,0,4) = '"+conditionObj[0].mix_type_id+"' ) ";
			}else if(mix_type_id == '<%=DevConstants.MIXTYPE_YIQI%>'){
				conditionStr += " and (((substr(aad.dev_ci_code,1,5) = '"+conditionObj[0].mix_type_id+"' or 'S'||substr(aad.dev_ci_code,0,4) = '"+conditionObj[0].mix_type_id+"') ";
				conditionStr += " and (substr(aad.dev_ci_code,1,9)!='<%=DevConstants.MIXTYPE_JIANBOQI%>' and 'S'||substr(aad.dev_ci_code,0,8)!='<%=DevConstants.MIXTYPE_JIANBOQI%>') ";
				conditionStr += " and (substr(aad.dev_ci_code,1,7)!='<%=DevConstants.MIXTYPE_XIAOZHESHEYI%>' and 'S'||substr(aad.dev_ci_code,0,6)!='<%=DevConstants.MIXTYPE_XIAOZHESHEYI%>') ";
				conditionStr += " and (substr(aad.dev_ci_code,1,9)!='<%=DevConstants.MIXTYPE_JIANBOQICESHIYI%>' and 'S'||substr(aad.dev_ci_code,0,8)!='<%=DevConstants.MIXTYPE_JIANBOQICESHIYI%>')) ";
				conditionStr += " or (substr(aad.dev_ci_code,1,5) = '<%=DevConstants.MIXTYPE_SHUZIJIANBOQI%>' or 'S'||substr(aad.dev_ci_code,0,4) = '<%=DevConstants.MIXTYPE_SHUZIJIANBOQI%>' )";
				conditionStr += " or (substr(aad.dev_ci_code,1,5) = '<%=DevConstants.MIXTYPE_YIQICHE%>' or 'S'||substr(aad.dev_ci_code,0,4) = '<%=DevConstants.MIXTYPE_YIQICHE%>')) ";
			}else if(mix_type_id == '<%=DevConstants.MIXTYPE_CELIANG%>'){
				conditionStr += " and ((substr(aad.dev_ci_code,1,7) = '<%=DevConstants.MIXTYPE_CELIANG_01%>' or 'S'||substr(aad.dev_ci_code,0,6) = '<%=DevConstants.MIXTYPE_CELIANG_01%>') ";
				conditionStr += " or (substr(aad.dev_ci_code,1,7) = '<%=DevConstants.MIXTYPE_CELIANG_02%>' or 'S'||substr(aad.dev_ci_code,0,6) = '<%=DevConstants.MIXTYPE_CELIANG_02%>')";
				conditionStr += " or (substr(aad.dev_ci_code,1,7) = '<%=DevConstants.MIXTYPE_CELIANG_03%>' or 'S'||substr(aad.dev_ci_code,0,6) = '<%=DevConstants.MIXTYPE_CELIANG_03%>')) ";
			}else{
				conditionStr += "and (((substr(aad.dev_ci_code,1,5) != '<%=DevConstants.MIXTYPE_ZHENYUAN%>' and 'S'||substr(aad.dev_ci_code,0,4) != '<%=DevConstants.MIXTYPE_ZHENYUAN%>')";
				conditionStr += " and (substr(aad.dev_ci_code,1,5) != '<%=DevConstants.MIXTYPE_YIQI%>'  and 'S'||substr(aad.dev_ci_code,0,4) != '<%=DevConstants.MIXTYPE_YIQI%>')";
				conditionStr += " and (substr(aad.dev_ci_code,1,5) != '<%=DevConstants.MIXTYPE_YIQICHE%>' and 'S'||substr(aad.dev_ci_code,0,4) != '<%=DevConstants.MIXTYPE_YIQICHE%>')";
				conditionStr += " and (substr(aad.dev_ci_code,1,7) != '<%=DevConstants.MIXTYPE_CELIANG_01%>' and 'S'||substr(aad.dev_ci_code,0,6) != '<%=DevConstants.MIXTYPE_CELIANG_01%>') ";
				conditionStr += " and (substr(aad.dev_ci_code,1,7) != '<%=DevConstants.MIXTYPE_CELIANG_02%>' and 'S'||substr(aad.dev_ci_code,0,6) != '<%=DevConstants.MIXTYPE_CELIANG_02%>')";
				conditionStr += " and (substr(aad.dev_ci_code,1,7) != '<%=DevConstants.MIXTYPE_CELIANG_03%>' and 'S'||substr(aad.dev_ci_code,0,6) != '<%=DevConstants.MIXTYPE_CELIANG_03%>')) ";
				conditionStr += " or ((substr(aad.dev_ci_code,1,9)='<%=DevConstants.MIXTYPE_JIANBOQI%>' or 'S'||substr(aad.dev_ci_code,0,8)='<%=DevConstants.MIXTYPE_JIANBOQI%>' ) ";
				conditionStr += " and (substr(aad.dev_ci_code,1,11)!='<%=DevConstants.MIXTYPE_SHUZIJIANBOQI%>' and 'S'||substr(aad.dev_ci_code,0,10)!='<%=DevConstants.MIXTYPE_SHUZIJIANBOQI%>')) ";
				conditionStr += " or (substr(aad.dev_ci_code,1,7)='<%=DevConstants.MIXTYPE_XIAOZHESHEYI%>' or 'S'||substr(aad.dev_ci_code,0,6)='<%=DevConstants.MIXTYPE_XIAOZHESHEYI%>' ) ";
				conditionStr += " or (substr(aad.dev_ci_code,1,9)='<%=DevConstants.MIXTYPE_JIANBOQICESHIYI%>' or 'S'||substr(aad.dev_ci_code,0,8)='<%=DevConstants.MIXTYPE_JIANBOQICESHIYI%>' )) ";
			}
			var prosql = "select aad.device_allapp_detid,aad.team,aad.teamid,aad.dev_ci_code,aad.isdevicecode,";
				...
				//补充数据类别权限
				//prosql += conditionStr;
				//prosql += "order by aad.device_addapp_id nulls last,aad.teamid,aad.team ";
		...
		for(var index=0;index<retObj.length;index++){
			...
			innerhtml += "<td width='5%'><input type='checkbox' name='detinfo' id='"+index+"' value='"+retObj[index].device_allapp_detid+"'/></td>";
			...
			innerhtml += "<td width='6%'><input name='applyednum"+index+"' id='applyednum"+index+"' value='"+retObj[index].applyed_num+"' size='6' type='text' readonly/></td>";
			innerhtml += "<td width='6%'><input name='applynum"+index+"' id='applynum"+index+"' detindex='"+index+"' value='' size='2' type='text' onkeyup='checkAssignNum(this)'/></td>";
