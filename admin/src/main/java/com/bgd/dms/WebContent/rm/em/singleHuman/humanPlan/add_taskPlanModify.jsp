...
['BGP_COMM_HUMAN_PLAN_DETAIL']
);
var defaultTableName = 'BGP_COMM_HUMAN_PLAN_DETAIL';
...
function addLine( plan_detail_ids, apply_teams, posts, people_numbers,profess_numbers, plan_start_dates, plan_end_dates, numss, notess){
	...
	if(post == ""){
		...
		td.className=classCss+"odd";
		td.innerHTML = '<select  style="width:150px;"  name="post' + '_' + rowNum + '" id="post' + '_' + rowNum + '" > <option value="">请选择</option> </select>';
	}else{	
		//选择岗位值
		var td = tr.insertCell(2);
		td.className=classCss+"odd";
		td.innerHTML = '<select  style="width:150px;" name="post' + '_' + rowNum + '" id="post' + '_' + rowNum + '" >'+getPostForList(apply_team,post)+'</select>';
	}
	...
	td.className=classCss+"odd";
	td.innerHTML = '<input type="text" class="input_width" id="profess_number' + '_' + rowNum + '" name="profess_number' + '_' + rowNum + '" value="'+profess_number+'" onpropertychange="if(isNaN(value)) value=value.substring(0,value.length-1);"/>';
...
function page_init(){
	...
	if(task_id!='null'){
		var querySql1 = " select t.name,t.planned_duration,t.planned_start_date,t.planned_finish_date  from bgp_p6_activity t where t.object_id='"+task_id+"' ";
		var queryRet1 = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql1);
		var datas1 = queryRet1.datas;
		
		if(datas1 != null){
			document.getElementById("name").value=datas1[0].name;
			document.getElementById("planned_duration").value=datas1[0].planned_duration;
			document.getElementById("start_date").value=datas1[0].planned_start_date;
			document.getElementById("finish_date").value=datas1[0].planned_finish_date;

		}
	}
	if(plan_detail_id!='null'){
		var querySql = "select d.plan_detail_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,d.people_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1) nums,d.notes,d.profess_number from bgp_comm_human_plan_detail d left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0' and s1.coding_mnemonic_id='<%=projectType%>'  left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' and s2.coding_mnemonic_id='<%=projectType%>'   where d.plan_detail_id='"+plan_detail_id+"' and d.spare1 is null  and d.bsflag='0' order by d.modifi_date  ";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var datas = queryRet.datas;
		for (var i = 0; datas && queryRet.datas.length; i++) {			
			addLine(datas[i].plan_detail_id,datas[i].apply_team,datas[i].post,datas[i].people_number,datas[i].profess_number,datas[i].plan_start_date,datas[i].plan_end_date,datas[i].nums,datas[i].notes);
		}			
	}
...
function toDownload(){
	...
	if(projectTyp!="" && projectTyp !="null"){
		if(projectTyp =="5000100004000000006"){  //深海模板
			elemIF.src = "<%=contextPath%>/rm/em/humanRequest/download.jsp?path=/rm/em/singleHuman/humanPlan/importHumanPlanListSh.xls&filename="+iName+".xls";
		}else if (projectTyp =="5000100004000000009"){  // 综合模板
			elemIF.src = "<%=contextPath%>/rm/em/humanRequest/download.jsp?path=/rm/em/singleHuman/humanPlan/importHumanPlanListZh.xls&filename="+iName+".xls";
		}else { //其他类型
			elemIF.src = "<%=contextPath%>/rm/em/humanRequest/download.jsp?path=/rm/em/singleHuman/humanPlan/importHumanPlanListLd.xls&filename="+iName+".xls";
		}
	}
...
function exportData(){
	var obj=window.showModalDialog('<%=contextPath%>/rm/em/singleHuman/humanPlan/humanPlanImportFile.jsp',"","dialogHeight:500px;dialogWidth:600px");
	if(obj!="" && obj!=undefined ){	
		...
		for (var i = 0; i<checkStr.length; i++) {	
			...
			var days=(new Date(check[5].replace(/-/g,'/'))-new Date(check[4].replace(/-/g,'/')))/3600/24/1000+1;
			addLine("",check[0],check[1],check[2],check[3],check[4],check[5],days,"");			
		}			
	}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
    <tr>
    	<td class="inquire_item6">作业名称：</td>
      	<td class="inquire_form6"><input type="hidden" id="task_id" name="task_id" value="<%=taskId %>" class="input_width" />
      	<input type="text" id="name" value="" class="input_width" /></td>
      	<td class="inquire_item6">&nbsp;</td>
      	<td class="inquire_form6"><input type="hidden" id="project_info_no" name="project_info_no" value="<%=projectInfoNo %>" class="input_width" /></td>
      	<td class="inquire_item6">&nbsp;</td>
      	<td class="inquire_form6">&nbsp;</td>
    </tr>
    <tr>
    	<td class="inquire_item6">计划开始时间：</td>
      	<td class="inquire_form6"><input id="start_date" class="input_width" type="text" readonly/></td>
      	<td class="inquire_item6">计划结束时间：</td>
      	<td class="inquire_form6"><input id="finish_date" class="input_width" type="text" readonly/></td>
      	<td class="inquire_item6">原定工期：</td>
      	<td class="inquire_form6"><input id="planned_duration" class="input_width" type="text" readonly/></td>
    </tr>
</table>
</div>  
	<div id="inq_tool_box">
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr align="right">
		<td width="88%">&nbsp;</td>
	    <td>
	    <span class="zj" style="display:<%=addButtonDisplay%>"  title="添加" ><a href="#" onclick="addLine()"></a></span>
	    <span class="xz" style="display:<%=addButtonDisplay%>"  title="下载附件" ><a href="#" onclick="toDownload()"></a></span>
	    <span class="dr" style="display:<%=addButtonDisplay%>"   title="导入" ><a href="#" onclick="exportData()"></a></span>
	    </td>
	  </tr>
	</table>
	</td>
	...
	<table id="lineTable" width="99%" border="0" cellspacing="0" cellpadding="0"  style="margin-top:2px;" >
    	<tr>
            ...
            <td class="bt_info_odd" width="8%">其中专业化人数</td>
            ...
            <td class="bt_info_even" width="4%">操作<input type="hidden" id="lineNum" value="0"/></td> 
        </tr>
    </table>	
