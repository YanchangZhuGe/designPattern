			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_odd" exp="<input name='demand_num_{recyclemat_info}'  type='text' value='{unit_num}' />" >需求数量</td>
			      <td class="bt_info_even" exp="<input name='regulate_num_{recyclemat_info}'  type='text' value='' />" >调剂数量</td>
		 	      <td class="bt_info_odd" exp="<input name='demand_date_{recyclemat_info}' id='demand_date_{recyclemat_info}' type='text' value='<%=str %>'/><img src='<%=contextPath%>/images/calendar.gif'id='tributton_{recyclemat_info}' width='16' height='16' style='cursor: hand;' onmouseover='calDateSelector(demand_date_{recyclemat_info},tributton_{recyclemat_info});'/>" >需求日期 </td>
	  ...
	  function toSave(){ 
		  ids = getSelIds('rdo_entity_id');
			if (ids == '') {
				...
				document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/plan/savePlanWZXZ.srq";
				openMask();
...
//			var ids=getSelIds('rdo_entity_id');
	...
	function loadDataDetail(shuaId){
		...
		for(var i=1;i<row.length;i++){
			var cell_6 = row[i].cells[10].childNodes[1].value;
			
			var cell_7 = row[i].cells[10].childNodes[2].value;
		}
