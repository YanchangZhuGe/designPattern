...
<%
	...
	Map map2 = srv.getActivityInformation(activity_object_id);
	String wbs_name = map2 == null || map2.get("wbs_name")==null ?"":(String) map2.get("wbs_name");
%>
<html>
<head>
...
function toAdd(){
	...
	document.form1.action="<%=contextPath%>/pm/dailyReport/singleProject/tqh/saveOrUpdateQhResourceAssignment.srq";
...
<table  border="0" cellpadding="0" cellspacing="0" <%if(wbs_name!=null && wbs_name.indexOf("采集")==-1){ %> style="display:none;"<%} %> class="form_info" width="100%" id="lineTable">
	<tr class="bt_info">
...
function saveStartEnd(){//保存线束（测线）号和起止桩号:
    debugger;
	if(wbs_name.indexOf('采集')!=-1){//如果选择的是采集工序，才 保存线束（测线）号和起止桩号:
 		<%-- 设计思路：保存数据之前删除<%=projectInfoNo%><%=produceDate%>作业=<%=activity_object_id%>的所有数据，
 		
		然后添加现有行所保存的数据，简化了添加和删除操作  --%>
 		var querySql = "select t.* from bgp_pm_daily_information t where t.bsflag ='0' and t.project_info_no ='<%=projectInfoNo%>' "+
 		"and t.produce_date = to_date('<%=produceDate%>','yyyy-MM-dd') and t.activity_object_id ='<%=activity_object_id%>' ";
		...
		var sql = "";
			...
			var start_end = document.getElementById("start_end").value;
			var qq_shot_num = document.getElementById("qq_shot_num").value;
			var jp_shot_num = document.getElementById("jp_shot_num").value;
			var zy_shot_num = document.getElementById("zy_shot_num").value;
			if( retObj.datas.length !=0 ){
				sql ="delete bgp_pm_daily_information t where t.project_info_no ='<%=projectInfoNo%>' and t.activity_object_id ='<%=activity_object_id%>'"+
				" and t.produce_date = to_date('<%=produceDate%>','yyyy-MM-dd');"+sql;  
				var retObj = jcdpCallService("QualitySrv", "saveQualityBySql", "sql="+sql);
			}
			sql =sql + "insert into bgp_pm_daily_information(information_id,project_info_no,produce_date,activity_object_id,activity_name,"+
			"qq_shot_num,jp_shot_num,zy_shot_num,bsflag,creator_date,creator,modifi_date,line_num,daily_no,start_end)values((lower(sys_guid())),"+
			"'<%=projectInfoNo%>',to_date('<%=produceDate%>','yyyy-MM-dd'),'<%=activity_object_id%>','<%=taskName%>','"+qq_shot_num+"',"+
			"'"+jp_shot_num+"','"+zy_shot_num+"','0',sysdate,'<%=user_name%>',sysdate,'"+line_num+"',(select r.daily_no from gp_ops_daily_report r"+
			" where r.bsflag ='0' and r.project_info_no ='<%=projectInfoNo%>' and r.produce_date = to_date('<%=produceDate%>','yyyy-MM-dd')),'"+start_end+"');";
...
function addLine(map){
	if(map!=null){
		with(map){
			var tr = document.getElementById("lineTable").insertRow(document.getElementById("lineTable").rows.length);
			var td = tr.insertCell();
			 
			td.innerHTML = "<input id='line_num' type='text' class='input_width' value='"+line_num+"'/>";
			td = tr.insertCell();
			td.innerHTML = "<input id='start_end' type='text' class='input_width' value='"+start_end+"'/>";
			td = tr.insertCell();
			td.innerHTML = "<input id='qq_shot_num' type='text' class='input_width' value='"+qq_shot_num+"'/>";
			td = tr.insertCell();
			td.innerHTML = "<input id='jp_shot_num' type='text' class='input_width' value='"+jp_shot_num+"'/>";
			td = tr.insertCell();
			td.innerHTML = "<input id='zy_shot_num' type='text' class='input_width' value='"+zy_shot_num+"'/>";
		}
	}else{
		var tr = document.getElementById("lineTable").insertRow(document.getElementById("lineTable").rows.length);
		var td = tr.insertCell();
 		td.innerHTML = "<input id='line_num' type='text' class='input_width' value=''/>";
		td = tr.insertCell();
		td.innerHTML = "<input id='start_end' type='text' class='input_width' value=''/>";
		td = tr.insertCell();
		td.innerHTML = "<input id='qq_shot_num' type='text' class='input_width' value=''/>";
		td = tr.insertCell();
		td.innerHTML = "<input id='jp_shot_num' type='text' class='input_width' value=''/>";
		td = tr.insertCell();
		td.innerHTML = "<input id='zy_shot_num' type='text' class='input_width' value=''/>";
	}
	line_index++;
}
</script>
