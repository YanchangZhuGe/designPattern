...
function save(){
	...
	// 检查是否已存在
	var checkSql="select * from bgp_wr_income_money where to_char(week_date,'yyyy-MM-dd')='" + week_date + "' and bsflag='0' and org_type='1' and type='2'";
    var queryRet = syncRequest('Post','<%=request.getContextPath()%>'+appConfig.queryListAction,'querySql='+checkSql);
	var datas = queryRet.datas;
	var martandpro_id="";
	var subflag="0";
	var action="add";
	if(datas && datas[0] && datas[0].income_id){// 数据已存在
		martandpro_id = datas[0].martandpro_id;
		subflag=datas[0].subflag;
		action="edit";
	}
	if(datas && datas[0] && datas[0].income_id){
		alert("日期已存在，请重新选择日期！");
	}else{
		window.location="<%=contextPath%>/market/month/addValueReport.srq?week_date="+week_date+"&week_end_date="+week_end_date+"&action=add";
	}
}
