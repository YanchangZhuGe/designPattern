...
<%
	...
	String if_country = request.getParameter("if_country");
	if(if_country==null || if_country.trim().equals("")){
		if_country = "";
	}
%>
...
<html xmlns="http://www.w3.org/1999/xhtml">
				...
				<tr>
					<td valign="middle">
						<div class="tongyong_box" >
							<div class="tongyong_box_title"><span class="kb"><a href="#"></a></span><a href="#"><font id="name"></font>基本情况统计表</a></div>
	...
	function refreshData(){
		...
		var if_country = '<%=if_country%>';
		var myChart = new FusionCharts("${applicationScope.fusionChartsURL}/Charts/MSColumn3D.swf", "myChartId", "97%", "400", "0", "0" );   
	 	var substr = "device_name="+device_name+"&parent_id="+parent_id+"&num_type="+num_type+"&if_country="+if_country;
	 	var retObj = jcdpCallService("MainEquiBasiStatSrv", "firstLevel", substr);
	...
	refreshData();
	function fusionChart(device_name,parent_id,num_type){
		if(num_type==null || num_type==undefined || num_type==""){
			num_type = "";
		}
		var obj = {};
		device_name = encodeURI(encodeURI(device_name));
		popWindow('<%=contextPath %>/dmsManager/use/statAnal/popSMainEquiStatChart.jsp?parent_id='+parent_id+'&num_type='+num_type+'&device_name='+device_name,'800:572');
	}
	</script>
