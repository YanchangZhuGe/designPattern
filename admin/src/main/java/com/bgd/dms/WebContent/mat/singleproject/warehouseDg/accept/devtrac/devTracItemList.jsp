...
<script type="text/javascript">
	...
	function toOut(){
	  ...
	  function toAdd(){ 
			  ...
			  popWindow("<%=contextPath%>/mat/singleproject/warehouseDg/accept/devtrac/devMatRet.jsp",'1024:800');
       ...
       function simpleSearch(){
    	   var sql ='';
			...
			sql+="select * from (select dui.dev_name,dui.dev_model,dui.self_num,dui.license_num,dui.dev_sign,t.note,t.out_date,t.total_money,t.out_info_id,t.invoices_id,i.org_name,decode(t.if_submit,'0','未提交','1','已提交') if_submit from gms_mat_out_info t  left join gms_device_account_dui dui on t.dev_acc_id=dui.dev_acc_id inner join comm_org_information i on t.org_id=i.org_id and t.bsflag='0'and t.out_type='5'and t.project_info_no='<%=projectInfoNo%>') ttt where ttt.invoices_id like'%"+wz_name+"%'"
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = sql;
    	  ...
    	  if(ids==''){ 
			    alert("请先选中一条记录!");
	     		return;
	 	    		 ...
	 	    		 popWindow("<%=contextPath%>/mat/singleproject/warehouseDg/accept/devtrac/devMatOutEdit.jsp?laborId="+ids,'1024:800');
			 	    	}
      }
      function toDelete(){
    	  ids = getSelIds('rdo_entity_id');
