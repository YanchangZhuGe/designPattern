...
<%-- 	var inspection_id='<%=inspection_id%>'; --%>
	...
	function submitInfo(){
		if(confirm("提交后数据不能修改，确认提交？")){
			...
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/bgpCommDeviceInspection.srq?state=9&inspection_id="+dev_appdet_id;
	...
	function loadDataDetail(){
			var querySql="select a.*,b.dev_name,b.dev_model,b.self_num,b.license_num,b.dev_coding,b.owning_org_name,b.usage_org_name,b.using_stat,b.tech_stat from BGP_COMM_DEVICE_INSPECTION a left join GMS_DEVICE_ACCOUNT b on a.fk_dev_acc_id = b.dev_acc_id where a.inspection_id ='"+dev_appdet_id+"'" ;
			...
			$("#RECTIFY_DATE").datebox("setValue", basedatas[0].rectify_date);
			$("#INSPECTION_DATE").datebox("setValue", basedatas[0].inspection_date);
