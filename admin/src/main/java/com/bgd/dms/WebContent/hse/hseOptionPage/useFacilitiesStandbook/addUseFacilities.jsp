...
<%
	...
	String usefacilities_no="";
	if(request.getParameter("usefacilities_no") != null){
		usefacilities_no=request.getParameter("usefacilities_no");	
	...
	function checkJudge(){
 	...
 	function submitButton(){   
 		...
 		var isProject = "<%=isProject%>";
			...
			var submitStr = 'JCDP_TABLE_NAME=BGP_USEFACILITIES_STAND&JCDP_TABLE_ID=&org_sub_id='+org_sub_id
			+'&second_org='+second_org+'&third_org='+third_org+'&management_number='+management_number+'&inspection_date='+inspection_date
			+'&creator=<%=userName%>&updator=<%=userName%>&create_date=<%=curDate%>&modifi_date=<%=curDate%>&bsflag=0';
			if(isProject!=null&&isProject=="2"){
				submitStr = submitStr + '&project_no=<%=user.getProjectInfoNo()%>';
			}
			var retObject = syncRequest('Post',path,encodeURI(encodeURI(submitStr)));  //保存主表信息		
			var usefacilities_no=retObject.entity_id; //获得 主表id 			
 			top.frames('list').refreshData();	 		
 		 
 				// window.open("<%=contextPath%>/hse/hseOptionPage/useFacilitiesStandbook/homeFrame.jsp?addTypes=1&usefacilities_no="+usefacilities_no,'homeMain','height=500,width=1000px,left=100px,top=100px,menubar=no,status=no,toolbar=no ', '');
 			...
 			if(isProject=="1"){
				 ...
				 window.open("<%=contextPath%>/hse/hseOptionPage/useFacilitiesStandbook/homeFrameP.jsp?addTypes=1&usefacilities_no="+usefacilities_no,'homeMain','height=500,width=1000px,left=100px,top=100px,menubar=no,status=no,toolbar=no ', '');
...
querySql1 = "select count(*)-(-1) as num_p from BGP_USEFACILITIES_STAND   where bsflag='0'";
...
if(queryRet1.returnCode=='0'){
  datas1 = queryRet1.datas;	 
	if(datas1 != null && datas1 != ''){	 
		...
		document.getElementsByName("management_number")[0].value=numDate;			
	}
}	
