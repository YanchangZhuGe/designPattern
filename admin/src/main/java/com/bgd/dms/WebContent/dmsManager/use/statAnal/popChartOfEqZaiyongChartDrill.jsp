...
<script type="text/javascript">
	...
	function getFusionChart(){
		...
		myChart.setXMLUrl("<%=contextPath%>/dms/use/getEqUseComDistribute.srq?code=<%=code%>");      
		myChart.render("chartContainer"); 
	}
	function drillEQUseProDistribute(obj){
		var chartReference = FusionCharts("myChartId"); 
		var paramobj = obj.split("~",-1);
		var retObj = jcdpCallService("DzyqStatSrv","drillEQUseProDistribute","code="+paramobj[0]+"&orgsubid="+paramobj[1]);
		var dataXml = retObj.xmldata;
		chartReference.setXMLData(dataXml);
	}
	function drillEQUseBack(obj){
		var chartReference = FusionCharts("myChartId"); 
		var retObj = jcdpCallService("DzyqStatSrv","getEqUseComDistribute","code="+obj);
		var dataXml = retObj.xmldata;
