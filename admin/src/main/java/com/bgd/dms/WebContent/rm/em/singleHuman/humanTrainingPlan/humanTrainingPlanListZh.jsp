...
<script type="text/javascript">
	...
	function toSubmit(){
		...
		var sql = "update BGP_COMM_HUMAN_TRAINING_PLAN set  modifi_date=sysdate ,proc_status='2' ,spare2='1'  where train_plan_no ='"+train_plan_no+"'";
	...
	function refreshData(){
		cruConfig.queryStr = "select pn.train_plan_no, pn.plan_number,pn.project_info_no,nvl(pn.train_amount,'0') train_amount,nvl(pn.spare2, '0') proc_status, p.project_name,  i.org_id,      nvl( pn.org_name ,i.org_abbreviation)applicant_org_name,   decode(pn.spare2,   '1',   '已提交','未提交') proc_status_name,pn.creator,       To_char(pn.modifi_date, 'yyyy-MM-dd hh24:mi') create_date,pn.spare1 ,  t2.employee_name applicant_name  from BGP_COMM_HUMAN_TRAINING_PLAN  pn                left join gp_task_project p    on pn.project_info_no = p.project_info_no   and p.bsflag = '0'    left join comm_org_information i    on pn.spare1 = i.org_id   and i.bsflag = '0'       left join comm_human_employee t2    on pn.creator = t2.employee_id   and t2.bsflag = '0'  where pn.bsflag='0' and pn.project_info_no='<%=projectInfoNo%>' order by pn.create_date desc ";
