...
<%
	...
	if (selectWzId != null
			&& (selectWzId.equals("undefined") || selectWzId
					.equals("null"))) {
		selectWzId = "";
	}
%>
					...
					<td background="<%=contextPath%>/images/list_15.png"><table
							...
							<tr>
								...
								<auth:ListButton functionId="" css="tj"
									event="onclick='saveBake()'" title="JCDP_btn_submit"></auth:ListButton>
							</tr>
						</table></td>
			...
			<table width="98%" border="0" cellspacing="0" cellpadding="0"
				...
				<tr >
					<td class="bt_info_odd"
						exp="<input type='checkbox' name='rdo_entity_name'   value='{self_num}' id='rdo_entity_name_{self_num}' {selectflag}/></td>
					<td class="bt_info_odd" exp="{self_num}">全选<input  onclick="checkAll(this.checked)"  type='checkbox'  id='allCheck' ></td>
...
<script type="text/javascript">
  ...
  var checked = false;
	...
	function refreshData(arrObj){
		...
		//str += "  where  t.owning_sub_id like 'C105006%'  and t.dev_type like 'S062301%'  and t.bsflag='0' and (ifcountry='国内' or ifcountry is null)";
		str+="  where  t.bsflag='0' and  t.owning_sub_id = '"+userid+"'  and t.dev_type like 'S062301%'    and (ifcountry='国内' or ifcountry is null) ";
		//if(selectWzId!="")
		//{	
		...
		for ( var key in arrObj) {
			if (arrObj[key].value != undefined && arrObj[key].value != '')
				str += "and " + arrObj[key].label + " like '%"
						+ arrObj[key].value + "%' ";
		}
		if(dev_model!=undefined&&dev_model!=''){
			str += "  and   t.dev_model  like '%"+dev_model+"%'  ";
		}
		str +=" and  account_stat='0110000013000000003' ";
