			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_even" exp="{wz_from}">物资来源</td>
			    </tr>
			  </table>
			</div>
...
<script type="text/javascript">
	...
	function refreshData(){
		var sql ="select t.invoices_id,t.invoices_no,t.total_money,t.input_date,t.operator,t.note,t.pickupgoods,t.storage,decode(t.wz_from,'1','大港分中心','2','转入',oi.org_abbreviation) wz_from from GMS_MAT_TEAMMAT_INVOICES t left join comm_org_information i on t.source=i.org_id and i.bsflag='0' left join comm_org_information oi on t.wz_from = oi.org_id and oi.bsflag='0' where t.project_info_no='<%=projectInfoNo%>' and t.invoices_type='5' and t.if_input='0' and t.bsflag='0' order by t.input_date desc,t.create_date desc";
	...
	var showTabBox = document.getElementById("tab_box_content0");
	  function toAdd(){ 
			  popWindow("<%=contextPath%>/mat/singleproject/warehouseDg/accept/selfacceptNew/acceptLedgerAdd.jsp",'1024:800');
		...
		function toEdit(){
			...
			for(var i=1;i<row.length;i++){
				var cell_0 = row[i].cells[0].firstChild.value;
				debugger;
				if(cell_0 == ids){
					if(row[i].cells[8].innerHTML=="大港分中心"||row[i].cells[8].innerHTML=="转入"){
						popWindow("<%=contextPath%>/mat/singleproject/warehouse/accept/selfaccept/queryAcceptDatasDg.srq?laborId="+ids,'1024:800');
					}else{
						alert("专业化单位调配物资不能够修改!");
					}
       ...
       function simpleSearch(){
    	   var sql ='';
			...
			sql+="select t.invoices_id,t.invoices_no,t.total_money,t.input_date,t.operator,t.note,t.pickupgoods,t.storage,decode(t.wz_from,'1','大港分中心','2','转入',oi.org_abbreviation) wz_from from GMS_MAT_TEAMMAT_INVOICES t left join comm_org_information i on t.source=i.org_id and i.bsflag='0' left join comm_org_information oi on t.wz_from = oi.org_id and oi.bsflag='0' where t.project_info_no='<%=projectInfoNo%>' and t.invoices_type='5' and t.if_input='0' and t.bsflag='0'  and t.invoices_id like'%"+wz_name+"%' order by t.input_date desc,t.create_date desc";
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = sql;
			cruConfig.currentPageUrl = "/mat/singleproject/matLedger/matItemList.jsp";
			queryData(1);
