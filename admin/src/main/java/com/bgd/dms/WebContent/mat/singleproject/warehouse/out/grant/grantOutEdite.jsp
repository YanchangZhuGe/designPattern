...
<form name="form1" id="form1" method="post"
	...
	<input type='hidden' name='dev_acc_id' id='dev_acc_id' value='<gms:msg msgTag="matInfo" key="dev_acc_id"/>'/>
	<input type='hidden' name='submite_number' id='submite_number' value='<gms:msg msgTag="matInfo" key="plan_invoice_id"/>'/>
...
<table border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<td class="inquire_item6"><input type="text"
			...
			value="<gms:msg msgTag="matInfo" key="device_use_name"/>" readonly/>
			
		</td>
		<td class="inquire_item6">班组/设备：</td>
		<td class="inquire_item6"><input type="text"
			name="selectText" id="selectText" class="input_width"
			value="<gms:msg msgTag="matInfo" key="tname"/>" readonly />
			</td>
	...
	<tr>
		...
		<td class="inquire_item6"><input type="text"
			...
			value="<gms:msg msgTag="matInfo" key="outmat_date"/>" readonly />
		...
		<td class="inquire_item6"><input type="text" name="drawer" id="drawer"
			class="input_width"
			value="<gms:msg msgTag="matInfo" key="drawer"/>" readonly/></td>
		<td class="inquire_item6">保管员：</td>
		<td class="inquire_item6"><input type="text" name="storage" id="storage"
			class="input_width" value="<gms:msg msgTag="matInfo" key="storage"/>" readonly/></td>
	</tr>
	<tr>
		<td class="inquire_item6">提料人：</td>
		<td class="inquire_item6"><input type="text" name="pickupgoods" id="pickupgoods"
			class="input_width" value="<gms:msg msgTag="matInfo" key="pickupgoods"/>" readonly/></td>
		<td class="inquire_item6">备注：</td>
		<td class="inquire_item6"><input type="text" name="note" id="note"
			class="input_width" value="<gms:msg msgTag="matInfo" key="note"/>" readonly/></td>
	...
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" id="queryRetTable">
			<tr>
				...
				<td class="bt_info_odd" exp="{mat_num}">虚拟出库数量</td>
				<td class="bt_info_odd" exp="<input type='text' name='mat_num_{wz_id}' id='mat_num_{wz_id}' value='{mat_num}'  style='width:40px'/>">出库数量</td>
				<td class="bt_info_even" exp="<input type='text' name='actual_price_{wz_id}' id='actual_price_{wz_id}' value='{actual_price}'  style='width:40px'/>">入库单价</td>
				<td class="bt_info_odd" exp="<input type='text' name='total_money_{wz_id}' id='total_money_{wz_id}' value='{total_money}'  style='width:40px'/><input type='hidden' name='data_{wz_id}' id='data_{wz_id}'>">金额</td>
...
<script type="text/javascript"><!--
	...
	cruConfig.queryStr = "";
	var checkIds="";
	var teammatOutId = "<gms:msg msgTag="matInfo" key="teammat_out_id"/>";
	function refreshData(){
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getOutEditLeaf";
		cruConfig.submitStr ="teammatOutId="+teammatOutId;
		queryData(1);
					 ...
					 if(checkNum())
						{
							...
							document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/warehouse/out/grant/saveEditGrant.srq";
							document.getElementById("form1").submit();
					 }
			 }
			 else{
					alert("本单据已是虚拟出库，请选择正常出库！");
					return;
			 }
		 }
	}
		...
		var tabNum='';
	  	   for(var i=0;i<rows.length;i++){
	  		   ...
	  		   if(cells[0].firstChild.checked){
		  		   ...
		  		   var cell_7 = cells[7].firstChild.value;
		  		   if(cell_6 != cell_7){
			  		   var num = i;
