...
function page_init(){
	...
	cdt_init();
<%-- 	cruConfig.queryStr = " select  l.labor_id,l.employee_name,    decode(l.employee_gender,'1','男','0','女') employee_gender_name ,  l.employee_gender, l.employee_nation, l.employee_birth_date, l.employee_id_code_no, l.employee_education_level, l.employee_address,l.phone_num, l.employee_health_info, l.specialty, l.elite_if, l.workerfrom ,d3.coding_name posts,  d2.coding_name technical_title_name   from bgp_comm_human_labor l  left join comm_coding_sort_detail d3    on l.post = d3.coding_code_id  left join comm_coding_sort_detail d2    on l.technical_title = d2.coding_code_id    left join bgp_comm_human_labor_list lt on l.labor_id = lt.labor_id and lt.bsflag='0' left join view_human_project_relation r on  l.labor_id=r.EMPLOYEE_ID where l.bsflag='0' and lt.list_id is null   and r.ACTUAL_START_DATE is not null  and  r.ACTUAL_END_DATE is null    and r.PROJECT_INFO_NO='<%=projectInfoNo%>' and l.if_engineer='<%=laborType%>' order by lt.create_date desc ";
...
<table border="0"  cellspacing="0"  cellpadding="0"  class="form_info"  width="100%" id="queryRetTable">
	...
	<tr class="bt_info">
   	    <td class="tableHeader" 	 exp="<input type='radio' name='chx_entity_id'  value='{employee_id}-{employee_name}-{id_code}' onclick=doCheck(this) />"></td>
		...
		<td class="tableHeader" 	 exp="{id_code}">身份证号</td>
