...
<%
	...
	String bywx_begin_date=request.getParameter("bywx_begin_date");
	String bywx_end_date=request.getParameter("bywx_end_date");
%>
					...
					<tr>
						<td width="99%">
							<div class="tongyong_box">
								...
								href="#"></a></span><a href="#">单台备件消耗明细</a>
								<span class="gd"><a href="#"></a></span>
								<span class="dc" style="float:right;margin-top:-4px;">
									<a href="#" onclick="exportDataDoc('dtbjxhmx')" title="导出excel"></a>
								</span>
								</div>
...
<script type="text/javascript">
	...
	function getFusionChart(){
		var retObj = jcdpCallServiceCache("EarthquakeTeamStatistics","getSingleMatBywxDetail","self_num=<%=self_num%>&coding_code_id=<%=coding_code_id%>&bywx_begin_date=<%=bywx_begin_date%>&bywx_end_date=<%=bywx_end_date%>");
	...
	function exportDataDoc(exportFlag){
		...
		var submitStr = "coding_code_id=<%=coding_code_id%>&self_num=<%=self_num%>&bywx_begin_date=<%=bywx_begin_date%>&bywx_end_date=<%=bywx_end_date%>&exportFlag="+exportFlag;
	...
	function popBjDetailTable(cb){
		...
		//保养日期范围结束时间
		var end=str[6];
		popWindow('<%=contextPath %>/rm/dm/kkzy/popBjDetailTable.jsp?self_num='+self_num+"&coding_code_id="+coding_code_id+"&wz_id="+wz_id+"&wz_name="+wz_name+"&totalPrice="+sum+"&begin="+begin+"&end="+end,'800:600');
	}
</script>
</html>
