...
if(message != "" && message != 'null'){
	...
	function uploadFile(){		
		...
		var paramsType='<%=request.getParameter("paramsType")%>';
		
		if(check(filename)){
			document.getElementById("confirmButton").disabled="disabled";	
			document.getElementById("fileForm").action = "<%=contextPath%>/rm/em/importExcelHumanG.srq?paramsType="+paramsType+"&orgId="+orgId+"&subId="+subId;
...
<table id="buttonTable" border="0" cellpadding="0" cellspacing="0" class="form_info">
	<tr align="right">
		<td>
		<a href="<%=contextPath%>/rm/em/humanLabor/download.jsp?path=/rm/em/commHumanInfo/ImpleHumanMessageG.xls&filename=正式工国际部信息模板.xls"><font color=red>下载人员国际部信息模板</font></a>
