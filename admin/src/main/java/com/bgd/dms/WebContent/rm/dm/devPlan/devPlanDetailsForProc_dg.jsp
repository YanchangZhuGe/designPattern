			...
			<fieldset style="margin-left:2px;width:98%"><legend>申请基本信息</legend>
		      ...
		      <div id="dantaidiv" style="display:none">
			     <fieldset style="margin-left:2px;width:98%"><legend>单台管理设备</legend>
				  <table style="width:98%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable1">		
			 ...
			 <div id="colldiv" style="display:none">
				 <fieldset style="margin-left:2px;width:98%"><legend>批量管理设备</legend>
				  <table style="width:98%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTab2">		
				     <tr id='device_appdet_id_{device_allapp_detid}' name='device_allapp_detid'>
				     	<td class="bt_info_odd" >序号</td>
						<td class="bt_info_even">班组</td>
						<td class="bt_info_odd">设备名称</td>
				...
				<div id="tag-container_3" style="display:block;">
				  <ul id="tags" class="tags">
				    <li class="selectTag" id="tag3_1"><a href="#" onclick="getContentTab(this,1)">审批明细</a></li>
				  </ul>
				</div>
...
<script type="text/javascript">
	...
	function refreshData(){
		var str = "select a.*,comm.org_abbreviation from ( select zd.device_app_detid,devapp.app_org_id,devapp.device_allapp_no,alldet.device_allapp_detid,alldet.project_info_no,sd.coding_name as unitname,teamsd.coding_name as teamname, ";
		...
		str += "case when (alldet.approve_num is null or nvl(alldet.approve_num, 0) = 0) then alldet.apply_num else alldet.approve_num end as approve_num,";
		str += "zd.apply_num as apply__num,";
		...
		str += " left join gms_device_app_detail zd  on alldet.device_allapp_detid = zd.device_allapp_detid ";
		...
		if(retObj && retObj.length>0){
			for(var index=0;index<retObj.length;index++){
				...
				}else if(allappType=="S9999"){
					innerhtml += "<td>"+(index+1)+"<input name='device_allapp_detid"+index+"' id='device_allapp_detid"+index+"' value='"+device_allapp_detid+"' type='hidden'/>";
				}else if(allappType=="S14050208"){
					innerhtml += "<td>"+(index+1)+"<input name='device_allapp_detid"+index+"' id='device_allapp_detid"+index+"' value='"+device_allapp_detid+"' type='hidden'/>";
				}else if(allappType=="S9998"){
					innerhtml += "<td>"+(index+1)+"<input name='device_allapp_detid"+index+"' id='device_allapp_detid"+index+"' value='"+device_allapp_detid+"' type='hidden'/>";
				}else if(allappType=="S10001"){
					innerhtml += "<td>"+(index+1)+"<input name='device_allapp_detid"+index+"' id='device_allapp_detid"+index+"' value='"+device_allapp_detid+"' type='hidden'/>";
				}else{
					innerhtml += "<td>"+(index+1)+"<input name='device_allapp_detid"+index+"' id='device_allapp_detid"+index+"' value='"+device_app_detid+"' type='hidden'/>";
				}
				...
				innerhtml += "<td><input id='neednum"+index+"' name='neednum"+index+"' value='"+approve_num+"' size='3' type='text' applynuminfo='"+approve_num+"' onkeyup='checkAppNum(this)'/></td>";
	...
	//0,8ad878dd3abcb5ae013abe94c37f0412,5@1,8ad878dd3abcb5ae013abe94c39e0413,5
	var numbers="";
	...
	var autoOrder = document.getElementById("queryRetTable1").getElementsByTagName("tr");
    var newTR = document.getElementById("queryRetTable1").insertRow(autoOrder);
      for(var i=2;i<autoOrder.length;i++){
        var td=autoOrder[i].getElementsByTagName("td");
        var id =td[0].getElementsByTagName("input")[0].value;
        var num=td[6].getElementsByTagName("input")[0].value;
        var info="";
        if(allappType=='S9999'){
        	info=jcdpCallService("DevCommInfoSrv", "updateHireapp", "id="+id+"&num="+num);
		}else if(allappType=='S0623'){
			info=jcdpCallService("DevCommInfoSrv", "updateAllapp", "id="+id+"&num="+num);
