...
<%-- 			    <auth:ListButton functionId="" css="dc" event="onclick='exportData()'" title="JCDP_btn_export"></auth:ListButton>
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{id}:{ucm_id}' id='rdo_entity_id_{id}' onclick=doCheck(this)/>" >选择</td>
			      <td class="bt_info_even" autoOrder="1">序号</td> 
			      <td class="bt_info_odd" exp="<a style='color:blue;' href=<%=contextPath%>/doc/downloadDocByUcmId.srq?docId={id}&emflag=0>{fild_name}</a>">收工验收报告</td>
			      <td class="bt_info_even" exp="{fild_date}">验收时间</td>
			      <td class="bt_info_odd" exp="{create_name}">上传人</td>
 			    </tr>
			  </table>
			</div>
...
cruConfig.queryOp = "getFileList";
var fild_status="<%=fild_status%>"

 

// 复杂查询
function refreshData(q_fild_status){
	 debugger;
	 if(q_fild_status==undefined) q_fild_status = fild_status;
	 fild_status = q_fild_status;
	 cruConfig.submitStr = "fild_status="+q_fild_status;
	 	queryData(1);
}
...
<%-- 	
	...
	function toAdd(){
		...
		popWindow('<%=contextPath%>/pm/project/singleProject/add_file.jsp?fild_status=<%=fild_status%>');		
		//popWindow('<%=contextPath%>/doc/multiproject/close_page_eps.jsp');
	}
	...
	function toDelete(){
		debugger;
		   ids = getSelIds('rdo_entity_id');
		    ...
		    var file_id = ids.split(":")[0];
		if(confirm('确定要删除吗?')){  
			var retObj = jcdpCallService("WtDailyReportSrv", "deleteFile", "file_id="+file_id);
	...
	function toEdit(){
	    ...
	    var file_id = ids.split(":")[0];
		popWindow('<%=contextPath%>/pm/project/singleProject/add_file.jsp?file_id='+file_id);		

 		//popWindow('<%=contextPath%>/doc/multiproject/edit_file_eps.jsp?fileId='+file_id);
