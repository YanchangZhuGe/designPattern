...
<body style="background:#fff" onload="refreshData()">
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <%if(proc_status){ %>
			   <auth:ListButton functionId="OP_TARGET_DQ_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_DQ_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_DQ_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			   <%} %>
			  </tr>
			...
			<div id="table_box">
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			            ...
			            <td  class="bt_info_even">配备数量</td>
			            <td class="bt_info_odd">规格型号</td>
			            <td class="bt_info_even">预计进入时间</td>		
			            <td class="bt_info_odd">预计离开时间</td>	
			       		<td class="bt_info_even">天数</td>		
			            <td class="bt_info_odd">单价(元/天)</td>
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getDerentShareInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
    		...
    		td.innerHTML ='<input type="hidden" id="fy'+i+'sum_derent" name="fy'+i+'sum_derent" value="'+datas[i].sumDerent+'" class="input_width"/>'+
    		'<INPUT id="fy'+i+'checkbox" name="rdo_entity_id"  value='+datas[i].costDerentId+' type=checkbox>';
    		
    		var td = tr.insertCell(1);
			td.innerHTML = datas[i].rownum;
			
			var td = tr.insertCell(2);
			td.innerHTML = datas[i].derentName;
			
			var td = tr.insertCell(3);
			td.innerHTML = datas[i].derentCount;
			
			var td = tr.insertCell(4);
			td.innerHTML = datas[i].derentSpeci;
			...
			td.innerHTML = datas[i].dayNum;
			
			var td = tr.insertCell(8);
			td.innerHTML = datas[i].derentMoney;
			
			var td = tr.insertCell(9);
			td.innerHTML = datas[i].sumDerent;
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareDeviceReEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	if (ids == '') {
		...
		popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareDeviceReEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	ids = ids.replace(/\,/g,"','");
	var sql = "update bgp_op_cost_tartet_derent t set t.bsflag='1' where t.COST_DERENT_ID in('"+ids+"')";
 ...
 function toShare(){
  ...
  function importDatas(){
	popWindow(cruConfig.contextPath+"/common/ExcelImportFile.jsp?modelName=OpTargetDeviceRe&redirectUrl=/common/close_page.jsp");
}
  function getSum(){
		var project_info_no = '<%=projectInfoNo%>';
		var querySql = "select sum(nvl(to_char((t.plan_end_date-t.plan_start_date+1)*t.derent_money*t.derent_count,'9999999999999999.00'),0)) sum_value"+
			" from bgp_op_cost_tartet_derent t where t.project_info_no='<%=projectInfoNo%>'"+
