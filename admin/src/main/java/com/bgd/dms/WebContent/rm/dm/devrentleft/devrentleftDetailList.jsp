...
<script type="text/javascript">
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model){
		var str = "select account.dev_acc_id,account.asset_coding,account.self_num,account.dev_sign,";
			str += "account.license_num,account.actual_in_time,account.planning_out_time,account.actual_out_time,";
...
<script type="text/javascript">	
   ...
   function toFillLeavePage(){
    	if(sonFlag == 'Y'){
	  	...
	  	selId = ids.split(','); 
	  	var idss = "";
			var querysql = "select * from gms_device_account_dui dui where dui.is_leaving='1' ";
			...
			if(basedatas.length == 0){
				popWindow('<%=contextPath%>/rm/dm/devrentleft/devrentleftFill.jsp?devaccid='+selId,'950:680');
			}else{
				alert("存在已离场的设备！");
				return;
			}
	...
	function toModefyDatas(obj){
		...
		popWindow('<%=contextPath%>/rm/dm/devrentleft/devModefyFill.jsp?devaccid='+selId,'950:680');
	}
</script>
</html>