    ...
    <div id="new_table_box_bg" style="width:98%">
    	      <table width="95%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
			  	<tr align="right">
			    	...
			    	<auth:ListButton functionId="" css="zj" event="onclick='addCollRows()'" title="添加一条数据"></auth:ListButton>
			    	<%-- <auth:ListButton functionId="" css="xg" event="onclick='changeData()'" title="修改数据"></auth:ListButton> --%>
			    	<auth:ListButton functionId="" css="sc" event="onclick='delCollRows()'" title="删除一条数据"></auth:ListButton>
			    	<%} %>
			  ...
			  <table width="99%" id="queryRetTable" name="queryRetTable" border="1" cellspacing="0" cellpadding="0" style="word-break :keep-all;" >
			   <tr class = "bt_info_even">
					<td colspan="19" align="center">
						<font size="3"><%=projectName %>地形分类确认表</font>
					</td>
			   </tr>
		       <tr class = "bt_info_odd">
					<td colspan = "11">检波线(公里)</td>
					<td colspan = "8">炮线(公里)</td>
			   </tr>
		       <tr>
				...
				%>
					...
					<td><input type='checkbox' name='selectrow' value='<%=measureMap.get("terrain_measure_id") %>' id='<%=i+1 %>'/></td>
					<td>&nbsp;&nbsp;<%=i+1 %>&nbsp;&nbsp;
...
<script type="text/javascript"> 
	...
	function addCollRows(){
		if(flag == 1){
			...
			innerhtml += "<td><input type='checkbox' name='selectrow' value='' id='"+tr_id+"'/></td>";
	...
	function measureExportData(){
		var flag = "<%=flag%>";		
		if(flag == "notempty"){
			window.location = '<%=contextPath%>/pm/project/measureExportExcel.srq?projectInfoNo=<%=projectInfoNo%>&measureType=1';
		}else if(flag == "empty"){
			alert("没有数据,无法导出!");
			return;
		}
