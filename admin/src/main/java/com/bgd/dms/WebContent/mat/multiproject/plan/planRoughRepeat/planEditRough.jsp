			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_odd" exp="<input name=''  type='text' value='{zhichenggan_num}' />">支撑杆数量(附件)</td>
			      <td class="bt_info_even" exp="<input name=''  type='text' value='{pengbu_num}' readonly/>">篷布数量(附件)</td>
			      <td class="bt_info_odd" exp="<textarea name=''  >{appearance}</textarea>">外观</td>
			      <td class="bt_info_even" exp="<textarea name=''  >{performance}</textarea>">性能</td>
			      <td class="bt_info_odd" exp="<textarea name=''  >{note}</textarea>">备注</td>
			      <td class="bt_info_even" exp="<input name=''  type='checkbox' value='C105008042567'>固城物资管理小站</input>&nbsp;&nbsp;&nbsp;&nbsp;<input type='text' name='' id='' value='{gucheng_num}' disabled='true'/><br/><input name=''  type='checkbox' value='C105008042568'>库尔勒物资管理小站</input>&nbsp;&nbsp;<input type='text' name='' id='' value='{kuerle_num}'/><input type='hidden' name='' value='{main_part}'/>">主办区域及数量</td>
		  ...
		  ids = getSelIds('rdo_entity_id');
			if (ids == '') {
				...
				if(loadDataDetail()){
					...
					document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/plan/repeat/saveRepeatRoughPlan.srq?ifTeam=1";
...
//		var obj = event.srcElement;
		for(var i=1;i<row.length;i++){
			...
			row[i].cells[6].firstChild.name = "zhichenggan_num_"+i;
			row[i].cells[7].firstChild.name = "pengbu_num_"+i;
			row[i].cells[8].firstChild.name = "appearance_"+i;
			row[i].cells[9].firstChild.name = "performance_"+i;
			row[i].cells[10].firstChild.name = "note_"+i;
			
			row[i].cells[11].firstChild.name = "main_part_"+i;
			row[i].cells[11].childNodes[4].name = "gucheng_num_"+i;
			row[i].cells[11].childNodes[6].name = "main_part_"+i;
			row[i].cells[11].childNodes[10].name = "kuerle_num_"+i;
			row[i].cells[11].childNodes[11].name = "main_part_value_"+i;
			
			debugger;
			//修改页面主板区域字段，对应复选框
			if(row[i].cells[11].childNodes[11].value!=""&&shuaId=="1"){
				var checkedValue = 	row[i].cells[11].childNodes[11].value.split(",");
			...
			row[i].cells[11].childNodes[11].value = ids;            //选中的项目部
			...
			if(row[i].cells[0].firstChild.checked==true){
				...
				if(cell_7!=""&&!re.test(cell_7)){
					alert("第"+i+"行篷布数量请输入数字!");
					flag = false;
					return ;
				}
				
				//判断复选框是否选中，文本框时候readonly
				if(row[i].cells[11].firstChild.checked == true){
					row[i].cells[11].childNodes[4].disabled = false;
					
					if(row[i].cells[11].childNodes[4].value!=""&&!re.test(row[i].cells[11].childNodes[4].value)){
						alert("第"+i+"行固城物资管理小站物资数量请输入数字!");
						flag = false;
						return ;
					}
					
				}else{
					row[i].cells[11].childNodes[4].disabled = true;
				}
				if(row[i].cells[11].childNodes[6].checked == true){
					row[i].cells[11].childNodes[10].disabled = false;
					
					if(row[i].cells[11].childNodes[10].value!=""&&!re.test(row[i].cells[11].childNodes[10].value)){
						alert("第"+i+"行库尔勒物资管理小站物资数量请输入数字!");
						flag = false;
						return ;
					}
					
				}else{
					row[i].cells[11].childNodes[10].disabled = true;
				}
			}
		}
		return flag;
