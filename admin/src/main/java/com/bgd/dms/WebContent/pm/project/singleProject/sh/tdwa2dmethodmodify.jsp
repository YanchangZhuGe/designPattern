...
function loadData(){
	...
	//if("5000100003000000003" == buildMethod){
		//气枪
		document.getElementById("table2").style.display = "none";
		document.getElementById("table3").style.display = "none";
		document.getElementById("table4").style.display = "none";
		document.getElementById("td_section_qq").innerHTML = '<span>&nbsp;&nbsp;&nbsp;&nbsp;<font size="2">2、激发参数: 气枪激发</font></span>';
		document.getElementById("td_section_instrument").innerHTML = '<span>&nbsp;&nbsp;&nbsp;&nbsp;<font size="2">3、仪器参数</font></span>';
		// 加载得气枪数据
		if(qqData != null){
			for(var i=0;i<qqData.length;i++){
				var record = qqData[i];
				var order_num = record.order_num;
				if(order_num == "1"){
					first_qq_rowid = record.wa2d_no;
					continue;
				}else{
					toAddQQ();
					var rowIndex = qqRows - 1;
					setQQData(record, rowIndex);
				}
			}
		}
	//}
	/**else if("5000100003000000001" == buildMethod){
		//井炮
		document.getElementById("table3").style.display = "none";
...
function toSave(){
	...
	var mainRow = {};
	if(maindata_rowid != ""){
		mainRow['wa2d_no'] = maindata_rowid;
	}
	...
	for(var i=1; i<layoutRows; i++){
		...
		var single_line_fold = document.getElementById("single_line_fold" + "_"+lineNum).value;
		if(single_line_fold.length < 1){
			single_line_fold = "";
		}
		var cmp_line_fold = document.getElementById("cmp_line_fold" + "_"+lineNum).value;
		if(cmp_line_fold.length < 1){
			cmp_line_fold = "";
		}
		if(i==1){
			if(first_layout_rowid != ""){
				fromRow['wa2d_no'] = first_layout_rowid;
			}
			...
			mainRow['single_line_fold'] = single_line_fold;
			mainRow['cmp_line_fold'] = cmp_line_fold;
		}else{
			if(method_no != ""){
		...
		fromRow['single_line_fold'] = single_line_fold;
		fromRow['cmp_line_fold'] = cmp_line_fold;
	...
	for(var i=1; i<spRows; i++){
		...
		if(i==1){
			if(first_sp_rowid != ""){
				fromRow['wa2d_no'] = first_sp_rowid;
			}
	...
	for(var i=1; i<sourceRows; i++){
		...
		if(i==1){
			if(first_source_rowid != ""){
				fromRow['wa2d_no'] = first_source_rowid;
			}
	...
	for(var i=1; i<geophoneRows; i++){
		...
		if(i==1){
			if(first_geophone_rowid != ""){
				fromRow['wa2d_no'] = first_geophone_rowid;
			}
	...
	for(var i=1; i<instrumentRows; i++){
		...
		if(i==1){
			if(first_instrument_rowid != ""){
				fromRow['wa2d_no'] = first_instrument_rowid;
			}
	...
	for(var i=1; i<qqRows; i++){
		var fromRow = {};
		fromRow['project_info_no'] = project_info_no;
		var lineNum = i;
		var method_no = document.getElementById("method6_no" + "_"+lineNum).value;
		var bsflag = document.getElementById("bsflag6" + "_"+lineNum).value;
		var qq_type =	document.getElementById("qq_type" + "_"+lineNum).value;
		if(qq_type.length < 1){
			qq_type = "";
		}
		var qq_zy_num = document.getElementById("qq_zy_num" + "_"+lineNum).value;
		if(qq_zy_num.length < 1){
			qq_zy_num = "";
		}
		var qq_zy_len = document.getElementById("qq_zy_len" + "_"+lineNum).value;
		if(qq_zy_len.length < 1){
			qq_zy_len = "";
		}
		var qq_total_len = document.getElementById("qq_total_len" + "_"+lineNum).value;
		if(qq_total_len.length < 1){
			qq_total_len = "";
		}
		var qq_total_pressure = document.getElementById("qq_total_pressure" + "_"+lineNum).value;
		if(qq_total_pressure.length < 1){
			qq_total_pressure = "";
		}
		var qq_len = document.getElementById("qq_len" + "_"+lineNum).value;
		if(qq_len.length < 1){
			qq_len = "";
		}
		var qq_zz_num = document.getElementById("qq_zz_num" + "_"+lineNum).value;
		if(qq_zz_num.length < 1){
			qq_zz_num = "";
		}
		var qq_zz_len = document.getElementById("qq_zz_len" + "_"+lineNum).value;
		if(qq_zz_len.length < 1){
			qq_zz_len = "";
		}
		var qq_peak = document.getElementById("qq_peak" + "_"+lineNum).value;
		if(qq_peak.length < 1){
			qq_peak = "";
		}
		var qq_bubble = document.getElementById("qq_bubble" + "_"+lineNum).value;
		if(qq_bubble.length < 1){
			qq_bubble = "";
		}
		if(i==1){
			if(first_qq_rowid != ""){
			...
			mainRow['qq_type'] = qq_type;
			mainRow['qq_zy_num'] =qq_zy_num;
			mainRow['qq_zy_len'] = qq_zy_len;
			mainRow['qq_total_len'] = qq_total_len;
			mainRow['qq_total_pressure'] = qq_total_pressure;
			mainRow['qq_len'] = qq_len;
			mainRow['qq_zz_num'] = qq_zz_num;
			mainRow['qq_zz_len'] = qq_zz_len;
			mainRow['qq_peak'] = qq_peak;
			mainRow['qq_bubble'] = qq_bubble;
		}else{
			if(method_no != ""){
				fromRow['wa2d_no'] = method_no;
			}
		}
		fromRow['qq_type'] = qq_type;
		fromRow['qq_zy_num'] =qq_zy_num;
		fromRow['qq_zy_len'] = qq_zy_len;
		fromRow['qq_total_len'] = qq_total_len;
		fromRow['qq_total_pressure'] = qq_total_pressure;
		fromRow['qq_len'] = qq_len;
		fromRow['qq_zz_num'] = qq_zz_num;
		fromRow['qq_zz_len'] = qq_zz_len;
		fromRow['qq_peak'] = qq_peak;
		fromRow['qq_bubble'] = qq_bubble;
		
		fromRow['section_type'] = "6";
		fromRow['order_num'] = i;
		fromRow['bsflag'] = bsflag;
		fromParams[fromParams.length] = fromRow;
	}
	...
	submitMainStr = "tableName=gp_ops_2dwa_design_basic_data&"+"rowParams="+mainRows;
	submitFromStr = "tableName=gp_ops_2dwa_design_data_from&"+"rowParams="+fromRows;
...
function setLayoutData(record, lineNum){
	document.getElementById("method1_no" + "_"+lineNum).value = record.wa2d_no;
	...
	document.getElementById("single_line_fold" + "_"+lineNum).value = record.single_line_fold;
	document.getElementById("cmp_line_fold" + "_"+lineNum).value = record.cmp_line_fold;
}
...
function setQQData(record, lineNum){
	document.getElementById("method6_no" + "_"+lineNum).value = record.wa2d_no;
	document.getElementById("qq_type" + "_"+lineNum).value = record.qq_type;
	document.getElementById("qq_zy_num" + "_"+lineNum).value = record.qq_zy_num;
	document.getElementById("qq_zy_len" + "_"+lineNum).value = record.qq_zy_len;
	document.getElementById("qq_total_len" + "_"+lineNum).value = record.qq_total_len;
	document.getElementById("qq_total_pressure" + "_"+lineNum).value = record.qq_total_pressure;
	document.getElementById("qq_len" + "_"+lineNum).value = record.qq_len;
	document.getElementById("qq_zz_num" + "_"+lineNum).value = record.qq_zz_num;
	document.getElementById("qq_zz_len" + "_"+lineNum).value = record.qq_zz_len;
	document.getElementById("qq_peak" + "_"+lineNum).value = record.qq_peak;
	document.getElementById("qq_bubble" + "_"+lineNum).value = record.qq_bubble;
}
...
function setGeophoneData(record, lineNum){
	document.getElementById("method4_no" + "_"+lineNum).value = record.wa2d_no;
...
function setInstrumentData(record, lineNum){
	document.getElementById("method5_no" + "_"+lineNum).value = record.wa2d_no;
...
function toAddLayout(){
	...
	if(rowNum > 1){
		...
		var pre_single_line_fold = document.getElementById('single_line_fold_'+preRowNum).value;
		var pre_cmp_line_fold = document.getElementById('cmp_line_fold_'+preRowNum).value;
		...
		tr.insertCell().innerHTML = '<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height" id="layout_table_'+layoutIndex+'">'
					+' <tr class="odd"><td class="inquire_item6">观测系统类型：</td>'
					+' <td class="inquire_form6"><input id="method1_no' + '_' + rowNum+'" value="" type="hidden" value=""/><input id="bsflag1' + '_' + rowNum+'" type="hidden" value="0"/>'
					...
					+' <td class="inquire_form6"><input id="track_interval' + '_' + rowNum+'" type="text"  value="'+pre_track_interval+'" class="input_width" style="margin-top:4px;"/><div>m</div></td></tr>'
					+'<tr class="even"><td class="inquire_item6">炮点距：</td>'
					...
					+'<td class="inquire_item6">拖缆数量：</td>'
	    			+' <td class="inquire_form6"><input id="receiving_line_distance' + '_' + rowNum+'" type="text"  value="'+pre_receiving_line_distance+'" class="input_width" style="margin-top:4px;" onkeypress="return on_key_press_int(this)"/><div>根</div></td>'
					+' <td class="inquire_item6">炮线距：</td>'
					+' <td class="inquire_form6"><input id="sp_line_interval' + '_' + rowNum+'" type="text"  value="'+pre_sp_line_interval+'" class="input_width" style="margin-top:4px;"/><div>m</div></td></tr>'
					+' </tr>'
					+' <tr class="odd"><td class="inquire_item6">覆盖次数：</td>'
	    			+' <td class="inquire_form6"><input id="fold' + '_' + rowNum+'" type="text"  value="'+pre_fold+'" class="input_width" style="margin-top:4px;"/><div>次</div></td>'
					+' <td class="inquire_item6">拖缆间距：</td>'
					+' <td class="inquire_form6"><input id="vertical_array' + '_' + rowNum+'" type="text"  value="'+pre_vertical_array+'" class="input_width" style="margin-top:4px;"/><div>m</div></td>'
					+' <td class="inquire_item6">拖缆长度：</td>'
					+' <td class="inquire_form6"><input id="single_line_fold' + '_' + rowNum+'" type="text"  value="'+pre_single_line_fold+'" class="input_width" style="margin-top:4px;"/><div>m</div></td>'
					+' </tr>'
					+' <tr class="even"><td class="inquire_item6">拖缆深度：</td>'
	    			+' <td class="inquire_form6"><input id="cmp_line_fold' + '_' + rowNum+'" type="text"  value="'+pre_cmp_line_fold+'" class="input_width" style="margin-top:4px;" /><div>m</div></td>'
		...
		tr.insertCell().innerHTML = '<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height" id="layout_table_'+layoutIndex+'">'
					...
					+' <td class="inquire_item6">道距：</td>'
					+' <td class="inquire_form6"><input id="track_interval' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>m</div></td></tr>'
					+'<tr class="even"><td class="inquire_item6">炮点距：</td>'
					...
					+'<td class="inquire_item6">拖缆数量：</td>'
	    			+' <td class="inquire_form6"><input id="receiving_line_distance' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;" onkeypress="return on_key_press_int(this)"/><div>根</div></td>'
					+' <td class="inquire_item6">炮线距：</td>'
					+' <td class="inquire_form6"><input id="sp_line_interval' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>m</div></td></tr>'
					+' </tr>'
					+' <tr class="odd"><td class="inquire_item6">覆盖次数：</td>'
	    			+' <td class="inquire_form6"><input id="fold' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>次</div></td>'
					+' <td class="inquire_item6">拖缆间距：</td>'
					+' <td class="inquire_form6"><input id="vertical_array' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>m</div></td>'
					+' <td class="inquire_item6">拖缆长度：</td>'
					+' <td class="inquire_form6"><input id="single_line_fold' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>m</div></td>'
					+' </tr>'
					+' <tr class="even"><td class="inquire_item6">拖缆深度：</td>'
	    			+' <td class="inquire_form6"><input id="cmp_line_fold' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;" /><div>m</div></td>'
					+' <td class="inquire_item6"></td>'
					+' <td class="inquire_form6"></td>'
					+' <td class="inquire_item6"></td>'
					+' <td class="inquire_form6"></td>'
	...
	if(rowNum > 1){
		...
		tr.insertCell().innerHTML = '<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height" id="sp_table_'+spIndex+'">'
					...
					+' <td class="inquire_item6">井数：</td>'
	   				+' <td class="inquire_form6"><input id="well_num' + '_' + rowNum+'" type="text" value="'+pre_well_num+'" style="margin-top:4px;" class="input_width"/><div>口</div></td>'
					+' </tr>'
...
function toAddQQ(){
	qqIndex++;
	var tr = document.getElementById("trQQ_"+qqIndex);
	if(tr != null){
		document.getElementById("trQQ_hr_"+qqIndex).style.display = "block";
		document.getElementById("trQQ_"+qqIndex).style.display = "block";
		return;
	}
	var rowNum = qqRows;
	if(rowNum > 1){
		//读取上一行数据
		var preRowNum = parseInt(rowNum) - 1;
		var pre_method2_no = document.getElementById('method6_no_'+preRowNum).value;
		var pre_qq_type = document.getElementById('qq_type_'+preRowNum).value;
		var pre_qq_zy_num = document.getElementById('qq_zy_num_'+preRowNum).value;
		var pre_qq_zy_len = document.getElementById('qq_zy_len_'+preRowNum).value;
		var pre_qq_total_len = document.getElementById('qq_total_len_'+preRowNum).value;
		var pre_qq_total_pressure = document.getElementById('qq_total_pressure_'+preRowNum).value;
		var pre_qq_len = document.getElementById('qq_len_'+preRowNum).value;
		var pre_qq_zz_num = document.getElementById('qq_zz_num_'+preRowNum).value;
		var pre_qq_zz_len = document.getElementById('qq_zz_len_'+preRowNum).value;
		var pre_qq_peak = document.getElementById('qq_peak_'+preRowNum).value;
		var pre_qq_bubble = document.getElementById('qq_bubble_'+preRowNum).value;
		
		qqRows++;
		var tr = document.getElementById("table6").insertRow();
		tr.id = "trQQ_hr_" + rowNum;
		tr.insertCell().innerHTML = '<table width="100%"><tr><td width="2%" align="right"></td><td width="98%"><hr width="95%"></td></tr></table>';
		var tr = document.getElementById("table6").insertRow();
		tr.id = "trQQ_" + rowNum;
		tr.insertCell().innerHTML = '<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height" id="sp_table_'+qqIndex+'">'
					+' <tr class="even"><td class="inquire_item6">气枪类型：</td>'
					+' <td class="inquire_form6"><input id="method6_no' + '_' + rowNum+'" type="hidden" value=""/><input id="bsflag6' + '_' + rowNum+'" type="hidden" value="0"/>'
					+' <input id="qq_type' + '_' + rowNum+'" type="text" value="'+pre_qq_type+'"  class="input_width"/></td>'
					+' <td class="inquire_item6">震源数量：</td>'
	    			+' <td class="inquire_form6"><input id="qq_zy_num' + '_' + rowNum+'" value="'+pre_qq_zy_num+'" type="text" style="margin-top:4px;" class="input_width"/><div>个</div></td>'
					+' <td class="inquire_item6">震源距离：</td>'
	   				+' <td class="inquire_form6"><input id="qq_zy_len' + '_' + rowNum+'" value="'+pre_qq_zy_len+'" type="text" style="margin-top:4px;" class="input_width"/><div>m</div></td>'
					+' </tr>'
					+' <tr class="odd"><td class="inquire_item6">气枪总容量：</td>'
	    			+' <td class="inquire_form6"><input id="qq_total_len' + '_' + rowNum+'" value="'+pre_qq_total_len+'" type="text" class="input_width" style="margin-top:4px;"/><div>cu.in.</div></td>'
					+' <td class="inquire_item6">压力：</td>'
					+' <td class="inquire_form6"><input id="qq_total_pressure' + '_' + rowNum+'" value="'+pre_qq_total_pressure+'" type="text" class="input_width" style="margin-top:4px;"/><div>psi</div></td>'
					+' <td class="inquire_item6">枪深：</td>'
					+' <td class="inquire_form6"><input id="qq_len' + '_' + rowNum+'" type="text" value="'+pre_qq_len+'" class="input_width" style="margin-top:4px;"/><div>m</div></td>'
					+' </tr>'
					+' <tr class="even"><td class="inquire_item6">子阵数量：</td>'
	    			+' <td class="inquire_form6"><input id="qq_zz_num' + '_' + rowNum+'" type="text" value="'+pre_qq_zz_num+'" class="input_width" style="margin-top:4px;"/><div>个</div></td>'
					+' <td class="inquire_item6">子阵间距：</td>'
					+' <td class="inquire_form6"><input id="qq_zz_len' + '_' + rowNum+'" type="text" value="'+pre_qq_zz_len+'" class="input_width" style="margin-top:4px;"/><div>m</div></td>'
					+' <td class="inquire_item6">峰峰值：</td>'
					+' <td class="inquire_form6"><input id="qq_peak' + '_' + rowNum+'" type="text" value="'+pre_qq_peak+'" class="input_width" style="margin-top:4px;"/><div>barm</div></td>'
					+' </tr>'
					+' <tr class="odd"><td class="inquire_item6">气泡比：</td>'
	    			+' <td class="inquire_form6"><input id="qq_bubble' + '_' + rowNum+'" type="text" value="'+pre_qq_bubble+'" class="input_width" style="margin-top:4px;"/><div></div></td>'
					+' <td class="inquire_item6"></td>'
					+' <td class="inquire_form6"><div></div></td>'
					+' <td class="inquire_item6"></td>'
					+' <td class="inquire_form6"><div></div></td>'
					+' </tr></table>';
	}else{
		...
		tr.insertCell().innerHTML = '<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height" id="sp_table_'+qqIndex+'">'
					+' <tr class="even"><td class="inquire_item6">气枪类型：</td>'
					+' <td class="inquire_form6"><input id="method6_no' + '_' + rowNum+'" type="hidden" value=""/><input id="bsflag6' + '_' + rowNum+'" type="hidden" value="0"/>'
					+' <input id="qq_type' + '_' + rowNum+'" type="text" value=""  class="input_width"/></td>'
					+' <td class="inquire_item6">震源数量：</td>'
	    			+' <td class="inquire_form6"><input id="qq_zy_num' + '_' + rowNum+'" type="text" value="" style="margin-top:4px;" class="input_width"/><div>个</div></td>'
					+' <td class="inquire_item6">震源距离：</td>'
	   				+' <td class="inquire_form6"><input id="qq_zy_len' + '_' + rowNum+'" type="text" value="" style="margin-top:4px;" class="input_width"/><div>m</div></td>'
					+' </tr>'
					+' <tr class="odd"><td class="inquire_item6">气枪总容量：</td>'
	    			+' <td class="inquire_form6"><input id="qq_total_len' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>cu.in.</div></td>'
					+' <td class="inquire_item6">压力：</td>'
					+' <td class="inquire_form6"><input id="qq_total_pressure' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>psi</div></td>'
					+' <td class="inquire_item6">枪深：</td>'
					+' <td class="inquire_form6"><input id="qq_len' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>m</div></td>'
					+' </tr>'
					+' <tr class="even"><td class="inquire_item6">子阵数量：</td>'
	    			+' <td class="inquire_form6"><input id="qq_zz_num' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>个</div></td>'
					+' <td class="inquire_item6">子阵间距：</td>'
					+' <td class="inquire_form6"><input id="qq_zz_len' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>m</div></td>'
					+' <td class="inquire_item6">峰峰值：</td>'
					+' <td class="inquire_form6"><input id="qq_peak' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div>barm</div></td>'
					+' </tr>'
					+' <tr class="odd"><td class="inquire_item6">气泡比：</td>'
	    			+' <td class="inquire_form6"><input id="qq_bubble' + '_' + rowNum+'" type="text" value="" class="input_width" style="margin-top:4px;"/><div></div></td>'
					+' <td class="inquire_item6"></td>'
					+' <td class="inquire_form6"><div></div></td>'
					+' <td class="inquire_item6"></td>'
					+' <td class="inquire_form6"><div></div></td>'
...
function toAddSource(){
	...
	if(rowNum > 1){
		...
		tr.insertCell().innerHTML = '<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height" id="source_table_'+sourceIndex+'">'
					+' <tr class="even"><td class="inquire_item6">震源型号：</td>'
...
function toDeleteQQ(){
	...
	var line = document.getElementById("trQQ_hr_"+rowNum);
	var dataLine = document.getElementById("trQQ_"+rowNum);
	var record_id = document.getElementById("method6_no_"+rowNum).value;
	if(record_id!=""){
		line.style.display = 'none';
		dataLine.style.display = 'none';
		document.getElementsByName("bsflag6_"+rowNum)[0].value = '1';
		qqIndex--;
	}else{
		line.parentNode.removeChild(line);
		dataLine.parentNode.removeChild(dataLine);
...
<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height">
 ...
 <table  border="0" cellpadding="0" cellspacing="0" width="100%" id="table2">
	<tr><td>
		<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height">
			...
			<td width="10%" align="right">
			<%if(action != "view" && !"view".equals(action)){ %>
			<table><tr><td><auth:ListButton functionId="" css="zj" event="onclick='toAddSP()'" title="JCDP_btn_add"></auth:ListButton></td>
 ...
 <table  border="0" cellpadding="0" cellspacing="0" width="100%" id="table6">
	<tr><td>
		<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height">
  			<tr style="background-color: #97cbfd">
			<td align="left" width="90%" id="td_section_qq"></td>
			<td width="10%" align="right">
			<%if(action != "view" && !"view".equals(action)){ %>
			<table><tr><td><auth:ListButton functionId="" css="zj" event="onclick='toAddQQ()'" title="JCDP_btn_add"></auth:ListButton></td>
				<td><auth:ListButton functionId="" css="sc" event="onclick='toDeleteQQ()'" title="JCDP_btn_edit"></auth:ListButton></td></tr></table>
			<%} %>	
			</td>
  			</tr>
 		</table>
 ...
 <table  border="0" cellpadding="0" cellspacing="0" width="100%" id="table3">
	<tr><td>
		<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height">
			...
			<td width="10%" align="right">
			<%if(action != "view" && !"view".equals(action)){ %><table><tr><td><auth:ListButton functionId="" css="zj" event="onclick='toAddSource()'" title="JCDP_btn_add"></auth:ListButton></td>
 ...
 <table  border="0" cellpadding="0" cellspacing="0" width="100%" id="table4">
	<tr><td>
		<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height">
			...
			<td width="10%" align="right">
			<%if(action != "view" && !"view".equals(action)){ %><table><tr><td><auth:ListButton functionId="" css="zj" event="onclick='toAddGeophone()'" title="JCDP_btn_add"></auth:ListButton></td>
				<td><auth:ListButton functionId="" css="sc" event="onclick='toDeleteGeophone()'" title="JCDP_btn_edit"></auth:ListButton></td></tr></table>
			<%} %>
			</td>
  			</tr>
 		</table>
 ...
 <table  border="0" cellpadding="0" cellspacing="0" width="100%" id="table5">
	<tr><td>
		<table  border="0" cellpadding="0" cellspacing="0" width="100%" class="tab_line_height">
			...
			<td width="10%" align="right">
			<%if(action != "view" && !"view".equals(action)){ %><table><tr><td><auth:ListButton functionId="" css="zj" event="onclick='toAddInstrument()'" title="JCDP_btn_add"></auth:ListButton></td>
