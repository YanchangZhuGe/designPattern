      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr> 
			      <td class="bt_info_even" exp="<input type='checkbox' name='chx_entity_id' id='chx_entity_id_{change_no}' value='{change_no}' onclick=doCheck(this) />" >
			      ...
			      <td class="bt_info_odd" exp="{change_name}">变更名称</td>
			      <td class="bt_info_even" exp="{type_name}">变更类型</td>
				...
				<div id="tab_box_content0" class="tab_box_content">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  ...
					  <tr>
					    ...
					    <td class="inquire_form6" colspan="4" align="center" ><textarea  style="width:800px;" id="change_content" name="change_content"   class="textarea" ></textarea></td>
...
cruConfig.cdtType = 'form';
	...
	function refreshData(){
		...
		cruConfig.cdtType = 'form';
		cruConfig.queryStr = "select oi3.org_abbreviation org_name,tr.change_no, tr.change_name,tr.change_type,tr.change_content,tr.change_date,oi1.org_abbreviation as second_org_name,oi2.org_abbreviation as third_org_name,decode(tr.change_type, '1', '管理变更', '2', '工艺安全类变更', '3', '设备设施变更', '4', '作业环境变更') type_name from BGP_CHANGE_MANAGEMENT tr left join comm_org_subjection os1 on tr.second_org = os1.org_subjection_id and os1.bsflag = '0'  left join comm_org_information oi1 on oi1.org_id = os1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on tr.third_org = os2.org_subjection_id and os2.bsflag = '0' left join comm_org_information oi2 on oi2.org_id = os2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on os3.org_subjection_id=tr.org_sub_id and os3.bsflag='0' left join comm_org_information oi3 on oi3.org_id = os3.org_id and oi3.bsflag='0' where tr.bsflag = '0' "+querySqlAdd+" order by tr.modifi_date desc";
	...
	function simpleSearch(){
		...
		if(changeName!=''&& changeName!=null){
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = "select oi3.org_abbreviation org_name,tr.change_no, tr.change_name,tr.change_type,tr.change_content,tr.change_date,oi1.org_abbreviation as second_org_name,oi2.org_abbreviation as third_org_name,decode(tr.change_type, '1', '管理变更', '2', '工艺安全类变更', '3', '设备设施变更', '4', '作业环境变更') type_name from BGP_CHANGE_MANAGEMENT tr left join comm_org_subjection os1 on tr.second_org = os1.org_subjection_id and os1.bsflag = '0'  left join comm_org_information oi1 on oi1.org_id = os1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on tr.third_org = os2.org_subjection_id and os2.bsflag = '0' left join comm_org_information oi2 on oi2.org_id = os2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on os3.org_subjection_id=tr.org_sub_id and os3.bsflag='0' left join comm_org_information oi3 on oi3.org_id = os3.org_id and oi3.bsflag='0' where tr.bsflag = '0' "+querySqlAdd+" and tr.change_name like'%"+changeName+"%' order by tr.modifi_date desc";
			cruConfig.currentPageUrl = "/hse/changeManagement/changeManagementList.jsp";
			queryData(1);
		}else{
			refreshData();
	...
	function loadDataDetail(shuaId){
		...
		if(shuaId !=null){			
			...
			if(projectType=="5000100004000000008"){
				sub_bt="5110000004100001075";   
			}else{
				sub_bt="5110000004100000067";             //编码管理中的编码ID
			}
		
			
			processNecessaryInfo={
					businessTableName:"BGP_CHANGE_MANAGEMENT",	
					businessType:sub_bt,
				    businessId: shuaId,
					businessInfo:"变更管理审批",					
					applicantDate: '<%=curDate%>'
					};
	...
	function toAdd(){
		popWindow("<%=contextPath%>/hse/changeManagement/addChangeManagement.jsp?isProject=<%=isProject%>&projectInfoNo=<%=projectInfoNo%>");
	...
	function dbclickRow(ids){
	 	popWindow("<%=contextPath%>/hse/changeManagement/addChangeManagement.jsp?isProject=<%=isProject%>&projectInfoNo=<%=projectInfoNo%>&change_no="+ids);
	}
	function toEdit(){  
	...
	function toDelete(){ 		
		...
		deleteEntities("update BGP_CHANGE_MANAGEMENT e set e.bsflag='1' where e.change_no in ("+id+")");
	 
	}

	function toSearch(){
		popWindow("<%=contextPath%>/hse/changeManagement/hse_search.jsp?isProject=<%=isProject%>");
	}
	 function chooseOne(cb){   
	        var obj = document.getElementsByName("chx_entity_id");   
	        for (i=0; i<obj.length; i++){   
