...
<%
	...
	String cost_device_id = request.getParameter("cost_device_id");
	if(cost_device_id==null){
		cost_device_id = "";
	}
 ...
 <div id="new_table_box" align="center">
  <div id="new_table_box_content"> 
    ...
    <table  width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
     	<tr>
     		<input type="hidden" name="cost_device_id" id="cost_device_id" value="<%=cost_device_id %>" />
    	...
    	<tr>	
	    	...
	    	<td class="inquire_form4"><input name="device_count" id="device_count" type="text" class="input_width" value="" /></td>
	    </tr>
    	<tr>
	    ...
	    <tr>	
	    	<td class="inquire_item4"><font color="red">*</font>日消耗油料类型:</td>
	    	<td class="inquire_form4"><select id="daily_oil_type" name="daily_oil_type" onchange="oilTypeChange()" class="select_width"></select></td>
	    	<td class="inquire_item4"><font color="red">*</font>日消耗油料（公斤）:</td>
	    	<td class="inquire_form4"><input name="daily_oil" id="daily_oil" type="text" class="input_width" value="" /></td>
	    </tr>
	    <tr>	
	    	<td class="inquire_item4">日消耗小油品（元/天）:</td>
	    	<td class="inquire_form4"><input name="daily_small_oil" id="daily_small_oil" type="text" class="input_width" value="" /></td>
	    	<td class="inquire_item4">单井消耗油料（公斤）:</td>
	    	<td class="inquire_form4"><input name="single_well_oil" id="single_well_oil" type="text" class="input_width" value="" /></td>
	    </tr>
	    <tr>
	    	<td class="inquire_item4">油料单价(元/公斤):</td>
	    	<td class="inquire_form4"><input name="oil_unit_price" id="oil_unit_price" type="text" class="input_width" value="" /></td>
	    </tr>
    </table> 
  </div> 
  <div id="oper_div">
...
<script type="text/javascript">
	...
	" and pi.project_info_no ='<%=project_info_no%>') p on t.price_name = p.price_name"+
	...
	if(retObj!=null && retObj.returnCode=='0' && retObj.datas!=null && retObj.datas.length>0){
		for(var i = 0;i<retObj.datas.length;i++){
			var map = retObj.datas[i];
			with(map){
				document.getElementById("daily_oil_type").options.add(new Option(price_name,oil_type));
				if(i==0){
	...
	function refreshData(){
		var id  = '<%=cost_device_id%>';
		var querySql = "select * from bgp_op_tartet_device_oil where bsflag='0' and if_change = '0' and cost_device_id = '"+id+"'";
		var retObj = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		if(retObj!=null && retObj.returnCode=='0' && retObj.datas!=null && retObj.datas[0]!=null){
			var map = retObj.datas[0];
			with(map){
				document.getElementById("cost_device_id").value = cost_device_id;
				...
				document.getElementById("daily_oil").value = daily_oil;
				document.getElementById("daily_small_oil").value = daily_small_oil;
				document.getElementById("single_well_oil").value = single_well_oil;
				document.getElementById("oil_unit_price").value = oil_unit_price;
				
				document.getElementById("dev_team").value = dev_team;
				if(daily_oil_type!=null && daily_oil_type!=''){
					document.getElementById("daily_oil_type").value = daily_oil_type;
				}
				
				if(oil_unit_price==null || oil_unit_price==''){
					if(daily_oil_type=='1'){
						document.getElementById("oil_unit_price").value = price_unit1;
					}else if(daily_oil_type=='2'){
						document.getElementById("oil_unit_price").value = price_unit2;
	...
	function newSubmit() {
		...
		var cost_device_id = document.getElementById("cost_device_id").value ;
		...
		var daily_oil = document.getElementById("daily_oil").value ;
		var daily_small_oil = document.getElementById("daily_small_oil").value ;
		var single_well_oil = document.getElementById("single_well_oil").value ;
		var oil_unit_price = document.getElementById("oil_unit_price").value ; 
		var dev_team = document.getElementById("dev_team").value ;
		var daily_oil_type = document.getElementById("daily_oil_type").value;
		
		var sql = "";
		if(cost_device_id!=null && cost_device_id!=''){
			var dev_acc_id = "";
			var querySql = "select t.dev_acc_id key_id from bgp_op_tartet_device_oil t where t.bsflag ='0' and t.if_change='0' and t.cost_device_id ='"+cost_device_id+"'";
			...
			" t.daily_oil='"+daily_oil+"',t.daily_small_oil='"+daily_small_oil+"',t.single_well_oil='"+single_well_oil+"',"+
			" t.oil_unit_price='"+oil_unit_price+"',t.dev_team='"+dev_team+"',t.daily_oil_type='"+daily_oil_type+"' "+
			" where t.cost_device_id='"+cost_device_id+"';";
			...
			sql = "insert into bgp_op_tartet_device_oil(cost_device_id,project_info_no,dev_name,dev_model,plan_start_date,plan_end_date,"+
			"daily_oil_type,daily_oil,daily_small_oil,single_well_oil,oil_unit_price,org_id,org_subjection_id,creator,create_date,updator,"+
			"update_date,bsflag,if_change,dev_team,device_count,dev_acc_id) values((lower(sys_guid())),'<%=project_info_no%>','"+dev_name+"',"+
			"'"+dev_model+"',to_date('"+plan_start_date+"','yyyy-MM-dd'),to_date('"+plan_end_date+"','yyyy-MM-dd'),'"+daily_oil_type+"',"+
			"'"+daily_oil+"','"+daily_small_oil+"','"+single_well_oil+"','"+oil_unit_price+"','<%=org_id%>','<%=org_subjection_id%>',"+
			"'<%=user_id%>',sysdate,'<%=user_id%>',sysdate,'0','0','"+dev_team+"','"+device_count+"','"+dev_acc_id+"');";
			...
			"'<%=org_subjection_id%>','<%=user_id%>',sysdate,'<%=user_id%>',sysdate,'0','0','0','"+dev_acc_id+"');";
			...
			"values((lower(sys_guid())),'<%=project_info_no%>','"+dev_name+"','"+dev_model+"','"+dev_team+"','"+device_count+"',"+
			"to_date('"+plan_start_date+"','yyyy-MM-dd'),to_date('"+plan_end_date+"','yyyy-MM-dd'),'<%=org_id%>',"+
			"'<%=org_subjection_id%>','<%=user_id%>',sysdate,'<%=user_id%>',sysdate,'0','0','"+dev_acc_id+"');";
			
			sql += "insert into bgp_op_target_device_rent(target_rent_id,project_info_no,dev_name,dev_model,dev_team,device_count,"+
			"plan_start_date,plan_end_date,org_id,org_subjection_id,creator,create_date,updator,update_date,bsflag,if_change,dev_acc_id)"+
			"values((lower(sys_guid())),'<%=project_info_no%>','"+dev_name+"','"+dev_model+"','"+dev_team+"','"+device_count+"',"+
			"to_date('"+plan_start_date+"','yyyy-MM-dd'),to_date('"+plan_end_date+"','yyyy-MM-dd'),'<%=org_id%>',"+
			"'<%=org_subjection_id%>','<%=user_id%>',sysdate,'<%=user_id%>',sysdate,'0','0','"+dev_acc_id+"');";
	...
	function checkValue(){
		var obj = document.getElementById("device_count") ;
		var value = obj.value ;
		if(obj ==null || value==''){
			alert("数量不能为空!");
			return false;
		...
		obj = document.getElementById("daily_oil_type") ;
		value = obj.value ;
		if(obj ==null || value==''){
			alert("日消耗油料类型不能为空!");
			return false;
		}
		obj = document.getElementById("daily_oil") ;
		value = obj.value ;
		if(obj ==null || value==''){
			alert("日消耗油料（公斤）不能为空!");
			return false;
