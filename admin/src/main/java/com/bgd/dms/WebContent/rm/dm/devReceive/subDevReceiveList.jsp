...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <title>单项目-设备接收-设备接收(自有单台)-接收页面</title> 
 </head> 
      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
<!-- 			    <auth:ListButton functionId="" css="zr" event="onclick='plAdd()'" title="批量接收"></auth:ListButton> -->
			...
			<div id="tab_box" class="tab_box">
			  ...
			  <tr>
				...
				<td class="inquire_form6"><input id="dev_dev_coding" name="" class="input_width" type="text" /></td>
			  </tr>
				<tr>
				<td class="inquire_item6">数量</td>
				<td class="inquire_form6"><input id="dev_real_assign_num" name="" class="input_width" type="text" /></td>
...
<script type="text/javascript">
	...
	function refreshData(v_dev_name,v_dev_model){
		...
		var querydatas = queryRetNum.datas; 
		if(querydatas != null && querydatas != ''){
			for (var i = 0; i< querydatas.length; i++) {
			str += "select da.dev_coding,case dad.state when '1' then '已接收' else '未接收' end as state_desc,teamid.coding_name as team_name,"+
			...
			//"where dm.device_mixinfo_id='"+mixId+"' and dad.project_no='<%=projectInfoNo%>'";--yangyuan原sql(只显示子项目设备)
    ...
    function loadDataDetail(shuaId){
	    ...
	    */ 
		//选中这一条checkbox
		//$("#rdo_entity_id_"+retObj.devicerecMap.device_mix_detid).attr("checked","checked");
		//------------------------------------------------------------------------------------
		//选中这一条checkbox
		//$("#rdo_entity_id_"+retObj.devicerecMap.device_mix_detid).attr("checked","checked");
		//取消其他选中的
		//$("input[type='checkbox'][name='rdo_entity_id'][id!='rdo_entity_id_"+retObj.devicerecMap.device_mix_detid+"']").removeAttr("checked");
		...
		document.getElementById("dev_dev_coding").value =retObj.devicerecMap.dev_coding;
		document.getElementById("dev_real_assign_num").value =retObj.devicerecMap.assign_num;
		...
		//作业标签－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
		var querySql = "select p6.* from gms_device_account_dui dui left join gms_device_receive_process dp on dui.dev_acc_id=dp.dev_acc_id left join bgp_p6_activity p6 on dp.task_id=p6.object_id where dui.fk_device_appmix_id='"+retObj.devicerecMap.device_mix_detid+"'";
...
// 				td.innerHTML = '<input type="checkbox" name="task_id" value="'+datas[i].id+'">';
    ...
    function deleteTableTr(tableID){
	 ...
	 function toAdd(){ 
	 	ids = getSelIds('rdo_entity_id');  
	  	...
	  	//设备名称
       	...
       	var dev_model =document.getElementById("dev_dev_model").value;
       		...
       		dev_model = encodeURI(dev_model);
       	//ERP设备编号
       	var dev_coding =document.getElementById("dev_dev_coding").value;
       		dev_coding =encodeURI(dev_coding);
       		dev_coding = encodeURI(dev_coding);
       	//数量
       	var assign_num =document.getElementById("dev_real_assign_num").value;
       		assign_num =encodeURI(assign_num);
       		assign_num = encodeURI(assign_num);
       	//自编
	 	...
	 	editUrl2 = "<%=contextPath%>/rm/dm/devReceive/devRecieveSubmit.jsp?id={id}&mixId="+mixId+"&mixTypeId="+mixTypeId;
