...
<!-- 			    <auth:ListButton functionId="" css="zr" event="onclick='plAdd()'" title="批量接收"></auth:ListButton> -->
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_mix_detid_{dev_acc_id}' name='device_mix_detid'>
...
<script type="text/javascript">
	...
	function refreshData(v_dev_name,v_dev_model){
		var str ="";
			str += "select da.dev_coding,case dad.state when '1' then '已接收' else '未接收' end as state_desc,"+
			"dm.project_info_no,da.dev_name,da.dev_model,da.dev_position,dad.* "+
			"from gms_device_mixinfo_form dm "+
			"left join gms_device_appmix_detail dad on dad.device_mix_subid= dm.device_mixinfo_id "+
    ...
    function loadDataDetail(shuaId){
    	var retObj;
		if(shuaId!=null){
			 retObj = jcdpCallService("DevInsSrv", "getDevRecDetailInfo", "devrecId="+shuaId+"&mixId="+mixId);
		    ...
		    if(ids==''){ 
		     ...
		     retObj = jcdpCallService("DevInsSrv", "getDevRecDetailInfo", "devrecId="+ids+"&mixId="+mixId);
		}
		//取消选中-------------------------------------------------------------------------
    	/*
    	var obj = document.getElementsByName("rdo_entity_id");  
	    ...
	    */ 
		...
		document.getElementById("dev_position").value=retObj.devicerecMap.dev_position;
    ...
    function deleteTableTr(tableID){
	 ...
	 function toAdd(){ 
		...
		for(var i=0;i<temp.length;i++){
	 	...
	 	editUrl2 = "<%=contextPath%>/rm/dm/devReceive/devRecieveXZSubmit.jsp?id={id}&mixId="+mixId;
	 	editUrl2 = editUrl2.replace('{id}',wz_ids); 
