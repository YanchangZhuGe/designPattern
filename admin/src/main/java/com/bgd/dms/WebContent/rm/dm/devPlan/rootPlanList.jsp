...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open2.js"></script>
  ...
  <title>单项目-计划管理-设备配置计划-配置计划明细查询</title> 
 </head> 
...
$().ready(function(){
	...
	if('<%=idinfo%>'==null||'<%=idinfo%>'=='null'){
		str += "where devapp.project_info_no = '<%=projectInfoNo%>' and devapp.bsflag='0' and wfmiddle.bsflag = '0' ";
	}else{
		str += "where devapp.device_allapp_id = '<%=idinfo%>' and devapp.bsflag='0' and wfmiddle.bsflag = '0' ";
	}
	...
	if((stateObj.length>=1 && (stateObj[0].proc_status == '0'||stateObj[0].proc_status == '3'
		||stateObj[0].proc_status == '1'))||sonFlag_tmp == 'Y'){
		$(".xg").hide();
		$(".sc").hide();
	}
	...
	function refreshData(v_dev_ci_name,v_order_info){
		...
		if(idinfo==null||idinfo=='null'){
			...
			if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目			
				str += "left join gp_task_project pro on alldet.project_info_no = pro.project_info_no and pro.bsflag = '0' and pro.project_type = '5000100004000000008' ";
	      ...
	      str += "left join gp_task_project pro on alldet.project_info_no = pro.project_info_no and pro.bsflag = '0' and pro.project_father_no is null and pro.project_type = '5000100004000000008' ";
				str += "where devapp.bsflag = '0' and devapp.project_info_no='"+projectInfoNos+"' " ;
	    }else{
	      str += "left join gp_task_project pro on alldet.project_info_no = pro.project_info_no and pro.bsflag = '0' ";
				str += "where devapp.bsflag = '0' and devapp.project_info_no='"+projectInfoNos+"' ";
		  }
		}else{
		...
		str += "from gms_device_allapp_detail alldet ";
		str += "left join gms_device_allapp_add addapp on addapp.device_addapp_id = alldet.device_addapp_id ";
		str += "left join common_busi_wf_middle wf on wf.business_id = addapp.device_addapp_id and wf.bsflag='0' ";
		...
		if(idinfo==null||idinfo=='null'){
			...
			if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目			
				str += "left join gp_task_project pro on alldet.project_info_no = pro.project_info_no and pro.bsflag = '0' and pro.project_type = '5000100004000000008' ";
				str += "where addapp.bsflag = '0' and addapp.project_info_no = '" +retFatherNo+"' ";
			}else if(projectType == "5000100004000000008" && retFatherNo.length <= 0){
	      str += "left join gp_task_project pro on alldet.project_info_no = pro.project_info_no and pro.bsflag = '0' and pro.project_father_no is null and pro.project_type = '5000100004000000008' ";
				str += "where addapp.bsflag = '0' and addapp.project_info_no='"+projectInfoNos+"' " ;
	    }else{
	      str += "left join gp_task_project pro on alldet.project_info_no = pro.project_info_no and pro.bsflag = '0' ";
				str += "where addapp.bsflag = '0' and addapp.project_info_no='"+projectInfoNos+"' ";
		  }
		}else{
			str += "where addapp.device_allapp_id = '<%=idinfo%>' and alldet.bsflag='0' ";
		}
		str += "and alldet.bsflag='0' and wf.proc_status='3' ";
		str += "union all "
		str += "select alldet.device_allapp_detid,alldet.project_info_no,sd.coding_name as unitname,teamsd.coding_name as teamname, ";
		str += "'' as dev_ci_code,";
		if(projectType == '5000100004000000008'){//井中项目工序为项目名称
		...
		str += "alldet.dev_name_input as dev_ci_name,devtype.coding_name as dev_ci_model, ";
		str += "alldet.apply_num,alldet.approve_num,alldet.teamid,alldet.team, ";
		str += "alldet.purpose,alldet.employee_id,emp.employee_name, ";
		...
		str += "left join common_busi_wf_middle wf on wf.business_id=alldet.device_addapp_id and wf.bsflag='0' ";
		if(projectType == "5000100004000000008"){//井中
    		str += "and wf.business_type = '5110000004100001064' "
        }
		str += "left join bgp_p6_activity p6 on alldet.teamid = p6.object_id ";
