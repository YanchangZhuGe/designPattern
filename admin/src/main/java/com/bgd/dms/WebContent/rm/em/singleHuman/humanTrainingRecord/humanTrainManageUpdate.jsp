...
<%
  ...
  //  if(resultMsg!=null && resultMsg.getMsgElements("trainInfo") != null){
   ...
   // }    
    ...
    List<MsgElement> trainInfo = new ArrayList<MsgElement>(0);
    if(resultMsg.getMsgElements("trainInfo") != null){
    	trainInfo = resultMsg.getMsgElements("trainInfo");
    }
...
<script language="javaScript"> 
	...
	*/
	...
	['bgp_comm_human_train_manage']
	);
	var defaultTableName = 'bgp_comm_human_train_manage';
	...
	*/
	      var message = "<%=message%>";
			...
			currentCount=currentCount2+currentCount+currentCount1;
			deviceCount=deviceCount2+deviceCount+deviceCount1;
		  function page_init(){
			...
			function addLine(){
				...
				document.getElementById("trainNumber").value=currentCount+1;
				...
				var tr = document.getElementById("lineTable").insertRow();
					...
					//	tr.insertCell().innerHTML = '<input style="width:120px;"  readonly="readonly" type="text" class="input_width" name="train_date' + '_' + rowNum +'" value="'+train_date+'" onFocus="this.select()"/>'+'<img src="'+'<%=contextPath%>'+'/images/calendar.gif" id="tributton3'+rowNum+'" width="16" height="16" style="cursor: hand;"'+ 'onmouseover="calDateSelector('+"'"+endTime+"'"+',tributton3'+rowNum+');" />';;
			...
			function deleteLines(deviceNum){
				...
				var rowDeleteId = document.getElementById("train_record_no_"+deviceNum).value;
						if(	rowDeleteId!=""&&rowDeleteId!=null){
							...
							var submitStr='JCDP_TABLE_NAME=BGP_COMM_HUMAN_TRAINING_RECORD&JCDP_TABLE_ID='+rowDeleteId+'&bsflag=1';
				...
				currentCount-=1;
				deleteChangeInfoNum('train_record_no');
				document.getElementById("trainNumber").value=currentCount;
			}
					...
					for(var i=0;i<deviceCount;i++){    
						...
						if(isCheck){
							...
							//	var train_date = document.getElementsByName("train_date_"+i)[0].value;  //数据库不添加
								...
								var train_record_no=document.getElementsByName("train_record_no_"+i)[0].value;  //身份证号
								//自动添加
								...
								rowParam['train_record_no'] =train_record_no;
		...
		function uploadFile(){		
			...
			if(check(filename)){
				...
				document.getElementById("fileForm").action = "<%=contextPath%>/rm/em/importExcelTemplateHumanEdit.srq?trainDetailNo="+trainDetailNo;
...
<tr  > 
	...
	<TD class="bt_info_odd"  width="7%" ><font color=black>费用合计（元）</font>  
	...
	<TD class="bt_info_odd" width="8%"><font color=black>培训结束日期 </font></TD>
		...
		<td><input type="text" class="input_width"  style="width:40px;color:gray;" id="trainNumber" name="trainNumber"     value="<%=detailInfo.getTrainNumber()==null?"":detailInfo.getTrainNumber()%>"  />&nbsp;
		</td> 
		...
		<td><input type="text"  class="input_width" style="width:70px;color:gray;"  name="trainTotal" readonly="readonly" id="trainTotal"      value="<%=detailInfo.getTrainTotal()==null?"":detailInfo.getTrainTotal()%>" />&nbsp;</td>
		 <td>   <input type="text" id="trainStartDate" name="trainStartDate" class="input_width"   style="width:70px;"  readonly="readonly"  onchange="dateOption();"  value="<%=detailInfo.getTrainStartDate()==null?"":detailInfo.getTrainStartDate()%>"  />
		  <img src="<%=contextPath%>/images/calendar.gif" id="tributton1" width="16" height="16" style="cursor: hand;" onmouseover="calDateSelector(trainStartDate,tributton1);" /> </td>
			...
			<td>  <input type="text" id="trainEndDate" name="trainEndDate" class="input_width"  style="width:70px;"    readonly="readonly"  onchange="dateOption();"  value="<%=detailInfo.getTrainEndDate()==null?"":detailInfo.getTrainEndDate()%>"  />
...
<table border="0" cellspacing="0" cellpadding="0" class="tab_info" width="100%"  id="lineTable"  style="BACKGROUND-COLOR:#CCCCCC;">
	    <tr   >
				      ...
				      <td class="bt_info_even" width="12%" ><font color="red">考核结果</font>&nbsp;</td>  
		...
		if(trainInfo!=null && trainInfo.size()>0){
			for(int j=0;j<trainInfo.size();j++){
				...
				Map fileMap=trainInfo.get(j).toMap();
	    ...
	    %>
	      <tr  class="<%=className%>"   id="row_<%=j%>trflag"> 
	             <td><%=j + 1%><input type="hidden" id="train_record_no_<%=j%>" name="train_record_no_<%=j%>" value="<%=fileMap.get("trainRecordNo")==null?"":fileMap.get("trainRecordNo").toString()%>" /></td> 

	     <%
	    if(fileMap.get("employeeType").toString().equals("1")){	
		%>
	             <td width="12%"> <input type="text"  style="width:110px;" class="input_width" name="humanName_<%=j%>"  value="<%=fileMap.get("laborName")==null?"":fileMap.get("laborName").toString()%>" onFocus="this.select()" readonly="readonly"/></td>
	          

	     <%}else if(fileMap.get("employeeType").toString().equals("0")){  %>
	     		<td width="12%"><input type="text" class="input_width"  style="width:110px;"  name="humanName_<%=j%>" value="<%=fileMap.get("laborName")==null?"":fileMap.get("laborName").toString()%>" readonly="readonly"/></td>
		         
			     <%}else if(fileMap.get("employeeType").toString().equals("2")){  %>
		     		<td width="12%"><input type="text" class="input_width"  style="width:110px;"  name="humanName_<%=j%>" value="<%=fileMap.get("laborName")==null?"":fileMap.get("laborName").toString()%>" readonly="readonly"/></td>
			         
				     <%}else if(fileMap.get("employeeType").toString().equals("3")){  %>
			     		<td width="12%"><input type="text" class="input_width"  style="width:110px;"  name="humanName_<%=j%>" value="<%=fileMap.get("laborName")==null?"":fileMap.get("laborName").toString()%>" readonly="readonly"/></td>
				       
					     <%}else if(fileMap.get("employeeType").toString().equals("4")){  %>
				     		<td width="12%"><input type="text" class="input_width"  style="width:110px;"  name="humanName_<%=j%>" value="<%=fileMap.get("laborName")==null?"":fileMap.get("laborName").toString()%>" readonly="readonly"/></td>
					 
				     		  <%}else if(fileMap.get("employeeType").toString().equals("5")){  %>
					     		<td width="12%"><input type="text" class="input_width"  style="width:110px;"  name="humanName_<%=j%>" value="<%=fileMap.get("laborName")==null?"":fileMap.get("laborName").toString()%>" readonly="readonly"/></td>
					     		 
					     		<%}else if(fileMap.get("employeeType").toString().equals("6")){  %>
						     		<td width="12%"><input type="text" class="input_width"  style="width:110px;"  name="humanName_<%=j%>" value="<%=fileMap.get("laborName")==null?"":fileMap.get("laborName").toString()%>" readonly="readonly"/></td>
							 
		        <%}	%>	
	    <td width="24%">
		<select id="employee_type_<%=j%>" name="employee_type_<%=j%>"  style="width:150px;" onchange="selectChange('<%=j%>')" >
			<option value="0" <%if(fileMap.get("employeeType").equals("0")){%> selected="selected" <%}%> >再就业人员</option>
			<option value="1" <%if(fileMap.get("employeeType").equals("1")){%> selected="selected" <%}%> >合同化员工</option>
			<option value="2" <%if(fileMap.get("employeeType").equals("2")){%> selected="selected" <%}%> >市场化用工</option>
			<option value="3" <%if(fileMap.get("employeeType").equals("3")){%> selected="selected" <%}%> >劳务用工</option>
			<option value="4" <%if(fileMap.get("employeeType").equals("4")){%> selected="selected" <%}%> >临时工固定期限合同</option>	 
			<option value="5" <%if(fileMap.get("employeeType").equals("5")){%> selected="selected" <%}%> >完成一定工作任务</option>	 
			<option value="6" <%if(fileMap.get("employeeType").equals("6")){%> selected="selected" <%}%> >非全日制用工</option>	 
		</select>
        </td>

	    <td class="tableHeader" width="5%">
		<input type="hidden" class="input_width" name="employee_id_<%=j%>" id="employee_id_<%=j%>" value="<%=fileMap.get("employeeId")==null?"":fileMap.get("employeeId").toString()%>"/>
		...
		<input type="hidden" class="input_width" name="employee_id_code_<%=j%>"  value="<%=fileMap.get("employeeIdCode")==null?"":fileMap.get("employeeIdCode").toString()%>"/>
		...
		<div id="div6_<%=j%>"  style="display:none">非全日制用工<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor: hand;" onclick="selectLabor('<%=j%>',6)" /></div>
		   ...
		   <td >
			...
			<option value="0" <%if(fileMap.get("trainResult").equals("0")){%> selected="selected" <%}%> >合格</option>
			<option value="1" <%if(fileMap.get("trainResult").equals("1")){%> selected="selected" <%}%> >不合格</option>
			</select>
	        </td>
	    ...
	    %>
	      <tr  class="<%=className%>"   id="row_<%=k+j%>trflag"> 
	             <td><%=j + 1+trainInfo.size()%><input type="hidden" id="train_record_no_<%=j%>" name="train_record_no_<%=k+j%>" value="" /></td>

	     <%
	    if(fileMap.get("workType").toString().equals("1")){	
		%>
	             <td width="12%"> <input type="text"  style="width:110px;" class="input_width" name="humanName_<%=k+j%>"  value="<%=fileMap.get("humanName")==null?"":fileMap.get("humanName").toString()%>" onFocus="this.select()" readonly="readonly"/></td>
	        

	     <%}else if(fileMap.get("workType").toString().equals("0")){  %>
	     		<td width="12%"><input type="text" class="input_width"  style="width:110px;"  name="humanName_<%=k+j%>" value="<%=fileMap.get("humanName")==null?"":fileMap.get("humanName").toString()%>" readonly="readonly"/></td>
		        ...
		        <%}	%>	
	    <td width="24%">
		<select id="employee_type_<%=k+j%>" name="employee_type_<%=k+j%>"  style="width:150px;" onchange="selectChange('<%=k+j%>')" >
	    ...
	    <td class="tableHeader" width="5%">
		<input type="hidden" class="input_width" name="employee_id_<%=k+j%>" id="employee_id_<%=k+j%>" value="<%=fileMap.get("workId")==null?"":fileMap.get("workId").toString()%>"/>
		<div id="div1_<%=k+j%>" style="display:none">合同化员工<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor:hand;" onclick="selectPerson('<%=k+j%>',1)"/></div> 
		<input type="hidden" class="input_width" name="employee_id_code_<%=k+j%>"  value="<%=fileMap.get("codeId")==null?"":fileMap.get("codeId").toString()%>"/>
		<div id="div0_<%=k+j%>"  style="display:block">再就业人员<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor: hand;" onclick="selectLabor('<%=k+j%>',0)" /></div>
		<div id="div2_<%=k+j%>" style="display:none">市场化用工<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor:hand;" onclick="selectPerson('<%=k+j%>',2)"/></div>
		<div id="div3_<%=k+j%>"  style="display:none">劳务用工<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor: hand;" onclick="selectLabor('<%=k+j%>',3)" /></div>
		<div id="div4_<%=k+j%>"  style="display:none">临时工固定期限合同<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor: hand;" onclick="selectLabor('<%=k+j%>',4)" /></div>
		<div id="div5_<%=k+j%>"  style="display:none">完成一定工作任务<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor: hand;" onclick="selectLabor('<%=k+j%>',5)" /></div>
		<div id="div6_<%=k+j%>"  style="display:none">非全日制用工<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor: hand;" onclick="selectLabor('<%=k+j%>',6)" /></div>
		</td>
		
		   <td >
			<select id="train_result_<%=k+j%>" style="width:130px;" name="train_result_<%=k+j%>"   >
			...
			<option value="1" <%if(fileMap.get("kaohe").equals("1")){%> selected="selected" <%}%> >不合格</option>
				...
				<td width="30%"><input type="text" style="width:250px;" class="input_width" name="notes_<%=k+j%>" id="notes_<%=k+j%>" value="<%=fileMap.get("notes")==null?"":fileMap.get("notes").toString()%>" onFocus="this.select()"/></td>
				<td width="10%"><input type="hidden" name="order" value=""/><img src="<%=contextPath%>/images/delete.png" width="16" height="16" style="cursor:hand;" onclick="deleteLines('<%=k+j%>')"/></td>
				<input type="hidden" name="testSize" value="">
				<input type="hidden" name="bsflag_<%=k+j%>" value="0"/>
	      </tr>
...
<script language="javaScript"> 
if(currentCount !=0){document.getElementById("trainNumber").value=currentCount;}
