					...
					<table id="mixcoll_grid">
					 ...
					 <div id="tb2" style="height:28px;">
						<div style="float:left;height:28px;">						
							...
							&nbsp;&nbsp;业务类型：<select id="q_thing_type" class="input_width query" style="width:100px;float:none;">
						...
						<div style="float:right;height:28px;">
							<a href="####" class="easyui-linkbutton" onclick="toAddInPlanPage()"><i class="fa fa-plus-circle fa-lg"></i> 入库</a>
							<a href="####" class="easyui-linkbutton" onclick="toAddOutPlanPage()"><i class="fa fa-minus-circle fa-lg"></i> 出库</a>
...
$(function(){
	...
	$("#mixcoll_grid").datagrid({ 
		...
		url:"${pageContext.request.contextPath}/dmsManager/safekeeping/queryKeepingConfInfoList.srq?JCDP_SRV_NAME=DevKeeping&JCDP_OP_NAME=queryKeepingConfInfoList"
	});
	//初始化详细信息
...
function toAddInPlanPage(){
	popWindow('${pageContext.request.contextPath}/dmsManager/safekeeping/add_keeping.jsp?addupflag=add&thing_type=1','890:590',"-设备入库验收");
}
//添加出库
function toAddOutPlanPage(){
	popWindow('${pageContext.request.contextPath}/dmsManager/safekeeping/add_keeping.jsp?addupflag=add&thing_type=-1','890:590',"-设备出库验收");
}
//修改
function toModifyMixPlanPage(){
	var row = $('#mixcoll_grid').datagrid('getSelected');
	if (row){
		popWindow('${pageContext.request.contextPath}/dmsManager/safekeeping/add_keeping.jsp?addupflag=up&keeping_id='+row.keeping_id,'890:590',"-修改出入库验收信息");
	}else{
	        ...
	        if (data) {
				...
				if(typeof retObj.operationFlag!="undefined"){
					...
					if(''!=dOperationFlag){
						...
						if("keepin"==dOperationFlag){
							$.messager.alert("提示","未出库设备不允许删除!","warning");
							return;
						}
					}
				}
