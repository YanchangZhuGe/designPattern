...
<%
    ...
    String transUserNo = request.getParameter("transUserNo");
%>
...
<script type="text/javascript">
	...
	function loadDataDetail(){
	    ...
	    if(ids!=''){ 
			// 加载当前行信息
			document.getElementById("transUserNo").value = ids;
			var resultObj = jcdpCallService("TransferUserInfoSrv", "getTransferUser", "transUserNo="+ids);
			document.getElementById("t_trans_user_name").value = resultObj.transferUser.trans_user_name;
			document.getElementById("t_trans_user_org").value = resultObj.transferUser.org_name;
			document.getElementById("t_trans_user_prj").value = resultObj.transferUser.prj_name;
			document.getElementById("orgIds").value = resultObj.transferUser.org_ids;
			document.getElementById("projectInfoNos").value = resultObj.transferUser.prj_ids;
	    }
	}
...
　　} 
	...
	function updateProjects(index){
		...
		document.getElementById("t_trans_user_prj").value = projectNames;					
	}
	...
	function updateteam(index){
		...
		document.getElementById("t_trans_user_org").value = teamNames;	
 	...
 	function checkForm() {
		...
		form.action="<%=contextPath%>/gpe/updateTransferUser.srq";
		form.submit();
	}
</script>
</head>

<body onload="loadDataDetail()">
...
<table  border="0" cellpadding="0" cellspacing="0" class="tab_line_height">
  <tr>
    ...
    <td class="inquire_form4"  colspan="3">
   		<input type="hidden" name="transUserNo" id="transUserNo" />
       	<input id="t_trans_user_name" type="text" name="transUserName" value="" class="input_width"/> 	<input id="t_trans_user_name" type="text" name="transUserName" value="" class="input_width"/>
  ...
  <tr>  
    ...
    <td class="inquire_form4" colspan="3">
    	...
    	<input type="text" name="teamNames" id="t_trans_user_org" class="input_width" readOnly/>
    	
    </td>
  </tr>    
  <tr id="tr1">
    <td class="inquire_item4">
    	<font color="red">*</font>&nbsp;项目：   	<img src="<%=contextPath%>/images/magnifier.gif" style="cursor:hand;" border="0" onclick="chooseProject();" />
    </td>
    <td class="inquire_form4" colspan="3">
    	<input type="hidden" name="projectInfoNos" id="projectInfoNos" />
	    <input type="text" name="projectNames" id="t_trans_user_prj" class="input_width" readOnly/>
    </td>
  </tr>
  </table>
  </div>
