...
<title>多项目 -出库分配-出库分配(综合物化探)-补充设备配置计划-明细出库子页面</title>
</head>
      ...
      <table border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td class="inquire_form4" >
          	...
          	<input name="deviceappid" id="deviceappid" type="hidden" value="<%=deviceaddappid%>" />
          </td>
          <td class="inquire_item4" ></td><td class="inquire_form4" ></td>
        </tr>
        <tr>
          <td class="inquire_item4" >补充配置计划单号:</td>
          <td class="inquire_form4" >
          	<input name="device_allapp_no" id="device_addapp_no" class="input_width" type="text"  value="" readonly/>
          </td>
          <td class="inquire_item4" >补充配置计划单名称:</td>
          <td class="inquire_form4" >
          	<input name="device_allapp_name" id="device_addapp_name" class="input_width" type="text"  value="" readonly/>
          </td>
        </tr>
      </table>
      </fieldset>
...
var device_addapp_id = '<%=deviceaddappid%>';
var projectInfoNos = '<%=projectInfoNo%>';
	...
	function submitInfo(){
		...
		if(confirm("确认提交？")){
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveMixAppDetailInfoZh.srq?mixownership=S0000&pbflag=N&state=0&count="+count+"&line_infos="+line_infos+"&idinfos="+idinfos+"&device_addapp_id="+device_addapp_id+"&projectInfoNo="+projectInfoNos;
	...
	function refreshData(){
		...
		var basedatas;
		//先查询基本信息 
		var basesql = "select pro.project_name,allapp.device_addapp_id,allapp.device_addapp_no,allapp.device_addapp_name from gms_device_allapp_add allapp "
			+"left join gp_task_project pro on allapp.project_info_no=pro.project_info_no "
			+"where allapp.device_addapp_id='<%=deviceaddappid%>' ";
		...
		$("#device_addapp_no").val(baseObj[0].device_addapp_no);
		$("#device_addapp_name").val(baseObj[0].device_addapp_name);
		var device_addapp_id = baseObj[0].device_addapp_id;	
		if(device_addapp_id!=null){
			...
			var prosql = "select aad.device_allapp_detid,aad.team,aad.teamid,aad.dev_ci_code,aad.isdevicecode,aad.dev_name as dev_ci_name,aad.dev_type as dev_ci_model,sd.coding_name as unit_name,teamsd.coding_name as teamname,p6.name as jobname,aad.approve_num as require_num,aad.purpose,aad.plan_start_date,aad.plan_end_date,aad.unitinfo,nvl(tmp.applyed_num, 0) as applyed_num,aad.project_info_no ";
				...
				prosql += "where aad.device_addapp_id='"+device_addapp_id+"' and aad.project_info_no='<%=projectInfoNo%>' and aad.bsflag='0' and aad.resourceflag='0' and r.supplyflag='0' and wf.proc_status='3' ";
		...
		for(var index=0;index<retObj.length;index++){
			...
			innerhtml += "<td width='10%'><input name='devicename"+index+"' id='devicename"+index+"' style='line-height:15px' value='"+retObj[index].dev_ci_name+"' size='10' type='text' readonly/>";
