...
<script type="text/javascript">
	...
	function newSubmit() {
		popWindow("<%=contextPath %>/qua/mProject/management/management_menu.jsp",'1280:680');
...
<div id="list_table">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td background="<%=contextPath%>/images/list_15.png">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<%if(subjection_id!=null && !subjection_id.equals("C105")){ %>
	...
	<div id="table_box">
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			<tr>
				<td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{file_id}:{ucm_id}' id='rdo_entity_id' onclick=doCheck(this)/>" >
					<input type='checkbox' name='rdo_entity_id' value=':' id='rdo_entity_id' onclick='check(this)'/></td>
...
<script type="text/javascript">
	...
	// 复杂查询
	function refreshData(subjection_id){
		...
		if(subjection_id!=null && '<%=subjection_id%>'!='C105'){
			...
			" where t.bsflag='0' and t.is_file='1' and t.relation_id = 'management:"+subjection_id+"' " +
	...
	function toAdd(){
		var subjection_id = document.getElementById("org_subjection_id").value;
		var relation_id = 'management:'+subjection_id;
	...
	function toDelete(){
	    ...
	    var file_id = ids.split(",");
	    debugger;
	    var file_ids = "";
	    for(var i =0;i<file_id.length;i++){
	    	if(file_ids ==null || file_ids==''){
	    		file_ids = file_id[i].split(":")[0];
	    	}else{
	    		if(file_id[i].split(":")!=null && file_id[i].split(":")[0]!=''){
		    		file_ids = file_ids +"," + file_id[i].split(":")[0];
		    	}
	    	}
	    }
		if(confirm('确定要删除吗?')){  
			var retObj = jcdpCallService("ucmSrv", "deleteFile", "docId="+file_ids);
