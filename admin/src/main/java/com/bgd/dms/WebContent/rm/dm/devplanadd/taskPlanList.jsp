...
<%
	...
	String allappid = request.getParameter("allappid");
			...
			<div id="table_box">
			  <table style="width:98%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			    <tr id='device_appdet_id_{device_allapp_detid}' name='device_allapp_detid'>
					...
					<td class="bt_info_odd" exp="{jobname}">工序</td>
...
$().ready(function(){
	...
	var str = "select devapp.device_allapp_id,nvl(wfmiddle.proc_status,'') as proc_status ";
		...
		str += "left join common_busi_wf_middle wfmiddle on wfmiddle.business_id = devapp.device_addapp_id  ";
	if('<%=idinfo%>'==null||'<%=idinfo%>'=='null'){
		str += "where devapp.project_info_no = '<%=projectInfoNo%>' and devapp.bsflag='0' ";
	}else{
		str += "where devapp.device_addapp_id = '<%=idinfo%>' ";
	}
	var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
	var stateObj = unitRet.datas;
	if((stateObj.length>=1 && (stateObj[0].proc_status == '1'||stateObj[0].proc_status == '3'))||sonFlag_tmp == 'Y'){
		$(".zj").hide();
		$(".xg").hide();
		$(".sc").hide();
	}
...
<script type="text/javascript">
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model){
		...
		str += "alldet.plan_start_date,alldet.plan_end_date,'非采集附属设备' as managetype,case when alldet.oper_state='pass' then '已处理' else'未处理' end as oper_state,'0' as seqinfo  ";
		...
		if(idinfo==null||idinfo=='null'){
			str += "where alldet.project_info_no = '<%=projectInfoNo%>' and (alldet.bsflag = '0' or alldet.bsflag='N') ";
		}else{
			str += "where alldet.device_addapp_id = '<%=idinfo%>' and (alldet.bsflag = '0' or alldet.bsflag='N') ";
		}
		...
		str += "alldet.apply_num,alldet.teamid,alldet.team, ";
		str += "alldet.purpose,alldet.employee_id,emp.employee_name, ";
		str += "alldet.plan_start_date,alldet.plan_end_date,'采集附属设备' as managetype,case when alldet.oper_state='pass' then '已处理' else'未处理' end as oper_state,'1' as seqinfo  ";
	...
	function toAddPage(){
		popWindow('<%=contextPath%>/rm/dm/devplanadd/devdetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>&taskId=<%=taskId%>&deviceaddappid=<%=idinfo%>&deviceallappid=<%=allappid%>&taskName=<%=taskName%>','900:680');
	}
	//打开修改界面
	function toModifyPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devplanadd/devdetail_modify_apply.jsp?deviceaddappid=<%=idinfo%>&deviceallappid=<%=allappid%>&deviceallappdetid='+selectedid,'900:680');
	}
    ...
    function toBack(){
		window.parent.location.href='<%=contextPath%>/rm/dm/devplanadd/devplanaddMainInfoList.jsp';
		}
