...
<%
	...
	if(rb != null){
		...
		System.out.println("collMixFlag == "+collMixFlag);
	}
...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>单项目-调配申请-调配申请(批量)-编辑明细页面</title> 
 </head> 
...
$(document).ready(lashen);
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				str += "  union select mix.device_name,mix.device_model,mix.unit_name,'' as a,mix.device_num,mix.mix_num from gms_device_coll_mixsubadd mix left join comm_coding_sort_detail d on mix.team=d.coding_code_id where mix.device_mixinfo_id='"+currentid+"'	"
	...
	%>
	if((basedatas.length>=1 && basedatas[0].proc_status == '9'))||sonFlag_tmp == 'Y'){
		$(".zj").hide();
		$(".xg").hide();
		$(".sc").hide();
	}
...
<script type="text/javascript">	
    ...
    function toBack(){
		window.location.href='<%=contextPath%>/rm/dm/collDevmixPlan/collMixplanMainInfoList.jsp';
		}
</script>
</html>