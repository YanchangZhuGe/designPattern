...
var splan_id = '<%=request.getParameter("splan_id")==null?"":request.getParameter("splan_id")%>';	
...
function addLine( plan_detail_ids, apply_teams, posts, people_numbers,profess_numbers, plan_start_dates, plan_end_dates, numss, notess){
	...
	td.innerHTML = '<select disabled="disabled" style="width:140px;"  id="apply_team' + '_' + rowNum + '" name="apply_team' + '_' + rowNum + '" onchange="getPost('+rowNum+')" >'+getApplyTeam(apply_team)+'</select>';

	if(post == ""){
		//请选择			
		var td = tr.insertCell(2);
		td.className=classCss+"odd";
		td.innerHTML = '<select disabled="disabled" style="width:140px;"   name="post' + '_' + rowNum + '" id="post' + '_' + rowNum + '" > <option value="">请选择</option> </select>';
	}else{	
		//选择岗位值
		var td = tr.insertCell(2);
		td.className=classCss+"odd";
		td.innerHTML = '<select disabled="disabled" style="width:140px;"  name="post' + '_' + rowNum + '" id="post' + '_' + rowNum + '" >'+getPostForList(apply_team,post)+'</select>';
	}
		
	var td = tr.insertCell(3);
	td.className=classCss+"even";
	td.innerHTML = '<input type="text" disabled="disabled"  class="input_width" id="people_number' + '_' + rowNum + '" name="people_number' + '_' + rowNum + '" value="'+people_number+'" onpropertychange="if(isNaN(value)) value=value.substring(0,value.length-1);" />';
...
function page_init(){ 
	if(splan_id!='null'){
		var querySql = "select d.plan_detail_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,d.people_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1 ) nums,d.notes,d.profess_number from bgp_comm_human_plan_detail d left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0' left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' where   d.spare1 ='"+splan_id+"'     and d.resourceflag='0'  and d.bsflag='0'  order by d.modifi_date  ";
...
function toDownload(){
	...
	if("5000100004000000009"==projectType){//综合物化探
		file="importHumanPlanList_wt.xls";
	}else{
		file="importHumanPlanList.xls";
	}
	
	elemIF.src = "<%=contextPath%>/rm/em/humanRequest/download.jsp?path=/rm/em/singleHuman/humanPlan/"+file+"&filename="+iName+".xls";
...
function copyEsimiDate(){ 
	var date1 = document.getElementById("plan_start_date_0").value;
	var date2 = document.getElementById("plan_end_date_0").value;
	
	for(var i=0;i<dataLength;i++){
		document.getElementById("plan_start_date_"+i).value=date1;
		document.getElementById("plan_end_date_"+i).value=date2;
	}
}
