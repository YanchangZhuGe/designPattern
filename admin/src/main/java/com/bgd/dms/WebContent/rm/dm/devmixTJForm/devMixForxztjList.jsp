      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			    <td class='ali_btn'><span class='zj'><a href='#' onclick='toAddMixPlanPage()'  title='新增'></a></span></td>
 
			    <td class='ali_btn'><span class='xg'><a href='#' onclick='toModifyMixPlanPage()'  title='修改'></a></span></td>
 
			    <td class='ali_btn'><span class='sc'><a href='#' onclick='toDelMixPlanPage()'  title='删除'></a></span></td>
 
			    <td class='ali_btn'><span class='dc'><a href='#' onclick='exportData()'  title='导出excel'></a></span></td>
 
			    <td class='ali_btn'><span class='tj'><a href='#' onclick='toSumbitMixApp()'  title='提交'></a></span></td>
...
<script type="text/javascript">
	...
	function refreshData(v_device_app_name,v_mixinfo_no,v_opr_state_desc){
		...
		var str = " select f.device_mixinfo_id,f.mixinfo_no,f.mixinfo_name, inorg.org_abbreviation as inorgname,outorg.org_abbreviation as outorgname,f.create_date,p.project_name,(case when f.state ='0' then '未提交' when f.state='1' then  '审批通过'  when f.state ='2' then  '待审批'  when f.state ='3' then  '审批不通过'  when f.state ='4' then  '待审批' end) as state,puser.user_name from GMS_DEVICE_MIXINFO_FORM f ";
			...
			str+= " where f.mixform_type='6' and  f.bsflag='0'  and f.org_subjection_id like '<%=userSubId%>%' ";
		...
		if(v_opr_state_desc=='2')
		{
			str += "and (f.state='"+v_opr_state_desc+"' or f.state='4') ";
		}
		else
		{
   ...
   var orgSubId='<%=userSubId%>';
		...
		else
		{
			...
			var sql = "update gms_device_mixinfo_form set state='"+state+"' where device_mixinfo_id ='"+ids+"'";
			var path = cruConfig.contextPath+"/rad/asyncDelete.srq";
			var params = "deleteSql="+sql;
			params += "&ids=";
			var retObject = syncRequest('Post',path,params);
			if(state=='1')
			{
			var sql2 = "update gms_device_account t set t.using_stat='0110000007000000001' , t.ifunused='0' where t.dev_acc_id in( select d.dev_acc_id  from GMS_DEVICE_APPMIX_DETAIL d  where d.device_mix_subid='"+ids+"')";
			var path2 = cruConfig.contextPath+"/rad/asyncDelete.srq";
			var params2 = "deleteSql="+sql2;
			params2 += "&ids=";
			var retObject1 = syncRequest('Post',path2,params2);
