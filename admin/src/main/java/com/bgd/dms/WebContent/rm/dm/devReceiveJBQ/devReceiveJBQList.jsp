			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_mix_subid_{device_mix_subid}' name='device_oif_subid'>
			     	<td class="bt_info_even" exp="<input type='checkbox' name='selectedbox' value='{device_mix_subid}' id='selectedbox_{device_mix_subid}' recstate='{recstate_desc}' onclick='chooseOne(this);'/>" >选择</td>
...
<script type="text/javascript">
    ...
    function loadDataDetail(shuaId){
		...
		var str = "select t.device_mix_subid,info.dev_name as dev_name, info.dev_model as dev_type,det.coding_name,";
			str += "d.apply_num,t.assign_num,d.plan_start_date,d.plan_end_date,case when t.state is null then '未接收' when t.state='9' then '已接收' end as recstate_desc ";
			str += "from gms_device_appmix_main t left join gms_device_app_detail d on t.device_app_detid=d.device_app_detid ";
			str += "left join gms_device_collectinfo info on t.dev_ci_code=info.device_id left join comm_coding_sort_detail det on d.unitinfo=det.coding_code_id ";
			str += "where t.device_mixinfo_id='<%=mixId%>' and t.device_mix_subid='"+ids+"' ";
	...
	function toAdd(){ 
		...
		popWindow('<%=contextPath%>/rm/dm/devReceiveJBQ/devRecieveJBQSubmit.jsp?id='+ids+'&outInfoId=<%=mixId%>'); 
	 } 
	 function toBack(){
	 	window.location.href='<%=contextPath%>/rm/dm/devReceiveJBQ/devReceiveList.jsp';
	 }
</script>
</html>