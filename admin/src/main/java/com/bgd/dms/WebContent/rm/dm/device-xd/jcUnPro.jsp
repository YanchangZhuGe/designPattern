...
<script type="text/javascript"> 
	...
	function submitInfo(){
		if(confirm("提交后数据不能修改，确认提交？")){
			...
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/bgpCommDeviceInspection.srq?state=9&ids="+dev_appdet_id;
	...
	function loadDataDetail(){
		if(inspection_id=="null"){
			...
			var querySql="select a.*,b.dev_name,b.dev_model,b.self_num,b.license_num,b.owning_org_name,b.usage_org_name,b.tech_stat from BGP_COMM_DEVICE_INSPECTION a left join Gms_Device_Account_Unpro b on a.DEVICE_ACCOUNT_ID=b.dev_acc_id  where a.inspection_id='"+inspection_id+"'" ;
			var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
			basedatas = queryRet.datas;
			$("#dev_name")[0].value=basedatas[0].dev_name;
			$("#dev_model")[0].value=basedatas[0].dev_model;
