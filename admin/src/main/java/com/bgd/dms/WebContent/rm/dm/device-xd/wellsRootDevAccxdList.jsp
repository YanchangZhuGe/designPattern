...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>单项目-设备台账查询-设备台账查询(井中单台)</title> 
 </head> 
      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr >
					<td class="bt_info_even" exp="<input type='checkbox' name='rdo_entity_id' value='{dev_acc_id}' transferstate='{transfer_state}' usingstat='{using_stat_desc}' id='rdo_entity_id_{dev_acc_id}'/>" >选择</td>
					...
					<td class="bt_info_even" exp="{transfer_state_desc}">转移状态</td>
					<td class="bt_info_odd" exp="{wells_project_name}">转移项目</td>
					<td class="bt_info_even" exp="{dev_repair_desc}">设备状况</td>
			     </tr> 
			  </table>
			</div>
...
<script type="text/javascript">
	...
	function refreshData(arrObj){
		...
		var str = "select t.*,t.dev_coding as erp_id,org.org_abbreviation owning_org_name_desc,teamsd.coding_name as team_name,oprtbl.operator_name as alloprinfo,";
			str += "case t.transfer_state when '0' then '已转入' when '1' then '已转出' when '2' then '转出中' when '3' then '转入已返还' when '4' then '转出已返还' else '' end as transfer_state_desc,";
			str += "case when t.transfer_state='0' and (t.transfer_type='1' or t.transfer_type is null) then out_pro.project_name when t.transfer_state='0' and t.transfer_type='2' then '井中设备分中心' ";
			str += "when t.transfer_state='1' and (t.transfer_type='1' or t.transfer_type is null) then in_pro.project_name when t.transfer_state='1' and t.transfer_type='2' then '井中设备分中心' ";
			str += " when t.transfer_state='3' and (t.transfer_type='1' or t.transfer_type is null) then out_pro.project_name  when t.transfer_state='3' and t.transfer_type='2' then '井中设备分中心' ";
			str += "when t.transfer_state='4' and (t.transfer_type='1' or t.transfer_type is null) then in_pro.project_name when t.transfer_state='4' and t.transfer_type='2' then '井中设备分中心' ";
			str += "else '' end as wells_project_name,";
			str += "case t.repair_state when '1' then '维修' else '' end as dev_repair_desc,";
			//str += "case t.transfer_state when '0' then out_pro.project_name when '1' then in_pro.project_name when '3' then out_pro.project_name when '4' then in_pro.project_name else '' end as wells_project_name,";
			...
			str += "left join gms_device_move out_ve on t.fk_wells_transfer_id = out_ve.dev_mov_id and (t.transfer_state = '0' or t.transfer_state = '3') ";
			str += "left join gp_task_project out_pro on out_pro.project_info_no = out_ve.out_project_info_id ";
			str += "left join gms_device_move in_ve on t.fk_wells_transfer_id = in_ve.dev_mov_id and (t.transfer_state = '1' or t.transfer_state = '4') ";
			str += "left join gp_task_project in_pro on in_pro.project_info_no = in_ve.in_project_info_id ";
    ...
    function doTransfer(){
    	var s_dev_acc_id;
    	var s_transfer_state;
    	$("input[type='checkbox'][name='rdo_entity_id']").each(function(i){
    		if(this.checked){
    			s_dev_acc_id = this.value;
    			s_transfer_state = this.transferstate;
    		}
    	});

		//if(s_dev_acc_id==undefined || s_transfer_state==undefined){
		//	alert("请选择需要返还的设备!");
		//	return;
		//}
		//if(s_transfer_state==''){
		...
		//	return;
		//}
	//	if(s_transfer_state=='1'){
	//		alert("转出设备不能返还!");
	//		return;
	//	}
	//	if(s_transfer_state=='3' || s_transfer_state=='4'){
	//		alert("此设备已返还!");
	//		return;
	//	}
		popWindow('<%=contextPath%>/rm/dm/devback/wellsBackDetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>','950:680');
