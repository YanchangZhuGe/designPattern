								...
								<tr>
									...
									<input name="commitments_id" id="commitments_id" class="input_width" value="" type="hidden" readonly="readonly"/>
										...
										<input name="doc_type" id="doc_type" class="input_width" value="" type="hidden" readonly="readonly"/>
											<input name="spare1" id="spare1" class="input_width" value="add" type="hidden" readonly="readonly"/>
...
function selectTeam(){
    ...
    var result = window.showModalDialog('<%=contextPath%>/td/vspExplain/vdzCommitments/searchVdzProjectList.jsp?types=vsp','dialogWidth:500px;dialogHeight:500px;status:yes');
    if(result!=""){
    	var checkStr = result.split(",");	
		    ...
		    var querySql =" select relation_id,file_name ,file_type ,ucm_id from  (select t.*,row_number() over(partition by t.project_info_no  order by t.ucm_id desc ) r from bgp_doc_gms_file t  where t.doc_file_type = '0110000061100000024' and t.bsflag='0' and t.project_info_no='"+checkStr[0]+"' )w where w.r='1' ";
			var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
			var datas = queryRet.datas;
			if(datas!=null && datas.length>0){
