	...
	<div id="list_content">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							<td>
								...
								<input id="dev_type_id" name="dev_type_id" class="" type="hidden" /></td>
														...
														<td class="bt_info_even" ><input id="train_content" name="train_content"/></td>
														<td class="bt_info_odd" >培训讲师</td>
													 
														<td class="bt_info_even" ><input id="train_teacher" name="train_teacher"/></td>
														</tr>
														<tr class="trHeader">
														<td class="bt_info_odd" colspan="2">培训时间</td>
														<td class="bt_info_even" colspan="2"> 开始时间:<input   id="start_time" name="start_time" type="text" />
			 	 <img src="<%=contextPath%>/images/calendar.gif" id="tributton1" width="16" height="16" style="cursor: pointer;" onmouseover="calDateSelector(start_time,tributton1);" />
 			 	  结束时间:<input   id="end_time" name="end_time" type="text" />
			 	 <img src="<%=contextPath%>/images/calendar.gif" id="tributton2" width="16" height="16" style="cursor: pointer;" onmouseover="calDateSelector(end_time,tributton2);" /></td>
														</tr>
												...
												<div class="tongyong_box_content_left" id="chartContainer1"
													...
													<table width="100%" border="0" cellspacing="0"
														...
														<td class="bt_info_odd">1</td>
															...
															<td class="bt_info_even"  ><input   id="start_work_date1" name="start_work_date1" type="text" />
			 	 <img src="<%=contextPath%>/images/calendar.gif" id="employeebutton1" width="16" height="16" style="cursor: pointer;" onmouseover="calDateSelector(start_work_date1,employeebutton1);" /></td>
															...
															<td class="bt_info_odd" id="td1" >   <img  onclick="F_Open_dialog(1)" id="human_image1" src="<%=contextPath%>/humanPhoto/zhaopian.JPG"
		 ...
		 <table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
			<td  align="center">
				<span class="bc_btn"><a href="####" onclick="toSubmit()"></a></span>
...
var noimg='';
	...
	//时间判断
	if(!dateCompare($("#start_time").val(),$("#end_time").val())){
	alert('开始时间不能大于结束时间');
	return;
	}
	...
	function get_Data(){    /**获取当前时间**/	
	 		 //申请单基本数据
		 var retObj = jcdpCallService("DevCardSrv", "queryOp_cardList","currentPage=1&pageSize=10&apply_id=<%=apply_id%>");
		  if (typeof retObj.datas != "undefined") {
			var data=retObj.datas[0];
			$("#apply_unit").html(data.apply_unit);
			$("#apply_date").html(data.apply_date);
			var retObj1 = jcdpCallService("DevCardSrv", "getApplyDeviceInfoByApply_idOfEdit","currentPage=1&pageSize=10&apply_id=<%=apply_id%>");				 
			if (typeof retObj1.data != "undefined"   ) {
			 
				var names=retObj1.data.name.split(',');
				var name_ids=retObj1.data.name_id.split(',');
				for(var i=0;i<names.length;i++){
				$("#dev_type_name").append("<option value='"+name_ids[i]+"'>"+names[i]+"</option>");
				}
			 
				 
				$("#dev_model").val(retObj1.data.name);
			}
	...
	function addEmployee(){
		...
		htmls=htmls+'<td class="bt_info_even"  ><input   id="start_work_date'+id_index+'" name="start_work_date'+id_index+'" type="text" />';
		htmls=htmls+'<img src="<%=contextPath%>/images/calendar.gif" id="employeebutton'+id_index+'" width="16" height="16" style="cursor: pointer;" onmouseover="calDateSelector(start_work_date'+id_index+',employeebutton'+id_index+');" /></td>';
		...
		htmls=htmls+"<td class='bt_info_even' id='td"+id_index+"'> <img  id='human_image"+id_index+"' style='width: 85px; height: 120px' src='<%=contextPath%>/humanPhoto/zhaopian.JPG' /> <input type='file' name='excel_content_public"+id_index+"' id='excel_content_public"+id_index+"' onchange='getFileInfoPublic(this)' style='display:none'/>  </td>";
	...
	function loadEmployeeInfo(obj){
		...
		if(val.length==18){			
			...
			//$("#employee_id"+obj).val("");
				...
				var retObj = jcdpCallService("DevCardSrv", "loadEmployeeInfo","employee_id="+$("#employee_id"+obj).val());
				if (typeof retObj.data != "undefined"){
					...
					var retObj1 = jcdpCallService("DevCardSrv", "getApplyDeviceInfoByEmployee_id","employee_id="+$("#employee_id"+obj).val());
					$("#lb"+obj).empty();
					if (typeof retObj1.data != "undefined"){						
