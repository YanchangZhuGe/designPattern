...
//		top.frames('list').frames[1].refreshData();
	...
	function uploadFile(){		
		...
		if(check(filename)){
			...
			document.getElementById("fileForm").action = "<%=contextPath%>/hse/safe/importSafeShare.srq?project="+project;
	...
	function downloadModel(modelname,filename){
		...
		filename = encodeURI(filename);
		window.location.href="<%=contextPath%>/rm/em/humanLabor/download.jsp?path=/hse/leaderAndPromise/safeShare/"+modelname+".xlsx&filename="+filename+".xlsx";
	}
...
<table id="buttonTable" border="0" cellpadding="0" cellspacing="0" class="form_info">
	<tr align="right">
		...
		<td>
		<a href="javascript:selectOrg()"><font color=red>查看单位名称(模板中用)</font></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:downloadModel('ImportSafeShare','HSE安全经验分享')"><font color=red>下载导入模板</font></a>
