      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			     <auth:ListButton functionId="" css="zj" event="onclick='toAdds()'" title="JCDP_btn_add"></auth:ListButton>	  	
			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{commitments_id}-{status_type}-{project_info_no}-{tecnical_id}-{spare1}' id='rdo_entity_id_{commitments_id}' onclick='chooseOne(this);' />" >选择</td>
...
<script type="text/javascript">
	       	  ...
	       	  datas1 = queryRet1.datas;	 
	       		if(datas1 != null && datas1 != ''){	  
    				...
    				for(var i = 0; i<datas1.length; i++){	 
	       				...
	       				var submitStr = 'JCDP_TABLE_NAME=GP_WS_VSP_COMMITMENTS&JCDP_TABLE_ID=&tecnical_id='+datas1[i].tecnical_id +'&project_info_no='+datas1[i].project_info_no+'&bsflag=0&doc_type=<%=doc_type%>&create_date=<%=appDate%>&creator_id=<%=userName%>'; 
	       			   syncRequest('Post',path,encodeURI(encodeURI(submitStr)));  //保存主表信息		
	       			   
	       			var submitStr_Z = 'JCDP_TABLE_NAME=GP_WS_TECNICAL_BASIC&JCDP_TABLE_ID='+datas1[i].tecnical_id +'&test_type=yes';  
	...
	function toDelete(){ 
		...
		var path_Z = '<%=request.getContextPath()%>/rad/addOrUpdateEntity.srq';
		
		var submitStr_Z = 'JCDP_TABLE_NAME=GP_WS_TECNICAL_BASIC&JCDP_TABLE_ID='+ids.split("-")[3] +'&test_type=no';  
		syncRequest('Post',path_Z,encodeURI(encodeURI(submitStr_Z)));  //保存主表信息		
...
/* 		if(ids.split("-")[1] == '2' ){
		 ...
		 */
		var spare1 = ids.split("-")[4];		
		if (spare1 =='add'){//手动添加的数据, spare2为 jz 维护的项目
			popWindow('<%=contextPath%>/td/vspExplain/vspCommitments/add_vspCommitments.jsp?docType=0110000061200000001&fileAbbr='+fileAbbr+'&parent_file_id=&id='+ids.split("-")[0]+'&p_type=0','800:700');
		}else{ 
    	...
    	popWindow('<%=contextPath%>/td/vspExplain/vspCommitments/add_commitments.jsp?docType=0110000061200000001&fileAbbr='+fileAbbr+'&parent_file_id='+parent_file_id+'&id='+ids.split("-")[0]+'&p_type='+ids.split("-")[1],'800:700');
   		 }
	}
    function toAdds(){ 
    	popWindow('<%=contextPath%>/td/vspExplain/vspCommitments/add_vspCommitments.jsp?docType=0110000061200000001&fileAbbr='+fileAbbr+'&parent_file_id=&id=&p_type=1','800:700');
    }
