...
<%
	...
	String create_id=(user==null)?"":user.getEmpId();
...
<div id="list_table" >
	<div id="inq_tool_box"  style="display:block">
	   ...
	   <div id="table_box" >
	   <table width="100%" height="800px" border="1" cellpadding="0" cellspacing="0" class="tab_info" id="queryRetTable">
	   <tr class="bt_info">
	       <td colspan="5" >
 ...
 $("#bireport").css("height",$(window).height());
			...
			if(queryRet.returnCode=='0'){
				if(datas != null && datas != ''  ){	
					...
					if(queryRetA.returnCode=='0'){
						...
						mapS.innerHTML="<AREA title='"+datasA[0].org_name+"' onclick=getdatePage2('"+datasA[0].pmain_id+"') href='#'  shape='rect' coords='"+datasA[0].notes+"' />";	 
				...
				var datas = queryRet.datas;		
				  if(queryRet.returnCode=='0'){
						if(datas != null && datas != ''  ){	
							    ...
							    var querySqlA = " select t.pmain_id, t.ucm_id, t.creator_id,t.notes, e.employee_name,oi.org_abbreviation  as org_name,t.org_id  from BGP_HSE_PICTURE_MAIN t left join comm_human_employee e on t.creator_id = e.employee_id  and e.bsflag = '0'  left  join comm_org_subjection os   on os.org_subjection_id = t.org_id  and os.bsflag = '0'  left  join comm_org_information oi    on os.org_id = oi.org_id   and oi.bsflag = '0'   where  t.bsflag='0' and t.spare1='1' and t.spare2='"+datas[0].pmain_id+"'  ";
								var queryRetA = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=10000&querySql='+encodeURI(encodeURI(querySqlA)));
								var datasA = queryRetA.datas;	
									if(datasA != null){
										for (var i = 0; i< queryRetA.datas.length; i++) {  
										 	document.getElementById("Map").innerHTML="<AREA title='"+datasA[i].org_name+"' onclick=getdatePage1('"+datasA[i].pmain_id+"')  href='#'  shape='rect' coords='"+datasA[i].notes+"' />";
										}
									}
	...
	function toOption(){
		...
		//	 <frame src="<%=contextPath %>/hse/hseOptionPage/hsePicture/tree.jsp?father_id=<%=father_id %>&sub_id=<%=sub_id %>&org_subjection_id=<%=org_subjection_id%>&org_type=<%=org_type%>&org_sub_id=<%=org_sub_id%>&second_org=<%=second_org%>" name="mainLeftframe" frameborder="no" scrolling="auto" id="mainLeftframe"  style="border-right: 2px solid #5796DD; cursor: w-resize;"/>
			 popWindow('<%=contextPath%>/hse/hseOptionPage/hsePicture/tree.jsp?father_id=<%=father_id %>&sub_id=<%=sub_id %>&org_subjection_id=<%=org_subjection_id%>&org_type=<%=org_type%>&org_sub_id=<%=org_sub_id%>&second_org=<%=second_org%>','400:680');
			// document.getElementById("bireport").src="<%=contextPath %>/hse/hseOptionPage/hsePicture/one.jsp?father_id=<%=father_id %>&sub_id=<%=sub_id %>&org_subjection_id=<%=org_subjection_id%>&org_type=<%=org_type%>&org_sub_id=<%=org_sub_id%>&second_org=<%=second_org%>";
		 }
	...
	function getdatePage1(org_idT){ 
		popWindow('<%=contextPath%>/hse/hseOptionPage/hsePicture/pageOne.jsp?org_type=0&org_id=<%=father_id%>&org_subjection_id=<%=org_subjection_id%>&org_sub_id=<%=org_sub_id%>&pmain_id='+org_idT,'900:680');
		
	}
	function getdatePage2(org_idT){  
		popWindow('<%=contextPath%>/hse/hseOptionPage/hsePicture/pageOne.jsp?org_type=0&org_id=<%=father_id%>&org_subjection_id=<%=org_subjection_id%>&org_sub_id=<%=org_sub_id%>&pmain_id='+org_idT,'900:680');
		
	}
	function getdatePage3(org_idT){  
		popWindow('<%=contextPath%>/hse/hseOptionPage/hsePicture/pageOne.jsp?org_type=2&org_id=<%=father_id%>&org_subjection_id=<%=org_subjection_id%>&org_sub_id=<%=org_sub_id%>&pmain_id='+org_idT,'900:680');
