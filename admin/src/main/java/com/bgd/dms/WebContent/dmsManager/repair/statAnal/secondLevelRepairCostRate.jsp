...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		...
		function getFusionChart(){
			//维修费用率图表
			getSecLevRepCostRateFusionChart();
		}
		//获取维修费用率
		function getSecLevRepCostRateFusionChart(){
			var chart1 = new FusionCharts( "${applicationScope.fusionChartsURL}/Charts/Column2D.swf", "chart1", "95%", "95%", "0", "0" ); 
			chart1.setXMLUrl("<%=contextPath%>/dms/repair/getRepairCostRateChartData.srq?level=<%=level%>&devTreeId=<%=devTreeId%>&orgSubId=<%=orgSubId%>&country=<%=country%>&startDate=<%=startDate%>&endDate=<%=endDate%>");
			chart1.render("chartContainer1");
		}
		//弹出子层级图表
		function popNextRepairCostRate(level,devTreeId,orgSubId,country,startDate,endDate){
			popWindow('<%=contextPath%>/dmsManager/repair/statAnal/thirdLevelRepairCostRate.jsp?level='+level+'&devTreeId='+devTreeId+'&orgSubId='+orgSubId+'&country='+country+'&startDate='+startDate+'&endDate='+endDate,'800:572');
		}
		//弹出维修费用率信息
