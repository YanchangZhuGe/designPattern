      	...
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <td align="right" style="padding-right: 20px;"><font color="red"><span id="sum_value"></span></font></td>
				<%if(proc_status){ %>
			   <auth:ListButton functionId="OP_ACTUAL_SH_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			   <auth:ListButton functionId="OP_ACTUAL_SH_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			   <auth:ListButton functionId="OP_ACTUAL_SH_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			   <%} %>
			  </tr>
			</table>
			</td>
			...
			<div id="table_box">
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			            ...
			            <td  class="bt_info_even">待工时间</td>
			            <td  class="bt_info_odd">租赁单价</td>
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getShocksActualInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
			...
			var td = tr.insertCell();
			td.innerHTML = datas[i].managePrice2;
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualShocksEdit.jsp");
}
function toModify(){
	...
	if (ids == '') {
		...
		popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualShocksEdit.jsp?target_mataxi_id="+ids);
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	ids = ids.replace(/\,/g,"','");
	var sql = "update BGP_OP_TARGET_DEVICE_MATAXI t set t.if_actual_change='1' where t.target_mataxi_id in('"+ids+"')";
...
function getSum(){
	...
	var querySql = "select sum(nvl(to_char(t.dev_count*t.taxi_unit*(nvl(t.actual_end_date, to_date(to_char(sysdate, 'yyyy-MM-dd'),'yyyy-MM-dd')) - t.actual_start_date+1 -nvl(t.passive_date ,0))*t.taxi_ratio,'9999999999999999.00'),0)) sum_value"+
