...
//alert("深海");
	...
	function toAdd(){
		...
		if(add_flag == "0"){
			...
			location.href = "<%=contextPath%>/pm/plan/planchangelist/sh/index.jsp?projectInfoNo=<%=projectInfoNo %>";
		}
	}
	...
	function dbclickRow(ids){
		...
		if(proc_status_value == "" || proc_status_value == "4"){
			//未提交和审批不通过状态,可以修改
			if(base_plan_id == ""){
				//alert("深海 checkPlanList。jsp  base_plan_id为空 初始计划修改    1111111111");
				location.href = "<%=contextPath%>/pm/plan/planchangelist/sh/editindex.jsp?projectInfoNo="+project_info_no;
			}else{
				//alert("深海 checkPlanList。jsp  base_plan_id"+base_plan_id+" 补充计划修改    222222222222222");
				location.href = "<%=contextPath%>/pm/plan/planchangelist/sh/editindex.jsp?projectInfoNo="+project_info_no+"&basePlanId="+base_plan_id;
			}
		}else if(proc_status_value == "1" || proc_status_value == "3"){
			//待审批和审批通过状态,不可修改
			if(base_plan_id == ""){
				location.href = "<%=contextPath%>/pm/plan/planchangelist/sh/planListView.jsp?projectInfoNo="+project_info_no;
			}else{
				location.href = "<%=contextPath%>/pm/plan/planchangelist/sh/planListView.jsp?projectInfoNo="+project_info_no+"&basePlanId="+base_plan_id;
			}
		}else if(proc_status_value == "0"){
			
			location.href = "<%=contextPath%>/pm/plan/planchangelist/sh/editindex.jsp?projectInfoNo="+project_info_no+"&notSaved=1";
		}
	}
