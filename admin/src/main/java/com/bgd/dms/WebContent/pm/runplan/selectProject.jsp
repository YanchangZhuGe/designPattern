...
<%
	...
	if("5000100004000000008".equals(projectType)) {
		//井中用户
		request.getRequestDispatcher("/common/commonFile/fileDispatch.jsp?isMulti=1&menuFalg=JZXMJH").forward(request,response);
	}else{
...
<frameset rows="*" cols="300,*" frameborder="no" border="0" framespacing="0" framespacing="0">
  ...
  <frame src="<%=contextPath %>/pm/runplan/projectList.jsp?action=<%=action %>&orgSubjectionId=<%=orgSubId %>&orgId=<%=orgId %>" name="mainRightframe" frameborder="no" scrolling="auto"  id="mainRightframe" style="border-left: 2px solid #5796DD; cursor: w-resize;"/>
</frameset>
