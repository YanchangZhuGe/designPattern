...
<![endif]-->
	...
	<script type="text/javascript">
		...
		function showDevTreePage(){
			var obj = new Object();
			window.showModalDialog("<%=contextPath%>/rm/dm/collectTreeNew/collectTreeForSelectdzyq.jsp",obj,"dialogWidth:400px; dialogHeight:600px");
			if(obj.id!=undefined){
		...
		function showOrgTreePage(str){
			...
			//判断添加的采集设备再同一机构下是否重复
			var sql = "SELECT * FROM GMS_DEVICE_COLL_ACCOUNT T WHERE T.DEVICE_ID='"+$('#device_id').val()+"' AND T.USAGE_ORG_ID='"+orgId[1]+"'";
	var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+sql);
	retObj = unitRet.datas;
		if(retObj!=null)
			{
			if(retObj.length>0)
				{
				alert(names[1]+"已经存在此设备,请更换所在单位!");
				document.getElementById(str+"_name").value="";
				document.getElementById(str+"_id").value ="";
				return;
				}
			}
		}
		function submitInfo(){
			...
			if(confirm('确定要提交吗?')){
				...
				var retObj = jcdpCallService("DevProSrv","saveCollectDevAccount",params);
