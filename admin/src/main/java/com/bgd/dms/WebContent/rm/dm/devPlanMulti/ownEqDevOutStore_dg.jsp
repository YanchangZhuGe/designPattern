...
<script type="text/javascript">
    ...
    function refreshData(v_project_name,v_org_name,v_assign_state){
		...
		var str = "select devapp.allapp_type,pro.project_name,devapp.device_allapp_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.project_info_no,";
			...
			str += "and (devapp.allapp_type = 'S0000'  or devapp.allapp_type = 'S9997' or devapp.allapp_type = 'S14050208' )";
	 ...
	 function viewStore(id,state,assignType){
		if(state=='0' || state==''){
			...
			//if(assignType != 'S9997'){
				...
				if(devappid != ''){
					popWindow('<%=contextPath%>/rm/dm/devPlanMulti/outEqDevStoreView_dg.jsp?devAppId='+devappid+'&assignType='+assignType,'1050:610');
				}
			//}else{
			//	var str = "select device_app_id from gms_device_collapp where device_allapp_id = '"+id+"' and bsflag = '0' ";
			//	var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
			//	var devappid = unitRet.datas[0].device_app_id;
			//	if(devappid != ''){
			//		popWindow('<%=contextPath%>/rm/dm/collectDevMixNew/collDevMixInfoView_dg.jsp?devappid='+devappid,'950:680');				
			//	}
			//}
		}			
	}
	...
	function toModifyDetail(obj){
	  ...
	  //  if(info[1] == 'S9997'){//大港原有自有地震仪器审批界面分配三站代码
	  //  	var str = "select device_app_id from gms_device_collapp where device_allapp_id = '"+info[0]+"' and bsflag = '0' ";
		//	var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		//	var devappid = unitRet.datas[0].device_app_id;
			
		//	if(devappid != ''){
		//		popWindow('<%=contextPath%>/rm/dm/collectDevMixNew/collDevMixInfoNew_dg.jsp?devappid='+devappid,'950:680');
		//	}else{
		//		alert("设备未分配！");
		//		return;
		//	}
		//}else{
		//	popWindow('<%=contextPath%>/rm/dm/devPlanMulti/ownEqDevOutStoreDet_dg.jsp?projectInfoNo=<%=projectInfoNo%>&deviceallappid='+info[0]+'&allapp_type='+info[1],'1050:680');
		//}
