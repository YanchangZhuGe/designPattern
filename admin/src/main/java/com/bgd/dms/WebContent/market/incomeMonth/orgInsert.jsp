...
<%
	...
	String weekDate="";
	if(orgId!=null){
		String sql2 ="select max(t.week_date) week_date from bgp_wr_income_money t where t.bsflag = '0' and t.org_type = '0' and t.type = '2' and t.org_id = '"+orgId+"'";
...
<!--  <td><input name="new_sign0" value="" size="10"/></td>  -->
...
<script type="text/javascript">
	...
	function afterSubmit(retObject,successHint,failHint){
		...
		if (retObject.returnCode != "0") alert(failHint);
			...
			window.location = "<%=contextPath%>/market/incomeMonth/incomeReport.srq?orgId=<%=orgId%>";
		}
	}
	function cancel()
	{
	...
	function save(){
		...
		var querySql = "select t.* from bgp_wr_income_money t where t.week_date = to_date('" + week_date + "','yyyy-MM-dd') and t.bsflag='0' and t.org_type='0' and t.type='2' and t.org_id='<%=orgId%>' order by org_id,country";
		...
		if(datas != null && queryRet.datas.length){
			...
			if(flag){
				...
				var title = "在月市场落实价值工作量中修改了一条月报开始日期为：“"+week_date+"”的信息";
			...
			var title = "在月市场落实价值工作量中添加了一条月报开始日期为：“"+week_date+"”的信息";
			var operationPlace = "月市场落实价值工作量";
			var rowParams = new Array();
			var rowParam = {};
			rowParam['operation_title'] = encodeURI(encodeURI(title));
	...
	if(action=="add"){
		...
		if(week_date_1!='null'){
			var querySql = "select t.* from bgp_wr_income_money t where to_char(t.week_date,'yyyy-MM-dd') = '" + week_date_1 + "' and bsflag='0' and  t.org_type='0' and t.type='2' and t.org_id='<%=orgId%>'  order by org_id,country";
	...
	if(data_week_date!='null'){
		...
		var querySql = "select t.* from bgp_wr_income_money t where to_char(t.week_date,'yyyy-MM-dd') = '" + data_week_date + "' and bsflag='0' and  t.org_type='0' and t.type='2' and t.org_id='<%=orgId%>'  order by org_id,country";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=10000&querySql='+querySql);
		var datas = queryRet.datas;
