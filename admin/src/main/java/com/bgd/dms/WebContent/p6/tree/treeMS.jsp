...
var objda = window.dialogArguments;//移动数据 p6树接收所选参数
Ext.require([
             'Ext.data.*',
             'Ext.grid.*',
             'Ext.tree.*',
...
Ext.onReady(function() {
    ...
    function celldblclick(grid,colspan, colIndex, rowdata, rowspan,rowIndex,e){
    	...
    	//alert(taskObjectId+ "   "+taskName);
    	//alert(" plannedStartDate  "+plannedStartDate+ " plannedEndDate  "+plannedEndDate);
    	//document.getElementById("form").action="<%=contextPath%>/mat/singleproject/plan/planItemListTo.srq?taskObjectId="+taskObjectId;
	  	//document.getElementById("form").submit();

	  	//计划开始时间 计划结束时间 
	  	objda.plannedStartDate = plannedStartDate;
	  	objda.plannedEndDate = plannedEndDate;
	  	
	  	//节点id
    	objda.taskObjectId = taskObjectId;
    ...
    function newClose(){
    	...
    	*/
    	...
    	if(top.topDialogs!=null&&top.topDialogs!=undefined){
    		var dialog = top.topDialogs.pop();
    		dialog[0].remove();
    		dialog[1].remove();
    	}
    }
...
<table border="0" cellpadding="0" cellspacing="0"  id="FilterLayer" width="100%">
	<tr>
		...
		<form name="form" id="form" method="post" action="">
		  	<input type="hidden" id="laborId" name="laborId" value="<%=laborId%>"></input>
		  </form>
