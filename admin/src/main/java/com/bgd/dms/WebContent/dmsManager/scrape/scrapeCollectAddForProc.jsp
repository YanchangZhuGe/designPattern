...
<script type="text/javascript" src="<%=contextPath%>/js/easyui/common_util.js"></script>
...
<div id="new_table_box">
  <div id="new_table_box_content">
    <div id="new_table_box_bg">
      ...
      <!-- add by zzb -->
      <fieldSet style="margin-left:2px"  class="noprint"><legend>报废汇总详细信息</legend>
	  	  ...
	  	  <div>
	  	  	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  	  		<tr>
	  	  			<td>&nbsp;</td>
					<auth:ListButton functionId="" css="xz" event="onclick='exportDevData()'" title="导出excel"></auth:ListButton>
	  	  		</tr>
	  	  	</table>
	  	  </div>
		  <div style="overflow:auto">
			  ...
			  <table style="width:97.9%"  id="processtable1"  border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       	<tr>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td colspan="2">
           ...
           <td colspan="1" >专家意见<font color="red">(为空时批复时不可见)</>:</td>
          <td colspan="2">
    ...
    <div id="oper_div" class="noprint">
      	<span class="dy_btn"><a href="#" onclick="myprint()"></a></span>
...
//add by zzb
	...
	function refreshData(){
		...
		if('<%=scrape_collect_id%>'!='null'){
			 baseData = jcdpCallService("ScrapeSrv", "getScrapeCollectAllInfoNew", "scrape_collect_id="+$("#scrape_collect_id").val());
			...
			if(baseData.datas!=null){
				...
				$("#devCount").val(devCount);
		 		for ( var tr_id = 0; tr_id < baseData.datas.length; tr_id++) {
					...
					var file_name = baseData.datas[tr_id].file_name;
					var scrape_type_name="";
					if(baseData.datas[tr_id].coding_code_id=='0'){
					scrape_type_name='正常报废';
					}else if(baseData.datas[tr_id].coding_code_id=='1'){
					scrape_type_name='技术淘汰';
					}else if(baseData.datas[tr_id].coding_code_id=='2'){
					scrape_type_name='毁损';
					}else{
					scrape_type_name='盘亏';
					}
					...
					innerhtml += "<td >"+baseData.datas[tr_id].duty_unit+"</td>";
					innerhtml += "<td >"+scrape_type_name+"</td>";
					innerhtml += "<input name='coding_code_id"+tr_id+"' id='coding_code_id"+tr_id+"'  type='hidden' value='"+baseData.datas[tr_id].coding_code_id+"'/>";
					innerhtml += "<input name='scrape_type_name"+tr_id+"' id='scrape_type_name"+tr_id+"'  type='hidden' value='"+scrape_type_name+"'/>";
					innerhtml += "<input name='scrape_apply_id"+tr_id+"' id='scrape_apply_id"+tr_id+"'  type='hidden' value='"+baseData.datas[tr_id].scrape_apply_id+"'/>";
					innerhtml += "<input name='file_type"+tr_id+"' id='file_type"+tr_id+"'  type='hidden' value='"+baseData.datas[tr_id].file_type+"'/>";
					innerhtml += "<td ><a href='#' onclick='subgo("+tr_id+")'>"+baseData.datas[tr_id].code_name+"</a></td>";
					innerhtml += "<td >"+baseData.datas[tr_id].asset_value+"</td>";
					innerhtml += "<td >"+baseData.datas[tr_id].net_value+"</td>";
					innerhtml += "<td>";
					if(file_name!=null&&file_name!=""){
						var file_names = file_name.split(",");
					}
					for(var i=0;i<file_names.length;i++){
						var id = file_names[i].split("&")[0];
						var name = file_names[i].split("&")[1];
						if(name!="")
						innerhtml+= "<a href='<%=contextPath%>/doc/downloadDoc.srq?docId="+id+"'>"+name+"<br/></a>";
					}
					innerhtml += "</td></tr>";
					$("#processtable0").append(innerhtml);
			}
			...
			$("#processtable0>tr:even>td:odd").addClass("even_odd");
			$("#processtable0>tr:even>td:even").addClass("even_even");
			aligntitle("#processtable1","#processtable0");
			}
...
//删除文件
	...
	function addLine() {
		...
		var s_times = document.getElementById("s_times").value;
 	    var message ="&wz_name="+s_wz_name+"&dev_model="+s_dev_model+"&times="+s_times;
		...
		baseData = jcdpCallService("ScrapeSrv", "getScrapeApplyCollect", "ids="+scrape_apply_id+message);
		if (baseData.datas != null) {
			...
			for ( var i = 0; i < baseData.datas.length; i++) {
				...
				innerhtml += "<td width='15%'>"+baseData.datas[i].scrape_types+"</td>";
	...
	function subgo(i){
		...
		times = encodeURI(encodeURI(times));
	 	<%-- var returnFlag = window.showModalDialog("<%=contextPath%>/dmsManager/scrape/selectScrapeListCollect.jsp?org_name="+org_name+"&org_id="+org_id+"&dev_name="+dev_name+"&dev_model="+dev_model+"&dev_type="+dev_type+"&times="+times+"&scrape_type="+scrape_type,obj,"dialogWidth=1240px;dialogHeight=480px");
		if(returnFlag=="1"){
			window.location.reload();
		} --%>
		popWindow("/DMS/dmsManager/scrape/selectScrapeListCollect.jsp?org_name="+org_name+"&org_id="+org_id+"&dev_name="+dev_name+"&dev_model="+dev_model+"&dev_type="+dev_type+"&times="+times+"&scrape_type="+scrape_type,'','报废评审查看');
	};
	function fujian(i){
		var scrape_collect_id ="'"+$("#scrape_collect_id").val()+"'";
		var scrape_apply_id = $("#scrape_apply_id").val();
		...
		dev_name = encodeURI(encodeURI(dev_name));
		times = encodeURI(encodeURI(times));
	 	window.showModalDialog("<%=contextPath%>/dmsManager/scrape/selectScrapeFileListCollect.jsp?org_name="+org_name+"&dev_name="+dev_name+"&dev_type="+dev_type+"&times="+times,"","dialogWidth=1240px;dialogHeight=480px");
	}
	/*打印评审单*/
	function myprint()
	{
	...
	function exportDevData(){
    	var ids = '<%=scrape_collect_id%>';
    	var exportFlag = 'bfpscx';
    	var path = cruConfig.contextPath+"/rm/dm/common/DmZhfxToExcel.srq";
    	var submitStr="scrape_collect_id='"+ids+"'&exportFlag="+exportFlag;
