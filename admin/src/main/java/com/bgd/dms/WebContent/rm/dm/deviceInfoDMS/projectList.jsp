				...
				<div id="tab_box_content0" class="tab_box_content">
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
					  <tr>
					    ...
					    <td class="inquire_form6" id="item0_0"><input type="hidden" name="task_log_id" id="task_log_id" /><input type="hidden" id="project_info_no" name="project_info_no" /><input type="text" id="project_name" name="project_name" class="input_width" /></td>
				...
				<div id="tab_box_content1" class="tab_box_content" style="display:none;">
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
					  <tr><input type="hidden" id="project_dynamic_no" name="project_dynamic_no" />
					    ...
					    <td class="inquire_item6" id="design_object_workload1_td"><input type="radio" id="workload1" name="workload" value="1" checked="checked"/><span class="red_star">*</span>设计实物工作量：</td>
					    <td class="inquire_form6" id="item1_1"><input type="text" id="design_workload1" name="design_workload1" class="input_width" />&nbsp;km</td>
					    <td class="inquire_item6" id="design_object_workload2_td"><input type="radio" id="workload2" name="workload" value="2"/><span class="red_star">*</span>设计满覆盖工作量：</td>
...
<script type="text/javascript">
	...
	function toUpdateProject(){
		...
		//判断项目状态是不是可以修改为施工结束 和 项目结束
		var project_sta = document.getElementById("project_status").value;
		var pro_info_no = document.getElementById("project_info_no").value;
		if(project_sta=="5000100001000000003"||project_sta=='5000100001000000005'){
			var obj = jcdpCallService("ProjectSrvOld", "validateProjectSta", "project_info_no="+pro_info_no);
			if(obj.message=='n'){
				alert("日报没有结束，不能修改项目状态");
				return;
			}
		}
		...
		str += "&vsp_team_leader="+encodeURI(encodeURI(document.getElementById("vsp_team_leader").value));
		
		var obj = jcdpCallService("ProjectSrvOld", "addProject", str);
	...
	function toUpdateWorkarea(){
		...
		if(obj != null && obj.message == "success") {
			var workarea_no = obj.workarea_no;
			var str_se = "project_info_no="+document.getElementById('project_info_no').value+"&project_dynamic_no="+document.getElementById('project_dynamic_no').value+"&workarea_no="+workarea_no;
			var obj_ = jcdpCallService("ProjectSrvOld", "saveLog", str_se);
			if(obj_ != null && obj_.message == "success"){
				alert("修改成功");
			}
		} else {
			alert("修改失败");
	...
	function checkText0() {
        ...
        if (!isTextPropertyNotNull("vsp_team_leader", "队经理")) return false;
		return true;
	}
	...
	function toUpdateQuality(){
		...
		var obj = jcdpCallService("ProjectSrvOld", "saveOrUpdateQuality", str);
	...
	function checkQuality() {
		var miss_radio_v = document.getElementById("miss_radio").value;
		if(miss_radio_v.length==0){
			alert("请填写必输项");
			return false;
		}
		
		var qualified_radio_v = document.getElementById("qualified_radio").value;
		if(qualified_radio_v.length==0){
			alert("请填写必输项");
			return false;
		}
		
		var waster_radio_v = document.getElementById("waster_radio").value;
		if(waster_radio_v.length==0){
			alert("请填写必输项");
			return false;
		}
	...
	function toDelete(){
		...
		if(confirm('确定要删除吗?')){
			var retObj = jcdpCallService("ProjectSrvOld", "deleteProject", "projectInfoNos="+ids);
			if(retObj != null && retObj.message == "success"){
				var str_ = "project_info_no="+document.getElementById('project_info_no').value+"&project_dynamic_no="+document.getElementById('project_dynamic_no').value+"&del=true";
				var obj_ = jcdpCallService("ProjectSrvOld", "saveLog", str_);
				if(obj_ != null && obj_.message == "success"){
					alert("操作成功");
				}
			}
	...
	function dbclickRow(ids){
		...
		window.location.href="<%=contextPath %>/rm/dm/deviceInfoDMS/deviceUseInfo.jsp";
