			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_backdet_id{device_backdet_id}' name='device_backdet_id'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' value='{device_backdet_id}' id='selectedbox_{device_backdet_id}' leavinginfo='{isleaving}' onclick='chooseOne(this)'/>" >选择</td>
...
<script type="text/javascript">
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model){
		...
		var str  = "select t.device_backdet_id,t.back_num as mix_num,account.dev_name,account.dev_model, case t.is_leaving when '0' then '未验收' when '1' then '已验收' end as isleaving,";
			str += " account.dev_unit,account.total_num,account.unuse_num,account.use_num,account.actual_in_time,unitsd.coding_name as dev_unit_name,dym.alter_date as alter_date";
			str += " from gms_device_collbackapp_detail t  left join gms_device_coll_account_dui account on t.dev_acc_id = account.dev_acc_id";
			str += " left join gms_device_coll_dym dym on dym.device_appmix_id = t.device_backapp_id ";
			str += " left join comm_coding_sort_detail unitsd on account.dev_unit = unitsd.coding_code_id  where t.device_backapp_id ='<%=devicebackappid%>'";
...
<script type="text/javascript">	
    ...
    function loadDataDetail(device_backdet_id){
		...
		var str = "select t.device_backdet_id,t.back_num as mix_num,backdet.device_backapp_id,backdet.dev_acc_id,backdet.dev_name,backdet.dev_model,backdet.back_num,backdet.planning_out_time,backdet.devremark,backdet.device_id, case t.is_leaving when '0' then '未验收' when '1' then '已验收' end as isleaving, account.dev_unit,account.total_num,account.unuse_num,account.use_num,account.actual_in_time,unitsd.coding_name as dev_unit_name from gms_device_collbackapp_detail t left join gms_device_collbackapp_detail backdet on t.device_backdet_id = backdet.device_backdet_id left join gms_device_coll_account_dui account on backdet.dev_acc_id = account.dev_acc_id left join comm_coding_sort_detail unitsd on account.dev_unit = unitsd.coding_code_id ";
		str += "where t.device_backdet_id ='"+devicebackdetid+"'";
		//alert(str);
		var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		retObj = unitRet.datas;
		
		//取消其他选中的 
		$("input[type='checkbox'][name='selectedbox'][id!='selectedbox_"+retObj[0].device_backdet_id+"']").removeAttr("checked");
		//选中这一条checkbox
		$("input[type='checkbox'][name='selectedbox'][id='selectedbox_"+retObj[0].device_backdet_id+"']").attr("checked",'checked');
	...
	function toSumbitDevApp(){
	  ...
	  */
	  editUrl = "<%=contextPath%>/rm/dm/DgCollectStore/sjbqSubmit.jsp?id="+selId;  
