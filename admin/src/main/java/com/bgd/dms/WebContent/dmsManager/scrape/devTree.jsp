...
Ext.onReady(function() {
    ...
    var store = Ext.create('Ext.data.TreeStore', {
        ...
        proxy: {
       	 type : 'ajax',
            method: 'get',
            url: '<%=contextPath%>/dmsManager/scrape/devqueryTree.srq?root=<%=root%>&scrapeType=<%=scrapeType %>&scrape_apply_id=<%=scrape_apply_id %>',
            reader: {
                type : 'json'
            }
        },
    ...
    function cellclick(grid,colspan, colIndex, rowdata, rowspan,rowIndex,e) {
        ...
        parent.menuFrame.location = "<%=contextPath%>/dmsManager/scrape/devList.jsp?bussId=<%=bussId %>&scrapeType=<%=scrapeType %>&fileId=<%=fileId %>&scrape_apply_id=<%=scrape_apply_id %>&codeId="+selectParentIdData+"&codeName="+selectText;
    }
});
