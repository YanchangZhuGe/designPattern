...
<%
	...
	String businessType_s="5110000004100000008";
	...
	if("5000100004000000009".equals(projectType)){//综合物化探
		response.sendRedirect(contextPath+"/rm/em/singleHuman/humanTrainingPlan/humanTrainingPlanListZh.jsp");
	}
%>
      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <auth:ListButton functionId="F_HUMAN_0014" css="dc" event="onclick='exportData()'" title="JCDP_btn_export"></auth:ListButton>
			    </td>
			  </tr>
			</table>
			</td>
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      <td class="bt_info_odd" 	 exp="<input type='checkbox' name='chx_entity_id'  id='chx_entity_id{train_plan_no}' value='{train_plan_no},{proc_status},{org_id},{plan_number},{project_name}'\>">选择</td>
					<td class="bt_info_even" 	 autoOrder="1">序号</td>
					<td class="bt_info_odd" 	 exp="{plan_number}" >单号</td>
					<td class="bt_info_even" 	 exp="{train_amount}">费用（元）</td>
...
<script type="text/javascript">
	...
	function toAdd(){
		...
		popWindow('<%=contextPath%>/rm/em/toHumanTrainingPlanEdit.srq?projectInfoNo=<%=projectInfoNo%>','900:700');
	...
	function toEdit(){
	    ...
	    if(proc_status == 1){
			...
			popWindow("<%=contextPath%>/rm/em/toHumanTrainingPlanEdit.srq?id="+train_plan_no,"900:700");
	 ...
	 function toUploadFile(){
			popWindow('<%=contextPath%>/rm/em/singleHuman/humanTrainingPlan/humanImportFile.jsp?projectInfoNo=<%=projectInfoNo%>');
	 }
	...
	function refreshData(){
		cruConfig.queryStr = "select pn.train_plan_no, pn.plan_number,pn.project_info_no,nvl(pn.train_amount,'0') train_amount,nvl(te.proc_status, '0') proc_status, p.project_name,  i.org_id,       nvl( pn.org_name ,i.org_abbreviation)applicant_org_name,   decode(te.proc_status,   '1',   '待审批',  '3',  '审批通过', '4', '审批不通过',te.proc_status) proc_status_name,pn.creator,       To_char(pn.create_date, 'yyyy-MM-dd hh24:mi') create_date,pn.spare1 ,  t2.employee_name applicant_name  from BGP_COMM_HUMAN_TRAINING_PLAN  pn        left join common_busi_wf_middle te on    te.business_id=pn.train_plan_no   and te.bsflag='0'           left join gp_task_project p    on pn.project_info_no = p.project_info_no   and p.bsflag = '0'    left join comm_org_information i    on pn.spare1 = i.org_id   and i.bsflag = '0'       left join comm_human_employee t2    on pn.creator = t2.employee_id   and t2.bsflag = '0'  where pn.bsflag='0' and pn.project_info_no='<%=projectInfoNo%>' order by pn.create_date desc ";
