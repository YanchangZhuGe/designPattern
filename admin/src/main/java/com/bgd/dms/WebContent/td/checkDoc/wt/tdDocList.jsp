...
<script type="text/javascript">
	...
	function refreshData(){
		cruConfig.queryStr = "  select t.* from (  select t.check_id,t.project_info_no,p.project_name,t.upload_id,e.employee_name,t.upload_date,t.check_date,t.check_type,decode(t.check_type ,'0','待审核','1','审核通过','2','审核不通过','未提交') check_type_name from bgp_doc_td_file_check t left join gp_task_project p on t.project_info_no=p.project_info_no left join comm_human_employee e on t.upload_id=e.employee_id  where t.bsflag='0' and t.project_info_no='<%=projectInfoNo%>' and  t.doc_type='"+docType+"' order by t.upload_date desc ) t where 1=1 ";
		cruConfig.currentPageUrl = "/td/checkDoc/wt/tdDocList.jsp";
		queryData(1);
	}
	...
	$(".tj").css({display: "none"});
	var methodType="";
	if("5110000068000000002"==docType){//重力
		methodType="5110000056000000001";
	}else if("5110000068000000003"==docType){//磁力
		methodType="5110000056000000002";
	}else if("5110000068000000006"==docType){//天然场源电磁法
		methodType="5110000056000000003";
	}else if("5110000068000000005"==docType){//人工场源电磁法
		methodType="5110000056000000004";
	}else if("5110000068000000004"==docType){//化学勘探
		methodType="5110000056000000005";
	}else if("5110000068000000007"==docType){//工程勘探
		methodType="5110000056000000006";
	}
		
	if(null!=retObj.explorationMap){
		var methods=retObj.explorationMap.exploration_method_super;
		//判断如果存在勘探方法，按钮启用(除测量外)
		if(methods.indexOf(methodType)>=0){
		...
		//测量启用按钮
		if("5110000068000000001"==methodType){
			$(".zj").css({display: "block"});
			$(".xg").css({display: "block"});
			$(".sc").css({display: "block"});
			$(".tj").css({display: "block"});
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/toCheckDocEdit.srq?docType='+docType,'800:700');
    }
	...
	function toEdit() {
		...
		if(ids.split("-")[1] == "0"){
			alert("该基础检查表已提交,不能修改");
			return;
		}
		if(ids.split("-")[1] == "1"){
			alert("该基础检查表已审核,不能修改");
			return;
		}
		popWindow('<%=contextPath%>/td/toCheckDocEdit.srq?id='+ids.split("-")[0]+'&docType='+docType,'800:600');
	}
	...
	function toDelete(){
		...
		if(ids.split("-")[1] == "0"){
			alert("该基础检查表已提交,不能删除");
			return;
		}
		if(ids.split("-")[1] == "1"){
			alert("该基础检查表已审核,不能删除");
			return;
		}
		...
		var sql = "update bgp_doc_td_file_check t set t.bsflag='1',modifi_date=sysdate where t.check_id = '"+ids.split("-")[0]+"' ";
	...
	function toSubmit(){
		...
		var sql = "update bgp_doc_td_file_check t set t.check_type='0',modifi_date=sysdate where t.check_id = '"+ids.split("-")[0]+"' ";
