...
<table border="0" cellpadding="0" cellspacing="0"class="tab_line_height" width="100%">
	<tr>
    	<td colspan="6" align="center">班组退库</td>
    </tr>
	<tr>
		...
		<td ><select class="select_width" id="s_apply_team" name="s_apply_team" onchange="getTeamDatas()"></select></td>
	</tr>
	<tr>
	...
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" id="queryRetTable">
			<tr>
				...
				<td class="bt_info_even" exp="{out_num}">领用总量</td>
...
<script type="text/javascript">
	...
	function save(){	
		...
		ids = getSelIds('rdo_entity_id');
		 if(ids==''){ 
			 ...
			 var storeroom = document.getElementById("s_apply_team").value;
			var invoices_id= document.getElementById("invoices_id").value;
			document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/warehouseDg/accept/rettrac/saveTeamOutDg.srq?laborId="+ids+"&invoices_id="+invoices_id+"&storeroom="+storeroom;
	...
	function loadDataDetail(shuaId){
	 ...
	 function showDevPage(trid){
			...
			var vReturnValue = popWindow('<%=contextPath%>/mat/singleproject/warehouse/accept/rettrac/selectTracList.jsp?','1024:800');
		}
	 function getMessage(arg){
		 for(var i=0;i<arg.length;i++){
			 refreshData(arg[i]);
			 document.getElementById("procure_no").value=arg[i];
				}
	 	}
...
//	    	var applyTeamList=jcdpCallService("HumanCommInfoSrv","queryApplyTeam","");	
	  ...
	  function getTeamDatas(){
			  if(document.getElementById("invoices_id").value==""){
					var myDate = new Date();
					  //计划编号
					 var invoices_id="BZTK_"+Math.floor(myDate.getTime()/1000);   
					 document.getElementById("invoices_id").value=invoices_id;
				}
			var team_id = document.getElementById("s_apply_team").value;
			refreshData(team_id);
		  }
	  var checked = false;
	  function check(){
