...
rootMenuName = "项目名称";//queryOrgRet.datas[0].org_abbreviation; 
rootMenuId ="0000,1111";//+= ","+queryOrgRet.datas[0].org_subjection_id; 
...
function getSubNodes(parentNode) {
	...
	Ext.Ajax.request({
		...
		params : {
			currentPage:"1",
			pageSize:"10000",
			querySql:"select '1' as is_leaf , tp.project_info_no , tp.project_name, wd.workarea_no , wd.workarea , tp.project_Year,tp.project_status , tp.project_id ,t2.org_id from Gp_Task_Project tp,  gp_workarea_diviede wd, gp_task_project_dynamic t2 where tp.workarea_no = wd.workarea_no and t2.project_info_no = tp.project_info_no and t2.org_id in ('"+orgid+"') and tp.bsflag = '0' and wd.bsflag = '0'  and t2.bsflag = '0' and tp.project_status not in ('5000100001000000003')"
		},
		method : 'Post',
...
function getTreeNode(nodeData){
	...
	if(nodeData.is_leaf=="1"){//叶子节点
		treeNode = new Ext.tree.TreeNode({
			id : nodeData.project_info_no,
			text : nodeData.project_name,// 显示内容									
			leaf : true,	
			singleClickExpand: false,
...
function getCheckedNode(scope) {   
    ...
    for (var i = 0; i < nodes.length; i++) {
        ...
        if(nodes[i].isLeaf()){
        	...
        	index[i] = nodes[i].id + "," + nodes[i].text;
        }else{
...
<table border="0" cellpadding="0" cellspacing="0"  id="FilterLayer" width="100%">
	<tr>
		<td align="left" style="padding-left:400px;padding-top:5px;padding-bottom:5px;">
			<input type="button" value="确定" onclick="getCheckedNode('1')" class="iButton2"/>
		</td>
	</tr>
	<tr>
		<td align="left">
