...
<body   onload="getApplyTeam();getConRatio()" class="odd_odd">
...
<!-- 						  	<td class="inquire_item4">倍数：</td>
 ...
 -->						<td class="inquire_item4">金额:</td>
						   	...
						   	<input type='hidden' name='if_purchase' id='if_purchase' value='2'/>
						   	<!-- 
						   	<tr>
...
//		sql +="select t.* from gms_mat_infomation t where t.bsflag='0'and wz_id='"+value+"' order by t.coding_code_id asc,t.wz_id asc";
		debugger;
		sql +="select i.*, t.stock_num, t.recyclemat_info, t.actual_price from gms_mat_recyclemat_info t inner join gms_mat_infomation i"
			+" on t.wz_id = i.wz_id and i.bsflag = '0' and t.bsflag = '0' and t.wz_type = '2' and i.wz_id='"+value+"' and t.stock_num>0 and t.org_subjection_id =  (select org_subjection_id from bgp_comm_org_wtc w where (select org_subjection_id from gp_task_project_dynamic where project_info_no = '"+projectInfoNo+"') like w.org_subjection_id || '%') "
			+" order by i.coding_code_id asc, i.wz_id asc";
		
		cruConfig.cdtType = 'form';
		cruConfig.queryStr = sql;
		cruConfig.currentPageUrl = "/mat/singleproject/plan/repMatPlan/planAddItemList.jsp";
		queryData(1);
	}
	function toDelete(){
		  ...
		  ids = getSelIds('rdo_entity_id');
			if (ids == '') {
				...
				if(loadDataDetail()){				
					...
					document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/plan/saveRepPlan.srq";
...
//	    	var applyTeamList=jcdpCallService("HumanCommInfoSrv","queryApplyTeam","");	
       ...
       function toAdd(trid){
			...
			var ids=getSelIds('rdo_entity_id');
				var vReturnValue = window.showModalDialog('<%=contextPath%>/mat/singleproject/plan/repMatPlan/selectRepMatList.jsp?ids='+ids,obj,'dialogWidth=1024px;dialogHigth=400px');
//				var vReturnValue = window.showModalDialog('<%=contextPath%>/mat/singleproject/plan/repMatPlan/selectMatList.jsp?ids='+ids,obj,'dialogWidth=1024px;dialogHigth=400px');
			if(vReturnValue!=undefined){
				var returnvalues = vReturnValue.split('~');
	...
	function getTimes(){
		...
		for(var i=1;i<row.length;i++){
			...
			if(cell_8!=undefined){
					if(cell_8=='&nbsp;'){
						cell_8=0;
						row[i].cells[9].firstChild.value = cell_8*times;
						}
					else{
						row[i].cells[9].firstChild.value = cell_8*times;
						}
