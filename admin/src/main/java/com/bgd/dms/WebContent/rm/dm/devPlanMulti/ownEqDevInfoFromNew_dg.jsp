    ...
    <div id="new_table_box_bg" style="width:95%">
	  ...
	  <fieldset style="margin-left:2px"><legend>调配数量明细</legend>
		  <div style="overflow:auto">
			  <table style="width:98%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
					<td class="bt_info_odd" width="5%"><input type="checkbox" name="alldetinfo" id="alldetinfo" checked/></td>
      ...
      <table style="width:98%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td width="5%"><span class="zj"><a href="#" id="addbtn" onclick='toAddMixInfos()' title="多选台帐明细"></a></span></td>
          <td width="90%"></td>
        </tr>
      </table>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		if('<%=devappid%>'!=null){
			...
			+"left join comm_org_subjection appsub on devapp.app_org_id=appsub.org_id and appsub.bsflag='0' "
			...
			if(dgys == 'Y'){//小车/运输设备服务中心
				$("#out_org_id").val(dgyxorg);
				$("#out_org_name").val('小车/运输设备服务中心');
			}else{
			...
			if(orgType=="Y"){
				...
				prosql += "appdet.dev_name as dev_ci_name,appdet.dev_type as dev_ci_model,";
				prosql += "case appdet.dev_out_org_id when 'C6000052795280' then '小车/运输设备服务中心' else info.org_abbreviation end as org_name,detsub.org_subjection_id,";
				...
				if(dgys == 'Y'){//小车/运输设备服务中心
					prosql += "and (appdet.dev_out_org_id ='"+dgyxorg+"' or appdet.dev_out_org_id = 'C6000000005280' or appdet.dev_out_org_id = 'C6000000005279') ";
				}else{
					prosql += "and detsub.org_subjection_id like '<%=orgsubid%>%' ";
				}
		...
		if(retObj!=undefined){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td width='5%'><input type='checkbox' name='detinfo' id='"+index+"' value='"+retObj[index].device_app_detid+"' checked/></td>";
				...
				innerhtml += "<td width='7%'><input name='apply_num"+index+"' id='apply_num"+index+"' style='line-height:15px' value='"+retObj[index].apply_num+"' size='5' type='text' readonly/></td>";
	...
	function toAddMixInfos(trid,devcicode,isdevicecode,devicename){
		...
		$("input[type='checkbox'][name='detinfo']").each(function(i){
			if(this.checked){
				var indexinfo1 = this.id;					
				var mixnuminfo1 = $("#mixnum"+indexinfo1).val();
				//累加调配数量
				mixNum += parseInt(mixnuminfo1);
			}
		});
		
		//回头加上转出单位
		var out_org_id = '<%=orgsubid%>';
		var dialogurl = "<%=contextPath%>/rm/dm/tree/selectAllAccountForMix.jsp?out_org_id="+out_org_id;
		var vReturnValue = window.showModalDialog(dialogurl, obj ,"dialogWidth=950px;dialogHeight=480px");
		if(vReturnValue!=undefined){
			//返回信息是 队级台账id
