...
<script type="text/javascript">
	...
	function newSubmit() {
		popWindow("<%=contextPath %>/qua/mProject/notice/notice_menu.jsp",'1280:680');
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.queryStr = " select concat(concat(t.file_id ,':'),t.ucm_id) ids,t.file_id,t.file_name,'物探处' file_from,'doc' file_del ,"+
		...
		" select concat(concat(t.file_id ,':'),t.ucm_id) ids,t.file_id,t.file_name,'东方' file_from, 'notice' file_del ,n.notice_id ," +
		...
		" where n.bsflag='0' and n.org_subjection_id ='"+org_subjection_id+"' and t.file_name like '%"+file_name+"%'";
		if(subjection_id!=null && subjection_id =='C105'){
			cruConfig.queryStr = " select concat(concat(t.file_id ,':'),t.ucm_id) ids,t.file_id,t.file_name,'东方' file_from, 'doc' file_del ," +
			" '' notice_id ,to_char(t.create_date,'yyyy-MM-dd') as create_date from bgp_doc_gms_file t " +
	...
	function toAdd(){
		...
		var relation_id = 'notice:'+subjection_id;
	  	...
	  	popWindow('<%=contextPath%>/qua/common/upload_file.jsp?relationId='+relation_id+'&path=company&index=');
	}
