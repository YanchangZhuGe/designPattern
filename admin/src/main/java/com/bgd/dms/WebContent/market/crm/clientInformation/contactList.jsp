...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.currentPageUrl = "<%=contextPath%>/market/crm/clientInformation/contactList.jsp";
		queryData(1);
		resizeNewTitleTable();
	}
	refreshData();
	...
	function toAdd() { 
		...
		popWindow("<%=contextPath%>/market/crm/clientInformation/contactEdit.jsp?company_id="+company_id);
	}
	function toEdit() {
		var company_id ='<%=company_id%>';
		if(company_id==null || company_id==''){
		...
		for(var i=1;i<obj.length;i++){
			if(obj[i].checked ==true){
				...
				if(contact_id!=null && window.confirm(text)){
					popWindow("<%=contextPath%>/market/crm/clientInformation/contactEdit.jsp?contact_id="+contact_id+"&company_id="+company_id);
					return;
				}
			}
		}
