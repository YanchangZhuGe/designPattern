...
<div id="new_table_box">
  <div id="new_table_box_content">
    <div id="new_table_box_bg">
      ...
      <fieldSet style="margin:2px:padding:2px;"><legend>评审信息</legend>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          <td style="width: 10%" >评审组成员:</td>
          <td style="width: 40%">
           <textarea id="experts" name="experts"  class="textarea" ></textarea>
          </td>
           <td style="width: 10%">评审意见<font color="red">(为空时批复时不可见)</>:</td>
          <td style="width: 40%">
...
//add by zzb
	...
	function saveInfo(){
		...
		if($("#apply_date").val()==""){
	 	...
	 	var  baseData = jcdpCallService("ScrapeSrv", "isAllPassDone", "scrape_apply_id="+$("#scrape_apply_id").val());
	 	if(baseData.message){
	 	alert(baseData.message);
	 	//return;
	 	}
	 	if(scrape_collect_id=='null'){
			scrape_collect_id=guid();
		...
		Ext.Ajax.request({
					url : "<%=contextPath%>/dmsManager/scrape/addScrapeallCollectNew.srq",
					method : 'Post',
					isUpload : true,  
					async :  true,
					form : "form1",
...
//删除文件
	...
	function simpleSearch() {
		...
		//var s_project_name = document.getElementById("s_project_name").value;//项目名和国家名
 	    var message ="&scrape_type="+s_scrape_type+"&s_dutyUnit="+s_org_name;
		...
		baseData = jcdpCallService("ScrapeSrv", "getScrapeApplyCollectNew", "ids="+scrape_apply_id+"&scrape_collect_id="+scrape_collect_id+message);
		if (baseData.datas != null) {
	...
	function subgo(i){
		...
		var scrape_type = $("#coding_code_id"+i).val();
		var file_type=$("#file_type"+i).val();
		var scrape_type_name=$("#scrape_type_name"+i).val();
	 	var returnFlag = popWindow("/DMS/dmsManager/scrape/selectScrapeListCollectNew.jsp?scrape_apply_id="+scrape_apply_id+"&scrape_type="+scrape_type+"&file_type="+file_type,''+document.body.offsetWidth+':'+document.body.offsetHeight+'',scrape_type_name+'-确定是否通过');
	 	if(returnFlag=="1"){
