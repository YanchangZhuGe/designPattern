					...
					<td colspan="3" align="center" >
						<div class="tongyong_box">
							<div class="tongyong_box_title">
								<span>设备利用率同比&nbsp;</span>
								<span>&nbsp;设备类型：</span>
								<input id="ajglyldbdev_type1" name="ajglyldbdev_type" />
								<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor:hand;" onclick="showDevTypeTreePage()"  />
								<input id="ajglyldbdev_type_id1" name="ajglyldbdev_type_id" class="" type="hidden" />
						    	<span>&nbsp;开始时间：</span>
								<input  id="ajglyldbdate_1_1" name="start_date"    required="true"/>
						 
					    		<span>&nbsp;</span>
					    		<input type="button" value="查询" class="input" style="width:45px;height:20px;" onclick="ajglyldbtoQuery1_tb()"/>
					    		<span>&nbsp;</span>
					    		<input type="button" value="清除" class="input" style="width:45px;height:20px;" onclick="ajglyldbtoClear()"/>
							</div>
							<div class="tongyong_box_content_left" id="ajglyldbchartContainer1" style="height: 280px;"></div>
						</div>
					</td>
				</tr>
...
$(function(){
	...
	function getAJGLYLDBFusionChart(){
		var ajglyldbchart1 = new FusionCharts( "${applicationScope.fusionChartsURL}/Charts/Column3D.swf", "ajglyldbcId1", "95%", "280", "0", "0" );    
		ajglyldbchart1.setXMLUrl("<%=contextPath%>/dms/device/getUseRateNew.srq?flag=tb");
		ajglyldbchart1.render("ajglyldbchartContainer1");
	}
	//查询
	function ajglyldbtoQuery1_tb(){
		var dev_type_id = $('#ajglyldbdev_type_id1') .val();
		var startDate= $('#ajglyldbdate_1_1').datebox("getValue"); 
		var chartReference1 = FusionCharts("ajglyldbcId1"); 
	...
	function ajglyldbtoClear(){
		$('#ajglyldbdev_type_id1') .val('');
		$('#ajglyldbdev_type1') .val('');
	...
	function showDevTypeTreePage(){
		...
		document.getElementById("ajglyldbdev_type1").value = returnValue.value;
		
		//var orgId = strs[1].split(":");
		document.getElementById("ajglyldbdev_type_id1").value = returnValue.fkValue;
	}
</script>
