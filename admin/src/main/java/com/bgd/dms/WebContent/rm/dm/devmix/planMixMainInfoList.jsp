      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td class="ali_query">
			    	<span class="cx"><a href="#" onclick="searchDevAppData()" title="查询"></a></span>
			    </td>
			    <td class="ali_query">
			    ...
			    <auth:ListButton functionId="" css="jl" event="onclick='toMixDetailList()'" title="查看明细"></auth:ListButton>
			  </tr>
			</table>
			</td>
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_app_id_{device_app_id}' name='device_app_id'>
...
$(function(){
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				str += "where devapp.bsflag='0' and devapp.device_app_id='"+currentid+"' and wfmiddle.proc_status='3' ";
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			...
			innerHTML += "<td>"+datas[i].record+"</td><td>"+datas[i].assigned_num+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
    ...
    function loadDataDetail(deviceappid){
    	
    	loadProcessHistoryInfo();
    
    	var retObj;
		if(deviceappid!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getMixAppBaseInfo", "deviceappid="+deviceappid);
	...
	function refreshData(v_device_app_no,v_project_name){
		var str = "select pro.project_name,devapp.device_app_id,devapp.device_app_no,devapp.device_app_name,devapp.project_info_no,";
			str += "devapp.employee_id,devapp.appdate,devapp.create_date,devapp.modifi_date,";
			str += "case wfmiddle.proc_status when '1' then '待审批' when '3' then '审批通过' when '4' then '审批不通过' else '未提交' end as state_desc,";
			str += "org.org_abbreviation as org_name,emp.employee_name,allapp.device_allapp_name,allapp.device_allapp_no,trim(devapp.mix_org_id) as org_id ";
			...
			str += "where devapp.bsflag = '0' and allwfmiddle.proc_status='3' and wfmiddle.proc_status='3' ";
		/* 补充查询条件
	...
	function toMixDetailList(obj){
		...
		window.location.href = '<%=contextPath%>/rm/dm/devmix/planMixDetailList.jsp?devappid='+id;
	}
	function dbclickRow(shuaId){
		window.location.href = '<%=contextPath%>/rm/dm/devmix/planMixDetailList.jsp?devappid='+shuaId;
	} 
