				...
				<div id="new_table_box_bg" style="width: 95%">
					<fieldset style="margin: 2px:padding:2px;">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<tr>
								...
								<td class="inquire_form4"><input name="repair_form_name" readonly
								...
								<td class="inquire_form4"><input name="apply_date" readonly
									id="apply_date" class="input_width" type="text" value=""
									readonly />  </td>
							</tr>
							<tr>
								<td class="inquire_item4">所属项目:</td>
								<td class="inquire_form4" colspan="1"><input readonly
								...
								<td class="inquire_form4"><input id="req_comp_name" readonly
									...
									onclick="showOrgTreePage('apply_org')" />
									 -->
									 <input
									id="apply_org" name="apply_org" class="" type="hidden" /></td>
							</tr>
							<tr>
								<td class="inquire_item4">维修单位:</td>
								<td class="inquire_form4"><input name="service_company" readonly
									id="service_company" class="input_width" type="text" value="" /></td>
								<td class="inquire_item4">币种:</td>
								<td class="inquire_form4">
								<devselect:combolist name='currency' selectdisabled='true'
							...
							<tr>
								...
								<td class="inquire_form4"><input name="rate" id="rate" readonly
									class="input_width" type="text" value="" /></td>
								<td class="inquire_item4">预计原币金额:</td>
								<td class="inquire_form4"><input name="buget_our" readonly
									id="buget_our" class="input_width" type="text" value="" /></td>
							</tr>
							<tr>
								<td class="inquire_item4">预计本币金额:</td>
								<td class="inquire_form4"><input name="buget_local" readonly
					...
					<fieldset>
						...
						<table id="table2" width="100%" border="0" cellspacing="0"
							...
							<tr>
								<td><textarea id='remark' name='remark' readonly rows="5" cols="74"></textarea>
								</td>
							</tr>
						</table>
					</fieldset>
					<fieldset style="margin-left: 2px">
						...
						<div style="overflow: auto">
							<table style="width: 97.9%" border="0" cellspacing="0"
								cellpadding="0" class="tab_line_height">
								<tr>
									<td width="1%" class="bt_info_odd"> </td>
...
$().ready(function(){
	...
	function refreshData(){
		...
		$("#service_company").val(repairObj[0].service_company);
		$("#currency").find("option[value="+repairObj[0].currency+"]").attr("selected",true);
		$("#rate").val(repairObj[0].rate);
		$("#buget_our").val(repairObj[0].buget_our);
		$("#buget_local").val(repairObj[0].buget_local);
		var querySql =
			"SELECT ACC.DEV_NAME AS DEV_NAME,\n" +
			"       ACC.DEV_MODEL AS DEV_MODEL,\n" + 
			"       ACC.DEV_TYPE AS DEV_TYPE,SUB.DEV_ACC_ID as DEV_ACC_ID,\n" + 
			"       SUB.ERROR_TYPE as ERROR_TYPE,\n" + 
			"       SUB.ID as ID,\n" + 
			"      SUB.ERROR_TYPE  AS ERROR_DESC,\n" + 
	...
	function addLine(retObj,readflag){  //flag 是否从数据库中读取  true  是
		...
		for(var index=0;index<retObj.length;index++){
			...
			innerhtml += "<td width='13%'><devselect:combolist selectdisabled='true' name='error_type"+line+"' id='error_type"+line+"' selectdefaultV='true' selectedValue='0' typeKey='errortype'></devselect:combolist></td>";
			innerhtml += "<td width='13%'><devselect:combolist selectdisabled='true' name='error_desc"+line+"'  id='error_desc"+line+"' selectdefaultV='true' selectedValue='0' typeKey='errordesc'></devselect:combolist></td>";			
			if(readflag){
				innerhtml += "<td width='13%'><input name='remark"+line+"' readonly id='remark"+line+"' value='"+retObj[index].remark+"' size='10'  type='text' /></td>";
			}else{
				innerhtml += "<td width='13%'><input name='remark"+line+"' readonly id='remark"+line+"' value='' size='10'  type='text' /></td>";
			}
			innerhtml += "</tr>";
