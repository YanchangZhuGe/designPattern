...
<body style="background:#fff"  onload="frameSize()">
      	<div id="list_table">
			...
			<input type="button" name="daily_mian" id="daily_mian" value="面积、长度/公里数" onclick="dailyChange();"></input> 
			 <input type="button" name="daily_mian1" id="daily_mian1" value="坐标点" onclick="dailyChange1();"></input></td>
			    <input type="button" name="daily_mian2" id="daily_mian2" value="审批" onclick="dailyChange2();"></input></td>
			...
			<div id="table_box">
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_odd" exp="{produce_date}<input type='hidden' name='audit_status_{daily_no}' id='audit_status_' value='{audit_status}'/>" >施工日期</td>
			...
			%>
					...
					<td class="bt_info_even" exp="{<%=mapFields.get("exploration_method_name"+i) %>}%" ><%=map.get("exploration_method_name"+i) %>完成百分比%</td>
			  <%	
				  }
				}
			  %>
...
<script type="text/javascript">
function dailyChange2(){
    popWindow('<%=contextPath%>/pm/dailyReport/singleProject/wt/reportListAuditDrillCollExcle.jsp','1280:800');
}
debugger;
	...
	function toDelete(){
	   ...
	   var ids = getSelIds('rdo_entity_id');
		...
		if(confirm('确定要删除吗?')){  
			...
			var retObj = jcdpCallService("WtDailyReportSrv", "deleteDailyReport", "projectInfoNo=<%=projectInfoNo %>&produceDate="+proDate);
		   ...
		   var retObj = jcdpCallService("WtDailyReportSrv", "getDailyReportInfo", "dailyNo="+daily_no+"&projectInfoNo="+project_info_no+"&produceDate="+produce_date);
			...
			if( audit_status!=""&&audit_status!=null ){
				if( audit_status=="0"||audit_status=="4"){		/* 审批状态 处于“未提交”和“审批不通过”的前提下，才能提交 */
					var retObj = jcdpCallService("WtDailyReportSrv", "submitDailyReport", "dailyNo="+daily_no+"&projectInfoNo="+project_info_no+"&produceDate="+produce_date+"&audit_status=1");
