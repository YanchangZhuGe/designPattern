...
<body>
<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="<%=contextPath%>/market/updateManage.srq">
<table border="0" cellpadding="0" cellspacing="0" width="95%" height="800" class="form_info">
	...
	<tr class="odd">
  	...
  	<tr class="odd">
    	...
    	<td class="inquire_form">
    		<% 
		    	String sql = "select t.code,t.code_name from bgp_infomation_type_info t where t.father_code='104' order by t.code";
			...
			%>
					<input type="radio" name="two_type_id" id="two_type_id" value="<%=code %>"><%=codeName %> &nbsp;
			<%		
				}
			%>
      </td>
      <td class="inquire_item"><font color=red>*</font>&nbsp;选择单位：</td>
      <td class="inquire_form">
      <select name="three_type_id" id="three_type_id" class="select_width">
      		<option value="">--请选择--</option>
      <% 
    	String sqlOrg = "select t.code,t.code_name from bgp_infomation_type_info t where t.father_code='10401' order by t.code";
		List listOrg = BeanFactory.getQueryJdbcDAO().queryRecords(sqlOrg);
		for(int i= 0;i<listOrg.size();i++){
			Map mapC2=(Map)listOrg.get(i);
			String code = (String)mapC2.get("code");
			String codeName = (String)mapC2.get("codeName");
			%>
...
<script type="text/javascript">
	...
	document.getElementById("three_type_id").value=<%=(String)map.get("threeTypeId")%>;

	var two = document.getElementsByName("two_type_id");
	for(var i=0;i<two.length;i++){
		if(two[i].value==<%=(String)map.get("twoTypeId")%>){
			two[i].checked="checked";
		}
	}
...
function cancel()
{
	window.location="<%=contextPath%>/market/marketScglPage/MarketManagePageList.jsp";
}
...
function checkText(){
	...
	var two=document.getElementsByName("two_type_id");
	var three=document.getElementById("three_type_id").value;
