...
function page_init(){
	...
	if(projectInfoNo!='null'){
		var querySql = "    select    p.project_name,   dc.org_id,dc.org_subjection_id,    nvl(d3.workarea, p.workarea_no) as workarea_name,        d4.coding_name surface_type       from gp_task_project p              left join  gp_task_project_dynamic dc          on dc.project_info_no = p.project_info_no  and dc.bsflag='0'                            left join gp_workarea_diviede d3      on p.workarea_no = d3.workarea_no       and d3.bsflag = '0'                    left join comm_coding_sort_detail d4       on d3.surface_type = d4.coding_code_id         and d4.bsflag = '0'     where p.bsflag = '0'           and p.project_info_no =   '"+projectInfoNo+"'";		
		...
		if(datas != null){		
			...
			//查询多个施工队伍，
			var org_ids = datas[0].org_id.split(","); 
			var org_id_sb="";
			for(var i=0;i<org_ids.length;i++){		
				org_id_sb = org_id_sb + "," + "'" + datas[0].org_id.split(",")[i]+ "'";
			}
			var querySqlw = "     select wm_concat(org_abbreviation) as org_name,wm_concat(org_id) as org_id from comm_org_information where org_id in("+org_id_sb.substr(1)+") and bsflag = '0'  ";		
			var queryRetw = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySqlw);
			var datasw = queryRetw.datas;
			
			if(datasw != null){		
				document.getElementById("org_name").value=datasw[0].org_name;
	...
	if(projectInfoNo!='null'){
		var querySql = " select  sum(c.sum_human_cost)sum_cost , t.*, nvl(sum(c.sum_human_cost),0) - nvl(t.sum_cost_act,0) as last_num   ,decode(t.coding_lever,'1','#4BAA34') t_color   from (   select t.coding_name,   s1.sum_human_cost sum_cost_act,  trunc(s.sum_human_cost)sum_human_cost,  trunc(s.cont_cost)cont_cost,  trunc(s.mark_cost)mark_cost,  trunc(s.temp_cost)temp_cost,  trunc(s.reem_cost)reem_cost,  trunc(s.serv_cost)serv_cost,  t.coding_code_id,  t.coding_lever,  t.superior_code_id    from comm_human_coding_sort  t   left join (select s.project_info_no,  s.subject_id,  sum(s.sum_human_cost) sum_human_cost,  sum(s.cont_cost) cont_cost,  sum(s.mark_cost) mark_cost,  sum(s.temp_cost) temp_cost,  sum(s.reem_cost) reem_cost,  sum(s.serv_cost) serv_cost  from bgp_comm_human_cost_act_deta s  where s.project_info_no =  '"+projectInfoNo+"'   and s.bsflag = '0'    and to_char(s.app_date, 'yyyy-MM') <= '"+appDate+"'   group by s.project_info_no, s.subject_id) s1  on t.coding_code_id = s1.subject_id    left join (select s.project_info_no,  s.subject_id,  sum(s.sum_human_cost) sum_human_cost,  sum(s.cont_cost) cont_cost,  sum(s.mark_cost) mark_cost,  sum(s.temp_cost) temp_cost,  sum(s.reem_cost) reem_cost,  sum(s.serv_cost) serv_cost  from bgp_comm_human_cost_act_deta s  where s.project_info_no =  '"+projectInfoNo+"'   and s.bsflag = '0'    and to_char(s.app_date, 'yyyy-MM') = '"+appDate+"'   group by s.project_info_no, s.subject_id) s    on t.coding_code_id = s.subject_id   where t.coding_sort_id = '0000000005'  and t.bsflag = '0'  group by t.coding_name,  t.coding_code_id,  t.coding_lever,  t.superior_code_id,  s1.sum_human_cost,  s.sum_human_cost,  s.cont_cost,  s.mark_cost,  s.temp_cost,  s.reem_cost,  s.serv_cost    order by t.coding_lever  )  t    left   join bgp_comm_human_cost_plan_sala c  on t.coding_code_id = c.subject_id  and c.project_info_no = '"+projectInfoNo+"'     and c.bsflag = '0'  where   c.sum_human_cost  is not null   group by t.coding_name,  t.coding_code_id,  t.coding_lever,  t.superior_code_id,  t.sum_cost_act,  t.sum_human_cost,  t.cont_cost,  t.mark_cost,  t.temp_cost,  t.reem_cost,  t.serv_cost   order by t.coding_code_id ";
