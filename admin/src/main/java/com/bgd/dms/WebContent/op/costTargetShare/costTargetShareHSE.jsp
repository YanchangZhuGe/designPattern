...
<body style="background:#fff" onload="refreshData()">
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <td align="right" style="padding-right: 20px;"><font color="red"><span id="sum_value"></span></font></td>
			  <%if(proc_status){ %>
			   <auth:ListButton functionId="OP_TARGET_HSE_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_HSE_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_HSE_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_HSE_EDIT" css="dr" event="onclick='importDatas()'" title="JCDP_btn_import"></auth:ListButton>
			   <%} %>
			   <auth:ListButton functionId="OP_TARGET_HSE_EDIT" css="dc" event="onclick='exportDatas()'" title="JCDP_btn_export"></auth:ListButton>
			  </tr>
			...
			<div id="table_box">
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			            ...
			            <td class="bt_info_even" onclick="getSum()">合计</td>		
			        </tr>            
			   </table>
			</div>
			<div id="fenye_box">
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getHseShareInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
    		...
    		td.innerHTML ='<input type="hidden" id="fy'+i+'hse_money" name="fy'+i+'hse_money" value="'+datas[i].hseMoney+'" class="input_width"/>'+
    		'<INPUT id="fy'+i+'checkbox" name="rdo_entity_id"  value='+datas[i].costHseId+' type=checkbox>';
    		
    		var td = tr.insertCell();
			td.innerHTML = datas[i].rownum;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].hseName;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].hseUnit;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].hseCountT;

			var td = tr.insertCell();
			td.innerHTML = datas[i].hseMoney;
		}
	}
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareHseEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	if (ids == '') {
		...
		popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareHseEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	var id=ids.split(",");
	$.each(id,function(){
		var sql = "update BGP_OP_COST_TARTET_HSE t set t.bsflag='1' where t.COST_HSE_ID ='"+this+"'";
 ...
 function loadDataDetail(clickId){
  ...
  function importDatas(){
	  popWindow(cruConfig.contextPath+"/op/ExcelImport.jsp?path=/op/OPCostSrv/commImportExcel.srq&import_function=importShareHSE");
  }
  function  exportDatas(){
	  var file_name = "安全设施费用测算";
	  file_name = encodeURI(encodeURI(file_name));
	  var project_info_no = "<%=projectInfoNo%>";
	  window.open("<%=contextPath%>/op/OPCostSrv/commExportExcel.srq?export_function=exportShareHSE&file_name="+file_name+"&project_info_no="+project_info_no);
  }
  function getSum(){
		var project_info_no = '<%=projectInfoNo%>';
		var querySql = "select sum(nvl(to_char(t.hse_unit*(t.hse_count_t),'9999999999999999.99'),0)) sum_value"+
			" from bgp_op_cost_tartet_hse t where t.project_info_no='<%=projectInfoNo%>'"+
