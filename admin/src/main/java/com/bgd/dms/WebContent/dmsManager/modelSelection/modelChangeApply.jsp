...
<form name="form1" id="form1" method="post" action="" enctype="multipart/form-data" >
  ...
  <div id="new_table_box_content" style="background-color: rgb(241, 242, 243)">
	...
	<fieldSet style="margin:2px:padding:2px;"><legend>设备技术改造项目申请</legend>
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
	     <input name="change_id" id="change_id" type="hidden" value="<%=change_id%>" />
	     <input name="dev_coding" id="dev_coding" type="hidden" value="" />
	    <tr>
    		<td class="inquire_item4" width="10%"><font color="red">*</font>&nbsp;项目名称：</td>
    		<td class="inquire_form4" class="inquire_form4"><input name="project_name" id="project_name" class="input_width easyui-validatebox main" type="text" value="" style="width: 100%" data-options="validType:'length[0,50]'" maxlength="50" required/></td>
    		<td class="inquire_item4" width="10%">&nbsp;合同编号：</td>
    		<td class="inquire_form4" ><input name="cont_num" id="cont_num" class="input_width easyui-validatebox main"  type="text" value="" style="width: 100%" data-options="validType:'length[0,50]'" maxlength="50" required/></td>
	    </tr>
	    <tr>
      		 ...
      		 <td class="inquire_item4" style="width:50px;">
				...
				<input id="apply_uno_code" name="apply_uno_code" value="<%=user.getOrgSubjectionId()%>" class="" type="hidden" />
		    </td>
		    <td class="inquire_item4" width="10%">&nbsp;项目金额：</td>
    		<td class="inquire_form4" ><input name="cont_money" id="cont_money" class="input_width easyui-validatebox main"  type="text" value="" style="width: 100%" data-options="validType:'length[0,50]'" maxlength="50" required/></td>
	     </tr>
	     <tr>
      		<td class="inquire_item4" ><font color="red">*</font>&nbsp;完成时间：</td>
      		<td class="inquire_form4"><input type="text" name="COMPLETE_DATE" id="COMPLETE_DATE" value="<%=appDate %>" class="input_width easyui-datebox main" style="width:258px" required/></td>
	    	<td class="inquire_item4" width="10%">&nbsp;资金来源：</td>
    		<td class="inquire_form4" ><input name="MONEY_FROM" id="MONEY_FROM" class="input_width easyui-validatebox main"  type="text" value="" style="width: 100%" data-options="validType:'length[0,50]'" maxlength="50" required/></td>
	    	
	     </tr>
	      <tr>
      		<td class="inquire_item4" width="10%">&nbsp;实施单位：</td>
    		<td class="inquire_form4" ><input name="EXPLOITING_ENTITY" id="EXPLOITING_ENTITY" class="input_width easyui-validatebox main"  type="text" value="" style="width: 100%" data-options="validType:'length[0,50]'" maxlength="50" required/></td>
	    	<td class="inquire_item4" width="10%">&nbsp;项目负责人：</td>
    		<td class="inquire_form4" ><input name="PROJECT_LEADER" id="PROJECT_LEADER" class="input_width easyui-validatebox main"  type="text" value="" style="width: 100%" data-options="validType:'length[0,50]'" maxlength="50" required/></td>
	     </tr>
	     <tr>
      		<td class="inquire_item4" ><font color="red">*</font>&nbsp;合同文本：</td>
      		<td  colspan="2" class="inquire_form4"><textarea id="cont_content" name="cont_content"  class="input_width easyui-textarea easyui-validatebox main" style="width:100%;height:40px;" overflow-x:hidded; data-options="validType:'length[0,250]'" maxlength="250" required onpropertychange="checkLength(this,250);" oninput="checkLength(this,250);"></textarea></td>
	     </tr>
	      
		 
		 <tr>
			<td class="inquire_item4" >&nbsp;改造申请附件：</td>
		 ...
		 <tr>
		  ...
		  <tr>
			<td class="inquire_item4" >&nbsp;评审附件：</td>
		 ...
		 <tr>
		  ...
		  <tr>
			<td class="inquire_item4" >&nbsp;验收申请附件：</td>
		 ...
		 <tr>
		  ...
		  <tr>
			<td class="inquire_item4" >&nbsp;验收报告附件：</td>
		...
		<fieldSet style="margin:2px:padding:2px;"><legend>设备明细</legend>
         <table  style="table-layout:fixed;text-align:center;" width="100%" border="0" cellspacing="0" cellpadding="0"class="tab_info" id="itemTable2">
			<tr>
				<td width="10%" class="ali_btn">
					<span class="zj"><a href="javascript:void(0);" onclick="insertTr2()" title="添加"></a></span>
				</td>
				<td width="10%">序号</td>
				<td width="20%">ERP设备编号</td>
				<td width="20%">设备名称</td>
				<td width="30%">规格型号</td>
				<td width="10%">原值(元)</td>
				<td width="10%">净值(元)</td>
...
<script type="text/javascript">
	...
	function refreshData(){ 
		var baseData; 
		if('<%=change_id%>'!='null'){ 
			baseData = jcdpCallService("EquipmentSelectionApply", "getModelChangeInfo", "change_id="+$("#change_id").val());
			
			var basedata = jcdpCallService("EquipmentSelectionApply", "getDevInfoWithModelChange", "change_id="+$("#change_id").val());
			basedatas = basedata.deviceappMap;
			
			$("#project_name").val(baseData.deviceappMap.project_name);
			$("#cont_num").val(baseData.deviceappMap.cont_num);
			$("#apply_unit").val(baseData.deviceappMap.apply_unit);
			$("#COMPLETE_DATE").val(baseData.deviceappMap.complete_date);
			$("#cont_money").val(baseData.deviceappMap.cont_money);
			$("#MONEY_FROM").val(baseData.deviceappMap.money_from); 
			$("#EXPLOITING_ENTITY").val(baseData.deviceappMap.exploiting_entity);
			$("#PROJECT_LEADER").val(baseData.deviceappMap.project_leader);
			$("#cont_content").val(baseData.deviceappMap.cont_content);
		 
			if(basedatas!=undefined && basedatas.length>=1){
				//先清空
			 	for(var i=0;i<basedatas.length;i++){
			 	insertTr3(basedatas[i]);
			 	}
	...
	function saveInfo(){
		...
		var cont_num = $.trim($("#cont_num").val());
		if(cont_num<=0){
		$.messager.alert("提示","合同编号不能为空!");
			return;
		}
		var cont_money =  $.trim($("#cont_money").val());
		if(cont_money<=0){
		$.messager.alert("提示","项目金额不能为空!");
			return;
		}
		...
		$.messager.confirm("操作提示", "您确定要执行操作吗？", function (data) {
            if (data) {
    			...
    			document.getElementById("form1").action = "<%=contextPath%>/dmsManager/modelSelection/addModelChange.srq";
	...
	function getTestFileInfoPublic(item){debugger;
		...
		jcdpCallService("EquipmentSelectionApply", "uploadFile", "change_id="+'<%=change_id%>');
	}
	//删除行
