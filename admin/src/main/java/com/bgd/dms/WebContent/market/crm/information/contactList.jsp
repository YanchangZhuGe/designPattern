...
<script type="text/javascript">
	...
	function toAdd() { 
		...
		popWindow("<%=contextPath%>/market/crm/information/contactEdit.jsp?company_id="+company_id+"&company_name="+encodeURI(company_name));
	}
	function toEdit() {
		var company_id ='<%=company_id %>';
		if(company_id==null || company_id==''){
		...
		for(var i=1;i<obj.length;i++){
			if(obj[i].checked ==true){
				...
				if(contact_id!=null && window.confirm(text)){
					popWindow("<%=contextPath%>/market/crm/information/contactEdit.jsp?contact_id="+contact_id+"&company_id="+company_id);
					return;
				}
			}
		}
