...
<body style="background:#fff" scroll="no">
	<div id="list_table">
		<div id="tag-container_3">
		  	<span>&nbsp;&nbsp;</span>
		  	<a style="width: 105px;" href="#" onclick="getTab(0)"><span id="div_a_span0" style="font-size:16px;">市场及分布</span></a>
		  	<span>&nbsp;&nbsp;</span>
		...
		<div id="tab_box" class="tab_box">
			...
			<div id="tab_box_content2" class="tab_box_content">
				<iframe width="100%" height="99%" name="list" id="iframe_list_2" frameborder="0" src="" marginheight="0" marginwidth="0" >
				</iframe>
			</div>
			<div id="tab_box_content3" class="tab_box_content">
				<iframe width="100%" height="99%" name="list" id="iframe_list_3" frameborder="0" src="" marginheight="0" marginwidth="0" >
				</iframe>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	function getTab(index){
		...
		if(index==0){
			$("#iframe_list_0").attr("src","<%=contextPath%>/dms/plan/yearPlan/forwardMarket.srq?applyId=<%=applyId%>&applyYear=<%=applyYear%>");
		}else if(index==1){
			$("#iframe_list_1").attr("src","<%=contextPath%>/dms/plan/yearPlan/forwardDeviceAnal.srq?applyId=<%=applyId%>&applyYear=<%=applyYear%>");
		}else if(index==2){
			$("#iframe_list_2").attr("src","<%=contextPath%>/dmsManager/plan/yearPlan/supp_data_file.jsp?applyId=<%=applyId%>&applyYear=<%=applyYear%>");
		}else if(index==3){
			$("#iframe_list_3").attr("src","<%=contextPath%>/dms/plan/yearPlan/forwardRemark.srq?applyId=<%=applyId%>&applyYear=<%=applyYear%>");
		}
	}
	function frameSize(){
