...
<%
	...
	String[] deviceallappdetids = request.getParameter("deviceallappdetid").split(",",-1);
	String deviceallappdetid = "";
	String seqinfo = "";
	for(int i=0 ;i<deviceallappdetids.length;i++){
		String[] info = deviceallappdetids[i].split("~",-1);
		if(deviceallappdetids.length>1){
			if(i==0){
				deviceallappdetid ="('"+info[0]+"'";
			}
			else if(i==deviceallappdetids.length-1){
				deviceallappdetid +=",'"+info[0]+"')";
			}
			else{
				deviceallappdetid +=",'"+info[0]+"'";
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		$("#resultdiv"+seqinfo).show();
			if('<%=seqinfo%>'=='0'){
				...
				str += "where alldet.device_allapp_detid in <%=deviceallappdetid%> ";
				...
				for(var index=0;index<retObj.length;index++){
					...
					innerhtml += "<td><input type='checkbox' name='idinfo' id='"+index+"'/><input name='devicename"+index+"' id='devicename"+index+"' style='line-height:15px' value='' size='15' type='text' readonly/>";
					innerhtml += "</td>";
					
					innerhtml += "<td><input id='devicetype"+index+"' name='devicetype"+index+"' value='' size='14'  type='text' readonly/>";
					...
					innerhtml += "<td><input id='neednum"+index+"' name='neednum"+index+"' class='input_width' value='' size='8' type='text' onkeyup='checkAppNum(this,\"approvenum"+index+"\")' readonly/>";
					//综合物化探 项目资源配置中 审批数量
				...
				for(var index=0;index<retObj.length;index++){
					...
					innerhtml += "<td><input type='checkbox' name='collidinfo' id='"+index+"'/><input name='colldevicename"+index+"' id='colldevicename"+index+"' style='line-height:15px' value='用户输入名称' size='15' type='text' readonly/></td>";
					...
					innerhtml += "<td><input name='collneednum"+index+"' id='collneednum"+index+"' class='input_width' value='' size='10' type='text' onkeyup='checkAppNum(this,\"collapprovenum"+index+"\")' readonly/>";
					//综合物化探 项目资源配置中 审批数量
	...
	function copyEsimiDate(){ 
		var seqinfo = '<%=seqinfo%>';
		if(seqinfo==0){
			var date1 = document.getElementById("startdate0").value;
			var date2 = document.getElementById("enddate0").value;
			
			for(var i=0;i<dataLength;i++){
				document.getElementById("startdate"+i).value=date1;
				document.getElementById("enddate"+i).value=date2;
			}
		}
		else{
			var date1 = document.getElementById("collstartdate0").value;
			var date2 = document.getElementById("collenddate0").value;
			
			for(var i=0;i<dataLength;i++){
				document.getElementById("collstartdate"+i).value=date1;
				document.getElementById("collenddate"+i).value=date2;
			}
			}
	}
</script>
