...
<%
	...
	if(request.getParameter("inspection_no") != null){
		inspection_no=request.getParameter("inspection_no");	
	...
	function checkJudge(){
 	...
 	function submitButton(){   
		if(checkJudge()){
			...
			var submitStr = 'JCDP_TABLE_NAME=BGP_MATERIAL_INSPECTION&JCDP_TABLE_ID=&org_sub_id='+org_sub_id
			+'&second_org='+second_org+'&third_org='+third_org+'&check_numbers='+check_numbers+'&check_time='+check_time
			+'&creator=<%=userName%>&updator=<%=userName%>&create_date=<%=curDate%>&modifi_date=<%=curDate%>&bsflag=0';
		 
				submitStr += '&project_no='+projectInfoNo;
			 
			var retObject = syncRequest('Post',path,encodeURI(encodeURI(submitStr)));  //保存主表信息		
			var inspection_no=retObject.entity_id; //获得 主表id 			
 			top.frames('list').frames[2].refreshData();	 			 
 			 window.open("<%=contextPath%>/hse/hseOptionPage/preparednessAndResponse/IhomeFrame.jsp?addTypes=1&isProject=<%=isProject%>&inspection_no="+inspection_no,'homeMain','height=500,width=1000px,left=100px,top=100px,menubar=no,status=no,toolbar=no ', '');
...
var datas1 =null;
var datePS='<%=curDate%>'; 
var datePSe = datePS.split('-')[0];
var datePSr = datePS.split('-')[1];
var datePSer=datePSe+datePSr+"-";
querySql1 = "select count(*)-(-1) as num_p from BGP_MATERIAL_INSPECTION   where bsflag='0'";
...
if(queryRet1.returnCode=='0'){
  datas1 = queryRet1.datas;	 
	if(datas1 != null && datas1 != ''){	 
		var numP=""; 
		if(datas1[0].num_p <10){
			numP="00"+datas1[0].num_p; 
		}
		if(datas1[0].num_p >=10){
			numP="0"+datas1[0].num_p; 
		}
		if(datas1[0].num_p >=100){
			numP=datas1[0].num_p; 
		}
		
		var numDate=datePSer+numP;
		document.getElementsByName("check_numbers")[0].value=numDate;			
	}
}	
 ...
 function  listInfo(){
	if(inspection_no !=null){
		...
		querySql = "    select  tr.org_sub_id,tr.inspection_no,tr.check_numbers,tr.check_content,tr.problem_description,tr.check_people,tr.check_time,tr.rectification_problem,tr.rectification_in,tr.rectification_time ,tr.second_org,tr.third_org,ion.org_name,tr.creator,tr.create_date,tr.bsflag,  oi1.org_abbreviation as second_org_name,     oi2.org_abbreviation as third_org_name  from BGP_MATERIAL_INSPECTION tr    left  join comm_org_subjection os1     on tr.second_org = os1.org_subjection_id    and os1.bsflag = '0'  left  join comm_org_information oi1     on oi1.org_id = os1.org_id    and oi1.bsflag = '0'  left  join comm_org_subjection os2     on tr.third_org = os2.org_subjection_id    and os2.bsflag = '0'  left  join comm_org_information oi2     on oi2.org_id = os2.org_id    and oi2.bsflag = '0'  left  join comm_org_information ion     on ion.org_id = os1.org_id     where tr.bsflag = '0' and tr.inspection_no='"+ inspection_no +"'";				 	 
