      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" class="tab_info" id="queryRetTable">		
			     <tr >
					<td class="bt_info_even" exp="<input type='checkbox' name='rdo_entity_id' value='{rm}' dev_name='{dev_name}' dev_model='{dev_model}' usage_org ='{org_abbreviation}' total_num='{total_num}' unuse_num='{unuse_num}' use_num='{use_num}' other_num='{other_num}' value='{rm}' id='rdo_entity_id_{rm}' />" >选择</td>
				...
				<div id="tab_box_content0" class="tab_box_content">
					<table id="devMap" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
						 ...
						 <tr>
						    ...
						    <td class="inquire_form6"><input id="unit_name" name="unit_name" value="串" class="input_width" type="text" /></td>
...
<script >
	...
	function searchDevData(){
		...
		refreshData(query_dev_name,query_dev_type,owning_sub_org_id);
	}
	//清空查询条件
	...
	//点击树节点查询
	function refreshData(query_dev_name,query_dev_type,owning_org_id){
		var userid = '<%=userSubid%>';
		var orgLength = userid.length;
		var sql='';
		//超级管理员
	 	if(orgLength==4){
		  sql=" select rownum rm,infom.org_abbreviation, t.*,(use_num + unuse_num + other_num) total_num from (select SUBSTR(b.owning_sub_id,0,7) sub_id， b.dev_name, b.dev_model, temp.dev_code, '串' dev_unit, sum(decode(b.using_stat, '0110000007000000001', 1, 0)) as use_num, sum(decode(b.using_stat, '0110000007000000002', 1, 0)) as unuse_num, sum(decode(b.using_stat, '0110000007000000006', 1, 0)) as other_num, b.ifcountry from gms_device_account_b b left join comm_org_information info on info.org_id = b.owning_org_id left join (select ci.device_id, ci.dev_code, ci.dev_name, ci.dev_model, dci.dev_ci_code, dci.dev_ci_name, dci.dev_ci_model from gms_device_collectinfo ci left join gms_device_coll_mapping cm on ci.device_id = cm.device_id left join gms_device_codeinfo dci on cm.dev_ci_code = dci.dev_ci_code) temp on temp.dev_ci_code = b.dev_type where (b.dev_type like 'S14050208%') and info.bsflag = '0' and b.bsflag = '0' and b.account_stat = '0110000013000000003' " 
		...
		if((typeof(owning_org_id)!="undefined"&&owning_org_id!='') )
		{
		sql=sql+" and b.owning_sub_id like'"+owning_org_id+"%'";
		}
		sql=sql+" group by b.dev_name, b.dev_model, temp.dev_code, b.ifcountry,SUBSTR(b.owning_sub_id,0,7)) t ,comm_org_subjection sub,comm_org_information infom where infom.org_id=sub.org_id and sub.org_subjection_id=sub_id ";
		} else{
		  sql=" select rownum rm, t.*,(use_num + unuse_num + other_num) total_num from (select info.org_abbreviation， b.dev_name, b.dev_model, temp.dev_code, '串' dev_unit, sum(decode(b.using_stat, '0110000007000000001', 1, 0)) as use_num, sum(decode(b.using_stat, '0110000007000000002', 1, 0)) as unuse_num, sum(decode(b.using_stat, '0110000007000000006', 1, 0)) as other_num, b.ifcountry from gms_device_account_b b left join comm_org_information info on info.org_id = b.owning_org_id left join (select ci.device_id, ci.dev_code, ci.dev_name, ci.dev_model, dci.dev_ci_code, dci.dev_ci_name, dci.dev_ci_model from gms_device_collectinfo ci left join gms_device_coll_mapping cm on ci.device_id = cm.device_id left join gms_device_codeinfo dci on cm.dev_ci_code = dci.dev_ci_code) temp on temp.dev_ci_code = b.dev_type where (b.dev_type like 'S14050208%') and info.bsflag = '0' and b.bsflag = '0' and b.account_stat = '0110000013000000003' "; 
		if((typeof(query_dev_name)!="undefined"&&query_dev_name!='')  ){
		sql=sql+" and b.dev_name like'"+query_dev_name+"%'";
		}
		if((typeof(query_dev_type)!="undefined"&&query_dev_type!='')  ){
		...
		if((typeof(owning_org_id)!="undefined"&&owning_org_id!='') )
		{
		sql=sql+" and b.owning_org_id like'"+owning_org_id+"%'";
		}else{
		sql=sql+" and b.owning_org_id like'"+userid+"%'";
		}
		sql=sql+" group by info.org_abbreviation, b.dev_name, b.dev_model, temp.dev_code,b.ifcountry) t ";
		}
    ...
    function loadDataDetail(shuaId){
		...
		//技术状况
		 retObj = jcdpCallService("DevCommInfoSrv", "getJbqInfosForJSZT", "dev_name="+$("#rdo_entity_id_"+shuaId).attr('dev_name')+"&dev_model="+$("#rdo_entity_id_"+shuaId).attr('dev_model')+"&org_abbreviation="+$("#rdo_entity_id_"+shuaId).attr('usage_org'));
