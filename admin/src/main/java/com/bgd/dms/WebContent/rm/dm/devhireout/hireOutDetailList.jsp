      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_hire_out_id_{device_hire_out_id}' name='device_hire_out_id' idinfo='{device_hire_out_id}'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' value='{device_hire_out_id}' id='selectedbox_{device_hire_out_id}' onclick='chooseOne(this)'/>" >选择</td>
					<td class="bt_info_even" autoOrder="1">序号</td>
					<td class="bt_info_even" exp="{device_hire_out_name}">外租设备返还单名称</td>
					<td class="bt_info_odd" exp="{device_hire_out_no}">外租设备返还单号</td>
					<td class="bt_info_even" exp="{out_org_name}">返还单位名称</td>
					<td class="bt_info_odd" exp="{user_name}">经办人</td>
					<td class="bt_info_even" exp="{out_date}">返还时间</td>
			...
			<div id="tab_box" class="tab_box">
				<div id="tab_box_content0" name="tab_box_content0" class="tab_box_content">
					<table id="projectMap" name="projectMap" border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
				      <td  class="inquire_item6">外租设备返还单名称：</td>
				      <td  class="inquire_form6"><input id="device_hire_out_name" class="input_width" type="text"  value="" disabled/> &nbsp;</td>
				      <td  class="inquire_item6">&nbsp;外租设备返还单号：</td>
				      <td  class="inquire_form6"  ><input id="device_hire_out_no" class="input_width" type="text"  value="" disabled/> &nbsp;</td>
				    ...
				    <tr >
				     ...
				     <td  class="inquire_item6">返还时间：</td>
...
$(function(){
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				//先进行查询
				var str = "select t.* from gms_device_hire_out_detail t where t.device_hire_out_id='"+currentid+"'";
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			var innerHTML = "<tr>";
			innerHTML += "<td>"+(i+1)+"</td>";
			innerHTML += "<td>"+datas[i].dev_name+"</td><td>"+datas[i].dev_model+"</td><td>台</td>";
...
<script type="text/javascript">
    ...
    function refreshData(v_device_app_no, v_device_app_name){
		var str = " select t.device_hire_out_id,t.device_hire_out_name,t.device_hire_out_no,org.org_abbreviation as out_org_name,t.out_date,u.user_name,case w.proc_status when '1' then '待审批' when '3' then '审批通过' when '4' then '审批不通过' else '未提交' end as state_desc from gms_device_hire_out t left join comm_org_information org on t.org_id=org.org_id left join p_auth_user u on t.creator_id=u.user_id left join common_busi_wf_middle w on t.device_hire_out_id=w.business_id where t.bsflag='0' and t.org_subjection_id like '<%=orgSubId%>%'";
    ...
    function loadDataDetail(device_hireapp_id){
		...
		if(device_hireapp_id!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getDevHireBackInfo", "devicehirebackid="+device_hireapp_id);
		    ...
		    retObj = jcdpCallService("DevCommInfoSrv", "getDevHireBackInfo", "devicehirebackid="+ids);
		}
		//选中这一条checkbox
		$("#selectedbox_"+retObj.deviceappMap.device_hire_out_id).attr("checked","checked");
		//取消其他选中的
		$("input[type='checkbox'][name='selectedbox'][id!='selectedbox_"+retObj.deviceappMap.device_hire_out_id+"']").removeAttr("checked");
		//给数据回填
		$("#device_hire_out_name","#projectMap").val(retObj.deviceappMap.device_hire_out_name);
		$("#device_hire_out_no","#projectMap").val(retObj.deviceappMap.device_hire_out_no);
		$("#state_desc","#projectMap").val(retObj.deviceappMap.state_desc);
		$("#org_name","#projectMap").val(retObj.deviceappMap.out_org_name);
		$("#employee_name","#projectMap").val(retObj.deviceappMap.user_name);
		$("#appdate","#projectMap").val(retObj.deviceappMap.out_date);
		var device_hire_out_id = retObj.deviceappMap.device_hire_out_id;
		var device_hire_out_name = retObj.deviceappMap.device_hire_out_name;
		var device_hire_out_no = retObj.deviceappMap.device_hire_out_no;
		//重新加载当前标签页信息
		getContentTab(undefined,selectedTagIndex);
		//工作流信息
		var submitdate =getdate();
    	processNecessaryInfo={        							//流程引擎关键信息
			businessTableName:"gms_device_hire_out",    			//置入流程管控的业务表的主表表明
			businessType:"5110000004100000106",    				//业务类型 即为之前设置的业务大类
			businessId:device_hire_out_id,           			//业务主表主键值
			businessInfo:"外租申请审批流程<外租返还单名称:"+device_hire_out_name+";外租申请单号:"+device_hire_out_no+">",
			applicantDate:submitdate,       						//流程发起时间
			deviceHireOutId:device_hire_out_id
		};
		processAppendInfo={ 
			projectName:projectName,									//流程引擎附加临时变量信息
			projectInfoNo:projectInfoNos,
			devicehirebackid:device_hire_out_id
		};
		loadProcessHistoryInfo();
    }
	function toAddHirePlanPage(){
		popWindow('<%=contextPath%>/rm/dm/devhireout/hireOut_new_apply.jsp');
	}
	function toModifyHirePlanPage(){
		...
		//判断状态
		var querySql = "select devapp.device_hire_out_id,nvl(wfmiddle.proc_status,'') as proc_status ";
		querySql += "from gms_device_hire_out devapp left join common_busi_wf_middle wfmiddle on devapp.device_hire_out_id=wfmiddle.business_id ";
		querySql += "where devapp.bsflag='0' and devapp.device_hire_out_id in ("+selectedid+")";
		...
		var devicehirebackid ;
		$("input[type='checkbox'][name='selectedbox']").each(function(){
			if(this.checked){
				devicehirebackid = this.value;
			}
		});
		popWindow('<%=contextPath%>/rm/dm/devhireout/hireOut_modefy.jsp?devicehirebackid='+devicehirebackid);
	}
	function toDelHirePlanPage(){
		...
		//什么状态不能删除，和业务专家确定
		if(confirm("是否执行删除操作?")){
			var sql = "update gms_device_hire_out set bsflag='1' where device_hire_out_id in ("+selectedid+")";
	...
	function toSumbitHireApp(){
		...
		querySql += "from gms_device_hire_out_detail devdet ";
		querySql += "where devdet.bsflag='0' and devdet.device_hire_out_id ="+selectedid+" ";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var basedatas = queryRet.datas;
		var detflag = false;
		for(var index = 0;index<basedatas.length;index++){
