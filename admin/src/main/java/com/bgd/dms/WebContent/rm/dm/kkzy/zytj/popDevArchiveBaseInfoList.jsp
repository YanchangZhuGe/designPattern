		...
		<div id="table_box">
			<table style="width: 98.5%" border="0" cellspacing="0"
				...
				<tr>
					...
					<td class="bt_info_odd" exp="{actual_in_time}">进场时间</td>
					...
					<tr>
						...
						<td class="bt_info_even"><span class="dc"
							style="float: right; margin-top: -4px;"> <a href="#"
								onclick="exportDataDoc('wxjldui')" title="导出excel"></a>
						</span></td>
					</tr>
					<tbody id="assign_body"></tbody>
				</table>
				...
				<table id="djMap" width="250%" border="0" cellspacing="0"
					...
					<tr>
						...
						<td class="bt_info_odd"><span class="dc"
							style="float: right; margin-top: -4px;"> <a href="#"
								onclick="exportDataDoc('zcjjlwxdui')" title="导出excel"></a>
						</span></td>
			...
			<div id="tab_box_content12" class="tab_box_content"
				...
				<table id="djMap" width="250%" border="0" cellspacing="0"
					...
					<tr>
						...
						<td class="bt_info_odd"><span class="dc"
							style="float: right; margin-top: -4px;"> <a href="#"
								onclick="exportDataDoc('zcjghjldui')" title="导出excel"></a>
						</span></td>
					</tr>
					<tbody id="assign_body"></tbody>
				</table>
				...
				<table id="djMap" width="250%" border="0" cellspacing="0"
					...
					<tr>
						...
						<td   align="left"><span class="dc"
							style="float: right; margin-top: -4px;"> <a href="#"
								onclick="exportDataDoc('kkzysglldui')" title="导出excel"></a>
						</span></td>
					</tr>
					<tbody id="assign_body"></tbody>
				</table>
			...
			<div id="tab_box_content11" class="tab_box_content"
				...
				<table id="djMap" width="250%" border="0" cellspacing="0"
					...
					<tr>
						...
						<td class="bt_info_odd"><span class="dc"
							style="float: right; margin-top: -4px;"> <a href="#"
								onclick="exportDataDoc('byjldui')" title="导出excel"></a>
						</span></td>
					</tr>
					<tbody id="assign_body"></tbody>
				</table>
...
<script type="text/javascript">
	...
	function searchDevData(){
		...
		var obj = new Array();
		obj.push({"label":"tt.dev_model","value":v_dev_model});
		obj.push({"label":"tt.self_num","value":v_self_num});
		refreshData(obj);
	...
	function refreshData(arrObj){
		...
		var project_info_id='<%=project_info_id%>';
			...
			" nvl(tt.ifcountry, '国内') as ifcountry_tmp,(select coding_name from comm_coding_sort_detail c where t.tech_stat=c.coding_code_id) as tech_stat_desc,t.*,"+
			" (select pro.project_name from gp_task_project pro where pro.project_info_no=t.project_info_id) as project_name_desc,"+
			...
			" from gms_device_account_dui t inner join (comm_org_subjection s inner join comm_org_information org on s.org_id=org.org_id) on t.owning_sub_id=s.org_subjection_id "+
			" left join comm_org_information i on t.usage_org_id = i.org_id and i.bsflag ='0'"+
			" left join gp_task_project p on t.project_info_id = p.project_info_no and p.bsflag ='0'"+
			...
			" left join gms_device_account tt on tt.dev_acc_id=t.fk_dev_acc_id"+
			" where t.bsflag='0' and t.account_stat!='0110000013000000005' "+
			" and t.owning_sub_id like '"+userid+"%' and t.dev_type like 'S062301%' and t.project_info_id='"+project_info_id+"' ";
		...
		if(country=='国内'){
			str +="  and ( tt.ifcountry is null  or tt.ifcountry='国内') ";
		}else if(country=='国外'){
			str +="  and tt.ifcountry='国外' ";
		}else if(country=='全部'){
    ...
    function loadDataDetail(shuaId){      
    	var retObj;
		if(shuaId!=null){		     
			 retObj = jcdpCallService("DevCommInfoSrv", "getDevAccInfoDui", "deviceId="+shuaId);			
		}else{
		    ...
		    if(ids==''){ 
		     ...
		     retObj = jcdpCallService("DevCommInfoSrv", "getDevAccInfoDui", "deviceId="+ids);
		}
	...
	*/
	function yzjl(shuaId){
		...
		var sum_work_hour="0";
        var querySql = "select pro.project_name, acc.dev_name, acc.asset_coding, info.dev_acc_id, nvl(sum(info.mileage), 0) as mileage_total,"+
          "   nvl(sum(info.drilling_footage), 0) as drilling_footage_total,nvl(sum(info.work_hour), 0) as work_hour_total, max(info.modify_date) as modify_date"+
          "   from gms_device_operation_info info  left join gms_device_archive_detail arc  on arc.dev_archive_refid = info.operation_info_id  left join gms_device_account acc  on acc.dev_acc_id = arc.dev_acc_id  left gms_device_account_dui dui on"+
          "   dui.fk_dev_acc_id=acc.dev_acc_id  left join gp_task_project pro  on arc.project_info_id = pro.project_info_no"+
          "   where arc.dev_archive_type = '1'   and dui.dev_acc_id = '"+shuaId+"'  group by pro.project_name, acc.dev_name, acc.asset_coding, info.dev_acc_id";
 ...
 +"              	    and wx.bsflag = '0'"
	 ...
	 +"                       where usemat_id is not null))"
      +"                and wx.bsflag = '0' and wx.project_info_id is not null and wx.maintenance_level = '无' and t.dev_acc_id = '"+shuaId+"'"
      +"             ) ttt) k"
	...
	*/
	function zcjwx(shuaId){
		...
		var querySql="select wx.create_date, wx.zcj_model,t.dev_name,t.self_num,wx.sequence,(select    t.coding_name  from   comm_coding_sort_detail  t where  t.coding_sort_id='5110000187' and t.coding_code_id=wx.zcj_name) as zcj_name,wx.wx_date,wx.work_hour, d.coding_name as wx_level,wx.wx_content,wx.worker ,wx.performance_desc  ,wx.worker_unit ,wx.bak  from gms_device_zy_zcjwx wx   left join comm_coding_sort_detail d on d.coding_code_id=wx.wx_level left join gms_device_account t on t.dev_acc_id=wx.dev_acc_id "+
				"where wx.bsflag='0' and t.dev_acc_id=(select k.fk_dev_acc_id from gms_device_account_dui    k  where   k.dev_acc_id='"+shuaId+"')  order by create_date desc  ";
	...
	*/
	function zcjgh(shuaId){
		...
		var retObj;
				...
				" and r.project_info_id is not null and w.project_info_id  is not null  and  w.zcj_type  is not null  and dui.dev_acc_id='"+shuaId+"'  "+
				" ) a  on a.zcj_type=code.coding_name  where code.coding_sort_id='5110000187' and  code.coding_code_id  !='5110000187000000015'  order by code.coding_code_id asc ,a.create_date desc";	
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=100000&querySql='+querySql);
			retObj= queryRet.datas;
	...
	*/
	function djxh(){
		...
		var querySql = "select  i.wz_id,i.wz_name,i.wz_prickie,i.wz_price, c.code_name,t.stock_num,t.recyclemat_info,t.actual_price,  mat.use_num,  (select p.project_name from gp_task_project p where p.project_info_no=t.project_info_id) as projectname, ";
		    querySql += " to_char(mat.create_date,'yyyy-MM-dd') as createdate,   (select d.coding_name  from comm_coding_sort_detail d  where d.coding_code_id = mat.coding_code_id) as coding_name  ";
		    querySql += "  from gms_mat_recyclemat_info t       inner join(gms_mat_infomation i       inner join gms_mat_coding_code c on i.coding_code_id =  c.coding_code_id   and i.bsflag = '0'     and c.bsflag = '0') on t.wz_id = i.wz_id   ";
		    querySql += "   left join gms_device_zy_wxbymat mat on mat.wz_id = t.wz_id      where t.bsflag = '0'  and t.project_info_id is not null      and t.wz_type = '3' ";
		    querySql += " and mat.usemat_id in(select wx.usemat_id  from gms_device_zy_bywx wx where   wx.project_info_id=t.project_info_id and  wx.dev_acc_id in(select dui.dev_acc_id from gms_device_account_dui dui where dui.dev_acc_id='"+ids+"') or wx.dev_acc_id ='"+ids+"')	 ";
	
		    if(startDate!=undefined && startDate!=''){
		    	   querySql+=" and mat.create_date>= to_date('"+startDate+"','yyyy-MM-dd')   ";
			}
		    if(endDate!=undefined && endDate!=''){
		    	   querySql+="  and mat.create_date<= to_date('"+endDate+"','yyyy-MM-dd') ";
			}
		    querySql+=" order by  bywx_date desc, mat.create_date desc";
		    var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=100000&querySql='+querySql);
			retObj= queryRet.datas;
			//if (shuaId != null){
				//retObj = jcdpCallService("DevCommInfoSrv", "getDevAccInfoBY", "deviceId=" + shuaId);
	...
	*/
	function byjl(shuaId){
		var retObj;
		 	...
		 	var  querySql = " select * from ( select  wx.create_date,  wx.bywx_date,wx.maintenance_level, wx.maintenance_desc, (select d.coding_name from comm_coding_sort_detail d where d.coding_code_id=wx.zcj_type) as zcj_type,  wx.usemat_id,   p.project_name ,";
