...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>多项目-调配调剂-调配单管理(自有设备)</title> 
 </head> 
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_mixinfo_id_{device_mixinfo_id}' name='device_mixinfo_id'>
			     	<td class="bt_info_even" exp="<input type='checkbox' name='selectedbox' mixstate='{opr_state}' value='{device_app_id}' id='selectedbox_{device_app_id}' />" >选择</td>
					...
					<td class="bt_info_even" exp="<a onclick=viewMixedStore('{device_app_id}','{opr_state}')>查看</a>">调配单查看</td>
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				if(currentid==undefined){
					...
					if(currentid!=''){
						//先进行查询
						var prosql = "select teamsd.coding_name as team_name,appdet.dev_name as dev_ci_name,";
						prosql += "appdet.dev_type as dev_ci_model,unitsd.coding_name as unit_name,";
						prosql += "appdet.apply_num,appdet.plan_start_date,appdet.plan_end_date,appdet.purpose ";
						prosql += "from gms_device_app_detail appdet left join gms_device_codeinfo ci on ci.dev_ci_code = appdet.dev_ci_code ";
						...
						prosql += "where appdet.bsflag = '0' and appdet.device_app_id ='"+currentid+"'";
...
function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			var innerHTML = "<tr>";
			innerHTML += "<td>"+(i+1)+"</td><td>"+datas[i].team_name+"</td><td>"+datas[i].dev_ci_name+"</td>";
			innerHTML += "<td>"+datas[i].dev_ci_model+"</td><td>"+datas[i].unit_name+"</td>";
			innerHTML += "<td>"+datas[i].apply_num+"</td><td>"+datas[i].plan_start_date+"</td><td>"+datas[i].plan_end_date+"</td>";
			innerHTML += "<td>"+datas[i].purpose+"</td></tr>";
...
<script type="text/javascript">
	...
	function exportDataDoc(){
		...
		if(info[0]!=''){
			...
			var submitStr = "baseTableName=gms_device_mixinfo_form&baseid="+info[0]+"&subTableName=gms_device_appmix_detail";
	...
	function searchDevData(){
		...
		refreshData(v_opr_state_desc, v_appinfo_name, v_appinfo_no, v_project_name);
	}
	...
	function refreshData(v_opr_state_desc, v_appinfo_name, v_appinfo_no, v_project_name){
		var str="";
		if(orgType=="Y"){//原综合物化探代码为Y
			str += "select device_app_name,dev1.device_app_id,device_app_no,to_char(appdate, 'yyyy-mm-dd') as appdate,"
			      +"mixorg.org_abbreviation as app_org_name,p2.device_mixinfo_id,p2.mixinfo_no,tp.project_name,"
			      +"mixorg.org_abbreviation as mix_org_name,outorg.org_abbreviation as out_org_name,p2.employee_name," 
			      +"to_char(p2.modifi_date, 'yyyy-mm-dd') as mix_date," 
			      +"case when dev1.appNum = p2.mixed_num then '已处理' when p2.mixed_num = '' or p2.mixed_num is null then '未处理' else '处理中' end as opr_state_desc,"
			      +"case p2.state when '0' then '调配中' when '9' then '已调配' else '待调配' end as mix_state_desc,p2.state " 
			      +"from (select p.device_app_id,l.dev_out_org_id orgId,sum(l.apply_num) as appNum,p.device_app_no,p.project_info_no,"
			      +"p.device_app_name,p.app_org_id,p.appdate,p.mix_org_id,p.mix_type_id,p.opr_state,p.modifi_date,p.bsflag "
	              +"from gms_device_app_detail l, gms_device_app p where l.device_app_id = p.device_app_id and l.dev_out_org_id is not null " 
	              +"group by p.device_app_id,l.dev_out_org_id,p.device_app_no,p.project_info_no,p.device_app_name,p.app_org_id," 
           		  +"p.appdate,p.mix_org_id,p.opr_state,p.bsflag,p.mix_type_id,p.modifi_date) dev1 "
                  +"left join (select * from gms_device_app_detail p1 left join (select (m.device_app_id || m.org_id) idorg,"
                  +"sum(m1.assign_num) mixed_num,m.device_mixinfo_id,m.mixinfo_no,m.state,m.modifi_date,he.employee_name,m.out_org_id from gms_device_appmix_main  m1,gms_device_mixinfo_form m,comm_human_employee he "
                  +"where m1.device_mixinfo_id = m.device_mixinfo_id and m1.bsflag = '0' and m.print_emp_id = he.employee_id "
                  +"group by m.device_app_id || m.org_id,m.device_mixinfo_id,m.mixinfo_no,m.state,m.modifi_date,he.employee_name,m.out_org_id) dv on (p1.device_app_id || p1.dev_out_org_id) = "
                  +"dv.idorg) p2 on (dev1.device_app_id || dev1.orgId) = p2.idorg "
				  +"left join comm_org_subjection sub on dev1.orgId = sub.org_id "
				  +"left join gp_task_project tp on dev1.project_info_no = tp.project_info_no "
				  +"left join(comm_org_subjection sub left join comm_org_information outorg on sub.org_id = outorg.org_id) on p2.out_org_id = sub.org_subjection_id "
				  +"left join comm_org_information mixorg on dev1.mix_org_id = mixorg.org_id "
				 // +"left join common_busi_wf_middle wfmiddle on dev1.device_app_id = wfmiddle.business_id "
				  +"where dev1.bsflag = '0' and dev1.mix_type_id = 'S0000' "
				  +"and sub.org_subjection_id like '<%=subid%>%' "				  

				if(v_appinfo_no!=undefined && v_appinfo_no!=''){
					str += "and dev1.device_app_no like '%"+v_appinfo_no+"%' ";
				}
				if(v_appinfo_name!=undefined && v_appinfo_name!=''){
					str += "and dev1.device_app_name like '%"+v_appinfo_name+"%' ";
				}
				...
				if(v_opr_state_desc!=undefined && v_opr_state_desc!=''){
					if(v_opr_state_desc == '0')//未处理
					{
						str += "and (p2.mixed_num = '' or p2.mixed_num is null) ";
					}else if(v_opr_state_desc == '9')//已处理
					{
						str += "and dev1.appNum = p2.mixed_num ";
					}else    //处理中
					{
						str += "and p2.mixed_num <> dev1.appNum ";
					}					
				}
				...
				str += "order by p2.state nulls first,p2.modifi_date desc,dev1.modifi_date desc,dev1.project_info_no "
			}else{
				var str = "select devapp.project_info_no,devapp.device_app_name,devapp.device_app_id,devapp.device_app_no,to_char(devapp.appdate,'yyyy-mm-dd') as appdate,apporg.org_abbreviation as app_org_name,"
					+"devapp.opr_state,case devapp.opr_state when '1' then '处理中' when '9' then '已处理' when '4' then '已关闭' else '未处理' end as opr_state_desc,"
					+"case devapp.mix_type_id when 'S10002' then devapp.device_app_name else tp.project_name end as project_name "
					...
					+"where wfmiddle.proc_status='3' and devapp.bsflag='0' and (devapp.mix_type_id = 'S10002' or devapp.mix_type_id='<%=DevConstants.MIXTYPE_COMMON%>') ";
					//加机构权限
	...
	function toAddDetailPage(){
		...
		//var info = shuaId.split("~" , -1);
		if(shuaId != ''){
			popWindow('<%=contextPath%>/rm/dm/devmixForm/devMixInfoFormNew.jsp?devappid='+shuaId+'&oprstate='+mixstate,'1050:680');
		}
	}
	...
	function toDelRecord(){
		...
		if(confirm("是否执行关闭操作?")){
			var sql = "update gms_device_app set opr_state='4' where device_app_id = '"+curvalue+"' ";
    ...
    function loadDataDetail(shuaId){
		...
		var str = "select devapp.project_info_no,devapp.device_app_name,devapp.device_app_id,devapp.device_app_no,to_char(devapp.appdate,'yyyy-mm-dd') as appdate,apporg.org_abbreviation as app_org_name,"
			+"tp.project_name,devapp.opr_state,case devapp.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as opr_state_desc "
			+"from gms_device_app devapp "
			//+"left join common_busi_wf_middle wfmiddle on devapp.device_app_id=wfmiddle.business_id "
			+"left join gp_task_project tp on devapp.project_info_no=tp.project_info_no " 
			+"left join comm_org_information apporg on devapp.app_org_id=apporg.org_id "
			+"left join comm_org_subjection orgsub on devapp.mix_org_id=orgsub.org_id and orgsub.bsflag='0' "
			+"where devapp.bsflag='0' and devapp.device_app_id='"+ids+"'";
    ...
    function viewMixedStore(id,state){
		if(state =='1' || state =='9' || state =='4'){
			if(state =='4'){
				...
				querySql += "left join common_busi_wf_middle wfmiddle on devapp.device_app_id = wfmiddle.business_id ";
				querySql += "where wfmiddle.proc_status = '3' and devapp.bsflag = '0' and (mif.bsflag is null or mif.bsflag = '0') and devapp.device_app_id = '"+id+"' ";
			...
			if(id != ''){
				popWindow('<%=contextPath%>/rm/dm/devmixForm/devMixedInfoList.jsp?devAppId='+id,'1050:680');
			}			
		}else{
			alert("该单据尚未调配,暂不能查看调配单信息!");
			return;
