...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.queryStr = "select d.plan_detail_id,d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,nvl(d.people_number,0) people_number,nvl(d.profess_number,0) profess_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date)+1 nums,d.notes from bgp_comm_human_plan_detail d  left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0' left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' where d.spare1 is null  and d.project_info_no='"+projectInfoNo+"' and d.bsflag='0' ";
	...
	function toAdd(){
		...
		popWindow('<%=contextPath%>/rm/em/singleHuman/humanPlan/jz_add_planning.jsp?projectInfoNo=<%=projectInfoNo%>&projectType=<%=projectType%>','1000:800');
	}
	...
	function toEdit() {
		...
		popWindow('<%=contextPath%>/rm/em/singleHuman/humanPlan/jz_add_planning.jsp?projectInfoNo=<%=projectInfoNo%>&projectType=<%=projectType%>&id='+ids,'1000:800');
	}
