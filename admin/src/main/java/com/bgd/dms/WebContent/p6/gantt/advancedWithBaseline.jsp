...
String targetProjectInfoNo = request.getParameter("targetProjectInfoNo");
...
<html>
  <head>
	...
	<!--Scheduler styles-->
    <link href="../css/sch-gantt-all-baselineproject.css" rel="stylesheet" type="text/css" />
    ...
    <!--Application files-->
    <script src="DemoGanttPanelBaseline.js" type="text/javascript"></script>
    <!--rt_base files-->
    ...
    createGantt : function() {
        ...
        var taskStore = Ext.create("Gnt.data.TaskStore", {
            ...
            proxy : {
                ...
                api : {
					read : '<%= contextPath %>/p6/resourceAssignment/getTasksWithBaseline.srq?targetProjectObjectId=<%=targetProjectObjectId %>&targetWbsObjectId=<%=targetWbsObjectId %>&projectObjectId=<%=projectObjectId%>&wbsObjectId=<%=wbsObjectId%>'
				}
            }
        });
        ...
        var dependencyStore = Ext.create("Gnt.data.DependencyStore", {
            ...
            proxy: {
                type : 'ajax',
                url: '<%= contextPath %>/p6/resourceAssignment/getDependencies.srq?projectInfoNo=<%=projectInfoNo%>&projectObjectId=<%=projectObjectId %>&wbsObjectId=<%=wbsObjectId %>',
                method: 'GET',
                reader: {
                    type : 'json'
                }
        ...
        g.on({
        	taskcontextmenu : function(gantt,clickedDate){
	          ...
	          select : function(gantt,clickedDate){
	                ...
	                if(isRoot == "true"){
	                	parent.mainDownframe.location.href = "<%=contextPath %>/p6/projectTask/activityInfoEmpty.jsp";
	                }
	                else{
		          		if(isWbs != "true"){
