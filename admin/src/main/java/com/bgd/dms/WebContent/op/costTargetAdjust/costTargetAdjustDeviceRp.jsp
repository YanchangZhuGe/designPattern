      	...
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <td align="right" style="padding-right: 20px;"><font color="red"><span id="sum_value"></span></font></td>
				<%if(proc_status){ %>
			     <auth:ListButton functionId="OP_ADJUST_MA_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			     <auth:ListButton functionId="OP_ADJUST_MA_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			     <auth:ListButton functionId="OP_ADJUST_MA_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			     <%} %>
			  </tr>
			  ...
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			           ...
			           <!--  <td  class="bt_info_odd">自编号</td>
			            ...
			            <td class="bt_info_even">车辆日消耗材料（元）</td>
			            <td  class="bt_info_odd">钻机日消耗材料（元）</td>
			            <td class="bt_info_even" onclick="getSum(1)">车辆材料费(元)</td>
			            <td  class="bt_info_odd" onclick="getSum(2)">钻机材料费(元)</td>
			            <td  class="bt_info_even" onclick="getSum(3)">恢复性修理费(元)</td>
			        </tr>            
			   </table>
			</div>
			<div id="fenye_box">
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getDeviceAdjustMaterialInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
			...
			var td = tr.insertCell();
			td.innerHTML =datas[i].vehicleDailyMaterial;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].drillingDailyMaterial;
			...
			var td = tr.insertCell();
			td.innerHTML =  datas[i].restoreRepails;
		} 
	}	
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetAdjust/costTargetAdjustDeviceRpEdit.jsp");
}
function toModify(){
	...
	popWindow(cruConfig.contextPath+"/op/costTargetAdjust/costTargetAdjustDeviceRpEdit.jsp?target_material_id="+ids);
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	ids = ids.replace(/\,/g,"','");
	var sql = "update bgp_op_tartet_device_material t set t.if_delete_change='1' where t.target_material_id in('"+ids+"')";
