...
<div style="overflow-y: hidden; overflow-x: hidden;">
	...
	function getTotal(){  //and p.project_status='3'  正在施工
		...
		" round((case when tt.total is null then 0 when tt.total = 0 then 0 else tt.first/tt.total*100 end),2) first_percent "+
		...
		" sum(nvl(rr.daily_acquire_firstlevel_num,0)-(-nvl(rr.collect_2_class,0))-(-nvl(rr.collect_waster_num,0))-(-nvl(rr.collect_miss_num,0)))total  "+ 
		...
		if(retObj!=null && retObj.returnCode=='0'){
			if(retObj.datas != null && retObj.datas.length>0){
				for(var i =0 ;i<retObj.datas.length ;i++){
					...
					p = p +"<p class='small' ><font color='blue'>"+ eps_name + "</font>(一级品率:<span >"+first_percent+"</span>%;&nbsp;累计完成炮数:<span >"+total+"</span>;&nbsp;累计完成一级品炮数:<span >"+first+"</span>);";
				}
			}else{
				querySql = "select t.eps_id ,t.eps_name ,t.org_id , sub.org_subjection_id ,'0' first, '0' total ,'0' first_percent from bgp_comm_org_wtc t "+
				...
				if(retObj.datas != null && retObj.datas.length>0){
					for(var i =0 ;i<retObj.datas.length ;i++){
						...
						var total = data.total;
						var eps_name = data.eps_name;
						p = p +"<p class='small' ><font color='blue'>"+ eps_name + "</font>(一级品指标≥<span >"+first_percent+"</span>%;&nbsp;累计完成炮数:<span >"+total+"</span>;&nbsp;累计完成一级品炮数:<span >"+first+"</span>);";
					}
				}
			}
		}
	...
	function projectFirst(org_subjection_id,org_name){
		...
		window.open('<%=contextPath%>/qua/mProject/analysis/company/first_project.jsp?org_subjection_id='+org_subjection_id+'&org_name='+encodeURI(org_name));
	}
	</script>
    </body>
</html>