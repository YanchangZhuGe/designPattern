			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<%if(proc_status){ %>
						 	<auth:ListButton functionId="OP_ACTUAL_PER_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
						    <auth:ListButton functionId="OP_ACTUAL_PER_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
						    <auth:ListButton functionId="OP_ACTUAL_PER_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
						    <%} %>
						</tr>
					</table>
				</td>
	...
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			<tr>
			  ...
			  <td class="bt_info_even" exp="{apply_team}">班组</td>
			  <td class="bt_info_odd" exp="{start_loc}">起运地点</td>
			  <td class="bt_info_even" exp="{end_loc}">到达地点</td>
			  <td class="bt_info_odd" exp="{person_money}">单价（元）</td>
			  <td class="bt_info_even" exp="{other_money}">其他费用（元）</td>
			  <td class="bt_info_odd" exp="{total_money}" onclick="getSum()">总计(元)</td>
			</tr>
		</table>
	</div> 
	<div id="fenye_box">
...
<script type="text/javascript">
	...
	function refreshData(){
		cruConfig.queryStr = "select t.*,to_char(nvl(t.person_num,0)*nvl(t.person_money,0)+nvl(other_money,0),'9999999999999999.00') total_money from BGP_OP_TARGET_DEVICE_HUM t where bsflag='0' and t.if_change ='2' and t.project_info_no ='"+project+"'";
		setTabBoxHeight();
		queryData(1);
	}
	refreshData();
	...
	function toAdd(){
		popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualDeviceHEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
	}
	function toModify(){
		...
		if (ids == '') {
			...
			popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualDeviceHEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
	}
	function toDelete(){
		ids = getSelIds('rdo_entity_id');
		if (ids == '') {
	...
	$(document).ready(lashen);
	 ...
	 function getSum(){
			...
			var querySql = "select sum(nvl(to_char(nvl(t.person_num,0)*nvl(t.person_money,0)-(-nvl(other_money,0)),'9999999999999999.00'),0)) sum_value"+
				" from bgp_op_target_device_hum t where t.project_info_no='<%=projectInfoNo%>' and t.if_change ='2' and t.bsflag='0'";
