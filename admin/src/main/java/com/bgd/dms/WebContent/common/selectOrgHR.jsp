...
function getSubNodes(parentNode) {
	...
	if("<%=select%>"=="employeeid" || "<%=select%>"=="userid"){
		...
		if("<%=select%>"=="employeeid" ){
			querySql = "Select e.employee_id as id,e.employee_name as name FROM comm_human_employee e WHERE e.bsflag='0' and e.org_id='"+info[1]+"'  order by e.employee_name  "
		}else{
			querySql = "Select u.user_id as id,u.user_name as name FROM p_auth_user u WHERE u.bsflag='0' and u.org_id='"+info[1]+"'  order by u.user_name "
		}
