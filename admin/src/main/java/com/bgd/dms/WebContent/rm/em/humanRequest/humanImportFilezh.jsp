...
cruConfig.contextPath='<%=contextPath %>';
	...
	function returnFile(){
		...
		if(content != ""){
			...
			content = encodeURI(content);
			var str = "checked="+content+"&isSet=1&project_type=<%=project_type%>";
 
				//处理综合物化探业务
				var humanList = jcdpCallService("HumanRequiredSrv","queryImportHumanListZh",str);	 
				...
				if(humanList.peopleList!=null){
					for(var i=0;i<humanList.peopleList.length;i++){
						...
						returncheck += templateMap.employee_id+'@'+templateMap.employee_name +'@'+templateMap.team+ "@" +templateMap.work_post+"@"+templateMap.start_date+"@"+templateMap.end_date+"@"+templateMap.qufen+",";			
					}		
				}
		}
...
<body>
<form action="<%=contextPath%>/rm/em/humanRequest/humanImportFile_zht.jsp" id="fileForm" method="post" enctype="multipart/form-data" target="targetIframe">
