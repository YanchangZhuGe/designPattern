			...
			<td background="<%=contextPath%>/images/list_15.png">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
					    ...
					    <%if(proc_status){ %>
					    <auth:ListButton functionId="OP_TARGET_RENT_EDIT" css="zj" event="onclick='toAdd()'" functionId="OP_TARGET_RENT_EDIT"  title="JCDP_btn_add"></auth:ListButton>
					    <auth:ListButton functionId="OP_TARGET_RENT_EDIT" css="xg" event="onclick='toEdit()'"  functionId="OP_TARGET_RENT_EDIT"  title="JCDP_btn_edit"></auth:ListButton>
					    <auth:ListButton functionId="OP_TARGET_RENT_EDIT" css="sc" event="onclick='toDelete()'"  functionId="OP_TARGET_RENT_EDIT"  title="JCDP_btn_delete"></auth:ListButton>
					    <auth:ListButton functionId="OP_TARGET_RENT_EDIT" css="bc" event="onclick='saveDatas()'"  functionId="OP_TARGET_RENT_EDIT"  title="JCDP_save"></auth:ListButton>
					    <auth:ListButton functionId="OP_TARGET_RENT_EDIT" css="dr" event="onclick='importDeviceInfoFromDm()'"  functionId="OP_TARGET_RENT_EDIT"  title="JCDP_btn_import"></auth:ListButton>
					    <%} %>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.queryStr = "select t.*,d.coding_name dev_team1,t.device_count*(t.plan_end_date-t.plan_start_date+1)*t.taxi_unit sum_money, " +
		...
		" where t.bsflag ='0' and t.if_change ='0' and t.project_info_no ='<%=projectInfoNo%>' "+
		" and t.dev_team like'%"+dev_team+"%' and t.dev_name like '%"+dev_name+"%' ";
		if(dev_model!=null && dev_model!=''){
			cruConfig.queryStr +=" and t.dev_model like '%"+dev_model+"%'";
		}
		queryData(1);
	}
	refreshData();
	...
	function toAdd(){
	  	popWindow('<%=contextPath%>/op/costTargetShare/costTargetShareDevRentEdit.jsp');
	}
	...
	function toEdit(){
		...
		popWindow('<%=contextPath%>/op/costTargetShare/costTargetShareDevRentEdit.jsp?target_rent_id='+ids);
	}
