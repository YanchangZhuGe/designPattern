...
<script type="text/javascript">
	...
	function refreshData(typeId ,typeName ,divisory_type){
		...
		if(typeId==null || typeId==''){
	    	...
	    	" where t.bsflag='0' and t.company_type ='2' order by t.create_date desc";
		}else{
			cruConfig.queryStr = "select t.company_id ,t.company_name ,t.company_short_name ,t.company_place ,t1.type_short_name company_place_name ,t.parent_company_id ,t2.type_short_name parent_company_name ,t.company_stork ,"+
			" t.key_name ,t.key_position ,t.key_telephone ,t.key_email ,c1.contract_num ,c2.contact_num ,c3.bid_total ,c4.bid_num ,c4.bid_num/c3.bid_total bid_pecent from bgp_market_oil_company t " +
			" left join (select distinct c.company_id ,count(c.company_id) contract_num "+
			...
			" where bid.bsflag='0' and bid.eva_result ='5' group by bid.owner) c4 on t.company_id = c4.owner"+
			" left join bgp_market_company_type t1 on t.company_place = t1.type_id and t1.bsflag='0' left join bgp_market_company_type t2 on t.parent_company_id = t2.type_id and t2.bsflag='0' "+
			" where t.bsflag='0' and t.company_type ='2' and (t.company_place ='"+typeId+"' or t.parent_company_id ='"+typeId+"') order by t.create_date desc ";
		}
		cruConfig.currentPageUrl = "<%=contextPath%>/market/crm/corrivalInformation/clientList.jsp";
		queryData(1);
		frameSize();
		resizeNewTitleTable();
	}
	...
	function loadDataDetail(id){
    	...
    	document.getElementById("contract").src = "<%=contextPath%>/market/crm/corrivalInformation/contractList.jsp?company_id="+id;
    	document.getElementById("contact").src = "<%=contextPath%>/market/crm/corrivalInformation/contactList.jsp?company_id="+id;
    	<%-- document.getElementById("bid").src = "<%=contextPath%>/market/crm/corrivalInformation/bidList.jsp?company_id="+id; --%>
	...
	function toAdd() { 
		...
		popWindow("<%=contextPath%>/market/crm/corrivalInformation/clientEdit.jsp?type_id="+type_id+"&type_name="+type_name+"&divisory_type="+divisory_type);
	}
	function toEdit() {
		...
		if(company_id!=null && window.confirm(text)){
			...
			var divisory_type = document.getElementById("divisory_type").value;
			popWindow("<%=contextPath%>/market/crm/corrivalInformation/clientEdit.jsp?company_id="+company_id+"&type_id="+type_id+"&type_name="+type_name+"&divisory_type="+divisory_type);
			return;
		}
		alert("请选择修改的记录!");
	}
