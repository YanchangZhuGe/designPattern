      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <auth:ListButton functionId="SCRWS_01" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			    <auth:ListButton functionId="SCRWS_02" css="xg" event="onclick='toEdit()'" title="JCDP_btn_edit"></auth:ListButton>
			    <auth:ListButton functionId="SCRWS_03" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
...
//  cruConfig.queryRetTable_id = "";
	...
	function refreshData(filter){
		...
		cruConfig.queryStr = "select object_id,name, work_type,case work_type when '1' then '测量' when '2' then '表层测量(微测井)' when '3' then '表层调查(小折射)' when '4' then '钻井' when '5' then '放线' when '6' then '爆炸班' when '7' then '震源' when '8' then '仪器' when '9' then '气枪'else '' end as work_type_name, to_char(create_date,'yyyy-MM-dd') as create_date, to_char(produce_date,'yyyy-MM-dd') as produce_date, audit_status, is_signed, ucm_id from bgp_ops_work_report where bsflag='0' and project_info_no='"+projectInfoNo+"'" + s_filter +"order by produce_date asc";
		queryData(1);
	}
