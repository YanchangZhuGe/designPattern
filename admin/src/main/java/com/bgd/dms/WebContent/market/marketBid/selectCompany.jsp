...
Ext.onReady(function() {
    ...
    var store = Ext.create('Ext.data.TreeStore', {
        ...
        proxy: {
       	 type : 'ajax',
            method: 'get',
            url: '<%=contextPath%>/crm/common/getOilCompanyTree.srq?divisory_type=2&key_id=<%=key_id%>',
            reader: {
                type : 'json'
            }
        },
    ...
    var grid = Ext.create('Ext.tree.Panel', {
    	id: 'gridId',
        title: '',
        width: 398,//document.body.clientWidth*0.8,
    ...
    function cellclick(grid,colspan, colIndex, rowdata, rowspan,rowIndex,e) {
        ...
        if(type_id!=null ){
        	...
        	parent.mainRightframe.refreshData(type_id, type_short_name );
        }
    }
    ...
    function celldblclick(grid,colspan, colIndex, rowdata, rowspan,rowIndex,e) {
		...
		var type_name = rowdata.data.name;
		var type_short_name = rowdata.data.short_name;
		var is_company = rowdata.data.is_company;
       	obj.fkValue = type_id;
       	obj.name = type_name;
       	obj.short_name = type_short_name;
       	if(is_company!=null && is_company=='true'){
       		window.close();
       	}
    }
});
