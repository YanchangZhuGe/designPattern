...
function save(){
	...
	form.action = "<%=contextPath%>/td/ws/toSavetestInsrumentDoc.srq?businessType=<%=businessType%>";
	form.submit();
	newClose();
}

function checkForm(){
	if (!isTextPropertyNotNull("title", "仪器名称")) return false;	
	if (!isLimitB100("title","仪器名称")) return false;  
	if (!isLimitB20("test_address","测试地点")) return false; 
	if (!isLimitB20("writer","测试人")) return false;
	return true;
}
...
function page_init(){
	...
	if(fileId!='null'){
		...
		if(datas!=null && datas.length>0){
			...
			document.getElementById("test_address").value = datas[0].test_address;
			//测试类型
			var sel = document.getElementById("test_type").options;
			var value = datas[0].test_type;
			for(var i=0;i<sel.length;i++)
			{
			    if(value==sel[i].value)
			    {
			       document.getElementById('test_type').options[i].selected=true;
			    }
			}
					...
					<div id="tab_box" class="tab_box">
						<div id="tab_box_content0" class="tab_box_content">
							<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
								...
								<tr>
									<td class="inquire_item4"><span class="red_star">*</span>仪器名称：</td>
								...
								<tr>
									<td class="inquire_item4">测试人：</td>
									<td class="inquire_form4"><input type="text" id="writer" name="writer" value="" class="input_width"/></td>
									<td class="inquire_item4">测试类型：</td>
									<td class="inquire_form4">
								    	<select class="select_width" name="test_type" id="test_type">
								    		<option value=''></option>
											<option value='年检测'>年检测</option>
											<option value='月检测'>月检测</option>
											<option value='检波器测试'>检波器测试</option>
											<option value='极性测试'>极性测试</option>
											<option value='极性测试'>极性测试</option>
										</select>
									</td>
								</tr>	
								<tr>
									<td class="inquire_item4">测试地点：</td>
									<td class="inquire_form4"><input type="text" id="test_address" name="test_address" value="" class="input_width" /></td>
									<td class="inquire_item4">测试日期：</td>
									<td class="inquire_form4">
								...
								<tr>
									<td class="inquire_item4">测试报告：</td>
									<td colspan="2">
										<input type="hidden" id="pk_0110000061000000043" name="pk_0110000061000000043" value=""/>
										<input type="file" name="0110000061000000043" id="0110000061000000043" class="input_width"/>
					    				<div id="down_0110000061000000043"></div>
									</td>
									<td class="inquire_form4"></td>
								</tr>
								<tr>
