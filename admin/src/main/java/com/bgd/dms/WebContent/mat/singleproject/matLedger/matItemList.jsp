      	...
      	<div id="list_table">
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">
			    ...
			    <li id="tag3_8"><a href="#" onclick="getTab3(8)">转出信息</a></li>
			...
			<div id="tab_box" class="tab_box">
				<div id="tab_box_content0" class="tab_box_content">
				    ...
				    <tr>
				      ...
				      <td   class="inquire_form6" ><input id="wz_name" class="input_width_no_color" type="text" value="" readonly/> &nbsp;</td>
				      <td  class="inquire_item6">&nbsp;计量单位：</td>
				      <td  class="inquire_form6"  ><input id="wz_prickie" class="input_width_no_color" type="text"  value="" readonly/> &nbsp;</td>
				      <td  class="inquire_item6">&nbsp;物资描述：</td>
				      <td  class="inquire_form6"  ><input id="describe" class="input_width_no_color" type="text"  value="" readonly/> &nbsp;</td>
				     </tr>
				     <tr >
				     <td  class="inquire_item6">物资分类：</td>
				     <td  class="inquire_form6"><input id="coding_code_id" class="input_width_no_color" type="text"  value="" readonly/> &nbsp;</td> 
				     <td  class="inquire_item6">&nbsp;参考单价：</td>
				     <td  class="inquire_form6"><input id="wz_price" class="input_width_no_color" type="text"  value="" readonly/> &nbsp;</td>  
				      <td  class="inquire_item6">&nbsp;库存数量：</td>
				     <td  class="inquire_form6"><input id="stock_num" class="input_width_no_color" type="text"  value="" readonly/> &nbsp;</td>  
				    </tr>
				  </table>
				</div>
				...
				<div id="tab_box_content2" class="tab_box_content" style="display:none;">
				   <table id="projectMaprk" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height"  >
			    	<tr>
			    		<td class="bt_info_odd">序号</td>
				...
				<div id="tab_box_content4" class="tab_box_content" style="display:none;">
				   <table id="projectMaptk" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height"  >
			    	<tr>
			    	   	<td class="bt_info_odd">序号</td>
			    	  	<td  class="bt_info_even">物资编码</td>
			            <td  class="bt_info_odd">物资名称</td>
				...
				<div id="tab_box_content8" class="tab_box_content" style="display:none;">
					<table  id="planDetail"border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" > 
				    <tr>
			    		<td class="bt_info_odd">序号</td>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		if(codeId !=null && codeId != rootId){
			//sql +="select g.wz_id ,g.coding_code_id,g.wz_name,g.wz_prickie,g.wz_code,g.wz_price,g.note,w.stock_num from gms_mat_infomation g inner join gms_mat_teammat_info w on g.wz_id = w.wz_id and w.bsflag ='0' and g.coding_code_id like'"+codeId+"%'where w.project_info_no='<%=projectInfoNo%>'and w.is_recyclemat='1'order by g.coding_code_id asc,g.wz_id asc";
			...
			sql +="select tid.wz_id,sum(tid.mat_num) mat_num from gms_mat_teammat_invoices mti inner join gms_mat_teammat_info_detail tid on mti.invoices_id=tid.invoices_id and tid.bsflag='0' where mti.bsflag='0'  and mti.invoices_type <> '2' and mti.invoices_type<>'1' and mti.project_info_no='<%=projectInfoNo%>' and mti.if_input='0' and mti.invoices_type<>'2' group by tid.wz_id ";
			sql +=")aa left join ( ";
			sql +="select tod.wz_id,sum(tod.mat_num) out_num from gms_mat_teammat_out mto inner join gms_mat_teammat_out_detail tod on mto.teammat_out_id=tod.teammat_out_id and tod.bsflag='0' where mto.bsflag='0' and mto.wz_type = '0' and mto.project_info_no='<%=projectInfoNo%>' group by tod.wz_id ";
			...
			sql +=" order by i.coding_code_id asc,aa.wz_id asc";
		}
		else{
			//sql +="select g.wz_id ,g.coding_code_id,g.wz_name,g.wz_prickie,g.wz_code,g.wz_price,g.note,w.stock_num from gms_mat_infomation g inner join gms_mat_teammat_info w on g.wz_id = w.wz_id and w.bsflag ='0'where w.project_info_no='<%=projectInfoNo%>'and w.is_recyclemat='1' order by g.coding_code_id asc,g.wz_id asc";
			sql +="select aa.wz_id,(aa.mat_num- case when bb.out_num is null then 0 else bb.out_num end) stock_num,i.coding_code_id,i.wz_name,i.wz_prickie,i.note,i.wz_price from ( ";
			sql +="select tid.wz_id,sum(tid.mat_num) mat_num from gms_mat_teammat_invoices mti inner join gms_mat_teammat_info_detail tid on mti.invoices_id=tid.invoices_id and tid.bsflag='0' where mti.bsflag='0' and mti.invoices_type <> '2' and mti.invoices_type<>'1' and mti.project_info_no='<%=projectInfoNo%>' and mti.if_input='0' and mti.invoices_type<>'2' group by tid.wz_id ";
			sql +=")aa left join ( ";
			sql +="select tod.wz_id,sum(tod.mat_num) out_num from gms_mat_teammat_out mto inner join gms_mat_teammat_out_detail tod on mto.teammat_out_id=tod.teammat_out_id and tod.bsflag='0' where mto.bsflag='0'  and mto.wz_type = '0' and mto.project_info_no='<%=projectInfoNo%>' group by tod.wz_id ";
	...
	function loadDataDetail(shuaId){
		...
		if(shuaId!= undefined){
		    ...
		    if(ids==''){ 
		     ...
		     retObj = jcdpCallService("MatItemSrv", "getSingleMatLedger", "laborId="+ids);
		     taskShow(ids);
		     detailRKShow(ids);
		     detailTKShow(ids);
		     detailZCShow(ids);
	  ...
	  function toAdd(){ 
       ...
       //出库明细
    	 function taskShow(value){
    			...
    			var retObj = jcdpCallService("MatItemSrv", "getTeamListXH", "wz_type=0&ids="+value);
    			...
    			for(var i =0; taskList!=null && i < taskList.length; i++){
    				...
    				var outmatDate = taskList[i].outmatDate;
    	 ...
    	 //入库明细
    	 function detailRKShow(value){
    			for(var j =1;j <document.getElementById("projectMaprk")!=null && j < document.getElementById("projectMaprk").rows.length ;){
    				document.getElementById("projectMaprk").deleteRow(j);
    			}
    			var retObj = jcdpCallService("MatItemSrv", "getTeamListRK", "ids="+value);
    			var taskList = retObj.matInfo;
    			for(var i =0; taskList!=null && i < taskList.length; i++){
    				var invoicesNo = taskList[i].invoicesNo;
    				var matNum = taskList[i].matNum;
    				var actualPrice = taskList[i].actualPrice; 
    				var totalMoney = taskList[i].totalMoney;
    				var warehouseNumber = taskList[i].warehouseNumber;
    				var goodsAllocation = taskList[i].goodsAllocation;
    				var procureNo = taskList[i].procureNo;
    				var inputType = taskList[i].inputType;
    				var inputDate = taskList[i].inputDate;
    				var matSourth = taskList[i].matSourth;
    				var autoOrder = document.getElementById("projectMaprk").rows.length;
    				var newTR = document.getElementById("projectMaprk").insertRow(autoOrder);
    				var tdClass = 'even';
    				if(autoOrder%2==0){
    					tdClass = 'odd';
    				}
		        ...
		        td.className = tdClass+'_odd';
  		        if(autoOrder%2==0){
    		        ...
    		        newTR.onclick = function(){
    		        	// 取消之前高亮的行
    		       		for(var i=1;i<document.getElementById("projectMaprk").rows.length;i++){
    		    			var oldTr = document.getElementById("projectMaprk").rows[i];
    		    			var cells = oldTr.cells;
    		    			for(var j=0;j<cells.length;j++){
    		    				cells[j].style.background="#96baf6";
    		    				// 设置列样式
     	...
     	//退库明细
     	 function detailTKShow(value){
     			for(var j =1;j <document.getElementById("projectMaptk")!=null && j < document.getElementById("projectMaptk").rows.length ;){
     				document.getElementById("projectMaptk").deleteRow(j);
     			}
     			var retObj = jcdpCallService("MatItemSrv", "getTeamListTK", "wz_type=0&ids="+value);
     			var taskList = retObj.matInfo;
     			for(var i =0; taskList!=null && i < taskList.length; i++){
     				var wzName = taskList[i].wzName;
     				var wzId = taskList[i].wzId;
     				...
     				var matNum = taskList[i].outNum;
     				var totalMoney = taskList[i].totalMoney;
     				var autoOrder = document.getElementById("projectMaptk").rows.length;
     				var newTR = document.getElementById("projectMaptk").insertRow(autoOrder);
     				var tdClass = 'even';
     				if(autoOrder%2==0){
     					tdClass = 'odd';
     				}
    	 ...
    	 function detailZCShow(value){
    			...
    			var retObj = jcdpCallService("MatItemSrv", "getTeamListZC", "ids="+value);
    			var taskList = retObj.matInfo;
    			for(var i =0; taskList!=null && i < taskList.length; i++){
    				var wzName = taskList[i].wzName;
    				var wzId = taskList[i].wzId;
    				...
    				var outDate = taskList[i].outDate;
