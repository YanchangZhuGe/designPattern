      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td class="inquire_form4" colspan="3">
          	...
          	<input name="device_hireapp_id" id="device_hireapp_id" type="hidden" value="<%=devicehireappid%>" />
          </td>
        </tr>
        <tr>
...
<script type="text/javascript">
	function saveInfo(){
		...
		//调配申请保存
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveHireAppBaseInfo.srq?state=0&mixtypename="+mixtypename;
	...
	function refreshData(){
		...
		if('<%=devicehireappid%>'!=null){
			//查询基本信息
			var querySql = "select pro.project_name,devapp.device_hireapp_id,devapp.device_hireapp_no,devapp.device_hireapp_name,devapp.project_info_no,allapp.appdate as allappdate,";
				querySql += "devapp.org_id,devapp.employee_id,devapp.appdate,devapp.create_date,devapp.modifi_date,devapp.mix_type_id,allapp.device_allapp_id,";
				...
				querySql += "  ,   devapp.mix_type_name  ";	
				querySql += "from gms_device_hireapp devapp ";
				querySql += "left join common_busi_wf_middle wfmiddle on wfmiddle.business_id = devapp.device_hireapp_id  ";
		...
		if(basedatas.length>=1){
			...
			$("#device_hireapp_name").val(basedatas[0].device_hireapp_name);
			$("#device_hireapp_no").val(basedatas[0].device_hireapp_no);
			...
			if(retObj!=undefined && retObj.length>=1){
				...
				for(var index=0;index<retObj.length;index++){
					if(retObj[index].coding_code_id==basedatas[0].mix_type_id)
						{
						optionhtml +=  "<option selected name='wzlx' id='wzlx"+index+"' value='"+retObj[index].coding_code_id+"'>"+retObj[index].coding_name+"</option>";
						}
					else
						{
