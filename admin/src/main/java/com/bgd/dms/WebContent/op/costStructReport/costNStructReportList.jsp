...
<script type="text/javascript">
	...
	function refreshData(){
		cruConfig.queryStr = "select t.*,decode(t.report_type,'0','经营状况预计汇总','1','主要设备及利用情况统计分析表','2','辅助生产单位经营状况表','3','机关及附属单位费用支出表','4','非施工期费用支出表','') report_type_name,he.employee_name from BGP_OP_NSTRUCT_REPORT t  left outer join comm_human_employee he on t.creator=he.employee_id where t.bsflag='0' and report_type!='5' ";
	...
	function toView(){
		...
		popWindow('<%=contextPath%>/op/costStructReport/costNStrutReportEdit.upmd?pagerAction=edit2View&id='+ids);
	}
	function toAdd(){
		popWindow('<%=contextPath%>/op/costStructReport/costNStrutReportEdit.upmd?pagerAction=edit2Add&userName=<%=userName%>');
	}
	...
	function toEdit() {
		...
		popWindow('<%=contextPath%>/op/costStructReport/costNStrutReportEdit.upmd?pagerAction=edit2Edit&userName=<%=userName%>&id='+ids);
	}
	function toDelete(){
		...
		var sql = "update bgp_op_nstruct_report t set t.bsflag='1' where t.NSTRUCT_REPORT_ID ='"+ids+"'";
