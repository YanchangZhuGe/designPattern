							...
							<tr>
								...
								<auth:ListButton functionId="" css="zj"
									event="onclick='toAddDevRepairPage()'" title="新增"></auth:ListButton>
									 -->
								<auth:ListButton functionId="" css="wxdj"
									event="onclick='toRegisterResult()'" title="登记"></auth:ListButton>
								<!-- 
			...
			<table style="width: 98.5%" border="0" cellspacing="0"
				...
				<tr id='dispatch_id_{dispatch_id}' name='dispatch_id' idinfo='{dispatch_id}'>
					<td class="bt_info_odd"
						exp="<input type='checkbox' name='selectedbox' subid='{subid}' value='{dispatch_id}' id='selectedbox_{dispatch_id}' onclick='chooseOne(this)'/>">选择</td>
...
<script type="text/javascript">
    ...
    function loadDataDetail(id){
    	var retObj;
		if(id!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getDispatchResultDetailInfo", "id="+id);
		    ...
		    retObj = jcdpCallService("DevCommInfoSrv", "getDispatchResultDetailInfo", "id="+ids);
		}
		//选中这一条checkbox
	...
	function toRegisterResult(){
		...
		//判断状态
		var querySql = 
			"SELECT count(*) as count FROM GMS_DEVICE_COL_REP_DETAIL T\n" +
			"WHERE t.bsflag = '0' and  T.rep_form_det_id ='"+subid+"' ";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var basedatas = queryRet.datas;
		if(basedatas[0].count > 0 ){
			alertinfo = "您选择的记录已经返还,无法修改!";
			alert(alertinfo);
			return;
		}
		popWindow('<%=contextPath%>/rm/dm/devRepair/repairResultRegister.jsp?id='+id,'800:700');
	}
	...
	function toReceiveOrReject(val){
		...
		var warn = '';
		if(val == 3){
			status = <%=DevConstants.DEV_REPAIR_RECIEVESTATE2 %>;//已派工拒接收
			warn = "确定要拒收吗?";
		}else if(val == 2){
			status = <%=DevConstants.DEV_REPAIR_RECIEVESTATE1 %>;//已派工已接收
			warn = "确定要接收吗?";
		}
