...
var jcdp_codes = new Array(
  	...
  	function toEdit(){
	    ...
	    if(subflag == '0'){
	    	...
	    	window.location = "<%=contextPath%>/pm/wr/income/orgInsert.jsp?orgId=<%=orgId%>&week_date="+week_date+"&week_end_date="+week_end_date+"&action=edit";
	    	}
	    }else{
	    	alert("该记录已经提交，不能修改!"); 
	    	return; 
...
function toAdd(){
	if(orgId=="C6000000000025"){
	window.location='<%=contextPath%>/pm/wr/income/martSelectWeek.jsp';
	
	}else{
	window.location='<%=contextPath%>/pm/wr/income/orgInsert.jsp?orgId=<%=orgId%>&action=add';
	}
}
...
<table border="0"  cellspacing="0"  cellpadding="0"  class="form_info"  width="100%" id="queryRetTable">
	...
	<tr class="bt_info">
		...
		<%}else{%>
		<td class="tableHeader" 	 exp="<a href=javascript:link2self('<%=contextPath%>/pm/wr/income/orgInsert.jsp?orgId=<%=orgId%>&week_date={week_date}&week_end_date={week_end_date}&action=view')><font color='3366ff'>{week_date}</font></a>">周报开始日期</td>
		<%} %>
