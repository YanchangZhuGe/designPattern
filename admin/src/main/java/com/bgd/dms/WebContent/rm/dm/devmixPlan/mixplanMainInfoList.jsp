...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>单项目-调配计划申请-调配申请(单台)</title>
 </head> 
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				str += "p6.name as jobname,pro.project_name,appdet.dev_name as dev_ci_name,";
				str += "appdet.dev_type as dev_ci_model,appdet.apply_num,appdet.teamid,appdet.team, ";
				str += "appdet.purpose,appdet.employee_id,emp.employee_name,appdet.plan_start_date,appdet.plan_end_date ";
...
<script type="text/javascript">
	...
	if(projectType == "5000100004000000009"){//综合物化探
    ...
    function refreshData(v_device_app_no, v_device_app_name, v_mix_type_id){
		var str = "select pro.project_name,devapp.device_app_id,devapp.device_app_no,devapp.device_app_name,devapp.project_info_no,";
			...
			str += "case devapp.mix_type_id when 'S0000' then '物探处自有设备' when 'S0623' then '专业化震源' when 'S1404' then '专业化测量' when 'S1405' then '专业化仪器' else '未知类别' end as mix_type_name,";
			...
			if(projectType == "5000100004000000009"){				
				...
				str += "and allwfmiddle.business_type = '"+businessType+"' "
	        }else{
			...
			str += " and allwfmiddle.proc_status='3' and devapp.mix_type_id != 'S14050208' and devapp.mix_type_id != 'S14059999' ";
		...
		str += "order by wfmiddle.proc_status nulls first,devapp.modifi_date,devapp.mix_type_id desc ";
    ...
    function loadDataDetail(device_app_id){
		...
		if(device_app_id!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getMixAppBaseInfo", "deviceappid="+device_app_id+"&business_type="+businessType);
		    ...
		    retObj = jcdpCallService("DevCommInfoSrv", "getMixAppBaseInfo", "deviceappid="+ids+"&business_type="+businessType);
		}
		//选中这一条checkbox
	...
	function toAddMixPlanPage(){
		if('<%=projectInfoNo%>'!=null){
			...
			if(projectType == "5000100004000000009"){//综合物化探
				...
				querySql += "and wfmiddle.business_type = '"+businessType+"' "
	        }else{
		...
		if(basedatas.length == 0){
			...
			popWindow('<%=contextPath%>/rm/dm/devmixPlan/mixplan_new.jsp?projectInfoNo=<%=projectInfoNo%>');
		}
	}
	...
	function toModifyDetail(obj){
		...
		if(sonFlag == 'Y'){
			window.location.href='<%=contextPath%>/rm/dm/devmixPlan/mixPlanDetailList.jsp?projectInfoNo='+retFatherNo+'&deviceappid='+idinfo+'&sonFlag='+sonFlag;
		}else{
			window.location.href='<%=contextPath%>/rm/dm/devmixPlan/mixPlanDetailList.jsp?projectInfoNo=<%=projectInfoNo%>&deviceappid='+idinfo+'&sonFlag='+sonFlag;
		}
	...
	function dbclickRow(shuaId){
		...
		if(sonFlag == 'Y'){
			window.location.href='<%=contextPath%>/rm/dm/devmixPlan/mixPlanDetailList.jsp?projectInfoNo='+retFatherNo+'&deviceappid='+shuaId+'&sonFlag='+sonFlag;
		}else{
			window.location.href='<%=contextPath%>/rm/dm/devmixPlan/mixPlanDetailList.jsp?projectInfoNo=<%=projectInfoNo%>&deviceappid='+shuaId+'&sonFlag='+sonFlag;
		}
