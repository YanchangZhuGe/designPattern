			...
			<div id="table_box">
				<table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			     <tr id='device_app_id_{device_app_id}' name='device_app_id' ondblclick='toModifyDetail(this)' idinfo='{device_app_id}'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' value='{device_app_id}' id='selectedbox_{device_app_id}' />" >选择</td>
					<td class="bt_info_even" autoOrder="1">序号</td>
					<td class="bt_info_odd" exp="{device_app_name}">调配申请单名称</td>
					<td class="bt_info_even" exp="{device_app_no}">调配申请单号</td>
					<td class="bt_info_odd" exp="{org_name}">申请单位名称</td>
...
<script type="text/javascript">
    ...
    function refreshData(v_device_app_no, v_device_app_name){
		var str = "select aa.*,case aa.state when '0' then '未提交' when '1' then '待审批' when '3' then '审批通过' when '4' then '审批不通过' else '' end as state_desc from (select pro.project_name,devapp.device_app_id,devapp.device_app_no,devapp.device_app_name,devapp.project_info_no,";
	...
	function toAddMixPlanPage(){
		...
		if(basedatas.length == 0){
			...
			popWindow('<%=contextPath%>/rm/dm/collDevmixPlan/collMixplan_new.jsp?projectInfoNo=<%=projectInfoNo%>');
		}
	}
	function toModifyMixPlanPage(){
		...
		%>
		querySql += "from gms_device_collapp devapp left join common_busi_wf_middle wfmiddle on devapp.device_app_id=wfmiddle.business_id ";
		...
		popWindow('<%=contextPath%>/rm/dm/collDevmixPlan/collMixplan_modify.jsp?projectInfoNo=<%=projectInfoNo%>&deviceappid='+deviceappid);
	}
	function toDelMixPlanPage(){
		...
		//什么状态不能删除，和业务专家确定
		if(confirm("是否执行删除操作?")){
			var sql = "update gms_device_collapp set bsflag='1' where device_app_id in ("+selectedid+")";
	...
	function toSumbitMixApp(){
		...
		querySql += "from gms_device_app_colldetail appdet where appdet.device_app_id in ("+selectedid+")";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var basedatas = queryRet.datas;
	...
	function toModifyDetail(obj){
		...
		window.location.href='<%=contextPath%>/rm/dm/collDevmixPlan/collMixPlanDetailList.jsp?projectInfoNo=<%=projectInfoNo%>&deviceappid='+idinfo;
	}
	function dbclickRow(shuaId){
		window.location.href='<%=contextPath%>/rm/dm/collDevmixPlan/collMixPlanDetailList.jsp?projectInfoNo=<%=projectInfoNo%>&deviceappid='+shuaId;
	}
