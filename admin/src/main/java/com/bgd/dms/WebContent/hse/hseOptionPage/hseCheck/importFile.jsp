...
<%
	...
	String check_no = "";
	if(respMsg != null){
		check_no = respMsg.getValue("check_no");
	}else{
...
//		top.frames('list').frames[1].refreshData();
	...
	function uploadFile(){		
		...
		if(check(filename)){
			...
			document.getElementById("fileForm").action = "<%=contextPath%>/hse/check/importHseCheck.srq?check_no=<%=check_no%>";
...
<table id="buttonTable" border="0" cellpadding="0" cellspacing="0" class="form_info">
	<tr align="right">
		...
		<td>
		<a href="javascript:downloadModel('ImportHseCheck','HSE检查')"><font color=red>下载导入模板</font></a>
