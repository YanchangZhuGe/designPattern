			...
			<div class="easyui-layout" data-options="fit:true">
				...
				<div data-options="region:'center',border:false,split:false" > 
					<table id="main_grid">
						<thead>
							<tr>
								<th data-options="field:'apply_id',checkbox:true,align:'center'" width="10">主键</th>
								<th data-options="field:'apply_num',align:'center',sortable:'true'" width="22">需求单号</th>
								<th data-options="field:'order_type_name',align:'center',sortable:'true'" width="25">需求类型</th>							
								<th data-options="field:'apply_year',align:'center',sortable:'true'" width="25">年度</th>
					...
					<div id="tb2" style="height:28px;">
						<div style="float:left;height:28px;">						
							&nbsp;&nbsp;需求单号：<input id="applynum" class="input_width query" style="width:80px;float:none;">
							&nbsp;&nbsp;需求类型：<input id="ordertype" class="easyui-combobox" data-options="editable:false,panelHeight:'auto'" style="width:110px;float:none;">
							&nbsp;&nbsp;年度：<input id="applyyear" class="easyui-combobox" data-options="editable:false,panelHeight:'auto'" style="width:110px;float:none;">
		...
		<div id="center" data-options="region:'center',title:'',split:true">	
			  <div id="tab" class="easyui-tabs" data-options="fit:true,plain:true">
                <div title="基本信息" style="padding:10px;">
					...
					<tr>
						 <td class="inquire_item">需求单号：</td>
					     <td class="inquire_form" colspan="3">
					    	<input type="text" id="apply_num" class="input_width only_read basedet" />
					     </td>
					     <td class="inquire_item">需求类型：</td>
					     <td class="inquire_form" colspan="3">
					    	<input type="text" id="order_type_name" class="input_width only_read basedet" />
					     </td>
					     <td class="inquire_item">年度：</td>
					     <td class="inquire_form" colspan="3">
					    	<input type="text" id="apply_year" class="input_width only_read basedet" />
					     </td>
					 </tr>
					 <tr>
...
$(function(){
	...
	//需求类型
	$('#ordertype').combobox({ 
		url:'<%=contextPath%>/rm/dm/toGetJsonPurch.srq?purchcode=5110000222',
	...
	$("#applyyear").combobox("loadData", data);//下拉框加载数据
	//$("#exeplanyear").combobox("setValue",thisYear);//设置默认值为今年
	//如果有行被选中，则加载选中标签的内容
	$('#tab').tabs({
		onSelect: function(title,index){
			...
			if(row){
				if(selectTabIndex==0){
					//设置基本信息
					loadMainInfo(row.apply_id);
				}else if(selectTabIndex == 1){
					//单据明细信息
					loadDetialList(row.apply_id);
				}else if(selectTabIndex==2){
					//审批明细信息
					loadDisList(row.apply_id);
				}
			}			
		}
	});
	...
	$("#main_grid").datagrid({ 
		...
		onClickRow:function(index,data){
			if(selectTabIndex==0){
				//设置基本信息
				loadMainInfo(data.apply_id);
			}else if(selectTabIndex == 1){
				//明细信息
				loadDetialList(data.apply_id);
			}else if(selectTabIndex==2){
				//分配明细信息
				loadDisList(data.apply_id);
			}
		},
		queryParams:{//必需参数
		},
		url:"${pageContext.request.contextPath}/rm/dm/getListDataBySrvAndMethod.srq?JCDP_SRV_NAME=ExePlanSrv&JCDP_OP_NAME=queryExePlanApplyList",
...
function loadMainInfo(applyid){
	if(applyid==""){
	 ...
	 $.ajax({
	        type: "POST",
	        url: '${pageContext.request.contextPath}/rm/dm/ajaxRetMapBySrvAndMethod.srq?JCDP_SRV_NAME=ExePlanSrv&JCDP_OP_NAME=getExePlanMainInfo',
...
function loadDetialList(applyid){
	$("#bill_grid").datagrid({
		...
		url:'${pageContext.request.contextPath}/rm/dm/getListDataBySrvAndMethod.srq?JCDP_SRV_NAME=ExePlanSrv&JCDP_OP_NAME=queryExePlanDet',
...
//加载审批明细明细
function loadDisList(applyid){
	$("#dist_grid").datagrid({
		queryParams:{'syncid': applyid},
...
function clearQueryText(){
	...
	$("#applyyear").combobox("setValue","请选择...");
...
function searchDevData(){
	...
	params["applyyear"] = $("#applyyear").combobox("getValue");
	queryParams = params;
