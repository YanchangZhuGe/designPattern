...
if(message != "" && message != 'null'){
	...
	function uploadFile(){		
		...
		if(check(filename)){
			...
			document.getElementById("fileForm").action = "<%=contextPath%>/hse/professional/importExcelDriver.srq?project="+project;
	...
	function downloadModel(modelname,filename){
		...
		filename = encodeURI(filename);
		window.location.href="<%=contextPath%>/rm/em/humanLabor/download.jsp?path=/hse/humanResource/hseDriver/"+modelname+".xlsx&filename="+filename+".xlsx";
	}
...
<table id="buttonTable" border="0" cellpadding="0" cellspacing="0" class="form_info">
	<tr align="right">
		<td>
		<a href="javascript:downloadModel('ImpleHseDriver','HSE驾驶人员信息')"><font color=red>下载导入模板</font></a>
