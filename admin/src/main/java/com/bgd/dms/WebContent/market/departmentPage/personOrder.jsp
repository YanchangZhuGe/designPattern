...
<script type="text/javascript">
	...
	function parentInfo(){
		window.dialogArguments.location.href="<%=contextPath%>/market/departmentPage/selectPerson.jsp?id=<%=map.get("deptId")%>&&corpId=<%=map.get("corpId")%>";
	}
</script>
</head>
...
<form name="form1" id="form1"  method="post" action="<%=contextPath%>/market/personOrderSave.srq" target="hiddenIframe">
	<table>
		...
		<tr>
			<td>
				<%=map.get("name") %>的序号为<%=map.get("orderNo") %>号，您可以将其调整到<input type="text" id="newOrderNo" name="newOrderNo" />号&nbsp;&nbsp;<input type="button" id="btu" value="确定" onclick="doOrder();"/>
			</td>
		</tr>
	</table>
