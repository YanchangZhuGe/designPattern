			  ...
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			            ...
			            <td class="bt_info_even">炮线(公里)</td>		
			             <td  class="bt_info_odd">服务费</td>
			            <td  class="bt_info_even">管理费</td>
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getMeasureActualInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
			...
			var td = tr.insertCell();
			td.innerHTML = datas[i].sumPriceSs;

			var td = tr.insertCell();
			td.innerHTML = datas[i].sumPriceMa;
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualMeasure.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	if (ids == '') {
		...
		popWindow(cruConfig.contextPath+"/op/costActualManager/costTargetActualMeasure.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	var sql = "update bgp_op_tartet_device_measure t set t.if_actual_change='1' where t.target_measure_id ='"+ids+"'";
