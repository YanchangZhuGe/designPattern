      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			     	<td class="bt_info_odd" exp="<input type='checkbox' id='selectedbox_{device_backapp_id}~{backdevtype}' name='selectedbox' value='{device_backapp_id}~{backdevtype}' stateinfo='{state}' onclick='chooseOne(this)'/>" >选择</td>
					...
					<td class="bt_info_even" exp="{allapp_type_name}">返还类型</td>
			...
			<div id="tab_box" class="tab_box">
				...
				<div id="tab_box_content1" name="tab_box_content1" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info" style="display:none" id="S90000000">
...
$(function(){
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				//如果是批量设备 by wangzheqin 2014-7-31
				if(currentid[1] =="S9000"){
					//先进行查询
					var str  = "select t.back_num as mix_num,t.*, d.dev_unit,d.total_num,d.unuse_num,d.use_num,d.actual_in_time,sd.coding_name as unit_name ";
						...
						str += " where t.device_backapp_id='"+currentid[0]+"'";
				}else if(currentid[1] =="S9996"){//如果是附属仪器
					var str = "select dui.dev_type,dui.dev_name,dui.dev_model,dui.self_num,dui.license_num,dui.dev_sign,dui.actual_in_time,dui.actual_out_time from gms_device_backapp_detail det ";
					str += "left join gms_device_account_dui dui on dui.dev_acc_id=det.dev_acc_id ";
					str += "where det.device_backapp_id='"+currentid[0]+"'";
				}
	...
	function appendDataToDetailTab(filterobj,datas,type){
		...
		$("#S90000000").css("display","none");
		for(var i=0;i<basedatas.length;i++){
			if(type == 'S9000'){
				$("#S90000000").css("display","block");
				...
				innerHTML += "<td>"+datas[i].dev_sign+"</td><td>"+datas[i].actual_in_time+"</td><td>"+datas[i].actual_out_time+"</td>";
				innerHTML += "</tr>";
			}
			//var innerHTML = "<tr>";
			//innerHTML += "<td>"+(i+1)+"</td><td>"+datas[i].dev_name+"</td><td>"+datas[i].dev_model+"</td><td>"+datas[i].unit_name+"</td>";
...
<script type="text/javascript">
    ...
    function loadDataDetail(devicebackappid){
    	var appid = devicebackappid.split("~");
    	var retObj;
		...
		querySql += " where t.device_backapp_id='"+appid[0]+"' and t.state='9'and t.bsflag='0'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		
		retObj = queryRet.datas;
		//取消其他选中的 
		$("input[type='checkbox'][name='selectedbox'][id!='selectedbox_"+appid[0]+"~"+appid[1]+"']").removeAttr("checked");
		//选中这一条checkbox
		$("input[type='checkbox'][name='selectedbox'][id='selectedbox_"+appid[0]+"~"+appid[1]+"']").attr("checked",'checked');
		//给数据回填
	...
	function refreshData(v_project_name,v_backapp_name,v_device_backapp_no,v_back_org_name){
		...
		str += " left join comm_org_information inorg on inorg.org_id = dui.in_org_id";
		str += " left join comm_org_subjection org on dui.in_org_id = org.org_id ";
		str += " where backapp.bsflag = '0' and backapp.state='9' and backapp.backdevtype='S9000' ";*/
		//str += " and org.org_subjection_id like '<%=orgsubid%>%' ";
		
		var str = "select case backdevtype when 'S9996'  then '附属仪器' when 'S9000' then '批量' end as allapp_type_name,t.* from (select distinct  backapp.backdevtype,pro.project_name,backapp.device_backapp_id,backapp.device_backapp_no,backapp.backapp_name, ";
		str += "backapp.project_info_id as project_info_no,backapp.back_org_id,backorg.org_abbreviation as back_org_name, ";
		str += "case backapp.state when '0' then '未提交' when '9' then '已提交' else '异常状态' end as state_desc, ";
		str += "backapp.opr_state,case backapp.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as oprstate_desc, ";
		str += "emp.employee_name as employee_name,to_char(backapp.modifi_date,'yyyy-mm-dd') as backdate ,inorg.org_abbreviation as receive_org_name ";
		str += "from gms_device_collbackapp backapp left join comm_org_information backorg on  backapp.back_org_id = backorg.org_id  ";
		str += "left join comm_human_employee emp on backapp.updator_id = emp.employee_id  ";
		str += "left join gp_task_project pro on backapp.project_info_id = pro.project_info_no ";
		str += "left join comm_org_subjection orgsub on backapp.backmix_org_id=orgsub.org_id and orgsub.bsflag='0' ";
		str += "left join gms_device_collbackapp_detail bad on bad.device_backapp_id = backapp.device_backapp_id ";
		str += "left join gms_device_coll_account_dui dui on bad.dev_acc_id = dui.dev_acc_id ";
		str += "left join gms_device_coll_account account on dui.fk_dev_acc_id = account.dev_acc_id ";
		str += "left join comm_org_information inorg on inorg.org_id = account.owning_org_id ";
		str += "left join comm_org_subjection org on account.owning_org_id = org.org_id ";
		str += "where backapp.bsflag = '0' and backapp.state='9' and backapp.backdevtype='S9000'  and backapp.backmix_org_id ='C6000000000040' ";
		str += "union all  ";
		str += "select distinct bac.backdevtype,tp.project_name,bac.device_backapp_id,bac.device_backapp_no,bac.backapp_name, ";
		str += "bac.project_info_id as project_info_no,bac.back_org_id,backorg.org_abbreviation as back_org_name, ";
		str += "case bac.state when '0' then '未提交' when '9' then '已提交' else '异常状态' end as state_desc, ";
		str += "bac.opr_state,case bac.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as oprstate_desc, ";
		str += "emp.employee_name, to_char(bac.modifi_date,'yyyy-mm-dd') as backdate,inorg.org_abbreviation as receive_org_name ";
		str += "from gms_device_backapp bac left join gp_task_project tp on bac.project_info_id = tp.project_info_no and tp.bsflag='0' ";
		str += "left join comm_org_information backorg on bac.back_org_id = backorg.org_id ";
		str += "left join comm_human_employee emp on bac.updator_id = emp.employee_id ";
		str += "left join gms_device_backapp_detail bd on bd.device_backapp_id = bac.device_backapp_id and bd.bsflag='0' ";
		str += "left join gms_device_account_dui accdui on accdui.dev_acc_id = bd.dev_acc_id and accdui.bsflag='0' ";
		str += "left join comm_org_information inorg on inorg.org_id = accdui.owning_org_id and inorg.bsflag='0' ";
		str += "left join comm_org_subjection org on accdui.owning_org_id = org.org_id ";
		str += "where  bac.bsflag='0' and bac.backdevtype='S9996' and bac.backmix_org_id ='C6000000000040' ) t ";
		...
		if(v_backapp_name!=undefined && v_backapp_name!=''){
			str += "and backapp_name like '%"+v_backapp_name+"%' ";
		}
		...
		if(v_back_org_name!=undefined && v_back_org_name!=''){
			str += "and org_abbreviation like '%"+v_back_org_name+"%' ";
		}
		str += "order by opr_state desc,backdate desc,backapp_name desc";
	...
	function toDetailPage(){
		...
		//如果是批量设备 by wangzheqin 2014-7-31
		if(shuaId[1]=="S9000"){
			window.location.href='<%=contextPath%>/rm/dm/DgCollectStore/subStockin.jsp?id='+shuaId[0];	
		}else if(shuaId[1]=="S9996"){//如果是附属仪器,跳转到收工验收-装备单台页面
			window.location.href='<%=contextPath%>/rm/dm/devstore/fssubStockin.jsp?id='+shuaId[0];
		}
	...
	function dbclickRow(shuaId){
		...
		if(shuaId[1]=="S9000"){
			...
			window.location.href='<%=contextPath%>/rm/dm/DgDevstore/subStockin_Dg.jsp?id='+shuaId[0];
		}
	}
