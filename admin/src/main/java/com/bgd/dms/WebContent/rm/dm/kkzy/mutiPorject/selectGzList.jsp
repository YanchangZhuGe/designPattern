...
function load()
{
	var usemat_id='<%=usemat_id%>';//唯一标识
		var baseData;
		 baseData = jcdpCallService("DevInsSrv", "getGzInfo", "usemat_id="+usemat_id);
			if(baseData.datas!=null)
			{
				...
				for (var i=0; i< baseData.datas.length; i++) {
					...
					if(baseData.datas[i].falut_case=='0'){
						...
						innerhtml +=  "<option   value='0' selected>现场解决</option>";
						innerhtml +="</select></td>";
...
function toAdd()
{
	...
	innerhtml += "<input name='falut_source"+tr_id+"' id='falut_source"+tr_id+"' value='2' size='16'  type='hidden' />";
...
function selectMatData(i)
{
	...
	var selected;
	selected = window.showModalDialog("<%=contextPath%>/rm/dm/kkzy/mutiPorject/selectMatList-zy.jsp?self_num="+self_num+"&bywx_type=1&selectWzId="+usemat_id+"&gz_bj_id="+gz_bj_id,"","dialogWidth=1240px;dialogHeight=680px");
...
function res_falut_case(i,falut_case){
	var innerhtml =  "";
	if(falut_case=="0"){
		innerhtml +=  "<option   value='0' selected>现场解决</option>";
	}else if(falut_case=="1"){
		innerhtml +=  "<option   value='1' selected>维修解决</option>";
	}
	$("#falut_case"+i).empty();
	$("#falut_case"+i).append(innerhtml); 
}
function changeGz(i){
	var parent_falut_id = "";
...
function submitInfo()
{
	...
	$("input[type='checkbox'][name='idinfo']").each(function(){
		if(this.checked){
			...
			if(count == 0){
				...
				falut_case = document.getElementById("falut_case"+count).value;
	...
	for(var index=0;index<selectedlines.length;index++){
		...
		//故障解决方式
		var falut_case1 = $("#falut_case"+selectedlines[index]).val();
		//故障原因
		var falut_reason1 = $("#falut_reason"+selectedlines[index]).val();
		//故障分组id
		var falut_group_id1 = $("#falut_group_id"+selectedlines[index]).val();
		//故障大类
		var parent_falut_id = $("#parent_falut_id"+selectedlines[index]).val();
		if(falut_desc1 == "" || (falut_case1=='0'&&falut_reason1 == ""&&parent_falut_id!="011") || falut_group_id1==""){
	...
	var submitStr = "has_falut_deal_flag="+has_falut_deal_flag+"&dev_acc_id="+dev_acc_id+"&count="+count+"&line_infos="+line_infos+"&usemat_id="+usemat_id+"&falut_source="+falut_source+"&falut_desc="+falut_desc+"&falut_reason="+falut_reason+"&falut_case="+falut_case+"&falut_group_id="+falut_group_id+"&id="+id+"&falut_deal_flag="+falut_deal_flag+"&gz_bj_id="+gz_bj_id;
