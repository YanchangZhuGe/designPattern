			...
			<div id="tab_box" class="tab_box">
				...
				<div id="tab_box_content1" name="tab_box_content1" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							<td class="bt_info_odd" width="11%">验收时间</td>
...
<script type="text/javascript">
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				//先进行查询
				var str = "select firm.torepair_num,firm.tocheck_num,firm.destroy_num,firm.create_date as check_date ";
				str += "from gms_device_coll_account_firm firm ";
				str += "where firm.device_backdet_id='"+currentid+"'";
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			...
			var innerHTML = "<tr>";
			innerHTML += "<td>"+datas[i].check_date+"</td><td>"+datas[i].torepair_num+"</td>";
...
<script type="text/javascript">
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model){
		...
		var str = "select backdet.*, case backdet.is_leaving when '0' then '未验收' when '1' then '已验收' end as isleaving, ";
		str += "account.dev_unit,account.total_num,account.unuse_num,account.use_num,account.actual_in_time,unitsd.coding_name as dev_unit_name ";
		...
		if(v_dev_ci_name!=undefined && v_dev_ci_name!=''){
			str += "and backdet.dev_name like '%"+v_dev_ci_name+"%' ";
		}
		if(v_dev_ci_model!=undefined && v_dev_ci_model!=''){
			str += "and backdet.dev_model like '%"+v_dev_ci_model+"%' ";
		}
	...
	function toSumbitDevApp(){
	  ...
	  //if(leavinginfo=="已验收"){
		//alert("本设备已验收，请查看!");
	  	//return;
	  //}
	  editUrl = "<%=contextPath%>/rm/dm/collectStoreBad/stockinSubmit.jsp?id="+selId;  
	  popWindow(editUrl);
