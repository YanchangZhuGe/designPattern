    ...
    <table  width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
     	<tr>
	    	...
	    	<td class="inquire_form4"><select id="dev_model" name="dev_model" onchange="changeUnit();" class="select_width">
				<option value="KZ-28">KZ-28</option>
			    <option value="KZ-28AS">KZ-28AS</option>
			    <option value="KZ-30">KZ-30</option>
			    <option value="KZ-34">KZ-34</option>
			    <option value="AHVⅣ">AHVⅣ</option>
	    	</select></td>
	    </tr>
    	<tr>	
	    	...
	    	<td class="inquire_item4">待工时间:</td>
	    	<td class="inquire_form4"><input name="passive_date" id="passive_date" type="text" class="input_width" value="" onkeydown="javascript:return checkIfNum(event);"/></td>
	    </tr>
    	<tr>
	    	...
	    	<td class="inquire_form4"><input name="actual_start_date" id="actual_start_date" type="text" class="input_width" value="" readonly="readonly" />
	    		<img width="16" height="16" id="cal_button7" style="cursor: hand;" onmouseover="calDateSelector(actual_start_date,cal_button7);" 
	    	...
	    	<td class="inquire_form4"><input name="actual_end_date" id="actual_end_date" type="text" class="input_width" value="" readonly="readonly" />
	    		<img width="16" height="16" id="cal_button8" style="cursor: hand;" onmouseover="calDateSelector(actual_end_date,cal_button8);" 
	    		src="<%=contextPath %>/images/calendar.gif" /></td>
	    </tr>
	    <tr>	
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		var querySql = "select * from bgp_op_target_device_mataxi where bsflag='0' and mataxi_type ='4' and target_mataxi_id = '"+id+"'";
		...
		if(retObj!=null && retObj.returnCode=='0' && retObj.datas!=null && retObj.datas[0]!=null){
			...
			with(map){
				...
				document.getElementById("passive_date").value = passive_date;
				document.getElementById("actual_start_date").value = actual_start_date;
				document.getElementById("actual_end_date").value = actual_end_date;
	...
	function newSubmit() {
		...
		var passive_date = document.getElementById("passive_date").value ;
		...
		if(target_mataxi_id!=null && target_mataxi_id!=''){
			...
			if(if_change==null || if_change =='0' || if_change =='1'){
				sql = "update bgp_op_target_device_mataxi t set t.if_actual_change='1' where t.target_mataxi_id='"+target_mataxi_id+"';";
				
				sql += "insert into bgp_op_target_device_mataxi(target_mataxi_id,project_info_no,dev_name,dev_model,actual_start_date,actual_end_date,"+
				"dev_count,passive_date,taxi_unit,spare1,taxi_ratio,org_id,org_subjection_id,creator,create_date,updator,"+
				"update_date,bsflag,if_change,mataxi_type,change_date) values((lower(sys_guid())),'<%=project_info_no%>','"+dev_name+"',"+
				"'"+dev_model+"',to_date('"+actual_start_date+"','yyyy-MM-dd'),to_date('"+actual_end_date+"','yyyy-MM-dd'),'"+dev_count+"',"+
				"'"+passive_date+"','"+taxi_unit+"','"+spare1+"','"+taxi_ratio+"','<%=org_id%>','<%=org_subjection_id%>',"+
				"'<%=user_id%>',sysdate,'<%=user_id%>',sysdate,'0','2','4',to_date('"+change_date+"','yyyy-MM-dd'));";
			}else{
				...
				" t.passive_date='"+passive_date+"',t.taxi_unit='"+taxi_unit+"',t.spare1='"+spare1+"',t.taxi_ratio='"+taxi_ratio+"',"+
	...
	function checkValue(){
		...
		obj = document.getElementById("actual_start_date") ;
		var value = obj.value ;
		if(obj ==null || value==''){
			alert("到达工区验收合格时间不能为空!");
			return false;
		}
		obj = document.getElementById("actual_end_date") ;
		value = obj.value ;
		if(obj ==null || value==''){
			alert("设备撤回存放地时间不能为空!");
			return false;
	...
	function changeUnit(){
		var units = "3600_KZ-28,3600_KZ-28AS,3600_KZ-30,5000_KZ-34,4300_AHVⅣ";
		var dev_model = document.getElementsByName("dev_model")[0].value; 
		var index = units.indexOf(dev_model);
		var unit = "";
		if(index!=-1){
			unit = units.substring(index-5,index-1);
