 ...
 <form name="form1" id="form1" method="post" action="" target="target_id">
 		<div id="list_table">
					...
					<table  border="0" cellpadding="0" cellspacing="0" id='tab_line_height' name='tab_line_height' class="tab_line_height" width="100%">
					    ...
					    <tr>
					    	<td class="inquire_item4">计划名称：</td>
					      	...
					      	<td class="inquire_item4" style="color:red;">计划指向：</td>
					      	<td class="inquire_form4"><input type="text" name="plan_invoice_type" id="plan_invoice_type" class="input_width" value="" readonly/></td>
					    </tr>
					    <tr>
			...
			<ul id="tags" class="tags">
				<li id="tag3_0" class="selectTag"><a href="#" onclick="getTab3(0)">申请明细</a></li>
				<li  id="tag3_1"><a href="#" onclick="getTab3(1)">审批明细</a></li>
			...
			<div id="tab_box_content0" class="tab_box_content" style="display: block; ">
			   <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
					<tr>
						...
						<td class="bt_info_even" exp="<font>{demand_num}</font>">需求数量</td>
						...
						<td class="bt_info_even" exp="{demand_date}">需求日期</td> 
		...
		<!-- <div id="tab_box_content2" class="tab_box_content" style="display: none; overflow: hidden;">
...
//		document.getElementById("team").src = "<%=contextPath%>/mat/singleproject/plansub/teamList.jsp?planInvoiceId="+planInvoiceId;
    ...
    function showTitle(value){
    	...
    	document.getElementById("main_part").value = retObj.matInfo.planName;
    	document.getElementById("memo").value = retObj.matInfo.memo; 
    	document.getElementById("plan_invoice_type").value = retObj.matInfo.planInvoiceType;
        }
	...
	function submitInfo(oprinfo){
	   ...
	   // var tab =document.getElementById("queryRetTable");
		//var row = tab.rows;
		//for(var i=1;i<row.length;i++){
		//	var cell_1 = row[i].cells[8].firstChild.value;
		//	if(cell_1!=null){
		//		ids+=","+cell_1
		//		}
	...
	var re = /^[0-9]+\.?[0-9]*$/;
		...
		for(var i=1;i<row.length;i++){
			var cell_1 = row[i].cells[8].firstChild.value;
			if(cell_1!=null){
			var number = row[i].cells[2].firstChild.innerHTML;
			if(cell_1>number){
				cell_1=number;
				}
			var cell_2 = row[i].cells[9].innerHTML;
					if(re.test(cell_1)&&re.test(cell_2)){
					row[i].cells[10].innerHTML=Math.round((cell_1*cell_2)*1000)/1000;
					if(parseInt(row[i].cells[10].innerHTML)<0){
						row[i].cells[10].innerHTML=0;
						}
				}
			}	
			}
