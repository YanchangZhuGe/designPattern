	      ...
	      <tr style="display:none" id="inCountry">
      	 ...
      	 <fieldset id="addTaskInfo" style="margin-left:2px"><legend>添加作业信息</legend>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:2px;">
		        ...
		        <tbody id="taskTable" name="taskTable">
...
cruConfig.cdtType = 'form';
	...
	$().ready(function(){
		...
		if(projectType == "5000100004000000008" || projectType == '5000100004000000009' || dg_Flag == "Y"){
	...
	function loadDataDetail(){
		...
		if(device_mix_detid!=null){
		    var querysql="select da.dev_coding,mif.project_info_no,dad.*,";
	...
	function submitInfo(){
	    ...
	    if(projectType != "5000100004000000008" && projectType != '5000100004000000009' && dg_Flag == "N" ){
		    if(taskids==""){
				alert("请填加作业信息！");
				return;
			}
		...
		if(confirm("确认提交？")){
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSubmitReceive.srq?id="+id+"&devaccId="+devaccId+"&projectInfoNo="+projectInfoNo+"&taskids="+taskids;
	...
	function addLine( receive_nos,task_ids,task_names,plan_start_dates,plan_end_dates,objcet_ids,planned_durations){
		...
		tr.insertCell().innerHTML = '<input type="text" readonly="readonly"  value="'+planned_duration+'" readonly/>';
