...
<!-- 						  	<td class="inquire_item4">倍数：</td>
 ...
 -->						<td class="inquire_item4">金额:</td>
						 ...
						 <%if(projectType!=null&&projectType.equals("5000100004000000001")){%>	
						   	<tr>
						   		<td class="inquire_item4"><font color="red">*</font>计划分类：</td>
							   	<td class="inquire_form4">
							   		<select id="if_purchase" name="if_purchase" class="select_width">
							   			<option value="1">物资采购计划</option>
							   			<option value="2">可重复物资采购计划</option>
							   		</select>
							   	</td>
						   	</tr>
						   <%}else{ %>	
...
//	    	var applyTeamList=jcdpCallService("HumanCommInfoSrv","queryApplyTeam","");	
       ...
       function toAdd(trid){
			...
			var if_purchase = document.getElementById("if_purchase").value;    //计划类型   1 供应中心物资  2 物探处可重复物资 -----lx
			if(if_purchase!=null&&if_purchase=="2"){
				var vReturnValue = window.showModalDialog('<%=contextPath%>/mat/singleproject/plan/selectRepMatList.jsp?ids='+ids,obj,'dialogWidth=1024px;dialogHigth=400px');
			}else{
