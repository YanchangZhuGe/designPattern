...
<script type="text/javascript">
    ...
    function refreshData(v_device_backapp_no, v_backapp_name){
		var str = "select pro.project_name,backapp.device_backapp_id,backapp.backapp_name,";
			...
			str += "case backapp.backdevtype when 'S9999' then '外租设备' when 'S0000' then '物探处自有设备' when 'S0623' then '专业化震源' ";
			str += "when 'S1405' then '专业化仪器' when 'S9998' then '调剂设备' else '专业化测量' end as backdevtypedesc,";
			str += "backapp.state,case backapp.state when '0' then '未提交' when '9' then '已提交' else '未知状态' end as state_desc,";
			str += "case wfmiddle.proc_status when '1' then '待审批' when '3' then '审批通过' when '4' then '审批不通过' else '未提交' end as procstate_desc,";
			...
			//str += "where backapp.bsflag = '0' and backapp.project_info_id = '"+projectInfoNos+"' ";
			...
			if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目
				...
				str += "where backapp.bsflag = '0' and backapp.backdevtype != 'S1405' and backapp.backdevtype != 'S14059999' and backapp.project_info_id = '" +retFatherNo+"' ";
				...
				str += "where backapp.bsflag = '0' and backapp.backdevtype != 'S1405' and backapp.backdevtype != 'S14059999' and backapp.project_info_id='"+projectInfoNos+"' " ;
	         }else{
		...
		str += " and (backapp.backapptype = '1' or backapp.backapptype is null) order by backapp.state nulls first,backapp.backdate desc  ";
	...
	function toAddBackPlanPage(){
		popWindow('<%=contextPath%>/rm/dm/devback/backdetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>','950:680');
	}
	...
	<%-- function dbclickRow(shuaId){
		...
		var querySql = "select devapp.device_backapp_id,nvl(wfmiddle.proc_status,'') as proc_status ";
		querySql += "from gms_device_backapp devapp left join common_busi_wf_middle wfmiddle on devapp.device_backapp_id=wfmiddle.business_id ";
		querySql += "where devapp.bsflag='0' and devapp.device_backapp_id ='"+shuaId+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var basedatas = queryRet.datas;
		var stateflag = false;
		var alertinfo ;
		if(basedatas !=undefined && basedatas.length>=1){
			if(basedatas[0].proc_status == '1' || basedatas[0].proc_status == '3'){
				return;
			}
		}
		window.location.href = '<%=contextPath%>/rm/dm/devback/backPlanDetailList.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+shuaId;
	} --%>
