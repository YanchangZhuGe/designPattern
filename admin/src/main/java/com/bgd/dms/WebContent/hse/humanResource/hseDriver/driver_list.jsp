      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{hse_driver_id}' id='rdo_entity_id_{hse_driver_id}' onclick='loadDataDetail();'/>" >选择</td>
			      ...
			      <td class="bt_info_odd" exp="<font color='{color}' style='font-weight:bold'>{type_num}</font>">准驾车型代号</td>
			      <td class="bt_info_even" exp="<font color='{color}' style='font-weight:bold'>{coding_name}</font>">内部准驾车型</td>
			      <td class="bt_info_odd" exp="<font color='{color}' style='font-weight:bold'>{days}</font>">临近审验日期时间</td>
			      ...
			      <td class="bt_info_odd" exp="{type_num}" isShow='Hide' isExport='Show'>准驾车型代号</td>  
			      <td class="bt_info_odd" exp="{coding_name}" isShow='Hide' isExport='Show'>内部准驾车型</td> 
			      
			      <td class="bt_info_odd" exp="{driver_date}" isShow='Hide' isExport='Show'>初领证日期</td>  
			      <td class="bt_info_odd" exp="{useful_life}" isShow='Hide' isExport='Show'>驾驶证有效日期</td>  
			      <td class="bt_info_odd" exp="{special_driver}" isShow='Hide' isExport='Show'>是否驾驶特种车辆</td>  
			      <td class="bt_info_odd" exp="{danger_driver}" isShow='Hide' isExport='Show'>是否驾驶危化品车辆</td>  
			      <td class="bt_info_odd" exp="{driving_num}" isShow='Hide' isExport='Show'>准驾证号</td>  
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">
			    ...
			    <li id="tag3_1"><a href="#" onclick="getTab3(1)">司驾信息</a></li>
			  </ul>
			</div>
			
			<div id="tab_box" class="tab_box">
			<input type="hidden" id="hse_driver_id" name="hse_driver_id"></input>
...
<script type="text/javascript">
	...
	function toUploadFile(){
		popWindow("<%=contextPath%>/hse/humanResource/hseDriver/humanImportFile.jsp?project=<%=project%>");
     }
	...
	function simpleSearch(){
		...
		if(name!=''&&name!=null){
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = "select * from (select hd.hse_driver_id,hd.project_info_no,hd.bsflag,hd.modifi_date,case when ee.employee_id is null then la.owning_subjection_org_id else os.org_subjection_id end org_subjection_id,hd.employee_id,case when ee.employee_id is null then la.employee_name else ee.employee_name end employee_name,case when ee.employee_id is null then la.employee_id_code_no else ee.employee_id_code_no end code_id,case when ee.employee_id is null then decode(la.employee_gender,'0','女','1','男') else decode(ee.EMPLOYEE_GENDER, '0', '女', '1', '男') end sex_type,hd.person_status, case when ee.employee_id is null then sd2.coding_name else sds.coding_name end coding_name_xl ,     cd.coding_name title,   hd.driver_date,hd.useful_life,decode(hd.special_driver,'1','是','2','否') special_driver,decode(hd.danger_driver,'1','是','2','否') danger_driver,hd.driving_num,hd.type_num,sd.coding_name,hd.useful_life-to_date(to_char(sysdate,'yyyy-MM-dd'),'yyyy-MM-dd') days,case when months_between(hd.useful_life,sysdate)<=0 then 'red'  when months_between(hd.useful_life,sysdate)>2 then '' else 'orange' end color,oi1.org_abbreviation second_org_name,oi2.org_abbreviation third_org_name,oi3.org_abbreviation fourth_org_name from bgp_hse_driver hd left join comm_human_employee ee on hd.employee_id=ee.employee_id and ee.bsflag='0'    left join comm_coding_sort_detail sds    on ee.employee_education_level = sds.coding_code_id   and sds.bsflag = '0'       left join bgp_comm_human_technic ht        on ht.employee_id = ee.employee_id       and ht.bsflag = '0'       left join comm_coding_sort_detail cd      on ht.expert_sort = cd.coding_code_id    and cd.bsflag = '0' left join bgp_comm_human_labor la on la.labor_id=hd.employee_id and la.bsflag='0'  left join comm_coding_sort_detail sd2 on la.employee_education_level=sd2.coding_code_id and sd2.bsflag='0'  left join comm_coding_sort_detail sd on hd.inner_type=sd.coding_code_id and sd.bsflag='0' left join comm_org_subjection os1 on hd.second_org=os1.org_subjection_id and os1.bsflag='0' left join comm_org_information oi1 on os1.org_id = oi1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on hd.third_org=os2.org_subjection_id and os2.bsflag='0' left join comm_org_information oi2 on os2.org_id = oi2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on hd.fourth_org=os3.org_subjection_id and os3.bsflag='0' left join comm_org_information oi3 on os3.org_id = oi3.org_id and oi3.bsflag = '0' left join comm_org_subjection os on os.org_id=ee.org_id and os.bsflag='0' where hd.bsflag='0') where bsflag='0' "+querySqlAdd+" and employee_name like '%"+name+"%' order by modifi_date desc";
			cruConfig.currentPageUrl = "hse/humanResource/hsedriver/driver_list.jsp";
			queryData(1);
		}else{
			refreshData();
	...
	function loadDataDetail(shuaId){
		var retObj;
		if(shuaId!=null){
			 retObj = jcdpCallService("HseSrv", "queryDriver", "hse_driver_id="+shuaId);
		    ...
		    retObj = jcdpCallService("HseSrv", "queryDriver", "hse_driver_id="+ids);
		}
		document.getElementById("hse_driver_id").value =retObj.map.hseDriverId;
		...
		document.getElementById("danger_driver").value = retObj.map.dangerDriver;
		document.getElementById("special_driver").value = retObj.map.specialDriver;
		document.getElementById("type_num").value = retObj.map.typeNum;
		document.getElementById("driver_date").value = retObj.map.driverDate;
		document.getElementById("doc_num").value = retObj.map.docNum;
		document.getElementById("driver_type").value = retObj.map.driverType;
		document.getElementById("duty").value = retObj.map.duty;
		document.getElementById("inner_type").value = retObj.map.innerType;
		document.getElementById("driving_num").value = retObj.map.drivingNum;
		document.getElementById("driving_org").value = retObj.map.drivingOrg;
		document.getElementById("signer").value = retObj.map.signer;
		document.getElementById("sign_date").value = retObj.map.signDate;
		document.getElementById("useful_life").value = retObj.map.usefulLife;
		document.getElementById("second_person").value = retObj.map.secondPerson;
	...
	function toAdd(){
		popWindow("<%=contextPath%>/hse/humanResource/hseDriver/addDriver.jsp?project=<%=project%>");
	...
	function toUpdate(){ 
		...
		var hse_driver_id = document.getElementById("hse_driver_id").value;
		var form = document.getElementById("form1");
		form.action="<%=contextPath%>/hse/resource/saveDriver.srq?project=<%=project%>&&hse_driver_id="+hse_driver_id;
		form.submit();
	} 
	...
	function toDelete(){
		...
		if(confirm('确定要删除吗?')){  
			var retObj = jcdpCallService("HseSrv", "deleteDriver", "hse_driver_id="+ids);
			queryData(cruConfig.currentPage);
		}
	}

	function toSearch(){
		popWindow("<%=contextPath%>/hse/humanResource/hseDriver/driver_search.jsp?project=<%=project%>");
	}
