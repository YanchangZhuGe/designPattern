					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
						  <tr>
						  	<td class="inquire_item4"><font color="red">*</font>计划编号：</td>
						   	<td class="inquire_form4"><input name="plannum" id="plannum" type="text" class="input_width" value="" disabled="disabled"/>
						   	</td>
						  	<!-- 
...
//		sql +="select t.* from gms_mat_infomation t where t.bsflag='0'and wz_id='"+value+"' order by t.coding_code_id asc,t.wz_id asc";
		...
		//	+" on t.wz_id = i.wz_id and i.bsflag = '0' and t.bsflag = '0' and t.wz_type = '2' and i.wz_id='"+value+"' and t.stock_num>0 ";
			sql+="select t.* from gms_mat_infomation t where t.bsflag='0'and wz_id='"+value+"' order by t.coding_code_id asc,t.wz_id asc";
		cruConfig.cdtType = 'form';
		cruConfig.queryStr = sql;
		cruConfig.currentPageUrl = "/mat/singleproject/plan/repMatPlan/planAddItemListDg.jsp";
		queryData(1);
	}
	function toDelete(){
		  ...
		  ids = getSelIds('rdo_entity_id');
			if (ids == '') {
				...
				if(loadDataDetail()){				
					...
					document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/plan/saveRepPlan.srq?plannum="+plannum;
...
debugger;
				var plannum=document.getElementById("plannum").value;
					...
					if(plannum==""){
						var myDate = new Date();
						  //计划编号
						 var plannum="ZYH_"+Math.floor(myDate.getTime()/1000);   
	...
	function checkText(){
		...
		if(plannum==""){
			var myDate = new Date();
			  //计划编号
			 var plannum="CF_"+Math.floor(myDate.getTime()/1000);   
			 document.getElementById("plannum").value=plannum;
		}
