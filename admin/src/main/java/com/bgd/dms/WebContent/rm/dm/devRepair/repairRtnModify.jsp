						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<tr>
								...
								<input name="org_id" id="org_id" class="input_width" type="hidden"
									value="" />
									<input name="todoorg_id" id="todoorg_id" class="input_width" type="hidden"
									value="" />
								<input
								...
								<td class="inquire_form4" colspan="1">
								<input name="repid" id="repid" class="input_width" type="hidden"
									value="" />
								<input
									name="repair_form_code" id="repair_form_code" class="input_width"
									type="text" value="" readonly />
									<img src="<%=contextPath%>/images/magnifier.gif" width="16"
									height="16" style="cursor: hand;" onclick="showFormPage()" /></td>
						...
						<div style="overflow: auto">
							<table style="width: 97.9%" border="0" cellspacing="0"
								...
								<tr>
									...
									<td width="13%" class="bt_info_even">设备状态</td>
...
$().ready(function(){
	...
	function submitInfo(){
		...
		****/
		//alert(count);
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/devRepair/repairRtnNewApply.srq?count="+count+"&line_infos="+line_infos;
	...
	$().ready(function(){
		$("#addProcess").click(function(){
			...
			var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/devRepair/selectDevDetailForBack.jsp?id="+id+"&projectinfono=projectInfoNo&backdevtype=backdevtype",paramobj,"dialogWidth=820px;dialogHeight=450px");
			...
			var condition ="(";
				...
				condition += ") ";
				var str = "SELECT REP.REPAIR_FORM_CODE AS REPAIR_FORM_NO,REP.REPAIR_FORM_NAME AS REPAIR_FORM_NAME,ACC.DEV_NAME AS DEV_NAME,ACC.DEV_MODEL AS DEV_MODEL, ACC.DEV_TYPE AS DEV_TYPE,DECODE(SUB.ERROR_TYPE,'0','无故障','1','普通','2','特殊','') as ERROR_TYPE\n" +
				" ,REQORG.ORG_ID AS ORG_ID,REQORG.ORG_ABBREVIATION AS REQ_COMP_NAME,DECODE(SUB.ERROR_DESC,'0','无故障','1','不通','2','数传','3','死站','4','年检不过','5','单边通','6','本道不通','7',"+
				"'共摸','8','本道','9','初始化不过','10','漏电','11','畸变不过','12','单边不通','13','短路','14','物理损坏','15','共模不过','') AS ERROR_DESC" +
				" ,ACC.DEV_SIGN AS DEV_SIGN,DECODE(SUB.DEV_STATUS,'0','待修','1','在修','2','待仪器检测','3','无法修复','4','完好','') AS DEV_STATUS,SUB.REMARK AS REMARK,SUB.ID as ID"+
		...
		$("#delProcess").click(function(){
			$("input[type='checkbox'][name='idinfo']").each(function(i){
				if(this.checked == true){
					...
					$("#tr"+id).remove();
					//修改该条记录的状态
					var sql = "delete GMS_DEVICE_COL_REP_DETAIL  where REP_FORM_DET_ID ='"+id+"'; update GMS_DEVICE_COLL_REPAIR_SUB set RETURN_FLG='0' where id='"+id+"'";
	...
	function addLine(retObj,readflag){  //flag 是否从数据库中读取  true  是
		...
		for(var index=0;index<retObj.length;index++){
			...
			innerhtml += "<td width='4%'><input type='checkbox' name='idinfo' id='"+retObj[index].id+"' value='"+retObj[index].id+"'/></td>";
			...
			innerhtml += "<td width='13%'>"+retObj[index].dev_status+"</td><td width='13%'>"+retObj[index].remark+"</td>";
			innerhtml += "</tr>";
			if(flag){//如果与现有的设备台账不重复
			$("#detaillist").append(innerhtml);
			}
			$("#org_name").val(retObj[0].req_comp_name);
			$("#org_id").val(retObj[0].req_comp);
	...
	$(
			function refreshData(){
				var sql = "SELECT T.*,REP.REPAIR_FORM_CODE AS REPAIR_FORM_CODE,REP.todo_comp as todo_comp,REP.ID AS REPID FROM GMS_DEVICE_COL_REP_FORM T,GMS_DEVICE_COLL_REPAIRFORM REP WHERE 1 = 1 AND T.BACKAPP_NO = REP.ID(+) ";
				sql += " and t.ID = '<%=id%>'";
				var repairQueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+sql);
				var repairObj = repairQueryRet.datas;
				$("#backapp_no").val(repairObj[0].backapp_no);
				$("#backapp_name").val(repairObj[0].backapp_name);
				$("#backdate").val(repairObj[0].backdate);
				$("#org_id").val(repairObj[0].org_id);
				$("#todoorg_id").val(repairObj[0].todo_comp);
				$("#org_name").val(repairObj[0].org_name);
				$("#remark").val(repairObj[0].remark);
				$("#send_id").val(repairObj[0].send_id);
				$("#repair_form_code").val(repairObj[0].repair_form_code);
				$("#repid").val(repairObj[0].repid);
				var str = 
					"SELECT REP.REPAIR_FORM_CODE AS REPAIR_FORM_NO,\n" +
					"       REP.ID AS REPID,\n" + 
	...
	function showFormPage(){
		var paramobj = new Object();
		var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/devRepair/selectRepairFormForBack.jsp?projectinfono=projectInfoNo&backdevtype=backdevtype",paramobj,"dialogWidth=820px;dialogHeight=450px");
		if(vReturnValue == undefined){
			return;
		}
		var id = vReturnValue.split("|","-1");
		var str =  
			"SELECT REP.*,REQORG.ORG_ABBREVIATION AS REQ_COMP_NAME\n" +
			"  FROM GMS_DEVICE_COLL_REPAIRFORM REP,COMM_ORG_INFORMATION REQORG\n" + 
			"  WHERE REP.REQ_COMP = REQORG.ORG_ID(+)\n" + 
			"  AND REP.ID = '" + id + "'";
	    var proqueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		var retObj = proqueryRet.datas;
		//送修单号改变liug  start------
		if($("#repair_form_code").val() != retObj[0].repair_form_code){
			$("input[type='checkbox'][name='idinfo']").each(function(i){
					var id=this.id;
					$("#tr"+id).remove();
					//修改该条记录的状态
		...
		//送修单号改变liug  end------
		$("#repair_form_code").val(retObj[0].repair_form_code);
		$("#org_name").val(retObj[0].req_comp_name);
		$("#org_id").val(retObj[0].req_comp);
		$("#todoorg_id").val(retObj[0].todo_comp);
