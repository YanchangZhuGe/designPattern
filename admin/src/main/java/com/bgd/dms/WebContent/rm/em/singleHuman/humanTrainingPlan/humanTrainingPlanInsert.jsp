...
function sucess() {
	...
	if(checkForm()){
		...
		form.action = "<%=contextPath%>/rm/em/toSaveHumanTrainingPlan.srq";
		form.submit();
		alert('保存成功');
		newClose();
	}
...
function addDevice1(trainDetailNos,trainContents,classifications,trainNumbers,trainClasss,trainCosts,trainTransportations,trainMaterialss,trainPlacess,trainAccommodations,trainOthers){
 //	trainDetailNo,bsflag,trainContent,classification,trainNumber,trainClass,trainCost,trainTransportation,trainMaterials,trainPlaces,trainAccommodation,trainOther,trainTotal
  ...
  //需求子表id,为空
 	tr.insertCell().innerHTML  =currentCount+1+'<input type="hidden" id="fy'+deviceCount+'trainDetailNo" name="fy'+deviceCount+'trainDetailNo" value="'+trainDetailNo+'"/><input type="hidden" id="fy'+deviceCount+'bsflag" name="fy'+deviceCount+'bsflag" value="0"/>';

 	tr.insertCell().innerHTML = '<input type="text" maxlength="32" id="fy'+deviceCount+'trainContent" name="fy'+deviceCount+'trainContent"  value="'+trainContent+'" class="input_width" />';	
	//请选择 
	if(classification == ""){ 
...
<tr>
  ...
  <% 
  String buttonViews = resultMsg.getValue("buttonView"); 
  if("true".equals(buttonViews)){
  %>
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr align="right">
  <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<tr> 
		...
		<TD class="bt_info_even" width="3%">操作</TD>
		 ...
		 <%
		  ...
		  <tr align="center">
		  <td width="34%" ><font color=red>合计（元）</font> </td>
		  ...
		  <td width="3%"><label id="a8" ></label></td>
		  </tr></table>
</table>	 
<% 
