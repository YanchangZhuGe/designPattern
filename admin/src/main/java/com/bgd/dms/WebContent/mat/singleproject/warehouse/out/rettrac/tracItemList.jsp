...
<body style="background:#fff"  onload="refreshData()">
	...
	<!--取消手动提交，改为自动提交 		<auth:ListButton functionId="" css="tj" event="onclick='toSubmit()'" title="JCDP_btn_submit"></auth:ListButton> -->
			    <td></td>
			  </tr>
			</table>
			</td>
			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input name = 'rdo_entity_name' id='rdo_entity_id' type='checkbox' value='{out_info_id}' onclick='loadDataDetail();chooseOne(this.value)'/>" >选择</td>
			      <td class="bt_info_even" autoOrder="1">序号</td>
			      <td class="bt_info_even" exp="{procure_no}">退库单号</td>
			      <td class="bt_info_odd" exp="{total_money}">合计金额</td>
			      <td class="bt_info_even" exp="{org_name}">领料单位</td>
			      <td class="bt_info_even" exp="{out_date}">退库时间</td>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		var sql ="select t.procure_no,t.note,t.out_date,t.total_money,t.out_info_id,i.org_abbreviation org_name,decode(t.if_submit,'0','未提交','1','已提交') if_submit from gms_mat_out_info t inner join gp_task_project_dynamic p on t.project_info_no=p.project_info_no and p.bsflag='0' left join comm_org_information i on p.org_id=i.org_id and i.bsflag='0' where  t.bsflag='0'and t.out_type='2' and t.project_info_no='<%=projectInfoNo%>'  order by t.out_date desc,t.modifi_date desc";
	...
	function toOut(){
	  ...
	  function toAdd(){ 
			  ...
			  popWindow("<%=contextPath%>/mat/singleproject/warehouse/out/rettrac/changeType.jsp");
       ...
       function simpleSearch(){
    	   var sql ='';
			...
			sql+="select t.procure_no,t.note,t.out_date,t.total_money,t.out_info_id,i.org_abbreviation org_name,decode(t.if_submit,'0','未提交','1','已提交') if_submit from gms_mat_out_info t inner join gp_task_project_dynamic p on t.project_info_no=p.project_info_no and p.bsflag='0' inner join comm_org_information i on p.org_id=i.org_id and t.bsflag='0'and t.out_type='2' and t.project_info_no='<%=projectInfoNo%>' and t.procure_no like'%"+wz_name+"%'"
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = sql;
...
//			 	    	}
//		 	    	else{
		 	 		popWindow("<%=contextPath%>/mat/singleproject/warehouse/out/rettrac/queryTracOut.srq?laborId="+ids);
//		 				}
			 	}
      }
      function toDelete(){
    	  ...
    	  if(confirm('确定要删除吗?')){  
				...
				var retObj = jcdpCallService("MatItemSrv", "deleteTracOut", "matId="+ids);
	 	    	...
	 	    	if(ifSubmit =='1'){
					...
					return;
	 	    		 ...
	 	    		 if(confirm('确定要提交吗?')){  
	 					...
	 					var retObj = jcdpCallService("MatItemSrv", "submitTrac", "matId="+ids);
