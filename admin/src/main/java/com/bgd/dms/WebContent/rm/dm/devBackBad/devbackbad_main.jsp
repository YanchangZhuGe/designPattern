      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			     	<td class="bt_info_odd" exp="<input type='checkbox' id='selectedbox_{device_backapp_id}' name='selectedbox' value='{device_backapp_id}' stateinfo='{state}' />" >选择</td>
...
<script type="text/javascript">
    ...
    function refreshData(v_device_backapp_no, v_backapp_name, v_org_name){
		...
		str += "org.org_abbreviation as back_org_name,emp.employee_name as back_employee_name,outorg.org_abbreviation as out_org_name,recorg.org_abbreviation as receive_org_name ";
		...
		str += "left join comm_org_information recorg on backapp.receive_org_id = recorg.org_id ";
		...
		if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目
			str += "left join gp_task_project pro on backapp.project_info_id = pro.project_info_no and pro.bsflag = '0' and pro.project_type = '5000100004000000008' ";
			str += "where backapp.bsflag = '0' and backapp.backdevtype != 'S0000' and backapp.project_info_id = '" +retFatherNo+"' ";
		}else if(projectType == "5000100004000000008" && retFatherNo.length <= 0){
        	str += "left join gp_task_project pro on backapp.project_info_id = pro.project_info_no and pro.bsflag = '0' and pro.project_father_no is null and pro.project_type = '5000100004000000008' ";
			str += "where backapp.bsflag = '0' and backapp.backdevtype = 'S0000' and backapp.project_info_id='"+projectInfoNos+"' " ;
         }else{
        	str += "left join gp_task_project pro on backapp.project_info_id = pro.project_info_no and pro.bsflag = '0' ";
			str += "where backapp.bsflag = '0' and backapp.backdevtype = 'S0000' and backapp.project_info_id='"+projectInfoNos+"' ";
	     }
		...
		str += " and backapp.backapptype = '2' order by backapp.state nulls first,backapp.backdate desc  ";
	...
	function toAddBackPlanPage(){
		popWindow('<%=contextPath%>/rm/dm/devBackBad/backbaddetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>','950:680');
	}
	//修改
	function toModifyBackPlanPage(){
		...
		for(var index = 0;index<basedatas.length;index++){
			if(basedatas[index].proc_status == '9' ){
				stateflag = true;
				alertinfo = "您选择的记录中存在状态为'已提交'的单据,不能修改!";
				break;
			}
		}
		if(stateflag){
		...
		popWindow('<%=contextPath%>/rm/dm/devBackBad/backbaddetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+devicebackappid,'950:680');
	}
	//删除
	...
	function toSumbitDevApp(){
		...
		var devicebackappid;
		for(var index = 0;index<basedatas.length;index++){
			devicebackappid = basedatas[0].device_backapp_id;
			if(basedatas[index].proc_status == '9' ){
				stateflag = true;
				alertinfo = "您选择的记录中存在状态为'已提交'的单据,不能提交!";
				break;
			}
		}
		if(stateflag){
		...
		var retObject = syncRequest('Post',path,params);
		//更新队级台账中井中设备的维修状态
		var detsql = "update gms_device_account_dui dui set repair_state='1',repair_wells_back_id='"+devicebackappid+"',modifi_date=sysdate ";
