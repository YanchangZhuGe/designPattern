...
<%
    ...
    String splanId = request.getParameter("splanId");
...
var splanId = '<%=request.getParameter("splanId")%>';	
...
function page_init(){
	...
	if(plan_detail_id!='null'){
		var querySql = "select d.plan_detail_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,d.people_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1 ) nums,d.notes,d.profess_number from bgp_comm_human_plan_detail d left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0' and s1.coding_mnemonic_id='<%=projectType%>' left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0'  and s2.coding_mnemonic_id='<%=projectType%>' where d.plan_detail_id='"+plan_detail_id+"' and d.spare1='"+splanId+"' and d.bsflag='0' order by d.modifi_date  ";
...
function toDownload(){
	...
	var iName ="人力补充配置计划模板";  
	iName = encodeURI(iName);
	iName = encodeURI(iName);
	 
	if(projectTyp!="" && projectTyp !="null"){
		if(projectTyp =="5000100004000000006"){  //深海模板
			elemIF.src = "<%=contextPath%>/rm/em/humanRequest/download.jsp?path=/rm/em/singleHuman/supplyHumanPlan/importHumanPlanListSh.xls&filename="+iName+".xls";
		}else if (projectTyp =="5000100004000000009"){  // 综合模板
			elemIF.src = "<%=contextPath%>/rm/em/humanRequest/download.jsp?path=/rm/em/singleHuman/supplyHumanPlan/importHumanPlanListZh.xls&filename="+iName+".xls";
		}else { //其他类型
			elemIF.src = "<%=contextPath%>/rm/em/humanRequest/download.jsp?path=/rm/em/singleHuman/supplyHumanPlan/importHumanPlanListLd.xls&filename="+iName+".xls";
		}
	}
...
function exportData(){
	var obj=window.showModalDialog('<%=contextPath%>/rm/em/singleHuman/supplyHumanPlan/humanPlanImportFile.jsp?splanId=<%=splanId%>',"","dialogHeight:500px;dialogWidth:600px");
