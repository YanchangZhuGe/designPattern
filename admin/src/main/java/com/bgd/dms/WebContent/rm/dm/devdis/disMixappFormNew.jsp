...
<script type="text/javascript"> 
	...
	function refreshData(){
		...
		if('<%=devappid%>'!=null){
			//回填基本信息
			var str = "select t.project_info_no,t.mix_type_id,t.device_app_id,t.device_app_no,t.device_app_name,gp.project_name from gms_device_app t left join gp_task_project gp on t.project_info_no=gp.project_info_no and gp.bsflag='0' " 
			+"where t.device_app_id='<%=devappid%>'";
			...
			var str = "select appdet.device_app_detid,appdet.project_info_no,sd.coding_name as unitname,teamsd.coding_name as teamname, ";
				str += "p6.name as jobname,pro.project_name,nvl(tmp.mixed_num, 0)+nvl(tmp1.dis_mixed_num,0) mixed_num,";
				...
				str += "left join (select device_app_detid, sum(apply_num) as dis_mixed_num from gms_device_dismixapp_detail det left join gms_device_dismixapp app on app.device_mixapp_id=det.device_mixapp_id where app.bsflag='0' group by device_app_detid) tmp1 on tmp1.device_app_detid = appdet.device_app_detid ";
