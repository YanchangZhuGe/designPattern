...
<%
	...
	int len = orgsubid.length();
	String dbFlag = "Y";
	if(orgsubid.startsWith("C105006")){
		orgsubid = "C105006";
	}	
	if(len > 7){//双击页面判断：如果是作业部人员登录双击调配单不可用
		dbFlag = "N";
	}
      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			    <auth:ListButton functionId="F_DEV_001" css="zj" event="onclick='toAddDetailPage()'" title="调配"></auth:ListButton>
			    <auth:ListButton functionId="F_DEV_002" css="sc" event="onclick='toDelRecord()'" title="关闭"></auth:ListButton>
			    <auth:ListButton functionId="F_DEV_003" css="dc" event="onclick='exportData()'" title="导出excel"></auth:ListButton>
			  </tr>
			</table>
			</td>
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_mixinfo_id_{device_mixinfo_id}' name='device_mixinfo_id'>
			     	<td class="bt_info_even" exp="<input type='checkbox' name='selectedbox' mixstate='{opr_state}' mixtypeid='{mix_type_id}' value='{device_app_id}~{mix_type_id}' id='selectedbox_{device_app_id}' onclick='chooseOne(this)'/>" >选择</td>
					...
					<td class="bt_info_even" exp="{opr_state_desc}">处理状态</td>
					<td class="bt_info_odd" exp="<a onclick=viewMixedStore('{device_app_id}','{opr_state}','{mix_type_id}')>查看</a>">调配单查看</td>					
...
<script type="text/javascript">
	...
	function getContentTab(obj,index) { 
		...
		$("input[type='checkbox'][name='selectedbox']").each(function(){
			if(this.checked){
				...
				if(index == 3){
					$("#attachement").attr("src","<%=contextPath%>/doc/common/common_doc_list.jsp?relationId="+info[0]);
				}else if(index == 4){
					...
					var device_app_id = info[0];
					var mixtypeid = info[1];
					var curbusinesstype = "";
					var curbusinesstable = "";
					
					if(mixtypeid =='S14059999'){//仪器附属设备
							if(project_type == '5000100004000000008'){//井中
								curbusinesstype = "5110000004100001058";
							}else{
								curbusinesstype = "5110000004100000216";
							}
						curbusinesstable = "gms_device_app";
					}else{
							if(project_type == '5000100004000000008'){//井中
						...
						curbusinesstable = "gms_device_collapp";
					}
					//工作流信息
			    	processNecessaryInfo={        							//流程引擎关键信息
						businessTableName:curbusinesstable,    			//置入流程管控的业务表的主表表明
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				if(info[0]!=''){
					if(info[1] =='S14059999'){//仪器附属设备
						prosql = "select '调配明细' as showtype,";					
						prosql += "appdet.dev_name as device_name,appdet.dev_type as device_model,nvl(appdet.apply_num,0) device_num ";
						...
						str = "select appdet.device_app_detid ";
						...
						str += "where appdet.bsflag = '0' and devapp.bsflag = '0' and devapp.device_app_id = '"+info[0]+"' ";
						var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str+'&pageSize=1000');
					    	retObj = queryRet.datas;
						//先进行查询
					    prosql = "select '1' as type,'调配明细' as showtype,cds.device_name,cds.device_model,detail.coding_name as unitname, ";
						prosql += "unit_id,nvl(cds.device_slot_num,0) as device_slot_num,nvl(cds.device_num, 0) as device_num ";
						prosql += "from gms_device_app_colldetsub cds ";
						prosql += "left join comm_coding_sort_detail detail on detail.coding_code_id = cds.unit_id ";
						prosql += "where cds.device_app_detid = '"+retObj[0].device_app_detid+"' and nvl(cds.device_num,0)>0 ";
						prosql += "union all ";
						prosql += "select '2' as type,'补充明细' as showtype,mix.device_name,mix.device_model,mix.unit_name,'',0,nvl(mix.device_num, 0) as device_num ";
						prosql += "from gms_device_coll_mixsubadd mix left join comm_coding_sort_detail d on mix.team=d.coding_code_id ";
						prosql += "where mix.device_mixinfo_id='"+retObj[0].device_app_detid+"' and nvl(mix.device_num,0)>0 "
					}
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			...
			innerHTML += "<td>"+datas[i].device_model+"</td><td>"+datas[i].device_num+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
	...
	function refreshData(v_opr_state_desc, v_appinfo_name, v_project_name){
		var str = "select * from (select devapp.mix_type_id,devapp.modifi_date,devapp.project_info_no,devapp.device_app_id,devapp.device_app_no,to_char(devapp.appdate,'yyyy-mm-dd') as appdate,apporg.org_abbreviation as app_org_name,"
		...
		+"left join comm_org_information apporg on devapp.app_org_id=apporg.org_id and apporg.bsflag='0' "
		+"left join comm_org_information mixorg on devapp.mix_org_id=mixorg.org_id and mixorg.bsflag = '0' "
		+"left join comm_org_subjection orgsub on devapp.mix_org_id=orgsub.org_id and orgsub.bsflag='0' "
		+"left join common_busi_wf_middle comm on devapp.device_app_id=comm.business_id and comm.bsflag='0' "
		+"where devapp.state='9' and comm.proc_status='3' and devapp.bsflag='0' "
		+"and orgsub.org_subjection_id like '<%=orgsubid%>%' "
		+"union all "
		+"select devapp.mix_type_id,devapp.modifi_date,devapp.project_info_no,devapp.device_app_id,devapp.device_app_no,to_char(devapp.appdate, 'yyyy-mm-dd') as appdate,apporg.org_abbreviation as app_org_name,"
		+"tp.project_name,mixorg.org_abbreviation as mix_org_name,devapp.device_app_name,"
		+"case devapp.opr_state when '1' then '处理中' when '9' then '已处理' when '4' then '已关闭' else '未处理' end as opr_state_desc,devapp.opr_state "
		+"from gms_device_app devapp "
		+"left join common_busi_wf_middle wfmiddle on devapp.device_app_id=wfmiddle.business_id and wfmiddle.bsflag='0' "
		+"left join gp_task_project tp on devapp.project_info_no=tp.project_info_no " 
		+"left join comm_org_information apporg on devapp.app_org_id=apporg.org_id and apporg.bsflag='0' "
		+"left join comm_org_information mixorg on devapp.mix_org_id=mixorg.org_id and mixorg.bsflag='0' "
		+"left join comm_org_subjection orgsub on devapp.mix_org_id=orgsub.org_id and orgsub.bsflag='0' "
		+"where wfmiddle.proc_status='3' and devapp.bsflag='0' and devapp.mix_type_id = 'S14059999' "
		...
		str += "order by opr_state nulls first,modifi_date desc,project_info_no ";
	...
	function toAddDetailPage(){
		...
		if(info[0]!=''){
			if(info[1] == 'S14059999'){
				popWindow('<%=contextPath%>/rm/dm/collectDevMixNew/insDevMixInfoNew.jsp?devappid='+info[0]+'&oprstate='+mixstate,'1080:680');
			}else{
				popWindow('<%=contextPath%>/rm/dm/collectDevMixNew/collDevMixInfoNew.jsp?devappid='+info[0]+'&oprstate='+mixstate,'1080:680');
			}
		}
	}
	...
	function toDelRecord(){
		...
		if(confirm("是否执行关闭操作?")){
			...
			if(info[1] == 'S14059999'){
				sql = "update gms_device_app set opr_state='4' where device_app_id = '"+info[0]+"' ";
			}else{
				sql = "update gms_device_collapp set opr_state='4' where device_app_id = '"+info[0]+"' ";
			}
    ...
    function loadDataDetail(shuaId){
		...
		//alert(info);
		//根据ids去查找 申请单
		var str = "select tp.project_info_no,devapp.device_app_id,devapp.device_app_name,devapp.device_app_no,to_char(devapp.appdate,'yyyy-mm-dd') as appdate,apporg.org_abbreviation as app_org_name,"
			+"tp.project_name,mixorg.org_abbreviation as mix_org_name, "
			+"case devapp.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as opr_state_desc,devapp.opr_state ";
			if(info[1] == 'S14059999'){
			...
			+"where devapp.device_app_id='"+info[0]+"'";
    ...
    function viewMixedStore(id,state,typeid){
		if(state =='1' || state =='9' || state =='4'){
			if(state =='4'){
				var querySql = "";
				if(typeid == 'S14059999'){
					querySql = "select count(1) as app_num from gms_device_mixinfo_form mif ";
					...
					querySql = "select count(1) as app_num from gms_device_collmix_form mif ";
					querySql += "left join gms_device_collapp devapp on mif.device_app_id = devapp.device_app_id ";
					querySql += "where (mif.bsflag is null or mif.bsflag = '0') and devapp.state = '9' and devapp.bsflag = '0' and devapp.device_app_id = '"+id+"' ";
				}
			...
			if(id != ''){
				if(typeid == 'S14059999'){
					...
					popWindow('<%=contextPath%>/rm/dm/collectDevMixNew/collDevMixedInfoList.jsp?devAppId='+id,'1050:680');
				}										
			}			
		}else{
			alert("该单据尚未调配,暂不能查看调配单信息!");
