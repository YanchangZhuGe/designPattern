			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
					...
					<td class="bt_info_odd" exp="{device_backapp_no}">送修申请单号</td>
					<td class="bt_info_even" exp="{backapp_name}">送修申请单名称</td>
...
<script type="text/javascript">
    ...
    function refreshData(v_device_backapp_no, v_backapp_name, v_org_name){
		var str = "select colback.device_backapp_id,colback.device_backapp_no,colback.backapp_name,pro.project_name,";
			...
			//str += "where colback.bsflag = '0' and colback.project_info_id = '"+projectInfoNos+"' and backapptype='2' ";
			...
			if(projectType == "5000100004000000008" && retFatherNo.length >= 1)//子项目
			{
				...
				str += "where colback.bsflag = '0' and colback.project_info_id = '" +retFatherNo+"' and backapptype='2' ";
		...
		str += "order by colback.backdate desc ";
	...
	function toAddBackPlanPage(){
		popWindow('<%=contextPath%>/rm/dm/collectDevBackBad/backbaddetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>','950:680');
	}
	//修改
	function toModifyBackPlanPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/collectDevBackBad/backbaddetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+devicebackappid,'950:680');
	}
	//删除
	...
	function toSumbitDevApp(){
		...
		str += "from gms_device_collbackapp_detail backdet ";
		str += "left join gms_device_coll_account_dui account on backdet.dev_acc_id = account.dev_acc_id ";
		str += "where backdet.device_backapp_id='"+id+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		basedatas = queryRet.datas;
		...
		if(confirm("是否提交返还单？")){
			//调用后台服务保存日志信息
			jcdpCallService("DevCommInfoSrv","saveCollectSubmitInfo","devicebackappid="+id);		
			refreshData();
		}
	}
	...
	function toModifyDetail(){
		...
		window.location.href = '<%=contextPath%>/rm/dm/collectDevBackBad/backbadPlanDetailList.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+id;
	}
	<%-- //双击进入明细编辑
	function dbclickRow(shuaId){
		...
		window.location.href = '<%=contextPath%>/rm/dm/collectDevBackBad/backbadPlanDetailList.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+shuaId;
	} --%>
