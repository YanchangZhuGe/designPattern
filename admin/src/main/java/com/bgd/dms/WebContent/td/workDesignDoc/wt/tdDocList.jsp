			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      ...
			      <td class="bt_info_odd" exp="{title}">角点编号</td>	
			      <td class="bt_info_even" exp="{north_location}">角点北坐标</td>
			      <td class="bt_info_odd" exp="{south_location}">角点东坐标</td>
			     </tr> 			        
			  </table>
			</div>
...
<script type="text/javascript">
	...
	var businessType="5110000004100000092";
	var planId = "";
	...
	function refreshData(str){
		/**
		//查询是否审批
		var querySql ="select r.proc_status,t.project_name from gp_ws_tecnical_plan p left join gp_task_project t on p.project_info_no=t.project_info_no "+
			   "left join common_busi_wf_middle r on p.plan_id = r.business_id "+
		       "and r.bsflag = '0' where p.bsflag = '0' and p.project_info_no ='' and p.business_type='"+businessType+"' ";
		var queryRet = syncRequest('Post',''+appConfig.queryListAction,'querySql='+querySql);
		var datas = queryRet.datas;
		if(null!=datas&&datas.length>0){
			proc_status = datas[0].proc_status;
			projectName=datas[0].project_name;
		}*/
		...
		/**processNecessaryInfo={         
	    		...
	    		businessId:objPlan.planId,         //业务主表主键值
	    		businessInfo:"工区设计边框文档审核",        //用于待审批界面展示业务信息
	    		applicantDate:''       //流程发起时间
	    	}; 
	    	processAppendInfo={ 	    		 
   				projectInfoNo:projectInfoNo,
   				action:"view",
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/workDesignDoc/wt/add_tdDocModify.jsp?businessType='+businessType+'&fileAbbr='+fileAbbr,'750:680');
    }
	...
	function toEdit() {
		...
		popWindow('<%=contextPath%>/td/workDesignDoc/wt/add_tdDocModify.jsp?businessType='+businessType+'&fileAbbr='+fileAbbr+'&id='+id,'750:680');
	}
	...
	function excelDataAdd(){
		popWindow('<%=contextPath%>/td/workDesignDoc/wt/excelDataAdd.jsp?businessType='+businessType);
	}
	
	function excelDataDown(){
		//window.location="/doc/downloadDocByUcmId.srq?docId=66805&emflag=0";
		...
		elemIF.src = "<%=contextPath%>/rm/em/humanRequest/download.jsp?path=/td/workDesignDoc/wt/workDesignList.xls&filename="+iName+".xls";
