...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>单项目-计划管理-调剂设备计划申请_大港</title> 
 </head> 
...
<script type="text/javascript">
    ...
    function refreshData(v_project_name,v_org_name,v_state_desc){

		var str = "select wfmiddle.create_date as approve_date,devapp.allapp_type,pro.project_name,devapp.device_allapp_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.project_info_no,";
			...
			str += "where devapp.allapp_type = 'S9998' and devapp.bsflag = '0' and devapp.project_info_no='"+projectInfoNos+"' ";		
    ...
    function loadDataDetail(idinfo){

        var info = idinfo.split("~" , -1);
        var device_allapp_id = info[0];
    	var retObj;
        ...
        processNecessaryInfo={        							//流程引擎关键信息
        			...
        			businessType:"5110000004100001030",    //业务类型 即为之前设置的业务大类
        			businessId:device_allapp_id,           				//业务主表主键值
        			businessInfo:"大港设备调剂配置计划审批列表信息<配置计划单名称:"+device_allapp_name+">",
	...
	function toAddMainPlanPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devPlan/devallplan_new_dg.jsp?projectInfoNo=<%=projectInfoNo%>&allapp_type=S9998');
	}
	function toModifyMainPlanPage(){
		...
		if(unitRet.datas[0].proc_status == '' ){
			popWindow('<%=contextPath%>/rm/dm/devPlan/devallplan_modify_dg.jsp?projectInfoNo=<%=projectInfoNo%>&allapp_type=S9998&deviceallappid='+deviceallappid);
		}else{
			alert("本单据已提交，不能修改!");
			return;
		}
	...
	function toDelPlanPage(){
		...
		var str = "select devapp.device_allapp_id,nvl(wfmiddle.proc_status,'') as proc_status ";
			str += "from gms_device_allapp devapp  ";
			str += "left join common_busi_wf_middle wfmiddle on wfmiddle.business_id = devapp.device_allapp_id  ";
			str += "where devapp.bsflag = '0' and devapp.allapp_type = 'S9998' and devapp.project_info_no='"+projectInfoNos+"' ";
			str += "and devapp.device_allapp_id='"+deviceallappid+"' ";
		var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		//什么状态不能删除
	...
	function toSumbitDevApp(){
		...
		if(unitRet.datas[0].proc_status == '' || unitRet.datas[0].proc_status == '4'){
			...
			//var appdate_tmp =getdate();
			//var sql = "update gms_device_allapp set appdate = '"+appdate_tmp+"' where device_allapp_id='"+deviceallappid+"' ";
			//var path = cruConfig.contextPath+"/rad/asyncDelete.srq";
			//var params = "deleteSql="+sql;
			//	params += "&ids=";
			//var retObject = syncRequest('Post',path,params);
	...
	function dbclickRow(shuaId){
		...
		window.location.href='<%=contextPath%>/rm/dm/devPlan/devplan_app_frame.jsp?projectInfoNo=<%=projectInfoNo%>&allappType='+info[1]+'&deviceallappid='+info[0]+'&dgFlag='+dgFlag;		
		//window.location.href='<%=contextPath%>/rm/dm/devPlan/devplan_app_frame.jsp?projectInfoNo=<%=projectInfoNo%>&deviceallappid='+shuaId;
	} 
