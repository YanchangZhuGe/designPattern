...
<%
	...
	String zbFlag = "N";//是否为装备服务处人员登录，默认否
...
<script type="text/javascript">
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model){
		var str = "select comm.org_abbreviation,account.*,unitsd.coding_name as unit_name "
				+ "from gms_device_coll_account_dui account "
				+ "left join gms_device_coll_account acc on account.fk_dev_acc_id = acc.dev_acc_id and acc.bsflag = '0' "
				+ "left join comm_coding_sort_detail unitsd on account.dev_unit=unitsd.coding_code_id and unitsd.bsflag = '0' "
				+ "left join comm_org_information comm on account.out_org_id=comm.org_id and comm.bsflag='0' "
				+ "where nvl(account.bsflag,0) != '1' and account.project_info_id='"+projectInfoNos+"' "
				+ "and (account.is_leaving is null or account.is_leaving='0') ";
			if(zbflag == 'Y'){
				str += "and acc.usage_sub_id like '"+orgsubId+"#gmslike#' ";
			}
			if(obj.selectStr != null){
				str += "and account.dev_acc_id not in ("+obj.selectStr+") ";
			}
			if(v_dev_ci_name!=undefined && v_dev_ci_name!=''){
				str += "and account.dev_name like '#gmslike#"+v_dev_ci_name+"#gmslike#' ";
			}
			if(v_dev_ci_model!=undefined && v_dev_ci_model!=''){
				str += "and account.dev_model like '#gmslike#"+v_dev_ci_model+"#gmslike#' ";
			}
