...
<%
		...
		String typeName=type.equals("1")?"在帐物资":type.equals("2")?"可重复物资":"自采购物资";
			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_even" exp="<input name='coding_code_id_{wz_id}'  type='text' value='{coding_code_id}'/>">分类编码</td>
			      <td class="bt_info_odd" exp="<input name='wz_name_{wz_id}'  type='text' value='{wz_name}'/>">资源名称</td>
			      <td class="bt_info_even" exp="<input name='wz_prickie_{wz_id}'  type='text' value='{wz_prickie}'/>">计量单位</td>
			      <td class="bt_info_odd" exp="<input name='wz_price_{wz_id}'  type='text' value='{wz_price}' onkeyup='loadDataDetail()'/>">参考单价</td>
			      <td class="bt_info_even" exp="<input name='wz_number_{wz_id}'  type='text' value='' />">数量</td>
			      <td class="bt_info_even" exp="<input name='wz_type_{wz_id}' id='wz_type_{wz_id}' type='text' value=''/>">物资类型</td>
...
//	projectType ="5000100004000000009"; //测试 
	...
	function addData(value){
		...
		queryData(1);
		document.getElementById("wz_type_"+value).value='<%=typeName%>';
	}
	function toDelete(){
	  ...
	  function toSave(){ 
		  ids = getSelIds('rdo_entity_id');
			if (ids == '') {
				...
				document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/plan/saveMatItem.srq?type="+<%=type%>;
