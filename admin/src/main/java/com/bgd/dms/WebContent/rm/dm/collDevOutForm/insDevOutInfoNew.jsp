...
<title>多项目-设备出库-设备出库(按量装备设备)-仪器附属出库单添加界面</title>
</head>
<body class="bgColor_f3f3f3" onload="refreshData();">
<form name="form1" id="form1" method="post" action="">
<div id="new_table_box" style="width:98%">
  <div id="new_table_box_content" style="width:100%">
    <div id="new_table_box_bg" style="overflow-x:hidden;overflow-y:auto;width:96%;">
      <fieldset style="margin-left:2px;width:97%;" ><legend >基本信息</legend>
	  ...
	  <fieldset style="margin-left:2px;width:97%;"><legend>出库数量</legend>
...
$().ready(function(){
	...
	function submitInfo(state){
		...
		var forwardurl = "<%=contextPath%>/rm/dm/toSaveInsEquOutFormDetailInfo.srq?state="+state;
		...
		$("input[type='checkbox'][name='detinfo']").each(function(i){
			if(this.checked){
				...
				for(var j = 0;j<parseInt(outnum);j++){
					...
					innerhtml += "<td width='14%'><input name='detdevicetype"+seqinfo+"' id='detdevicetype"+seqinfo+"' value='"+devicemodel+"' style='line-height:18px;' size='10' type='text' readonly/><img src='<%=contextPath%>/images/magnifier.gif' width='16' height='16' style='cursor:hand;' onclick=showDevPage("+seqinfo+") /></td>";
	...
	function toAddAddedDetailInfos(){
		...
		innerhtml += "<td width='11%'><input name='addedplanstartdate"+addedseqinfo+"' id='addedplanstartdate"+addedseqinfo+"' value='"+planstartdate+"' style='line-height:18px;width:98%' type='text' readonly/>";
		innerhtml += "<td width='11%'><input name='addedplanenddate"+addedseqinfo+"' id='addedplanenddate"+addedseqinfo+"' value='"+planenddate+"' style='line-height:18px;width:98%' type='text' readonly/></td>";
	...
	function refreshData(){
		...
		if('<%=mixInfoId%>'!=null){
			//回填基本信息
			var mainsql = "select mix.device_mixinfo_id,mix.mixinfo_no,pro.project_name,mix.create_date,mix.in_org_id,insub.org_subjection_id in_sub_id,orgsub.org_subjection_id out_org_id,mix.project_info_no, "+
			...
			"left join comm_org_subjection insub on mix.in_org_id = insub.org_id "+
			...
			var prosql = "select amm.device_mix_subid,amm.dev_ci_code,amm.assign_num,amm.isdevicecode,";
				...
				prosql += "nvl(temp.outednum,0) outednum,unitsd.coding_name as unit_name,appdet.unitinfo, ";
				prosql += "appdet.plan_start_date,appdet.plan_end_date,amm.devremark ";
			...
			if(retObj!=undefined){
				for(var index=0;index<retObj.length;index++){
					...
					innerhtml += "<td width='20%'><input title='' name='devremark"+index+"' id='devremark"+index+"' value='"+retObj[index].devremark+"' size='28' type='text' readonly/></td>";
