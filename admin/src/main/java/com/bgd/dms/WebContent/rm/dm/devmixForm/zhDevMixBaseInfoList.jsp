			...
			if(currentid != undefined && idinfo == currentid){
				...
				if(currentid==undefined){
					...
					if(info[0]!=''){
						...
						prosql += "amd.asset_coding,amd.self_num,amd.dev_sign,amd.license_num,org.org_abbreviation,e.employee_name ";
						...
						prosql += "left join comm_human_employee e on e.employee_id = amd.operator_id ";
	...
	function appendDataToDetailTab(filterobj,datas){
		if(basedatas[0].dev_ci_name != null){
			for(var i=0;i<basedatas.length;i++){
				...
				innerHTML += "<td>"+datas[i].dev_sign+"</td><td>"+datas[i].license_num+"</td><td>"+datas[i].org_abbreviation+"</td><td>"+datas[i].employee_name+"</td>";
				innerHTML += "</tr>";
...
<script type="text/javascript">
	...
	function refreshData(v_appinfo_no,v_opr_state_desc, v_appinfo_name, v_project_name){
		var	str = "select distinct dev1.project_info_no,device_app_name,dev1.device_app_id,device_app_no,to_char(appdate, 'yyyy-mm-dd') as appdate,"
				...
				str += "order by p2.state nulls first,appdate desc,dev1.project_info_no "
	...
	function toAddDetailPage(){
		...
		var info = shuaId.split("~" , -1);
		if(info[1]!=''){
			popWindow('<%=contextPath%>/rm/dm/devmixForm/zhDevMixInfoFormNew.jsp?devappid='+info[1],'1050:680');
		}
	}
	...
	function toDelRecord(){
		...
		if(confirm("是否执行关闭操作?")){
			var sql = "update gms_device_app set opr_state='4' where device_app_id = '"+curvalue[1]+"' ";
    ...
    function viewProject(projectNo){
		if(projectNo != ''){
			popWindow('<%=contextPath%>/pm/project/multiProject/wt/viewProject.jsp?projectInfoNo='+projectNo,'1080:680');						
		}		
	}
</script>
</html>