		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<!-- <td class="ali_cdn_name">班组名称：</td>
		 	    ...
		 	    <td class="ali_cdn_input" style="width: 120px;"><select id="submite_type" value=""><option value="0">全部</option><option value="1">已提交</option><option value="2">未提交</option></select></td>
...
<script type="text/javascript">
	...
	function toDelete(){
	 	     	...
	 	     	return;
		 	    	...
		 	    	if(submiteIf =='3'){ alert("申请单已被审批!");
		 	     	return;
						...
						popWindow('<%=contextPath%>/mat/singleproject/plan/queryPlanDg.srq?laborId='+ids,'1024:800');
			}
			}
		    }
	 }
	 function toAdd(){ 
			  ...
			  popWindow("<%=contextPath%>/mat/singleproject/plan/repMatPlan/planAddItemListDg.jsp?taskObjectId="+taskObjectId+"&projectInfoNo="+projectInfoNo,'1024:800');
...
*/
		    ...
		    function simpleSearch(){
		    	   var sql =" select t.submite_number,t.create_date,t.total_money,t.submite_id,u.user_name,o.org_abbreviation,d.coding_name,decode(t.if_submit_dg, '0', '未提交', '1', '已提交','','未提交') if_submit_dg,decode(t.if_purchase,'0','是','1','否') if_purchase  from gms_mat_demand_plan_bz t inner join comm_org_information o on t.org_id=o.org_id and o.bsflag='0' inner join p_auth_user u on t.creator_id=u.user_id left join comm_coding_sort_detail d on t.s_apply_team=d.coding_code_id and d.bsflag='0' left join common_busi_wf_middle m on t.submite_number=m.business_id and m.bsflag='0' where t.bsflag='0'and t.task_object_id='"+taskObjectId+"'and t.project_info_no='"+projectInfoNo+"' and t.if_purchase='2' ";
					var submite_id = document.getElementById("submite_id").value;
					var submite_type = document.getElementById("submite_type").value;
					if(submite_id!=null){
						sql+="and submite_id like '%"+submite_id+"%'";
						}
					if(submite_type==1){
						sql+="and if_submit_dg = '1'";
					}else if(submite_type==0){
					}else{
						sql+="and if_submit_dg = '0'";
						}
