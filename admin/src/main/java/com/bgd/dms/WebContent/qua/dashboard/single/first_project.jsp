...
<div style="overflow-y: hidden; overflow-x: hidden;">
	    	...
	    	if(retObj.datas!=null ){
    			...
    			var j =0;
   				...
   				for(var i=0;i<retObj.datas.length && retObj.datas[i]!=null;i++){
    				...
    				"<p class='small'>一级品率≥<span id='first_percent_"+i+"'>0</span>%;累计完成炮数:<span id='total_"+i+"'>0</span>;</p>"+
	...
	function getTotal(index,project_info_no){
		var querySql = " select t.firstlevel_radio first ,t.qualified_radio qualified ,t.waster_radio waster ,t.all_miss_radio miss"+
		...
		querySql = "select decode(sum(t.daily_acquire_qualified_num),'0',sum(t.daily_acquire_firstlevel_num)-(-sum(t.collect_2_class)),sum(t.daily_acquire_qualified_num)) qualified ,sum(t.collect_waster_num) waster ,(case when sum(t.daily_acquire_firstlevel_num) is null then 0 else sum(t.daily_acquire_firstlevel_num) end) first ,"+
		...
		" sum(t.collect_miss_num) miss from gp_ops_daily_report t where t.bsflag='0' and t.project_info_no ='"+project_info_no+"'"
