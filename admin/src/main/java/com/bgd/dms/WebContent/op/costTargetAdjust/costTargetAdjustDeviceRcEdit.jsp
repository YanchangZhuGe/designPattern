 ...
 <div id="new_table_box" align="center">
  <div id="new_table_box_content"> 
    ...
    <table  width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
	    ...
	    <tr>	
	    	<td class="inquire_item4">设备原值:</td>
	    	...
	    	<td class="inquire_item4">设备折旧年限:</td>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		var querySql = "select * from bgp_op_tartet_device_depre where bsflag='0' and if_change != '2' and if_delete_change is null and target_depre_id = '"+id+"'";
		...
		if(retObj!=null && retObj.returnCode=='0' && retObj.datas!=null && retObj.datas[0]!=null){
			...
			with(map){
				document.getElementById("target_depre_id").value = target_depre_id;
				...
				document.getElementById("asset_value").value = asset_value;
				document.getElementById("depreciation_value").value = depreciation_value;
	...
	function updateDatas(dev_acc_id){
		...
		" t.asset_value='"+asset_value+"',t.depreciation_value='"+depreciation_value+"',t.dev_team='"+dev_team+"',t.change_date = sysdate "+
		" where t.target_depre_id='"+target_depre_id+"';";
	...
	function insertDatas(){
		...
		sql = "insert into bgp_op_tartet_device_depre(target_depre_id,project_info_no,dev_name,dev_model,plan_start_date,plan_end_date,asset_value,"+
		"depreciation_value,org_id,org_subjection_id,creator,create_date,updator,update_date,bsflag,if_change,record_type,dev_team,device_count,"+
		"dev_acc_id,change_date) values((lower(sys_guid())),'<%=project_info_no%>','"+dev_name+"','"+dev_model+"',to_date('"+plan_start_date+"','yyyy-MM-dd'),"+
		"to_date('"+plan_end_date+"','yyyy-MM-dd'),'"+asset_value+"','"+depreciation_value+"','<%=org_id%>','<%=org_subjection_id%>',"+
		"'<%=user_id%>',sysdate,'<%=user_id%>',sysdate,'0','1','0','"+dev_team+"','"+device_count+"','"+dev_acc_id+"',sysdate);";
