...
<script type="text/javascript">
	...
	function refreshData(v_dev_name,v_dev_model){
		var str = "select backdet.*,dui.dev_name,dui.dev_model,dui.check_time,sd.coding_name as stat_desc, "
				+ "case backdet.state when '0' then '未验收' when '1' then '已验收' end as isleaving, backapp.BACKDEVTYPE "
				+ "from gms_device_backapp_detail backdet "
				+ "left join gms_device_account_dui dui on backdet.dev_acc_id=dui.dev_acc_id "
				+ "left join comm_coding_sort_detail sd on dui.account_stat=sd.coding_code_id "
				+ "left join gms_device_backapp backapp on backapp.device_backapp_id=backdet.device_backapp_id "
				+ "where backdet.device_backapp_id='<%=devicebackinfoid%>' ";
		//补充查询条件
	...
	function toSumbitDevApp(){
		...
		if(basedatas.length == 0){			
			var querysql = "select * from gms_device_backapp app "
						  + "left join gms_device_backapp_detail det on det.device_backapp_id=app.device_backapp_id "
						  + "where app.backdevtype='S0623' and det.device_backdet_id in ( ";		
			for(var i=0;i<selId.length;i++){
				if(idss!="") idss += ",";
				idss += "'"+selId[i]+"'";
			}
			...
			var basedatas = queryRet.datas;
			if(basedatas.length>0){
				popWindow('<%=contextPath%>/rm/dm/EqDevstore/stockinKKZYSubmit.jsp?id='+selId,'800:680');
			}else{
				popWindow('<%=contextPath%>/rm/dm/EqDevstore/stockinSubmit.jsp?id='+selId+'&devBakAppId=<%=devicebackinfoid%>','800:680');
			}			
		}else{
			alert("存在已验收的设备！");
			return;
