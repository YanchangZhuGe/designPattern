...
function save(){
	...
	// 检查是否已存在
	var checkSql="select * from bgp_wr_record where to_char(week_date,'yyyy-MM-dd')='" + week_date + "' and create_org_id='<%=user.getOrgId()%>' and bsflag='0'";
    var queryRet = syncRequest('Post','<%=request.getContextPath()%>'+appConfig.queryListAction,'querySql='+checkSql);
	var datas = queryRet.datas;
	var record_id="";
	var subflag="0";
	var action="add";
	if(datas && datas[0] && datas[0].record_id){// 数据已存在
		record_id = datas[0].record_id;
	...
	if(subflag=="1"){// 数据已提交
	    ...
	    //return;
		window.location="editIndex.jsp?week_date="+week_date+"&week_end_date="+week_end_date+"&org_id=<%=user.getCodeAffordOrgID()%>&action=view";
	}else{

		// 提交数据
		var submitStr='JCDP_TABLE_NAME=bgp_wr_record&JCDP_TABLE_ID=&record_id='+record_id;
		submitStr += '&week_date=' + week_date;
		submitStr += '&week_end_date=' + week_end_date;
		submitStr += '&create_org_id=<%=user.getOrgId()%>';
		submitStr += '&create_org_subjection_id=<%=user.getOrgSubjectionId()%>';
		submitStr += '&org_id=<%=user.getCodeAffordOrgID()%>';
		submitStr += '&org_subjection_id=<%=user.getSubOrgIDofAffordOrg()%>';
		submitStr += '&create_user=' + encodeURI(encodeURI('<%=user.getUserName()%>'));
		submitStr += '&mondify_user=' + encodeURI(encodeURI('<%=user.getUserName()%>'));
		submitStr += '&bsflag=0&subflag=0';
		...
		if (retObject.returnCode != "0") alert('提交失败');
			...
			window.location="editIndex.jsp?week_date="+week_date+"&week_end_date="+week_end_date+"&org_id=<%=user.getCodeAffordOrgID()%>&action="+action;
		}
	}
}
