...
<%
	...
	if(!"C105".equals(orgSubId)) {
		//非局级用户
		request.getRequestDispatcher("/pm/dailyReport/multiProject/dailyDataAnalysisList.jsp?orgSubjectionId="+orgSubId+"&orgId="+orgId).forward(request,response);
	}
...
<frameset rows="*" cols="300,*" frameborder="no" border="0" framespacing="0" framespacing="0">
  ...
  <frame src="<%=contextPath %>/pm/dailyReport/multiProject/dailyDataAnalysisList.jsp" name="mainRightframe" frameborder="no" scrolling="auto"  id="mainRightframe" style="border-left: 2px solid #5796DD; cursor: w-resize;"/>
</frameset>
