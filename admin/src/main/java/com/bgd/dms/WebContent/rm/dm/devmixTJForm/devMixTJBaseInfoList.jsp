      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			    <auth:ListButton functionId="" css="zj" event="onclick='toAddDetailPage()'" title="调剂"></auth:ListButton>
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_mixinfo_id_{device_mixinfo_id}' name='device_mixinfo_id'>
			     	<td class="bt_info_even" exp="<input type='checkbox' name='selectedbox' mixstate='{state}' value='{device_mixinfo_id}~{device_mixapp_id}' id='selectedbox_{device_mixinfo_id}~{device_mixapp_id}' />" >选择</td>
					...
					<td class="bt_info_even" exp="{mix_org_name}">调剂单位</td>
					<td class="bt_info_odd" exp="{mix_state_desc}">调剂状态</td>
					<td class="bt_info_even" exp="{mixinfo_no}">调剂单号</td>
					<td class="bt_info_odd" exp="{out_org_name}">转出单位</td>
					<td class="bt_info_even" exp="{employee_name}">调剂人</td>
					<td class="bt_info_odd" exp="{mix_date}">调剂时间</td>
			     </tr> 
			  </table>
			</div>
...
<script type="text/javascript">
	...
	function refreshData(v_opr_state_desc,v_mixinfo_no,v_appinfo_name, v_project_name){
		var str = "select devapp.device_mixapp_id,devapp.device_mixapp_no,to_char(devapp.appdate,'yyyy-mm-dd') as appdate,apporg.org_name as app_org_name,"
		+"mif.device_mixinfo_id,mif.mixinfo_no,devapp.device_mixapp_name,tp.project_name,mixorg.org_abbreviation as mix_org_name,"
		...
		+"case mif.state when '0' then '调剂中' when '9' then '已调剂' else '待调剂' end as mix_state_desc,mif.state "
		+"from gms_device_dismixapp devapp "
		+"left join gp_task_project tp on devapp.project_info_no=tp.project_info_no " 
		+"left join gms_device_mixinfo_form mif on (mif.device_app_id = devapp.device_mixapp_id or mif.device_app_id = devapp.device_app_id) and mif.mixform_type='3' " 
		...
		+" left join comm_org_subjection mixsub on devapp.mix_org_id = mixsub.org_subjection_id left join comm_org_information mixorg on mixsub.org_id = mixorg.org_id "
		...
		+"where devapp.state='9' and devapp.bsflag='0' and (devapp.mix_type_id = 'S9998' or devapp.mix_type_id='<%=DevConstants.MIXTYPE_COMMON%>') and (mif.bsflag is null or mif.bsflag='0') ";
		//加机构权限
		str += "and devapp.mix_org_id like '<%=orgsubid%>%' ";
		//if(v_appinfo_no!=undefined && v_appinfo_no!=''){
		//	str += "and devapp.device_mixapp_no like '%"+v_appinfo_no+"%' ";
		//}
		...
		if(v_appinfo_name!=undefined && v_appinfo_name!=''){
			str += "and devapp.device_mixapp_name like '%"+v_appinfo_name+"%' ";
		}
		if(v_project_name!=undefined && v_project_name!=''){
			str += "and tp.project_name like '%"+v_project_name+"%' ";
		}
		str += "order by mif.state nulls first,mif.modifi_date desc,devapp.project_info_no ";
	...
	function toAddDetailPage(){
		...
		var info = shuaId.split("~" , -1);
		if(info[1]!=''){
			popWindow('<%=contextPath%>/rm/dm/devmixTJForm/devMixInfoTJFormNew.jsp?devappid='+info[1],'1000:680');
		}
	}
	function toModifyDetailPage(){
		...
		var info = shuaId.split("~" , -1);
		if(info[0]!=''){
			popWindow('<%=contextPath%>/rm/dm/devmixTJForm/devMixInfoTJFormModify.jsp?mixInfoId='+info[0]+'&devappid='+info[1],'1000:680');
		}else{
			alert("您尚未开据本调剂申请单对应的调剂单，请查看!");
			return;
		}
	}
    ...
    function loadDataDetail(shuaId){
		...
		if(info[0]!=''){
			//根据ids去查找
			var str = "select devapp.device_mixapp_id,devapp.device_mixapp_no,to_char(devapp.appdate,'yyyy-mm-dd') as appdate,apporg.org_abbreviation as app_org_name,"
			+"mif.device_mixinfo_id,mif.mixinfo_no,tp.project_name,mixorg.org_abbreviation as mix_org_name,"
			+"outorg.org_abbreviation as out_org_name,he.employee_name,to_char(mif.modifi_date,'yyyy-mm-dd') as mix_date,"
			+"case mif.state when '0' then '调剂中' when '9' then '已调剂' else '待调剂' end as mix_state_desc,mif.state "
			+"from gms_device_dismixapp devapp "
			+"left join gp_task_project tp on devapp.project_info_no=tp.project_info_no " 
			+"left join gms_device_mixinfo_form mif on (mif.device_app_id = devapp.device_mixapp_id or mif.device_app_id = devapp.device_app_id) " 
			+"left join comm_human_employee he on mif.print_emp_id=he.employee_id "
			+"left join comm_org_information apporg on devapp.app_org_id=apporg.org_id "
			+"left join comm_org_subjection mixsub on devapp.mix_org_id = mixsub.org_subjection_id left join comm_org_information mixorg on mixsub.org_id = mixorg.org_id "
			+"left join comm_org_subjection outsub on mif.out_org_id=outsub.org_subjection_id left join comm_org_information outorg on outsub.org_id = outorg.org_id "
			...
			//取消选中框--------------------------------------------------------------------------
			//取消其他选中的
			$("input[type='checkbox'][name='selectedbox'][id!='selectedbox_"+retObj[0].device_mixinfo_id+"~"+retObj[0].device_mixapp_id+"']").removeAttr("checked");
			//选中这一条checkbox
			$("input[type='checkbox'][name='selectedbox'][id='selectedbox_"+retObj[0].device_mixinfo_id+"~"+retObj[0].device_mixapp_id+"']").attr("checked",'true');
			//------------------------------------------------------------------------------------
			...
			//根据ids去查找 申请单
			var str = "select devapp.device_mixapp_id,devapp.device_mixapp_no,to_char(devapp.appdate,'yyyy-mm-dd') as appdate,apporg.org_abbreviation as app_org_name,"
			+"tp.project_name,mixorg.org_abbreviation as mix_org_name,'待调剂' as mix_state_desc "
			+"from gms_device_dismixapp devapp "
			+"left join gp_task_project tp on devapp.project_info_no=tp.project_info_no " 
			+"left join comm_org_information apporg on devapp.app_org_id=apporg.org_id "
			+"left join comm_org_subjection mixsub on devapp.mix_org_id = mixsub.org_subjection_id left join comm_org_information mixorg on mixsub.org_id = mixorg.org_id "
			+"where devapp.state='9' and devapp.device_mixapp_id='"+info[1]+"'";
			var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
			retObj = unitRet.datas;
			//取消其他选中的
			$("input[type='checkbox'][name='selectedbox'][id!='selectedbox_~"+retObj[0].device_mixapp_id+"']").removeAttr("checked");
			//选中这一条checkbox
			$("input[type='checkbox'][name='selectedbox'][id='selectedbox_~"+retObj[0].device_mixapp_id+"']").attr("checked",'true');
			//选中这一条checkbox
			//------------------------------------------------------------------------------------
			$("#project_name").val(retObj[0].project_name);
			$("#device_app_no").val(retObj[0].device_mixapp_no);
