      	...
      	<div id="list_table">
			...
			<div id="tab_box" class="tab_box">
				...
				<div id="tab_box_content7" name="tab_box_content7" class="tab_box_content" style="display:none;">
			      ...
			      <table id ='byMap'width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
					  	<tr align="right">
					  		<td class="ali_cdn_name" ></td>
					  		<td class="ali_cdn_input" ></td>
					  		<td class="ali_cdn_name" ></td>
					...
					<div style="overflow:auto">
				      	<table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
						  	<tr align="right">
								...
								<auth:ListButton functionId="" css="xg" event="onclick='toEditDJ()'" title="更换操作手"></auth:ListButton>
...
<script type="text/javascript">
	...
	function toAddQZBY(){
		...
		var ids = getSelIds('rdo_entity_id');
	 	...
	 	popWindow("<%=contextPath%>/rm/dm/device-xd/qzby.jsp?ids="+ids,'950:680'); 
    ...
    function toViewQZBY(obj){
    	...
    	popWindow("<%=contextPath%>/rm/dm/device-xd/qzbydetail.jsp?ids="+ids+"&repair_info="+repairinfoval,'950:680'); 
        }
    //修改界面
     function toEditQZBY(obj){ 
    	...
    	popWindow("<%=contextPath%>/rm/dm/device-xd/qzby.jsp?ids="+ids+"&repair_info="+repairinfoval,'950:680'); 
	 }
...
// 					newTd1.innerText=columnsObj(2).innerText; 
					var newTd1=newTr.insertCell();
					newTd1.innerText="第"+retObj[i].plan_num+"次";
	...
	function yzjl(shuaId){
		if (shuaId != null) {
			var querySql="select t.operation_info_id,t.dev_acc_id,t.modify_date,round(t.mileage,2) mileage,"+
			"round(sum(t.mileage) over(order by t.modify_date),2) as mileage_total,round(t.drilling_footage,2) drilling_footage,"+
			"round(sum(t.drilling_footage) over(order by t.modify_date),2) as drilling_footage_total,"+
			"round(t.work_hour,2) work_hour,round(sum(t.work_hour) over(order by t.modify_date),2) as work_hour_total from GMS_DEVICE_OPERATION_INFO t";
	...
	function loaddata(ids,index){
		...
		//根据设备id隐藏操作权限按钮(基本信息,保养计划,日常检查,状态监测记录,油水记录[没有按钮不用控制],附件,备注,分类码[不用控制]  )
		if(index!=0&index!=15&index!=16&index!=11&index!=3&index!=4&index!=5){
			hideOrShowOperButtonBydevAccId(ids,index);
		}
	 ...
	 function hideOrShowOperButtonBydevAccId(devAccId,index){
		if(devAccId==null || devAccId==''){
			hideOperButton();
		}else{
			//如果设备所属组织为装备服务处,隐藏操作权限按钮
			var queryDevOrgSql = "select nvl(count(*), 0) dev_count from gms_device_account_dui t where t.bsflag = '0' and t.owning_sub_id like  'C105006#gmslike#'";
			queryDevOrgSql += " and t.dev_acc_id ='"+devAccId+"'"; 
			var devOrgRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+queryDevOrgSql);
			var dev_count=devOrgRet.datas[0].dev_count;
			if(dev_count!=0){
				//隐藏操作权限按钮
				hideOrShowOperButton(true,index);
			}else{
				//显示操作权限按钮
				hideOrShowOperButton(false,index);
			}
		}
	 }
	 ...
	 function toAddBY(){   
		var ids = getSelIds('rdo_entity_id');
	 	...
	 	popWindow("<%=contextPath%>/rm/dm/device-xd/by.jsp?ids="+ids+"&flag=add"); 	
	 }
	 ...
	 function toEditBY(){   
		...
		popWindow("<%=contextPath%>/rm/dm/device-xd/by_edit.jsp?ids="+ids); 	
	}
