				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<div class="tongyong_box_title"><span class="kb"><a
							href="#"></a></span><a href="#">一线员工用工性质分析</a>
...
<script type="text/javascript">
 ...
 function getFusionChart(){
	...
	myChart1.setXMLUrl("<%=contextPath%>/cache/rm/em/getChart7.srq?orgId=<%=orgId%>");      
	myChart1.render("chartContainer1"); 

	var myChart2 = new FusionCharts( "${applicationScope.fusionChartsURL}/Charts/Pie3D.swf", "myChartId2", "100%", "250", "0", "0" );    
	myChart2.setXMLUrl("<%=contextPath%>/cache/rm/em/getChart5.srq?orgId=<%=orgId%>");      
	myChart2.render("chartContainer2"); 

	var myChart3 = new FusionCharts( "${applicationScope.fusionChartsURL}/Charts/MSStackedColumn2D.swf", "myChartId3", "100%", "250", "0", "0" );    
	myChart3.setXMLUrl("<%=contextPath%>/cache/rm/em/getChart4.srq");      
 ...
 function changeOrg1(){
     ...
     chartReference.setXMLUrl("<%=contextPath%>/cache/rm/em/getChart7.srq?orgId="+s_org_id);
 }
 ...
 function changeOrg2(){
     ...
     chartReference.setXMLUrl("<%=contextPath%>/cache/rm/em/getChart5.srq?orgId="+s_org_id);
 }
 ...
 function myJS(orgSub){
	 var orgsubId=orgSub.split('_')[1]; 
	 var paramS=orgSub.split('_')[0];  
	 popWindow('<%=contextPath%>/rm/em/humanChart/orgHumanList.jsp?orgSub='+orgsubId+'&paramS='+paramS,'850:770');
 }
 
 function myJS1(orgSub){  
	 var orgsubId=orgSub.split('_')[1]; 
	 var paramS=orgSub.split('_')[0];              
	 popWindow('<%=contextPath%>/rm/em/humanChart/humanChart15.jsp?orgSub='+orgsubId+'&paramS='+paramS,'850:700');
 }
 
 function myJS2(nums){
	 var paramS=nums.split('_')[0]; 
	 var nums_sub=nums.split('_')[1];  
	 var org_sub=nums.split('_')[2];  
	 popWindow('<%=contextPath%>/rm/em/humanChart/orgProjectList.jsp?nums='+nums_sub+'&paramS='+paramS+'&org_sub='+org_sub,'850:770');
 }
