...
<script type="text/javascript">
	...
	function refreshData(){
		...
		var querySql = "select * from bgp_op_target_device_rent where bsflag='0' and if_change != '2' and target_rent_id = '"+id+"'";
		...
		if(retObj!=null && retObj.returnCode=='0' && retObj.datas!=null && retObj.datas[0]!=null){
			...
			with(map){
				document.getElementById("target_rent_id").value = target_rent_id;
	...
	function newSubmit() {
		...
		if(target_rent_id!=null && target_rent_id!=''){
			...
			if(if_change!=null && if_change !='0'){
				...
				sql = insertDatas();
				sql += "update bgp_op_target_device_rent t set t.if_delete_change='1' where t.target_rent_id='"+target_rent_id+"';";
	...
	function updateDatas(dev_acc_id){
		...
		" t.taxi_unit='"+taxi_unit+"',t.dev_team='"+dev_team+"',t.change_date = sysdate "+
		" where t.target_rent_id='"+target_rent_id+"';";
	...
	function insertDatas(){
		...
		sql = "insert into bgp_op_target_device_rent(target_rent_id,project_info_no,dev_name,dev_model,plan_start_date,plan_end_date,taxi_unit,"+
		"org_id,org_subjection_id,creator,create_date,updator,update_date,bsflag,if_change,dev_team,device_count,dev_acc_id,change_date)"+
		" values((lower(sys_guid())),'<%=project_info_no%>','"+dev_name+"','"+dev_model+"',to_date('"+plan_start_date+"','yyyy-MM-dd'),"+
		"to_date('"+plan_end_date+"','yyyy-MM-dd'),'"+taxi_unit+"','<%=org_id%>','<%=org_subjection_id%>',"+
