...
<body style="background:#fff" onload="refreshData()">
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <%if(proc_status){ %>
			   <auth:ListButton functionId="OP_TARGET_ME_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_ME_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_ME_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_ME_EDIT" css="dr" event="onclick='importDatas()'" title="JCDP_btn_import"></auth:ListButton>
			   <%} %>
			  </tr>
			</table>
			</td>
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getMataxiShareInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
			...
			var td = tr.insertCell();
			if(datas[i].devModel=='ARIESI'){
				td.innerText= "ARIESI&II";
			}else{
				td.innerText= datas[i].devModel;
			}
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareMataxiEdit.jsp");
}
function toModify(){
	...
	if (ids == '') {
		...
		popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareMataxiEdit.jsp?target_mataxi_id="+ids);
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	ids = ids.replace(/\,/g,"','");
	var sql = "update BGP_OP_TARGET_DEVICE_MATAXI t set t.bsflag='1' where t.target_mataxi_id in('"+ids+"')";
 ...
 function getSum(){
		...
		var querySql = "select sum(nvl(t.dev_count*t.taxi_unit*(t.plan_end_date-t.plan_start_date+1)*t.taxi_ratio,0)) sum_value"+
