...
cruConfig.contextPath =  "<%=contextPath%>";
	...
	if(retObj.map == undefined || retObj.map.baseline_plan_object_id == undefined){
		...
		if(temp.datas != null){
				...
				document.getElementById("project_name").innerHTML = '&nbsp;'+map.project_name+'<input type="button" value="选择目标计划" onclick="toSelectProject()" <%if(action == "view" || "view".equals(action)){ %> disabled="disabled"  readonly="readonly" <%} %>/>';
		} else {
			document.getElementById("project_name").innerHTML = '&nbsp;<input type="button" value="选择目标计划" onclick="toSelectProject()" <%if(action == "view" || "view".equals(action)){ %> disabled="disabled"  readonly="readonly" <%} %>/>';
		}
		var temp = jcdpCallService("ProjectPlanSrv", "getActivityWt", "objectId="+retObj.map.baseline_plan_object_id+"&projectInfoNo="+ids);
		list = temp;
		if(temp.datas != null) {
			var j = 0;
			var k = 0;
...
function updateData(projectObjectId,targetProjectObjectId){
	...
	if(temp.datas != null){
			debugger;
			var map = temp.datas[0];
			document.getElementById("project_name").innerHTML = "<font color='red'>&nbsp;"+map.project_name+"</font>"+'<input type="button" value="选择目标计划" onclick="toSelectProject()" <%if(action == "view" || "view".equals(action)){ %> disabled="disabled"  readonly="readonly" <%} %>/>';
