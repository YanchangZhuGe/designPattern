...
<body style="background:#fff;overflow-y: auto" >
	...
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			<tr>
			  <td class="bt_info_odd" exp="<input type='checkbox' name='chk_entity_id' value='{contract_id}' onclick=check(this)/>" >
			  	<input type='checkbox' name='chk_entity_id' value='' onclick='check(this)'/></td>
			  <td class="bt_info_even" autoOrder="1">序号</td> 
			  <td class="bt_info_odd" exp="{contract_name}">合同名称</td>
			  <td class="bt_info_even" exp="{contract_money}">合同金额</td>
			  <td class="bt_info_odd" exp="{org_name}">乙方单位</td>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.queryStr = "select t.contract_id ,t.contract_name ,t.contract_money ,t.contract_bid ,t.workload ,t.execution ,t.start_date ,t.end_date ,"+
		...
		cruConfig.currentPageUrl = "<%=contextPath%>/market/crm/corrivalInformation/contractList.jsp";
		queryData(1);
	...
	function toAdd() { 
		...
		popWindow("<%=contextPath%>/market/crm/corrivalInformation/contractEdit.jsp?company_id="+company_id);
	}
	function toEdit() {
		var company_id ='<%=company_id%>';
		if(company_id==null || company_id==''){
		...
		for(var i=1;i<obj.length;i++){
			if(obj[i].checked ==true){
				var contract_id = obj[i].value;
				var text = '你确定要修改第'+i+'行吗?';
				if(contract_id!=null && window.confirm(text)){
					popWindow("<%=contextPath%>/market/crm/corrivalInformation/contractEdit.jsp?contract_id="+contract_id+"&company_id="+company_id);
					return;
				}
			}
		}
	...
	function toDel() {
		...
		if(window.confirm('你确定要删除吗?')){
			for (var i = objLen-1;i > 0;i--){  
				if (obj [i].checked == true) { 
					contract_id=obj [i].value;
					substr = substr + "update bgp_market_contract t set t.bsflag ='1' where t.contract_id ='"+contract_id+"';" 
				}   
			} 
		}
		if(substr!=''){
