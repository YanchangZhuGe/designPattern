...
<%
	...
	String back_dev_type = request.getParameter("back_dev_type");
	//用户机构对应的orgsubid信息
...
<input type="hidden" id="back_dev_type" name="back_dev_type" value="<%=back_dev_type %>" />
		...
		<fieldset style="margin-left:2px"><legend >验收信息</legend>
			 ...
			 <tr style="display:none" id="inCountry">
				...
				<td class="inquire_form6" colspan="3">
				 <input class="input_width"  id="province_in"  name="province_in" style="width: 30%; float: none;margin-bottom: 7px;height:20px;line-height:20px;"/>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<input id="dev_position_in" name="dev_position_in" class="input_width" type="text" style="width: 60%; float: none;margin-bottom: 7px;"/>
				</td>
			</tr>
...
$(function() {
    $('#province_in').autocomplete(emails, {
        max: 120000,    //列表里的条目数
...
var emails = [
	...
	function loadDataDetail(){
	    ...
	    var querySql = "select pro.project_country from gms_device_backapp_detail backdet ";
	    	querySql += "left join gms_device_account_dui dui on backdet.dev_acc_id = dui.dev_acc_id ";
	    	querySql += "left join gp_task_project pro on dui.project_info_id = pro.project_info_no ";
		...
		if(device_backdet_id!=null){
		    var querySql="select oper.operator_name,pro.project_country,backdet.*,dui.dev_name,dui.fk_dev_acc_id,dui.dev_model,";
		    	querySql+="dui.project_info_id,sd.coding_name as stat_desc ";
		    	querySql+="from gms_device_backapp_detail backdet ";
		    	querySql+="left join gms_device_account_dui dui on backdet.dev_acc_id=dui.dev_acc_id ";
		    	querySql+="left join comm_coding_sort_detail sd on dui.account_stat = sd.coding_code_id ";
		    	querySql+="left join gp_task_project pro on dui.project_info_id = pro.project_info_no ";
		    	querySql+="left join gms_device_equipment_operator oper on oper.device_account_id = dui.dev_acc_id ";
				querySql+="where backdet.bsflag='0' ";					
				querySql+="and backdet.device_backdet_id in (";
				for(var i=0;i<temp.length;i++){
					if(idss1!="") idss1 += ",";
	...
	function submitInfo(){
	    ...
	    if(confirm("确认提交吗？")){
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSubmitStock.srq";
