      	...
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <auth:ListButton functionId="OP_ADJUST_OIL_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			    <auth:ListButton functionId="OP_ADJUST_OIL_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			    <auth:ListButton functionId="OP_ADJUST_OIL_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			    <%} %>
			  </tr>
			...
			<div id="table_box">
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			            ...
			            <td class="bt_info_odd">日消耗油料类型</td>
			            <td class="bt_info_even">日消耗油料 （公斤）</td>
			            <td  class="bt_info_odd">日消耗小油品 （元/天）</td>
			            <td class="bt_info_even">单井消耗油料（公斤）</td>
			            <td  class="bt_info_odd">油料单价(元/公斤)</td>
			            <td  class="bt_info_even" onclick="getSum(1)">本项目承担油料费(元)</td>
			            <td  class="bt_info_odd" onclick="getSum(2)">本项目承担小油品费用(元)</td>
			        </tr>             
			   </table>
			</div>
			<div id="fenye_box">
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getTargetAdjustOilInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
			...
			var td = tr.insertCell();
			td.innerHTML = datas[i].dailyOilTypeName;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].dailyOil;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].dailySmallOil;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].singleWellOil;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].oilUnitPrice;
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetAdjust/costTargetAdjustDeviceOilEdit.jsp");
}
function toModify(){
	...
	popWindow(cruConfig.contextPath+"/op/costTargetAdjust/costTargetAdjustDeviceOilEdit.jsp?cost_device_id="+ids);
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	ids = ids.replace(/\,/g,"','");
	var sql = "update bgp_op_tartet_device_oil t set t.if_delete_change='1' where t.COST_DEVICE_ID in('"+ids+"')";
...
function getSum(type){
	...
	var querySql = "select sum(nvl(to_char(td.oil_unit_price * (nvl(td.daily_oil,0)+nvl(td.single_well_oil,0)) *(td.plan_end_date - td.plan_start_date+1) * nvl(td.device_count,1),'9999999999999999.00'),0)) sum_value1,"+
		...
		" where td.bsflag='0' and  td.project_info_no = '<%=projectInfoNo%>' and td.if_change !='2' and td.if_delete_change is null" ;
