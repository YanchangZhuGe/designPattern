      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr> 
			      ...
			      <td class="bt_info_even" exp="{org_name}">检查单位</td>
			      <td class="bt_info_odd" exp="{second_org_name}">检查基层单位</td>
			      ...
			      <td class="bt_info_even" exp="{check_start_time}">检查开始时间</td>
			      <td class="bt_info_odd" exp="{check_end_time}">检查结束时间</td>
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">    
			    ...
			    <li id="tag3_1"><a href="#" onclick="getTab3(1)">问题清单</a></li>		
			...
			<form name="form" id="form"  method="post" action="">
				...
				<div id="tab_box_content1" class="tab_box_content" style="display:none;">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
		                <tr align="right" height="30">
		                  <td>&nbsp;</td>
		                  <td width="150" id="buttonDis3" >		     
		                  <span class="zj"  onclick="toAddDetail()"><a href="#"></a></span>
		                    <span class="xg"  onclick="toExitDetails()"><a href="#"></a></span>
		                  <span class="dr"  onclick="toUploadFile()" ><a href="#"></a></span>
					  ...
					  <tr>
			            ...
			            <td class="bt_info_odd">整改完成情况</td>		
			            <td  class="bt_info_even">整改完成时间</td>
					  </tr>
					</table>
...
cruConfig.cdtType = 'form';
			...
			if(retObj.flag!="false"){
				...
				if(len>0){
					if(retObj.list[0].organFlag!="0"){
						querySqlAdd = " and (tr.org_sub_id like '"+retObj.list[0].orgSubId+"%' or tr.check_unit_org like '"+retObj.list[0].orgSubId+"%' or tr.check_roots_org like '"+retObj.list[0].orgSubId+"%')";
						if(len>1){
							if(retObj.list[1].organFlag!="0"){
								querySqlAdd = " and (tr.second_org like '"+retObj.list[1].orgSubId+"%' or tr.check_unit_org like '"+retObj.list[1].orgSubId+"%' or tr.check_roots_org like '"+retObj.list[1].orgSubId+"%')";
							}
						}
					}
				}
	...
	function simpleSearch(){
		var materialName = document.getElementById("materialName").value;
			if(materialName!=''&& materialName!=null){
				...
				cruConfig.cdtType = 'form';
				cruConfig.queryStr = " select tr.check_no,case when length(tr.check_unit_org)<= 6 then tr.check_unit_org else concat(substr(tr.check_unit_org,0,6),'...') end check_unit_org, decode(tr.check_type,'1','日常检查','2','专项检查','3','联系点到位','','其他') check_typename, decode(tr.leadership_led,'1','是','2','否') leadership_ledname , case when length(tr.check_roots_org)<= 6 then tr.check_roots_org else concat(substr(tr.check_roots_org,0,6),'...') end check_roots_org,tr.check_parts,tr.check_type,tr.if_contractor_team,tr.contact,tr.leadership_led, tr.led_leadership,tr.check_members,tr.check_start_time,tr.check_end_time,tr.check_name,tr.number_problems,tr.verifier,tr.verifier_time,tr.verifier_opinion,  tr.second_org,tr.third_org,ion.org_abbreviation as org_name,tr.creator,tr.create_date,tr.bsflag, tr.modifi_date, tr.updator,oi1.org_abbreviation as second_org_name   from BGP_HSE_CHECK tr  left join comm_org_subjection os1   on tr.second_org = os1.org_subjection_id  and os1.bsflag = '0'  left join comm_org_information oi1  on oi1.org_id = os1.org_id  and oi1.bsflag = '0'      left join comm_org_subjection ose    on tr.org_sub_id = ose.org_subjection_id   and ose.bsflag = '0'   left join comm_org_information ion    on ion.org_id = ose.org_id   where tr.bsflag = '0' "+querySqlAdd+" and tr.check_name like '%"+materialName+"%' order by  tr.modifi_date desc ";
				cruConfig.currentPageUrl = "/hse/hseOptionPage/hseCheck/hseCheckList.jsp";
			}else{
				refreshData();
			}
		...
		if(shuaId !=null){
			...
			var datas1 =null;
			 deleteTableTr("hseTableInfo3"); 
				   querySql1 = "  select cn.check_detail_no,cn.check_no, case when length(cn.exist_unit)<= 6 then cn.exist_unit else concat(substr(cn.exist_unit,0,6),'...') end exist_unit,   case when length(cn.roots_unit)<= 6 then cn.roots_unit else concat(substr(cn.roots_unit,0,6),'...') end roots_unit,   case when length(cn.exist_parts)<= 6 then cn.exist_parts else concat(substr(cn.exist_parts,0,6),'...') end exist_parts, case when length(cn.notes)<= 11 then cn.notes else concat(substr(cn.notes,0,11),'...') end notes,  case when length(cn.gause_analysis)<= 6 then cn.gause_analysis else concat(substr(cn.gause_analysis,0,6),'...') end gause_analysis,   case when length(cn.requirements)<= 6 then cn.requirements else concat(substr(cn.requirements,0,6),'...') end requirements,  case when length(cn.completion)<= 6 then cn.completion else concat(substr(cn.completion,0,6),'...') end completion,cn.completion_time from BGP_HSE_CHECK_DETAIL cn  left join BGP_HSE_CHECK tr on cn.check_no=tr.check_no and tr.bsflag='0'  where cn.bsflag='0' and cn.check_no='"+shuaId+"' ";
	...
	function toAdd(){
		popWindow("<%=contextPath%>/hse/hseOptionPage/hseCheck/addhseCheck.jsp?isProject=<%=isProject%>&projectInfoNo=<%=projectInfoNo%>");
		
	}
	
	 function toImportPage(){ 
		    popWindow("<%=contextPath%>/hse/hseOptionPage/hseCheck/importFileMain.jsp?isProject=<%=isProject%>&projectInfoNo=<%=projectInfoNo%>");
	 }
	...
	function toCopyAdd(){
		...
		var queryRet = null;
		var  datas =null;		
	    querySql = "   select CHECK_NO,  ORG_SUB_ID,  SECOND_ORG,  THIRD_ORG,  CHECK_UNIT_ORG,  CHECK_ROOTS_ORG,  CHECK_PARTS,  CHECK_TYPE,  IF_CONTRACTOR_TEAM,  CONTACT,  LEADERSHIP_LED,  LED_LEADERSHIP,  CHECK_MEMBERS,  CHECK_START_TIME,  CHECK_END_TIME,  CHECK_NAME,  NUMBER_PROBLEMS,  VERIFIER,  VERIFIER_TIME,  VERIFIER_OPINION,  CREATOR,  CREATE_DATE,  UPDATOR,  MODIFI_DATE,  BSFLAG,  NOTES,  SPARE1,  SPARE2,  SPARE3,  SPARE4,  SPARE5,  PROJECT_NO,  CHECK_LEVEL,  CHECK_UNIT_ID,  CHECK_ROOTS_ID  from BGP_HSE_CHECK  where CHECK_NO='"+ ids +"'";				 	 
		queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+encodeURI(encodeURI(querySql)));
		if(queryRet.returnCode=='0'){
			datas = queryRet.datas;
			if(datas != null && datas != ""){			
				...
				rowParam['check_level'] = datas[0].check_level;
				rowParam['check_unit_org'] = encodeURI(encodeURI(datas[0].check_unit_org));
				rowParam['check_roots_org'] = encodeURI(encodeURI(datas[0].check_roots_org));
				rowParam['check_parts'] = encodeURI(encodeURI(datas[0].check_parts));
				rowParam['check_type'] = encodeURI(encodeURI(datas[0].check_type));		 
				rowParam['if_contractor_team'] = encodeURI(encodeURI(datas[0].if_contractor_team));
				rowParam['contact'] = encodeURI(encodeURI(datas[0].contact));
				rowParam['leadership_led'] = encodeURI(encodeURI(datas[0].leadership_led));
				rowParam['led_leadership'] = encodeURI(encodeURI(datas[0].led_leadership));		 
				rowParam['check_members'] = encodeURI(encodeURI(datas[0].check_members));
				rowParam['check_start_time'] = encodeURI(encodeURI(datas[0].check_start_time));
				rowParam['check_end_time'] = encodeURI(encodeURI(datas[0].check_end_time));
				rowParam['check_name'] = encodeURI(encodeURI(datas[0].check_name));
				rowParam['number_problems'] =datas[0].number_problems;
				rowParam['check_unit_id'] = encodeURI(encodeURI(datas[0].check_unit_id));
				rowParam['check_roots_id'] = encodeURI(encodeURI(datas[0].check_roots_id));
				
				//rowParam['verifier'] = encodeURI(encodeURI(datas[0].verifier));
				//rowParam['verifier_time'] = encodeURI(encodeURI(datas[0].verifier_time));
				//rowParam['verifier_opinion'] = encodeURI(encodeURI(datas[0].verifier_opinion));
 ...
 function toUploadFile(){
	 ids = getSelIds('chx_entity_id');
	    ...
	    popWindow("<%=contextPath%>/hse/hseOptionPage/hseCheck/importFile.jsp?check_no="+ids);
	 }
	...
	function toSearch(){
		popWindow("<%=contextPath%>/hse/hseOptionPage/hseCheck/hse_search.jsp?isProject=<%=isProject%>");
	}
	 function chooseOne(cb){   
	        var obj = document.getElementsByName("chx_entity_id");   
	        for (i=0; i<obj.length; i++){   
