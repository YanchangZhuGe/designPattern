...
Ext.onReady(function() {
    ...
    function cellclick(grid,colspan, colIndex, rowdata, rowspan,rowIndex,e) {
        ...
        if(leaf == true){
        	parent.mainRightframe.location.href = "<%= contextPath%>/hse/weekReport/"+url+"?isProject=<%=isProject%>&week_date=<%=week_date%>&week_end_date=<%=week_end_date%>&org_id=<%=org_id%>&organ_flag=<%=organ_flag%>&subflag=<%=subflag%>&action=<%=action%>";
  		}
    }
