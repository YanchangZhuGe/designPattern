		  ...
		  <div style="overflow:auto">
			  <table style="width:97.9%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
					...
					<td class="bt_info_odd" width="12.9%">设备名称</td>
					<td class="bt_info_even" width="12.9%">规格型号</td>
					<td class="bt_info_odd" width="7.3%">计量单位</td>
					<td class="bt_info_even" width="7.3%">需求数量</td>
					<td class="bt_info_odd" width="7.4%">已申请数量</td>
					<td class="bt_info_even" width="7.4%">申请数量</td>
					<td class="bt_info_odd" width="8%">用途</td>
					<td class="bt_info_even" width="14%">开始时间</td>
					<td class="bt_info_odd" width="14%">结束时间</td>
				</tr>
				</table>
...
<script type="text/javascript"> 
	...
	function refreshData(){
		...
		var basedatas;
		if('<%=deviceappid%>'!=null){
			var conditionsql = "select mix_type_id from gms_device_disapp where device_app_id='<%=deviceappid%>'";
			...
			var conditionStr = "";
			if(conditionObj[0].mix_type_id == '<%=DevConstants.MIXTYPE_ZHENYUAN%>'){
				conditionStr += " and substr(aad.dev_ci_code,1,5) = '"+conditionObj[0].mix_type_id+"' ";
			}else if(conditionObj[0].mix_type_id == '<%=DevConstants.MIXTYPE_YIQI%>'){
				conditionStr += " and (substr(aad.dev_ci_code,1,5) = '"+conditionObj[0].mix_type_id+"' ";
				conditionStr += " or substr(aad.dev_ci_code,1,5) = '<%=DevConstants.MIXTYPE_YIQICHE%>') ";
			}else if(conditionObj[0].mix_type_id == '<%=DevConstants.MIXTYPE_CELIANG%>'){
				conditionStr += " and (substr(aad.dev_ci_code,1,7) = '<%=DevConstants.MIXTYPE_CELIANG_01%>' ";
				conditionStr += " or substr(aad.dev_ci_code,1,7) = '<%=DevConstants.MIXTYPE_CELIANG_02%>' ";
				conditionStr += " or substr(aad.dev_ci_code,1,7) = '<%=DevConstants.MIXTYPE_CELIANG_03%>') ";
			}else{
				conditionStr += " and substr(aad.dev_ci_code,1,5) != '<%=DevConstants.MIXTYPE_ZHENYUAN%>' ";
				conditionStr += " and substr(aad.dev_ci_code,1,5) != '<%=DevConstants.MIXTYPE_YIQI%>' ";
				conditionStr += " and substr(aad.dev_ci_code,1,5) != '<%=DevConstants.MIXTYPE_YIQICHE%>' ";
				conditionStr += " and substr(aad.dev_ci_code,1,7) != '<%=DevConstants.MIXTYPE_CELIANG_01%>' ";
				conditionStr += " and substr(aad.dev_ci_code,1,7) != '<%=DevConstants.MIXTYPE_CELIANG_02%>' ";
				conditionStr += " and substr(aad.dev_ci_code,1,7) != '<%=DevConstants.MIXTYPE_CELIANG_03%>' ";
			}
			var prosql = "select aad.device_allapp_detid,aad.team,aad.teamid,aad.dev_ci_code,aad.isdevicecode,";
				...
				prosql += "left join gms_device_disapp da on allapp.device_allapp_id=da.device_allapp_id ";
				...
				prosql += "from (select project_info_no,bsflag,device_allapp_detid,dev_ci_code,apply_num,'1' as type ";
				prosql += "from gms_device_disapp_detail union ";
				prosql += "select project_info_no,bsflag,device_allapp_detid,dev_ci_code,apply_num,'2' as type ";
				prosql += "from gms_device_hireapp_detail )tmp ";
		...
		for(var index=0;index<retObj.length;index++){
			...
			innerhtml += "<td width='4%'><input type='checkbox' name='detinfo' id='"+index+"' value='"+retObj[index].device_allapp_detid+"'/>";
			innerhtml += "<input name='teamid"+index+"' id='teamid"+index+"' style='line-height:15px' value='"+retObj[index].teamid+"' type='hidden' /></td>";
			innerhtml += "<td width='8%'><input name='teamname"+index+"' id='teamname"+index+"' value='"+retObj[index].teamname+"' size='5' type='text' readonly/>";
			innerhtml += "<input name='team"+index+"' id='teamid"+index+"' value='"+retObj[index].team+"' type='hidden'/></td>";
			
			innerhtml += "<td width='12.9%'><input name='devicename"+index+"' id='devicename"+index+"' style='line-height:15px' value='"+retObj[index].dev_ci_name+"' size='10' type='text' readonly/></td>";
			innerhtml += "<td width='12.9%'><input name='devicetype"+index+"' id='devicetype"+index+"' value='"+retObj[index].dev_ci_model+"' size='10'  type='text' readonly/>";
			...
			innerhtml += "<td width='7.3%'><input name='unitname"+index+"' id='unitname"+index+"' value='"+retObj[index].unit_name+"' size='4' type='text' readonly>";
			innerhtml += "<input name='unitinfo"+index+"' id='unitinfo"+index+"' value='"+retObj[index].unitinfo+"' type='hidden'></td>";
			
			innerhtml += "<td width='7.3%'><input name='neednum"+index+"' id='neednum"+index+"' value='"+retObj[index].require_num+"' size='4' type='text' readonly/></td>";
			innerhtml += "<td width='7.4%'><input name='applyednum"+index+"' id='applyednum"+index+"' value='"+retObj[index].applyed_num+"' size='4' type='text' readonly/></td>";
			innerhtml += "<td width='7.4%'><input name='applynum"+index+"' id='applynum"+index+"' detindex='"+index+"' value='' size='4' type='text' onkeyup='checkAssignNum(this)'/></td>";
			
			innerhtml += "<td width='8%'><input name='purpose"+index+"' id='purpose"+index+"' value='"+retObj[index].purpose+"' size='4' type='text' readonly/></td>";
			innerhtml += "<td width='14%'><input name='startdate"+index+"' id='startdate"+index+"' style='line-height:15px' value='"+retObj[index].plan_start_date+"' size='10' type='text' readonly/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton2"+index+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(startdate"+index+",tributton2"+index+");'/></td>";
			innerhtml += "<td width='14%'><input name='enddate"+index+"' id='enddate"+index+"' style='line-height:15px' value='"+retObj[index].plan_end_date+"' size='10' type='text' readonly/><img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton3"+index+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(enddate"+index+",tributton3"+index+");'/></td>";
			innerhtml += "</tr>";
