...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.currentPageUrl = "<%=contextPath%>/market/crm/provider/contractList.jsp";
		queryData(1);
	...
	function toAdd() { 
		...
		popWindow("<%=contextPath%>/market/crm/provider/contractEdit.jsp?company_id="+company_id);
	}
	function toEdit() {
		var company_id ='<%=company_id%>';
		if(company_id==null || company_id==''){
		...
		for(var i=1;i<obj.length;i++){
			if(obj[i].checked ==true){
				...
				if(contract_id!=null && window.confirm(text)){
					popWindow("<%=contextPath%>/market/crm/provider/contractEdit.jsp?contract_id="+contract_id+"&company_id="+company_id);
					return;
				}
			}
		}
