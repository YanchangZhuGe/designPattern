...
<div style="overflow-y: hidden; overflow-x: hidden;">
	...
	function refreshData(){
		var myChart = new FusionCharts("${applicationScope.fusionChartsURL}/Charts/Column2D.swf", "myChartId", "100%", "300", "0", "1");
	 	var substr = "name=first";
	 	var retObj = jcdpCallService("QualityChartSrv", "firstChartByOrg", substr);
	...
	function getTotal(){  //and p.project_status='3'  正在施工
		var querySql = " select tt.eps_id ,tt.eps_name ,tt.org_id ,tt.org_subjection_id ,case when tt.waster is null then 0 else tt.waster end waster ,"+
			" case when tt.miss is null then 0 else tt.miss end miss,case when tt.first is null then 0 else tt.first end first ,case when tt.qualified is null then 0 else tt.qualified end qualified ,"+
			" case when tt.total is null then 0 else tt.total end total,round((case when tt.total is null then 0 else tt.first/tt.total*100 end),2) first_percent from( "+
			...
			" sum(r.daily_acquire_firstlevel_num) first from bgp_eps_code t left join comm_org_subjection sub on t.org_id = sub.org_id and sub.bsflag='0'" +
		...
		if(retObj!=null && retObj.returnCode==''){
			if(retObj.datas != null && retObj.datas.length>0){
				for(var i =0 ;i<retObj.datas.length ;i++){
					...
					var first_percent = data.first_percent;
					var first = data.first;
					var total = data.total;
					var eps_name = data.eps_name;
					p = p +"<font color='blue'>"+ eps_name + "</font>(一级品指标≥<span >"+first_percent+"</span>%;&nbsp;累计完成炮数:<span >"+total+"</span>;&nbsp;累计完成一级品炮数:<span >"+first+"</span>);";
					if(i!=0 && i%2 ==1){
						p = p+"</p><p class='small' >";
					}
				}
			}else{
				querySql = "select t.eps_id ,t.eps_name ,t.org_id , sub.org_subjection_id ,'0' first, '0' total ,'0' first_percent from bgp_eps_code t "+
				" join comm_org_subjection sub on t.org_id = sub.org_id and sub.bsflag='0' "+
				" where t.bsflag='0' and t.parent_object_id =(select c.object_id from bgp_eps_code c where c.org_id ='C6000000000001')"
				retObj = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+encodeURI(encodeURI(querySql))+'&pageSize='+cruConfig.pageSizeMax);
				if(retObj.datas != null && retObj.datas.length>0){
					for(var i =0 ;i<retObj.datas.length ;i++){
						var data = retObj.datas[i];
						var first_percent = data.first_percent;
	...
	function projectFirst(org_subjection_id,org_name){
		...
		window.open('<%=contextPath%>/qua/mProject/analysis/first.jsp?org_subjection_id='+org_subjection_id+'&org_name='+encodeURI(org_name));
	}
	</script>
    </body>
</html>