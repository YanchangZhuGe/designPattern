...
<!-- 				
 ...
 -->				 		
        ...
        <tr>
		 	...
		 	<td class="inquire_form4">
		 				<input type="text" id="actual_price" name="actual_price" class="input_width" value="0"/>
		 	</td>
	...
	<div id="list_table">
		<div id="inq_tool_box">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td>
						<auth:ListButton functionId="" css="zj" event="onclick='showDevAccountPage()'" title="JCDP_btn_add"></auth:ListButton>
					</td>
				</tr>
			</table>
		</div>
		<div id="table_box" >
			...
			<tr>
				<td class="bt_info_odd" exp="<input name = 'rdo_entity_id'  type='checkbox' value='{dev_acc_id}' onclick='loadDataDetail()'/>" ><input type='checkbox' name='rdo_entity_id' value='' onclick='check()'/></td>
			      ...
			      <td class="bt_info_even" exp="<input name ='oil_num_{dev_acc_id}' id ='oil_num_{dev_acc_id}' type='text' value='' size='12' onkeyup='checkNumber(this);loadDataDetail();' />" >数量(升)</td>
...
function submitInfo(){
	...
	form.action="<%=contextPath%>/dmsManager/safekeeping/saveDeviceExpendInfo.srq?devAccId="+ids;
	form.submit();
...
function showDevAccountPage(){
	var obj = new Object();
	var returnValue = window.showModalDialog("<%=contextPath%>/dmsManager/safekeeping/equipmentOperator/selectEOAccount.jsp","","dialogWidth=1200px;dialogHeight=480px");			
	if(!returnValue){
		return;
	}
	refreshData(returnValue);
}
