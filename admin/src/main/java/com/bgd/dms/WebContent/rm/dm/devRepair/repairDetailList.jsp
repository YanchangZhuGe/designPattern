...
$(function(){
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			...
			if(flag > 2){
				innerHTML += "<td><a href=\"#\" onclick = \"showResult(\'"+datas[i].subid+"\')\" ><font color = \"green\">"+datas[i].dev_name+"</font></a></td>";	
			}else{
			...
			innerHTML += "<td>"+datas[i].dev_status+"</td><td>"+datas[i].remark+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
	...
	popWindow('<%=contextPath%>/rm/dm/devRepair/repairDevResultShow.jsp?id='+info,'800:700');
	};
    ...
    function refreshData(v_device_repair_app_no, v_device_repair_app_name){
		var str = " SELECT REP.*,EMP.EMPLOYEE_NAME AS CREATOR_NAME,";
			...
			str += " COMM_ORG_INFORMATION       TODOORG,COMM_HUMAN_EMPLOYEE EMP WHERE 1 = 1";
    ...
    function loadDataDetail(id){
    	var retObj;
		if(id!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getDevRepairInfo", "id="+id);
		...
		//选中这一条checkbox
		$("#selectedbox_"+retObj.deviceRepairMap.id).attr("checked","checked");
		//取消其他选中的
		$("input[type='checkbox'][name='selectedbox'][id!='selectedbox_"+retObj.deviceRepairMap.id+"']").removeAttr("checked");
		//给数据回填
	...
	function toModifyDevRepairPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devRepair/repairModify.jsp?id='+id);
	}
	...
	function toSumbitRepairApp(){
		...
		/*******************************验证送修单位 维修单位是否填写***/
		var orgSql = "SELECT T.* FROM GMS_DEVICE_COLL_REPAIRFORM T WHERE t.ID in ("+selectedid+") ";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+orgSql);
		var basedatas = queryRet.datas;
		for(var index = 0;index<basedatas.length;index++){
			if(basedatas[index].req_comp == ""){
				detflag = true;
				alertinfo = "您选择的记录中送修单位为空,不能提交!";
				break;
			}
			if(basedatas[index].todo_comp == ""){
				detflag = true;
		...
		if (window.confirm("确认要提交吗?")) {
			...
			$("input[type='checkbox'][name='selectedbox']").each(function(){
				if(this.checked){
					sql = "insert into GMS_DEVICE_COLL_REPAIR_REC(ID,REPARE_ID,BSFLAG,TODO_COMP,REC_STATUS) "+
					"select sys_guid(), id,'0',TODO_COMP,'0' FROM GMS_DEVICE_COLL_REPAIRFORM WHERE ID = '"+this.value+"'";
					var retObj = jcdpCallService("QualityItemsSrv","saveQuality", "sql="+sql);
				}
			});
			//向设备动态表添加数据
			//document.getElementById("form1").action = "<%=contextPath%>/rm/dm/devRepair/repairListSubmit.srq?formid="+selectedid;
			//document.getElementById("form1").submit();
			retObj = jcdpCallService("DevCommInfoSrv", "repairListSubmit", "formid="+selectedid);
			alert('提交成功!');
			refreshData();
		}
	}
