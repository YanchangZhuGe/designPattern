...
input,textarea{
 ...
 <body style="background:#fff" onload="refreshData('','','<%=scrape_report_id%>')">
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    <td class="ali_cdn_name">报废上报名称</td>
			    <td class="ali_cdn_input">
			    	<input id="scrape_report_name" name="scrape_report_name" type="text" class="input_width" />
			    </td>
			       <td class="ali_cdn_name">报废上报单号</td>
			    <td class="ali_cdn_input">
			    	<input id="scrape_report_no" name="scrape_report_no" type="text" class="input_width" />
			    </td>
			    <td class="ali_query">
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='scrape_report_id_{scrape_report_id}' name='scrape_report_id'  idinfo='{scrape_report_id}'>
			     	<td class="bt_info_odd" exp="<input type='radio' name='selectedbox' value='{scrape_report_id}' id='selectedbox_{scrape_report_id}' onclick='chooseOne(this)'/>" >选择</td>
					<td class="bt_info_even" autoOrder="1">序号</td>
					<td class="bt_info_odd" exp="{scrape_report_no}">报废上报单号</td>
					<td class="bt_info_odd" exp="{scrape_report_name}">报废上报单名称</td>
					<td class="bt_info_even" exp="{employee_name}">上报人</td>
					<td class="bt_info_odd" exp="{org_name}">上报单位</td>
					<td class="bt_info_even" exp="{report_date}">上报时间</td>
			...
			<div id="tab_box" class="tab_box">
				<div id="tab_box_content0" name="tab_box_content0" class="tab_box_content">
					<div style="overflow: auto">
						<table width="100%" border="0" cellspacing="0" cellpadding="0"
							...
							<tr align="right">
								...
								<td class="ali_cdn_input"></td>
								<td>&nbsp;</td>
								<auth:ListButton functionId="" css="bc"	event="onclick='toEditbyjh()'" title="保存"></auth:ListButton>
							</tr>
						</table>
					</div>
					<table id="scrapeMap" name="scrapeMap" border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:0px;background:#efefef"> 
					<tr>
				      <td  class="inquire_item6">设备报废单号：</td>
				      <td  class="inquire_form6" ><input id="scrape_report_no" class="input_width" type="text" value="" disabled/>&nbsp;</td>
				      <td  class="inquire_item6">设备报废单名称：</td>
				      <td  class="inquire_form6"><input id="scrape_report_name" class="input_width" type="text"  value="" disabled/> &nbsp;</td>
				      <td  class="inquire_item6">&nbsp;上报单位：</td>
				    ...
				    <tr>
				      ...
				      <td  class="inquire_item6">&nbsp;上报时间：</td>
				      <td  class="inquire_form6"  ><input id="report_date" class="input_width" type="text"  value="" disabled/> &nbsp;</td>
				      <td  class="inquire_item6">&nbsp;补录报废文号：</td>
				      <td  class="inquire_form6"  ><input id="scrape_number" class="input_width" type="text"  /> &nbsp;</td>
				     </tr>
					</table>
				</div>
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
		  ...
		  //设备信息显示
			...
			$("#scrapeApplyList").attr("src","<%=contextPath%>/dmsManager/scrape/scrapeApplyListforReport.jsp?relationId="+currentid);
		}
		$(filternotobj).hide();
		$(filterobj).show();
	}
...
<script type="text/javascript">
	...
	cruConfig.queryOp = "queryScrapeReportList";
	...
	function searchDevData(){
		var scrape_report_no = document.getElementById("scrape_report_no").value;
		var scrape_report_name = document.getElementById("scrape_report_name").value;
		refreshData(scrape_report_no, scrape_report_name,'');
	}
	function clearQueryText(){
		document.getElementById("scrape_report_no").value = "";
		document.getElementById("scrape_report_name").value = "";
		refreshData("", "",'');
	}
    function refreshData(scrape_report_no, scrape_report_name,scrape_report_id){
      	var temp = "";
		if(typeof scrape_report_no!="undefined" && scrape_report_no!=""){
			temp += "&scrape_report_no="+scrape_report_no;
		}
		if(typeof scrape_report_name!="undefined" && scrape_report_name!=""){
			temp += "&scrape_report_name="+scrape_report_name;
		}
		if(typeof scrape_report_id!="undefined" && scrape_report_id!=""){
			temp += "&scrape_report_id="+scrape_report_id;
		}
		cruConfig.submitStr = temp;	
		queryData(1);

	}
    function loadDataDetail(scrape_report_id){
    	var retObj;
		if(scrape_report_id!=null){
			 retObj = jcdpCallService("ScrapeSrv", "getScrapeReportInfo", "scrape_report_id="+scrape_report_id);
		    ...
		    retObj = jcdpCallService("ScrapeSrv", "getScrapeReportInfo", "scrape_report_id="+ids);
		}
	
		//选中这一条checkbox
		$("#selectedbox_"+retObj.deviceappMap.scrape_report_id).attr("checked","checked");
		//取消其他选中的
		$("input[type='checkbox'][name='selectedbox'][id!='selectedbox_"+retObj.deviceappMap.scrape_report_id+"']").removeAttr("checked");
		//给数据回填
		$("#scrape_report_no","#scrapeMap").val(retObj.deviceappMap.scrape_report_no);
		$("#scrape_report_name","#scrapeMap").val(retObj.deviceappMap.scrape_report_name);
		$("#report_date","#scrapeMap").val(retObj.deviceappMap.report_date);
		$("#employee_name","#scrapeMap").val(retObj.deviceappMap.employee_name);
		$("#org_name","#scrapeMap").val(retObj.deviceappMap.org_name);
		$("#scrape_number","#scrapeMap").val(retObj.deviceappMap.scrape_number);
		...
		var curbusinesstype="5110000181000000016";
    	processNecessaryInfo={        							//流程引擎关键信息
			businessTableName:"dms_scrape_report",    			//置入流程管控的业务表的主表表明
			businessType:curbusinesstype,    				//业务类型 即为之前设置的业务大类
			businessId:scrape_report_id,           			//业务主表主键值
			businessInfo:"报废上报审批流程>报废上报单名称:"+scrape_report_name+";报废上报单号:"+scrape_report_no+">",
			applicantDate:submitdate       						//流程发起时间
		};
		processAppendInfo={ 
			scrape_report_id:scrape_report_id
		};
		loadProcessHistoryInfo(); */
    }
	function toAddScrapePage(){
		popWindow('<%=contextPath%>/dmsManager/scrape/scrapeReportAdd.jsp','','报废上报');
	}
	function toModifyScrapePage(){
		var baseData;
		 ...
		 baseData = jcdpCallService("ScrapeSrv", "getScrapeReportState", "scrape_report_id="+ids);
		...
		popWindow('<%=contextPath%>/dmsManager/scrape/scrapeReportAdd.jsp?scrape_report_id='+ids,'','报废上报');
	}
	function toDelScrapePage(){
		...
		if(confirm('确定要删除吗?')){  
			retObj = jcdpCallService("ScrapeSrv", "deleteScrapeReportInfo", "scrape_report_id="+ids);
			alert('删除成功!');
			refreshData();
		}
	}
	function toSumbitScrapeApp(){
		 ...
		 baseData = jcdpCallService("ScrapeSrv", "getScrapeReportState", "scrape_report_id="+ids);
		 var scrape_report_type = baseData.deviceappMap.scrape_report_type;
		 var curbusinesstype="";
		 if(scrape_report_type!=null&&scrape_report_type!=""){
			if(scrape_report_type=="A类"){
				curbusinesstype="5110000181000000016";
			}else if(scrape_report_type=="B类"){
				curbusinesstype="5110000181000000017";
			}else if(scrape_report_type=="C类"){
				curbusinesstype="5110000181000000018";
			}
		 }
		 var submitdate =getdate();
			...
			if(baseData.deviceappMap.proc_status=='3')
		{
		 ...
		 retObj = jcdpCallService("ScrapeSrv", "getScrapeDetail", "scrape_report_id="+ids);
 ...
 function toEditbyjh(){
	 var ids = getSelIds('selectedbox');
	 var scrape_number=$("#scrape_number").val();
	 if(scrape_number !=""){
			Ext.MessageBox.wait('请等待','处理中');
			Ext.Ajax.request({
				url : "<%=contextPath%>/dmsManager/scrape/updateScrapeNumber.srq",
				method : 'Post',
				isUpload : true,  
				params : {
					ids :ids,
