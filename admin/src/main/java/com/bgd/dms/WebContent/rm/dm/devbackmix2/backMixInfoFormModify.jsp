      ...
      <table border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td class="inquire_form4" >
          	...
          	<input name="device_mixinfo_id" id="device_mixinfo_id" type="hidden" value="<%=mixInfoId%>" />
		...
		<div id="tab_box" class="tab_box" style="height:190px;overflow:auto">
			<table style="width:97.9%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr class="FixedTitleRow">
					<td class="bt_info_odd" width="11.5%">设备名称</td>
					<td class="bt_info_even" width="11.5%">规格型号</td>
					<td class="bt_info_odd" width="14.5%">设备编号</td>
					<td class="bt_info_even" width="11.5%">自编号</td>
					<td class="bt_info_odd" width="11.5%">实物标识号</td>
					<td class="bt_info_even" width="11.5%">牌照号</td>
					<td class="bt_info_odd" width="14%">实际进场时间</td>
					<td class="bt_info_even" width="14%">计划离场时间</td>
					<td class="bt_info_odd" width="14%">存放地</td>
				</tr>
...
<script type="text/javascript"> 
	...
	function showOrgTreePage(str){
		...
		var devicemixinfoid = $("#device_mixinfo_id").val();
		var returnValue=window.showModalDialog("<%=contextPath%>/rm/dm/devbackmix2/selectOwnOrgId.jsp?devbackappid="+devicebackappid+"&devicemixinfoid="+devicemixinfoid,"test","");
	...
	function toMixDetailInfos(){
		...
		var str = "select ci.dev_ci_name,ci.dev_ci_model,ci.dev_ci_code,detail.device_backdet_id,detail.dev_acc_id,detail.dev_position,";
			str +="detail.dev_coding,detail.self_num,detail.dev_sign,detail.license_num,detail.actual_in_time,detail.planning_out_time ";
		...
		if(retObj!=undefined){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td><input name='devcoding"+index+"' id='devcoding"+index+"' value='"+retObj[index].dev_coding+"' size='12' type='text' readonly/></td>";
				innerhtml += "<td><input name='selfnum"+index+"' id='selfnum"+index+"' style='line-height:15px' value='"+retObj[index].self_num+"' size='12' type='text' /></td>";
				innerhtml += "<td><input name='devsign"+index+"' id='devsign"+index+"' value='"+retObj[index].dev_sign+"' size='12'  type='text' readonly/></td>";
				innerhtml += "<td><input name='licensenum"+index+"' id='licensenum"+index+"' value='"+retObj[index].license_num+"' size='12' type='text' readonly/>";
				
				innerhtml += "<td><input name='actualintime"+index+"' id='actualintime"+index+"' style='line-height:15px' value='"+retObj[index].actual_in_time+"' size='13' type='text' readonly/></td>";
				innerhtml += "<td><input name='planningouttime"+index+"' id='planningouttime"+index+"' style='line-height:15px' value='"+retObj[index].planning_out_time+"' size='13' type='text' readonly/></td>";
	...
	function refreshData(){
		...
		if('<%=devappid%>'!=null){
			...
			+"frm.own_org_id,frmorg.org_abbreviation as own_org_name,frm.device_mixinfo_id "
			+"from gms_device_backinfo_form frm "
			+"join gms_device_backapp devapp on frm.device_backapp_id=devapp.device_backapp_id "
			...
			+"left join comm_org_information frmorg on frm.own_org_id=frmorg.org_id "
		...
		var str = "select ci.dev_ci_name,ci.dev_ci_model,ci.dev_ci_code,detail.device_backdet_id,detail.dev_acc_id,detail.dev_position,";
			...
			str +="join gms_device_account_dui dui on detail.dev_acc_id = dui.dev_acc_id ";
			str +="join gms_device_codeinfo ci on dui.dev_type = ci.dev_ci_code ";
			str +="where detail.device_backapp_id = '"+devicebackappid+"' and dui.owning_org_id='"+own_org_id+"' ";
		//查询设备明细并进行回填
		var proqueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str+'&pageSize=1000');
			retObj = proqueryRet.datas;
		if(retObj!=undefined){
