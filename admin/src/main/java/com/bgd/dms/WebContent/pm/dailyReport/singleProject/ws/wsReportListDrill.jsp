...
//	}
	...
	function openGis(projectInfoNo,dailyNo,produceDate){
		...
		popWindow('<%=contextPath%>/pm/dailyReport/singleProject/ws/viewDailyReport.jsp?daily_no_ws='+dailyNo+'&project_info_no='+projectInfoNo+'&produce_date='+produceDate,'1280:730');
	...
	function toSubmit(){
	    ...
	    if(row_data_str==''){ alert("请先选中一条记录!");
		    ...
		    var retObj = jcdpCallService("WsDailyReportSrv", "getDailyReportInfo", "dailyNo="+daily_no+"&projectInfoNo="+project_info_no+"&produceDate="+produce_date);
			...
			if( !strIsNullOrEmpty(audit_status) ){
				if( audit_status=="0" || audit_status=="4" ){		/* 审批状态 处于“未提交”和“审批不通过”的前提下，才能提交 */
					var retObj = jcdpCallService("WsDailyReportSrv", "submitDailyReport", "dailyNo="+daily_no+"&projectInfoNo="+project_info_no+"&produceDate="+produce_date);
