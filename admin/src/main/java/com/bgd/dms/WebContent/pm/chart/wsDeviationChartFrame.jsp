...
cruConfig.contextPath='<%=contextPath%>';
 function getFusionChart(){
	...
	//myChart1.setXMLUrl("<%=contextPath%>/ws/pm/chart/getDeviation.srq?type=measuredailylist");
	//myChart1.render("chartContainer1");
	
	//var myChart2 = new FusionCharts( "${applicationScope.fusionChartsURL}/Charts/MSLine.swf", "myChartId2", "100%", "100%", "0", "0" );    
	//myChart2.setXMLUrl("<%=contextPath%>/ws/pm/chart/getDeviation.srq?type=drilldailylist");
	//myChart2.render("chartContainer2");
	
	//var myChart3 = new FusionCharts( "${applicationScope.fusionChartsURL}/Charts/MSLine.swf", "myChartId3", "100%", "100%", "0", "0" );    
	//myChart3.setXMLUrl("<%=contextPath%>/ws/pm/chart/getDeviation.srq?type=colldailylist");
	...
	//myChart4.setXMLUrl("<%=contextPath%>/ws/pm/chart/getDeviation.srq?type=surfacedailylist");
	//myChart4.render("chartContainer4");
	debugger;
	var retObj = jcdpCallService("WsChartSrv","getDeviation","projectInfoNo=<%=projectInfoNo %>");
	var dataXml = retObj.Str;
	if(dataXml == undefined){
		dataXml = "";
	}
	var existDrillData = retObj.existDrillData;
	if(existDrillData == undefined){
		existDrillData = "";
	}
...
<script type="text/javascript">
	...
	var querySql = "select t.build_method from gp_task_project t where t.bsflag ='0' and t.project_info_no ='<%=projectInfoNo%>'";
	...
	if(retObj!=null && retObj.returnCode =='0' && retObj.datas[0]!=null){
		...
		if(buildMethod == "5000100003000000001" || buildMethod == "5000100003000000004" || buildMethod == "5000100003000000005" || buildMethod == "5000100003000000007"){
			document.getElementById("trDrill").style.display = 'block';
		}else{
			document.getElementById("trDrill").style.display = 'none';
		}
	}
