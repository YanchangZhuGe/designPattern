...
function toAdd(){
	...
	tr.insertCell().innerHTML = '<input type="text" id="line_group_id_'+rowNum+'" size="10" />';
	tr.insertCell().innerHTML = '<input type="text" id="measure_fullfold_area_'+rowNum+'" size="10" />';
	tr.insertCell().innerHTML = '<input type="text" id="full_fold_area_'+rowNum+'" size="10" />';
...
function initData(){
	...
	if(retObj.datas != null){
		for(var i=1;i<=retObj.datas.length;i++){
			...
			tr.insertCell().innerHTML = '<input type="text" id="line_group_id_'+record.order_num+'" size="10" value="'+record.line_group_id+'"/>';//线束号
			tr.insertCell().innerHTML = '<input type="text" id="measure_fullfold_area_'+record.order_num+'" size="10"  value="'+record.measure_fullfold_area+'"/>';//设计测线满覆盖
			tr.insertCell().innerHTML = '<input type="text" id="full_fold_area_'+record.order_num+'" size="10"  value="'+record.full_fold_area+'"/>';//实际完成满覆盖
			...
			+'<input type="hidden" id="record_id_'+record.order_num+'" value="'+record.wa3d_group_id+'"/>';
			...
			exportRow["5"] = record.full_fold_area;
...
function toSave(){
	...
	for(var i=1;i<rowNum;i++){
		...
		var line_group_id = document.getElementById("line_group_id_"+i);
		if(line_group_id!=null){
			rowParam['line_group_id']=line_group_id.value;
		}
		
		var measure_fullfold_area = document.getElementById("measure_fullfold_area_"+i);
		if(measure_fullfold_area!=null){
			rowParam['measure_fullfold_area']=measure_fullfold_area.value;
		}
		...
		var full_fold_area = document.getElementById("full_fold_area_"+i);
		if(full_fold_area!=null){
			rowParam['full_fold_area']=full_fold_area.value;
		}
		...
		if(record_id != null){
			rowParam['wa3d_group_id'] = record_id.value;
...
function exportExcel(){
	...
	var fromPage = "sh3dlineCompleted";
...
function downloadTemplate(){
	window.location.href="<%=contextPath%>/pm/dailyReport/singleProject/download.jsp?path=/pm/lineCompleted/sh3dlineCompleted.xls&filename=sh3dlineCompleted.xls";
}
...
function importData(){
	...
	if(checkFile(filename)){
		document.getElementById("fileForm").action = "<%=contextPath%>/pm/gpe/importLineCompletedSh.srq?workMethod=3&teamName="+team_name;
