...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <title>单项目-设备接收-设备接收(外租设备)</title>
 </head> 
      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td class="ali_cdn_name">外租设备类型</td>
			    <td class="ali_cdn_input">
			    	<select id="s_hire_dev_type" name="s_hire_dev_type" class="select_width" >
			    	    <option value="" selected="selected">--请选择--</option>
						<option value="5110000184000000002">外租单台</option>
						<option value="5110000184000000003">外租检波器</option>
			    	</select>
			    </td>
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_hireapp_id_{device_hireapp_id}' name='device_hireapp_id' ondblclick='toModifyDetail(this)' idinfo='{device_hireapp_id}'>
					...
					<td class="bt_info_even" exp="{state_desc}">处理状态</td>
			     </tr> 
			  </table>
			</div>
...
$(function(){
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			var innerHTML = "<tr>";
			innerHTML += "<td>"+(i+1)+"</td><td>"+datas[i].teamname+"</td>";
...
<script type="text/javascript">
	...
	function searchDevData(){
		...
		var v_hire_dev_type = document.getElementById("s_hire_dev_type").value;
		...
		refreshData(v_device_app_no, v_device_app_name,v_opr_state_desc,v_hire_dev_type);
	}
	function clearQueryText(){
		document.getElementById("s_opr_state_desc").value="";
		document.getElementById("s_hire_dev_type").value="";
		document.getElementById("s_device_app_no").value = "";
		document.getElementById("s_device_app_name").value = "";
	}
    function refreshData(v_device_app_no, v_device_app_name,v_opr_state_desc,v_hire_dev_type){
		var str = "select pro.project_name,devapp.device_hireapp_id,devapp.device_hireapp_no,devapp.device_hireapp_name,devapp.project_info_no,";
			...
			str += " case devapp.opr_state   when '1' then  '处理中'   when '9' then    '已处理'   when '4' then   '关闭'   else  '未处理'    end as state_desc, ";
			...
			str += "left join common_busi_wf_middle allwfmiddle on allwfmiddle.business_id = allapp.device_allapp_id and allwfmiddle.bsflag = '0' and allwfmiddle.proc_status = '3' ";
			...
			if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目
				...
				str += "where devapp.bsflag = '0' and devapp.project_info_no = '" +retFatherNo+"' and wfmiddle.proc_status='3' ";
				...
				str += "where devapp.bsflag = '0' and devapp.project_info_no='"+projectInfoNos+"' and wfmiddle.proc_status='3' " ;
	        }else{
	        	str += "left join gp_task_project pro on devapp.project_info_no = pro.project_info_no and pro.bsflag = '0' ";
				str += "where devapp.bsflag = '0' and devapp.project_info_no='"+projectInfoNos+"' and wfmiddle.proc_status='3' ";
		    }
		     
			if(v_hire_dev_type!=undefined && v_hire_dev_type!=''){
				if(v_hire_dev_type == '5110000184000000003'){//外租检波器
					str += "and devapp.mix_type_id = '5110000184000000003' ";
				}else{
					str += "and devapp.mix_type_id = '5110000184000000002' ";
				}
			}
			...
			str += "order by devapp.opr_state nulls first,devapp.modifi_date ";
    ...
    function loadDataDetail(device_hireapp_id){
		...
		if(projectType == '5000100004000000008'){//井中
			curbusinesstype = "5110000004100001098";
		}else{
			curbusinesstype = "5110000004100001095";
		}
		}
		if(mix_type_id=='5110000184000000002')
		{
			mixtype="外租单台";
		if(projectType == '5000100004000000008'){//井中
    ...
    function toDetailPage(){
		...
		window.location.href='<%=contextPath%>/rm/dm/hireReceive/hirePlanPassedDetailList.jsp?mixId='+shuaId+'&sonFlag='+sonFlag;
    }
