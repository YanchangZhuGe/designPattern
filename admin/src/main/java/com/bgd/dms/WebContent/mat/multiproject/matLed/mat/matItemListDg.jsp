      	...
      	<div id="list_table">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_even" exp="{org_abbreviation}<input style='display:none' id='org_id_id{wz_id}' value='{org_id}'/></td>">所属单位</td>
			    </tr>
			  </table>
					<%
			}else{
			...
			<div id="tab_box" class="tab_box">
				<div id="tab_box_content0" class="tab_box_content">
				    ...
				    <tr>
				     ...
				     <tr >
				     <td  class="inquire_item6">入库总数：</td>
...
<script type="text/javascript">
	...
	function refreshData(){
		if(orgId=="C6000000005263"){
			...
			var sql="select i.*,c.code_name, tt.stock_num,tt.actual_price, tt.org_id,tt.org_abbreviation from (select t.wz_id, case when  sum(t.stock_num)- case  when  flat.plan_num is null then 0 else flat.plan_num end is null then 0 else  sum(t.stock_num)- case  when  flat.plan_num is null then 0 else flat.plan_num end end stock_num, round(sum(t.stock_num * t.actual_price) / case when sum(t.stock_num) = 0 then  1 else sum(t.stock_num) end, 3) actual_price, t.org_id,comm.org_abbreviation  from gms_mat_recyclemat_info t  inner join comm_org_information comm on t.org_id = comm.org_id left join  (select sum(plan_num) plan_num,wz_id  from gms_mat_demand_plan_flat where  plan_flat_type='1' and wz_type='"+type+"' group by wz_id ) flat   on t.wz_id=flat.wz_id where t.bsflag = '0' and t.wz_type = '"+type+"' and t.org_id = 'C6000000000039' and t.wz_type = '"+type+"' and t.bsflag = '0'  or t.org_id = 'C6000000000040' and t.bsflag = '0' and t.wz_type = '"+type+"' or t.org_id = 'C6000000005269' and t.bsflag = '0' and t.wz_type = '"+type+"' or t.org_id = 'C6000000005275' and t.bsflag = '0' and t.wz_type = '"+type+"' or t.org_id = 'C6000000005279' and t.bsflag = '0' and t.wz_type = '"+type+"' or t.org_id = 'C6000000005280' and t.bsflag = '0' and t.wz_type = '"+type+"' or t.org_id = 'C6000000005278' and t.bsflag = '0' and t.wz_type = '"+type+"' group by t.wz_id, t.org_id,comm.org_abbreviation,flat.plan_num) tt inner join(gms_mat_infomation i inner join gms_mat_coding_code c on i.coding_code_id = c.coding_code_id and i.bsflag = '0' and c.bsflag = '0') on tt.wz_id = i.wz_id  and tt.stock_num>0 order by i.coding_code_id asc, i.wz_id asc";
			}else{
			//var sql="select i.*,c.code_name,t.stock_num,t.recyclemat_info from gms_mat_recyclemat_info t inner join (gms_mat_infomation i inner join gms_mat_coding_code c on i.coding_code_id=c.coding_code_id and i.bsflag='0' and c.bsflag='0') on t.wz_id=i.wz_id and t.bsflag='0'and t.wz_type='"+type+"' and t.org_id='"+orgId+"'";
			var sql="select i.*, c.code_name, tt.stock_num, tt.actual_price  from (select t.wz_id, case when  sum(t.stock_num)- case  when  flat.plan_num is null then 0 else flat.plan_num end is null then 0 else  sum(t.stock_num)- case  when  flat.plan_num is null then 0 else flat.plan_num end end stock_num, round(sum(t.stock_num * t.actual_price) / case when sum(t.stock_num) = 0 then 1 else sum(t.stock_num) end, 3) actual_price from gms_mat_recyclemat_info  t  left join  (select sum(plan_num) plan_num,wz_id  from gms_mat_demand_plan_flat where  plan_flat_type='1' and wz_type='"+type+"' group by wz_id ) flat   on t.wz_id=flat.wz_id where t.bsflag = '0' and t.wz_type = '"+type+"' and t.org_id = '"+orgId+"'  group by t.wz_id,flat.plan_num) tt inner join(gms_mat_infomation i  inner join gms_mat_coding_code c on i.coding_code_id = c.coding_code_id and i.bsflag = '0' and c.bsflag = '0') on tt.wz_id = i.wz_id  and tt.stock_num>0 order by i.coding_code_id asc, i.wz_id asc";
			}
		cruConfig.cdtType = 'form';
		cruConfig.queryStr = sql;
		cruConfig.currentPageUrl = "/mat/multiproject/matLed/mat/matItemListDg.jsp";
		queryData(1);
	}
	...
	function loadDataDetail(shuaId){
		...
		var retObj;
		if(shuaId!= undefined){
			 retObj = jcdpCallService("MatItemSrv", "getMatRepDg", "laborId="+shuaId+"&orgId="+orgId+"&type="+type);
		    ...
		    if(ids==''){ 
		     ...
		     retObj = jcdpCallService("MatItemSrv", "getMatRepDg", "laborId="+ids+"&orgId="+orgId+"&type="+type);
		}
	...
	var showTabBox = document.getElementById("tab_box_content0");
       ...
       function simpleSearch(){
    	   var sql ='';
			...
			if(orgId=="C6000000005263"){
				//var sql="select i.*,c.code_name,t.stock_num,t.recyclemat_info,org_abbreviation from gms_mat_recyclemat_info t inner join comm_org_information comm on t.org_id=comm.org_id inner join (gms_mat_infomation i inner join gms_mat_coding_code c on i.coding_code_id=c.coding_code_id and i.bsflag='0' and c.bsflag='0') on t.wz_id=i.wz_id and t.bsflag='0'and t.wz_type='"+type+"' and t.org_id in (select org_id from comm_org_information  where org_id= 'C6000000000039' and bsflag='0' or org_id= 'C6000000000040' and bsflag='0' or org_id= 'C6000000005269' and bsflag='0' or org_id= 'C6000000005275' and bsflag='0' or org_id= 'C6000000005279' and bsflag='0' or org_id= 'C6000000005280' and bsflag='0' or org_id= 'C6000000005278' and bsflag='0')";
				var sql="select i.*,c.code_name, tt.stock_num,tt.actual_price, tt.org_id,tt.org_abbreviation from (select t.wz_id, sum(t.stock_num) stock_num, round(sum(t.stock_num * t.actual_price) / case when sum(t.stock_num) = 0 then  1 else sum(t.stock_num) end, 3) actual_price, t.org_id,comm.org_abbreviation  from gms_mat_recyclemat_info t  inner join comm_org_information comm on t.org_id = comm.org_id where t.bsflag = '0' and t.wz_type = '"+type+"' and t.org_id = 'C6000000000039'  and t.bsflag = '0'  or t.org_id = 'C6000000000040' and t.bsflag = '0'  or t.org_id = 'C6000000005269' and t.bsflag = '0' or t.org_id = 'C6000000005275' and t.bsflag = '0' or t.org_id = 'C6000000005279' and t.bsflag = '0' or t.org_id = 'C6000000005280' and t.bsflag = '0' or t.org_id = 'C6000000005278' and t.bsflag = '0' group by t.wz_id, t.org_id,comm.org_abbreviation) tt inner join(gms_mat_infomation i inner join gms_mat_coding_code c on i.coding_code_id = c.coding_code_id and i.bsflag = '0' and c.bsflag = '0') on tt.wz_id = i.wz_id  where wz_name like '%"+wz_name+"%' order by i.coding_code_id asc, i.wz_id asc";
				}else{
				//var sql="select i.*,c.code_name,t.stock_num,t.recyclemat_info from gms_mat_recyclemat_info t inner join (gms_mat_infomation i inner join gms_mat_coding_code c on i.coding_code_id=c.coding_code_id and i.bsflag='0' and c.bsflag='0') on t.wz_id=i.wz_id and t.bsflag='0'and t.wz_type='"+type+"' and t.org_id='"+orgId+"'";
				var sql="select i.*, c.code_name, tt.stock_num, tt.actual_price  from (select t.wz_id, sum(t.stock_num) stock_num, round(sum(t.stock_num * t.actual_price) / case when sum(t.stock_num) = 0 then 1 else sum(t.stock_num) end, 3) actual_price from gms_mat_recyclemat_info t where t.bsflag = '0' and t.wz_type = '"+type+"' and t.org_id = '"+orgId+"'  group by t.wz_id) tt inner join(gms_mat_infomation i  inner join gms_mat_coding_code c on i.coding_code_id = c.coding_code_id and i.bsflag = '0' and c.bsflag = '0') on tt.wz_id = i.wz_id where wz_name like '%"+wz_name+"%'  order by i.coding_code_id asc, i.wz_id asc";
				}
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = sql;
			cruConfig.currentPageUrl = "/mat/multiproject/matLedger/matItemListDg.jsp";
			queryData(1);
       ...
       //入库明细信息
    	 function taskShow(value){
    			...
    			var org_id="";
    			if(orgId=="C6000000005263"){
    				org_id=document.getElementById("org_id_id"+value).value;
        			}else{
        				org_id=orgId;
            			}
    			
    			var retObj = jcdpCallService("MatItemSrv", "findMatInDg", "ids="+value+"&orgId="+org_id+"&type="+type);
    			...
    			for(var i =0; taskList!=null && i < taskList.length; i++){
    				...
    				var stock_num = taskList[i].stock_num;
    				var total_money = taskList[i].total_money;
    				var modifi_date = taskList[i].modifi_date;
    	 ...
    	 function taskOutShow(value){
    			...
    			var retObj = jcdpCallService("MatItemSrv", "findMatOutRepDg", "ids="+value+"&orgId="+org_id+"&type="+type);
    			var taskList = retObj.matInfo;
    			for(var i =0; taskList!=null && i < taskList.length; i++){
    				var org_abbreviation = taskList[i].org_abbreviation;
    	 ...
    	 function AddExcelData(){
		    	popWindow("<%=contextPath%>/mat/multiproject/matLed/repmat/excelRepMatAddDg.jsp?type="+type+"",'1024:800');
		        }   
	        //导出
		 ...
		 function toAdd() {
			 popWindow("<%=contextPath%>/mat/multiproject/matLed/mat/AddItemListDg.jsp?type=<%=type%>",'1024:800');
			}
</script>
