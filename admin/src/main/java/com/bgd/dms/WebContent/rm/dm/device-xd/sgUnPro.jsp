...
<script type="text/javascript"> 
	...
	function loadDataDetail(){
		if(accident_info_id=="null"){
			var querySql="select dev_acc_id, dev_name,dev_model,dev_sign,self_num,license_num,asset_coding,owning_org_name,usage_org_name,dev_position,sd.coding_name as tech_stat,asset_value,net_value,case account_stat when '0' then '账内' when '1' then '账外' else '外租' end as stat_desc from Gms_Device_Account_Unpro b  left join comm_coding_sort_detail sd on b.tech_stat=sd.coding_code_id  where dev_acc_id='"+dev_appdet_id+"'" ;
			...
			var querySql="select a.*,b.dev_name,b.dev_model,b.self_num,b.license_num,b.owning_org_name,b.usage_org_name,sd.coding_name as tech_stat from BGP_COMM_DEVICE_ACCIDENT_INFO a left join Gms_Device_Account_Unpro b on a.DEVICE_ACCOUNT_ID=b.dev_acc_id left join comm_coding_sort_detail sd on b.tech_stat=sd.coding_code_id  where a.accident_info_id='"+accident_info_id+"'" ;
			var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
			basedatas = queryRet.datas;
			$("#dev_name")[0].value=basedatas[0].dev_name;
			$("#dev_model")[0].value=basedatas[0].dev_model;
