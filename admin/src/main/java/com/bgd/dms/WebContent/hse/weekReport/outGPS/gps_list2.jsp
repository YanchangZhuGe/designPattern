...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.queryStr = "select g.hse_gps_id,case when length(g.wrong_reason)>5 then concat(substr(g.wrong_reason, 1, 5), '...') else g.wrong_reason end wrong_reason,"
						        ...
						        +"case when length(g.no_install_reason)>5 then concat(substr(g.no_install_reason, 1, 5), '...') else g.no_install_reason end no_install_reason,"
						      	+"g.use_no,g.install_no,g.normal_no,g.wrong_no,g.rule_no,i.org_abbreviation as org_name,i2.org_abbreviation as org_name2"
						        +" from bgp_hse_common c join bgp_hse_week_gps_out g on c.hse_common_id = g.hse_common_id join comm_org_subjection s on c.org_id = s.org_subjection_id and s.bsflag = '0'"
	...
	function toAddLine(hse_gps_id,orgName,use_no,install_no,normal_no,wrong_no,no_install_reason,wrong_reason,wrong_step,rule_no,rule_reason,rule_step,order){
		var num = order+1;
		...
		td.innerHTML = no_install_reason;
	    td.className = tdClass+'_odd';
	    if(autoOrder%2==0){
			td.style.background = "#f6f6f6";
		}else{
...
//	    div.style.overflow = "auto;
	...
	function loadDataDetail(hse_gps_id){
		...
		if(hse_gps_id!=null&&hse_gps_id!=""){
			 popWindow("<%=contextPath%>/hse/weekReport/outGPS/showDetail.jsp?hse_gps_id="+hse_gps_id);
		}else{
			alert("请选择一条记录");
			return;
		}
