...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <title>单项目-设备接收-设备接收(装备补充单台)</title> 
 </head> 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				if(info[1] =='S14059999'){//仪器附属设备
					prosql = "select amd.actual_in_time,amd.dev_acc_id, amd.asset_coding,amd.dev_plan_start_date,amd.dev_plan_end_date,pro.project_name, ";
					...
					prosql = "select amd.dev_acc_id, amd.asset_coding,amd.dev_plan_start_date,amd.dev_plan_end_date,pro.project_name, ";
					prosql += "amd.self_num, amd.dev_sign, amd.license_num, amd.state,account.dev_name,account.dev_model ";
					prosql += "from gms_device_equ_outdetail_added amd ";
					prosql += "left join gms_device_account account on amd.dev_acc_id=account.dev_acc_id ";
					prosql += "left join gms_device_equ_outform eof on eof.device_outinfo_id=amd.device_outinfo_id ";
					prosql += "left join gp_task_project pro on pro.project_info_no=eof.project_info_no ";
...
<script type="text/javascript">
	...
	function refreshData(v_device_app_name,v_mixinfo_no,v_opr_state_desc){
		var str = "select * from (select devapp.device_app_name,outform.device_outinfo_id,outform.outinfo_no,outform.project_info_no,pro.project_name,";
			...
			str += "case outform.added_opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as oprstate_desc,mif.mix_type_id ";
			...
			//str += "left join gp_task_project pro on outform.project_info_no=pro.project_info_no ";
		...
		if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目
			...
			str += "where outform.bsflag='0' and outform.project_info_no = '" +retFatherNo+"' ";
        	...
        	str += "where outform.bsflag='0' and outform.project_info_no='"+projectInfoNos+"' ";
         }else{
        	str += "left join gp_task_project pro on outform.project_info_no = pro.project_info_no and pro.bsflag = '0' ";
        	str += "where outform.bsflag='0' and outform.project_info_no='"+projectInfoNos+"' ";
	     }

			//str += "where outform.state='9' and outform.bsflag='0' "
			//str += "and outform.project_info_no='"+projectInfoNos+"' "
			...
			str += "union all ";
			str += "select app.device_app_name,t.device_outinfo_id,t.outinfo_no,t.project_info_no,";
			str += "pro.project_name,inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,";
			str += "he.employee_name,t.receive_state,t.create_date,"
			str += "case t.added_opr_state when '1' then '处理中'  when '9' then '已处理'  else '未处理' end as oprstate_desc,'S1405' as mix_type_id ";
			str += "from gms_device_coll_outform t left join gms_device_collmix_form mif on t.device_mixinfo_id=mif.device_mixinfo_id ";
			str += "left join gms_device_collapp app on mif.device_app_id = app.device_app_id ";
			str += "left join gms_device_allapp allapp on allapp.device_allapp_id = app.device_allapp_id ";
			//str += "left join gp_task_project gp on t.project_info_no=gp.project_info_no ";
			str += "left join comm_org_information inorg on t.in_org_id = inorg.org_id ";
			str += "left join comm_org_information outorg on t.out_org_id = outorg.org_id ";
			str += "left join comm_human_employee he on t.print_emp_id = he.employee_id ";

			//井中地震 
			if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目
				str += "left join gp_task_project pro on t.project_info_no = pro.project_info_no and pro.bsflag = '0' and pro.project_type = '5000100004000000008' ";
				str += "where (allapp.allapp_type = 'S1405' or allapp.allapp_type is null) and t.bsflag='0' and t.project_info_no = '" +retFatherNo+"' ";
			}else if(projectType == "5000100004000000008" && retFatherNo.length <= 0){
	        	str += "left join gp_task_project pro on t.project_info_no = pro.project_info_no and pro.bsflag = '0' and pro.project_father_no is null and pro.project_type = '5000100004000000008' ";
	        	str += "where (allapp.allapp_type = 'S1405' or allapp.allapp_type is null) and t.bsflag='0' and t.project_info_no='"+projectInfoNos+"' ";
	         }else{
	        	str += "left join gp_task_project pro on t.project_info_no = pro.project_info_no and pro.bsflag = '0' ";
	        	str += "where (allapp.allapp_type = 'S1405' or allapp.allapp_type is null) and t.bsflag='0' and t.project_info_no='"+projectInfoNos+"' ";
		     }
			
			str += "and exists (select 1 from gms_device_equ_outdetail_added added where added.device_outinfo_id = t.device_outinfo_id)";

			str += "union "+
			"select devapp.device_app_name,outform.device_outinfo_id,outform.outinfo_no,outform.project_info_no,pro.project_name, "+
	        ...
	        "where outform.bsflag='0' and outform.project_info_no='"+projectInfoNos+"' "+
			"and mif.mix_type_id = 'S14059999' ) where 1=1 ";
		if(v_opr_state_desc!=undefined && v_opr_state_desc!=''){
			if(v_opr_state_desc == '1'){//处理中
				str += "and oprstate_desc = '处理中' ";
			}else if(v_opr_state_desc == '9'){//已处理
				str += "and oprstate_desc = '已处理' ";
			}else{//未处理
				str += "and oprstate_desc = '未处理' ";
			}					
		}
		...
		str += "order by oprstate_desc,out_date desc";
    ...
    function loadDataDetail(shuaId){
    	...
    	var mainsql ="select outform.device_outinfo_id,outform.outinfo_no,outform.PROJECT_INFO_NO,pro.project_name, "+
		"inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,"+
		"he.employee_name,outform.receive_state,outform.out_date, "+
		"case outform.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as oprstate_desc "+
		"from gms_device_equ_outform outform "+
		"left join gp_task_project pro on outform.PROJECT_INFO_NO=pro.project_info_no "+
		"left join comm_org_information inorg on outform.in_org_id=inorg.org_id "+
		"left join comm_org_subjection sub on outform.out_org_id=sub.org_subjection_id left join comm_org_information outorg on sub.org_id = outorg.org_id "+
		...
		" union all "+
		"select t.device_outinfo_id,app.device_app_name,t.outinfo_no, gp.project_name,inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,he.employee_name,t.receive_state,t.create_date, case t.opr_state when '1' then '处理中'  when '9' then '已处理'  else '未处理' end as oprstate_desc from gms_device_coll_outform t left join gms_device_collmix_form mif on t.device_mixinfo_id=mif.device_mixinfo_id left join gms_device_collapp app on mif.device_app_id = app.device_app_id left join gp_task_project gp on t.project_info_no=gp.project_info_no  left join comm_org_information inorg on t.in_org_id = inorg.org_id left join comm_org_information outorg on t.out_org_id = outorg.org_id left join comm_human_employee he on t.print_emp_id = he.employee_id where t.state = '9' and t.bsflag = '0'and t.device_outinfo_id = '"+shuaId+"' and exists (select 1 from gms_device_equ_outdetail_added added where added.device_outinfo_id = t.device_outinfo_id)";
    ...
    function toDetailPage(){
		...
		if(info[1] == 'S14059999'){
			window.location.href='<%=contextPath%>/rm/dm/EqDevReceive/subDevReceiveList.jsp?mixId='+shuaId;
		}else{
			window.location.href='<%=contextPath%>/rm/dm/EqDevReceiveAdded/subDevReceiveList.jsp?mixId='+shuaId;
		}    
	}
