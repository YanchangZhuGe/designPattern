      			...
      			<fieldset style="margin-left:2px"><legend>设备技术状况</legend>
			      		<table id="table1" width="100%" border="0" cellspacing="0" cellpadding="1" class="tab_line_height">
							...
							<tr>
								...
								<td class="inquire_form6"><input id="good_num" name="good_num" class="input_width" type="text" onkeyup="techAutoCal(this)"/>
								<input id="good_num1" name="good_num1" class="input_width" type="hidden"/></td>
						  	...
						  	<tr>
								...
								<td class="inquire_form6"><input id="torepair_num" name="torepair_num" class="input_width" type="text" onkeyup="techAutoCal(this)"/>
								<input id="torepair_num1" name="torepair_num1" class="input_width" type="hidden"/></td>
								</td>
						  	...
						  	<tr>
								...
								<td class="inquire_form6"><input id="destroy_num" name="destroy_num" class="input_width" type="text" onkeyup="techAutoCal(this)"/>
								<input id="destroy_num1" name="destroy_num1" class="input_width" type="hidden"/></td>
						    ...
						    <tr>
								...
								<td class="inquire_form6"><input id="tocheck_num" name="tocheck_num" class="input_width" type="text" onkeyup="techAutoCal(this)"/>
								<input id="tocheck_num1" name="tocheck_num1" class="input_width" type="hidden"/></td>
						  	...
						  	<tr>
								...
								<td class="inquire_form6"><input id="noreturn_num" name="noreturn_num" class="input_width" type="text" onkeyup="techAutoCal(this)" />
								<input id="noreturn_num1" name="noreturn_num1" class="input_width" type="hidden"  /></td>
...
<script type="text/javascript"> 
	...
	function loadDataDetail(){
		...
		if(device_backdet_id!=null){
			var querySql="select * from ( select backdet.device_backdet_id,backdet.device_backapp_id,backdet.dev_acc_id,backdet.dev_name,backdet.dev_model,";
		    	...
		    	querySql+="acc.total_num as use_old_total_num,acc.unuse_num as old_unusing_num,acc.use_num as old_using_num,acc.other_num as old_other_num, ";//原数量
		    	querySql+="tech.good_num,tech.touseless_num,tech.torepair_num,tech.tocheck_num,tech.destroy_num ,tech.noreturn_num, ";
		    	querySql+="  nvl(fire.good_num,0) as good_num1,nvl( fire.torepair_num,0)as torepair_num1,nvl( fire.tocheck_num,0)as tocheck_num1,nvl( fire.destroy_num,0)as destroy_num1,nvl(fire.noreturn_num,0) as  noreturn_num1,fire.create_date ";
		    	...
		    	querySql+=" left join gms_device_coll_account_firm fire on fire.device_backdet_id=t.device_coll_backdet_id  ";
		    	querySql+="where t.device_coll_backdet_id='"+device_backdet_id+"' order by fire.check_time desc ) where rownum=1 ";
		...
		//验收时间
		document.getElementById("actual_out_time").value=retObj[0].create_date;
		...
		document.getElementById("old_noreturn_num").value =retObj[0].noreturn_num-retObj[0].noreturn_num1;
		document.getElementById("old_good_num").value =retObj[0].good_num-retObj[0].good_num1;
		//业务需求，验收中不需要待报废，2012-12-29
		//document.getElementById("old_usless_num").value =retObj[0].touseless_num;
		document.getElementById("old_tocheck_num").value =retObj[0].tocheck_num-retObj[0].tocheck_num1;
		document.getElementById("old_torepair_num").value =retObj[0].torepair_num-retObj[0].torepair_num1;
		document.getElementById("old_destroy_num").value =retObj[0].destroy_num-retObj[0].destroy_num1;
		document.getElementById("noreturn_num").value =retObj[0].noreturn_num1;
		document.getElementById("good_num").value =retObj[0].good_num1;
		document.getElementById("tocheck_num").value =retObj[0].tocheck_num1;
		document.getElementById("torepair_num").value =retObj[0].torepair_num1;
		document.getElementById("destroy_num").value =retObj[0].destroy_num1;
		document.getElementById("noreturn_num1").value =retObj[0].noreturn_num1;
		document.getElementById("good_num1").value =retObj[0].good_num1;
		document.getElementById("tocheck_num1").value =retObj[0].tocheck_num1;
		document.getElementById("torepair_num1").value =retObj[0].torepair_num1;
		document.getElementById("destroy_num1").value =retObj[0].destroy_num1;
	...
	function valimation(obj){
		...
		if(!re.test(value)){
			...
			if(parseInt(document.getElementById("good_num").value)<parseInt(document.getElementById("good_num1").value)){
				alert("完好数量必须大于等于"+document.getElementById("good_num1").value+"!");
				obj.value = "";
				return false;
			}
			if(parseInt(document.getElementById("tocheck_num").value)<parseInt(document.getElementById("tocheck_num1").value)){
				alert("盘亏数量必须大于等于"+document.getElementById("tocheck_num1").value+"!");
				obj.value = "";
				return false;
			}
			if(parseInt(document.getElementById("torepair_num").value)<parseInt(document.getElementById("torepair_num1").value)){
				alert("待维修数量必须大于等于"+document.getElementById("torepair_num1").value+"!");
				obj.value = "";
				return false;
			}
			if(parseInt(document.getElementById("destroy_num").value)<parseInt(document.getElementById("destroy_num1").value)){
				alert("毁损数量必须大于等于"+document.getElementById("destroy_num1").value+"!");
				obj.value = "";
				return false;
			}
		}
	...
	function setrecordNum(){
		var good_num1 = Number($("#good_num").val())-Number($("#good_num1").val());
		$("#good_num1").val(good_num1);
		var torepair_num1 =Number($("#torepair_num").val())-Number($("#torepair_num1").val());
		$("#torepair_num1").val(torepair_num1);
		var destroy_num1 =Number($("#destroy_num").val())-Number($("#destroy_num1").val());
		$("#destroy_num1").val(destroy_num1);
		var tocheck_num1 =Number($("#tocheck_num").val())-Number($("#tocheck_num1").val());
		$("#tocheck_num1").val(tocheck_num1);
		var noreturn_num1 =Number($("#noreturn_num").val())-Number($("#noreturn_num1").val());
		$("#noreturn_num1").val(noreturn_num1);
	}
	function submitInfo(){
		...
		if(window.confirm("确认验收?")){
			setrecordNum();
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSubmitCollectStockUpdate.srq?device_backdet_id="+device_backdet_id+"&devaccId="+devaccId+"&device_backapp_id="+device_backapp_id+"&projectInfoNo="+projectInfoNo;
