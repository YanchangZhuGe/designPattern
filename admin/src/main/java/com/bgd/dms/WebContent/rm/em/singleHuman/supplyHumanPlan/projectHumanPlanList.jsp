...
function page_init(){
	...
	if(projectInfoNo!='null'){
		var querySql = "select t.*,rownum from ( select p.apply_team_name, p.apply_team,sum(nvl(p.people_number, 0)) people_number from (select d.apply_team, s1.coding_name apply_team_name, d.people_number from bgp_comm_human_plan_detail d left join comm_coding_sort_detail s1  on d.apply_team = s1.coding_code_id and s1.bsflag = '0' left join comm_coding_sort_detail s2 on d.post = s2.coding_code_id  and s2.bsflag = '0' where d.project_info_no = '"+projectInfoNo+"' and d.bsflag = '0') p  group by p.apply_team_name, p.apply_team order by p.apply_team ) t ";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=10000&querySql='+querySql);
		var datas = queryRet.datas;
		
		var querySql1 = "select p.apply_team_name, p.post_name,p.apply_team, p.post,sum(nvl(p.people_number,0)) people_number from ( select d.plan_detail_id,d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,d.people_number from bgp_comm_human_plan_detail d  left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0' left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' where d.project_info_no='"+projectInfoNo+"' and d.bsflag='0' ) p group by p.apply_team_name,p.post_name ,p.apply_team, p.post  ";
