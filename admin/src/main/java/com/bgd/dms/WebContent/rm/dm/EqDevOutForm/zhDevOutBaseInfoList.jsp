...
<%
	...
	if(!orgsubid.startsWith("C105008")){//不是综合物化探处的人员登录
		orgsubid = "C105008";
	}
%>
...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>多项目-设备出库-设备出库(综合物化探)</title> 
 </head> 
      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			    <auth:ListButton functionId="" css="jh" event="onclick='toDetailPage()'" title="查看明细"></auth:ListButton>
			    <auth:ListButton functionId="" css="tj" event="onclick='toSumbitDevApp()'" title="确认出库"></auth:ListButton>
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_mixinfo_id_{device_mixinfo_id}' name='device_mixinfo_id'>
			     	<td class="bt_info_odd" exp="<input type='checkbox' name='selectedbox' oprstate='{state}' value='{device_mixinfo_id}' id='selectedbox_{device_mixinfo_id}'  onclick='chooseOne(this);'/>" >选择</td>
				...
				<div id="tab_box_content1" name="tab_box_content1" idinfo="" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							...
							<!-- <td class="bt_info_odd" width="13%">操作手</td> -->
				        </tr>
...
<script type="text/javascript">
	...
	function refreshData(v_device_app_name,v_mixinfo_no,v_opr_state_desc){
		var str = "select dm.state,dm.create_date,dm.mixinfo_no,devapp.device_app_name,he.employee_name,pro.project_name,dm.device_mixinfo_id,inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,";
			str+= "case dm.state when '8' then '未处理' when '9' then '已处理' else '未处理' end as oprstate_desc ";
        	...
        	str+= "left join comm_org_subjection orgsub on dm.mix_org_id = orgsub.org_id ";
			str+= "where (dm.state='8' or dm.state='9') and dm.bsflag='0' and (dm.mixform_type='1' or dm.mixform_type='3') and dm.mix_type_id='S0000' ";
			//加机构权限
			str+= "and dm.out_org_id like '<%=orgsubid%>%' ";
    ...
    function loadDataDetail(shuaId){
		...
		document.getElementById("dev_state").value =retObj.devicerecMap.zhoprstate_desc;
		//重新加载当前标签页信息
    ...
    function toDetailPage(){
		...
		window.location.href='<%=contextPath%>/rm/dm/EqDevOutForm/zhDevOutInfoList.jsp?mixId='+shuaId;
    }
	...
	function toSumbitDevApp(){
		...
		$("input[type='checkbox'][name='selectedbox']").each(function(){
			if(this.checked){
				opr_state = this.oprstate;
				mixinfoid = this.value;
				length = length+1;
			}
		});
		if(length == 0){
		...
		if(confirm("确认提交？")){
			var retObj = jcdpCallService("DevInsSrv", "toUpDevOperator","mixinfoid="+mixinfoid);
