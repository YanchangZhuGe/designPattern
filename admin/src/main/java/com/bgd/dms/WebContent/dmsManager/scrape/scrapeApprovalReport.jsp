...
var scrape_report_id='<%=scrape_report_id%>';
	function pass(){
			var form = document.getElementById("CheckForm");
			...
			if(result=="END_NODE"){
				baseData = jcdpCallService("ScrapeSrv", "updateScrapeReportInfo", "scrape_report_id="+scrape_report_id);
			}
			form.submit();
			window.close();
...
<div id="tab_box_content1" class="tab_box_content" style="display:none;height: 730px" >
	<iframe id="businessIframe" name="businessIframe" width="100%" height="100%" style="display: block" src='<%=contextPath%>/dmsManager/scrape/scrapeReportAddForProc.jsp<%=linkParaStr%>'> </iframe>
</div>
