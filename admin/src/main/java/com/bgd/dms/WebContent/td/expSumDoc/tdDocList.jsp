...
<%
	...
	if("5000100004000000008".equals(projectType)){//井中
		response.sendRedirect(contextPath+"/td/expSumDoc/ws/tdDocList.jsp?fileAbbr=JSSYZJ");
	}
	if("5000100004000000009".equals(projectType)){//综合物化探
		response.sendRedirect(contextPath+"/td/expSumDoc/wt/tdDocList.jsp?fileAbbr=JSSYZJ");
	}
...
<script type="text/javascript">
    ...
    function loadDataDetail(ids){
  	  ...
  	  processNecessaryInfo={         
	    		...
	    		businessType:"5110000004100000063",        //业务类型 即为之前设置的业务大类
	    		businessId: ids.split("-")[0],         //业务主表主键值
	    		businessInfo:"试验总结文档审核",        //用于待审批界面展示业务信息
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/ConsdesignDoc/add_tdDocModifyNew.jsp?docType=0110000061100000006&fileAbbr='+fileAbbr+'&parent_file_id=<%=parent_file_id%>');
	...
	function toEdit() {
	    ...
	    popWindow('<%=contextPath%>/td/ConsdesignDoc/edit_tdDocModifyNew.jsp?id='+ids.split("-")[0]+'&parent_file_id=<%=parent_file_id%>');
