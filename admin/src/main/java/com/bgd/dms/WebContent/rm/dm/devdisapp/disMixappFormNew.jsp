...
<script type="text/javascript"> 
	...
	function refreshData(){
		...
		if('<%=devappid%>'!=null){
			//回填基本信息
			var str = "select device_app_no,devplan.project_info_no,mix_type_id,tp.project_name ";
			str += "from gms_device_disapp devplan "
			+"left join gp_task_project tp on devplan.project_info_no=tp.project_info_no " 
			+"where devplan.device_app_id='<%=devappid%>'";
			...
			//回填明细信息
			var str = "select appdet.device_app_detid,appdet.project_info_no,sd.coding_name as unitname,teamsd.coding_name as teamname,appdet.isdevicecode,";
				prosql += "case when appdet.isdevicecode='N' then ci.dev_ci_name else ct.dev_ct_name end as dev_ci_name,";
				prosql += "case when appdet.isdevicecode='N' then ci.dev_ci_model else '' end as dev_ci_model, ";
				str += "appdet.unitinfo,appdet.apply_num,appdet.teamid,appdet.team,nvl(tmp.mixappednum,0) as mixappednum, ";
				str += "appdet.purpose,appdet.employee_id,";
				...
				str += "where appdet.device_app_id='<%=devappid%>' and appdet.bsflag='0' ";
		...
		if(retObj!=undefined){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td><input name='mixappednum"+index+"' id='mixappednum"+index+"' value='"+retObj[index].mixappednum+"' size='6'  type='text' readonly/></td>";
