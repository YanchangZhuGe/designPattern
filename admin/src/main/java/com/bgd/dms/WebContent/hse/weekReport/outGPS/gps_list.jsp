...
function toAddLine(hse_gps_id,use_no,install_no,normal_no,wrong_no,no_install_reason,wrong_reason,wrong_step,rule_no,rule_reason,rule_step){
	
	var hse_gps_id  = hse_gps_id ? hse_gps_id : "";
	var use_no = use_no ? use_no : ""; 
	var install_no = install_no ? install_no : ""
	var normal_no = normal_no ? normal_no : "";
	var wrong_no = wrong_no ? wrong_no : "";
	var no_install_reason = no_install_reason ? no_install_reason : "";
    ...
    td.innerHTML = "<input id='install_no"+rowNum+"' value='"+install_no+"' size='7'/>";
    td.className =tdClass+'_even'
    if(autoOrder%2==0){
		td.style.background = "#FFFFFF";
	}else{
	...
	td.innerHTML = "<textarea id='no_install_reason"+rowNum+"'>"+no_install_reason+"</textarea>";
    td.className = tdClass+'_odd';
    if(autoOrder%2==0){
		td.style.background = "#f6f6f6";
	}else{
...
function toAdd(){ 
	...
	for (var i="0";i<orders.length;i++){
		...
		var install_no = document.getElementById("install_no"+order).value;
		var normal_no = document.getElementById("normal_no"+order).value;
		var wrong_no = document.getElementById("wrong_no"+order).value;
		var no_install_reason = document.getElementById("no_install_reason"+order).value;
		...
		if(install_no!=null&&install_no!=""){
			if (!re.test(install_no))
			   {
			       alert("GPS安装台数台数请输入数字！");
			       return ;
			    }
		}
		...
		var jcdp_tables="[['bgp_hse_common'],['bgp_hse_week_gps_out','hse_common_id=bgp_hse_common']]"
		             +"&bgp_hse_common={'hse_common_id':'"+hse_common_id+"','week_start_date':'<%=week_date%>','week_end_date':'<%=week_end_date%>','org_id':'<%=org_id%>','subflag':'0',"
		           	...
		           	jcdp_tables = jcdp_tables +"&bgp_hse_week_gps_out={'hse_gps_id':'"+hse_gps_id+"','use_no':'"+use_no+"','install_no':'"+install_no+"','normal_no':'"+normal_no+"','wrong_no':'"+wrong_no+"','no_install_reason':'"+encodeURI(encodeURI(no_install_reason))+"','wrong_reason':'"+encodeURI(encodeURI(wrong_reason))+"','wrong_step':'"+encodeURI(encodeURI(wrong_step))+"','rule_no':'"+rule_no+"','rule_reason':'"+encodeURI(encodeURI(rule_reason))+"','rule_step':'"+encodeURI(encodeURI(rule_step))+"','gps_order':'"+order+"'}";
		...
		var checkSql="select c.hse_common_id id,d.* from bgp_hse_common c left join bgp_hse_week_gps_out d  on c.hse_common_id=d.hse_common_id where to_char(c.week_start_date,'yyyy-MM-dd')='<%=week_date %>' and c.bsflag='0' and c.org_id='<%=org_id%>' order by d.gps_order asc";
	...
	//删除已经删除的某条明细
	var retObj = jcdpCallService("HseSrv", "deleteDetail", "ids="+hse_gps_ids+"&table=bgp_hse_week_gps_out&key=hse_gps_id");
	...
	function toEdit(){
		...
		if(datas==null||datas==""){
			...
			action="add";	
			if(organ_flag=="0"){
				var checkSql3="select g.* from bgp_hse_common c join bgp_hse_org ho on c.org_id = ho.org_sub_id left join bgp_hse_week_gps_out g on c.hse_common_id = g.hse_common_id where to_char(c.week_start_date, 'yyyy-MM-dd') = '<%=week_date %>' and c.bsflag = '0' and c.subflag = '1' and ho.father_org_sub_id = '<%=org_id%>'";
				...
				if(datas3!=null&&datas3!=""){
					for (var i = 0; i<datas3.length; i++) {		
						toAddLine(
							...
							datas3[i].install_no ? datas3[i].install_no : "",
							datas3[i].normal_no ? datas3[i].normal_no : "",
							datas3[i].wrong_no ? datas3[i].wrong_no : "",
							datas3[i].no_install_reason ? datas3[i].no_install_reason : "",
			...
			for (var i = 0; i<datas.length; i++) {		
				...
				toAddLine(
						...
						datas[i].install_no ? datas[i].install_no : "",
						datas[i].normal_no ? datas[i].normal_no : "",
						datas[i].wrong_no ? datas[i].wrong_no : "",
						datas[i].no_install_reason ? datas[i].no_install_reason : "",
