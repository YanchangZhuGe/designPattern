...
<title>多项目-计划审批-自有设备申请-分配出库-大港自有地震仪器明细出库页面</title>
</head>
      ...
      <table border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td class="inquire_form4" >
          	...
          	<input name="deviceallappid" id="deviceallappid" type="hidden" value="" />
          </td>
        ...
        <tr>
          ...
          <td class="inquire_form4" >
          	...
          	<img id="show-btn" src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" 
          	     style="cursor:hand;" onclick="showOwingTreePage('out_org')" />
         ...
         <!-- <tr>
          <td class="inquire_item4" ><font color=red>*</font>&nbsp;转出单位:</td>
          <td class="inquire_form4" >
          	 <input name="owning_org_name" id="owning_org_name" class="input_width" type="text"  value="" readonly/>
          	<img id="show-btn" src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" 
          	     style="cursor:hand;" onclick="showOwingTreePage('owning_org')" />
          	<input name="owning_org_id" id="owning_org_id" class="input_width" type="hidden"  value="" />

          </td>
          <td class="inquire_item4" ><font color=red>*</font>&nbsp;所在位置:</td>
          <td class="inquire_form4" >
          	 <input name="out_org_name" id="out_org_name" class="input_width" type="text"  value="" readonly/>
          	<img id="show-btn" src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" 
...
<script type="text/javascript">
	...
	function submitInfo(state){
		...
		//给调配单号设置成空
		$("#mixinfo_no").val("");
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveEQBatchMixFormDetailInfoDg.srq?state="+state+"&count="+count+"&line_infos="+line_infos+"&idinfos="+idinfos+"&detailcount="+detailcount+"&addedcount="+addedcount+"&addedline_info="+addedline_info;
	...
	function fillAppDetailNouseNum(orgidvalue){
		...
		var mainsql = "select usage_org_id,device_id,total_num,unuse_num from gms_device_coll_account account ";
			mainsql += "where account.usage_org_id='"+orgidvalue+"' and account.owning_org_id='"+owingOrgId+"' and account.bsflag='0' and account.ifcountry !='国外' and ";
			//mainsql += "where account.usage_sub_id like '%"+orgidvalue+"%' and account.owning_sub_id like '%"+owingOrgId+"%' and account.bsflag='0' and account.ifcountry !='国外' and ";
	...
	function toAddAddedDetailInfos(){
		...
		innerhtml += "<td width='4%'><input type='checkbox' name='addedseq' id='"+addedseqinfo+"' checked/></td>";
	...
	function refreshData(){
		...
		if('<%=devappid%>'!=null){
			//回填基本信息
			var mainsql = "select devapp.device_allapp_id,devapp.device_app_no,devapp.project_info_no,devapp.app_org_id as in_org_id,";
			...
			retObj = proqueryRet.datas;
			if(retObj!=undefined && retObj.length>0){				
				$("#deviceallappid").val(retObj[0].device_allapp_id);
		...
		if(retObj!=undefined){
			for(var index=0;index<retObj.length;index++){
				...
				innerhtml += "<td width='8%'><input type='checkbox' name='detinfo' id='"+index+"' value='"+retObj[index].device_detsubid+"' checked/></td>";
				innerhtml += "<td width='10%'><input name='teamname"+index+"' id='teamname"+index+"' style='line-height:15px' value='"+retObj[index].team_name+"' size='8' type='text' readonly/>";
				innerhtml += "<input name='team"+index+"' id='team"+index+"' style='line-height:15px' value='"+retObj[index].team+"' type='hidden' /></td>";
				innerhtml += "<td width='15%'><input name='devicename"+index+"' id='devicename"+index+"' style='line-height:15px' value='"+retObj[index].device_name+"' size='12' type='text' readonly/>";
				innerhtml += "<input name='deviceid"+index+"' id='deviceid"+index+"' style='line-height:15px' value='"+retObj[index].device_id+"' type='hidden' /></td>";
				innerhtml += "<td width='16%'><input name='devicemodel"+index+"' id='devicemodel"+index+"' value='"+retObj[index].device_model+"' size='12' type='text' readonly/></td>";
				innerhtml += "<td width='20%'><input name='unitname"+index+"' id='unitname"+index+"' value='"+retObj[index].unit_name+"' size='8' type='text' readonly/>";
				innerhtml += "<input name='unitid"+index+"' id='unitid"+index+"' value='"+retObj[index].unit_id+"' type='hidden' /></td>";
				
				innerhtml += "<td width='10%'><input name='applynum"+index+"' id='applynum"+index+"' style='line-height:15px' value='"+retObj[index].applynum+"' size='8' type='text' readonly/></td>";
				...
				innerhtml += "<td width='12%'><input name='devremark"+index+"' id='devremark"+index+"' value='' size='10' type='text' /></td>";
		...
		if(devObj!=undefined){
			for(var index=0;index<devObj.length;index++){
				...
				innerhtml += "<td><input type='checkbox' name='addedseq' id='"+index+"' checked/></td>";
				innerhtml += "<td><input name='addedteamname"+index+"' id='addedteamname"+index+"' value='"+devObj[index].coding_name+"' style='line-height:18px;width:98%' size='10' type='text' readonly/>";
				innerhtml += "<input name='addedteam"+index+"' id='addedteam"+index+"' value='"+devObj[index].team+"' type='hidden' /></td>";
				innerhtml += "<td><input name='addeddevicename"+index+"' id='addeddevicename"+index+"' idindex='"+index+"' value='"+devObj[index].device_name+"' style='line-height:18px;width:98%' size='10' type='text' /></td>";
				innerhtml += "<td><input name='addeddevicetype"+index+"' id='addeddevicetype"+index+"' value='"+devObj[index].device_model+"' style='line-height:18px;width:98%' size='10' type='text' /></td>";
				innerhtml += "<td><input name='addedunit"+index+"' id='addedunit"+index+"' value='"+devObj[index].unit_name+"' style='line-height:18px;width:98%' size='8' type='text' /></td>";
				innerhtml += "<td><input name='addedassignnum"+index+"' id='addedassignnum"+index+"' style='line-height:18px;width:98%' type='text' size='8' onkeyup='checkaddedNum(this)' value='"+devObj[index].device_num+"'/></td>";
				innerhtml += "<td><input name='addedremark"+index+"' id='addedremark"+index+"' value='"+devObj[index].devremark+"' style='line-height:18px;width:98%' size='10' type='text' /></td>";
				innerhtml += "</tr>";
				$("#addeddetailtable").append(innerhtml);
				addedseqinfo++;
			}
