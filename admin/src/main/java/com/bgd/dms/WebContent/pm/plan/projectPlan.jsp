...
function save(){
	...
	if(ctt.frames.length != 0){
		ctt.frames["if1"].saveBaseInfo();
		ctt.frames["if2"].saveOrgInfo();
	}
...
function updateData(projectObjectId,targetProjectObjectId){
	...
	document.getElementById("selectProject").style.display="none";
	document.getElementById("data").style.display="block";
	var temp = jcdpCallService("P6ProjectSrv", "queryBaselineProject", "projectObjectId="+projectObjectId);
	if(temp.datas != null){
			debugger;
			var map = temp.datas[0];
