	...
	<div id="list_table">
		...
		<div id="table_box">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
				<tr>
					<td style="width:40px;" class="bt_info_odd" exp="<input type='radio' name='rdo_entity_id' value='{info_id}' id='rdo_entity_id_{info_id}'/>" >选择</td>
		...
		<div id="tab_box" class="tab_box">
			<div id="tab_box_content0" class="tab_box_content">
				<iframe width="100%" height="100%" name="list" id="d_list" frameborder="0" src="<%=contextPath%>/dmsManager/assessment/jbzx/jbzx_detail_list.jsp" marginheight="0" marginwidth="0" >
				</iframe>	
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	...
	cruConfig.queryOp = "queryJbzxInfoList";
	...
	//新增
	function toAdd(){ 
		window.location.href='<%=contextPath %>/dmsManager/assessment/jbzx/jbzx_edit.jsp?flag=add';	
	}
	//修改
	function toEdit(){ 
		...
		window.location.href='<%=contextPath %>/dmsManager/assessment/jbzx/jbzx_edit.jsp?flag=update&id='+ids;	
	}
	...
	function toDelete(){
		...
		if(confirm('确定要删除吗?')){  
			var retObj = jcdpCallService("IndexAssessSrv", "deleteJbzxInfo", "id="+ids);
			if(typeof retObj.operationFlag!="undefined"){
				...
				if(''!=dOperationFlag){
					...
					if("success"==dOperationFlag){
						...
						$("#d_list").attr("src","<%=contextPath%>/dmsManager/assessment/jbzx/jbzx_detail_list.jsp?id=");
					}
				}
			}
		}
	...
	function loadDataDetail(ids){
		selected_id=ids;
		if(0==tab_index){
			$("#d_list").attr("src","<%=contextPath%>/dmsManager/assessment/jbzx/jbzx_detail_list.jsp?id="+ids);
		}
	}
	//选择年月
