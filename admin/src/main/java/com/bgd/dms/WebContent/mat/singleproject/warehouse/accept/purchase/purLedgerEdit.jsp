...
<form name="form1" id="form1" method="post"
	...
	<input name='invoices_id' id='invoices_id'  type='hidden' value='<gms:msg msgTag="matInfo" key="invoices_id"/>'readonly/>
	...
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" id="queryRetTable">
			<tr>
				...
				<td class="bt_info_odd" exp="<input name='invoice_num_{wz_id}' type='text' style='width:20px' value='{receive_number}'/>">发票号</td>
				<td class="bt_info_even" exp="{wz_name}">名称</td>
				<td class="bt_info_odd" exp="{wz_prickie}">单位</td>
				<td class="bt_info_even"
					exp="<input name='mat_num_{wz_id}'  type='text' style='width:20px' value='{mat_num}'  onkeyup='checkNum()'/>">数量</td>
				<td class="bt_info_odd"
					exp="<input name='actual_price_{wz_id}' type='text' style='width:20px' value='{actual_price}'  onkeyup='checkNum()'/>">入库单价</td>
					<td class="bt_info_even" exp="<input name='total_money_{wz_id}' type='text' style='width:20px' value='{total_money}'/>">金额</td>
					<td class="bt_info_odd" exp="<input name='warehouse_number_{wz_id}' type='text' style='width:20px' value='{warehouse_number}'/>">收料库</td>
					<td class="bt_info_even" exp="<input name='goods_allocation_{wz_id}' type='text' style='width:20px' value='{goods_allocation}'/>">货位</td>
			</tr>
		</table>
	</div>
...
<script type="text/javascript"><!--
	...
	function refreshData(){
		...
		document.getElementById("plan_invoice_name").value = "<gms:msg msgTag="matInfo" key="submite_id"/>";
		...
		document.getElementById("supplier").value = "<gms:msg msgTag="matInfo" key="supplier"/>";
		...
		cruConfig.submitStr ="invoicesId="+invoicesId;
		queryData(1);
	}							
	function save(){	
		//if (!checkForm()) return;
