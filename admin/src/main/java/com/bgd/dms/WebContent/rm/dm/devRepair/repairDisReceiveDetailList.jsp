...
<%
	...
	String team = user.getOrgId();
	String empId = user.getEmpId();
	//System.out.println("subId == "+subId);
	//"8a9588b63618fc0d01361a93e0bf0018";单据状态编制0 生效1 作废2
	...
	<div id="list_table">
					...
					<td background="<%=contextPath%>/images/list_15.png"><table
							...
							<tr>
								...
								<auth:ListButton functionId="" css="zj"
									event="onclick='toAddDevRepairPage()'" title="新增"></auth:ListButton>
									 -->
								<auth:ListButton functionId="" css="pgjsn"
									event="onclick='toReceiveOrReject(3)'" title="拒收"></auth:ListButton>
								<auth:ListButton functionId="" css="pgjsy"
									event="onclick='toReceiveOrReject(2)'" title="接收"></auth:ListButton>
								<!-- 
								...
								<auth:ListButton functionId="" css="dc"
									...
									event="onclick='toSumbitRepairApp()'" title="提交"></auth:ListButton>
									 -->
		...
		<div id="table_box">
			<table style="width: 98.5%" border="0" cellspacing="0"
				...
				<tr id='dispatch_id_{dispatch_id}' name='dispatch_id' idinfo='{dispatch_id}'>
					<td class="bt_info_odd"
						exp="<input type='checkbox' name='selectedbox' value='{dispatch_id}'  stu='{assign_status}' id='selectedbox_{dispatch_id}' onclick='chooseOne(this)'/>">选择</td>
					...
					<td class="bt_info_even" exp="{dispath_user_name}">派工人</td>
					<td class="bt_info_odd" exp="{dispath_date}">派工日期</td>
					...
					<td class="bt_info_even" exp="{assign_status}">状态</td>
					<!-- 
					<td class="bt_info_odd" exp="{rec_status_desc}">接收状态</td>
					 -->
				</tr>
			</table>
		</div>
		...
		<div id="tab_box" class="tab_box">
			<div id="tab_box_content0" name="tab_box_content0"
				...
				<table id="projectMap" name="projectMap" border="0" cellpadding="0"
					...
					<td class="inquire_form6"><input id="repair_form_code"
					...
					<tr>
						...
						<td class="inquire_form6"><input id="todo_comp_name"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
						<td class="inquire_item6">&nbsp;设备名称：</td>
						<td class="inquire_form6"><input id="dev_name"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
					</tr>
					<tr>
						<td class="inquire_item6">规格型号：</td>
						<td class="inquire_form6"><input id="dev_model"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
						<td class="inquire_item6">实物标识号：</td>
						<td class="inquire_form6"><input id="dev_sign"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
							<td class="inquire_item6">派工人：</td>
						<td class="inquire_form6"><input id="dispath_user_name"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
					</tr>
					<tr>
						<td class="inquire_item6">派工日期：</td>
						<td class="inquire_form6"><input id="dispath_date"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
							<td class="inquire_item6">维修班组：</td>
						<td class="inquire_form6"><input id="repair_team"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
						<td class="inquire_item6">状态：</td>
						<td class="inquire_form6"><input id="assign_status"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
					</tr>
				</table>
			</div>
...
$(function(){
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			...
			innerHTML += "<td>"+datas[i].dev_status+"</td><td>"+datas[i].assign_status+"</td><td>"+datas[i].remark+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
    ...
    function loadDataDetail(id){
    	var retObj;
		if(id!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getDispatchReciveDetailInfo", "id="+id);
		    ...
		    retObj = jcdpCallService("DevCommInfoSrv", "getDispatchReciveDetailInfo", "id="+ids);
		}
		//选中这一条checkbox
		$("#selectedbox_"+retObj.dispatchMap.dispatch_id).attr("checked","checked");
		//取消其他选中的
		$("input[type='checkbox'][name='selectedbox'][id!='selectedbox_"+retObj.dispatchMap.dispatch_id+"']").removeAttr("checked");
		//给数据回填
		$("#repair_form_code","#projectMap").val(retObj.dispatchMap.repair_form_code);
		$("#repair_form_name","#projectMap").val(retObj.dispatchMap.repair_form_name);
		$("#project_name","#projectMap").val(retObj.dispatchMap.project_name);
		$("#req_comp_name","#projectMap").val(retObj.dispatchMap.req_comp_name);
		$("#todo_comp_name","#projectMap").val(retObj.dispatchMap.todo_comp_name);
		$("#dev_name","#projectMap").val(retObj.dispatchMap.dev_name);
		$("#dev_model","#projectMap").val(retObj.dispatchMap.dev_model);
		$("#dev_sign","#projectMap").val(retObj.dispatchMap.dev_sign);
		$("#dispath_user_name","#projectMap").val(retObj.dispatchMap.dispath_user_name);
		$("#dispath_date","#projectMap").val(retObj.dispatchMap.dispath_date);
		$("#assign_status","#projectMap").val(retObj.dispatchMap.assign_status);
		$("#repair_team","#projectMap").val(retObj.dispatchMap.repair_team);
		var device_repair_name = retObj.dispatchMap.repair_form_name;
		var device_repair_no = retObj.dispatchMap.repair_form_code;
		//重新加载当前标签页信息
		getContentTab(undefined,selectedTagIndex);
    }
	function toAddDevRepairPage(){
		popWindow('<%=contextPath%>/rm/dm/devRepair/repairNewApply.jsp');
	}
	function toModifyDevRepairPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devRepair/repairDispatchDetail.jsp?id='+id,'800:650');
	}
	function toDelDevRepairPage(){
		...
		//什么状态不能删除，和业务专家确定
		if(confirm("是否执行删除操作?")){
			var sql = "update GMS_DEVICE_COLL_REPAIRFORM set bsflag='1' where id in ("+selectedid+")";
	...
	function toSumbitRepairApp(){
		...
		//判断状态
		var querySql = 
			"SELECT T.STATUS FROM GMS_DEVICE_COLL_REPAIRFORM T\n" +
		...
		//查看是否添加了明细
		var querySql = "SELECT COUNT(1) as SUMNUM FROM GMS_DEVICE_COLL_REPAIR_SUB T\n" +
			"WHERE T.BSFLAG = 0 AND T.REPAIRFORM_ID in("+selectedid+") ";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		var basedatas = queryRet.datas;
		var detflag = false;
		for(var index = 0;index<basedatas.length;index++){
		...
		if (window.confirm("确认要提交吗?")) {
			//submitProcessInfo();
			//将送修表的编制状态改为生成
			var sql = "update GMS_DEVICE_COLL_REPAIRFORM set status='1' where ID in ("+selectedid+")";
			var accsql = 
	...
	function toReceiveOrReject(val){
		var length = 0;
		var selectedid = "";
		var rlt = "操作成功!";
		var rtn = false;
		...
		$("input[type='checkbox'][name='selectedbox']").each(function(){
			if(this.checked){
				...
				selectedid += "'"+this.value+"'";
				length = length+1;
				if(this.stu.indexOf("已接")>=0){
					alert("包含状态为："+this.stu+"的单据，无法提交！");
					rtn = true;
					return true;
				}
				if(this.stu.indexOf("拒")>=0){
					alert("包含状态为："+this.stu+"的单据，无法提交！");
					rtn = true;
					return true;
				}
		...
		var status = '';
		if(val == 3){
			status = <%=DevConstants.DEV_REPAIR_RECIEVESTATE2 %>;
			warn = "确定要拒收吗?";
		}else if(val == 2){
			status = <%=DevConstants.DEV_REPAIR_RECIEVESTATE1 %>;
			warn = "确定要接收吗?";
		}
		if (window.confirm(warn)) {
			var sql = "update GMS_DEVICE_COLL_REPAIR_ASSIGN set RECIEVESTATE='"
			+ status + "',in_user='<%=empId %>',in_date=sysdate where ID in ("+selectedid+")";
			var path = cruConfig.contextPath+"/rad/asyncDelete.srq";
			var params = "deleteSql="+sql;
			params += "&ids=";
			syncRequest('Post',path,params);
			if(val == 2){
				var sql2 = "update GMS_DEVICE_COLL_REPAIR_SUB set dev_status='"
					 + "<%=DevConstants.DEV_REPAIR_REPAIRSTATE1 %>' where ID =( select t.repair_id from GMS_DEVICE_COLL_REPAIR_ASSIGN t where t.id = "+selectedid+")";
					var path2 = cruConfig.contextPath+"/rad/asyncDelete.srq";
					var params2 = "deleteSql="+sql2;
					params2 += "&ids=";
					syncRequest('Post',path2,params2);
			}
			refreshData();
		}
	}
