			...
			<div id="table_box">
			  <table width="98%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable" >		
			     <tr id='dev_acc_id_{dev_acc_id}' name='dev_acc_id' idinfo='{dev_acc_id}'>
			     	<td class="bt_info_odd" exp="<input type='radio' name='selectedbox' value='{dev_acc_id}' id='selectedbox_{dev_acc_id}' />" >选择</td>
					...
					<td class="bt_info_even" exp={orgname}>所属单位</td>
			     </tr>
			  </table>
			</div>
...
<script type="text/javascript">
	...
	function searchDevData(){
		...
		refreshData(v_dev_ci_name,v_dev_ci_model,v_dev_sign,v_self_num,v_license_num);
	}
	
	function refreshData(v_dev_ci_name,v_dev_ci_model,v_dev_sign,v_self_num,v_license_num){
	
		var str= "select account.*,f.org_abbreviation as orgname from gms_device_account account ";
		str += "  left join comm_org_information f on f.org_id=account.owning_org_id where account.bsflag='0' and account.owning_sub_id like '<%=suborgid%>%'";
		str += " and account.ifproduction='5110000186000000002' ";
		str += " and (account.dev_type like 'S08%' or account.dev_type like 'S0623%' or account.dev_type like 'S060102%' or account.dev_type like 'S060101%' or account.dev_type like 'S060199%' ) ";
	...
	function submitInfo(){
		...
		for(var index = 0;index<obj.length;index++){
			...
			if(selectedobj.checked == true){
				...
				//返回信息是 队级台账id + 设备编号+ 设备名称 + 规格型号 + 自编号 + 实物标识号 + 牌照号  + 设备编码 + 所属单位(新补充)
				selectedids += "~"+selectedobj.parentNode.parentNode.cells[2].innerText+"~"+selectedobj.parentNode.parentNode.cells[3].innerText+"~"+selectedobj.parentNode.parentNode.cells[4].innerText;
