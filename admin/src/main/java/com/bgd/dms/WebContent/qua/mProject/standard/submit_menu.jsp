			...
			<div id="new_table_box_bg">
				<table width="100%" style="height: 100%" border="0" cellspacing="0" cellpadding="0" class=""><!-- tab_line_height -->
			     	<tr style="width: 100%;height: 100%">
				    	<td width="50%" height="100%"><iframe src="<%=contextPath %>/qua/mProject/standard/project_tree.jsp" name="mainTopframe" frameborder="no" scrolling="auto"  id="mainTopframe" width="100%" height="100%"></iframe></td><!-- style="border-right: 2px solid #5796DD; cursor: w-resize;" -->
				    	<td width="50%" height="100%"><iframe src="<%=contextPath %>/qua/mProject/standard/standard_list.jsp" name="menuFrame" frameborder="no" scrolling="auto"  id="menuFrame" width="100%" height="100%"></iframe></td> <!-- style="border-left: 2px solid #5796DD; cursor: w-resize;" -->
				    </tr>
			    </table>
			</div> 
			<div id="oper_div">
	...
	function newSubmit() {
		...
		for(var i =0 ;i<org_array.length;i++){
			...
			if(project_info_no!=null && project_info_no!=''){
				if(ids==null || ids==''){
					substr = substr + "delete from bgp_qua_files t where t.project_info_no = '"+project_info_no+"' ;";
				}else{
					substr = substr + "delete from bgp_qua_files t where t.project_info_no = '"+project_info_no+"' and t.file_id not in("+ids+");";
				}
			}else{
				if(ids==null || ids==''){
					substr = substr + "delete from bgp_qua_files t where t.org_subjection_id = '"+org_subjection_id+"' ;";
				}else{
					substr = substr + "delete from bgp_qua_files t where t.org_subjection_id = '"+org_subjection_id+"' and t.file_id not in("+ids+");";
				}
			}
			for(var j =0;j<file_ids.length && file_ids[j]!=null && file_ids[j]!='';j++){
				var file_id = file_ids[j];
				var sql = "select * from bgp_qua_files t where t.bsflag ='0' "+
				...
				substr = substr + "insert into bgp_qua_files(qua_file_id,project_info_no,file_id,org_id,org_subjection_id,"+
