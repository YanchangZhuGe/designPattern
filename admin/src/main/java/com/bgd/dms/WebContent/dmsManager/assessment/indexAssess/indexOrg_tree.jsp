...
function clickNode(selectedNode){
	parent.mainFrame.location.href =contextPath+"/dmsManager/assessment/indexAssess/indexOrg_list.jsp?org_id="+selectedNode.id.split('_')[0]+"&subOrg_id="+selectedNode.id.split('_')[1];
}
...
function getSubNodes(parentNode) {
	...
	var querySql;
	if("C105"==parentNode.id){
		querySql= "select wtc.org_id||'_'|| wtc.org_subjection_id id,wtc.org_abbreviation name from dms_comm_org_wtc wtc where wtc.bsflag='0' order by wtc.order_num";
	}else{
		querySql= "select wtc.org_id||'_'|| wtc.org_subjection_id id,wtc.org_abbreviation name from dms_comm_org_wtc wtc where wtc.org_id is null";
	}
	var nodes;
	Ext.Ajax.request({
