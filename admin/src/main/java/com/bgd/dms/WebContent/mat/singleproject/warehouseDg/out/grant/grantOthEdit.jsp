...
<body onload='refreshData();getApplyTeam()'>
<form name="form1" id="form1" method="post"
	action="">
	<input type='hidden' name='teammat_out_id' id='teammat_out_id' value='<gms:msg msgTag="map" key="teammatOutId"/>'/>
...
<table border="0" cellpadding="0" cellspacing="0" class="tab_line_height" width="100%">
	...
	<tr>
		...
		<td class="inquire_item6">
    	<input type="radio" name="coding_code" value="2" onclick='checkdiv(2)' >班组
    	</td>
	</tr>
</table>
...
<table border="0" cellpadding="0" cellspacing="0" class="tab_line_height" width="100%">
	<tr>
		...
		<td class="inquire_item6">
			...
			<input type='hidden' name='wz_type' id='wz_type' value=''/>
...
<table border="0" cellpadding="0" cellspacing="0" class="tab_line_height" width="100%">
	<tr>
		...
		<td class="inquire_item6"><input type="text"
			...
			value="<gms:msg msgTag="map" key="outmatDate"/>" readonly />
...
<fieldset>
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" id="queryRetTable">
			<tr>
				<td class="bt_info_odd" exp="<input name = 'rdo_entity_id' id = 'rdo_entity_id' type='checkbox'  checked = 'true' value='{wz_id}' onclick='loadDataDetail()'/>" ><input type='checkbox' name='rdo_entity_id' value='' onclick='check()'/></td>
				...
				<td class="bt_info_odd"exp="<input type='text' name='mat_num_{wz_id}' id='mat_num_{wz_id}' value='{sss}'  onkeyup='loadDataDetail()' style='width:40px'/>">本次发放数量</td>
				...
				<td class="bt_info_odd" exp="<input type='text' name='total_money_{wz_id}' id='total_money_{wz_id}' value='{total_money}'  style='width:40px' reandonly/>">金额</td>
			</tr>
		</table>
	</div>
...
<script type="text/javascript"><!--
	...
	function refreshData(){
		var team_id = "<gms:msg msgTag="map" key="teamId"/>";
		var radioValue ;
		if(team_id != null&&team_id!=""){
			...
			radioValue = 1;
			debugger;
			document.getElementById("device_name").value = "<gms:msg msgTag="map2" key="devName"/>";
			document.getElementById("dev_acc_id").value = "<gms:msg msgTag="map" key="devAccId"/>";
			document.getElementById("device_code").value = "<gms:msg msgTag="map" key="deviceId"/>";
			document.getElementById("license_num").value = "<gms:msg msgTag="map2" key="licenseNum"/>";
			document.getElementById("self_num").value = "<gms:msg msgTag="map2" key="selfNum"/>";
				}
		var val = "<gms:msg msgTag="map" key="use_type"/>";
		document.getElementById("wz_type").value = "<gms:msg msgTag="map2" key="wzType"/>";
		debugger;
		getConRatio(val,radioValue);
		cruConfig.submitStr ="value="+"<gms:msg msgTag="map" key="teammatOutId"/>"+"&wz_type="+"<gms:msg msgTag="map" key="wzType"/>";
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getOutDetailDg";
		queryData(1);
	...
	function save(){	
		...
		if(outDate!=null && outDate!=""){
			if(loadDataDetail()){
			document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/warehouse/out/grant/saveOthEdit.srq?laborId="+ids;
	...
	function showDevPage(){
		var obj = new Object();
		var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/tree/selectAccount.jsp?org_id=<%=owning_org_id%>&projectinfono="+projectInfoNo,obj,"dialogWidth=820px;dialogHeight=380px");
...
//    	var applyTeamList=jcdpCallService("HumanCommInfoSrv","queryApplyTeam","");	
    	...
    	debugger;
    	document.getElementById("s_apply_team").value = "<gms:msg msgTag="map" key="teamId"/>";
    }	
	...
	function checkdiv(value){
		...
		case 2:{
			 ...
			 document.getElementById("dev_acc_id").value = "";
		...
		debugger;
		var coding_code = document.getElementsByName("coding_code");
		for(var i=0;i<coding_code.length;i++){
			var temp = coding_code[i];
			if(temp.value==value){
				temp.checked = true;
			}else{
				temp.checked = false;
	...
	function getConRatio(value,radioValue){
    	...
    	document.getElementById("plan_type").value = "<gms:msg msgTag="map" key="useType"/>";
		}
	...
	function getRepeatDatas(){
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getOtherRepOutEdit";
		queryData(1);
		}
	...
	function refreshData2(){
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getOtherGantOutEdit";
		queryData(1);
		}
	
	function getPurRepeatDatas(){
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getOtherPurOutEdit";
		queryData(1);
		}
	...
	function openMatHave(){
		...
		var selected = window.showModalDialog("<%=contextPath%>/mat/singleproject/warehouseDg/out/grant/selectGrantList.jsp?selectWzId="+selectWzId,"","dialogWidth=1240px;dialogHeight=480px");
		//popWindow("<%=contextPath%>/mat/singleproject/warehouseDg/out/grant/selectGrantList.jsp?selectWzId="+selectWzId,'1024:800');
		debugger;
...
*/		
	...
	function deleteMatHave(){
	   ...
	   // for(var i=0;i<tab.rows.length;i++) {
		...
		//重新刷新列表页面
		document.getElementById("selectWzId").value = wz_ids;
		var teamOutId = document.getElementById("teammat_out_id").value;
		if(wz_ids!=null&&wz_ids!=""){
			cruConfig.submitStr ="wz_ids="+wz_ids+"&teamOutId="+teamOutId;
			if(wz_ids!=null&&wz_ids!=""){
