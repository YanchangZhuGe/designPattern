...
function save(){
	if(checkForm()){
		...
		form.action = "<%=contextPath%>/td/toSaveInvalidStandDoc.srq";
		form.submit();
		newClose();
	}
}
...
function checkForm(){
	if(!notNullForCheck("projectInfoNo","所属项目")) return false;
	if(!notNullForCheck("fileName","标准名称")) return false;
    ...
    <div style="border:1px #aebccb solid;background:#f1f2f3;padding:10px;width:98%">
 	<table width="99%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
 	 ...
 	 <tr>
           ...
           <td class="inquire_item4">销毁时间：</td>
           <td class="inquire_form4">
 	 ...
 	 <tr>
           ...
           <td class="inquire_item4">标准名称：</td>
