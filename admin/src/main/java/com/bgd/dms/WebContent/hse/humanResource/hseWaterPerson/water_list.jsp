      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{hse_water_id}' id='rdo_entity_id_{hse_water_id}' onclick='loadDataDetail();'/>" >选择</td>
			      ...
			      <td class="bt_info_odd" exp="{certificate_flag}">是否取得《海船船员专业培训合格证》</td>
			      <td class="bt_info_even" exp="{certificate_date}">发证日期</td>
			      <td class="bt_info_odd" exp="{certificate_org}">签发机关</td>
			      <td class="bt_info_even" exp="{certificate_num}">合格证号</td>
			    </tr>
			  </table>
			</div>
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">
			    ...
			    <li id="tag3_1"><a href="#" onclick="getTab3(1)">水域施工信息</a></li>
			  </ul>
			</div>
			
			<div id="tab_box" class="tab_box">
			<input type="hidden" id="hse_water_id" name="hse_water_id"></input>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
					  <tr>
				    	<td class="inquire_item6"><font color="red">*</font>选择人员：</td>
				     	...
				     	<td class="inquire_item6"><font color="red">*</font>是否取得《海船船员专业培训合格证》：</td>
				      	<td class="inquire_form6">
				      		<select id="certificate_flag" name="certificate_flag" class="select_width" onclick="ifYesOrNo()">
					          <option value="" >请选择</option>
					  ...
					  <tr>
					    <td class="inquire_item6"><font id="ifYes1" color="red" style="display: none;">*</font>发证日期：</td>
					    ...
					    &nbsp;<img src="<%=contextPath%>/images/calendar.gif" id="tributton4" width="16" height="16" style="cursor: hand;" onmouseover="calDateSelector(certificate_date,tributton4);" />&nbsp;
					  	...
					  	<td class="inquire_item6"><font id="ifYes2" color="red" style="display: none;">*</font>签发单位：</td>
					    <td class="inquire_form6">
							<input type="text" id="certificate_org" name="certificate_org" class="input_width"></input>
						</td>
				     	<td class="inquire_item6"><font id="ifYes3" color="red" style="display: none;">*</font>合格证号：</td>
				      	<td class="inquire_form6">
...
<script type="text/javascript">
	...
	function simpleSearch(){
		...
		if(name!=''&&name!=null){
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = "select * from (select hw.hse_water_id,hw.project_info_no,hw.employee_id,decode(hw.certificate_flag,'1','是','2','否') certificate_flag,hw.certificate_date,hw.certificate_num,hw.certificate_org,hw.modifi_date,hw.bsflag,case when ee.employee_id is null then la.owning_subjection_org_id else os.org_subjection_id end org_subjection_id, case when ee.employee_id is null then la.employee_name else ee.employee_name end employee_name,case when ee.employee_id is null then la.employee_id_code_no else ee.employee_id_code_no end code_id,case when ee.employee_id is null then decode(la.employee_gender,'0','女','1','男') else decode(ee.EMPLOYEE_GENDER, '0', '女', '1', '男') end sex_type,oi1.org_abbreviation second_org_name,oi2.org_abbreviation third_org_name,oi3.org_abbreviation fourth_org_name   from bgp_hse_water hw left join comm_human_employee ee on hw.employee_id=ee.employee_id and ee.bsflag='0' left join comm_org_subjection os1 on hw.second_org=os1.org_subjection_id and os1.bsflag='0' left join comm_org_information oi1 on os1.org_id = oi1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on hw.third_org=os2.org_subjection_id and os2.bsflag='0' left join comm_org_information oi2 on os2.org_id = oi2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on hw.fourth_org=os3.org_subjection_id and os3.bsflag='0' left join comm_org_information oi3 on os3.org_id = oi3.org_id and oi3.bsflag = '0'   left join comm_org_subjection os on os.org_id=ee.org_id and os.bsflag='0'  left join bgp_comm_human_labor la on la.labor_id=hw.employee_id and la.bsflag='0' left join comm_coding_sort_detail sd2 on la.employee_education_level=sd2.coding_code_id and sd2.bsflag='0' where hw.bsflag='0') where bsflag='0' "+querySqlAdd+"  and employee_name like '%"+name+"%' order by modifi_date desc";
			cruConfig.currentPageUrl = "hse/humanResource/hseWaterPerson/water_list.jsp";
			queryData(1);
		}else{
			refreshData();
	...
	function clearQueryText(){
  	...
  	function toUploadFile(){
		popWindow("<%=contextPath%>/hse/humanResource/hseWaterPerson/humanImportFile.jsp?project=<%=project%>");
     }
	...
	function ifYesOrNo(){
		...
		if(certificate_flag == "1"){
			document.getElementById("ifYes1").style.display = "";
	...
	function loadDataDetail(shuaId){
		var retObj;
		if(shuaId!=null){
			 retObj = jcdpCallService("HseSrv", "queryWaterPerson", "hse_water_id="+shuaId);
		    ...
		    retObj = jcdpCallService("HseSrv", "queryWaterPerson", "hse_water_id="+ids);
		}
		document.getElementById("hse_water_id").value =retObj.map.hseWaterId;
		...
		document.getElementById("certificate_flag").value = retObj.map.certificateFlag;
		document.getElementById("certificate_date").value = retObj.map.certificateDate;
		...
		ifYesOrNo();  // 根据是否取得《海船船员专业培训合格证》 看下面三项是否必填
	}
	...
	function toAdd(){
		popWindow("<%=contextPath%>/hse/humanResource/hseWaterPerson/addWater.jsp?project=<%=project%>");
	...
	function toUpdate(){ 
		...
		var hse_water_id = document.getElementById("hse_water_id").value;
		var form = document.getElementById("form1");
		form.action="<%=contextPath%>/hse/resource/saveWaterPerson.srq?project=<%=project%>&&hse_water_id="+hse_water_id;
		form.submit();
	} 
	...
	function checkText(){
		...
		if(certificate_flag==""){
			alert("是否取得《海船船员专业培训合格证》不能为空，请填写！");
			return true;
		}
		if(certificate_flag=="1"){
			if(certificate_date==""){
	...
	function toDelete(){
		...
		if(confirm('确定要删除吗?')){  
			var retObj = jcdpCallService("HseSrv", "deleteWaterPerson", "hse_water_id="+ids);
			queryData(cruConfig.currentPage);
		}
	}

	function toSearch(){
		popWindow("<%=contextPath%>/hse/humanResource/hseWaterPerson/water_search.jsp?project=<%=project%>");
	}
