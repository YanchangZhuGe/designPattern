...
<div id="new_table_box" style="width:98%">
  <div id="new_table_box_content" style="width:100%">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" >
    	...
    	<input type="hidden" id="wz_type" name="wz_type" value=""/>
     	<tr>
...
<!-- 				        <option value=''>请选择</option>
 ...
 -->				 		
        ...
        <tr>
		 	...
		 	<td class="inquire_form4">
		 		<input type="text" id="actual_price" name="actual_price" class="input_width"  readonly value="<gms:msg msgTag="matInfo" key="actual_price"/>"/>
		 	</td>
		 	<td class="inquire_item6">加油时间:</td>
			<td class="inquire_item6">
				<input type="text" name="outmat_date" id="outmat_date" class="input_width" value="<gms:msg msgTag="matInfo" key="outmat_date"/>"  />
				<img src="<%=contextPath%>/images/calendar.gif" id="tributton1" width="16" height="16" style="cursor: pointer;" onmouseover="calDateSelector(outmat_date,tributton1);" />
			</td>
        </tr>  
        </table>
    </fieldset>
	...
	<div id="list_table">
		<div id="table_box" >
			<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" id="queryRetTable" >
				<tr>
					<td class="bt_info_odd" exp="<input name='rdo_entity_id' type='checkbox' checked='true' value='{out_detail_id}' />" >
						<input type='checkbox' name='rdo_entity_id' value='' onclick='check()'/></td>
				      ...
				      <td class="bt_info_even" exp="<input name ='oil_num_{out_detail_id}' id ='oil_num_{out_detail_id}' type='text' value='{oil_num}' size='13' onkeyup='checkNumber(this);loadDataDetail();' />" >数量(升)</td>
...
<script type="text/javascript"> 
	...
	function refreshData(){
		...
		cruConfig.queryOp = "getDevAccDatasEdit";
	...
	function submitInfo(){
		...
		if(confirm("确认提交？")){
			...
			form.action="<%=contextPath%>/mat/devMode/updateDevConsumption.srq?ids="+ids;
			form.submit();
		}
	}
			...
			if(selectIndex==i){
				if(cell_7!=undefined && cell_8!=undefined && wzId!=undefined){
					...
					if(wzId==""){
						...
						row[i].cells[8].firstChild.value=Math.round((outNum*0.72/1000)*10000)/10000;
					}else{
						row[i].cells[8].firstChild.value=Math.round((outNum*0.85/1000)*10000)/10000;
					}
	...
	function ifOilNum(){
		...
		if(wz_id!=null&&wz_id!=""){
				...
				if(retObj.map!=null){
					...
					if(oil_type=="1"){
						document.getElementById("actual_price").value=Math.round(Number(wz_price)/1000*0.72*1000)/1000;
					}else{
//						mat_num=Math.round((oil_num*0.86/1000)*10000)/10000;
						document.getElementById("actual_price").value=Math.round(Number(wz_price)/1000*0.85*1000)/1000;
					}
			}
		}else{
			document.getElementById("actual_price").value=0;
		}
