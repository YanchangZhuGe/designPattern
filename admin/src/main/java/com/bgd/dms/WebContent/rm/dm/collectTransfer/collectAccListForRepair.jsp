			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
					...
					<td class="bt_info_odd" exp="{device_repapp_no}">送修申请单号</td>
					<td class="bt_info_even" exp="{repapp_name}">送修申请单名称</td>
					<td class="bt_info_odd" exp="{rep_org_name}">送修申请单位</td>
				...
				<div id="tab_box_content1" name="tab_box_content1" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							...
							<td class="bt_info_odd" width="9%">送修数量</td>
				        </tr>
...
<script type="text/javascript">
    ...
    function refreshData(v_device_backapp_no, v_backapp_name, v_org_name){
		var str = "select collrep.device_repapp_id,collrep.device_repapp_no,collrep.repapp_name,pro.project_name,"
			...
			+"left join common_busi_wf_middle middle on middle.business_id = collrep.device_repapp_id "
			...
			str += "where collrep.bsflag = '0' and repapptype='1' ";
	...
	function loadDataDetail(devicebackappid){
    	...
    	processNecessaryInfo={        							//流程引擎关键信息
			...
			businessType:"5110000004100000070",    				//业务类型 即为之前设置的业务大类
	...
	function toAddBackPlanPage(){
		popWindow('<%=contextPath%>/rm/dm/collectTransfer/plan_new_apply.jsp');
	}
	//修改 需要调整 用新界面
	function toModifyBackPlanPage(){
		...
		querySql += "where device_repapp_id ='"+devicerepappid+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		...
		popWindow('<%=contextPath%>/rm/dm/collectTransfer/plan_new_modefy.jsp?devicerepappid='+devicerepappid);
	}
	//删除
