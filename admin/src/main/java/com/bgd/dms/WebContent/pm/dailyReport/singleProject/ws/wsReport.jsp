...
function initData(){
	...
	document.getElementById("mage").style.fontSize="13";
	debugger;
	if(result.dailyMap!=null){
		if(result.dailyMap==" "){
			document.getElementById("mage").innerHTML="未保存";
	    }else if(result.dailyMap.auditStatus=="1"){
			document.getElementById("mage").innerHTML="已保存";
		} 
		if(result.dailyMap.procStatus=="3"){
			document.getElementById("mage").innerHTML="审批通过";
			
		}else if(result.dailyMap.procStatus=="4"){
			document.getElementById("mage").innerHTML="审批不通过";
		} else{
			 if(result.dailyMap.auditStatus=="2"){
					document.getElementById("mage").innerHTML="已提交，待审批";
				}  
		}
	}else{
		document.getElementById("mage").innerHTML="未保存";
 ...
 debugger;
	...
	function importReportWs(){
		popWindow(cruConfig.contextPath+"/pm/comm/ExcelImport.jsp?path=/pm/dailyReport/importReportWs.srq");
	}
	function exportReportWs(){
		if($("#mage").html()=="未保存"){
			alert("请先保存！");
		}else{
			window.location.href="<%=contextPath%>/pm/dailyReport/exportReportWs.srq";
		}		
	}
...
<div id="inq_tool_box">
	   ...
	   <tr>
	       <td id="audit_status">  	<span id="mage" style="color:red"></span> </td>
	   </tr>
		<tr>
			<td colspan="2">
				<form action="" id="fileForm" method="post" enctype="multipart/form-data">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
						 ...
						 <!-- 
						 <td width="60%"><td>
			    		<font color=red  size="2">选择文件：</font>
	      	        	<input type="file"  id="fileName" name="fileName"/>
	      	        	<auth:ListButton functionId="" css="dr" event="onclick='importData()'" title="导入数据"></auth:ListButton>
	      	        	 -->
	      	        	
			    	    <auth:ListButton functionId="" css="dr" event="onclick='importReportWs()'" title="导入数据"></auth:ListButton>
			    		<input type="hidden" id="lineNum" value="0"/>
						<auth:ListButton functionId="" css="bc" event="onclick='toSave()'" title="保存" />
						<auth:ListButton functionId="" css="tj" event="onclick='toSubmit()'" title="提交" />
			    	    <!--<auth:ListButton functionId="" css="dc" event="onclick='exportExcel()'" title="导出excel"></auth:ListButton>-->
			    	    <auth:ListButton functionId="" css="dc" event="onclick='exportReportWs()'" title="导出excel"></auth:ListButton>
		...
		<tr>
			<td width="100%" align="center"><font color="blue" size="3"><%=projectName %>生产周报表</font></td>
		</tr>
	</table>
