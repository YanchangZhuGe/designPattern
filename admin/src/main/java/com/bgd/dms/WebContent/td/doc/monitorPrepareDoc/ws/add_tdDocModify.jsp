...
function save(){
	...
	form.action = "<%=contextPath%>/td/ws/toSavemonitorPrepareDoc.srq?businessType=<%=businessType%>";
	form.submit();
	newClose();
}

function checkForm(){
	var old1 = document.getElementById("pk_0110000061100000021_0").value;
 
	if(""==old1){
		if (!isTextPropertyNotNull("0110000061100000021_0", "背景噪声")) return false;
	}
...
function page_init(){
	...
	if(fileId!='null'){
		var querySql = " select * from(select t.file_id,t.ucm_id,t.file_abbr,b.business_type,t.doc_file_type,t.file_name,b.tecnical_id,b.test_type,b.title,b.written_time,b.writer,b.test_address  ,t.create_date   from gp_ws_tecnical_basic b left join bgp_doc_gms_file t on  b.tecnical_id = t.relation_id and t.bsflag='0')m where  m.doc_file_type='0110000061100000021'  and  m.tecnical_id ='"+fileId+"' order by  create_date asc ";
		...
		if(datas!=null && datas.length>0){
			...
			for(var i=0;i<datas.length;i++){
				if(""!=datas[i].ucm_id){
					...
					document.getElementById("pk_0110000061100000021_"+i).value =ucmId;
					var str=datas[i].file_name==""?"":datas[i].file_name.substr(0,10)+'...';
					$("#down_0110000061100000021_"+i).append("&nbsp;<a style='color:blue;'  href=<%=contextPath%>/doc/downloadDocByUcmId.srq?docId="+ucmId+"&emflag=0>"+str+"</a>");
...
function addRow(){ 
	...
	var tr=document.all.tableDoc.insertRow();
  	tr.insertCell(0).innerHTML="<input type='hidden' id='pk_0110000061100000021_"+num+"' name='pk_0110000061100000021_"+num+"' value=''/>";
  	
	var td = tr.insertCell(1);
	td.setAttribute("colspan",3);
	td.innerHTML="<input type='file' name='0110000061100000021_"+num+"' id='0110000061100000021_"+num+"' class='input_width'/><div id='down_0110000061100000021_"+num+"'></div>";
...
function delRow(){
	//获得当前点击标签的附件ucm_id值 
    var oldUcmId=$("#pk_0110000061100000021_"+event.srcElement.name).val();
    //如果存在旧文件，则先删除
					...
					<div id="tab_box" class="tab_box">
						<div id="tab_box_content0" class="tab_box_content">
							<table id="tableDoc"   width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
								<tr>
									...
									<td class="inquire_form4">
								        ...
								        <input name="project_info_no" id="project_info_no" class="input_width" value="<%=projectInfoNo%>" type="hidden" readonly="readonly"/>           
							           	...
							           	<input name="title" id="title" class="input_width" value="检测准备" type="hidden" readonly="readonly"/>
									</td>
									<td class="inquire_item4"></td>
								...
								<tr>
								<td class="inquire_item4"><span class="red_star">*</span>背景噪声：</td>
								<td colspan="2">
									<input type="hidden" id="pk_0110000061100000021_0" name="pk_0110000061100000021_0" value=""/>
									<input type="file" name="0110000061100000021_0" id="0110000061100000021_0" class="input_width"/>
				    				<div id="down_0110000061100000021_0"></div>
								</td>
