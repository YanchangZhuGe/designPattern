							...
							<tr>
								...
								<auth:ListButton functionId="" css="zj"
									event="onclick='toAddDevRepairPage()'" title="新增"></auth:ListButton>
									 -->
								<auth:ListButton functionId="" css="sxjs"
									event="onclick='toModifyDevRepairPage()'" title="接收"></auth:ListButton>
								<!-- 
			...
			<div id="tab_box_content0" name="tab_box_content0"
				...
				<table id="projectMap" name="projectMap" border="0" cellpadding="0"
					...
					<tr>
						...
						<td class="inquire_form6"><input id="rec_status_desc"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
					</tr>
				</table>
			</div>
...
<script type="text/javascript">
    ...
    function refreshData(v_device_repair_app_no, v_device_repair_app_name){
		...
		if(v_device_repair_app_name!=undefined && v_device_repair_app_name!=''){//根据送修单名称查询
			...
			str += " ORDER BY REP.CREATE_DATE DESC,REC.CREATE_DATE DESC";
    ...
    function loadDataDetail(id){
		...
		$("#rec_status_desc","#projectMap").val(retObj.deviceRepairMap.rec_status_desc);
    	...
    	processNecessaryInfo={        							//流程引擎关键信息
			businessTableName:"GMS_DEVICE_COLL_REPAIRFORM",    			//置入流程管控的业务表的主表表明
			businessType:"5110000004100001034",    				//业务类型 即为之前设置的业务大类
			businessId:id,           			//业务主表主键值
	...
	function toModifyDevRepairPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devRepair/repairReceiveAll.jsp?id='+id+'&recid='+recid);
	}
