      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{hse_cater_id}' id='rdo_entity_id_{hse_cater_id}' onclick='loadDataDetail();'/>" >选择</td>
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">
			    ...
			    <li id="tag3_1"><a href="#" onclick="getTab3(1)">餐饮从业信息</a></li>
			  </ul>
			</div>
			
			<div id="tab_box" class="tab_box">
			<input type="hidden" id="hse_cater_id" name="hse_cater_id"></input>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.queryStr = "select * from (select hc.hse_cater_id,hc.project_info_no,hc.check_date,hc.bsflag,hc.modifi_date,case when ee.employee_id is null then la.owning_subjection_org_id else os.org_subjection_id end org_subjection_id,case when ee.employee_id is null then la.employee_name else ee.employee_name end employee_name,case when ee.employee_id is null then la.employee_id_code_no else ee.employee_id_code_no end code_id,case when ee.employee_id is null then decode(la.employee_gender,'0','女','1','男') else decode(ee.EMPLOYEE_GENDER, '0', '女', '1', '男') end sex_type,hc.certificate_name,hc.certificate_num,hc.certificate_org,hc.check_date-to_date(to_char(sysdate,'yyyy-MM-dd'),'yyyy-MM-dd') days,case when months_between(hc.check_date,sysdate)<=0 then 'red'  when months_between(hc.check_date, sysdate)>2 then '' else 'orange' end color,oi1.org_abbreviation second_org_name,oi2.org_abbreviation third_org_name,oi3.org_abbreviation fourth_org_name from bgp_hse_cater hc left join comm_human_employee ee on hc.employee_id=ee.employee_id and ee.bsflag='0' left join bgp_comm_human_labor la on la.labor_id=hc.employee_id and la.bsflag='0' left join comm_coding_sort_detail sd2 on la.employee_education_level=sd2.coding_code_id and sd2.bsflag='0' left join comm_org_subjection os1 on hc.second_org=os1.org_subjection_id and os1.bsflag='0' left join comm_org_information oi1 on os1.org_id = oi1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on hc.third_org=os2.org_subjection_id and os2.bsflag='0' left join comm_org_information oi2 on os2.org_id = oi2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on hc.fourth_org=os3.org_subjection_id and os3.bsflag='0' left join comm_org_information oi3 on os3.org_id = oi3.org_id and oi3.bsflag = '0' left join comm_org_subjection os on os.org_id=ee.org_id and os.bsflag='0' where hc.bsflag='0') where bsflag='0' "+querySqlAdd+" order by modifi_date desc";
		cruConfig.currentPageUrl = "hse/humanResource/hseCater/cater_list.jsp";
		queryData(1);
	}
	...
	function simpleSearch(){
		...
		if(name!=''&&name!=null){
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = "select * from (select hc.hse_cater_id,hc.project_info_no,hc.check_date,hc.bsflag,hc.modifi_date,case when ee.employee_id is null then la.owning_subjection_org_id else os.org_subjection_id end org_subjection_id,case when ee.employee_id is null then la.employee_name else ee.employee_name end employee_name,case when ee.employee_id is null then la.employee_id_code_no else ee.employee_id_code_no end code_id,case when ee.employee_id is null then decode(la.employee_gender,'0','女','1','男') else decode(ee.EMPLOYEE_GENDER, '0', '女', '1', '男') end sex_type,hc.certificate_name,hc.certificate_num,hc.certificate_org,hc.check_date-to_date(to_char(sysdate,'yyyy-MM-dd'),'yyyy-MM-dd') days,case when months_between(hc.check_date,sysdate)<=0 then 'red'  when months_between(hc.check_date, sysdate)>2 then '' else 'orange' end color,oi1.org_abbreviation second_org_name,oi2.org_abbreviation third_org_name,oi3.org_abbreviation fourth_org_name from bgp_hse_cater hc left join comm_human_employee ee on hc.employee_id=ee.employee_id and ee.bsflag='0' left join bgp_comm_human_labor la on la.labor_id=hc.employee_id and la.bsflag='0' left join comm_coding_sort_detail sd2 on la.employee_education_level=sd2.coding_code_id and sd2.bsflag='0' left join comm_org_subjection os1 on hc.second_org=os1.org_subjection_id and os1.bsflag='0' left join comm_org_information oi1 on os1.org_id = oi1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on hc.third_org=os2.org_subjection_id and os2.bsflag='0' left join comm_org_information oi2 on os2.org_id = oi2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on hc.fourth_org=os3.org_subjection_id and os3.bsflag='0' left join comm_org_information oi3 on os3.org_id = oi3.org_id and oi3.bsflag = '0' left join comm_org_subjection os on os.org_id=ee.org_id and os.bsflag='0' where hc.bsflag='0') where bsflag='0' "+querySqlAdd+" and employee_name like '%"+name+"%' order by modifi_date desc";
			cruConfig.currentPageUrl = "hse/humanResource/hseCater/cater_list.jsp";
			queryData(1);
		}else{
			refreshData();
	...
	function clearQueryText(){
 	...
 	function toUploadFile(){
		popWindow("<%=contextPath%>/hse/humanResource/hseCater/humanImportFile.jsp?project=<%=project%>");
     }
	...
	function loadDataDetail(shuaId){
		var retObj;
		if(shuaId!=null){
			 retObj = jcdpCallService("HseSrv", "queryCater", "hse_cater_id="+shuaId);
		    ...
		    retObj = jcdpCallService("HseSrv", "queryCater", "hse_cater_id="+ids);
		}
		document.getElementById("hse_cater_id").value =retObj.map.hseCaterId;
	...
	function toAdd(){
		popWindow("<%=contextPath%>/hse/humanResource/hseCater/addCater.jsp?project=<%=project%>");
	...
	function toUpdate(){ 
		...
		var hse_cater_id = document.getElementById("hse_cater_id").value;
		var form = document.getElementById("form1");
		form.action="<%=contextPath%>/hse/resource/saveCater.srq?project=<%=project%>&&hse_cater_id="+hse_cater_id;
		form.submit();
	} 
	...
	function toDelete(){
		...
		if(confirm('确定要删除吗?')){  
			var retObj = jcdpCallService("HseSrv", "deleteCater", "hse_cater_id="+ids);
			queryData(cruConfig.currentPage);
		}
	}

	function toSearch(){
		popWindow("<%=contextPath%>/hse/humanResource/hseCater/cater_search.jsp?project=<%=project%>");
	}
