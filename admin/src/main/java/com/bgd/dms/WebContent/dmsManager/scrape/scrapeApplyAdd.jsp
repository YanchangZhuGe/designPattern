  ...
  <div id="new_table_box_content">
    <div id="new_table_box_bg">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        ...
        <tr>
          <td class="inquire_item6">报废申请单名称:<font color="red">*</font></td>
...
//add by zzb
	function saveInfo(){
		...
		//折损部分结束
		document.getElementById("form1").action = "<%=contextPath%>/dmsManager/scrape/addScrapeall.srq";
		...
		if('<%=scrape_apply_id%>'!='null'){
			...
			if(baseData.fdataAsset!=null){
				...
				for (var tr_id = 0; tr_id< baseData.datas.length; tr_id++) {
					...
					innerhtml += "<td width='9%'><input name='bak_asset"+tr_id+"' id='bak_asset"+tr_id+"'  size='9'  type='text' value='"+baseData.datas[tr_id].bak1+"' /></td>";
					innerhtml += "</tr>";
					$("#processtable0").append(innerhtml);
					}
...
//删除文件
	...
	function insertFileAsset(name,type,id){
		$("#file_tableAsset").append(
					...
					"<td class='inquire_form5'>附件:</td>"+
	      			"<td class='inquire_form5' colspan='2' style='text-align:left;'><a href='<%=contextPath%>/doc/downloadDoc.srq?docId="+id+"'>"+name+"</a></td>"+
					"<td class='inquire_form5'><span class='sc'><a href='javascript:void(0);' onclick=deleteFileAsset(this,\""+id+"\") title='删除'></a></span></td>"+
					"</tr>"
			);
	}
	//删除文件
	...
	function insertFileDamage(name,type,id){
		$("#file_tableDamage").append(
					...
					"<td class='inquire_form5'>附件:</td>"+
	      			"<td class='inquire_form5' colspan='2' style='text-align:left;'><a href='<%=contextPath%>/doc/downloadDoc.srq?docId="+id+"'>"+name+"</a></td>"+
					"<td class='inquire_form5'><span class='sc'><a href='javascript:void(0);' onclick=deleteFileDamage(this,\""+id+"\") title='删除'></a></span></td>"+
					"</tr>"
			);
	}
	//删除文件
