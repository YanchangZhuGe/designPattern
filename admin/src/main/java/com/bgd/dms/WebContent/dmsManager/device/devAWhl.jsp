...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		...
		function getAmountWhlFusionChart(){
			...
			chart1.setXMLUrl("<%=contextPath%>/dms/device/getAmountWhlChartData.srq?orgSubId="+orgSubId);
		...
		function toQuery1(){
			...
			chartReference1.setXMLUrl("<%=contextPath%>/dms/device/getAmountWhlChartData.srq?orgSubId="+orgSubId+"&country="+country+"&startDate="+startDate+"&endDate="+endDate+"&ifproduction="+ifproduction);
		}
		//清除
		function toClear(){
			$('#org_sub_id_1').val('');
			...
			var chartReference1 = FusionCharts("chart1"); 
			chartReference1.setXMLUrl("<%=contextPath%>/dms/device/getAmountWhlChartData.srq");
		}
		//弹出子层级完好率（按量计算）
		function popNextLevelAWhl(level,devTreeId,orgSubId,country,startDate,endDate,ifproduction){
			popWindow('<%=contextPath%>/dmsManager/device/secondLevelAWhl.jsp?level='+level+'&devTreeId='+devTreeId+'&orgSubId='+orgSubId+'&country='+country+'&startDate='+startDate+'&endDate='+endDate+'&ifproduction='+ifproduction,'800:572');
		}
		//弹出设备信息
