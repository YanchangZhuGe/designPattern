...
<body style="background:#fff" onload="refreshData()">
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <%if(proc_status){ %>
			  <auth:ListButton functionId="OP_TARGET_DE_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			  <auth:ListButton functionId="OP_TARGET_DE_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			  <auth:ListButton functionId="OP_TARGET_DE_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			  <auth:ListButton functionId="OP_TARGET_DE_EDIT" css="bc" event="onclick='saveDatas()'" title="JCDP_save"></auth:ListButton>
			  <%-- <auth:ListButton functionId="OP_TARGET_DE_EDIT" css="qr" event="onclick='getDeviceInfoFromDm()'" title="JCDP_btn_refresh"></auth:ListButton> --%>
			  <auth:ListButton functionId="OP_TARGET_DE_EDIT" css="dr" event="onclick='importDeviceInfoFromDm()'" title="JCDP_btn_import"></auth:ListButton>
			  <%} %>
			  ...
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			           ...
			           <!--  <td  class="bt_info_odd">自编号</td>
			            ...
			            <td class="bt_info_odd">设备原值(元)</td>
			            <td  class="bt_info_even">折旧年限(年)</td>
			            <td class="bt_info_odd">月提取折旧额及资产占用费（元）</td>
			            <td  class="bt_info_even" onclick="getSum()">本项目承担折旧额及资产占用费（元）</td>
			        </tr>            
			   </table>
			</div>
		<div id="fenye_box">
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getDeviceDepreShareInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
    		...
    		'<input type="hidden" id="fy'+i+'sum_depreciation" name="fy'+i+'sum_depreciation" value="'+datas[i].sumDepreciation+'" class="input_width"/>';
			...
			td.innerHTML =  '<input type="text" id="fy'+i+'asset_value" name="fy'+i+'asset_value" value="'+datas[i].assetValue+'" class="input_width"/>' ;
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareDeviceRcEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareDeviceRcEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
...
function importDatas(){
	popWindow(cruConfig.contextPath+"/common/ExcelImportFile.jsp?modelName=OpTargetDeviceDerent&redirectUrl=/common/close_page.jsp");
}
...
function exportDeviceInfoFromDm(){
	window.location.href="<%=contextPath%>/op/OpCostSrv/exportDeviceInfoForCostRc.srq";
}
...
function getSum(){
	...
	" where td.bsflag='0' and  td.project_info_no = '"+project_info_no+"'  and td.record_type='0'  and td.if_change ='0'";
