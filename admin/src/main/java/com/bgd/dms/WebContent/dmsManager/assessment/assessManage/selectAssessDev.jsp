...
function button_ok(){
	...
	if(null!=nodes &&  nodes.length>0){
		...
		var checkText = "";// 存放选中的节点的名称
		for (var i = 0; i < nodes.length; i++) { 
			checkIds += "," + nodes[i].id+"";
			checkText += "," + nodes[i].text+"";
		}
		checkIds = checkIds=="" ? "" : checkIds.substr(1);
		checkText = checkText=="" ? "" : checkText.substr(1);
		
		window.returnValue = checkIds+"~"+checkText;
		window.close();
	}else{
		alert("请选择设备");
	}
...
function getSubOrgNodes(parentNode,dbClickFlag){
	...
	var querysql;
	if(info == ','){
		querysql="select t1.dev_tree_id id,t1.device_name name,t1.father_id code,t1.device_code,t1.device_type from dms_device_tree t1 where t1.father_id is null order by t1.code_order ";
	}else{
		querysql="select t1.dev_tree_id id,t1.device_name name,t1.father_id code,t1.device_code,t1.device_type from dms_device_tree t1 where t1.father_id='"+info[0]+"' order by t1.code_order ";
	}
	Ext.Ajax.request({
		url : "<%=contextPath%>"+appConfig.queryListAction,
		params : {
