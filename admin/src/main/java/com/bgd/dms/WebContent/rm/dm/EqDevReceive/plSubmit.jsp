  ...
  <div id="new_table_box_content">
    ...
    <fieldSet style="margin-left:2px"><legend>设备信息</legend>
    	...
    	<tr>   
		    ...
		    <td class="bt_info_even">保养周期</td>
		    <td class="bt_info_odd">存放地</td>
		    <td class="bt_info_even">计划进场时间</td>
...
<script type="text/javascript"> 
	...
	function loadDataDetail(){
		...
		if(device_mix_detids!=null){
		    var querysql="select dt.*,acc.dev_name,acc.dev_model,acc.dev_unit,acc.dev_position,"+
		"case dt.state when '0' then '未接收' when '9' then '已接收'  else '未接收' end as state_desc, "+
			"teamid.coding_name as team_name,out.in_org_id,out.out_org_id "+
			...
			"left join gms_device_equ_outsub sub on sub.device_oif_subid=dt.device_oif_subid "+
			"left join gms_device_equ_outform out on out.device_outinfo_id = sub.device_outinfo_id "+
			"where dt.device_oif_detid in "+device_mix_detids;
			...
			for (var i = 0; i < retObj.length; i++) {
				...
				newTd0.innerHTML = "<input id='team_name"+i+"' name='team_name"+i+"' type='hidden' value='"+retObj[i].team+"' /> "+retObj[i].team_name;
				var newTd1 = newTr.insertCell();
				newTd1.innerHTML = "<input id='dev_name"+i+"' name='dev_name"+i+"' type='hidden' value='"+retObj[i].dev_name+"' /> "+retObj[i].dev_name;
				var newTd2 = newTr.insertCell();
				newTd2.innerText = retObj[i].dev_model;
				var newTd3 = newTr.insertCell();
				newTd3.innerText = retObj[i].dev_unit;
				var newTd4 = newTr.insertCell();
				newTd4.innerText = retObj[i].self_num;
				var newTd5 = newTr.insertCell();
				newTd5.innerText = retObj[i].license_num;
				var newTd6 = newTr.insertCell();
				newTd6.innerText = retObj[i].dev_sign;
				var newTd7 = newTr.insertCell();
				newTd7.innerHTML = "<font color=red>*</font><input type='text' id='actual_start_date"+i+"' name='actual_start_date"+i+"' value='"+retObj[i].dev_plan_start_date+"' style='width:70'/><img src='<%=contextPath%>/images/calendar.gif' id='tributton1"+i+"' width='16' height='16' style='cursor: hand;'"+
					" onmouseover='calDateSelector(actual_start_date"+i+",tributton1"+i+");' />";
				var newTd8 = newTr.insertCell();
				newTd8.innerHTML = "<font color=red>*</font><select id='maintenance_cycle"+i+"' name='maintenance_cycle"+i+"' devciinfo='"+retObj[i].dev_ci_code+"' style='width:40px;' onchange='changeMCV("+i+",this)'><input type='hidden' id='maintenance_cycle_value"+i+"' name='maintenance_cycle_value"+i+"' value=''><input id='dev_in_org"+i+"' name='dev_in_org"+i+"' type='hidden' value='"+retObj[i].in_org_id+"' /><input id='dev_out_org"+i+"' name='dev_out_org"+i+"' value='"+retObj[i].out_org_id+"' type='hidden' />";
				var newTd9 = newTr.insertCell();
				newTd9.innerText = retObj[i].dev_position;
				var newTd10 = newTr.insertCell();
				newTd10.innerHTML = "<input id='dev_plan_start_date"+i+"' name='dev_plan_start_date"+i+"' type='hidden' value='"+retObj[i].dev_plan_start_date+"' /> "+retObj[i].dev_plan_start_date;
				var newTd11 = newTr.insertCell();
				newTd11.innerHTML = "<input id='dev_plan_end_date"+i+"' name='dev_plan_end_date"+i+"' type='hidden' value='"+retObj[i].dev_plan_end_date+"' /> "+retObj[i].dev_plan_end_date;
				if(i==0){
	...
	function submitInfo(){
		...
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveEQPLReceive.srq?id="+id+"&devaccId="+devaccId+"&projectInfoNo="+projectInfoNo+"&taskids="+taskids+"&mixId=<%=mixId%>";
