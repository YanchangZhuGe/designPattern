...
String ASSESS_ORG_NAME="";
if("add".equals(flag)){
	ASSESS_ORG_NAME = resultMsg.getValue("ADD_ASSESS_ORG_NAME");//被审核单位名称
	ASSESS_ORG_NAME = AssessTools.valueOf(ASSESS_ORG_NAME, resultMsg.getValue("ADD_ASSESS_ORG_NAME".toLowerCase()));
}else{
	ASSESS_ORG_NAME = resultMsg.getValue("ASSESS_ORG_NAME");//被审核单位名称
...
function countScore(tag,assessID,score_Standard){
	...
	if(isNaN(tagValue)==true||parseFloat(tagValue)>parseFloat(score_Standard)||parseFloat(tagValue)<0){
		$.messager.alert('提示','请您输入有效数字;<br>且单项扣分不能大于要素总分!','warning');
...
//提交
function toSubmit(){
	var modelname = $.trim($("#MODEL_NAME").val());
	var assessname = $.trim($("#assess_name").val());
	var employeename = $.trim($("#employee_name").val());
	var startdate = $("#startDate").datebox("getValue");
	var enddate = $("#endDate").datebox("getValue");
	if(modelname.length<=0){
		$.messager.alert("提示","模板不能为空!","warning");
		return;
	}
	if(assessname.length<=0){
		$.messager.alert("提示","被审核单位不能为空!","warning");
		return;
	}
	...
	if(startdate.length<=0 || enddate.length<=0){
		$.messager.alert("提示","审核时间不能为空!","warning");
		return;
	}
	...
	for (var i = 0; i< assess_ids.length; i++) {
		...
		FACT_DESCJson = "\"FACT_DESC\":\"" + $.trim(FACT_DESC) + "\",";
		...
		REMARKJson = "\"REMARK\":\"" + replaceAllBlank(REMARK," ").replace("\r","").replace("\n","") + "\",";
	...
	for (var i = 0; i< group_detail_ids.length; i++) {
		...
		if("up"==flag){
			document.form1.action="<%=contextPath%>/assess/updateScoreReportByID.srq";
		}
...
<body style="background: #fff" >
	<form action="<%=contextPath%>/assess/createScoreReport.srq" method="post" name="form1" onsubmit="toSubmit()">
		<input type="hidden" name="assessList" /> 
		<input type="hidden" name="group_detailList" /> 
		<input type="hidden" name="flag" value="<%=flag%>" />
        <input type="hidden" name="CREATE_ORG_ID" value="<%=orgId%>" />
		<input type="hidden" name="ASSESS_ORG_ID" value="<%=ASSESS_ORG_ID%>" />
		<input type="hidden" name="ASSESS_ORG_NAME" value="<%=ASSESS_ORG_NAME%>" />
		<input type="hidden" name="ASSESS_USER_ID" value="<%=ASSESS_USER_ID%>" />
		<input type="hidden" name="ASSESS_USER_NAME" value="<%=ASSESS_USER_NAME%>" />
		<input type="hidden" name="ASSESS_START_DATE" value="<%=ASSESS_START_DATE%>" />
		<input type="hidden" name="ASSESS_END_DATE" value="<%=ASSESS_END_DATE%>" />
		...
		<div id="inq_tool_box">
			...
			<tr>
				...
				<td background="<%=contextPath%>/images/list_15.png">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
						<td style="width:100px;"><font color=red>*</font>被审核单位：</td>
				        ...
				        <td class=ali_btn>
				    		<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" id="show-btn" style="cursor: pointer;" 
		    					onclick="showOrgTreePage('assess')" />
		    			</td>
						<td style="width:80px;"><font color=red>*</font>选择模板：</td>
				  		<td class="ali_cdn_input">
							<input id="MODEL_NAME" value="<%=MODEL_NAME==null?"": MODEL_NAME %>" name="MODEL_NAME" type="text" readonly="readonly" size="3"/>							
						</td>
						<td class=ali_btn>
				    		<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" id="org_button" style="cursor: pointer;" 
						...
						<td class="ali_cdn_input" width="20%">
					     ...
					     <td class="ali_cdn_name" ><font color=red>*</font>&nbsp;审核时间：</td>
						    <td style="width:280px;">
						    	<input id="startDate" name="startDate" type="text" value="<%=ASSESS_START_DATE==null?"": ASSESS_START_DATE %>" 
						    		class="input_width easyui-datebox" data-options="prompt:'请选择开始日期',onSelect:onSelect" style="width:108px" editable="false" />
						    	&nbsp;至&nbsp;
						    	<input id="endDate" name="endDate" type="text" value="<%=ASSESS_END_DATE==null?"": ASSESS_END_DATE %>" 
						    		class="input_width easyui-datebox" data-options="prompt:'请选择结束日期',validType:'equaldDate[\'#startDate\']'" style="width:108px" editable="false" />
					    	</td>
		    			<td>
		    			</td>
			...
			<table align="center" id="model_title_table">
				<tr>
					<td id="model_title_" align="center" style="font-size: 18px;line-height:2;">
			...
			<div id="table_box_div" style="overflow-x:auto;overflow-y:auto;word-wrap:break-word;word-break:normal;" >
				<table border="1" cellspacing="2" cellpadding="1" class="tab_info" id="queryRetTable">
					...
					<%
						...
						if("add".equals(flag)){
							groupDetailList = detailServ.findAllDetailsByModelID("1",modelID);
							standardDetailList = detailServ.findAllDetailsByModelID("0",modelID,ASSESS_ORG_ID);
					...
					<tr bgcolor="<%=tr_color%>">
						...
						<div style="visibility: <%=visibility%>">						
							<input type="checkbox"  name="isMark" <%=checked%>>
						</div>
						</td>
						<td width="400px" >
						...
						%>
						<img src='<%=contextPath%>/images/magnifier.gif' width='16' height='16' id='show-btn' style='cursor: pointer;' onclick='showAssessOrgTree("<%=assess_id %>")' />
						<input name="assessorgID" id="<%=assessorgID %>" type='hidden' value='<%=ASSESS_ORG_ID==null?"":ASSESS_ORG_ID %>' />
					   	<input name="assessorgName" id="<%=assessorgName %>" type='text' value='<%=ASSESS_ORG_NAME==null?"":ASSESS_ORG_NAME %>' size="12" />
					   	</div>
						</td>
						...
						<!-- 评分标准 -->
							...
							<%
								...
								for(int j = 0;j<standardDetailList_Size;j++){
									...
									//单项自动评分得分
									String AUTO_SCORE = AssessTools.valueOf(detailMap.get("AUTO_SCORE".toLowerCase()), "0");
									//自动评分总和
									ALL_AUTO_SCORE+=Float.parseFloat(AUTO_SCORE);
									%>
									<tr>
									<td width="200" style="word-break: break-all">
									<!-- 评分标准 -->
									...
									<%
										...
										String multiCHECKStr = AssessTools.newLines(CHECK_CONTENTline, 12);
											   ...
											   multiCHECKStr = multiCHECKStr.replace("<br>", "");
								...
								if(s_score_Standard!=0){
									if("add".equals(flag)){
										//要素扣分
										ITEM_DEDUCT_SCORE=s_score_Standard-ALL_AUTO_SCORE+"";
										//要素实际得分
										ITEM_FACT_SCORE=ALL_AUTO_SCORE+"";
										//要素综合得分
										ITEM_COMPRE_SCORE=(ALL_AUTO_SCORE*1000/s_score_Standard)+"";
									}
						...
						%>
						<div style="visibility: <%=visibility%>">
						 <input type="text" value="<%=ITEM_DEDUCT_SCORE==null||"".equals(ITEM_DEDUCT_SCORE)?0:ITEM_DEDUCT_SCORE %>"  name="SINGLE_DEDUCT" id = "<%=SINGLE_DEDUCT_ID%>" size="3" onkeyup="countScore('SINGLE_DEDUCT_','<%=assess_id%>','<%=s_score_Standard%>')">
...
<%
						...
						<!-- 备注-->
						<div style="visibility: <%=visibility%>">
						<textarea rows="5" cols="5" name="REMARK"><%=REMARK==null?"":REMARK %></textarea>
						</div>
						</td> --%>
...
function selectModelInfo(){
	...
	if(assessOrgId == ''){
		$.messager.alert('提示','请首先选择“被审核单位”!','warning');
		return;
	}
	var modeltype = '';
	...
	if(result!=undefined){
    	var checkStr = result.split("~");	
	       ...
	       // document.getElementByName("model_name").value = checkStr[1];
	        ...
	        $.messager.progress({title:'请稍后',msg:'数据计算中....'});
	        document.form1.method = "post";
