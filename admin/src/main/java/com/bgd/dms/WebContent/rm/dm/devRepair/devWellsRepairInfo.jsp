...
<%
	...
	String repair_flag=request.getParameter("repairflag");
	String device_backapp_id=request.getParameter("devicebackappid");
	String device_backdet_id=request.getParameter("devicebackdetid");
%>
	  ...
	  <table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		  <tr>					    
			...
			<td class="inquire_form6">
				<input id="dev_acc_id" name="dev_acc_id" type ="hidden" value="<%=dev_acc_id%>"/>
				<input id="device_backapp_id" name="device_backapp_id" type ="hidden"  value="<%=device_backapp_id%>"/>
				<input id="device_backdet_id" name="device_backdet_id" type ="hidden"  value="<%=device_backdet_id%>"/>
				<input id="repair_info" name="repair_info" class="input_width"  type="hidden" readonly/>
		  ...
		  <tr>			
			...
			<td class="inquire_item6" ><font color=red>*</font>&nbsp;截止日期</td>
			...
			<td class="inquire_form6"><input name="CHECK_COST" id="CHECK_COST" class="input_width" type="text" value="0" onBlur="AutoCal(this)"/></td>		
		  </tr>
		  <tr>
		   ...
		   <td class="inquire_item6" >工时费(元)</td>
			<td class="inquire_form6"><input name="HUMAN_COST" id="HUMAN_COST" class="input_width" type="text" value="0" onBlur="AutoCal(this)"/></td>		
			<td class="inquire_item6" >材料费(元)</td>
			<td class="inquire_form6"><input id="MATERIAL_COST" name="MATERIAL_COST" class="input_width" type="text" value="0" onBlur="AutoCal(this)"/></td>
		  </tr>
		  <tr>
		   ...
		   <td class="inquire_item6" >修理详情</td>
			<td class="inquire_form6" colspan="5">
				<textarea id="REPAIR_DETAIL" name="REPAIR_DETAIL" rows="10" cols="90" ></textarea>
			</td>
		  </tr>			
	  </table>	  
	  </fieldset>	
...
<script type="text/javascript"> 
	...
	var repair_flag='<%=repair_flag%>';
	var device_backapp_id='<%=device_backapp_id%>';
	...
	function submitInfo(){
		...
		if(confirm("确认提交？")){
			//调配数量提交			
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/saveWellsDevRepairInfo.srq?";
	...
	function checks(){
		...
		if($("#CHECK_COST")[0].value!=''&&!reg.test($("#CHECK_COST")[0].value)){
			alert("检测费的格式不正确");
			$("#CHECK_COST")[0].value="";
			$("#CHECK_COST")[0].focus();
			return false;
		...
		if($("#TOTAL_COST")[0].value!=''&&!reg.test($("#TOTAL_COST")[0].value)){
			alert("合计金额的格式不正确！");
			$("#TOTAL_COST")[0].value="";
			$("#TOTAL_COST")[0].focus();
			return false;
		}
		if($("#TOTAL_COST")[0].value=='0'){
			alert("维修合计金额不能为0!");
			return false;
		}
		return true;
	...
	function refreshData(){
		...
		if(repair_flag=="0"){
			var querySql = "select dev_acc_id,dev_coding,dev_name,dev_model,dev_sign,self_num,license_num,";
				querySql += "asset_coding,owning_org_name,usage_org_name,dev_position,using_stat,tech_stat,";
				querySql += "asset_value,net_value from gms_device_account_dui where dev_acc_id='"+dev_acc_id+"'" ;
			...
			var querySql = "select info.repair_info,info.repair_start_date,info.repair_end_date,(info.check_cost+info.material_cost+info.human_cost) as total_cost,info.repair_detail,info.check_cost,info.work_hour,info.material_cost,info.human_cost,b.dev_name,b.dev_model,b.self_num,b.license_num,b.dev_sign,b.dev_coding ";
			   ...
			   querySql += "where info.devback_repair_id='"+device_backapp_id+"' and info.device_account_id='"+dev_acc_id+"' ";
			var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
				basedatas = queryRet.datas;
				$("#dev_name")[0].value=basedatas[0].dev_name;
				$("#dev_model")[0].value=basedatas[0].dev_model;
				...
				$("#repair_info")[0].value=basedatas[0].repair_info;
		}
	}
	...
	//	alert(Number(value));
		...
		var checkcost = document.getElementById("CHECK_COST").value;//检测费
		if(checkcost == ""){
			document.getElementById("CHECK_COST").value = 0;
		}
		var humancost = document.getElementById("HUMAN_COST").value;//人工费
		if(humancost == ""){
			document.getElementById("HUMAN_COST").value = 0;
		}
		var materialcost = document.getElementById("MATERIAL_COST").value;//材料费
		if(materialcost == ""){
			document.getElementById("MATERIAL_COST").value = 0;
		}
		$("#TOTAL_COST").val(parseFloat($("#CHECK_COST").val())+parseFloat($("#HUMAN_COST").val())+parseFloat($("#MATERIAL_COST").val()));
	}
</script>
</html>
