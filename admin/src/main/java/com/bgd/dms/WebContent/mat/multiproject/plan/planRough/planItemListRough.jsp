...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
	<tr>
		...
		<td class="bt_info_even" exp="{erp_no}">ERP编号</td>
...
<table border="0" cellpadding="0" cellspacing="0" id='taskTable'
	...
	<tr>
		...
		<td class="bt_info_odd" >名称规格型号</td>
		<td class="bt_info_even" >计量单位</td>
		<td class="bt_info_odd" >申请数量</td>
		<td class="bt_info_even" >审批数量</td>
		<td class="bt_info_odd" >预计单价</td>
		<td class="bt_info_even" >预计金额</td>
		<td class="bt_info_odd" >执行记录</td>
		<td class="bt_info_even" >主办区域</td>
	...
	function loadDataDetail(shuaId){
		if(shuaId!= undefined){
			...
			for(var i=1;i<row.length;i++){
				...
				if(cell_0 == true){
					var cell_8 = row[i].cells[8].innerHTML;
					totalMoney = cell_8;
					if((-totalMoney < 0) && (-totalMoney >= -10000)){
						businessType = "5110000004100000100";
					}else if((-totalMoney < -10000) && (-totalMoney >= -50000)){
						businessType = "5110000004100000101";
					}else if((-totalMoney < -50000) && (-totalMoney >= -100000)){
						businessType = "5110000004100000102";
					}else if((-totalMoney < -100000) && (-totalMoney >= -90000000)){
						businessType = "5110000004100000103";
					}
				}
			}
	...
	function toDelete(){
		    ...
		    if(ids==''){ alert("请先选中一条记录!");
		    	...
		    	var retObj = jcdpCallService("MatItemSrv", "queryRoughPlanSubmit", "plan_id="+ids);
	 	    	...
	 	    	if(submiteIf =='1'){ alert("申请单已被提交!");
					...
					popWindow('<%=contextPath%>/mat/multiproject/plan/planRough/planEditRough.jsp?plan_id='+ids,'1024:800');
		 	  	}
		    }
	 }
	 function toAdd(){ 
     ...
     //汇总信息
    	 function taskShow(value){
    			...
    			var retObj = jcdpCallService("MatItemSrv", "queryRoughPlanDetail", "plan_id="+value);
    			...
    			for(var i =0; taskList!=null && i < taskList.length; i++){
					...
					var totalPrice = taskList[i].totalPrice;
    		        ...
    		        td = newTR.insertCell(8);
      		        td.innerHTML = "<input name='main_part"+i+"' type='checkbox' value='C105008013' disabled>设备物资科</input>&nbsp;<input name='main_part"+i+"' type='checkbox' value='C105008044' disabled>塔里木项目部</input><br/><input name='main_part"+i+"' type='checkbox' value='C105008037' disabled>北疆项目部</input>&nbsp;<input name='main_part"+i+"' type='checkbox' value='C105008040' disabled>敦煌项目部</input>&nbsp;&nbsp;&nbsp;";
