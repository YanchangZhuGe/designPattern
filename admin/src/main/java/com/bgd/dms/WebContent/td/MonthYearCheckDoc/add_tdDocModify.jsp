...
function page_init(){
	...
	cruConfig.contextPath = "<%=contextPath%>";
	cruConfig.openerUrl = "/td/MonthYearCheckDoc/tdDocList.jsp";
	cru_init();
...
function save(){
	if(checkForm()){
		...
		form.action = "<%=contextPath%>/td/toSaveMonthYearTdDoc.srq";
		form.submit();
		newClose();
	}
}
...
function page_init(){
	...
	if(fileId!='null'){
		var querySql = " select t.file_id,t.file_name,t.ucm_id,t.doc_type,to_char(t.upload_date,'yyyy-MM-dd') upload_date,t.creator_id,e.employee_name  from bgp_doc_gms_file t left join comm_human_employee e on t.creator_id=e.employee_id and e.bsflag='0'  where t.file_id='"+fileId+"'";
    ...
    <div style="border:1px #aebccb solid;background:#f1f2f3;padding:10px;width:98%">
 	<table width="99%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
 	 <tr>
           ...
           <td class="inquire_item4">检查时间：</td>
           <td class="inquire_form4">
