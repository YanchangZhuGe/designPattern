			...
			<div id="tab_box" class="tab_box">
				<div id="tab_box_content0" name="tab_box_content0" class="tab_box_content">
					...
					<tr>
				      ...
				      <td  class="inquire_form6"  ><input id="out_progect_name" class="input_width" type="text"  value="" disabled/> &nbsp;</td>
				     </tr>
				    <tr >
				...
				<div id="tab_box_content1" name="tab_box_content1" idinfo="" class="tab_box_content" style="display:none">
					<table id="detailtitletable" border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" style="margin-top:10px;background:#efefef;width:98.5%"> 
						<tr class="bt_info">
							...
							<td class="bt_info_odd">实际离场日期</td>
				        </tr>
				        <!-- 
...
$(function(){
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			...
			innerHTML += "<td>"+datas[i].dev_coding+"</td><td>"+datas[i].actual_out_time+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
    ...
    function refreshData(v_project_name,v_org_name){
		var str = "select t.dev_mov_id,t.dev_mov_no,t.dev_mov_name,outgp.project_name as out_progect_name,";
			...
			str += "case when t.out_project_info_id='"+projectInfoNos+"' then '转出' else '转入' end as mov_state,";
			...
			str += "left join gp_task_project ingp on t.in_project_info_id=ingp.project_info_no left join p_auth_user u on t.opertor_id=u.user_id ";
			str += "left join common_busi_wf_middle wfmiddle on t.dev_mov_id=wfmiddle.business_id and wfmiddle.bsflag ='0' ";
			str += "where (t.out_project_info_id='"+projectInfoNos+"' or t.in_project_info_id='"+projectInfoNos+"') and t.dev_type='dev' and t.bsflag='0' ";
		    str += "order by wfmiddle.proc_status nulls first,t.apply_date desc ";
    ...
    function loadDataDetail(dev_mov_id){
		...
		if(project_type == '5000100004000000008'){//井中
			curbusinesstype = "5110000004100001061";
		}else{
			curbusinesstype = "5110000004100000094";
		}
		//重新加载当前标签页信息
	...
	function toAddDevInfo(){
		...
		popWindow('<%=contextPath%>/rm/dm/devmove/devMoveNewApply.jsp?projectInfoNo=<%=projectInfoNo%>','950:680');
	...
	function toModifyMainPlanPage(){
		...
		if(unitRet.datas[0].proc_status == '' ){
			...
			popWindow('<%=contextPath%>/rm/dm/devmove/devMoveNewModify.jsp?projectInfoNo=<%=projectInfoNo%>&devMovId='+devMovId,'950:680');
		}else{
			alert("本单据已提交，不能修改!");
			return;
		}
	...
	function toDelPlanPage(){
		...
		if(unitRet.datas[0].proc_status == null||unitRet.datas[0].proc_status == ''||unitRet.datas[0].proc_status == '4'){
			if(confirm("是否执行删除操作?")){
				...
				var sql = "update gms_device_account_dui dui set transfer_state='' where dui.dev_acc_id in (select det.dev_acc_id from gms_device_move_detail det where det.dev_mov_id='"+devMovId+"' and det.bsflag='0') ";
				var path = cruConfig.contextPath+"/rad/asyncDelete.srq";
				var params = "deleteSql="+sql;
					params += "&ids=";
				var retObject = syncRequest('Post',path,params);
				
				var sqlDel = "delete from gms_device_account_dui where fk_device_appmix_id ='"+devMovId+"' and bsflag = 'N' ";
				var path = cruConfig.contextPath+"/rad/asyncDelete.srq";
