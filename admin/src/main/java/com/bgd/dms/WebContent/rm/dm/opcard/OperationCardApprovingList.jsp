...
<body style="background:#cdddef" onload="ZJTJ()">
	<div id="list_table">
		<div id="inq_tool_box">
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
					  			...
					  			<td class="ali_cdn_input">
					   				<input id="q_approving_no" name="q_approving_no" type="text" class="input_width" />
					  			</td>
				   				<td class="ali_cdn_name">审核人：</td>
				  				<td class="ali_cdn_input">
							   		 <input id="q_approving_person" name="q_approving_person" type="text" class="input_width" />
							  	</td>
				  				<td class="ali_query">
		...
		<div id="table_box">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
				<tr>
					<td class="bt_info_even" exp="<input type='checkbox' name='selectedbox'   value='{approving_date_id}' id='selectedbox_{approving_date_id}' />" >选择</td>
					<td class="bt_info_odd" autoOrder="1">序号</td> 
					<td class="bt_info_even" exp="{approving_no}">审核单号</td>
					<td class="bt_info_odd" exp="{approving_unit}">审核单位</td>
					<td class="bt_info_even" exp="{approving_date}">审核日期</td>
					<td class="bt_info_odd" exp="{approving_person}">审核人</td>
				</tr>
			</table>
		</div>
		<div id="fenye_box">
...
<script type="text/javascript">
	...
	cruConfig.queryOp = "queryOp_cardApprovingList";
	...
	function getContentTab(obj,index) { 
		...
		if(index == 0){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				//先进行查询
				var retObj = jcdpCallService("DevCardSrv", "queryOp_cardApprovingList","currentPage=1&pageSize=10&approving_id="+currentid);
				if (typeof retObj.datas != "undefined") {
					var data=retObj.datas[0];
					 $("#apply_unit").val(data.approving_unit);
					 $("#apply_date").val(data.approving_date);
					var retObj1 = jcdpCallService("DevCardSrv", "getApplyDeviceInfoByApproving_id","currentPage=1&pageSize=10&approving_id="+currentid);
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				//先进行查询
					var retObj = jcdpCallService("DevCardSrv", "loadEmployeeInfoByApprovingID","approving_id="+currentid);
				if (typeof retObj.datas != "undefined"   ) {
	...
	function refreshData(apply_num,apply_status){
		...
		if(typeof apply_num!="undefined" && apply_num!=""){
			temp += "&q_approving_no="+apply_num;
		}
		if(typeof apply_status!="undefined" && apply_status!=""){
			temp += "&q_approving_person="+apply_status;
		}
	...
	function simpleSearch(){
	 	var q_apply_num = $("#q_approving_no").val(); 
	    var q_apply_status = $("#q_approving_person").val();
	...
	//新增
	function toAdd(){
		window.location.href='<%=contextPath %>/rm/dm/opcard/OperationCertificateApproving.jsp?flag=add';	
	} 
	//清空表格
