			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_outinfo_id_{device_outinfo_id}' name='device_outinfo_id'>
			     	<td class="bt_info_even" exp="<input type='checkbox' name='selectedbox' outstate='{state}' value='{device_outinfo_id}~{device_mixinfo_id}~{mix_type_id}' id='selectedbox_{device_outinfo_id}~{device_mixinfo_id}' />" >选择</td>
				...
				<div id="tab_box_content1" name="tab_box_content1" idinfo="" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							...
							<td class="bt_info_odd" width="11%">设备类别</td>
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				if(mixtype == 'S14059999'){
					prosql = "select '调配出库' as showtype,ci.dev_ci_name as device_name,ci.dev_ci_model as device_model,self_num,ct.dev_ct_name,asset_coding,license_num,1 as out_num,";
					prosql += "dev_sign,case outform.receive_state when '0' then '未接收' when '1' then '接收中' when '9' then '接收完毕' else '' end as recstate_desc from gms_device_equ_outdetail od left join gms_device_codeinfo ci on od.dev_ci_code = ci.dev_ci_code ";
					prosql += "left join gms_device_codetype ct on ct.dev_ct_code = ci.dev_ct_code left join gms_device_equ_outsub os on os.device_oif_subid = od.device_oif_subid ";
					prosql += "left join gms_device_equ_outform outform on outform.device_outinfo_id = os.device_outinfo_id ";
					prosql += "where os.device_outinfo_id='"+searchkeyid[0]+"' ";
				}else{
					//先进行查询
					prosql = "select '' as dev_ct_name,'1' as type,'' as self_num,'' as dev_sign,'' as license_num,'调配出库' as showtype,cos.device_name,cos.device_model,cos.out_num,cos.receive_state,";
...
//	 				prosql += "where cof.device_outinfo_id='"+searchkeyid[0]+"' and device_mif_subid is not null) ";
					prosql += "union (select '' as dev_ct_name,'3' as type,'' as self_num,'' as dev_sign,'' as license_num,'补充出库' as showtype,cos.device_name,cos.device_model,cos.out_num,cos.receive_state,";
					prosql += "case cos.receive_state when '0' then '未接收' when '1' then '接收完毕' when '9' then '接收中' when 'U' then '' else '异常状态' end as recstate_desc ";
					prosql += "from gms_device_coll_outsubadd cos ";
					prosql += "left join gms_device_coll_outform cof on cos.device_outinfo_id=cof.device_outinfo_id ";
					prosql += "where cof.device_outinfo_id='"+searchkeyid[0]+"' and device_mif_subid is null) ";
					prosql += "union ";
					prosql += "(select '' as dev_ct_name,'3' as type,eq.self_num,eq.dev_sign,eq.license_num,'补充出库' as showtype,acc.dev_name as device_name,acc.dev_model as device_model, 1 as out_num,";
	...
	function appendDataToDetailTab(filterobj,datas){
		for(var i=0;i<basedatas.length;i++){
			...
			innerHTML += "<td>"+datas[i].device_model+"</td><td>"+datas[i].self_num+"</td><td>"+datas[i].license_num+"</td><td>"+datas[i].dev_sign+"</td><td>"+datas[i].dev_ct_name+"</td><td>"+datas[i].out_num+"</td><td>"+datas[i].recstate_desc+"</td>";
			innerHTML += "</tr>";
...
<script type="text/javascript">
	...
	function exportDataDoc(){
		...
		if(info[0]!=''){
			...
			var submitStr;
			if(info[2] == 'S14059999'){
				submitStr = "baseTableName=gms_device_equ_outform&baseid="+info[0]+"&subTableName=gms_device_equ_outdetail";
			}else{
				submitStr = "baseTableName=gms_device_coll_outform&baseid="+info[0]+"&subTableName=gms_device_coll_outsub";
			}
	...
	function refreshData(v_outinfo_no){
		var str = "select * from ( select '' as mix_type_id,cmf.project_info_no,cmf.device_mixinfo_id,cof.device_outinfo_id,cmf.mixinfo_no,cmf.modifi_date as mix_date,he.employee_name,tp.project_name,"
		+"inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,cof.outinfo_no,cof.out_date, " 
		...
		+"case cof.receive_state when '0' then '未接收' when '1' then '接收中' when '9' then '接收完毕' else '' end as recstate_desc "
		...
		//加机构权限
		+"and cmf.device_mixinfo_id = '<%=devmixid%>' and orgsub.org_subjection_id like '<%=orgsubid%>%' "
		+"union all "
		+"select mix.mix_type_id,mix.project_info_no,mix.device_mixinfo_id,outform.device_outinfo_id,mix.mixinfo_no,mix.modifi_date as mix_date,he.employee_name,"
	    +"pro.project_name,inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,outform.outinfo_no,"
	    +"outform.out_date,devapp.device_app_no,devapp.device_app_name,case outform.state when '0' then '出库中' when '9' then '已出库' else '待出库' end as outstate_desc,"
	    +"outform.state,case outform.receive_state when '0' then '未接收' when '1' then '接收中' when '9' then '接收完毕' else '' end as recstate_desc "
		...
		+"left join comm_human_employee he on outform.print_emp_id = he.employee_id left join comm_org_subjection orgsub on mix.out_org_id = orgsub.org_id and orgsub.bsflag = '0' "
		+"where mix.state = '9' and mix.bsflag = '0' and mix.mix_type_id = 'S14059999' and mix.device_mixinfo_id = '<%=devmixid%>' "
		...
		//}
		str += "order by state nulls first,mix_date desc,project_info_no ";
	...
	function toAddDetailPage(){
		...
		if(info[1]!=''){
			if(info[2] == 'S14059999'){
				popWindow('<%=contextPath%>/rm/dm/collDevOutForm/insDevOutInfoNew.jsp?mixInfoId='+info[1],'950:680');
			}else{
...
//		}
    ...
    function loadDataDetail(shuaId){
		...
		if(info[0]!=''){
			//根据ids去查找
			if(info[2] == 'S14059999'){
				str ="select mix.mix_type_id,mix.project_info_no,mix.device_mixinfo_id,outform.device_outinfo_id,mix.mixinfo_no,mix.modifi_date as mix_date,he.employee_name,"
			    	+"pro.project_name,inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,outform.outinfo_no,"
			    	+"outform.out_date,devapp.device_app_no,devapp.device_app_name,case outform.state when '0' then '出库中' when '9' then '已出库' else '待出库' end as outstate_desc,"
			    	+"outform.state,case outform.receive_state when '0' then '未接收' when '1' then '接收中' when '9' then '接收完毕' else '' end as recstate_desc "
					+"from gms_device_mixinfo_form mix left join gms_device_app devapp on mix.device_app_id = devapp.device_app_id left join gp_task_project pro on mix.project_info_no = pro.project_info_no "
					...
					+"and outform.device_outinfo_id='"+info[0]+"'";
			}else{
				str = "select cmf.device_mixinfo_id,cof.device_outinfo_id,cmf.mixinfo_no,cmf.modifi_date as mix_date,he.employee_name,tp.project_name,"
					...
					+"collapp.device_app_no,collapp.device_app_name,"
					+"case cof.state when '0' then '出库中' when '9' then '已出库' else '待出库' end as outstate_desc,cof.state, "
					+"case cof.opr_state when '0' then '未接收' when '1' then '接收中' when '9' then '接收完毕' else '' end as recstate_desc "
					+"from gms_device_collmix_form cmf "
					+"left join gms_device_collapp collapp on cmf.device_app_id=collapp.device_app_id " 
					+"left join gp_task_project tp on cmf.project_info_no=tp.project_info_no " 
					+"left join comm_org_information inorg on cmf.in_org_id=inorg.org_id "
			...
			//根据ids去查找 调配单
			if(info[2] == 'S14059999'){
				str = "select cmf.device_mixinfo_id,tp.project_name,inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name "
					+"from gms_device_mixinfo_form cmf  "
					+"left join gp_task_project tp on cmf.project_info_no=tp.project_info_no " 
					+"left join comm_org_information inorg on cmf.in_org_id=inorg.org_id "
					+"left join comm_org_information outorg on cmf.out_org_id=outorg.org_id "
