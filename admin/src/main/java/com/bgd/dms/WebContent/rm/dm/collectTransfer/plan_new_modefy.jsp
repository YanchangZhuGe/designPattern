...
<script type="text/javascript">
	function saveInfo(){
		...
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/modefyTransforRepCollectAppInfo.srq?state=0&repapptype=1&subkeyinfo="+subkeyinfo;
	...
	function getDevDatas(devicerepappid){
		var devdetSql = "select d.*,ca.torepair_num,sd.coding_name as unit_name from GMS_DEVICE_COLLREPAPP_DETAIL d join gms_device_coll_account_tech ca on d.dev_acc_id=ca.dev_acc_id join comm_coding_sort_detail sd on d.dev_unit=sd.coding_code_id where d.device_repapp_id='"+devicerepappid+"' and d.bsflag='0'";
		var proqueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+devdetSql);
		var retObj = proqueryRet.datas;
		for(var index=0;index<retObj.length;index++){
			var innerhtml = "<tr id='tr"+retObj[index].dev_acc_id+"' name='tr' midinfo='"+retObj[index].dev_acc_id+"'>";
