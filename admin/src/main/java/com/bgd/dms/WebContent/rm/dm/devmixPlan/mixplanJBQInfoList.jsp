			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			     <tr id='device_app_id_{device_app_id}' name='device_app_id' ondblclick='toModifyDetail(this)' idinfo='{device_app_id}'>
					...
					<td class="bt_info_odd" exp="{mix_type_name}">位置类别</td>
...
<script type="text/javascript">
    ...
    function refreshData(v_device_app_no, v_device_app_name, v_mix_type_id){
		var str = "select pro.project_name,devapp.device_app_id,devapp.device_app_no,devapp.device_app_name,devapp.project_info_no,";
			...
			str += "case devapp.mix_type_id when 'S0000' then '物探处自有设备' when 'S0623' then '专业化震源' when 'S1404' then '专业化测量' when 'S14050208' then '检波器' when 'S1405' then '专业化仪器' else '位置类别' end as mix_type_name,";
			...
			str += " and allwfmiddle.proc_status='3' and devapp.mix_type_id='S14050208' ";
	...
	function toAddMixPlanPage(){
		...
		if(basedatas.length == 0){
			...
			popWindow('<%=contextPath%>/rm/dm/devmixPlan/mixJBQplan_new.jsp?projectInfoNo=<%=projectInfoNo%>');
		}
	}
	function toModifyMixPlanPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devmixPlan/mixJBQplan_modify.jsp?projectInfoNo=<%=projectInfoNo%>&deviceappid='+deviceappid);
	}
