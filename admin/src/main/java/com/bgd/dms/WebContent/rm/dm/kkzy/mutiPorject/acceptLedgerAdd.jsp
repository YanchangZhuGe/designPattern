		...
		<div id="new_table_box_content" style="width: 100%">
			<div id="new_table_box_bg" style="width: 95%">
				<div style="overflow: auto">
					...
					<auth:ListButton id="zj0" functionId="" css="zj" event="onclick='toAddzy()'" title="选择设备"></auth:ListButton>
		    	<auth:ListButton id="zj1" functionId="" css="zj" event="onclick='toAdd()'" title="手工录入"></auth:ListButton>
							...
							<td><span class="sc"><a href="#" id="delProcess"
									name="delXgllEq" onclick="deleteMatHave()"></a></span>
				...
				<fieldset style="margin-left: 2px">
					...
					<div style="height: 300px; overflow: auto">
						...
						<td class="bt_info_odd" style="width:5.5%">自编号</td>
					<td class="bt_info_even" style="width:5.5%">日期</td>
					<td class="bt_info_even" style="width:5.5%">累计工作小时</td>
					...
					<td class="bt_info_even" style="width:5.5%">主要总成件名称</td>
					<td class="bt_info_even" style="width:5.5%">更换主要备件</td>
					<td class="bt_info_even" style="width:7.5%">保养级别</td>
					<td class="bt_info_even" style="width:15%">主要保养内容</td>
					<td class="bt_info_even" style="width:5.5%">性能描述</td>
					<td class="bt_info_even" style="width:5.5%">遗留问题</td>
					<td class="bt_info_even" style="width:7.5%">承修单位</td>
					<td class="bt_info_even" style="width:7.5%">主修人</td>
					<td class="bt_info_even" style="width:5.5%">备注</td>
							</tr>
...
function submitInfo()
{
	...
	document.getElementById("form1").action = "<%=contextPath%>/rm/dm/saveBywxInfos.srq?count="+count+"&line_infos="+line_infos;
...
var checked = false;
	...
	function load()
	{
		var wx_ids=<%=wx_ids%>;
			var baseData;
			 baseData = jcdpCallService("DevInsSrv", "getwxbyInfos", "wx_ids="+wx_ids);
				if(baseData.datas!=null)
				{
					...
					for (var i=0; i< baseData.datas.length; i++) {
						...
						innerhtml += "<td><input name='work_hours"+tr_id+"' id='work_hours"+tr_id+"' value='"+baseData.datas[i].work_hours+"' size='16' /></td>";
	...
	function selectMatData(i)
	{
		...
		if(zcj_type=="")
			{
		 selected = window.showModalDialog("<%=contextPath%>/rm/dm/kkzy/mutiPorject/selectMatList-zy.jsp?selectWzId="+usemat_id,"","dialogWidth=1240px;dialogHeight=680px");
			}
		else
			{
		 selected = window.showModalDialog("<%=contextPath%>/rm/dm/kkzy/mutiPorject/zcjMatList-zy.jsp?selectWzId="+usemat_id,"","dialogWidth=1240px;dialogHeight=680px");
		}
	 ...
	 baseData = jcdpCallService("DevInsSrv", "getdeviceZyInfos", "wz_id="+wz_id);
		if(baseData.datas!=null)
		{
			...
			for (var i=0; i< baseData.datas.length; i++) {
				...
				innerhtml += "<img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton2"+tr_id+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(bywx_date"+tr_id+",tributton2"+tr_id+");'/></td>";
				innerhtml += "<td><input name='work_hours"+tr_id+"' id='work_hours"+tr_id+"' value='' size='16' /></td>";
...
function toAdd()
{
	...
	innerhtml += "<img src='"+"<%=contextPath%>"+"/images/calendar.gif' id='tributton2"+tr_id+"' width='16' height='16' style='cursor: hand;'"+"onmouseover='calDateSelector(bywx_date"+tr_id+",tributton2"+tr_id+");'/></td>";
	innerhtml += "<td>&nbsp;<input name='work_hours"+tr_id+"' id='work_hours"+tr_id+"' value='' size='16' /></td>";
	innerhtml += "<td>&nbsp;<input name='falut_desc"+tr_id+"' id='falut_desc"+tr_id+"' value='' class='inputWidth' /></td>";
	innerhtml += "<td>&nbsp;<input name='falut_reason"+tr_id+"' id='falut_reason"+tr_id+"' value='' size='16' /></td>";
	innerhtml += "<td>&nbsp;<input name='falut_case"+tr_id+"' id='falut_case"+tr_id+"' value=''  class='inputWidth'/></td>";
	innerhtml += "<td>&nbsp;<select name='zcj_type"+tr_id+"' id='zcj_type"+tr_id+"'  onchange='changeZcj("+tr_id+")'>";
	if(usingdatas != null){
		innerhtml +=  "<option   value=''>无</option>";
		for (var j = 0; j< usingdatas.length; j++) {
	...
	innerhtml += "<td>&nbsp;<input type='button' id='xuanze"+tr_id+"' value='更换备件' onclick='selectMatData("+tr_id+")'/></td>";
	innerhtml += "<td>&nbsp;<select name='maintenance_level"+tr_id+"' id='maintenance_level"+tr_id+"'><option value='无'>无</option><option value='B'>B级</option><option value='C'>C级</option><option value='D'>D级</option></select></td>";
	innerhtml += "<td>&nbsp;<input name='maintenance_desc"+tr_id+"' id='maintenance_desc"+tr_id+"' value=''  class='inputWidth'/></td>";
	innerhtml += "<td>&nbsp;<select name='performance_desc"+tr_id+"' id='performance_desc"+tr_id+"'><option value='0'>良好</option><option value='1'>待修</option><option value='2'>待查</option></select></td>";
	innerhtml += "<td>&nbsp;<input name='legacy"+tr_id+"' id='legacy"+tr_id+"' value='无' size='16' /></td>";
	innerhtml += "<td>&nbsp;<select name='repair_unit"+tr_id+"' id='repair_unit"+tr_id+"'><option value='东部维修组'>东部维修组</option><option value='西部维修组'>西部维修组</option><option value='其他'>其他</option></select></td>";
	innerhtml += "<td>&nbsp;<input name='repair_men"+tr_id+"' id='repair_men"+tr_id+"' value='' size='16' /></td>";
