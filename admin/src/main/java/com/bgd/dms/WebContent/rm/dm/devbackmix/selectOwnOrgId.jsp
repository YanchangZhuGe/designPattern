...
function getSubNodes(parentNode) {
	...
	Ext.Ajax.request({
		...
		params : {
			currentPage:"1",
			pageSize:"10000",
			querySql:"select distinct dui.owning_org_id,org.org_abbreviation as owning_org_name,'1' as is_leaf from gms_device_backapp_detail bad left join gms_device_account_dui dui on bad.dev_acc_id=dui.dev_acc_id left join comm_org_information org on org.org_id = dui.owning_org_id where bad.device_backapp_id = '<%=devbackappid%>' order by dui.owning_org_id"
		},
		method : 'Post',
...
function getTreeNode(nodeData){
	var treeNode = new Ext.tree.AsyncTreeNode({
			id : nodeData.owning_org_id+"~"+nodeData.owning_org_name,
