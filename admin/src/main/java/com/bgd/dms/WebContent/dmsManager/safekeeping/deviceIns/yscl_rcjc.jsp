...
<%
	...
	String dev_acc_ids = request.getParameter("dev_acc_id");
%>
<fieldset>
...
<fieldset>
	...
	<table border="0" cellpadding="0" cellspacing="0" class="tab_line_height" width="100%"> 
		<tr> 
			...
			<td class="inquire_form6">
				<input type="text" name="mileage_write" id="mileage_write" class="input_width" onblur="checkMileageToday()"/>
			</td>
			<%
 				String mysql = "select mileage_write from (select g.mileage_write from gms_device_inspectioin g where g.dev_acc_id ='"+dev_acc_ids+"' and g.bsflag = '0' order by g.inspectioin_update_time desc) where rownum = 1";
  				List mylist = BeanFactory.getQueryJdbcDAO().queryRecords(mysql); 
  				for (int i = 0; i < mylist.size(); i++) {
					Map map = (Map)mylist.get(i);
					String mileage_writeOld = (String)map.get("mileageWrite");
			%>
			<td class="inquire_item6">上次里程表读数(公里)</td>
			<td class="inquire_form6">
				<input type="text" name="mileage_writeOld" id="mileage_writeOld" class="input_width" value="<%=mileage_writeOld%>"/></td>
			<%}%>
			<td class="inquire_item6">当日里程(公里)：</td>
			<td class="inquire_form6">
				<input type="text" name="mileage_today" id="mileage_today" class="input_width" onblur="checkMileageWrite()"/>
			</td>
		</tr>
	</table>
</fieldset>
...
function checkMileageToday(){
	var mileage_write=document.getElementById("mileage_write").value;//新公里数
	var mileage_writeOld=document.getElementById("mileage_writeOld").value;//上次公里数
	var mileage_today = 0;
	if(mileage_writeOld!=null&&mileage_writeOld!=''){
		mileage_today = mileage_write - mileage_writeOld;
	}else{
		mileage_today = mileage_write;
	}
	document.getElementById("mileage_today").value = mileage_today;

}
function checkMileageWrite(){
	var mileage_today=document.getElementById("mileage_today").value;//当天里程数
	var mileage_writeOld=document.getElementById("mileage_writeOld").value;//上次公里数
	var mileage_write = 0;
	if(mileage_writeOld!=null&&mileage_writeOld!=''){
		mileage_write = eval(mileage_today) + eval(mileage_writeOld);
	}else{
		mileage_write = mileage_today;
	}
	document.getElementById("mileage_write").value = mileage_write;
