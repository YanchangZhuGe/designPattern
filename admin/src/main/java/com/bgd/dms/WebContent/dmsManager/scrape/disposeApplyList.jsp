			  ...
			  <tr>
			   <td class="ali_cdn_name">申请单名称</td>
			    ...
			    <td class="ali_cdn_name">开始时间</td>
		 	    <td class="ali_cdn_input">
		 	    	<input name="start_date" id="start_date" class="input easyui-datebox" type="text" style="width:100px" editable="false"/>
		 	    </td>
			    <td class="ali_cdn_name">结束时间</td>
		 	    <td class="ali_cdn_input">
		 	    	<input name="end_date" id="end_date" class="input_width easyui-datebox" type="text" style="width:100px" editable="false"/>
		 	    </td> 
			    <td class="ali_query">
			    ...
			    <td width="20%">&nbsp;</td>
			    <auth:ListButton functionId="" css="zj" event="onclick='toAddDisposePage()'" title="新增"></auth:ListButton>
			    <auth:ListButton functionId="" css="xg" event="onclick='toModifyDisposePage()'" title="修改"></auth:ListButton>
			    <auth:ListButton functionId="" css="sc" event="onclick='toDelDisposePage()'" title="删除"></auth:ListButton>
			    <auth:ListButton functionId="" css="dc" event="onclick='exportData()'" title="导出excel"></auth:ListButton>
			    <auth:ListButton functionId="" css="tj" event="onclick='toSumbitDisposeApp()'" title="提交"></auth:ListButton>
			    <td><img src="<%=contextPath%>/images/ck.png" onclick="toDisposePage()" align="right" width="22" height="22" style="cursor:hand;" title="查看"/></td>
			  </tr>
			</table>
			</td>
...
$(function(){
	...
	function searchDevData(){
		...
		refreshData(app_no, app_name,'',start_date,end_date);
	}
	function clearQueryText(){
		document.getElementById("app_no").value = "";
		document.getElementById("app_name").value = "";
		$('#start_date').combo('setText','');
		$('#start_date').datebox('setValue', '');
		$('#end_date').combo('setText','');
		$('#end_date').datebox('setValue', '');
		refreshData("", "",'');
	}
    function refreshData(app_no, app_name,dispose_apply_id,start_date,end_date){
      	var temp = "";
    ...
    function loadDataDetail(dispose_apply_id){
		...
		if(dispose_apply_id!=null){
			 ...
			 var baseData = jcdpCallService("ScrapeSrv", "getDisposeState", "dispose_apply_id="+dispose_apply_id);
			//如果资产大于一千万 走 千万流程
    ...
    function getProcessInfo(dispose_apply_id){
		...
		curbusinesstype="5110000181000000029";
		if(activity_type=="2"){
