...
function save(){
	if(checkForm()){
		...
		form.action = "<%=contextPath%>/td/toSaveStandardGrantDoc.srq";
		form.submit();
		newClose();
	}
}
...
function checkForm(){
	if(!notNullForCheck("projectInfoNo","领取项目")) return false;
	if(!notNullForCheck("docFileType","标准类别")) return false;
	if(!notNullForCheck("receiveOrg","所属单位")) return false;
	if(!notNullForCheck("receiveId","领取人")) return false;
...
function page_init(){
	...
	if(fileId!='null'){
		...
		if(datas!=null && datas.length>0){
			...
			document.getElementById("receiveId").value = datas[0].receive_id;
			document.getElementById("receiveName").value = datas[0].receive_name;
    ...
    <div style="border:1px #aebccb solid;background:#f1f2f3;padding:10px;width:98%">
 	<table width="99%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
 	 ...
 	 <tr>
           ...
           <td class="inquire_item4">发放时间：</td>
           <td class="inquire_form4">
 	 ...
 	 <tr>
 	       <td class="inquire_item4">领取项目：</td>
           <td class="inquire_form4">
	    ...
	    <tr>
           ...
           <td class="inquire_form4">
 	       ...
 	       <td class="inquire_item4">领取人：</td>
           <td class="inquire_form4">
                <input name="receiveId" id="receiveId" class="input_width" value="" type="hidden" readonly="readonly"/>
           		<input name="receiveName" id="receiveName" class="input_width" value="" type="text" readonly="readonly"/>
           		<img src="<%=contextPath%>/images/magnifier.gif" width="16" height="16" style="cursor:hand;" onclick="selectEmployee()"/>   
           </td>
	  </tr>	   
	   <tr>
