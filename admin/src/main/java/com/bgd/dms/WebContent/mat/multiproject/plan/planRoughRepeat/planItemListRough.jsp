		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td class="ali_cdn_name">计划编号：</td>
		 	    <td class="ali_cdn_input"  style="width: 100px;"><select id="planNo" name="planNo" class="select_width"></select></td>
		 	    <td class="ali_cdn_name">申请时间：</td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
	<tr>
		...
		exp="<input name = 'rdo_entity_name' id='rdo_entity_id' type='checkbox' value='{plan_id}' onclick='chooseOne(this);loadDataDetail();'/>">选择</td>
		<td class="bt_info_even" autoOrder="1">序号</td>
		<td class="bt_info_odd" exp="{plan_no}">计划编号</td>
		<td class="bt_info_even" exp="{org_abbreviation}">申请单位</td>
		<td class="bt_info_odd" exp="{apply_date}">申请时间</td>
		<td class="bt_info_even" exp="{apply_person}">申请人</td>
		<td class="bt_info_odd" exp="{apply_phone}">联系电话</td>
		<td class="bt_info_even" exp="{if_submit}">状态</td>
	</tr>
</table>
</div>
<div id="fenye_box">
<table width="100%" border="0" cellspacing="0" cellpaodding="0"
	id="fenye_box_table">
	<tr>
...
<ul id="tags" class="tags">
	<li  class="selectTag" id="tag3_0"><a href="#" onclick="getTab3(0)">申请单信息</a></li>
	<li  id="tag3_1"><a href="#" onclick="getTab3(1)">详细计划</a></li>
...
<table border="0" cellpadding="0" cellspacing="0" id='taskTable'
	...
	<tr>
		...
		<td class="bt_info_odd" >名称规格(设备编号)</td>
		<td class="bt_info_even" >计量单位</td>
		<td class="bt_info_odd" >申请数量</td>
		<td class="bt_info_even" >审批数量</td>
		<td class="bt_info_odd" >支撑杆数量(附件)</td>
		<td class="bt_info_even">篷布数量(附件)</td>
		<td class="bt_info_odd" >外观</td>
		<td class="bt_info_even" >性能</td>
		<td class="bt_info_odd" >备注</td>
		<td class="bt_info_even" >主办区域及数量</td>
		</tr>
</table>
</div>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		var sql ='';
		sql +="select t.plan_id,t.plan_no,oi.org_abbreviation,t.apply_date,t.apply_person,t.apply_phone,t.erp_no,(case when comm.proc_status = '1' then '待审核' when comm.proc_status = '3' then '审批通过' when comm.proc_status = '4' then '审批不通过' else '未提交' end) if_submit,t.total_money from gms_mat_demand_plan_rough t left join comm_org_information oi on t.org_id = oi.org_id and oi.bsflag='0'  left join common_busi_wf_middle comm on t.plan_id = comm.business_id where t.bsflag='0' and t.if_repeat='1' and t.if_submit='1'";
		if(projectInfoNo!=null&&projectInfoNo!=""&&projectInfoNo!="null"){
	...
	function loadDataDetail(shuaId){
		if(shuaId!= undefined){
			retObj = jcdpCallService("MatItemSrv","queryRoughPlan","plan_id="+shuaId);
			debugger;
			...
			//配置流程			
			var businessType="5110000004100000107";
			
			processNecessaryInfo={
					businessTableName:"gms_mat_demand_plan_rough",	
					businessType:businessType,
					businessId: shuaId,
					businessInfo:"<%=projectName%>-可重复物资计划申请",
	...
	function toDelete(){
		 ids = getSelIds('rdo_entity_id');
		    if(ids==''){ alert("请先选中一条记录!");
		    	...
		    	var retObj = jcdpCallService("MatItemSrv", "queryRoughPlan", "plan_id="+ids);
	 	    	var submiteIf = retObj.map.ifSubmit;
	 	    	if(submiteIf =='1'){ alert("申请单已被提交!");
	 	     	return;
		 	    }else{
					if(confirm('确定要删除吗?')){	  
						var retObj = jcdpCallService("MatItemSrv", "deleteRoughPlanStatus", "plan_id="+ids);
						refreshData();
					}
				}
		    }
	 ...
	 function toEdit(){ 
		 ids = getSelIds('rdo_entity_id');
		    if(ids==''){ alert("请先选中一条记录!");
		    	...
		    	var retObj = jcdpCallService("MatItemSrv", "queryRoughPlanSubmit", "plan_id="+ids);
	 	    	var submiteIf = retObj.map.procStatus;
	 	    	if(submiteIf =='1'){ alert("申请单已被提交!");
	 	     		return;
		 	    }else if(submiteIf =='3'){ alert("申请单已被审批!");
		 	     	return;
			 	}else{
					popWindow('<%=contextPath%>/mat/multiproject/plan/planRoughRepeat/planEditRough.jsp?plan_id='+ids,'1024:800');
		 	  	}
		    }
	 }
	 function toAdd(){ 
		  
			  popWindow("<%=contextPath%>/mat/singleproject/plan/planRough/planAddRough.jsp?taskObjectId="+taskObjectId+"&projectInfoNo="+projectInfoNo,'1024:800');
     ...
     //汇总信息
    	 function taskShow(value){
    			...
    			var retObj = jcdpCallService("MatItemSrv", "queryRoughPlanRepeat", "plan_id="+value);
    			var taskList = retObj.mapInfo;
    			for(var i =0; taskList!=null && i < taskList.length; i++){
					...
					var submitNum = taskList[i].submitNum;
					var zhichengganNum = taskList[i].zhichengganNum;
					var pengbuNum = taskList[i].pengbuNum;
					var appearance = taskList[i].appearance;
					var performance = taskList[i].performance;
					var note = taskList[i].note;
					var mainPart = taskList[i].mainPart;
					var guchengNum = taskList[i].guchengNum;
					var kuerleNum = taskList[i].kuerleNum;
    		        ...
    		        td.innerHTML = "<textarea name=''  type='text' disabled>"+appearance+"</textarea>";
    		        td.className =tdClass+'_odd'
    		        if(autoOrder%2==0){
    					td.style.background = "#f6f6f6";
    				}else{
    		        ...
    		        td.innerHTML = "<textarea name=''  type='text' disabled>"+performance+"</textarea>";
    		        td.className =tdClass+'_even'
    		        if(autoOrder%2==0){
    					td.style.background = "#FFFFFF";
    				}else{
    		        ...
    		        td.innerHTML = "<textarea name=''  type='text' disabled>"+note+"</textarea>";
    		        td.className =tdClass+'_odd'
    		        if(autoOrder%2==0){
    					td.style.background = "#f6f6f6";
    				}else{
    		        ...
    		        td = newTR.insertCell(10);
      		        td.innerHTML = "&nbsp;<input name='main_part"+i+"' type='checkbox' value='C105008042567' disabled>固城物资管理小站</input>&nbsp;&nbsp;&nbsp;&nbsp;<input type='text' name='' id='' value='"+guchengNum+"' disabled='true'/>&nbsp;&nbsp;&nbsp;&nbsp;<br/><input name='main_part"+i+"' type='checkbox' value='C105008042568' disabled>库尔勒物资管理小站</input>&nbsp;<input type='text' name='' id='' value='"+kuerleNum+"' disabled='true'/>&nbsp;&nbsp;&nbsp;";
      		        if(mainPart!=null&&mainPart!=""){
 						var checkedValue = 	mainPart.split(",");
 						var checkboxes = document.getElementsByName("main_part"+i);
    			    	...
    			    	var retObj = jcdpCallService("MatItemSrv", "queryRoughPlan", "plan_id="+ids);
    		 	    	...
    		 	    	if(submiteIf =='1'){ alert("申请单已被提交!");
    		 	     	return;
							...
							if(confirm('确定要提交吗?')){	  
								var retObj = jcdpCallService("MatItemSrv", "updateRoughPlanStatus", "plan_id="+ids);
								refreshData();
							}
    					}
    				}
