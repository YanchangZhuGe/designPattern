      ...
      <fieldSet style="margin-left:2px"><legend>固定资产设备报废明细</legend>
		  ...
		  <div style="overflow:auto">
				...
				<input name="flag_asset" id="flag_asset" type="hidden"/>
			  	...
			  	<table id="processtable1" border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:1px;background:#efefef;width:98.5%"> 
		       <tr class="bt_info">
					<td class="bt_info_even">设备信息</td>
					<td class="bt_info_odd">鉴定组长</td>
...
//add by zzb
	function refreshData(){
		...
		if('<%=scrape_apply_id%>'!='null'){
			 baseData = jcdpCallService("ScrapeSrvNew", "getScrapeAllInfoNew", "scrape_apply_id="+$("#scrape_apply_id").val());
			...
			if(baseData.fdataAsset!=null){
				...
				for (var tr_id = 0; tr_id< baseData.datas.length; tr_id++) {
					...
					innerhtml += "<td  >"+baseData.datas[tr_id].employee_name+"</td>";
					innerhtml += "<td  >"+baseData.datas[tr_id].count+"</td>";
...
//删除文件
	...
	function subgo(i){
		...
		var dev_type=$("#coding_code_id"+i).val();
		var file_type=$("#file_type"+i).val();
		var obj = new Object();
		var pageselectedstr = null;
		obj.pageselectedstr = pageselectedstr;
	 	window.showModalDialog("<%=contextPath%>/dmsManager/scrape/selectScrapeListNew.jsp?scrape_apply_id="+scrape_apply_id+"&dev_type="+dev_type+"&file_type="+file_type,obj,"dialogWidth=1240px;dialogHeight=480px");
	};
