        ...
        if(isWbs == "true"){
  			...
  			if(isRoot == "true"){
  				...
  				if('<%=rootBackUrl %>'=='null'){
  					...
  					parent.mainRightframe.location.href = "<%= contextPath%><%=rootBackUrl %>?taskObjectId="+taskObjectId+"&taskId="+taskId+"&projectInfoNo=<%=projectInfoNo %>&splanId=<%=splanId%>&show_p=<%=show_p%>&taskName="+taskName+"<%=customString %>";
  				}
  			} else {
  				//wbs
				if('<%=wbsBackUrl %>'=='null'){
  					...
  					parent.mainRightframe.location.href = "<%= contextPath%><%=wbsBackUrl %>?taskObjectId="+taskObjectId+"&taskId="+taskId+"&projectInfoNo=<%=projectInfoNo %>&splanId=<%=splanId%>&show_p=<%=show_p%>&taskName="+taskName+"<%=customString %>";
  				}
  			}
  		} else {
  			//任务
  			if('<%=taskBackUrl %>'=='null'){
  				...
  				parent.mainRightframe.location.href = "<%= contextPath%><%=taskBackUrl %>?taskObjectId="+taskObjectId+"&taskId="+taskId+"&projectInfoNo=<%=projectInfoNo %>&splanId=<%=splanId%>&show_p=<%=show_p%>&taskName="+taskName+"<%=customString %>";
			}
  		}
    }
...
if('<%=taskBackUrl %>'== '<%=wbsBackUrl %>' ){
	//加载右边页面
	 	parent.mainRightframe.location.href = "<%= contextPath%><%=rootBackUrl %>?taskObjectId=&taskId=&projectInfoNo=<%=projectInfoNo %>&taskName=<%=customString %>&splanId=<%=splanId%>&show_p=<%=show_p%>";
...
function fun(){
	...
	parent.mainRightframe.location.href = "<%= contextPath%><%=taskBackUrl %>?taskObjectId="+CheckOther1s+"&taskId="+TaskIds+"&projectInfoNo=<%=projectInfoNo %>&splanId=<%=splanId%>&show_p=<%=show_p%>&taskName="+Names+"<%=customString %>";
