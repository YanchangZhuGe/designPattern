...
<%
	...
	String devsyncaccid =  request.getParameter("devsyncaccid");
	String devlocalaccid =  request.getParameter("devlocalaccid");	
%>
      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			    <auth:ListButton functionId="" css="sx" event="onclick='toSync()'" title="同步"></auth:ListButton>
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" class="tab_info" id="queryRetTable">
			     <tr >
					<td class="bt_info_odd" exp="<input type='hidden' name='rdo_entity_id' value='{dev_acc_id}' id='rdo_entity_id_{dev_acc_id}' checked/>" ></td>
...
<script type="text/javascript">
	...
	var devsyncaccid_tmp = '<%=devsyncaccid%>';
	var devlocalaccid_tmp = '<%=devlocalaccid%>';
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model,license_num,self_num,dev_sign){
		...
		str += "where t.dev_acc_id in ('"+devsyncaccid_tmp+"','"+devlocalaccid_tmp+"') ";
		str += ")aa ";
    ...
    function toSync(){
		if(confirm("确认提交？")){
			var retObj = jcdpCallService("DevInsSrv", "toSyncErpLocalDev","erpDeviceId="+devsyncaccid_tmp+"&localDeviceId="+devlocalaccid_tmp);
			if(retObj==null || !retObj.returnCode=='0'){
				alert('同步失败!');
			}else{
				window.parent.frames[4].frames["mainTopframe"].refreshParent();
				newClose();
			}
		}
	}
