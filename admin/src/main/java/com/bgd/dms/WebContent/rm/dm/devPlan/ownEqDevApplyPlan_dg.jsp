			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_appdet_id_{device_allapp_id}' name='device_allapp_id' idinfo='{device_allapp_id}'>
					...
					<td class="bt_info_odd" exp="{allapp_type_name}">设备类型</td> 
...
<script type="text/javascript">
    ...
    function refreshData(v_devapp_name,v_org_name,v_state_desc,v_allapp_type){
		...
		var str = "select wfmiddle.create_date as approve_date,pro.project_name,devapp.device_allapp_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.project_info_no,devapp.allapp_type,";
			str+=" case devapp.allapp_type when 'S0000' then '单台' when 'S14050208' then '检波器' when 'S9997' then '批量' end as allapp_type_name, ";
			...
			str += "where devapp.bsflag = '0' and (devapp.allapp_type = 'S0000' or  devapp.allapp_type='S14050208' or devapp.allapp_type='S9997') and  devapp.project_info_no='"+projectInfoNos+"' ";		
		...
		if(v_allapp_type!=undefined && v_allapp_type!=''){
			if(v_allapp_type == '0'){//单台
				str += "and devapp.allapp_type = 'S0000' ";
			}else if(v_allapp_type == '1'){//地震仪器
				str += "and devapp.allapp_type = 'S9997' ";
			}else if(v_allapp_type == '2'){//检波器
				str += "and devapp.allapp_type = 'S14050208' ";
			}				
		}
    ...
    function loadDataDetail(device_allapp_id){
		...
		if(allappType=='S0000'){
			businessTypeDg='5110000004100001033';
		}else if(allappType=='S14050208'){
			businessTypeDg='5110000004100001027';
		}else if(allappType='S9997'){//
			businessTypeDg='5110000004100001026';
		}
        ...
        processNecessaryInfo={        							//流程引擎关键信息
        			...
        			businessInfo:"大港自有设备配置计划审批列表信息<配置计划单名称:"+device_allapp_name+">",
	...
	function toAddMainPlanPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devPlan/devallplanOwn_new_dg.jsp?projectInfoNo=<%=projectInfoNo%>');
	}
	function toModifyMainPlanPage(){
		...
		var str = "select devapp.device_allapp_id,nvl(wfmiddle.proc_status,'') as proc_status ,devapp.allapp_type ";
			...
			str += "where (devapp.allapp_type = 'S0000' or  devapp.allapp_type='S14050208' or devapp.allapp_type='S9997') and devapp.bsflag = '0' and devapp.project_info_no='"+projectInfoNos+"' ";
		...
		if(unitRet.datas[0].proc_status == '' ){
			popWindow('<%=contextPath%>/rm/dm/devPlan/devallplanOwn_modify_dg.jsp?projectInfoNo=<%=projectInfoNo%>&allapp_type='+atype+'&deviceallappid='+info[0]);
		}else{
			alert("本单据已提交，不能修改!");
			return;
		}
	...
	function toSumbitDevApp(){
		...
		//	var  retObj9 = jcdpCallService("DevCommInfoSrv", "saveOwnAppDetailInfoDg", "device_allapp_id="+deviceallappid);		
		
			if(info[1]=='S9997'){
				var  retObjS1405 = jcdpCallService("DevCommInfoSrv", "saveCollMixAppDetailInfoDg", "device_allapp_id="+info[0]+"&mixownership=S9997");		
			}
			submitProcessInfo();
			refreshData();
			alert('提交成功！');
