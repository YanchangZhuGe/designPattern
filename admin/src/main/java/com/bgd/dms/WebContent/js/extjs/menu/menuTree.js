...
ibp.menu.treePanel = Ext.extend(Ext.tree.TreePanel, {
			...
			constructor : function(config){
				Ext.applyIf(config,{
					...
					url : _path+"/ibp/auth2/getMenuTreeData.srq"
				});
						...
						,listeners : {
							'click' : function(b,e){
                               ...
                               Ext.getBody().mask("保存中,请等待...","x-mask-loading");
								Ext.Ajax.request({
									url : _path+"/ibp/auth2/saveMenuTreeData.srq",
			...
			_getUpdateNode : function(node,arr){
			   ...
			   if(node.attributes.leaf && node.attributes.checked){
				   //alert(node.attributes.text+"======"+node.attributes.orgChecked+"============"+node.attributes.checked);
