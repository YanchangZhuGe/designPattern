...
<%
	...
	if("5000100004000000008".equals(projectType)){//井中
		response.sendRedirect(contextPath+"/td/ConsSumDoc/ws/tdDocList.jsp?fileAbbr=JSSGZJ&ifPorject="+ifPorject);
	}
	if("5000100004000000009".equals(projectType)){//综合物化探
		response.sendRedirect(contextPath+"/td/ConsSumDoc/wt/tdDocList.jsp?fileAbbr=JSSGZJ");
	}
...
<script type="text/javascript">
    ...
    function loadDataDetail(ids){
  	  ...
  	  processNecessaryInfo={         
	    		...
	    		businessType:"5110000004100000064",        //业务类型 即为之前设置的业务大类
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/ConsdesignDoc/add_tdDocModifyNew.jsp?docType=0110000061100000007&fileAbbr='+fileAbbr+'&parent_file_id=<%=parent_file_id%>');
	...
	function toEdit() {
	    ...
	    popWindow('<%=contextPath%>/td/ConsdesignDoc/edit_tdDocModifyNew.jsp?id='+ids.split("-")[0]+'&parent_file_id=<%=parent_file_id%>&fileAbbr=<%=fileAbbr%>');
