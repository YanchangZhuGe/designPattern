...
<%
	...
	String work_hours_begin = request.getParameter("begin");
	String work_hours_end = request.getParameter("end");
						...
						<td width="99%">
							<div class="tongyong_box">
								...
								<span class="dc" style="float:right;margin-top:-4px;">
									<a href="#" onclick="exportDataDoc('whsbjxhmxh')" title="导出excel"></a>
								</span>
								</div>
...
<script type="text/javascript">
	...
	function getFusionChart(){
		var retObj = jcdpCallService("EarthquakeTeamStatistics","getBjWorkHourDetailForm","real_dev_acc_id=<%=real_dev_acc_id%>&work_hours=<%=work_hours%>&self_num=<%=self_num%>&coding_code_id=<%=coding_code_id%>&work_hours_begin=<%=work_hours_begin%>&work_hours_end=<%=work_hours_end%>&wz_id=<%=wz_id%>");
	...
	function exportDataDoc(exportFlag){
		...
		var submitStr = "real_dev_acc_id=<%=real_dev_acc_id%>&work_hours=<%=work_hours%>&self_num=<%=self_num%>&coding_code_id=<%=coding_code_id%>&work_hours_begin=<%=work_hours_begin%>&work_hours_end=<%=work_hours_end%>&wz_id=<%=wz_id%>&exportFlag="+exportFlag;
