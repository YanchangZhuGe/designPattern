...
<%
	...
	String virtualproject_id="";
	if(request.getParameter("virtualproject_id") != null){
		virtualproject_id=request.getParameter("virtualproject_id");	
		
	}
	String id = request.getParameter("id"); 
	if(id!= null){
      if(virtualproject_id.equals("")){
    	  virtualproject_id=id;
      } 
	}
...
<input type="hidden" id="virtualproject_id" name="virtualproject_id" value="<%=virtualproject_id %>"/>
				...
				<tr>
				 ...
				 <tr>
			     	<td class="inquire_item6">地表类型：</td>
			      	...
			      	<select id="surface_type" name="surface_type" class="select_width"> 
					</select>
			      	</td>
...
var virtualproject_ids = '<%=virtualproject_id%>'; 
	...
	function submitButton(){ 
		...
		var virtualproject_id=document.getElementById("virtualproject_id").value;
		
		var project_name=document.getElementById("project_name").value;
		var jiafang=document.getElementById("jiafang_org").value;
		var c_team=document.getElementById("c_team_id").value;
		var surface_type=document.getElementById("surface_type").value;
		var bidding_time=document.getElementById("bidding_time").value;
		var bid_time=document.getElementById("bid_time").value;
		...
		var bsflag = document.getElementsByName("bsflag")[0].value;
		 ...
		 if(virtualproject_ids !=null && virtualproject_ids !=''){
			    ...
			    rowParam['virtualproject_id'] = virtualproject_id;
			    
			    rowParam['project_name'] = encodeURI(encodeURI(project_name));
			    rowParam['jiafang'] = encodeURI(encodeURI(jiafang));
			    rowParam['c_team'] = encodeURI(encodeURI(c_team));
			    rowParam['surface_type'] = encodeURI(encodeURI(surface_type));
			    rowParam['bidding_time'] =bidding_time;
			    rowParam['bid_time'] =bid_time;
			...
			saveFunc("BGP_MARKET_VIRTUALPROJECT",rows);	
...
//		}   	
	...
	function checkText0(){
		...
		var project_name=document.getElementById("project_name").value;
		var jiafang=document.getElementById("jiafang").value;
		var bidding_time=document.getElementById("bidding_time").value;
		var bid_time=document.getElementById("bid_time").value;
	 ...
	 function  listInfo(){ 
		...
		if(virtualproject_ids !=null  && virtualproject_ids !=''){
			...
			querySql = " 	select  t.virtualproject_id,t.jiafang,t.project_name,t.c_team,t.surface_type,t.bidding_time,t.bid_time,t.bsflag, t.creator,t.create_date,t.modifi_date, t.updator, t.second_org ,     oi.org_abbreviation as c_team_name,   ccsd.coding_name as jiafang_name,  ccsl.coding_name as surface_type_name   from BGP_MARKET_VIRTUALPROJECT t    left join comm_org_information oi on oi.org_id = t.c_team  and oi.bsflag = '0'    left join comm_coding_sort_detail ccsd on t.jiafang = ccsd.coding_code_id and ccsd.bsflag = '0'   left join comm_coding_sort_detail ccsl on t.surface_type =ccsl.coding_code_id and ccsl.bsflag='0'  where t.bsflag = '0' and t.virtualproject_id='"+ virtualproject_ids +"'";			 	 
			queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+encodeURI(encodeURI(querySql)));
			if(queryRet.returnCode=='0'){
				datas = queryRet.datas;
				if(datas != null){	 
		            document.getElementById("virtualproject_id").value=datas[0].virtualproject_id;
