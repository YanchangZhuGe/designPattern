      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{hse_tech_id}' id='rdo_entity_id_{hse_tech_id}' onclick='loadDataDetail();'/>" >选择</td>
			      ...
			      <td class="bt_info_even" exp="<font color='{color}' style='font-weight:bold'>{certificate_org}</font>">注册机构</td>
			      <td class="bt_info_odd" exp="<font color='{color}' style='font-weight:bold'>{comm_date}</font>">注册时间</td>
			      ...
			      <td class="bt_info_odd" exp="{code_nos}" isShow='Hide' isExport='Show'>身份证号</td>
			      <td class="bt_info_odd" exp="{tech_title}" isShow='Hide' isExport='Show'>职称级别</td>
			      <td class="bt_info_odd" exp="{coding_name_xl}" isShow='Hide' isExport='Show'>学历</td>  	
			      
			      <td class="bt_info_odd" exp="{assessor_types}" isShow='Hide' isExport='Show'>类别</td>  
			      <td class="bt_info_odd" exp="{certificate_name}" isShow='Hide' isExport='Show'>证件名称</td>  
			      <td class="bt_info_odd" exp="{certificate_org}" isShow='Hide' isExport='Show'>注册机构</td>  
			      <td class="bt_info_odd" exp="{comm_date}" isShow='Hide' isExport='Show'>注册时间</td>  
			      <td class="bt_info_odd" exp="{the_end_date}" isShow='Hide' isExport='Show'>有效期截至时间</td>  
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">
			    ...
			    <li id="tag3_1"><a href="#" onclick="getTab3(1)">注册HSE审核员信息</a></li>
			  </ul>
			</div>
			
			<div id="tab_box" class="tab_box">
			<input type="hidden" id="hse_tech_id" name="hse_tech_id"></input>
				...
				<div id="tab_box_content1" class="tab_box_content" style="display:none;">
					...
					<input type="hidden" id="model_type" name="model_type" value="3"/>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
					  ...
					  <tr>
					    ...
					    <td class="inquire_form6">
							<select id="assessor_type" name="assessor_type" class="select_width">
					          <option value="" >请选择</option>
					          <option value="1" >实习审核员</option>
					          <option value="2" >审核员</option>
					          <option value="3" >高级审核员</option>
						    </select>
						</td>
					    <td class="inquire_item6"><font color="red">*</font>有效期截至时间：</td>
					   	<td class="inquire_form6"><input type="text" id="the_end_date" name="the_end_date" class="input_width"  readonly="readonly"/>
					    &nbsp;<img src="<%=contextPath%>/images/calendar.gif" id="tributton2" width="16" height="16" style="cursor: hand;" onmouseover="calDateSelector(the_end_date,tributton2);" />&nbsp;
					    </td>
					  	<td class="inquire_item6"></td>
					    <td class="inquire_form6"></td>
					  </tr>
...
<script type="text/javascript">
	...
	function simpleSearch(){
		...
		if(name!=''&&name!=null){
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = "select * from (select tr.hse_tech_id,tr.certificate_name,tr.project_info_no,tr.bsflag,tr.modifi_date,decode(tr.plate_property,'1','野外一线','2','固定场所','3','科研单位','4','培训接待','5','矿区') plate_property,decode(tr.org_level,'1','公司集团','2','公司','3','二级单位','4','三级单位') org_lev,decode(tr.assessor_type,'1','实习审核员','2','审核员','3','高级审核员') assessor,case when ee.employee_id is null then la.owning_subjection_org_id else os.org_subjection_id end org_subjection_id, case when ee.employee_id is null then la.employee_name else ee.employee_name end employee_name, case when ee.employee_id is null then la.employee_id_code_no else ee.employee_id_code_no end code_id, case when ee.employee_id is null then decode(la.employee_gender, '0', '女', '1', '男') else decode(ee.EMPLOYEE_GENDER, '0', '女', '1', '男') end sex_type, tr.plate_property property,tr.comm_date,tr.certificate_org,tr.the_end_date - to_date(to_char(sysdate, 'yyyy-MM-dd'), 'yyyy-MM-dd') days,tr.the_end_date, case when months_between(tr.the_end_date, sysdate) <= 0 then 'red' when months_between(tr.the_end_date, sysdate) > 2 then '' else 'orange' end color, oi1.org_abbreviation second_org_name, oi2.org_abbreviation third_org_name, oi3.org_abbreviation fourth_org_name from bgp_hse_tech_resource tr left join comm_human_employee ee on tr.employee_id = ee.employee_id and ee.bsflag = '0' left join comm_org_subjection os1 on tr.second_org = os1.org_subjection_id  and os1.bsflag = '0'  left join comm_org_information oi1  on os1.org_id = oi1.org_id  and oi1.bsflag = '0'  left join comm_org_subjection os2  on tr.third_org = os2.org_subjection_id  and os2.bsflag = '0'  left join comm_org_information oi2  on os2.org_id = oi2.org_id  and oi2.bsflag = '0'  left join comm_org_subjection os3  on tr.fourth_org = os3.org_subjection_id  and os3.bsflag = '0'  left join comm_org_information oi3  on os3.org_id = oi3.org_id  and oi3.bsflag = '0'  left join comm_org_subjection os  on os.org_id = ee.org_id  and os.bsflag = '0'  left join bgp_comm_human_labor la  on la.labor_id = tr.employee_id  and la.bsflag = '0' left join comm_coding_sort_detail sd2  on la.employee_education_level = sd2.coding_code_id  and sd2.bsflag = '0'  where tr.bsflag = '0' and tr.model_type = '3') where bsflag='0' "+querySqlAdd+" and employee_name like '%"+name+"%' order by modifi_date desc";
			cruConfig.currentPageUrl = "hse/techAndResource/hseChecker/checker_list.jsp";
			queryData(1);
		}else{
			refreshData();
	...
	function loadDataDetail(shuaId){
		var retObj;
		if(shuaId!=null){
			 retObj = jcdpCallService("HseSrv", "queryTechResource", "hse_tech_id="+shuaId);
		    ...
		    retObj = jcdpCallService("HseSrv", "queryTechResource", "hse_tech_id="+ids);
		}
		document.getElementById("hse_tech_id").value =retObj.map.hseTechId;
		...
		document.getElementById("comm_date").value = retObj.map.commDate;
		document.getElementById("certificate_num").value = retObj.map.certificateNum;
		document.getElementById("assessor_type").value = retObj.map.assessorType;
		document.getElementById("the_end_date").value = retObj.map.theEndDate;
	...
	function toAdd(){
		popWindow("<%=contextPath%>/hse/techAndResource/hseChecker/addChecker.jsp?isProject=<%=isProject%>");
		
	}
	
	   function toUploadFile(){
			popWindow("<%=contextPath%>/hse/techAndResource/hseTrainer/humanImportFile.jsp?project=<%=isProject%>&addType=3");
	     }
	...
	function toUpdate(){ 
		...
		var hse_tech_id = document.getElementById("hse_tech_id").value;
		var form = document.getElementById("form1");
		form.action="<%=contextPath%>/hse/techResource/saveChecker.srq?hse_tech_id="+hse_tech_id;
		form.submit();
	} 
	...
	function checkText(){
		...
		var assessor_type = document.getElementById("assessor_type").value;
		var the_end_date = document.getElementById("the_end_date").value;
		if(employee_id==""){
			alert("人员姓名不能为空，请选择！");
			return true;
		}
	...
	function toDelete(){
		...
		if(confirm('确定要删除吗?')){  
			var retObj = jcdpCallService("HseSrv", "deleteTechResource", "hse_tech_id="+ids);
			queryData(cruConfig.currentPage);
		}
	}

	function toSearch(){
		popWindow("<%=contextPath%>/hse/techAndResource/hseChecker/checker_search.jsp?isProject=<%=isProject%>");
	}
