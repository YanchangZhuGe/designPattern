...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <title>单项目-设备接收-设备接收(自有单台)</title> 
 </head> 
	...
	function getContentTab(obj,index) { 
		...
		if(index == 1){
			...
			if(currentid != undefined && idinfo == currentid){
				...
				var prosql="";
				//判断是否是闲置设备调剂
				var sql = "select * from gms_device_mixinfo_form f where f.mixform_type='6' and f.bsflag='0' and f.device_mixinfo_id='"+currentid+"' ";
				var queryRet1 = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+sql+'&pageSize=10000');
				basedatas1 = queryRet1.datas;
				if(basedatas1!=undefined && basedatas1.length>=1){
					prosql = "select amd.dev_acc_id,account.dev_coding,amd.dev_plan_start_date,amd.dev_plan_end_date,pro.project_name, ";
					...
					prosql += "left join gms_device_mixinfo_form mif on mif.device_mixinfo_id=amd.device_mix_subid ";
...
<script type="text/javascript">
	...
	function refreshData(v_device_app_name,v_mixinfo_no,v_opr_state_desc){
		...
		var str = "select * from(select devapp.device_app_name,he.employee_name,pro.project_name,dm.*,inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,";
			...
			str+= "left join (comm_org_subjection sub left join comm_org_information outorg on sub.org_id=outorg.org_id )  on dm.out_org_id = sub.org_subjection_id ";
		...
		if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目
			...
			str += "where '1'='1' and dm.state='9' and dm.bsflag='0' and dm.mixform_type='1' and dm.mix_type_id='S0000' and dm.project_info_no='"+retFatherNo+"'";
			...
			str += "where '1'='1' and dm.state='9' and dm.bsflag='0' and dm.mixform_type='1' and dm.mix_type_id='S0000' and dm.project_info_no='"+projectInfoNos+"'";
    	}else{
      		str += "left join gp_task_project pro on devapp.project_info_no=pro.project_info_no and pro.bsflag = '0' ";
			str += "where '1'='1' and dm.state='9' and dm.bsflag='0' and dm.mixform_type = '1' and dm.mix_type_id='S0000' and dm.project_info_no='"+projectInfoNos+"' ";
	  	}
		str +="union all ";
		str+= "select devapp.backapp_name device_app_name,he.employee_name,pro.project_name,dm.*,inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,";
		str+= "case dm.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as oprstate_desc ";
		str+= "from gms_device_mixinfo_form dm ";
		str+= "left join gms_device_backapp devapp on dm.device_app_id=devapp.device_backapp_id ";  
		str+= "left join comm_human_employee he on dm.print_emp_id=he.employee_id ";
		str+= "left join comm_org_information inorg on dm.in_org_id=inorg.org_id ";
		...
		if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目
			...
			str += "left join gp_task_project pro on devapp.project_info_id=pro.project_info_no and pro.bsflag = '0' and pro.project_type = '5000100004000000008' ";
			str += "where '1'='1' and dm.state='9' and dm.bsflag='0' and dm.mixform_type='5' and dm.mix_type_id='S0000' and dm.project_info_no='"+retFatherNo+"'";
		}else if(projectType == "5000100004000000008" && retFatherNo.length <= 0){
	  		str += "left join gp_task_project pro on devapp.project_info_id=pro.project_info_no and pro.bsflag = '0' and pro.project_father_no is null and pro.project_type = '5000100004000000008' ";
			str += "where '1'='1' and dm.state='9' and dm.bsflag='0' and dm.mixform_type='5' and dm.mix_type_id='S0000' and dm.project_info_no='"+projectInfoNos+"'";
		}else{
	  		str += "left join gp_task_project pro on devapp.project_info_id=pro.project_info_no and pro.bsflag = '0' ";
			str += "where '1'='1' and dm.state='9' and dm.bsflag='0' and dm.mixform_type = '5' and dm.mix_type_id='S0000' and dm.project_info_no='"+projectInfoNos+"' ";
	  	}
		str +="union all ";
		str+= " select dm.mixinfo_name as device_app_name,   p.user_name as employee_name, pro.project_name, dm.*, inorg.org_abbreviation as in_org_name, outorg.org_abbreviation as out_org_name,";
		str+= "case dm.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as oprstate_desc ";
		str+= "from gms_device_mixinfo_form dm ";
		str+= "  left join(comm_org_subjection sub  left join comm_org_information inorg on sub.org_id = inorg.org_id) on dm.in_org_id = sub.org_subjection_id ";  
		str+= " left join(comm_org_subjection sub  left join comm_org_information outorg on sub.org_id =outorg.org_id) on dm.out_org_id = sub.org_subjection_id ";
		str+= "left join p_auth_user p on p.user_id=dm.creator_id left join gp_task_project pro on dm.project_info_no =pro.project_info_no  and pro.bsflag = '0' ";
		str += "where '1' = '1'    and dm.state = '1'  and dm.bsflag = '0'   and dm.mixform_type = '6' and dm.project_info_no ='"+projectInfoNos+"' ";
	  	str +="union all ";
		str +="select devapp.device_app_name,he.employee_name,pro.project_name,dm.*,inorg.org_abbreviation as in_org_name,outorg.org_abbreviation as out_org_name,";
		str +="case dm.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as oprstate_desc ";
		str +="from gms_device_mixinfo_form dm ";
		str +="left join gms_device_dismixapp p on p.device_mixapp_id = dm.device_app_id ";
		str +="left join gms_device_app devapp on p.device_app_id = devapp.device_app_id ";
		str +="left join comm_human_employee he on dm.PRINT_EMP_ID=he.employee_id ";
		str +="left join comm_org_information inorg on devapp.app_org_id=inorg.org_id ";
		str +="left join (comm_org_subjection sub left join comm_org_information outorg on sub.org_id=outorg.org_id )  on dm.out_org_id = sub.org_subjection_id ";
		//井中地震 
		if(projectType == "5000100004000000008" && retFatherNo.length >= 1){//子项目	
			str += "left join gp_task_project pro on devapp.project_info_no=pro.project_info_no and pro.bsflag = '0' and pro.project_type = '5000100004000000008' ";
			str += "where '1'='1' and dm.state='9' and dm.bsflag='0' and dm.mixform_type='3' and dm.mix_type_id='S0000' and dm.project_info_no='"+retFatherNo+"' ) where 1=1  ";
    	}else if(projectType == "5000100004000000008" && retFatherNo.length <= 0){
      		str += "left join gp_task_project pro on devapp.project_info_no=pro.project_info_no and pro.bsflag = '0' and pro.project_father_no is null and pro.project_type = '5000100004000000008' ";
			str += "where '1'='1' and dm.state='9' and dm.bsflag='0' and dm.mixform_type='3' and dm.mix_type_id='S0000' and dm.project_info_no='"+projectInfoNos+"' ) where 1=1 ";
    	}else{
      		str += "left join gp_task_project pro on devapp.project_info_no=pro.project_info_no and pro.bsflag = '0' ";
			str += "where '1'='1' and dm.state='9' and dm.bsflag='0' and dm.mixform_type='3' and dm.mix_type_id='S0000' and dm.project_info_no='"+projectInfoNos+"' ) where 1=1 ";
	  	}	     	
    ...
    function toDetailPage(){
		...
		var ids=shuaId.substring(0,shuaId.length-1);
		//判断是否是闲置设备调剂
		var sql = "select * from gms_device_mixinfo_form f where f.mixform_type='6' and f.bsflag='0' and f.device_mixinfo_id='"+ids+"' ";
		var queryRet1 = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+sql+'&pageSize=10000');
		basedatas1 = queryRet1.datas;
		if(basedatas1!=undefined && basedatas1.length>=1){
			window.location.href='<%=contextPath%>/rm/dm/devReceive/subDevReceiveXZList.jsp?mixId='+shuaId;

		}
		else{
			window.location.href='<%=contextPath%>/rm/dm/devReceive/subDevReceiveList.jsp?mixId='+shuaId+'&sonFlag='+sonFlag;
		}
    }
