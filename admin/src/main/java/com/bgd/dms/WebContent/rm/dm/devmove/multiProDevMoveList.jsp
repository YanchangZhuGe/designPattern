...
<script type="text/javascript">
	...
	function toAddDevInfo(){
		...
		popWindow('<%=contextPath%>/rm/dm/devmove/multiDevMoveNewApply.jsp','950:680');
	...
	function toModifyMainPlanPage(){
		...
		if(unitRet.datas[0].move_status == '0' ){
			popWindow('<%=contextPath%>/rm/dm/devmove/multiDevMoveModify.jsp?devMovId='+devMovId,'950:680');
		}else{
			alert("本单据已转移，不能修改!");
			return;
		}
	...
	function toSumbitDevApp(){
		...
		//判断状态如果是已提交
		var str = "select devapp.in_project_info_id,devapp.out_project_info_id,devapp.dev_mov_id,devapp.move_status from gms_device_move devapp ";
			str += "where devapp.dev_mov_id ='"+devMovId+"' ";
		var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		//什么状态不能删除，和业务专家确定
		if(unitRet.datas[0].move_status == null || unitRet.datas[0].move_status == '' || unitRet.datas[0].move_status == '0'){
			...
			var detsql = "update gms_device_account_dui dui set dui.actual_out_time=(select t.actual_out_time from gms_device_move_detail t where t.dev_mov_id in ('"+devMovId+"') and t.dev_acc_id=dui.dev_acc_id),";
				detsql += "dui.is_leaving='1',dui.transfer_state='',dui.check_time=(select t.actual_out_time from gms_device_move_detail t where t.dev_mov_id in ('"+devMovId+"') and t.dev_acc_id=dui.dev_acc_id) where exists(select 1 from gms_device_move_detail t where t.dev_acc_id=dui.dev_acc_id and t.dev_mov_id in ('"+devMovId+"'))";
			var params = "deleteSql="+detsql;
				params += "&ids=";
			var retObject = syncRequest('Post',path,params);
			
			var retObj = jcdpCallService("DevCommInfoSrv", "saveWellsDevMoveAuditInfowfpa", "dev_mov_id="+devMovId+"&in_project_no="+unitRet.datas[0].in_project_info_id+"&out_project_no="+unitRet.datas[0].out_project_info_id+"&multiflag=Y");
