	...
	<tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td>&nbsp;</td>
				<td></td>
			    <auth:ListButton functionId="" css="zj" event="onclick='toAdd()'" title="编辑"></auth:ListButton>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
	<tr>
		...
		exp="<input name = 'rdo_entity_name' id='rdo_entity_id' type='checkbox' value='{detail_id}' onclick='chooseOne(this);loadDataDetail();'/>">选择</td>
		<td class="bt_info_even" autoOrder="1">序号</td>
		<td class="bt_info_odd"  exp="{wz_name}">名称规格(设备编号)</td>
		<td class="bt_info_even" exp="{wz_prickie}">计量单位</td>
		<td class="bt_info_odd" exp="{apply_num}">申请数量</td>
		<td class="bt_info_even" exp="{submit_num}">审批数量</td>
		<td class="bt_info_odd" exp="{zhichenggan_num}">支撑杆数量(附件)</td>
		<td class="bt_info_even" exp="{pengbu_num}">篷布数量(附件)</td>
		<td class="bt_info_odd" exp="<textarea name=''  type='text' readonly>{appearance}">外观</td>
		<td class="bt_info_even" exp="<textarea name=''  type='text' readonly>{performance}">性能</td>
		<td class="bt_info_odd" exp="<textarea name=''  type='text'  readonly>{note}">备注</td>
		<td class="bt_info_even" exp="{if_write}">状态</td>
		<td class="bt_info_odd" exp="{if_submit}">是否提交</td>
	</tr>
</table>
</div>
<div id="fenye_box">
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		var sql ='';
		sql +="select d.*,case when d.plan_invoice_id is not null then '已编辑' else '未编辑' end if_write,case when m.proc_status='3' then '已提交' else '未提交' end if_submit from  gms_mat_demand_rough_repeat d inner join gms_mat_demand_plan_rough t on t.plan_id = d.plan_id join common_busi_wf_middle w on t.plan_id = w.business_id and w.proc_status='3' left join common_busi_wf_middle m on d.plan_invoice_id=m.business_id and m.bsflag='0'  where t.bsflag='0' and t.if_repeat = '1' and t.bsflag='0' and d.main_part like '%"+orgSubjectionId+"%'";
	...
	function loadDataDetail(shuaId){
		if(shuaId!= undefined){
			retObj = jcdpCallService("MatItemSrv","queryPlanWZXZ","detail_id="+shuaId);
			debugger;
	 ...
	 function toEdit(){ 
		 ids = getSelIds('rdo_entity_id');
		    if(ids==''){ alert("请先选中一条记录!");
		    	...
		    	var retObj = jcdpCallService("MatItemSrv", "queryWZXZPlanSubmit", "plan_id="+ids);
	 	    	var if_write = retObj.mapInfo.ifWrite;
	 	    	var if_submit = retObj.mapInfo.ifSubmit;
	 	    	if(if_write =='未编辑'){ alert("申请单未被编辑,不能修改!");
	 	     		return;
		 	    }else {
		 	    	if(if_submit =='已提交'){ alert("申请单已被提交!");
		 	     	return;
		 	    	}else{
						popWindow('<%=contextPath%>/mat/multiproject/plan/planWZXZ/planUpdateProject.jsp?plan_id='+ids,'1024:800');
		 	    	}
		 	    }
		 	}
	 }
	 function toAdd(){
	    ...
	    if(ids==''){ alert("请先选中一条记录!");
	    	...
	    	var retObj = jcdpCallService("MatItemSrv", "queryWZXZPlanSubmit", "plan_id="+ids);
 	    	var if_write = retObj.mapInfo.ifWrite;
 	    	var if_submit = retObj.mapInfo.ifSubmit;
 	    	if(if_write =='已编辑'){ alert("申请单已被编辑!");
 	     		return;
	 	    }else {
	 	    	if(if_submit =='已提交'){ alert("申请单已被提交!");
	 	     	return;
	 	    	}else{
				popWindow('<%=contextPath%>/mat/multiproject/plan/planWZXZ/planEditProject.jsp?plan_id='+ids,'1024:800');
	 	  		}
	 	    }
	    }
	 }  
		 ...
		 ids = getSelIds('rdo_entity_id');
		    if(ids==''){ alert("请先选中一条记录!");
		    	...
		    	var retObj = jcdpCallService("MatItemSrv", "queryWZXZPlanSubmit", "plan_id="+ids);
	 	    	var if_write = retObj.mapInfo.ifWrite;
	 	    	var if_submit = retObj.mapInfo.ifSubmit;
	 	    	if(if_write =='未编辑'){ alert("申请单未被编辑,不能提交!");
	 	     		return;
		 	    }else {
		 	    	if(if_submit =='已提交'){ alert("申请单已被提交!");
		 	     	return;
						...
						if(confirm('确定要提交吗?')){	  
							var retObj = jcdpCallService("MatItemSrv", "submitWZXZPlan", "plan_id="+ids);
							refreshData();
						}
		 	    	}
				}
     ...
     //汇总信息
    	 function taskShow(value){
    			...
    			var retObj = jcdpCallService("MatItemSrv", "queryProjectPlanWZXZ", "detail_id="+value);
    			debugger;
    			...
    			for(var i =0; taskList!=null && i < taskList.length; i++){
					...
					var stockNum = taskList[i].stockNum;
					var demandDate = taskList[i].demandDate;
					var orgAbbreviation = taskList[i].orgAbbreviation;
