...
<script type="text/javascript">
	...
	function refreshData(){
		...
		if('<%=devappid%>'!=null){
			...
			+"mixorg.org_abbreviation as mix_org_name,devapp.mix_org_id,"
			...
			if(orgType=="Y"){
				str += "where devapp.bsflag='0' and devapp.device_app_id='<%=devappid%>' ";
			}else{
				str += "left join common_busi_wf_middle wfmiddle on devapp.device_app_id=wfmiddle.business_id ";
				str += "where wfmiddle.proc_status='3' and devapp.bsflag='0' and devapp.device_app_id='<%=devappid%>'";
			}
			...
			if(orgType=="Y"){
				...
				prosql += "appdet.plan_start_date,appdet.plan_end_date,appdet.teamid,appdet.unitinfo,nvl(tmp.mixed_num, 0) + nvl(tmp1.dis_mixed_num, 0) mixed_num,";
				...
				prosql += "left join (select device_app_detid, sum(apply_num) as dis_mixed_num from gms_device_dismixapp_detail det ";
				prosql += "left join gms_device_dismixapp app on app.device_mixapp_id=det.device_mixapp_id ";
				prosql += "where app.bsflag='0' group by device_app_detid) tmp1 on tmp1.device_app_detid = appdet.device_app_detid ";
