...
<body style="background:#fff; overflow-y: auto;">
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    <td>&nbsp;</td>
			    <auth:ListButton functionId="" css="zj" event="onclick='toAdd()'" title="上传文档"></auth:ListButton>
...
<script type="text/javascript">
	...
	cruConfig.queryOp = "getAttachmentCollList";
	...
	// 复杂查询
	function refreshData(q_file_name,q_doc_type,q_doc_keyword,q_doc_importance,q_create_date){
		...
		cruConfig.submitStr = "relationId=<%=relation_id%>&file_name="+q_file_name+"&doc_type="+q_doc_type+"&doc_keyword="+q_doc_keyword+"&doc_importance="+q_doc_importance+"&create_date="+q_create_date;	
		queryData(1);
	}
	// 简单查询
	...
	function toAdd(){
	  	...
	  	popWindow('<%=contextPath%>/doc/common/upload_file_common.jsp?relationId=<%=relation_id%>&rootFolderId=<%=root_folderid%>&qualityControl=<%=quality_control%>');
	...
	function toEdit(){
	    ...
	    var file_id = ids.split(":")[0];
		var ucm_id = ids.split(":")[1];
		popWindow('<%=contextPath%>/doc/singleproject/edit_attach.jsp?fileId='+file_id);
