...
<script type="text/javascript"> 
	...
	function loadDataDetail(){
	   ...
	   var querySql ="select acc.dev_name,acc.dev_model,acc.self_num,acc.license_num,acc.dev_coding,max(info.repair_end_date) last_date, "
				...
				+"where acc.bsflag='0' and acc.dev_acc_id='"+dev_appdet_id+"' "
...
// 			$("#modify_date")[0].value=basedatas[0].modify_date;
			...
			$("#work_hour")[0].value=basedatas[0].work_hour;
			$("#fk_dev_acc_id")[0].value=dev_appdet_id;
			
			var querySql2 = "select t.maintenance_id,t.plan_date,t.mileage,t.work_hour from gms_device_maintenance_plan t where t.fk_dev_acc_id='"+dev_appdet_id+"' order by t.plan_num";
			...
			if(basedatas2!=null){
				...
				$("#mileage1")[0].value=basedatas2[0].mileage;
				$("#work_hour1")[0].value=basedatas2[0].work_hour;
				for(var i = 1 ; i < basedatas2.length ; i++){
					var arrayObj = {"maintenance_id":basedatas2[i].maintenance_id,"value":basedatas2[i].plan_date,"mileage":basedatas2[i].mileage,"work_hour":basedatas2[i].work_hour}; 
					addrow(arrayObj);
				}	
			}
	...
	function addrow(obj){
		...
		newTd3.className="inquire_item6";
		var newTd1=newTr.insertCell();
		newTd1.className="inquire_form6";
		var newTd4=newTr.insertCell();
		newTd4.className="inquire_item6";
		var newTd2=newTr.insertCell();
		newTd2.className="inquire_form6";
		...
		if(obj==undefined){
			...
			newTd3.innerText="累计里程(公里)";
			newTd1.innerHTML="<input type='text' name='mileage"+optnum+"' id='mileage"+optnum+"' value='' class='input_width' />";
			newTd4.innerText="累计工作小时(小时)";
			newTd2.innerHTML="<input type='text' name='work_hour"+optnum+"' id='work_hour"+optnum+"' value='' class='input_width' />";
		}else{
			newTd.innerHTML="<input name=maintenance_id"+optnum+" id=maintenance_id"+optnum+"  class=input_width type=hidden value="+obj.maintenance_id+"  /><input type='text' name='by_date"+optnum+"' id='by_date"+optnum+"' value='"+obj.value+"' readonly class='input_width' /><img src='<%=contextPath%>/images/calendar.gif' id='tributton"+optnum+"' width='16' height='16' style=cursor: hand ; onmouseover='calDateSelector(by_date"+optnum+",tributton"+optnum+")'; />";
			newTd3.innerText="累计里程(公里)";
			newTd1.innerHTML="<input type='text' name='mileage"+optnum+"' id='mileage"+optnum+"' value='"+obj.mileage+"' class='input_width' />";
			newTd4.innerText="累计工作小时(小时)";
			newTd2.innerHTML="<input type='text' name='work_hour"+optnum+"' id='work_hour"+optnum+"' value='"+obj.work_hour+"' class='input_width' />";
		}
