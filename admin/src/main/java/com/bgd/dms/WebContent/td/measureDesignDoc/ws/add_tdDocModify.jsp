...
function save(){
	...
	form.action = "<%=contextPath%>/td/ws/toSaveMeasureDoc.srq";
	form.submit();
	newClose();
}
		...
		if(datas!=null && datas.length>0){
			...
			for(var i=0;i<datas.length;i++){
				if(""!=datas[i].ucm_id){
					...
					document.getElementById("pk_0110000061100000012_"+i).value =ucmId;
					var str=datas[i].file_name==""?"":datas[i].file_name.substr(0,6)+'...';
					$("#down_0110000061100000012_"+i).append("&nbsp;<a style='color:blue;'  href=<%=contextPath%>/doc/downloadDocByUcmId.srq?docId="+ucmId+"&emflag=0>"+str+"</a>");
				}
			}
		}
	}else{
...
function addRow(){
	...
	var tr=document.all.tableDoc.insertRow();
  	tr.insertCell(0).innerHTML="<input type='hidden' id='pk_0110000061100000012_"+num+"' name='pk_0110000061100000012_"+num+"' value=''/>";
  	
	var td = tr.insertCell(1);
	td.setAttribute("colspan",2);
	td.innerHTML="<input type='file' name='0110000061100000012_"+num+"' id='0110000061100000012_"+num+"' class='input_width'/><div id='down_0110000061100000012_"+num+"'></div>";

	//tr.insertCell(2).innerHTML="";
	if("view"!=action){
		tr.insertCell(2).innerHTML="<img name='"+num+"' onclick='delRow()' src='<%=contextPath%>/images/delete.png' width='20' height='20' style='cursor: hand;'/>";
	}
}

function delRow(){
	//获得当前点击标签的附件ucm_id值 
    var oldUcmId=$("#pk_0110000061100000012_"+event.srcElement.name).val();
    //如果存在旧文件，则先删除
					...
					<div id="tab_box" class="tab_box">
						<div id="tab_box_content0" class="tab_box_content">
							<table id="tableDoc" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
								...
								<tr>
									<td class="inquire_item4">测量设计文档：</td>
									<td colspan="2">
										<input type="hidden" id="pk_0110000061100000012_0" name="pk_0110000061100000012_0" value=""/>
										<input type="file" name="0110000061100000012_0" id="0110000061100000012_0" class="input_width"/>
										<div id="down_0110000061100000012_0"></div>
									</td>
