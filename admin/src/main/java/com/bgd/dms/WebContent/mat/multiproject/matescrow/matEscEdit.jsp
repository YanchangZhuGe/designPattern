...
<%
	...
	//2013.10.15 物资调剂中需要加上选择区域（库尔勒，固城），编码管理中添加，以便后期维护-----lx
	String sqlFrom = "select t.*,t.rowid from comm_coding_sort_detail t where t.bsflag='0' and t.coding_code_id like '5110000057%' order by t.coding_show_id asc";
	List list = BeanFactory.getQueryJdbcDAO().queryRecords(sqlFrom);
%> 
...
<input type="hidden" name="project_info_no" id="project_info_no" class="input_width" value="<gms:msg msgTag="matInfo" key="project_info_no" />" />
	<table  border="0" cellpadding="0" cellspacing="0" class="tab_line_height" width="100%">
  	...
  	<tr>
    	<td colspan="4" align="center">代管物资调剂</td>
    </tr>
    <tr>
...
<table  border="0" cellpadding="0" cellspacing="0" class="tab_line_height" width="100%">
<input type='hidden' name ='invoices_id' id='invoices_id' value='<gms:msg msgTag="matInfo" key="invoices_id" />'>
    <tr>
    	
    	<td class="inquire_item6">调剂单号：</td>
      	<td class="inquire_form6"><input type="text" name="invoices_no" id="invoices_no" class="input_width" value="<gms:msg msgTag="matInfo" key="invoices_no" />" /></td>
      	<td class="inquire_item6">经办人：</td>
      	<td class="inquire_form6"><input type="text" name="operator" id="operator" class="input_width" value="<gms:msg msgTag="matInfo" key="operator" />" /></td>
      	<td class="inquire_item6">调剂时间：</td>
      	<td class="inquire_form6"><input type="text" name="input_date" id="input_date" class="input_width" value="<gms:msg msgTag="matInfo" key="input_date" />" />
      	<img src='<%=contextPath%>/images/calendar.gif'id='tributton_input_date' width='16' height='16' style='cursor: hand;' onmouseover='calDateSelector(input_date,tributton_input_date);'/></td>
    </tr>
    <tr>
      	<td class="inquire_item6">备注：</td>
      	<td class="inquire_form6"><input type="text" name="note" id="note" class="input_width" value="<gms:msg msgTag="matInfo" key="note" />" /></td>
      	<td class="inquire_item6">合计金额：</td>
		<td class="inquire_item6"><input type="text" name="total_money" id="total_money"
			class="input_width"
			value="<gms:msg msgTag="matInfo" key="total_money" />" readonly/></td>
		<td class="inquire_item6">区域选择：</td>
		<td class="inquire_form6">
		<select name="area" id="area" class="select_width">
				<option value="">请选择</option>
				<%if(list!=null&&list.size()>0){
					for(int i=0;i<list.size();i++){
						Map map = (Map)list.get(i);
						String coding_name = (String)map.get("codingName");
						String coding_code_id = (String)map.get("codingCodeId");
				 %>
				<option value="<%=coding_code_id%>"><%=coding_name %></option>
				<%}} %>
			</select>
		</td>		
    </tr>
...
<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input name = 'teammat_info_idetail_id_{teammat_info_idetail_id}' id='rdo_entity_id' type='checkbox' checked='checked' value='{teammat_info_idetail_id}' onclick='loadDataDetail()'/>"><input type='checkbox' name='task_entity_id' value='' onclick='check()'/></td>
			      <td class="bt_info_even" autoOrder="1">序号</td>
			      <td class="bt_info_odd" exp="{wz_id}">资源编码</td>
			      ...
			      <td class="bt_info_odd" exp="{regulate_num}">审批数量</td>
			      <td class="bt_info_even" exp="{mat_num}">调剂数量</td>
			      <td class="bt_info_odd" exp="<input name='actual_price_{teammat_info_idetail_id}'  type='text' value='{actual_price}'/>">调剂单价</td>
			      <td class="bt_info_even" exp="<input name='total_money_{teammat_info_idetail_id}'  type='text' value='{total_money}'readonly/> " >金额</td>
			    </tr>
			  </table>
			</div>
...
<script type="text/javascript">
	...
	document.getElementById("area").value = "<gms:msg msgTag="matInfo" key="area" />";
    var value = "<gms:msg msgTag="matInfo" key="plan_invoice_id" />"+","+"<gms:msg msgTag="matInfo" key="invoices_id" />";
    function refreshData(){
		...
		cruConfig.queryOp = "findEscLeaf";
		cruConfig.submitStr ="ids="+value;
		queryData(1);
	}
	function save(){	
	    ...
	    if(loadDataDetail()){
		    document.getElementById("form1").action = "<%=contextPath%>/mat/multiproject/matescrow/updateMatEscEdit.srq?laborId="+ids;
	 ...
	 function loadDataDetail(shuaId){
			...
			for(var i=1;i<row.length;i++){
				...
				if(cell_7!=undefined && cell_8!=undefined){
						...
						if(Number(cell_6)<Number(outNum)){
							alert("调剂数量不能大于审批数量!");
							flag=false;
							return;
						}
					row[i].cells[9].firstChild.value=Math.round(outNum*wzPrice*1000)/1000;
					totalMoney+=Number(row[i].cells[9].firstChild.value);
				}
			}
