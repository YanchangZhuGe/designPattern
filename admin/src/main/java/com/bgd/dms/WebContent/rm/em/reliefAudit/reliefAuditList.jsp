      	...
      	<div id="list_table">
			...
			<div id="table_box" style="height:100%">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{human_relief_no}-{project_type}' id='rdo_entity_id_{human_relief_no}' />" >选择</td>
...
<script type="text/javascript">
	...
	function toSubmit(){
		...
		popWindow('<%=contextPath%>/rm/em/toReliefAuditEdit.srq?id='+ids,'1000:800');
		
	}
	
	function refreshData(arrObj){

		var str = "select  t.project_type, te.proc_status,decode(te.proc_status,   '1',   '待审批',  '3',  '审批通过', '4', '审批不通过',te.proc_status) proc_status_name,  p.human_relief_no,p.project_info_no,t.project_name, p.apply_no,p.apply_state,p.applicant_id,e.employee_name, p.apply_company,i.org_name, p.apply_date from bgp_project_human_relief p  left join common_busi_wf_middle te on    te.business_id=p.human_relief_no   and te.bsflag='0'  	 and te.business_type in ('5110000004100000026','5110000004100001053')   left join comm_org_information i on p.apply_company=i.org_id and i.bsflag='0'  left join comm_org_subjection cosb   on i.org_id=cosb.org_id  and cosb.bsflag='0'  left join gp_task_project t on p.project_info_no=t.project_info_no and t.bsflag='0' left join comm_human_employee e on p.applicant_id=e.employee_id and e.bsflag='0' where  p.bsflag='0'    and cosb.org_subjection_id like'"+org_subjection_idA+"%'  ";
		...
		cruConfig.currentPageUrl = "/rm/em/reliefAudit/reliefAuditList.jsp";
		queryData(1);
    ...
    function loadDataDetail(ids){ 
    	...
    	var businessType_s="5110000004100000026";
    	...
    	processNecessaryInfo={         
	    		...
	    		businessInfo:"多项目人员调剂审核调配流程",        //用于待审批界面展示业务信息
		...
		var querySql = "select rownum ,p.* from ( select p.apply_team,d1.coding_name apply_team_name,p.post,d2.coding_name post_name, p.people_number,p.plan_start_date,p.plan_end_date,(p.plan_end_date-p.plan_start_date+1) nums,decode(p.age,'','','0','20-30岁','1','30-40岁','2','40岁以上') age, decode(p.work_years,'','','0','3年以下','1','3-5年','2','5-10年','3','10年以上') work_years, decode(p.culture,'','','0','博士研究生','1','硕士研究生','2','大学本科以上','3','大学专科(高职)以上','4','中专','5','技校','6','高中以上','7','初中以上','8','小学以上','9','其他') culture  from bgp_project_human_reliefdetail p left join comm_coding_sort_detail d1 on p.apply_team=d1.coding_code_id and d1.bsflag='0' left join comm_coding_sort_detail d2 on p.post=d2.coding_code_id and d2.bsflag='0' where p.human_relief_no='"+ids.split("-")[0]+"' and p.bsflag='0' order by p.apply_team, p.post ) p ";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=10000&querySql='+querySql);
		var datas = queryRet.datas;
		
		
		var querySql1 = "select rownum, p.* from (select d.apply_team, d1.coding_name apply_team_name, d.post, d2.coding_name post_name, d.people_number,d.deploy_org,i.org_name, d.plan_start_date, d.plan_end_date,(d.plan_end_date - d.plan_start_date+1) nums,decode(d.age,'','','0','20-30岁','1','30-40岁','2','40岁以上') age, decode(d.work_years,'','','0','3年以下','1','3-5年','2','5-10年','3','10年以上') work_years, decode(d.culture,'','','0','博士研究生','1','硕士研究生','2','大学本科以上','3','大学专科(高职)以上','4','中专','5','技校','6','高中以上','7','初中以上','8','小学以上','9','其他') culture,d.notes from bgp_project_human_relief_deta d inner join bgp_project_human_reliefdetail p on d.post_no = p.post_no and p.bsflag = '0'   inner join bgp_project_human_relief f on p.human_relief_no=f.human_relief_no and f.bsflag='0' left join comm_coding_sort_detail d1 on d.apply_team = d1.coding_code_id and d1.bsflag = '0' left join comm_coding_sort_detail d2 on d.post = d2.coding_code_id and d2.bsflag = '0' left join comm_org_information i on d.deploy_org=i.org_id and i.bsflag='0'  where f.human_relief_no='"+ids.split("-")[0]+"' and d.bsflag = '0'  order by d.apply_team, d.post ) p ";
