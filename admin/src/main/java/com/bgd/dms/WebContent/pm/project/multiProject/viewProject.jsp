...
<%
	...
	if(org_subjection_id.startsWith("C105007")){
		response.sendRedirect(contextPath+"/pm/project/multiProject/tqh/viewProject.jsp");//滩浅海项目
	}
	if(org_subjection_id.startsWith("C105008")){//综合物化探
		response.sendRedirect(contextPath+"/pm/project/multiProject/wt/viewProject.jsp");
	}
	if(org_subjection_id.startsWith("C105005001")){//新兴
		response.sendRedirect(contextPath+"/pm/project/singleProject/ws/viewProject.jsp?projectInfoNo="+projectInfoNo);
	}
	else{
		if("5000100004000000006".equals(projectType)){//深海
			response.sendRedirect(contextPath+"/pm/project/multiProject/sh/viewProject.jsp");
		}
	}
%>
<html>
