...
<%
	...
	String projectFatherNo = request.getParameter("projectInfoNo")==null?"":request.getParameter("projectInfoNo");
	String isws = request.getParameter("isws");
%>
<html>
<head>
...
function page_init(){
	...
	cruConfig.submitStr = "isSingle=<%=isSingle %>&isws=true&orgSubjectionId=<%=orgSubjectionId %>&orgId=<%=orgId %>&projectType=<%=projectType%>&projectFatherNo=<%=projectFatherNo%>";
...
function openProjectHealth(projectInfoNo,flag){
	...
	var querySql = "select d.org_subjection_id from gp_task_project_dynamic d where d.bsflag ='0' and d.project_info_no ='"+projectInfoNo+"' and rownum =1"
	...
	if(retObj!=null && retObj.returnCode =='0' && retObj.datas !=null && retObj.datas[0]!=null){
		org_subjection_id = retObj.datas[0].org_subjection_id ==null?"":retObj.datas[0].org_subjection_id;
	}
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_odd" exp="{tp_project_name}<input type='hidden' id='projectName{project_info_no}' value='{tp_project_name}'/>" >项目名称</td>
