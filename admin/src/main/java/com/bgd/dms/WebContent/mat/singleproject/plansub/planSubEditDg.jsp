...
<input type='hidden' name='laborId' id='laborId' value=''/>
    ...
    <tr>
	    ...
	    <td class="inquire_item4">计划类型：</td>
	    <td class="inquire_form4"><input type="text" name="plan_invoice_type" id="plan_invoice_type" class="input_width" value=""></td>
    </tr>
    <tr>
...
<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_even" exp="<input name = 'apply_num{wz_id}'  type='text' value='{demand_num}' />">需求数量</td>
			      ...
			      <td class="bt_info_odd" exp="<input name='plan_money{wz_id}'  type='text' value='' readonly/>">计划金额</td>
			   	 <td class="bt_info_even" isShow='Hide' exp="<input name='have_num{wz_id}'  type='text' value='{have_num}' readonly/>">现有数量</td>
			    </tr>
			  </table>
			</div>
...
<script type="text/javascript">
    ...
    function refreshData(){
    	...
    	cruConfig.queryOp = "getTeamEditDg";
	...
	function save(){	
	    ...
	    openMask();
	    document.getElementById("form1").action = "<%=contextPath%>/mat/singleproject/plansub/updatePlanSubDg.srq";
	...
	for(var i=1;i<row.length;i++){
		...
		if(cell_0){
			...
			if(cell_4!=undefined ){
					...
					row[i].cells[6].firstChild.value=Math.round((cell_3*cell_4)*1000)/1000;
				if(parseInt(row[i].cells[6].firstChild.value)<0){
				...
				totalMoney +=  Math.round((row[i].cells[6].firstChild.value)*1000)/1000;;
			}
		}
	}
