...
function submitButton(){  
		...
		var submitStr = 'JCDP_TABLE_NAME=BGP_CORRECTINVE_INFORMATION&JCDP_TABLE_ID=&org_sub_id='+org_sub_id
		+'&second_org='+second_org+'&third_org='+third_org+'&rectification_numbers='+rectification_numbers+'&rectification_state='+rectification_state
		...
		var corrective_no=retObject.entity_id; //获得 主表id 			
		 top.frames('list').frames[2].refreshData();	 
		 window.open("<%=contextPath%>/hse/hseOptionPage/hiddeniInformation/ChomeFrame.jsp?corrective_no="+corrective_no,'homeMain','height=500,width=1000px,left=100px,top=100px,menubar=no,status=no,toolbar=no ', '');
...
function checkText0(){
  ...
  querySql1 = "select count(*)-(-1) as num_p from BGP_CORRECTINVE_INFORMATION  where bsflag='0'";
	...
	if(queryRet1.returnCode=='0'){
	  datas1 = queryRet1.datas;	 
		if(datas1 != null && datas1 != ''){	 
			...
			document.getElementsByName("rectification_numbers")[0].value=numDate;			
		}
   }	
