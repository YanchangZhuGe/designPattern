...
<body onload='getApplyTeam();getConRatio()'>
...
<table border="0" cellpadding="0" cellspacing="0" class="tab_line_height" width="100%">
	<tr>
		...
		<td class="inquire_item6">
			...
			<input type='hidden' name='wz_type' id='wz_type' value='<%=wz_type%>'/>
...
<script type="text/javascript"><!--
	...
	if(wz_type=="11"){
		checkdiv(1);
		document.getElementById("banzu").style.display = "none";
	}else{
		checkdiv(2);
	}
	var checked = false;
	...
	function refreshData(){
		...
		cruConfig.queryOp = "getOtherGantOutDg";
	...
	function showDevPage(){
		var obj = new Object();
		var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/tree/selectAccount.jsp?org_id=<%=owning_org_id%>&projectinfono="+projectInfoNo,obj,"dialogWidth=820px;dialogHeight=500px");
...
//    	var applyTeamList=jcdpCallService("HumanCommInfoSrv","queryApplyTeam","");
	...
	function checkdiv(value){
		...
		var sa = document.getElementsByName("coding_code");
	...
	function getRepeatDatas(){
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getOtherRepOut_Dg_FaFang";
		queryData(1);
		}
	...
	function openMatHave(){
		...
		//popWindow("<%=contextPath%>/mat/singleproject/warehouseDg/out/grant/selectGrantList.jsp?wz_type="+wz_type+"&selectWzId="+selectWzId,'1024:800');
		debugger;
