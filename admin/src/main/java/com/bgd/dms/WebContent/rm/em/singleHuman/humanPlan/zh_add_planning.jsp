...
<%
    ...
    String plan_ids=request.getParameter("id");
...
function addLine( plan_detail_ids, apply_teams, posts, people_numbers,profess_numbers, plan_start_dates, plan_end_dates, numss, notess){
	...
	td.className=classCss+"even";
	td.innerHTML = '<input  disabled="disabled" type="text" class="input_width" id="people_number' + '_' + rowNum + '" name="people_number' + '_' + rowNum + '" value="'+people_number+'" onpropertychange="if(isNaN(value)) value=value.substring(0,value.length-1);" />';
...
var dataLength="";
function page_init(){
	var plan_detail_id = "<%=plan_ids%>"; 
	if(plan_detail_id!='null'){
		var querySql = "select d.plan_detail_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,d.people_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1 ) nums,d.notes,d.profess_number from bgp_comm_human_plan_detail d left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0' left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' where d.plan_detail_id in ("+plan_detail_id+") and d.spare1 is null  and d.bsflag='0'  order by d.modifi_date  ";
