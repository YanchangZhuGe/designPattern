      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td>&nbsp;</td>
			    <%if(user.getOrgSubjectionId().equals("C105006")||user.getOrgSubjectionId().equals("C105")||user.getOrgSubjectionId().equals("C105006019")) {%> 
			    <td class='ali_btn'><span class='xg'><a href='####' onclick='toModifyPlanPage()'  title='修改'></a></span></td>
			   <% }%>
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			     	<td class="bt_info_odd" exp="<input type='checkbox' id='selectedbox_{device_coll_mixinfo_id}' name='selectedbox' value='{device_coll_mixinfo_id}' stateinfo='{state}' onclick='chooseOne(this)' leavinginfo='{oprstate_desc}'/>" >选择</td>
				...
				<div id="tab_box_content1" name="tab_box_content1" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							...
							<td class="bt_info_even" width="11%">进队总数量</td>
...
<script type="text/javascript">
	...
	function refreshData(v_project_name,v_backapp_name,v_device_backapp_no,v_back_org_name){
		var str = "select t.device_coll_mixinfo_id,gp.project_name,t.device_mixapp_no,";
			...
			str += "collback.backdate,case t.opr_state when '1' then '处理中'   when '2' then   '存在未交回' when '9' then '已处理' else '未处理' end as oprstate_desc ";
			...
			str += "left join comm_org_subjection orgsub on orgsub.org_id=t.receive_org_id and orgsub.bsflag='0' ";
			str += "where t.state='9'and t.bsflag='0' and t.back_dev_type='S9000' ";
			//加机构权限
		...
		if(v_backapp_name!=undefined && v_backapp_name!=''){
			str += "and collback.backapp_name like '%"+v_backapp_name+"%' ";
		}
		if(v_device_backapp_no!=undefined && v_device_backapp_no!=''){
			str += "and collback.device_backapp_no like '%"+v_device_backapp_no+"%' ";
		}
		if(v_back_org_name!=undefined && v_back_org_name!=''){
			str += "and i.org_abbreviation like '%"+v_back_org_name+"%' ";
		}
		str += "order by t.opr_state nulls first,t.opr_state desc,collback.backdate desc";
	...
	function toDetailPage(){
		...
		window.location.href='<%=contextPath%>/rm/dm/collectStore/subStockin.jsp?id='+shuaId;
    }
	function dbclickRow(shuaId){
		window.location.href='<%=contextPath%>/rm/dm/collectStore/subStockin.jsp?id='+shuaId;
	}
	function toModifyPlanPage()
	{
		var shuaId ;
    	$("input[type='checkbox'][name='selectedbox']").each(function(){
    		if(this.checked){
		...
		var leavinginfo = $("input[type='checkbox'][name='selectedbox'][value='"+shuaId+"']").attr("leavinginfo");
		  if(leavinginfo=="未处理"){
			  popWindow('<%=contextPath%>/rm/dm/collectStore/stockin-modify.jsp?shuaId='+shuaId,'950:680');
