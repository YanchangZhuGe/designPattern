...
Ext.onReady(function() {
    ...
    var store = Ext.create('Ext.data.TreeStore', {
        ...
        proxy: {
       	 type : 'ajax',
            method: 'get',
            url: '<%=contextPath%>/crm/common/getCompanyTypeTree.srq',
            reader: {
                type : 'json'
            }
        },
    ...
    function cellclick(grid,colspan, colIndex, rowdata, rowspan,rowIndex,e) {
        ...
        //var parent = rowdata.parendNode.data.id;
        //alert(parent);
...
function search(name){
	...
	var root = Ext.getCmp('gridId').store.getRootNode();
	clearCls(root);
	var retObj = jcdpCallService("ClientRelationSrv","search", "name="+name+"&type="+type);
	if(retObj.returnCode =='0'){
		var ids = retObj.ids;
		return ids;
	}
}
function clearCls(root){
	if(root!=null &&root.childNodes && root.childNodes.length>0){
