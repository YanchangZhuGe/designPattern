...
function refreshData(arrObj){
	...
	if(isProject=="1"){
		...
		var querySqlAddP = "and tr.project_no='<%=user.getProjectInfoNo()%>'";
		
	    querySql = "  select * from (   select   ion.org_abbreviation as org_name,  tr.second_org,tr.org_sub_id,        oi1.org_abbreviation as second_org_name, tr.emergency_no,       tr.supplies_name, tr.supplies_category,  decode(tr.supplies_category,'1','人身防护','2','医疗急救','3','消防救援','4','防洪防汛','5','应急照明','6','交通运输','7','通讯联络','8','检测监测','9','工程抢险','10','剪切破拆','11','电力抢修','12','其他') supplies_categorys_s,acquisition_time, tr.bsflag   from BGP_EMERGENCY_STANDBOOK tr     join comm_org_subjection os1     on tr.second_org = os1.org_subjection_id    and os1.bsflag = '0'   join comm_org_information oi1     on oi1.org_id = os1.org_id    and oi1.bsflag = '0'  left   join comm_org_subjection os2     on tr.third_org = os2.org_subjection_id    and os2.bsflag = '0'  left   join comm_org_information oi2     on oi2.org_id = os2.org_id    and oi2.bsflag = '0'   left join comm_org_subjection ose    on tr.org_sub_id = ose.org_subjection_id   and ose.bsflag = '0'  join comm_org_information ion    on ion.org_id = ose.org_id      where     tr.emergency_no not in (select dil.emergency_no  from BGP_MATERIALS_DETAIL dil left join  BGP_MATERIALS_ACCEPTANCE  ion on ion.MATERIALS_NO=dil.MATERIALS_NO and dil.bsflag='0' where ion.bsflag='0') " + querySqlAddP+
				" union " +
				" select  ai.org_abbreviation  as org_name,  w.org_subjection_id as second_org ,w.org_id as org_sub_id,  oi2.org_abbreviation as second_org_name,ein.information_no  as emergency_no,       g.wz_name as supplies_name,  ein.supplies_category,    decode(ein.supplies_category,'1','人身防护','2','医疗急救','3','消防救援','4','防洪防汛','5','应急照明','6','交通运输','7','通讯联络','8','检测监测','9','工程抢险','10','剪切破拆','11','电力抢修','12','其他') supplies_category_s,       ein.acquisition_time,        w.bsflag  from gms_mat_infomation g  inner join gms_mat_teammat_info w    on g.wz_id = w.wz_id   and w.bsflag = '0'    left join comm_org_subjection os1    on w.org_id = os1.org_subjection_id   and os1.bsflag = '0'  left join comm_org_information oi1    on oi1.org_id = os1.org_id   and oi1.bsflag = '0'  left join BGP_EMERGENCY_INFORMATION ein    on ein.teammat_info_id = w.teammat_info_id  left join comm_org_subjection os2    on w.org_subjection_id = os2.org_subjection_id   and os2.bsflag = '0'  left join comm_org_information oi2    on oi2.org_id = os2.org_id   and oi2.bsflag = '0' left join comm_org_information ai   on w.org_id = ai.org_id   and ai.bsflag = '0'  where g.coding_code_id like '45%'       and  w.project_info_no = '<%=user.getProjectInfoNo()%>'   and ein.information_no is not null    and ein.information_no  not in       (select dil.emergency_no          from BGP_MATERIALS_DETAIL dil          left join BGP_MATERIALS_ACCEPTANCE ion            on ion.MATERIALS_NO = dil.MATERIALS_NO           and dil.bsflag = '0'         where ion.bsflag = '0')     )  ok  where ok.bsflag='0'  ";
...
function add(){ 
  ...
  self.parent.frames["rightframe"].location="<%=contextPath %>/hse/hseOptionPage/preparednessAndResponse/rightPage.jsp?addTypes=<%=addTypes%>&emergency_no="+certificate_no+"&isProject=<%=isProject%>&materials_no="+materials_no;
	}
