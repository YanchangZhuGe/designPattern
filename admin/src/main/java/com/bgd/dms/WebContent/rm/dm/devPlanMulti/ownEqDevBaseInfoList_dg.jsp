			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">	<!-- {device_app_id}~{mix_state_desc}~{allapp_type}	 -->
			     <tr id='device_mixinfo_id_{device_mixinfo_id}' name='device_mixinfo_id'>
			     	<td class="bt_info_even" exp="<input type='checkbox' name='selectedbox' mixstate='{opr_state}' value='{device_mixinfo_id}~{device_app_id}~{allapp_type}~{device_allapp_id}' id='selectedbox_{device_mixinfo_id}~{device_app_id}' />" >选择</td>
				...
				<div id="tab_box_content1" name="tab_box_content1" idinfo="" class="tab_box_content" style="display:none">
					<table border="0" cellpadding="0" cellspacing="0"  class="tab_line_height" width="100%" style="margin-top:10px;background:#efefef"> 
						<tr class="bt_info">
							...
							<td class="bt_info_odd" width="11%">ERP编号</td>
...
var strbase="";
	...
	if('<%=orgType%>'=='Y'){
		 strbase="select * from (select tt1.project_info_no,tt1.device_allapp_id,'' as device_mixinfo_id,tt1.allapp_type,tt1.device_app_id,tt1.project_name,tt1.device_app_name,tt1.device_app_no,tt1.app_org_name,tt1.appdate,'' as opr_state,case  when tt3.assign_num=0 then '未分配' when tt2.apply_num-tt3.assign_num=0 then '已分配' when  tt2.apply_num-tt3.assign_num>0 then '分配中'else ''  end as mix_state_desc from ("
			...
			+"WHERE  app.BSFLAG='0' and allapp.bsflag='0' and (allapp.ALLAPP_TYPE='S0000' or allapp.ALLAPP_TYPE='S14050208' ) and app.DEVICE_APP_ID IN(SELECT DISTINCT detail.DEVICE_APP_ID FROM gms_device_app_detail detail WHERE detail.BSFLAG='0' ";
			if(dgys == 'Y'){//小车/运输设备服务中心
				strbase += "and (detail.DEV_OUT_ORG_ID =  '<%=orgId%>' or detail.DEV_OUT_ORG_ID ='"+dgyxorg+"') ";
			 }else{
			 	strbase += "and detail.DEV_OUT_ORG_ID='<%=orgId%>' ";
			 }
			strbase += ")) tt1 left join ( "
			+"select appdet.DEVICE_APP_ID, sum(appdet.APPLY_NUM) as apply_num from GMS_DEVICE_APP_DETAIL appdet where appdet.BSFLAG='0' ";
		 	if(dgys == 'Y'){//小车/运输设备服务中心
				strbase += "and (appdet.DEV_OUT_ORG_ID =  '<%=orgId%>' or appdet.DEV_OUT_ORG_ID ='"+dgyxorg+"') ";
			 }else{
			 	strbase += "and appdet.DEV_OUT_ORG_ID='<%=orgId%>' ";
			 }
		 	strbase +="group by appdet.DEVICE_APP_ID) tt2 on tt1.DEVICE_APP_ID=tt2.DEVICE_APP_ID "
			+"left join ("
			+"select  device_app_id,case when sum(t2.assign_num) is null then 0  else sum(t2.assign_num)  end assign_num from GMS_DEVICE_APP_DETAIL t1 left join (select DEVICE_APP_DETID,  sum(ASSIGN_NUM) as assign_num from GMS_DEVICE_APPMIX_MAIN where BSFLAG = '0' group by DEVICE_APP_DETID) t2 on t1.DEVICE_APP_DETID = t2.DEVICE_APP_DETID where t1.BSFLAG = '0' ";
		 	if(dgys == 'Y'){//小车/运输设备服务中心
				strbase += "and (t1.DEV_OUT_ORG_ID =  '<%=orgId%>' or t1.DEV_OUT_ORG_ID ='"+dgyxorg+"') ";
			 }else{
			 	strbase += "and t1.DEV_OUT_ORG_ID =  '<%=orgId%>' ";
			 }
		 	strbase += "group by device_app_id) tt3 on tt1.DEVICE_APP_ID=tt3.DEVICE_APP_ID "
		    +"union "
		    +"select collapp.project_info_no,allapp.device_allapp_id,cmf.device_mixinfo_id,allapp.allapp_type,collapp.device_app_id,tp.project_name,collapp.device_app_name,collapp.device_app_no,inorg.org_abbreviation as app_org_name,collapp.appdate,"
		    +"cmf.opr_state,case cmf.opr_state when '1' then '分配中' when '9' then '已分配' else '未分配' end as mix_state_desc from gms_device_app collapp left join gms_device_allapp allapp on collapp.device_allapp_id = allapp.device_allapp_id "
		    +"left join gms_device_collmix_form cmf on cmf.device_app_id = collapp.device_app_id left join gp_task_project tp on collapp.project_info_no = tp.project_info_no left join gms_device_app_detail tail on tail.device_app_id = collapp.device_app_id "
		    +" left join comm_org_information inorg on collapp.app_org_id = inorg.org_id left join comm_org_information outorg on tail.dev_out_org_id = outorg.org_id "
		    +"left join gms_device_coll_outform cof on cof.device_mixinfo_id = cmf.device_mixinfo_id left join comm_human_employee he on collapp.employee_id = he.employee_id left join comm_org_subjection orgsub "
		    +"on cmf.out_org_id = orgsub.org_id where allapp.allapp_type = 'S9997' and allapp.bsflag = '0' and collapp.bsflag='0' and (cof.devouttype is null or cof.devouttype = '1') "
		    +"and (cof.bsflag is null or cof.bsflag = '0') ";
		 if(dgys == 'Y'){//小车/运输设备服务中心
			strbase += "and (tail.dev_out_org_id ='<%=orgId%>' or tail.dev_out_org_id='"+dgyxorg+"') ";
		 }else{
		 	strbase += "and tail.dev_out_org_id ='<%=orgId%>' ";
		 }
		 strbase += ") where 1=1 ";
	}else{
		strbase = "select '' device_mixinfo_id,tmp.* from(select tt1.*,nvl(tt2.apply_num,0) apply_num,case when nvl(tt3.assign_num,0)=0 then '未分配' when nvl(tt2.apply_num,0) - nvl(tt3.assign_num,0)=0 then '已分配' when nvl(tt2.apply_num,0) - nvl(tt3.assign_num,0)>0 then '分配中'else ''  end as mix_state_desc "
				...
				+"WHERE app.BSFLAG='0' and allapp.bsflag='0' AND (allapp.ALLAPP_TYPE='S0000'  OR allapp.ALLAPP_TYPE='S14050208') "
				...
				+"where allapp.ALLAPP_TYPE='S9997' and allapp.bsflag = '0' and collapp.bsflag='0' "
				+") tt1 left join (                                                                                                                                                                                          "
					...
					+") tt3 on tt2.DEVICE_APP_ID=tt3.DEVICE_APP_ID ) tmp where 1=1    ";
		}
			...
			if(currentid != undefined && idinfo == currentid){
				...
				if(currentid==undefined){
					...
					if(info[1]!=''){
						//管理员查询该申请单的全部
						if(info[2] == 'S9997'){
							var prosql = "select '1' as type,'调配明细' as showtype,'' dev_coding,outsub.device_name,outsub.device_model,"+
								"nvl(outsub.out_num, 0) as outednum from gms_device_coll_outsub outsub "+
								"left join gms_device_coll_outform form on outsub.device_outinfo_id = form.device_outinfo_id "+
								"where form.device_mixinfo_id = '"+info[0]+"' "+
								"union all "+
								"select '2' as type,'补充明细' as showtype,'' dev_coding,acc.dev_name as device_name,acc.dev_model asdevice_model,1 as outednum "+
								"from gms_device_equ_outdetail_added added left join gms_device_coll_outform outform on added.device_outinfo_id = outform.device_outinfo_id "+
								"left join gms_device_account acc on acc.dev_acc_id = added.dev_acc_id where outform.device_mixinfo_id = '"+info[0]+"' ";
						}else if(info[2] == 'S14050208'){
							var prosql = "select '1' as type,'调配明细' as showtype,'' dev_coding,ci.dev_name as device_name,ci.dev_model as device_model,amd.assign_num as outednum "+
								...
								"where amd.device_mixinfo_id in (select form.device_mixinfo_id from gms_device_mixinfo_form form where form.device_app_id='"+info[1]+"' and form.bsflag='0' )  order by  ci.dev_name";//and form.out_org_id='<%=orgId%>'						
						}else{
							var prosql = "select '1' as type,'调配明细' as showtype,account.dev_coding,account.self_num,account.license_num,account.dev_name as device_name,account.dev_model as device_model,'1' as outednum ";
								...
								if('<%=orgType%>'=='Y'){
									//如果是服务中心的执行下面这条
									prosql +="where amm.device_mixinfo_id in (select form.device_mixinfo_id from GMS_DEVICE_MIXINFO_FORM form where form.DEVICE_APP_ID='"+info[1]+"' and form.BSFLAG='0' ";
									if(dgys == 'Y'){//小车/运输设备服务中心
										prosql += "and (form.OUT_ORG_ID =  '<%=orgId%>' or form.OUT_ORG_ID ='"+dgyxorg+"') ";
									 }else{
										prosql += "and form.OUT_ORG_ID='<%=orgId%>' ";
									 }
									prosql +=") order by account.dev_name";								
								}else{
									prosql +="where amm.device_mixinfo_id in (select form.device_mixinfo_id from GMS_DEVICE_MIXINFO_FORM form where form.DEVICE_APP_ID='"+info[1]+"' and form.BSFLAG='0')  order by account.dev_name";
								}
						}						
					...
					if(currentid==undefined){
						...
						//alert(info);
							...
							if(mixtype == 'S9997'){
								...
								var prosql = "select form.DEVICE_MIXINFO_ID,form.MIXINFO_NO,case form.OUT_ORG_ID when 'C6000052795280' then '小车/运输设备服务中心' else outorg.ORG_ABBREVIATION end as ORG_ABBREVIATION,form.CREATE_DATE from GMS_DEVICE_MIXINFO_FORM form "
	...
	function loadDataDetail(shuaId){
			...
			//	}									
				...
				str += "left join comm_org_information apporg on devapp.app_org_id=apporg.org_id ";
				str += "left join comm_org_information mixorg on devapp.mix_org_id=mixorg.org_id ";
				str += "where devapp.device_app_id='"+info[1]+"'";
	...
	function dbclickRow(shuaId){
		...
		if(info[2]=='S9997'){//地震仪器
			popWindow('<%=contextPath%>/rm/dm/collDevOutForm/collDevOutInfoNew_dg.jsp?deviceallappid='+info[3]+'&devappid='+info[1]+'&mixstate='+mixState,'1050:680');
	...
	function appendDataToDetailTab(filterobj,datas){
		if(filterobj == '#detailList'){
			for(var i=0;i<basedatas.length;i++){
				var innerHTML = "<tr>";
				innerHTML += "<td>"+(i+1)+"</td><td>"+datas[i].showtype+"</td><td>"+datas[i].dev_coding+"</td><td>"+datas[i].device_name+"</td>";
				if(dgys == 'Y'){
