		  ...
		  <div style="overflow:auto">
			  <table style="width:97.9%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
					...
					<td class="bt_info_even" width="8%">需求数量</td>
					<td class="bt_info_odd" width="6%">已申请数量</td>
					<td class="bt_info_even" width="6%">申请数量</td>
					<td class="bt_info_odd" width="10%">开始时间</td>
...
$().ready(function(){
	...
	function refreshData(){
		...
		if('<%=deviceappid%>'!=null){
			...
			var prosql = "select * from (select aad.device_addapp_id,aad.device_allapp_detid,aad.team,aad.teamid,aad.dev_ci_code,aad.isdevicecode,";
				prosql += "aad.dev_name as dev_ci_name,aad.dev_type as dev_ci_model, ";
				...
				prosql += "where aad.device_allapp_id='"+device_allapp_id+"' and aad.project_info_no='<%=projectInfoNo%>' and aad.bsflag='0' and aad.device_addapp_id is null and aad.resourceflag is null ";
				prosql += "union all ";
				prosql += "select aad.device_addapp_id,aad.device_allapp_detid,aad.team,aad.teamid,aad.dev_ci_code,aad.isdevicecode,";
				prosql += "aad.dev_name as dev_ci_name,aad.dev_type as dev_ci_model,";
				prosql += "sd.coding_name as unit_name,teamsd.coding_name as teamname,p6.name as jobname, ";
				prosql += "aad.approve_num as require_num,aad.purpose,aad.plan_start_date,aad.plan_end_date,aad.unitinfo,";
				prosql += "nvl(tmp.applyed_num,0) as applyed_num,aad.project_info_no ";
				...
				prosql += ") order by device_addapp_id nulls last,teamid,team ";
		...
		for(var index=0;index<retObj.length;index++){
			...
			innerhtml += "<td width='11%'><input name='devicename"+index+"' id='devicename"+index+"' style='line-height:15px' value='"+retObj[index].dev_ci_name+"' size='11' type='text' readonly/></td>";
			innerhtml += "<td width='11%'><input name='devicetype"+index+"' id='devicetype"+index+"' value='"+retObj[index].dev_ci_model+"' size='11'  type='text' readonly/>";
			...
			innerhtml += "<td width='5%'><input name='neednum"+index+"' id='neednum"+index+"' value='"+retObj[index].require_num+"' size='6' type='text' /></td>";
			...
			innerhtml += "<td width='10%'><input name='purpose"+index+"' id='purpose"+index+"' value='"+retObj[index].purpose+"' size='10' type='text' readonly/></td>";
			innerhtml += "</tr>";
