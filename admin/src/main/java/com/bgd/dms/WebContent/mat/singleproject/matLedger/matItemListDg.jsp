      	...
      	<div id="list_table">
			...
			<div id="tab_box" class="tab_box">
				<div id="tab_box_content0" class="tab_box_content">
				    ...
				    <tr>
				     ...
				     <td  class="inquire_form6"><input id="wz_price" class="input_width_no_color" type="text"  value=""/> &nbsp;</td>  
				      <td  class="inquire_item6">&nbsp;入库数量：</td>
				...
				<div id="tab_box_content2" class="tab_box_content" style="display:none;">
				   <table id="projectMapTable" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height"  >
			    	<tr>
			    		<td class="bt_info_odd">序号</td>
			            <td  class="bt_info_even">入库数量</td>
			            <td class="bt_info_odd">入库单价</td>
			            <td  class="bt_info_even">金额</td>
			            <td class="bt_info_odd">收料库</td>
			            <td  class="bt_info_even">货位</td>
			            <td class="bt_info_odd">接受编号</td>
			            <td  class="bt_info_even">入库类别</td>
			            <td class="bt_info_odd">入库时间</td>
			        </tr>
			        </table>
				</div>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		if(codeId !=null && codeId != rootId){
			...
			sql +=")aa left join ( ";
			sql +="select tod.wz_id,sum(tod.mat_num) out_num from gms_mat_teammat_out mto inner join gms_mat_teammat_out_detail tod on mto.teammat_out_id=tod.teammat_out_id and tod.bsflag='0' where mto.bsflag='0' and (mto.oil_from != '1' or mto.oil_from is null)  and mto.project_info_no='<%=projectInfoNo%>' group by tod.wz_id ";
			...
			sql +=" and mti.project_info_no =  '<%=projectInfoNo%>' and mti.if_input = '0'  group by tid.wz_id) aa";
			sql +="  left join (select tod.wz_id, sum(tod.mat_num) out_num from gms_mat_teammat_out mto  inner join gms_mat_teammat_out_detail tod on mto.teammat_out_id = tod.teammat_out_id and tod.bsflag = '0' where mto.bsflag = '0' and mto.wz_type = '"+wz_type+"'  and (mto.oil_from != '1' or mto.oil_from is null) and mto.project_info_no = '<%=projectInfoNo%>' group by tod.wz_id) bb on aa.wz_id = bb.wz_id";
//			sql +="  left join ( select deta.wz_id,sum(deta.out_num) out_num from gms_mat_out_info info inner join gms_mat_out_info_detail deta on info.out_info_id=deta.out_info_id  where info.bsflag = '0' and info.project_info_no =  '<%=projectInfoNo%>' and info.out_type='3' and deta.bsflag='0' group by deta.wz_id )  cc on aa.wz_id = cc.wz_id";
//			sql +="  left join (  select deta.wz_id,sum(deta.out_num) out_num from gms_mat_out_info info inner join gms_mat_out_info_detail deta on info.out_info_id=deta.out_info_id  where info.bsflag = '0'  and info.project_info_no =  '<%=projectInfoNo%>'  and info.out_type='2' and deta.bsflag='5' group by deta.wz_id )  dd on aa.wz_id = dd.wz_id "; 
...
//			 retObj = jcdpCallService("MatItemSrv", "getSingleMatLedger", "laborId="+shuaId);
			 retObj = jcdpCallService("MatItemSrv", "getSingleRepMatLedgerDg", "laborId="+shuaId);
			 taskShow(shuaId);
			 projectMapTable(shuaId);
		}else{
		    ...
		    if(ids==''){ 
		     ...
		     retObj = jcdpCallService("MatItemSrv", "getSingleRepMatLedgerDg", "laborId="+ids);
		     taskShow(ids);
		     projectMapTable(shuaId);
		}
	...
	function toOut(){
       ...
       function simpleSearch(){
    	   sql ="select * from ("+sql;
			...
			if(wz_name !='' && wz_name != null || wz_id !='' && wz_id != null){
				if(wz_name !=''){
					sql += ") abc where abc.wz_name like'%"+wz_name+"%'";
					if(wz_id !='' && wz_id != null){
						sql += "and abc.wz_id like  '%"+wz_id+"%'";
						}
				}else{
					if(wz_id !='' && wz_id != null){
					sql += ") abc where abc.wz_id like  '%"+wz_id+"%'";
					}
					}
     ...
     //发放信息
  	 function taskShow(value){
		  	...
		  	//入库信息
		  	 function projectMapTable(value){
		  			for(var j =1;j <document.getElementById("projectMapTable")!=null && j < document.getElementById("projectMapTable").rows.length ;){
		  				document.getElementById("projectMapTable").deleteRow(j);
		  			}
		  			debugger;
		  			var retObj = jcdpCallService("MatItemSrv", "findTeammatDg", "project_info_no="+'<%=projectInfoNo%>'+"&ids="+value);
		  			var taskList = retObj.matInfo;
		  			for(var i =0; taskList!=null && i < taskList.length; i++){
		  				...
		  				var modifi_date      =  taskList[i].create_date;
		  				var receive_number   =  taskList[i].receive_number;
		  				var input_type       =  taskList[i].input_type;
		  				var autoOrder = document.getElementById("projectMapTable").rows.length;
		  				var newTR = document.getElementById("projectMapTable").insertRow(autoOrder);
		  				var tdClass = 'even';
		  				if(autoOrder%2==0){
		  					tdClass = 'odd';
		  				}
				      ...
				      td = newTR.insertCell(7);
				        ...
				        if(autoOrder%2==0){
		  		        ...
		  		        newTR.onclick = function(){
		  		        	// 取消之前高亮的行
		  		       		for(var i=1;i<document.getElementById("projectMapTable").rows.length;i++){
		  		    			var oldTr = document.getElementById("projectMapTable").rows[i];
		  		    			var cells = oldTr.cells;
		  		    			for(var j=0;j<cells.length;j++){
		  		    				cells[j].style.background="#96baf6";
		  		    				// 设置列样式
