      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{hse_medical_id}' id='rdo_entity_id_{hse_medical_id}' onclick='loadDataDetail();'/>" >选择</td>
			      ...
			      <td class="bt_info_odd" exp="<font color='{color}' style='font-weight:bold'>{certificate_num}</font>">证件编号</td>
			      <td class="bt_info_even" exp="<font color='{color}' style='font-weight:bold'>{certificate_org}</font>">发证机关</td>
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">
			    ...
			    <li id="tag3_1"><a href="#" onclick="getTab3(1)">医护信息</a></li>
			  </ul>
			</div>
			
			<div id="tab_box" class="tab_box">
			<input type="hidden" id="hse_medical_id" name="hse_medical_id"></input>
					  ...
					  <tr>
				    	<td class="inquire_item6"><font color="red">*</font>选择人员：</td>
				     	...
				     	<td class="inquire_item6"><font color="red">*</font>从事工种：</td>
				      	<td class="inquire_form6">
				    	<input type="text" id="work_type" name="work_type" class="input_width"></input>
				      	</td>
					  </tr>
					  <tr>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		cruConfig.queryStr = "select * from (select hm.hse_medical_id,hm.project_info_no,hm.employee_id,hm.work_type,hm.certificate_name,hm.certificate_num,hm.certificate_org,hm.bsflag,hm.modifi_date,hm.check_date-to_date(to_char(sysdate,'yyyy-MM-dd'),'yyyy-MM-dd') days,hm.check_date,case when months_between(hm.check_date,sysdate)<=0 then 'red'  when months_between(hm.check_date, sysdate)>2 then '' else 'orange' end color,case when ee.employee_id is null then la.owning_subjection_org_id else os.org_subjection_id end org_subjection_id, case when ee.employee_id is null then la.employee_name else ee.employee_name end employee_name,case when ee.employee_id is null then la.employee_id_code_no else ee.employee_id_code_no end code_id,case when ee.employee_id is null then decode(la.employee_gender,'0','女','1','男') else decode(ee.EMPLOYEE_GENDER, '0', '女', '1', '男') end sex_type,oi1.org_abbreviation second_org_name,oi2.org_abbreviation third_org_name,oi3.org_abbreviation fourth_org_name  from bgp_hse_medical hm left  join comm_human_employee ee on hm.employee_id=ee.employee_id and ee.bsflag='0' left join comm_org_subjection os1 on hm.second_org=os1.org_subjection_id and os1.bsflag='0' left join comm_org_information oi1 on os1.org_id = oi1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on hm.third_org=os2.org_subjection_id and os2.bsflag='0' left join comm_org_information oi2 on os2.org_id = oi2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on hm.fourth_org=os3.org_subjection_id and os3.bsflag='0' left join comm_org_information oi3 on os3.org_id = oi3.org_id and oi3.bsflag = '0'  left join comm_org_subjection os on os.org_id=ee.org_id and os.bsflag='0' left join bgp_comm_human_labor la on la.labor_id=hm.employee_id and la.bsflag='0' left join comm_coding_sort_detail sd2 on la.employee_education_level=sd2.coding_code_id and sd2.bsflag='0' where hm.bsflag='0') where bsflag='0' "+querySqlAdd+" order by modifi_date desc";
		cruConfig.currentPageUrl = "hse/humanResource/hseMedical/medical_list.jsp";
		queryData(1);
	}
	...
	function simpleSearch(){
		...
		if(name!=''&&name!=null){
			cruConfig.cdtType = 'form';
			cruConfig.queryStr = "select * from (select hm.hse_medical_id,hm.project_info_no,hm.employee_id,hm.work_type,hm.certificate_name,hm.certificate_num,hm.certificate_org,hm.bsflag,hm.modifi_date,hm.check_date-to_date(to_char(sysdate,'yyyy-MM-dd'),'yyyy-MM-dd') days,hm.check_date,case when months_between(hm.check_date,sysdate)<=0 then 'red'  when months_between(hm.check_date, sysdate)>2 then '' else 'orange' end color,case when ee.employee_id is null then la.owning_subjection_org_id else os.org_subjection_id end org_subjection_id, case when ee.employee_id is null then la.employee_name else ee.employee_name end employee_name,case when ee.employee_id is null then la.employee_id_code_no else ee.employee_id_code_no end code_id,case when ee.employee_id is null then decode(la.employee_gender,'0','女','1','男') else decode(ee.EMPLOYEE_GENDER, '0', '女', '1', '男') end sex_type,oi1.org_abbreviation second_org_name,oi2.org_abbreviation third_org_name,oi3.org_abbreviation fourth_org_name  from bgp_hse_medical hm left  join comm_human_employee ee on hm.employee_id=ee.employee_id and ee.bsflag='0' left join comm_org_subjection os1 on hm.second_org=os1.org_subjection_id and os1.bsflag='0' left join comm_org_information oi1 on os1.org_id = oi1.org_id and oi1.bsflag = '0' left join comm_org_subjection os2 on hm.third_org=os2.org_subjection_id and os2.bsflag='0' left join comm_org_information oi2 on os2.org_id = oi2.org_id and oi2.bsflag = '0' left join comm_org_subjection os3 on hm.fourth_org=os3.org_subjection_id and os3.bsflag='0' left join comm_org_information oi3 on os3.org_id = oi3.org_id and oi3.bsflag = '0'  left join comm_org_subjection os on os.org_id=ee.org_id and os.bsflag='0' left join bgp_comm_human_labor la on la.labor_id=hm.employee_id and la.bsflag='0' left join comm_coding_sort_detail sd2 on la.employee_education_level=sd2.coding_code_id and sd2.bsflag='0' where hm.bsflag='0') where bsflag='0' "+querySqlAdd+" and employee_name like '%"+name+"%' order by modifi_date desc";
			cruConfig.currentPageUrl = "hse/humanResource/hseMedical/medical_list.jsp";
			queryData(1);
		}else{
			refreshData();
	...
	function toUploadFile(){
		popWindow("<%=contextPath%>/hse/humanResource/hseMedical/humanImportFile.jsp?project=<%=project%>");
     }
	
	function loadDataDetail(shuaId){
		var retObj;
		if(shuaId!=null){
			 retObj = jcdpCallService("HseSrv", "queryMedical", "hse_medical_id="+shuaId);
		    ...
		    retObj = jcdpCallService("HseSrv", "queryMedical", "hse_medical_id="+ids);
		}
		document.getElementById("hse_medical_id").value =retObj.map.hseMedicalId;
	...
	function toAdd(){
		popWindow("<%=contextPath%>/hse/humanResource/hseMedical/addMedical.jsp?project=<%=project%>");
	...
	function toUpdate(){ 
		...
		var hse_medical_id = document.getElementById("hse_medical_id").value;
		var form = document.getElementById("form1");
		form.action="<%=contextPath%>/hse/resource/saveMedical.srq?project=<%=project%>&&hse_medical_id="+hse_medical_id;
		form.submit();
	} 
	...
	function toDelete(){
		...
		if(confirm('确定要删除吗?')){  
			var retObj = jcdpCallService("HseSrv", "deleteMedical", "hse_medical_id="+ids);
			queryData(cruConfig.currentPage);
		}
	}

	function toSearch(){
		popWindow("<%=contextPath%>/hse/humanResource/hseMedical/medical_search.jsp?project=<%=project%>");
	}
