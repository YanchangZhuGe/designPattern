...
<!-- 						    		<option value="C105006">装备服务处</option> -->
	...
	<script type="text/javascript">
		...
		function getAmountWhlFusionChart(){
			...
			chart1.setXMLUrl("<%=contextPath%>/dms/use/getAssetsIncomeData.srq");
		...
		function toQuery1(){
			...
			chartReference1.setXMLUrl("<%=contextPath%>/dms/use/getAssetsIncomeData.srq?orgSubId="+orgSubId+"&projectInfoNo="+projectInfoNo+"&year="+year);
		}
		//清除
		function toClear(){
			$('#org_sub_id_1') .val('');
			...
			var chartReference1 = FusionCharts("chart1"); 
			chartReference1.setXMLUrl("<%=contextPath%>/dms/use/getAssetsIncomeData.srq");
		}
		//弹出子层级资产创收
		function popNextLevelAI(){
			popWindow('<%=contextPath%>/dmsManager/use/assetsIncome/secondLevelAssetsIncome.jsp','800:572');
		}
		//选择年份
