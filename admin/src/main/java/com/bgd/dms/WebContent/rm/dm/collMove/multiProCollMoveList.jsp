			...
			<div id="tab_box" class="tab_box">
				<div id="tab_box_content0" name="tab_box_content0" class="tab_box_content">
					...
					<tr>
				      ...
				      <td  class="inquire_form6"  ><input id="s_out_project_name" class="input_width" type="text"  value="" disabled/> &nbsp;</td>
				     </tr>
				    <tr >
				     <td  class="inquire_item6">转入项目名称：</td>
				     <td  class="inquire_form6"><input id="s_in_project_name" class="input_width" type="text"  value="" disabled/> &nbsp;</td> 
...
<script type="text/javascript">
    ...
    function refreshData(v_out_project_name,v_in_project_name,v_move_status){
		...
		str += "where t.dev_type='wcoll' and t.bsflag='0' and t.org_subjection_id like '%"+suborgId+"%' ";		    
	...
	function toAddDevInfo(){
		...
		popWindow('<%=contextPath%>/rm/dm/collMove/multiCollMoveNewApply.jsp','950:680');
	...
	function toModifyMainPlanPage(){
		...
		//判断状态如果是已提交
		var str = "select devapp.move_status ";
		...
		if(unitRet.datas[0].move_status == null||unitRet.datas[0].move_status == ''||unitRet.datas[0].move_status == '0'){
			popWindow('<%=contextPath%>/rm/dm/collMove/multiCollMoveModefy.jsp?devMovId='+devMovId,'950:680');
		}else{
			alert("本单据已提交，不能修改!");
			return;
		}
	...
	function toDelPlanPage(){
		...
		var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		//什么状态不能删除，和业务专家确定
		if(unitRet.datas[0].move_status == null||unitRet.datas[0].move_status == ''||unitRet.datas[0].proc_status == '0'){
			if(confirm("是否执行删除操作?")){
	...
	function toSumbitDevApp(){
		...
		//判断状态如果是已提交
		var str = "select devapp.dev_mov_id,devapp.out_project_info_id,devapp.in_project_info_id,devapp.move_status ";
			str += "from gms_device_move devapp ";
			str += "where devapp.dev_mov_id ='"+devMovId+"' ";
		var unitRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
		//什么状态不能删除，和业务专家确定
		if(unitRet.datas[0].move_status == null || unitRet.datas[0].move_status == '' || unitRet.datas[0].move_status == '0' ){
			...
			var retObj = jcdpCallService("DevInsSrv", "saveMultiCollMoveInfowfpa", "dev_mov_id="+devMovId+"&in_project_no="+unitRet.datas[0].in_project_info_id+"&outProjectNo="+unitRet.datas[0].out_project_info_id);
