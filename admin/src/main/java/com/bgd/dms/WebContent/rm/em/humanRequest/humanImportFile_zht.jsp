					...
					for (int i = 3; i <= sheet.getLastRowNum(); i++) {   
						...
						for (int j = row.getFirstCellNum(); j < row.getLastCellNum(); j++) {
							...
							if(ss!=null && !"".equals(ss.toString())){
								...
								case 1:ss.setCellType(1); idCode = ss.getStringCellValue();  break;
								case 2:ss.setCellType(1); id = ss.getStringCellValue();  break;
								case 3:ss.setCellType(1); team = ss.getStringCellValue(); break;
								case 4:ss.setCellType(1); post = ss.getStringCellValue(); break;
								case 5:								
									if(ss.getCellType()==0){
						...
						if(  !"".equals(idCode)  || !"".equals(id)){ 
					
						if(name!=null && !"".equals(name) && id!=null && !"".equals(id)){ 
							if(idCode.equals("") ){
								System.out.println("idCode ="+idCode+id);
								 ...
								 String deploy_status="";
									...
									if(list.size()>0){  
										...
										deploy_status = (String)map.get("deployStatus");
										 ...
										 errors+="第"+(i+1)+"行正式工人员编号不存在,请正确输入;\\n";
									} 
								
							}else{
								errors+="第"+(i+1)+"行正式工姓名或人员编号为空;\\n";
							}	
							
						}
						
						if(name!=null && !"".equals(name) && idCode!=null && !"".equals(idCode)){
							if(id.equals("")  ){ 
								contents.append(idCode).append("@").append(name).append("@");
								if(team != null && !"".equals(team)){
									contents.append(team).append("@");
								}else{
									contents.append("").append("@");
								...
								if(endDate != null && !"".equals(endDate)){
								 ...
								 String sql = "  select lr.employee_name,lr.employee_id_code_no,lr.labor_id,  nvl(lr.spare1,0) deploy_status   from     bgp_comm_human_labor lr where lr.bsflag='0'  and lr.employee_id_code_no='"+idCode+"' ";
								 System.out.println("sql ="+sql);	
								 String deploy_status="";
									List list = BeanFactory.getQueryJdbcDAO().queryRecords(sql); 
									if(list.size()>0){  
										...
										deploy_status = (String)map.get("deployStatus");
										 ...
										 errors+="第"+(i+1)+"行临时工身份证号不存在,请正确输入;\\n";
									} 
								
							}else{
								errors+="第"+(i+1)+"行临时工姓名或身份证号为空;\\n";
							}	
							
						}
						
						}else{
							errors+="第"+(i+1)+"行人员编号或身份证号为空;\\n";
						}
