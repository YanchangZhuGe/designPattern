...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		...
		function getAmountWhlFusionChart(){
			...
			chart1.setXMLUrl("<%=contextPath%>/dms/use/getDeviceUseAndWhole.srq?orgSubId="+$("#org_sub_id_1").val());
		...
		function toQuery1(){
			...
			chartReference1.setXMLUrl("<%=contextPath%>/dms/use/getDeviceUseAndWhole.srq?orgSubId="+orgSubId+"&country="+country+"&startDate="+startDate+"&endDate="+endDate);
		}
		//清除
		function toClear(){
			$('#org_sub_id_1') .val('');
			...
			var chartReference1 = FusionCharts("chart1"); 
			chartReference1.setXMLUrl("<%=contextPath%>/dms/use/getDeviceUseAndWhole.srq");
		}
		//弹出子层级利用率
		function popNextLevelUseRate(level,devTreeId,orgSubId,country,startDate,endDate){
			popWindow('<%=contextPath%>/dmsManager/use/useAWh/secondDeviceUseAWh.jsp?level='+level+'&devTreeId='+devTreeId+'&orgSubId='+orgSubId+'&country='+country+'&startDate='+startDate+'&endDate='+endDate,'800:572');
		}
