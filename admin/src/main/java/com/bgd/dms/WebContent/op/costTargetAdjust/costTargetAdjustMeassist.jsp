      	...
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <td>&nbsp;</td>
				...
				<%if(proc_status){ %>
			   <auth:ListButton functionId="OP_ADJUST_MS_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			   <auth:ListButton functionId="OP_ADJUST_MS_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			   <auth:ListButton functionId="OP_ADJUST_MS_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			   <%} %>
			  </tr>
			...
			<div id="table_box">
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			            ...
			            <td  width="5%" class="bt_info_even">是否变更内容</td>
			            <td  class="bt_info_odd">变更日期</td>
			            <td   width="15%" class="bt_info_even" onclick="getSum()">金额</td>
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getMeassistAdjustInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
    		...
    		td.innerHTML ='<INPUT id="fy'+i+'checkbox" name="rdo_entity_id"  value='+datas[i].targetMeassistId+' type=checkbox>';
    		
    		var td = tr.insertCell();
			td.innerHTML = datas[i].rownum;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].meassistName;
			...
			var td = tr.insertCell();
			td.innerHTML = datas[i].meassistMoney;
			
			var td = tr.insertCell();
			td.innerHTML = datas[i].meassistContent;
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetAdjust/costTargetAdjustMeassistEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	if (ids == '') {
		...
		popWindow(cruConfig.contextPath+"/op/costTargetAdjust/costTargetAdjustMeassistEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	ids = ids.replace(/\,/g,"','");
	var sql = "update bgp_op_tartet_device_meassist t set t.if_delete_change='1' where t.target_meassist_id in('"+ids+"')";
...
function getSum(){
	...
	var querySql = "select sum(nvl(t.meassist_money,0)) sum_value"+
		" from bgp_op_tartet_device_meassist t where t.project_info_no='<%=projectInfoNo%>'"+
