...
<script type="text/javascript"> 
	function submitInfo(state){
		...
		//给调配单号设置成空
		$("#outinfo_no").val("");
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveOutFormDetailInfo.srq?state="+state+"&count="+count+"&line_infos="+line_infos+"&idinfos="+idinfos;
	...
	function refreshData(){
		...
		if('<%=mixInfoId%>'!=null){
			...
			var prosql = "select cms.device_backdet_id,accountdui.device_id,cms.dev_name,cms.dev_model,cms.back_num,accountdui.dev_team,cms.devremark as mixdevmark,";
				prosql += "nvl(temp.outednum,0) outednum,nvl(account.unuse_num, 0) unuse_num,account.dev_acc_id,account.dev_unit,unitsd.coding_name as unit_name ";
				...
				prosql += "left join gms_device_coll_account account on accountdui.fk_dev_acc_id=account.dev_acc_id and (account.bsflag is null or account.bsflag = '0') and account.ifcountry != '国外' ";
				prosql += "left join comm_coding_sort_detail unitsd on unitsd.coding_code_id=account.dev_unit ";
				prosql += "where cms.device_backapp_id='<%=mixInfoId%>'";
			...
			if(retObj!=undefined){
				for(var index=0;index<retObj.length;index++){
					...
					innerhtml += "<td width='4%'><input type='checkbox' name='detinfo' id='"+index+"' value='"+retObj[index].device_backdet_id+"' checked/></td>";
