...
<script type="text/javascript">
	...
	function toAdd() { 
		...
		popWindow("<%=contextPath%>/market/crm/information/contractEdit.jsp?company_id="+company_id);
	}
	function toEdit() {
		var company_id ='<%=company_id%>';
		if(company_id==null || company_id==''){
		...
		for(var i=1;i<obj.length;i++){
			if(obj[i].checked ==true){
				...
				if(contract_id!=null && window.confirm(text)){
					popWindow("<%=contextPath%>/market/crm/information/contractEdit.jsp?contract_id="+contract_id+"&company_id="+company_id);
					return;
				}
			}
		}
