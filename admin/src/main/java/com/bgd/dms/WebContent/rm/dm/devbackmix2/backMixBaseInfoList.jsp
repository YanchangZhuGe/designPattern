...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>多项目-调配调剂-调配单管理(项目返还)</title>
 </head> 
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_mixinfo_id_{device_mixinfo_id}' name='device_mixinfo_id'>
			     	<td class="bt_info_even" exp="<input type='checkbox' name='selectedbox' back_apptype='{backapptype}' mixstate='{state}' value='{device_backapp_id}' id='selectedbox_{device_backapp_id}' />" >选择</td>
					...
					<td class="bt_info_even" exp="{device_backapp_no}">调配申请单号</td>
					...
					<td class="bt_info_even" exp="<a onclick=viewMixedStore('{device_backapp_id}','{opr_state}','{backapptype}')>查看</a>">调配单查看</td>
...
<script type="text/javascript">
	...
	function refreshData(v_opr_state_desc, v_appinfo_name, v_project_name){
		var str = "select devapp.backapptype,devapp.project_info_id,tp.project_name,devapp.device_backapp_id,devapp.device_backapp_no,to_char(devapp.backdate,'yyyy-mm-dd') as appdate,"
			+"apporg.org_abbreviation as app_org_name,devapp.backapp_name,devapp.opr_state,case devapp.opr_state when '1' then '处理中' when '9' then '已处理' else '未处理' end as opr_state_desc "
			...
			//增加更换设备返还devapp.backapptype = '3'
			+"where devapp.state='9' and (devapp.backapptype = '1' or devapp.backapptype is null or devapp.backapptype = '3') and devapp.bsflag='0' and devapp.backdevtype='<%=DevConstants.MIXTYPE_COMMON%>' ";
			//加机构权限
	...
	function toAddDetailPage(){
		...
		if(shuaId!=''){
			if(backapptype!=''&&backapptype=='3'){
				popWindow('<%=contextPath%>/rm/dm/devbackmix2/backMixChanDevNew.jsp?devappid='+shuaId,'1000:680');
			}else{
				popWindow('<%=contextPath%>/rm/dm/devbackmix2/backMixInfoFormNew.jsp?devappid='+shuaId,'1000:680');
			}
		}
	}
    ...
    function viewMixedStore(id,state,type){
		if(state !='1' && state !='9'){
			alert("该单据尚未调配,暂不能查看调配单信息!");
		}else{
			if(id != ''){
				if(type=='3'){//更换设备
					popWindow('<%=contextPath%>/rm/dm/devbackmix2/chanMixedInfoList.jsp?devBackAppId='+id,'1050:680');
				}else{
