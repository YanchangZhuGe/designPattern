...
var proStatus= '<%=proStatus%>'
	function saveInfo(){
		<%-- popWindow('<%=contextPath%>/dmsManager/scrape/scrapeApply_addAssetSee.jsp?scrape_apply_id='+scrape_apply_id+'&proStatus='+proStatus,'840:650','评审通过明细查看'); --%>
		$("#scrape_apply_no").val("");
		...
		if(($("#excel_content_public").val()==undefined||$("#excel_content_public").val()=='')&&flag_public==0&&'<%=scrape_apply_id%>'=='null'){
			...
			document.getElementById("form1").action = "<%=contextPath%>/dmsManager/scrape/addScrapeListSee.srq";
