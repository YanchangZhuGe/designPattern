      	...
      	<div id="list_table">
			    ...
			    <td background="<%=contextPath%>/images/list_15.png">
			    	<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
					    ...
					    <td class="ali_cdn_input"><input id="materialName" name="materialName" type="text" class="input_width"/></td>
		 				<td class="ali_query">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr> 
			      <td class="bt_info_even" exp="<input type='checkbox' name='chx_entity_id' id='chx_entity_id_{check_no}' value='{check_no}' onclick='chooseOne(this);loadDataDetail();'/>" >选择</td>
			      <td class="bt_info_odd" autoOrder="1">序号</td> 
			      <td class="bt_info_even" exp="{check_unit_org}">检查单位</td>			      
			      <td class="bt_info_odd" exp="{check_roots_org}">检查基层单位</td>
			      <td class="bt_info_even" exp="{contact}">联系人</td>
			      <td class="bt_info_odd" exp="{check_start_time}">检查开始时间</td>
			      <td class="bt_info_even" exp="{check_end_time}">检查结束时间</td>
			      <td class="bt_info_odd" exp="{check_name}">检查名称</td>
			...
			<form name="form" id="form"  method="post" action="">
				...
				<div id="tab_box_content0" class="tab_box_content">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  ...
					  <tr>	
							<td class="inquire_item6"><font color="red">*</font>检查单位：</td>
						  	<td class="inquire_form6">				     
						...
						<td class="inquire_item6"><font color="red">*</font>检查基层单位：</td>
						 	<td class="inquire_form6">				     
						   ...
						   <td class="inquire_item6"><font color="red">*</font>检查部门/部位：</td>					 
						<td class="inquire_form6"> 
					    ...
					    <td class="inquire_form6"><input type="text" id="contact" name="contact"    class="input_width"/></td>
						</tr>						
					  ...
					  <tr>				
					    ...
					    <td class="inquire_item6"><font color="red">*</font>检查名称：</td>
					    <td class="inquire_form6"><input type="text" id="check_name" name="check_name" class="input_width" /></td>			
					  </tr>					
					  <tr>				
					 		 
					    <td class="inquire_item6"><font color="red">*</font>问题数量：</td>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height"  id="hseTableInfo2">			
				  ...
				  <tr>  
				    ...
				    <td class="inquire_item6"><font color="red">*</font>整改验证时间：</td>
				 ...
				 <tr>
				    <td class="inquire_item6"><font color="red">*</font>整改验证意见：</td>			 
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		var sql = " select tr.check_no,case when length(tr.check_unit_org)<= 6 then tr.check_unit_org else concat(substr(tr.check_unit_org,0,6),'...') end check_unit_org, decode(tr.check_type,'1','日常检查','2','专项检查','3','联系点到位','','其他') check_typename, decode(tr.leadership_led,'1','是','2','否') leadership_ledname ,case when length(tr.led_leadership)<= 6 then tr.led_leadership else concat(substr(tr.led_leadership,0,6),'...') end led_leadership, case when length(tr.check_roots_org)<= 6 then tr.check_roots_org else concat(substr(tr.check_roots_org,0,6),'...') end check_roots_org,tr.check_parts,tr.check_type,tr.if_contractor_team,tr.contact,tr.leadership_led, tr.check_members,tr.check_start_time,tr.check_end_time,tr.check_name,tr.number_problems,tr.verifier,tr.verifier_time,tr.verifier_opinion,  tr.second_org,tr.third_org,ion.org_name,tr.creator,tr.create_date,tr.bsflag, tr.modifi_date, tr.updator,oi1.org_abbreviation as second_org_name,oi2.org_abbreviation as third_org_name from BGP_HSE_CHECK tr join comm_org_subjection os1   on tr.second_org = os1.org_subjection_id  and os1.bsflag = '0'  join comm_org_information oi1  on oi1.org_id = os1.org_id  and oi1.bsflag = '0'  join comm_org_subjection os2  on tr.third_org = os2.org_subjection_id  and os2.bsflag = '0'  join comm_org_information oi2  on oi2.org_id = os2.org_id  and oi2.bsflag = '0'  join comm_org_information ion  on ion.org_id = os1.org_id  where tr.bsflag = '0' and tr.check_type ='3' ";
		var materialName = document.getElementById("materialName").value;
		sql = sql + " and tr.check_name like '%"+materialName+"%'";
		sql = sql + " order by  tr.modifi_date desc";
		cruConfig.queryStr = sql;
		cruConfig.currentPageUrl = "/hse/leaderAndPromise/hseCheck/hseCheckList.jsp";
		queryData(1);
	}
	...
	function loadDataDetail(shuaId){
		if(shuaId !=null){
			...
			querySql = "  select  tr.org_sub_id, cl.sumnum,tr.check_no,tr.check_unit_org,tr.check_roots_org, tr.check_parts,tr.check_type,tr.if_contractor_team,tr.contact,tr.leadership_led, tr.led_leadership,tr.check_members,tr.check_start_time,tr.check_end_time,tr.check_name,tr.number_problems,tr.verifier,tr.verifier_time,tr.verifier_opinion,  tr.second_org,tr.third_org,ion.org_name,tr.creator,tr.create_date,tr.bsflag, tr.modifi_date, tr.updator,oi1.org_abbreviation as second_org_name,oi2.org_abbreviation as third_org_name from BGP_HSE_CHECK tr   left join ( select   count(check_detail_no) sumnum ,check_no  from   BGP_HSE_CHECK_DETAIL where check_no='"+ shuaId +"' and bsflag='0' group by check_no) cl  on  cl.check_no=tr.check_no  join comm_org_subjection os1   on tr.second_org = os1.org_subjection_id  and os1.bsflag = '0'  join comm_org_information oi1  on oi1.org_id = os1.org_id  and oi1.bsflag = '0'  join comm_org_subjection os2  on tr.third_org = os2.org_subjection_id  and os2.bsflag = '0'  join comm_org_information oi2  on oi2.org_id = os2.org_id  and oi2.bsflag = '0'  join comm_org_information ion  on ion.org_id = os1.org_id  where tr.bsflag = '0' and tr.check_no='"+ shuaId +"'";				 	 
	...
	function toAdd(){
		popWindow("<%=contextPath%>/hse/hseOptionPage/hseCheck/addhseCheck.jsp");
	...
	function selectTeam1(){
	    ...
	    window.showModalDialog('/gms3/rm/em/humanLabor/selectProject.lpmd',teamInfo);
	    if(teamInfo.fkValue!=""){
	...
	function toEdit(){  
	    ...
	    if(ids==''){ alert("请先选中一条记录!");
	     return;
	  	...
	  	popWindow("<%=contextPath%>/hse/hseOptionPage/hseCheck/addhseCheck.jsp?check_no="+ids);
	...
	function toDelete(){
		...
		deleteEntities("update BGP_HSE_CHECK  e set e.bsflag='1' where e.check_no='"+ids+"'");
	 
	}

	function toSearch(){
		popWindow("<%=contextPath%>/hse/leaderAndPromise/hseContact/contact_search.jsp");
	}
	 function chooseOne(cb){   
	        var obj = document.getElementsByName("chx_entity_id");   
	        for (i=0; i<obj.length; i++){   
