...
<%
	...
	String riskConfirm = request.getParameter("riskConfirm")!=null?request.getParameter("riskConfirm"):"";
%> 
...
<div id="new_table_box">
  <div id="new_table_box_content">
    ...
    <tr>
    	...
    	<input type="hidden" name="risk_confirm" id="risk_confirm" class="input_width" value="<%=riskConfirm%>"/>
...
<script type="text/javascript">
	...
	if('<%=pageAction%>'!=""&&'<%=pageAction%>'!='null'){
		if('<%=pageAction%>'=='Add'){
			if(add_type == "flow"){
				document.getElementById("form1").action = "<%=contextPath%>/risk/addRiskIdentify.srq?addType=flow";
			}else{
				document.getElementById("form1").action = "<%=contextPath%>/risk/addRiskIdentify.srq";
			}		
		}
		if('<%=pageAction%>'=='Edit'){
			document.getElementById("form1").action = "<%=contextPath%>/risk/editRiskIdentify.srq";
		}
	}
