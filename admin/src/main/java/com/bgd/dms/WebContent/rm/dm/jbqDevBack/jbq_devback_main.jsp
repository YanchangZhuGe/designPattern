...
<script type="text/javascript">
    ...
    function refreshData(v_device_backapp_no, v_backapp_name, v_org_name){
		var str = "select colback.device_backapp_id,colback.device_backapp_no,colback.backapp_name,pro.project_name,"
			...
			str += "where colback.bsflag = '0' and colback.project_info_id = '"+projectInfoNos+"' and colback.backapptype='1' and colback.backdevtype='S14050208'";
	...
	function toAddBackPlanPage(){
		popWindow('<%=contextPath%>/rm/dm/jbqDevBack/jbq_backdetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>','950:680');
	}
	//修改
	function toModifyBackPlanPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/jbqDevBack/jbq_backdetail_new_apply.jsp?projectInfoNo=<%=projectInfoNo%>&devicebackappid='+devicebackappid,'950:680');
	}
	//删除
	...
	function toSumbitDevApp(){
		...
		//查询是否有明细，有则才能提交
		var str = "select pp.state,backdet.*, ";
		str += "account.dev_unit,account.total_num,account.unuse_num,account.use_num,account.actual_in_time ";
		str += "from gms_device_collbackapp_detail backdet ";
		str += "left join gms_device_coll_account_dui account on backdet.dev_acc_id = account.dev_acc_id ";
		str += "left join gms_device_collbackapp pp on pp.device_backapp_id = backdet.device_backapp_id ";
		...
		if(confirm("是否提交返还单？")){
			...
			jcdpCallService("DevCommInfoSrv","saveCollectSubmitInfo","devicebackappid="+selectedid);
			refreshData();
		}
