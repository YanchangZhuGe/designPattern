      	...
      	<div id="list_table">
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">
			    ...
			    <li id="tag3_10" ><a href="#" onclick="getContentTab(this,10);loaddata('',10)">设备检查</a></li>
					...
					<div style="overflow:auto">
				      	<table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
						  	<tr align="right">
						  		...
						  		<td class="ali_cdn_input" ></td>
						  		<td>&nbsp;</td>
						    	<auth:ListButton functionId="" css="zj" event="onclick='toAddDJ()'" title="JCDP_btn_add"></auth:ListButton>
								<auth:ListButton functionId="" css="xg" event="onclick='toEditDJ()'" title="JCDP_btn_edit"></auth:ListButton>
							</tr>
						  </table>
					  </div>
	...
	*/
	...
	function drdj(shuaId){
		if (shuaId != null) {
			...
			querySql+="org.org_abbreviation as owning_org_name,b.project_info_id,c.PROJECT_NAME,a.* ";
			querySql+="from GMS_DEVICE_EQUIPMENT_OPERATOR a left join GMS_DEVICE_ACCOUNT_DUI b on a.DEVICE_ACCOUNT_ID=b.dev_acc_id ";
			querySql+="left join gp_task_project c on b.PROJECT_INFO_ID=c.project_info_no  left join gp_task_project_dynamic dyn on c.project_info_no=dyn.project_info_no left join  comm_org_information org on dyn.org_id = org.org_id  left join comm_human_employee d on a.OPERATOR_ID=d.employee_id where DEVICE_ACCOUNT_ID='"+shuaId+"'";
			var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=100000&querySql='+querySql);
			retObj = queryRet.datas;
	 ...
	 function toAddDJ(){
		...
		var ids = getSelIds('rdo_entity_id');
	 	...
	 	popWindow("<%=contextPath%>/rm/dm/device-xd/dj.jsp?ids="+ids);  
