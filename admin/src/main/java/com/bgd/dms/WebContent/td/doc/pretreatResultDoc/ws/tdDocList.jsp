			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      ...
			      <td class="bt_info_even" exp="<a style='color:blue;' href=<%=contextPath%>/doc/downloadDocByUcmId.srq?docId={ucm_id}&emflag=0>{file_name}</a>">现场预处理成果</td>
			     </tr> 			        
			  </table>
			</div>
...
<script type="text/javascript">
	...
	function refreshData(str){
		cruConfig.queryStr = "select t.*from (select t.project_info_no,t.tecnical_id,t.title,t.written_time,t.writer,f.ucm_id as ucm_id,f.file_name as file_name from gp_ws_tecnical_basic t left join bgp_doc_gms_file f on t.tecnical_id=f.relation_id and f.doc_file_type='0110000061100000023' and f.bsflag='0' where t.bsflag = '0' and t.project_info_no = '<%=projectInfoNo%>' and t.business_type = '<%=businessType%>' "+str+" order by t.modifi_date desc) t";
		queryData(1);
	}
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/doc/pretreatResultDoc/ws/add_tdDocModify.jsp?businessType='+businessType+'&fileAbbr='+fileAbbr,'800:680');
    }
	...
	function toEdit() {
		...
		popWindow('<%=contextPath%>/td/doc/pretreatResultDoc/ws/add_tdDocModify.jsp?businessType='+businessType+'&id='+ids,'800:680');
	}
