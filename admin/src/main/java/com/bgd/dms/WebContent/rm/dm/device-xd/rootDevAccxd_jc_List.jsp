...
<script type="text/javascript"><!--
	...
	//打开新增界面
	 function toAdd(){   
		var ids = getSelIds('rdo_entity_id');
	 	...
	 	popWindow("<%=contextPath%>/rm/dm/device-xd/jc.jsp?ids="+ids); 
    ...
    //修改界面
     function toEdit(obj){  
    	...
    	popWindow("<%=contextPath%>/rm/dm/device-xd/jc.jsp?ids="+ids+"&inspection_id="+obj.id); 
	  } 
	  //选择一条记录
    ...
    function loadDataDetail(obj){
		...
		if(shuaId!=null){
			...
			querySql+="from BGP_COMM_DEVICE_INSPECTION a left join GMS_DEVICE_ACCOUNT_DUI b on a.DEVICE_ACCOUNT_ID=b.dev_acc_id ";
			querySql+="left join gp_task_project c on b.PROJECT_INFO_ID=c.project_info_no where DEVICE_ACCOUNT_ID='"+shuaId+"' order by a.inspection_date desc";
			...
			for(var i=0;i<retObj.length;i++){
				...
				newTr.id=retObj[i].inspection_id;
