...
function save(){
	...
	form.action = "<%=contextPath%>/td/wt/toSaveMeasureResultDoc.srq?businessType=<%=businessType%>&isSingle="+isSingle;
	form.submit();
	newClose();
}
...
function checkForm(){
	...
	if(""==document.getElementById("pk_0110000061000000030").value){
		if (!isTextPropertyNotNull("0110000061000000030", "测量技术报告")) return false;	
	}
	if (!isLimitB20("writer","编写人")) return false; 
	if (!isLimitB20("auditor","审核人")) return false;
	if (!isLimitB100("observation_accuracy","观测精度指标")) return false;  
	if (!isLimitB100("point_offset","点位偏移统计")) return false;  
	return true;
}
...
function page_init(){
	...
	if(fileId!='null'){
		var querySql = " select * from(select b.project_info_no,p.project_name,t.file_id,t.ucm_id,t.file_abbr,b.business_type,t.doc_file_type,t.file_name,b.tecnical_id,b.title,b.written_time,b.writer,b.auditor,b.observation_accuracy,b.point_offset from gp_ws_tecnical_basic b left join gp_task_project p on b.project_info_no=p.project_info_no and p.bsflag='0' left join bgp_doc_gms_file t on  b.tecnical_id = t.relation_id and t.bsflag='0')m where m.tecnical_id ='"+fileId+"'";
		...
		if(datas!=null && datas.length>0){
			...
			document.getElementById("observation_accuracy").value = datas[0].observation_accuracy;
			document.getElementById("point_offset").value = datas[0].point_offset;
			if(null!=document.getElementById("projectName")){
						...
						<div id="tab_box_content0" class="tab_box_content">
							<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
								...
								<tr>
									...
									<td class="inquire_item4">观测精度指标：</td>
									<td class="inquire_form4"><input type="text" id="observation_accuracy" name="observation_accuracy" value="" class="input_width" /></td>
								</tr>
								<tr>
									<td class="inquire_item4">点位偏移统计：</td>
									<td class="inquire_form4"><input type="text" id="point_offset" name="point_offset" value="" class="input_width" /></td>
									<td class="inquire_item4"></td>
									<td class="inquire_form4"></td>
								</tr>
								<tr>
									<td class="inquire_item4"><span class="red_star">*</span>测量技术报告：</td>
									<td colspan="2">
										<input type="hidden" id="pk_0110000061000000030" name="pk_0110000061000000030" value=""/>
										<input type="file" name="0110000061000000030" id="0110000061000000030" class="input_width"/>
					    				<div id="down_0110000061000000030"></div>
									</td>
									<td class="inquire_form4"></td>
								</tr>
								<tr>
