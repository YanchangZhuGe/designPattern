...
function page_init(){
	...
	if(projectInfoNo!='null'){
		var querySql = "select t.coding_name,c.sum_human_cost sum_cost, s1.sum_human_cost sum_cost_act, s.sum_human_cost,s.cont_cost,s.mark_cost,s.temp_cost,s.reem_cost,s.serv_cost from comm_human_coding_sort t   left join (select s.project_info_no, s.subject_id, sum(s.sum_human_cost) sum_human_cost, sum(s.cont_cost) cont_cost, sum(s.mark_cost) mark_cost, sum(s.temp_cost) temp_cost, sum(s.reem_cost) reem_cost, sum(s.serv_cost) serv_cost from bgp_comm_human_cost_act_deta s where s.project_info_no = '"+projectInfoNo+"' and s.org_subjection_id <%=orgSql%> and to_char(s.app_date, 'yyyy-MM') <= '"+appDate+"'  group by s.project_info_no, s.subject_id) s1 on t.coding_code_id =  s1.subject_id  left join bgp_comm_human_cost_act_deta s on t.coding_code_id = s.subject_id and s.project_info_no='"+projectInfoNo+"' and s.org_subjection_id <%=orgSql%> and to_char(s.app_date,'yyyy-MM')='"+appDate+"' left join bgp_comm_human_cost_plan_sala c on t.coding_code_id=c.subject_id  and c.project_info_no='"+projectInfoNo+"' and c.org_subjection_id <%=orgSql%> and c.bsflag='0' left join bgp_comm_human_plan_cost c1 on c1.plan_id=c.plan_id  where t.coding_sort_id = '0000000002' and t.bsflag = '0' and c1.cost_state = '"+costState+"' order by t.coding_code, t.coding_show_order";
