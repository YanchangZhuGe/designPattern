...
<div id="tab_box_content2" class="tab_box_content"
	style="display: none;">
<table id="projectApp" width="100%" border="0" cellspacing="0"
	cellpadding="0" class="tab_line_height">
	<tr>
		<td class="bt_info_odd">序号</td>
...
var showTabBox = document.getElementById("tab_box_content0");
	...
	function refreshData(){
		var sql ='';
			sql +="select t.compile_date,u.user_name ,t.plan_invoice_id,t.submite_number, decode(t.submite_if,'1','待审批','2','审批未通过','3','审批通过','4','调剂未通过','5','调剂通过') submite_if from gms_mat_demand_plan_invoice t inner join p_auth_user u on t.creator_id=u.user_id and u.bsflag='0'  where t.bsflag = '0' and t.project_info_no='<%=projectInfoNo%>' and t.submite_if>0 order by t.compile_date desc";
			
		cruConfig.cdtType = 'form';
		cruConfig.queryStr = sql;
		cruConfig.currentPageUrl = "/mat/singleproject/plansub/planAppList.jsp";
		queryData(1);
	}
	...
	//审批信息
	 function applyShow(value){
			for(var j =1;j <document.getElementById("projectApp")!=null && j < document.getElementById("projectApp").rows.length ;){
				document.getElementById("projectApp").deleteRow(j);
			}
			var retObj = jcdpCallService("MatItemSrv", "getappDetaile", "ids="+value);
			var taskList = retObj.matInfo;
			for(var i =0; taskList!=null && i < taskList.length; i++){
				var submiteIf = taskList[i].submiteIf;
				var modifiDate = taskList[i].modifiDate;
				var userName =taskList[i].userName
				var remark = taskList[i].remark;
				var autoOrder = document.getElementById("projectApp").rows.length;
				var newTR = document.getElementById("projectApp").insertRow(autoOrder);
				var tdClass = 'even';
				if(autoOrder%2==0){
					tdClass = 'odd';
				}
		        ...
		        newTR.onclick = function(){
		        	// 取消之前高亮的行
		       		for(var i=1;i<document.getElementById("projectApp").rows.length;i++){
		    			var oldTr = document.getElementById("projectApp").rows[i];
		    			var cells = oldTr.cells;
		    			for(var j=0;j<cells.length;j++){
		    				cells[j].style.background="#96baf6";
		    				// 设置列样式
	 ...
	 function chooseOne(cb){   
	    	 ...
	    	 ids = getSelIds('rdo_entity_id');
		 	    if(ids==''){ alert("请先选中一条记录!");
		 	     	return;
		 	   		...
		 	   		popWindow('<%=contextPath%>/mat/singleproject/plansub/planAppEdit.jsp?planInvoiceId='+ids,'1024:800');
		 	    }
		    }   
	    //查询函数
	    function  testQuery(selectThis){
			//alert(selectThis.options[selectThis.selectedIndex].text);
	 		var sql = "";
			if(selectThis.options[selectThis.selectedIndex].value=='4'){
				sql +="select t.compile_date,u.user_name ,t.plan_invoice_id,t.submite_number, decode(t.submite_if,'1','待审批','2','审批未通过','3','审批通过','4','调剂未通过','5','调剂通过') submite_if from gms_mat_demand_plan_invoice t inner join p_auth_user u on t.creator_id=u.user_id and u.bsflag='0'  where t.bsflag = '0' and t.project_info_no='<%=projectInfoNo%>' and t.submite_if>0 order by t.compile_date desc";
			}else{
			 
				sql += "select t.compile_date,u.user_name ,t.plan_invoice_id,t.submite_number, decode(t.submite_if,'1','待审批','2','审批未通过','3','审批通过','4','调剂未通过','5','调剂通过') submite_if from gms_mat_demand_plan_invoice t inner join p_auth_user u on t.creator_id=u.user_id and u.bsflag='0'  where t.bsflag = '0' and t.project_info_no='<%=projectInfoNo%>' and t.submite_if>0 and t.submite_if='"+selectThis.options[selectThis.selectedIndex].value+"' order by t.compile_date desc";
