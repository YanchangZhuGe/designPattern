      ...
      <table id="table1" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
         <tr>
				...
				<!-- <td class="inquire_form6">
					...
					<td class="inquire_form6"><input id="account_stat_desc" name="account_stat_desc" class="input_width" type="text" readonly/>
					<input id="account_stat" name="account_stat" class="" type="hidden" /></td>
				<!-- </td> -->
			  ...
			  <tr>
				...
				<td class="inquire_form6">
					<input id="owning_org_name" name="owning_org_name" class="input_width" type="text" readonly/>
...
<script type="text/javascript"> 
	...
	//	}
		...
		//
		var querySql="select (select coding_name from comm_coding_sort_detail c where t.using_stat=c.coding_code_id) as using_stat_desc, (select coding_name from comm_coding_sort_detail c ";
			querySql += "where t.tech_stat=c.coding_code_id) as tech_stat_desc,";
			querySql += "t.*,(select org_abbreviation from comm_org_information org where t.owning_org_id=org.org_id) as owning_org_name_desc,";
			querySql += "(select coding_name from comm_coding_sort_detail co where co.coding_code_id=t.account_stat) as account_stat_desc from GMS_DEVICE_ACCOUNT t where t.dev_acc_id='"+'<%=devaccid%>'+"'";
		var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
		datas = queryRet.datas;
		...
		document.getElementById("account_stat_desc").value=datas[0].account_stat_desc;
		...
		//document.getElementById("account_stat").options.add(datas[0].coding_code_id)); 
		selectChange();//关联技术状况
		//技术状况默认完好
