...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  <title>多项目-调配调剂-调配单管理(自有设备)-调配单查看子页面</title> 
 </head> 
...
<script type="text/javascript">
	...
	function refreshData(v_mixinfo_no){
		...
		if(orgType=="Y"){//原综合物化探代码
			str += "select device_app_name,dev1.device_app_id,device_app_no,to_char(appdate, 'yyyy-mm-dd') as appdate,"
				  ...
				  +"where wfmiddle.proc_status = '3' and dev1.bsflag = '0' and dev1.mix_type_id = 'S0000' "
				...
				str += "select * from (select tp.project_name,mif.device_app_id, mif.device_mixinfo_id,mif.mixinfo_no,mixorg.org_abbreviation as mix_org_name,"
				+"outorg.org_abbreviation as out_org_name,he.employee_name,to_char(mif.modifi_date, 'yyyy-mm-dd') as mix_date,mif.state "
				+"from gms_device_mixinfo_form mif left join gms_device_app devapp on mif.device_app_id = devapp.device_app_id "
				...
				+"left join comm_human_employee he on mif.print_emp_id = he.employee_id left join(comm_org_subjection sub left join comm_org_information outorg "
				+"on sub.org_id = outorg.org_id) on mif.out_org_id = sub.org_subjection_id "
				+"left join comm_org_subjection orgsub on devapp.mix_org_id = orgsub.org_id and orgsub.bsflag = '0' " 
				+" where wfmiddle.proc_status = '3' and devapp.bsflag = '0' and (devapp.mix_type_id = 'S0000' or devapp.mix_type_id = 'S10002') " 
				+"and (mif.bsflag is null or mif.bsflag = '0')  "
				+"and devapp.device_app_id = '<%=devappid%>' "
				+"union all "
				+"select tp.project_name,mif.device_app_id, mif.device_mixinfo_id,mif.mixinfo_no,mixorg.org_abbreviation as mix_org_name,"
				+"outorg.org_abbreviation as out_org_name,he.employee_name,to_char(mif.modifi_date, 'yyyy-mm-dd') as mix_date,mif.state "
				+"from gms_device_mixinfo_form mif left join gms_device_dismixapp p on p.device_mixapp_id = mif.device_app_id left join gms_device_app devapp on p.device_app_id = devapp.device_app_id "
				+"left join common_busi_wf_middle wfmiddle on devapp.device_app_id = wfmiddle.business_id "
				+"left join comm_org_information mixorg on devapp.mix_org_id = mixorg.org_id "
				+"left join gp_task_project tp on devapp.project_info_no = tp.project_info_no "
				+"left join comm_human_employee he on mif.print_emp_id = he.employee_id left join(comm_org_subjection sub left join comm_org_information outorg "
				+"on sub.org_id = outorg.org_id) on mif.out_org_id = sub.org_subjection_id "
				+"left join comm_org_subjection orgsub on devapp.mix_org_id = orgsub.org_id and orgsub.bsflag = '0' " 
				+" where wfmiddle.proc_status = '3' and devapp.bsflag = '0' and devapp.mix_type_id = 'S0000' " 
				+"and (mif.bsflag is null or mif.bsflag = '0')  "
				+"and devapp.device_app_id = '<%=devappid%>' ) where 1=1 ";
	...
	//	}
		//var info = shuaId.split("~" , -1);
		if(dev_appid !=''){
			popWindow('<%=contextPath%>/rm/dm/devmixForm/devMixInfoFormNew.jsp?devappid='+dev_appid,'1050:680');
		}
	}
...
//		}else{
//			popWindow('<%=contextPath%>/rm/dm/devmixForm/devMixInfoFormNew.jsp?devappid='+info[1],'1050:680');
	//	}
//	}
    ...
    function loadDataDetail(shuaId){
		...
		if(info[0]!=''){
			...
			+"union all "
			+"select mif.device_app_id,devapp.device_app_no,to_char(devapp.appdate,'yyyy-mm-dd') as appdate,apporg.org_abbreviation as app_org_name,"
			+"mif.device_mixinfo_id,mif.mixinfo_no,tp.project_name,mixorg.org_abbreviation as mix_org_name,"
			+"outorg.org_abbreviation as out_org_name,he.employee_name,to_char(mif.modifi_date,'yyyy-mm-dd') as mix_date,"
			+"case mif.state when '0' then '调配中' when '9' then '已调配' else '待调配' end as mix_state_desc,mif.state "
			+"from gms_device_mixinfo_form mif "
			+"left join gms_device_dismixapp p on p.device_mixapp_id = mif.device_app_id "
			+"left join gms_device_app devapp on p.device_app_id = devapp.device_app_id "
			+"left join common_busi_wf_middle wfmiddle on devapp.device_app_id=wfmiddle.business_id "
			+"left join gp_task_project tp on devapp.project_info_no=tp.project_info_no " 
			+"left join comm_human_employee he on mif.print_emp_id=he.employee_id "
			+"left join comm_org_information apporg on devapp.app_org_id=apporg.org_id "
