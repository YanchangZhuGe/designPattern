			...
			<form action="" id="form0" name="form0" method="post" target="record">
				<table width="100%"  border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
				    ...
				    <tr>
				    	...
				    	<td class="inquire_form6"><input name="report_date" id="report_date" type="text" class="input_width" value="" readonly="readonly" />
...
<script type="text/javascript">
	...
	function refreshData(project_name , org_subjection_id){		
		//if(subjection_id!=null && subjection_id=='C105'){
			if(org_subjection_id!=null && org_subjection_id=='C105'){
				cruConfig.queryStr = "select t.accident_id , t.project_info_no ,p.project_name , "+
				" t.org_id ,o.org_abbreviation org_name , t.report_id , e.employee_name , t.report_date ,"+
				" (t.small_num-(-t.large_num)-(-t.great_num)-(-t.super_num)) accident_num"+
				" from bgp_qua_accident t "+
				" join bgp_comm_org_wtc wtc on t.org_subjection_id = wtc.org_subjection_id "+
				" left join gp_task_project p on t.project_info_no = p.project_info_no and p.bsflag ='0'"+
				" left join comm_human_employee e on t.report_id = e.employee_id and e.bsflag ='0' "+
				" left join comm_org_information o on t.org_id = o.org_id and o.bsflag ='0' "+
				" where t.bsflag = '0' order by t.report_date desc";
			}else if(0<=org_subjection_id.indexOf("C105")){
				var strsql = "select t.accident_id , t.project_info_no ,p.project_name , "+
				" t.org_id ,o.org_abbreviation org_name , t.report_id , e.employee_name , t.report_date ,"+
				" (t.small_num-(-t.large_num)-(-t.great_num)-(-t.super_num)) accident_num"+
				" from bgp_qua_accident t "+
				" left join gp_task_project p on t.project_info_no = p.project_info_no and p.bsflag ='0'"+
				...
				if(0<=org_subjection_id.indexOf(":")){
					var org_subjection_ids=org_subjection_id;
					var org_subjection_idss = org_subjection_ids.split(":");
					strsql=strsql+" where t.bsflag = '0' and t.org_subjection_id like '"+org_subjection_idss[1]+"%'  and p.PROJECT_TYPE='"+org_subjection_idss[0]+"'";
				}else{						
					strsql=strsql+" where t.bsflag = '0' and t.org_subjection_id like '"+org_subjection_id+"%'";

				}
				cruConfig.queryStr=strsql+" order by t.report_date desc";
			}else{
				cruConfig.queryStr=" select t.accident_id , t.project_info_no ,p.project_name , "
				+" t.org_id ,o.org_abbreviation org_name , t.report_id , e.employee_name , t.report_date ,"
				+" (t.small_num-(-t.large_num)-(-t.great_num)-(-t.super_num)) accident_num"
				...
				+" left join comm_human_employee e on t.report_id = e.employee_id and e.bsflag ='0' "
				+" left join comm_org_information o on t.org_id = o.org_id and o.bsflag ='0' "
				+" where t.bsflag = '0' and (p.PROJECT_FATHER_NO='"+org_subjection_id+"' or p.project_info_no='"+org_subjection_id+"') " 
		...
		//}else{
			...
			" left join comm_human_employee e on t.report_id = e.employee_id and e.bsflag ='0' "+
			" left join comm_org_information o on t.org_id = o.org_id and o.bsflag ='0' "+
			" where t.bsflag = '0' and t.org_subjection_id like '"+org_subjection_id+"%'  "+
			" and t.org_subjection_id not in (select org_subjection_id from bgp_comm_org_wtc)"+
			" order by t.report_date desc";
			*/
		//}
		queryData(1);
	...
	function loadDataDetail(id){
		...
		document.getElementById("report").src = "<%=contextPath%>/qua/mProject/accident/reportList.jsp?accident_id="+id;
		...
		document.getElementById("project_name").value = retObj.accidentDetail.projectName;
		setTabBoxHeight();
		resizeNewTitleTable();
	}
	/* 修改 */
	function newSubmit() {
		...
		form.submit();
		var project_id = document.getElementById("project_info_no").value ;
		var project_name = document.getElementById("project_name").value ;
		refreshData(project_id , project_name , org_id);
	}
	function checkValue(){
		...
		for (var i = 1;i< objLen-1 ;i++){   
		    if (obj [i].checked==true) { 
				...
				if(window.confirm(text)){
					...
					popWindow("<%=contextPath%>/qua/mProject/accident/accidentView.jsp?projectName="+projectName+"&project_info_no="+project_info_no+"&accident_id="+accident_id);
					return;
				}
		  	}   
		} 
	...
	function toAdd() { 
		...
		var project_info_no = document.getElementById("project_info_no").value;
		var projectName = document.getElementById("project_name").value;
		popWindow("<%=contextPath%>/qua/sProject/accident/accidentEdit.jsp?projectName="+projectName+"&project_info_no="+project_info_no);
	}
	function toEdit() {
		var obj = document.getElementsByName("chk_entity_id");
		var objLen= obj.length; 
		...
		for (var i = 1;i< objLen-1 ;i++){   
		    if (obj [i].checked==true) { 
				...
				if(window.confirm(text)){
					var project_info_no = document.getElementById("project_info_no").value;
					var projectName = document.getElementById("project_name").value;
					popWindow("<%=contextPath%>/qua/sProject/accident/accidentEdit.jsp?projectName="+projectName+"&project_info_no="+project_info_no+"&accident_id="+accident_id);
					return;
				}
		  	}   
		} 
