      	...
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <input id="ifBuild" name="ifBuild" class="input_width"  style="width:120px" type="hidden" readonly="readonly"/>&nbsp;			  
			    </td>
 				<td class="ali_query">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr> 
			      <td class="bt_info_even" exp="<input type='checkbox' name='chx_entity_id' id='chx_entity_id_{recore_id}'  value='{recore_id},{org_sub_id},{month_no},{subflag},{org_name},{unit_subflag},{spare1}' onclick=doCheck(this)/>" >选择</td>
			      <td class="bt_info_even" autoOrder="1">序号</td> 
			      <td class="bt_info_odd" exp="{month_no}">月份日期</td> 
			      <td class="bt_info_even" exp="{unit_subflag}">状态</td>
			      <td class="bt_info_even" exp="{spare2}">填写人</td>
			    </tr>
			  </table>
			</div>
...
<script type="text/javascript"> 
	...
	function refreshData(){		
		...
		cruConfig.queryStr = "select *　from (select   row_number() over(partition by tr.month_no order by tr.month_no ,spare1 ) num , i.org_abbreviation as org_name ,tr.spare2, u.employee_name,tr.recore_id,tr.org_sub_id,tr.spare1,tr.month_no,tr.month_start_date,tr.month_end_date,decode(tr.unit_subflag, '0', '未提交', '1', '已提交','2','审批通过','5','审批不通过') unit_subflag ,tr.unit_subflag as subflag  from   BGP_HSE_MONTH_RECORD tr  left join comm_org_subjection s    on tr.org_sub_id = s.org_subjection_id   and s.bsflag = '0'  left join comm_org_information i    on s.org_id = i.org_id   and i.bsflag = '0'     left    join  comm_human_employee u   on tr.creator = u.EMPLOYEE_ID   and u.bsflag = '0'   where tr.bsflag='0' and tr.subflag='3' )  where num = 1";	 
	...
	function simpleSearch(){
			var ifBuild = document.getElementById("ifBuild").value;
				if(ifBuild!=''&&ifBuild!=null){
					cruConfig.cdtType = 'form';
					cruConfig.queryStr = "select *　from (select   row_number() over(partition by tr.month_no order by tr.month_no ,spare1 ) num , i.org_abbreviation as org_name ,tr.spare2, u.employee_name,tr.recore_id,tr.org_sub_id,tr.spare1,tr.month_no,tr.month_start_date,tr.month_end_date,decode(tr.unit_subflag, '0', '未提交', '1', '已提交','2','审批通过','5','审批不通过') unit_subflag ,tr.unit_subflag as subflag  from   BGP_HSE_MONTH_RECORD tr   join comm_org_subjection s    on tr.org_sub_id = s.org_subjection_id   and s.bsflag = '0'  join comm_org_information i    on s.org_id = i.org_id   and i.bsflag = '0'        join  comm_human_employee u   on tr.creator = u.EMPLOYEE_ID   and u.bsflag = '0'   where tr.bsflag='0' and tr.subflag='3'  and tr.month_no='"+ifBuild+"' )  where num = 1";	 
					cruConfig.currentPageUrl = "/hse/hseOptionPage/hseMonthlyReport/unitMonthlyReportList.jsp";
					queryData(1);
				}else{
					alert('请输入查询信息');
	...
	function dbclickRow(ids){
		 var temp = ids.split(','); 
            ...
            var spare1=temp[6];
            	...
            	window.location='<%=contextPath%>/hse/hseOptionPage/hseMonthlyReport/pageLink/monthlyMenu.jsp?record_id='+record_id+'&org_sub_id='+org_sub_id+'&month_no='+month_no+'&org_name='+org_name+'&subflag='+subflag+'&action=edit';
	...
	function toAdd(){
		window.location='<%=contextPath%>/hse/hseOptionPage/hseMonthlyReport/selectDate.jsp?action=add';
	}
		  ...
		  ids = getSelIds('chx_entity_id');
			...
			if(subflag == '已提交'){
				...
				var path = '<%=request.getContextPath()%>/rad/addOrUpdateEntity.srq';	       	
   				var submitStr = 'JCDP_TABLE_NAME=BGP_HSE_MONTH_RECORD&JCDP_TABLE_ID='+record_id +'&unit_subflag='+subflags;
	...
	function toSearch(){
		popWindow("<%=contextPath%>/hse/hseOptionPage/hseMonthlyReport/hse_searchA.jsp");
	}
	 function chooseOne(cb){   
	        var obj = document.getElementsByName("chx_entity_id");   
	        for (i=0; i<obj.length; i++){   
