...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <input type="hidden" name="export_name"  id="export_name" value="设备运转记录"/>
      	<div id="list_table">
...
$(function(){
	...
	function refreshData(){
		var str = "select * from ( select dui.dev_name,dui.dev_model,dui.self_num,dui.license_num,dui.dev_coding,dui.dev_sign, p.project_name,";
			...
			str += "inner join gms_device_account_dui dui on dui.dev_acc_id=t.dev_acc_id left join gms_device_account account on account.dev_acc_id=dui.fk_dev_acc_id left join gp_task_project p on p.project_info_no=dui.project_info_id ";
			str += "where account.dev_acc_id='"+devaccid+"' and  to_char(t.modify_date,'yyyy')='"+year+"' ";
			str += "union all ";
			str += "select account.dev_name,account.dev_model,account.self_num,account.license_num,account.dev_coding,account.dev_sign, '' as project_name,";
			str += "t.operation_info_id,t.dev_acc_id,t.modify_date,nvl(t.mileage,0) mileage,nvl(t.drilling_footage,0) drilling_footage,";
			str += "nvl(t.work_hour,0) work_hour from gms_device_operation_info t ";
			str += "inner join gms_device_account account on account.dev_acc_id=t.dev_acc_id ";
			str += "where account.dev_acc_id='"+devaccid+"' and  to_char(t.modify_date,'yyyy')='"+year+"' ) tmp order by tmp.modify_date desc";
