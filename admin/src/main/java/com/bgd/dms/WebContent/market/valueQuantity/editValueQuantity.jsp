...
<%
    ...
    Map valueMap=resultMsg.getMsgElement("valueMap").toMap();
    Date d = new Date();
...
<body>
<form name="form1" id="form1" enctype="multipart/form-data" method="post" action="<%=contextPath%>/market/updateValueQuantity.srq">
<table border="0" cellpadding="0" cellspacing="0" class="form_info">
	<input type="hidden" name="valueQuantityId" id="valueQuantityId" value="<%=(String)valueMap.get("valueQuantityId") %>"/>
	<tr class="odd">
     ...
     <tr class="odd">
     	...
     	<td class="inquire_form">
    		<input type="text" name="planValue" id="planValue" value="<%=(String)valueMap.get("planValue") %>"></input>
     	</td>
	</tr>
  	<tr class="odd">
    	<td class="inquire_item"><font color=red>*</font>&nbsp;落实价值工作量（万元）：</td>
    	<td class="inquire_form">
    		<input type="text" name="implValue" id="implValue" value="<%=(String)valueMap.get("implValue") %>" />
     	</td>
     </tr>
     <tr class="odd">
      	<td class="inquire_item"><font color=red>*</font>&nbsp;累计完成价值量（万元）：</td>
    	<td class="inquire_form">
			<input type="text" name="totalValue" id="totalValue" value="<%=(String)valueMap.get("totalValue") %>" />		
      </td>
    </tr>    
    <tr class="odd">
...
document.getElementById("recordYear").value = "<%=valueMap.get("recordYear") %>"
document.getElementById("recordMonth").value = "<%=valueMap.get("recordMonth") %>"
