			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<div class="tongyong_box">
						<div class="tongyong_box_title"><span class="kb"><a
							href="#"></a></span><a href="#">主要设备完好率、利用率月份分析</a>
							<select id="monthinfo" name="monthinfo" style="select_width" onchange="changeMonth()">
								<%
									for(int showmonth=monthinfo;showmonth>0;showmonth--){
										String showmonthtd = null;
										if(showmonth<10){
											showmonthtd = yearinfo+"-0"+showmonth;
										}else{
											showmonthtd = yearinfo+"-"+showmonth;
										}
								%>
								<option value="<%=showmonthtd%>"><%=yearinfo%>年<%=showmonth%>月</option>
								<%
									}
								%>
							</select>
...
function getFusionChart(){
	...
	myChart2.setXMLUrl("<%=contextPath%>/rm/dm/getCompDevRatioChart.srq");
	...
	myChart3.setXMLUrl("<%=contextPath%>/rm/dm/getCompLVMonth.srq");      
...
function changeYear(){
     ...
     chartReference.setXMLUrl("<%=contextPath%>/rm/dm/getCompDevRatioChart.srq?yearinfo="+yearinfo);
}
function changeMonth(){
     var chartReference = FusionCharts("myChartId3");     
     var monthinfo = document.getElementsByName("monthinfo")[0].value;
     chartReference.setXMLUrl("<%=contextPath%>/rm/dm/getCompLVMonth.srq?monthinfo="+monthinfo);
}
</script>  
<script type="text/javascript">
	/**/function frameSize() {
