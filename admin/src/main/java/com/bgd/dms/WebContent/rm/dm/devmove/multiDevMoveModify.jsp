...
<script type="text/javascript">
	$().ready(function(){
		$("#addProcess").click(function(){	
			...
			var vReturnValue = window.showModalDialog("<%=contextPath%>/rm/dm/devmove/selectAccountForBack.jsp?projectinfono="+projectInfoNo+"&backdevtype="+backdevtype,paramobj,"dialogWidth=1200px;dialogHeight=480px");
			...
			var devdetSql = "select account.dev_acc_id,account.asset_coding, ";
				...
				devdetSql += "account.license_num,account.actual_in_time, ";
		...
		$("#delProcess").click(function(){
			$("input[type='checkbox'][name='idinfo']").each(function(i){
				if(this.checked){
					var id=this.id;
					var fkid=this.fk_id;
					var inprono=this.in_pro_no;
					var detid=this.det_id;

					var detsql = "delete from gms_device_move_detail where dev_mov_det_id='"+detid+"' and bsflag='0' ";
					var path = "<%=contextPath%>/rad/asyncDelete.srq";
					var params = "deleteSql="+detsql;
						params += "&ids=";
					var retObject = syncRequest('Post',path,params);
					
					var sql = "update gms_device_account_dui dui set transfer_state='' where dev_acc_id='"+id+"' and bsflag='0' ";
					var path = "<%=contextPath%>/rad/asyncDelete.srq";
					var params = "deleteSql="+sql;
						params += "&ids=";
					var retObject = syncRequest('Post',path,params);
					
					var sqlDel = "delete from gms_device_account_dui where project_info_id ='"+inprono+"' and fk_dev_acc_id ='"+fkid+"' and bsflag = 'N' ";
	...
	function addLine(retObj,strFlag){
		...
		for(var index=rows;index<rows+retObj.length;index++){
			...
			innerhtml += "<td width='4%'><input type='checkbox' name='idinfo' id='"+retObj[index-rows].dev_acc_id+"' fk_id='"+retObj[index-rows].fk_dev_acc_id+"' in_pro_no='"+retObj[index-rows].in_project_info_id+"' det_id='"+retObj[index-rows].dev_mov_det_id+"' checked/></td>";
	...
	function submitInfo(){
		...
		if(confirm("确认提交？")){
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/devmove/devMoveNewApply.srq?count="+count+"&idinfos="+idinfos+"&enddateinfo="+enddateinfo+"&multiflag=Y";
	...
	function refreshData(){
		...
		var proSql = "select t.dev_mov_id,t.dev_mov_no,t.out_project_info_id,t.in_project_info_id,t.dev_mov_name,";
			...
			proSql += "where t.dev_type = 'wdev' and t.bsflag='0' and t.dev_mov_id='"+devMovId+"'";
		...
		if(devMovId!=null && devMovId!=''){
			...
			var devdetSql = "select dui.dev_coding,dui.dev_name,t.dev_acc_id,dui.dev_model,dui.self_num,dui.license_num,dui.dev_sign,dui.asset_coding,dui.actual_in_time,t.actual_out_time as planning_out_time, "+
           					"mov.in_project_info_id,t.dev_mov_det_id,dui.fk_dev_acc_id from gms_device_move_detail t left join gms_device_account_dui dui on t.dev_acc_id = dui.dev_acc_id "+
           					"left join gms_device_move mov on mov.dev_mov_id = t.dev_mov_id "+
                			"where t.dev_mov_id = '"+devMovId+"' ";
			var proqueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+devdetSql+'&pageSize=10000');
			var retObj = proqueryRet.datas;
			addLine(retObj,'modifi');
		}
	}
	$().ready(function(){
