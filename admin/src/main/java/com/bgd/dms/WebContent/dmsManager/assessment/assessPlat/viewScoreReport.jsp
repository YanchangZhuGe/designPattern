				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<td background="<%=contextPath%>/images/list_15.png"><table
								...
								<tr>
									<td class="ali_cdn_name">要素名称</td>
									<td class="ali_cdn_input"><input id="s_elementName"
										name="s_elementName" type="text" class="input_width" /></td>
									<td class="ali_query"><span class="cx"><a
											href="####" onclick="searchDevData()" title="查询"></a></span></td>
									<td class="ali_query"><span class="qc"><a
											href="####" onclick="clearQueryText()" title="清除"></a></span></td>
									<td>&nbsp;</td>
									<auth:ListButton functionId="" css="zj"
										event="onclick='toAddNewAssessPage()'" title="新增"></auth:ListButton>
									<auth:ListButton functionId="" css="xg"
										event="onclick='toModifyAssessPage()'" title="修改"></auth:ListButton>
									<auth:ListButton functionId="" css="sc"
										event="onclick='toDelAssessPage()'" title="删除"></auth:ListButton>
									<auth:ListButton functionId="" css="dc"
										event="onclick='exportToExcel(\"list_table\")'" title="导出excel"></auth:ListButton>
								</tr>
							</table></td>
		...
		<table width="98%" border="1" cellspacing="2" cellpadding="1"
			...
			<tr  bgcolor="<%=tr_color%>">
				...
				<td width="10px">
						...
						<div style="display: <%=display%>">
						 <%-- <%=s_score_Standard%> --%>
						 <%="".equals(display)? s_score_Standard:"" %>
						</div>
						<!-- 标准分数 -->
				</td>
				<td width="10px">
						...
						<div style="display: <%=display%>">
						<%-- <%=ITEM_DEDUCT_SCORE %> --%>
						<%="".equals(display)? ITEM_DEDUCT_SCORE:"" %>
						</div>
						<!-- 单项扣分 -->
						</td>
				<td width="50px">
						...
						<div style="display: <%=display%>">
						<%-- <%=ITEM_FACT_SCORE %> --%>
						<%="".equals(display)? ITEM_FACT_SCORE:"" %>
						</div> 
						<!-- 要素实际得分-->
						</td>
						<td>
						...
						<div style="display: <%=display%>">
						<%-- <%=ITEM_COMPRE_SCORE %> --%>
						<%="".equals(display)? ITEM_COMPRE_SCORE:"" %>
						</div>
						<!--要素综合得分-->
						</td>				
							...
							<table align="center"  style="width:100%; height:100%"  border="1">
								...
								<tr>
									...
									<input type="hidden" name="group_detail_id"
										...
										<%-- <%=CHECK_SCORE %> --%>
										<%="".equals(display_group)? ELEMENT_SCORE:"" %>
										</div>
