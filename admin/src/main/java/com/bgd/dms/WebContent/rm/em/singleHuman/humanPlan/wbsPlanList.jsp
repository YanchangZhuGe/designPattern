...
<script type="text/javascript">
	...
	function refreshData(taskId,projectInfoNo,taskObjectId){
		...
		cruConfig.queryStr = "select  p.plan_detail_id, p.apply_team_name, p.post_name,p.apply_team, p.post,sum(nvl(p.people_number,0)) people_number , sum(nvl(p.profess_number,0)) profess_number,min(p.plan_start_date) plan_start_date, max(p.plan_end_date) plan_end_date,(max(p.plan_end_date)-min(p.plan_start_date) + 1) nums from ( select d.plan_detail_id,d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,d.people_number,d.profess_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1) nums from bgp_comm_human_plan_detail d  left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0' left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' where d.project_info_no='"+projectInfoNo+"'  and d.spare1 is null  and d.task_id in (select d.id || '' from (select c.* from (select a.object_id as id, a.id as task_id, a.name, a.wbs_object_id as parent_id, 'true' as is_activity from bgp_p6_activity a union all select b.object_id as id, b.code as task_id, b.name, b.parent_object_id as parent_id, 'false' as is_activity from bgp_p6_project_wbs b) c start with c.id = '"+taskObjectId+"' connect by prior c.id = c.parent_id ) d where d.is_activity='true') and d.bsflag='0' ) p group by p.apply_team_name,p.post_name ,p.apply_team, p.post , p.plan_detail_id ";
		cruConfig.currentPageUrl = "/rm/em/singleHuman/humanPlan/wbsPlanList.jsp";
		queryData(1);
	}
    ...
    function loadDataDetail(ids){
    	...
    	var querySql = "select rownum,p.* from (select d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,nvl(d.people_number,0) people_number,nvl(d.profess_number,0) profess_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1) nums,a.name,a.planned_start_date,a.planned_finish_date,a.planned_duration from bgp_comm_human_plan_detail d  left join bgp_p6_activity a on d.task_id=a.object_id and a.bsflag='0' left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0' left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' where d.project_info_no='"+projectInfoNos+"'  and d.spare1 is null  and d.apply_team='"+ids.split("-")[0]+"' and d.post='"+ids.split("-")[1]+"' and d.task_id in (select d.id from (select c.* from (select a.object_id as id, a.id as task_id, a.name, a.wbs_object_id as parent_id, 'true' as is_activity from bgp_p6_activity a union all select b.object_id as id, b.code as task_id, b.name, b.parent_object_id as parent_id, 'false' as is_activity from bgp_p6_project_wbs b) c start with c.id = '"+taskObjectIds+"' connect by prior c.id = c.parent_id ) d where d.is_activity='true') and d.bsflag='0' ) p ";
