...
<%
	...
	if(request.getParameter("projectType") != null){
		projectType = request.getParameter("projectType");//局级用户进度后 会通过点击eps树传入projectType
	}else{
		projectType = user.getProjectType();//非局级用户进入后 无项目类型 取当前项目的类型
      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{projectInfoNo}' id='rdo_entity_id_{projectInfoNo}' onclick=doCheck(this)/>" >选择</td>
			      <td class="bt_info_even" autoOrder="1">序号</td>
			      <td class="bt_info_odd" exp="{projectName}<input type='hidden' id='projectName{projectInfoNo}' value='{projectName}'/>" >项目名称</td>
			      <td class="bt_info_even" exp="{projectStatus}"  func="getOpValue,projectStatus1">项目状态</td>
			      <td class="bt_info_odd" exp="{projectType}"  func="getOpValue,projectType1">项目类型</td>
			      <td class="bt_info_even" exp="{codingName}">甲方单位</td>
			      <td class="bt_info_odd" exp="{plannedStartDate}">计划采集开始时间</td>
			      <td class="bt_info_even" exp="{plannedFinishDate}">计划采集结束时间</td>
			    </tr>
			  </table>
			</div>
...
<script type="text/javascript">
	...
	cruConfig.queryOp = "queryProjectPlans";
	...
	function dbclickRow(ids){
		...
		if(orgSubjectionId != "C105"){
			location.href="<%=contextPath %>/pm/runplan/view.jsp?projectInfoNo="+ids+"&projectType=<%=projectType%>";
		} else {
			parent.location.href="<%=contextPath %>/pm/runplan/view.jsp?projectInfoNo="+ids+"&projectType=<%=projectType%>";
		}
	}
