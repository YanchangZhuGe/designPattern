...
<div id="list_table">
	<div id="inq_tool_box">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
      	<input type="hidden" id="hse_safe_id" name="hse_safe_id" value=""></input>
      	<input type="hidden" id="hse_common_id" name="hse_common_id" value=""></input>
        <tr>
          <td class="inquire_item4">安全观察沟通次数/周：</td>
          <td class="inquire_form4">
          	<input name="safe_times" id="safe_times" class="input_width" type="text" value="" />
          </td>
      </table>
      </fieldSet>
  </div>
...
//	}
	...
	function toAdd(){
		var hse_safe_id = document.getElementById("hse_safe_id").value;
		var hse_common_id = document.getElementById("hse_common_id").value;
		var safe_times = document.getElementById("safe_times").value;
		var re = /^[0-9]+\.?[0-9]*$/;   /////判断字符串是否为数字     //判断正整数 /^[1-9]+[0-9]*]*$/ 
		if (!re.test(safe_times))
		   {
		       alert("请输入数字！");
		       return ;
		    }
		...
		var jcdp_tables="[['bgp_hse_common'],['bgp_hse_week_safe','hse_common_id=bgp_hse_common']]"
		             +"&bgp_hse_common={'hse_common_id':'"+hse_common_id+"','week_start_date':'<%=week_date%>','week_end_date':'<%=week_end_date%>','org_id':'<%=org_id%>','subflag':'0',"
		           	...
		           	jcdp_tables = jcdp_tables+"&bgp_hse_week_safe={'hse_safe_id':'"+hse_safe_id+"','safe_times':'"+safe_times+"'}";
	...
	function toEdit(){
		var checkSql="select c.hse_common_id id,s.* from bgp_hse_common c left join bgp_hse_week_safe s on c.hse_common_id=s.hse_common_id where to_char(c.week_start_date,'yyyy-MM-dd')='<%=week_date %>' and c.bsflag='0' and c.org_id='<%=org_id%>'";
	    var queryRet = syncRequest('Post','<%=request.getContextPath()%>'+appConfig.queryListAction,'querySql='+checkSql);
		var datas = queryRet.datas;
		debugger;
		if(datas==null||datas==""||datas[0].hse_safe_id==null||datas[0].hse_safe_id==""){
			if(datas!=null&&datas!=""){
				document.getElementById("hse_common_id").value=datas[0].id;
			}
			action="add";	
			if(organ_flag=="0"){
				var checkSql3="select sum(s.safe_times) safe_times from bgp_hse_common c join bgp_hse_org ho on c.org_id = ho.org_sub_id left join bgp_hse_week_safe s on c.hse_common_id = s.hse_common_id where to_char(c.week_start_date, 'yyyy-MM-dd') = '<%=week_date %>' and c.bsflag = '0' and c.subflag = '1' and ho.father_org_sub_id = '<%=org_id%>'";
				...
				if(datas3!=null&&datas3!=""){
					document.getElementById("safe_times").value=datas3[0].safe_times;
				}
			}else{
				var checkSql2="select count(t.hse_check_id) safe_times from bgp_hse_safecheck t where t.bsflag='0' and t.check_date >= to_date('<%=week_date %>', 'yyyy-mm-dd') and t.check_date <= to_date('<%=week_end_date %>', 'yyyy-MM-dd') and t.third_org = '<%=org_id%>'";
				...
				if(datas2!=null&&datas2!=""){
					document.getElementById("safe_times").value=datas2[0].safe_times;
				}
			}
		}else{
			document.getElementById("hse_common_id").value=datas[0].id;
			document.getElementById("hse_safe_id").value=datas[0].hse_safe_id;
			document.getElementById("safe_times").value=datas[0].safe_times;
			action = "edit";
		}
	} 
