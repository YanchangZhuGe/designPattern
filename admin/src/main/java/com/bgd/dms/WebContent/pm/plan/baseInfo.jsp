...
function loadData(){
	...
	if(retObj.map == undefined){
		...
		if(retObj.map.manage_org_name == undefined || retObj.map.project_name == undefined || retObj.map.exploration_method == undefined ||  retObj.map.org_id == undefined || retObj.map.project_name == "" || retObj.map.org_id == "" || retObj.map.manage_org_name == "" || retObj.map.exploration_method == "" ||retObj.map.project_design_end_date == undefined || retObj.map.project_design_start_date == undefined || retObj.map.project_design_start_date == "" || retObj.map.project_design_end_date == "" || retObj.map.project_duration_date == undefined || retObj.map.project_duration_date == ""){
			//该页面所需的值有缺失 需更新记录
			document.getElementById("message").innerHTML = '<font color="red">该信息尚未保存</font>';
			retObj = jcdpCallService("ProjectSrv", "getProjectInfo", "projectInfoNo="+ids);
			document.getElementById("manage_org_name_td").innerHTML= '本项目来源于：<font style="text-decoration: underline;" >'+retObj.map.manage_org_name+'</font>';
