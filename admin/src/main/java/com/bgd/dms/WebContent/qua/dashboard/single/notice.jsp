		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			<tr>
				<td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{file_id}:{ucm_id}' id='rdo_entity_id_{file_id}' onclick=doCheck(this)/>" >
					<input type='checkbox' name='rdo_entity_id' value=':' id='rdo_entity_id_{file_id}' onclick='check(this)'/></td>
...
<script type="text/javascript">
	...
	cruConfig.queryOp = "getNoticeList";
	$("#tab_box").css("height",$(window).height()-$("#inq_tool_box").height()-$("#fenye_box").height()-$("#line").height());
	resizeNewTitleTable();
	cruConfig.submitStr = "file_name=&subjection_id="+'<%=subjection_id%>';
	queryData(1);
	...
	function refreshData(q_file_name,subjection_id){
		...
		cruConfig.submitStr = "file_name="+file_name+"&subjection_id="+subjection_id;
		queryData(1);
	}
	...
	function toAdd(){
		...
		if(subjection_id !=null && subjection_id =='C105'){
	  	...
	  	popWindow('<%=contextPath%>/qua/common/upload_file.jsp?relationId='+relation_id);
	}
