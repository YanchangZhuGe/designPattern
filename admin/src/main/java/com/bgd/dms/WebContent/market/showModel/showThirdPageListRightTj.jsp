...
function changeDivStyle2(id,typeName){
	...
	function linkPageList(page){
		parent.mainFrame.location="<%=contextPath%>/market/show/viewListTreeTj.srq?typeId=<%=typeId%>&&typeName=<%=typeName%>&&currentPage="+page+"&&pageType=<%=pageType%>";		
	}
	function linkTwoList(typeId,typeName){
		parent.mainFrame.location="<%=contextPath%>/market/show/viewListTreeTj.srq?typeId="+typeId+"&&typeName="+typeName+"&&currentPage=<%=currentPage%>&&pageType=<%=pageType%>";		
	}
	function linkThirdList(typeId,typeName){
		parent.mainFrame.location="<%=contextPath%>/market/show/viewDetail.srq?id="+typeId+"&&typeName="+typeName;	
	}
	function linkSecondPage(info){
		if(info=="tjfx"){
			parent.mainFrame.location="<%=contextPath%>/market/show/showSecondPage_tj.srq?pageType="+info;
		}else{
			parent.mainFrame.location="<%=contextPath%>/market/show/showSecondPage.srq?pageType="+info;
		}
	}
...
<body onload="showtime()">
  ...
  <div id="list_div" style="float: right">
    <div id="gl_news_mbx">
      <ul>
        <li class="mleft"></li>
        <li class="mbg"><span class="mbx_text"><a href="#"><%=typeName %></a></span></li>
        <li class="mright"></li>
      </ul>
    </div>
    <div id="main_content">
    ...
    <div id="fy" style="width:713px">
    		...
    		if(currentPage.equals("1")) {
    			...
    			lastPage=Integer.parseInt(pageCount)-1;
    			nextPage=Integer.parseInt(pageCount)+1;
    		}
    	%>
    	<a href="#">找到 <%=totalRows%> 条记录,总共 <%=pageCount%> 页,当前第 <%=currentPage%> 页</a>
...
<ul>
	<li class="mleft"></li>
	<li class="mbg"><span class="gl_mbx_text"><a href="javascript:linkSecondPage('<%=pageType %>')"><%=headingInfo%></a></span></li>
	<li class="mright"></li>
</ul>
</div>
<div id="gl_left_botbox">
	...
	%>
		<div class="<%=oneClass %>"><a href="javascript:<%=javascriptMethod %>('<%=code%>','<%=codeName%>')"><%=codeName%></a></div>
		<%
		...
		<div id="<%="twoList" + code%>" style="display: <%=twoDisplay%>">
			...
			<div class="<%=twoClass %>"><a href="javascript:changeDivStyle2('<%=subCode%>','<%=subCodeName%>')"><%=subCodeName%></a></div>
			<%
		...
		%>
			<div id="<%="twoList" + subCode%>" style="display: <%=thirdDisplay%>">
				...
				<div class="<%="10801001".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10801001','规划部署')">规划部署</a></div>
				<div class="<%="10801002".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10801002','油气发现')">油气发现</a></div>
				<div class="<%="10801003".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10801003','客户管理')">客户管理</a></div>
				<div class="<%="10801004".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10801004','行业新闻')">行业新闻</a></div>
				<div class="<%="10801005".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10801005','政经动态')">政经动态</a></div>
				<%} else if(subCode.startsWith("106")){%>
				<div class="<%="10802001".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10802001','技术装备能力')">技术装备能力</a></div>
				<div class="<%="10802002".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10802002','竞争伙伴动态分析')">竞争伙伴动态分析</a></div>
				<div class="<%="10802003".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10802003','竞争伙伴管理')">竞争伙伴管理</a></div>
				<div class="<%="10802004".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10802004','国际原油价格')">国际原油价格</a></div>
				<div class="<%="10802005".equals(typeId)&&subCode.equals(twoId)?"third_sel_notNews":"third_notNews" %>"><a href="javascript:linkThirdList('<%=subCode %>','10802005','收入对比')">收入对比</a></div>
				<%} %>
			</div>
			<%
				}
