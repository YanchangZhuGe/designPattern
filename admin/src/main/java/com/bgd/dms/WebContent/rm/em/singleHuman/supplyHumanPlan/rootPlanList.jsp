      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{apply_team}-{post}-{plan_detail_id}-{spare1}' id='rdo_entity_id_{plan_detail_id}' />" >选择</td>
...
<script type="text/javascript">
	...
	function refreshData(taskId,projectInfoNo){
		...
		cruConfig.queryStr = "select p.spare1, p.apply_team_name, p.post_name,p.apply_team,p.plan_detail_id, p.post,sum(nvl(p.people_number,0)) people_number , sum(nvl(p.profess_number,0)) profess_number,min(p.plan_start_date) plan_start_date, max(p.plan_end_date) plan_end_date,(max(p.plan_end_date)-min(p.plan_start_date) + 1 ) nums from ( select d.plan_detail_id,d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,d.people_number,d.profess_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1 ) nums ,d.spare1 from bgp_comm_human_plan_detail d  left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0'  and s1.coding_mnemonic_id='<%=projectType%>'   left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' and s2.coding_mnemonic_id='<%=projectType%>'  where d.spare1='"+splan_id+"' and d.project_info_no='<%=projectInfoNo%>'  and d.bsflag='0' ) p group by p.apply_team_name,p.post_name ,p.apply_team, p.post ,p.plan_detail_id,p.spare1   ";
		cruConfig.currentPageUrl = "/rm/em/singleHuman/supplyHumanPlan/rootPlanList.jsp";
		queryData(1);
	...
	function toBack(){
	 	...
	 	//parent.mainRightframe.location.href = "<%= contextPath%>/rm/em/singleHuman/supplyHumanPlan/supplyHumanPlan.jsp";
	}
	...
	function toTj(){
		...
		//popWindow('<%=contextPath%>/rm/em/singleHuman/supplyHumanPlan/humanPostChart.jsp?team='+'0110000001000000001','800:680');
	}
    ...
    function chooseOne(cb){   
 	...
 	function loadDataDetail(ids){
    	...
    	var querySql = "select rownum,p.* from (select d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,nvl(d.people_number,0) people_number,nvl(d.profess_number,0) profess_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date + 1) nums,a.name,a.planned_start_date,a.planned_finish_date,a.planned_duration from bgp_comm_human_plan_detail d  left join bgp_p6_activity a on d.task_id=a.object_id and a.bsflag='0' left join comm_coding_sort_detail s1 on d.apply_team=s1.coding_code_id and s1.bsflag='0' and s1.coding_mnemonic_id='<%=projectType%>'  left join comm_coding_sort_detail s2 on d.post=s2.coding_code_id and s2.bsflag='0' and s2.coding_mnemonic_id='<%=projectType%>'   where d.project_info_no='<%=projectInfoNo%>' and d.spare1='"+ids.split("-")[3]+"' and d.apply_team='"+ids.split("-")[0]+"' and d.post='"+ids.split("-")[1]+"' and d.bsflag='0' ) p ";
