			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      ...
			      <td class="bt_info_odd"  exp="{plan_no}" >提交单号</td>
...
//	cruConfig.funcCode = "F_HUMAN_003";
	...
	function view_page(plan_id,cost_state){		
		if(plan_id != ""){
			window.location= "<%=contextPath%>/rm/em/singleHuman/humanCostPlanTwo/humanCostPlanManager.jsp?buttonN=0&planIds="+plan_id+"&costState="+cost_state;
		}
	...
	function refreshData(){
		...
		cruConfig.queryStr = " select   c.cost_state ,decode(te.proc_status,  '1',  '待审批',  '3',  '审批通过',    '4',  '审批不通过',  te.proc_status) proc_status_name, c.plan_id, e.employee_name,  p.project_name,to_char(c.modifi_date,'yyyy-mm-dd hh:ss')modifi_dates,c.modifi_date,c.project_info_no,  (select wmsys.wm_concat(i.org_abbreviation)  from gp_task_project_dynamic d  left join comm_org_information i  on d.org_id = i.org_id  and i.bsflag = '0'  where p.project_info_no = d.project_info_no  and d.bsflag = '0') org_name,  nvl(c.plan_no, '申请提交后系统自动生成') plan_no,   te.proc_status   from  bgp_comm_human_plan_cost c  left join gp_task_project p  on p.project_info_no = c.project_info_no     and p.bsflag = '0'     left join common_busi_wf_middle te  on te.business_id = c.plan_id  and te.bsflag = '0'  left join comm_human_employee e  on c.creator = e.employee_id  and e.bsflag = '0'      where   c.cost_state = '"+costState+"'  and c.bsflag = '0'  and c.spare5 = '1'  and   te.proc_status in ('1','3','4')  order by te.proc_status asc, c.modifi_date desc  ";
	...
	function loadDataDetail(ids){ 
  	  processNecessaryInfo={         
  	    		...
  	    		businessType:"5110000004100000048",        //业务类型 即为之前设置的业务大类
	...
	function dbclickRow(ids){
		...
		popWindow("<%=contextPath%>/rm/em/singleHuman/humanCostPlanTwo/addMainPage.jsp?projectInfoNo="+ids.split("-")[0]+"&costState="+costState+"&plan_id="+ids.split("-")[1]);
	}
