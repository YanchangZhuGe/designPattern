		...
		<div id="tab_box" class="tab_box">
			<div id="tab_box_content0" name="tab_box_content0"
				...
				<table id="projectMap" name="projectMap" border="0" cellpadding="0"
					...
					<td class="inquire_form6"><input id="backapp_org_name"
						class="input_width" type="text" value="" disabled /> &nbsp;</td>
					</tr>
					<tr><td class="inquire_item6">&nbsp;申请人：</td>
						<td class="inquire_form6"><input id="back_employee_name"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
						<td class="inquire_item6">申请日期：</td>
						<td class="inquire_form6"><input id="backdate"
							class="input_width" type="text" value="" disabled /> &nbsp;</td>
...
<script type="text/javascript">
    ...
    function refreshData(v_device_repair_app_no, v_device_repair_app_name){
		var str = 
			...
			"       ORG_ABBREVIATION AS BACKAPP_ORG_NAME,\n" + 
			...
			"    AND RFORM.REP_TYPE = '0'" +
			"   AND RFORM.BACK_ORG_ID = '<%=orgId%>' \n" ;
    ...
    function loadDataDetail(id){
    	var retObj;
		if(id!=null){
			 retObj = jcdpCallService("DevCommInfoSrv", "getDevRepairRtnBaseInfo", "id="+id);
		    ...
		    retObj = jcdpCallService("DevCommInfoSrv", "getDevRepairRtnBaseInfo", "id="+ids);
		}
		//选中这一条checkbox
		...
		$("#back_employee_name","#projectMap").val(retObj.deviceSendRepairRtnMap.back_employee_name);
		$("#backdate","#projectMap").val(retObj.deviceSendRepairRtnMap.backdate);
		$("#repair_form_no","#projectMap").val(retObj.deviceSendRepairRtnMap.repair_form_no);
		getContentTab(undefined,selectedTagIndex);
    }
	function toAddDevSendRepairPage(){
		popWindow('<%=contextPath%>/rm/dm/devRepair/repairReturnNewApply.jsp');
	}
	function toModifyDevRepairPage(){
		...
		popWindow('<%=contextPath%>/rm/dm/devRepair/repairRtnModify.jsp?id=' + id);
	}
	...
	function toSumbitRepairSend(){
		...
		if (window.confirm("确认要提交吗?")) {
			...
			***/
			...
			//encodeAndsyncRequest('Post',path,accparams);
			//向设备动态表添加数据
			//document.getElementById("form1").action = "<%=contextPath%>/rm/dm/devRepair/repairReturnListSubmit.srq?formid="+selectedid;
			//document.getElementById("form1").submit();
			retObj = jcdpCallService("DevCommInfoSrv", "repairReturnListSubmit", "formid="+selectedid);
