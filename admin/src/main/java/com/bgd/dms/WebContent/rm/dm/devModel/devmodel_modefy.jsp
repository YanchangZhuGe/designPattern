...
<%
	...
	String[] info = request.getParameter("devmodelid").split("~",-1);
	String devmodelid = info[0];
      ...
      <table border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td class="inquire_item4" >
          	...
          	<input name="dev_model_id" id="dev_model_id" class="input_width" type="hidden"  value=""  />
      ...
      <fieldset style="margin-left:2px"><legend>设备信息</legend>
		<div id="tag-container_4" style="float:left">
		  <ul id="tags" class="tags">
		    <li class="selectTag" id="tag3_0" ><a href="#" onclick="">单台管理</a></li>
		    <li id="tag3_1"><a href="#" onclick="">批量管理</a></li>
		  </ul>
		</div>
		  <div id="resultdiv0" name="resultdiv0" style="float:left;height:210px;">
		  ...
		  <div id="resultdiv1" name="resultdiv1" style="float:left;height:210px;overflow:auto;display:none">
...
<script type="text/javascript"> 
	...
	function refreshData(){
		...
		if('<%=devmodelid%>'!='null'){
			if('<%=seqinfo%>'=='0'){
				var proSql = "select m.dev_model_name from gms_dev_model m "; 
				proSql += "where m.dev_model_id='<%=devmodelid%>' and m.bsflag='0'";
				
				var proqueryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+proSql);
				retObj = proqueryRet.datas;
				$("#dev_model_name").val(retObj[0].dev_model_name);
				$("#model_type").val(retObj[0].model_type);
				//查询子表信息
				var str = "select * from gms_dev_model_detail d";
				str += " where d.gms_dev_model_id='<%=devmodelid%>' and d.bsflag='0' ";
				...
				for(var index=0;index<retObj.length;index++){
					...
					var model_detail_id = retObj[index].model_detail_id;
					var dev_name = retObj[index].dev_name;
					var dev_type = retObj[index].dev_type;
					var sign_type = retObj[index].sign_type;
					var is_dev_code = retObj[index].is_dev_code;
					var unit = retObj[index].unit;
					...
					innerhtml += "<input name='dev_name"+index+"' id='dev_name"+index+"' style='line-height:15px' value='' size='15' type='text' readonly/>";
					innerhtml += "<input type='button' style='width:20px' value='...' onclick='showDevPage("+index+")'/></td>";
					
					innerhtml += "<td><input id='dev_type"+index+"' name='dev_type"+index+"' value='' size='14'  type='text' readonly/>";
					innerhtml += "<input id='model_detail_id"+index+"' name='model_detail_id"+index+"' value='' type='hidden' />";
					innerhtml += "<input id='sign_type"+index+"' name='sign_type"+index+"' value='' type='hidden' />";
					innerhtml += "<input id='is_dev_code"+index+"' name='is_dev_code"+index+"' value='' type='hidden' /></td>";
					...
					//给数据回填
					$("#deviceallappdetid"+index).val(model_detail_id);
					$("#dev_name"+index).val(dev_name);
					$("#dev_type"+index).val(dev_type);
					$("#sign_type"+index).val(sign_type);
					$("#is_dev_code"+index).val(is_dev_code);
					$("#unit"+index).val(unit);
				...
				$("#model_type").val(retObj[0].model_type);
				//查询子表信息
				var str = "select * from gms_dev_model_detail d where d.gms_dev_model_id='<%=devmodelid%>' and d.bsflag='0' ";
				var detailRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+str);
				retObj = detailRet.datas;
				for(var index=0;index<retObj.length;index++){
					if(index==0){
					...
					innerhtml += "<input type='checkbox' name='collidinfo' id='"+index+"'/>";
					innerhtml += "<td><input name='coll_dev_name"+index+"' id='coll_dev_name"+index+"' style='line-height:15px' value='用户输入名称' size='15' type='text' /></td>";
					
					innerhtml += "<td><select name='coll_dev_type"+index+"' id='coll_dev_type"+index+"' class='select_width' ></selcted>";
					innerhtml += "<input id='coll_model_detail_id"+index+"' name='coll_model_detail_id"+index+"' value='' type='hidden' /></td>";
					
					innerhtml += "<td><select name='coll_unit"+index+"' id='coll_unit"+index+"' /></select></td>";
					innerhtml += "<td><input name='coll_purpose"+index+"' id='coll_purpose"+index+"' class='input_width' value='' size='10' type='text'/></td>";
					innerhtml += "</tr>";
					...
					$("#coll_unit"+index).append(optionhtml);
					//查询公共代码，并且回填到界面的申请类型中
					var colltypeObj;
					var colltypeSql = "select t.coding_code_id as value,t.coding_name as label ";
					colltypeSql += "from comm_coding_sort_detail t "; 
					...
					$("#coll_dev_type"+index).append(colltypeoptionhtml);
					
					//给数据回填
					$("#coll_model_detail_id"+index).val(model_detail_id);
					$("#coll_dev_name"+index).val(dev_name);
					$("#coll_dev_type"+index).val(dev_type);
					$("#coll_unit"+index).val(unit);
					$("#coll_purpose"+index).val(purpose);
				}
				$("#processtable1>tr:odd>td:odd").addClass("odd_odd");
				$("#processtable1>tr:odd>td:even").addClass("odd_even");
				$("#processtable1>tr:even>td:odd").addClass("even_odd");
