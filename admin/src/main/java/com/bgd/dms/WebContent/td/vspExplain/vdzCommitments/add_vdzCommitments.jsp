						...
						<div id="tab_box_content0" class="tab_box_content">
							<table id="tableDoc" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
								<tr>
    								...
    								<input name="project_name" id="project_name" class="input_width" value="" type="text"  readonly="readonly" />   
    								    <% 
			  		  						if(p_type.equals("1")){
			    						%>
								...
								<tr>
									...
									<input name="commitments_id" id="commitments_id" class="input_width" value="" type="hidden" readonly="readonly"/>
							            ...
							            <input name="file_id" id="file_id" class="input_width" value="" type="hidden" readonly="readonly"/>
       								    <input name="bsflag" id="bsflag" class="input_width" value="0" type="hidden" readonly="readonly"/>
							           	...
							           	<input name="a_s_name" id="a_s_name" class="input_width" value="" type="hidden" readonly="readonly"/>
							           	  	 <input name="spare2" id="spare2" class="input_width" value="" type="hidden" readonly="readonly"/>
									...
									<td  class="inquire_form4" >
									    <input type="hidden" id="pk_0110000061200000006" name="pk_0110000061200000006" value=""/>
										<input type="file" name="0110000061200000006" id="0110000061200000006" class="input_width"/>
...
/* 	if (!isTextPropertyNotNull("writer", "上交人")) return false;
	if (!isLimitB20("writer","上交人")) return false; 
	  */
  	...
  	if(p_type=='0'){ 
		if (!isTextPropertyNotNull("project_info_no", "项目名称")) return false;
		if (!isTextPropertyNotNull("pk_0110000061200000006", "处理解释任务书")) return false;
		if(radioType =='1'){
		...
		if (!isTextPropertyNotNull("project_info_no", "项目名称")) return false;
		if (!isTextPropertyNotNull("0110000061200000006", "处理解释任务书新增")) return false;
		if(radioType =='1'){
			if (!isTextPropertyNotNull("s_id", "处理人员")) return false;
			if (!isTextPropertyNotNull("a_s_id", "解释人员")) return false;
		}
...
function page_init(){
	...
	if(fileId !=''){
		...
		if(datas!=null && datas.length>0){
			...
			if(datas[0].ucm_id_a!=""){
				document.getElementById("pk_0110000061200000006").value =datas[0].ucm_id_a;
		...
		document.getElementById("file_abbr").value = "<%=fileAbbr%>";
		document.getElementById("commitments_id").value = "";
		document.getElementById("business_type").value = "<%=businessType%>";
...
function selectTeam(){
    ...
    var result = window.showModalDialog('<%=contextPath%>/td/vspExplain/vdzCommitments/searchVdzProjectList.jsp?types=vdz','dialogWidth:500px;dialogHeight:500px;status:yes');
    if(result!=""){
    	var checkStr = result.split(",");	
	        document.getElementById("project_info_no").value = checkStr[0];
	        document.getElementById("project_name").value = checkStr[1];
	        document.getElementById("spare2").value = checkStr[3];//维护项目区分字段
 
		    var querySql =" select relation_id,file_name ,file_type ,ucm_id from  (select t.*,row_number() over(partition by t.project_info_no  order by t.ucm_id desc ) r from bgp_doc_gms_file t  where t.doc_file_type = '0110000061100000124' and t.bsflag='0' and t.project_info_no='"+checkStr[0]+"' )w where w.r='1' ";
			var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
			var datas = queryRet.datas;
			if(datas!=null && datas.length>0){
				document.getElementById("tecnical_id").value = datas[0].relation_id;
				var str=datas[0].file_name==""?"":datas[0].file_name.substr(0,30)+'...';
				$("#down_0110000061100000124").append("&nbsp;<a style='color:blue;'  href=<%=contextPath%>/doc/downloadDocByUcmId.srq?docId="+datas[0].ucm_id+"&emflag=0>"+str+"</a>");
...
function save(){
	...
	document.getElementById("a_s_name").value =a_s_name;
 	...
 	if (!checkForm()) return;
 	  ...
 	  var p_file_id=""; 
 	 var optioning_type=  document.getElementById("spare2").value; 
		
		if(optioning_type =='jz'){
			document.getElementById("parent_file_id").value ="01A8DE665EA27111111111111111101";
			document.getElementById("file_abbr").value = "JZVSPPROJECT";
			
		}else{
		 	  var p_info_no= document.getElementById("project_info_no").value;
		 	 var querySql ="select * from bgp_doc_gms_file f where f.bsflag='0' and is_file='0' and f.project_info_no='"+p_info_no+"' and f.file_abbr='"+fileAbbr+"'";
				var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
				var datas = queryRet.datas;
				if(datas!=null && datas.length>0){
					p_file_id = datas[0].file_id; 
				} 
				document.getElementById("parent_file_id").value = p_file_id;
		}
	    
	var form = document.getElementById("CheckForm");
	form.action = "<%=contextPath%>/td/ws/toSaveWdzListDoc.srq?businessType=<%=businessType%>";
	form.submit();
	newClose(); 
}
