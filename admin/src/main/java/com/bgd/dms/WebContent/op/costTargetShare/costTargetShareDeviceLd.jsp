...
<body style="background:#fff" onload="refreshData()">
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <%if(proc_status){ %>
			  <auth:ListButton functionId="OP_TARGET_DO_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			  <auth:ListButton functionId="OP_TARGET_DO_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			  <auth:ListButton functionId="OP_TARGET_DO_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			  <%} %>
			  </tr>
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getDeviceDepreOtherShareInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
    		...
    		td.innerHTML ='<input type="hidden" id="fy'+i+'dev_acc_id" name="fy'+i+'dev_acc_id" value="'+datas[i].devAccId+'" class="input_width"/>'+
    		'<INPUT id="fy'+i+'checkbox" name="rdo_entity_id"  value='+datas[i].targetDepreId+' type=checkbox>'+
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareDeviceLdEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareDeviceLdEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
 ...
 function getSum(){
		...
		var querySql = "select sum(nvl(to_char(td.asset_value*0.97/td.depreciation_value/8*td.device_count/30*(td.plan_end_date-td.plan_start_date+1),'9999999999999999.00'),0)) sum_value"+
		...
		" where td.bsflag='0' and  td.project_info_no = '"+project_info_no+"'  and td.record_type='1'  and td.if_change ='0'";
