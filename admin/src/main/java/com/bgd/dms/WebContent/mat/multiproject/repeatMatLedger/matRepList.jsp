			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_even" exp="{coding_name}">区域</td>
			      <td class="bt_info_odd" exp="{invoices_id}"  isShow="TextHide" style="display: none">调剂主键</td>
			    </tr>
			  </table>
			</div>
...
<script type="text/javascript">
	...
	function refreshData(){
		var sql ="select p.plan_invoice_id,p.submite_number,tp.project_name,i.org_abbreviation,t.invoices_id,t.invoices_no,decode(t.if_input,'1','已调剂')if_input,t.create_date,u.user_name,p.compile_date,u2.user_name as apply_name,cd.coding_name from gms_mat_demand_plan_invoice p left join (GMS_MAT_TEAMMAT_INVOICES t inner join p_auth_user u on t.creator_id=u.user_id and u.bsflag='0'and t.invoices_type='2' left join comm_coding_sort_detail cd on cd.coding_code_id=t.area and cd.bsflag='0' ) on p.plan_invoice_id=t.plan_invoice_id and t.bsflag='0' inner join gp_task_project tp on p.project_info_no=tp.project_info_no and tp.bsflag='0' inner join comm_org_information i on p.org_id=i.org_id and i.bsflag='0'inner join common_busi_wf_middle wm on wm.business_id=p.plan_invoice_id  and wm.proc_status = '3' inner join gms_mat_demand_plan pd on p.plan_invoice_id=pd.plan_invoice_id left join p_auth_user u2 on u2.user_id=p.creator_id and u2.bsflag='0'  where p.bsflag='0' and pd.regulate_num is not null group by p.plan_invoice_id,tp.project_name,p.submite_number,i.org_abbreviation,t.invoices_id,t.invoices_no,if_input,t.create_date,u.user_name,p.compile_date,u2.user_name,cd.coding_name order by p.compile_date desc";
	  ...
	  function toAdd(){ 
		  ids = getSelIds('rdo_entity_id');
		    if(ids==''){ alert("请先选中一条记录!");
		    	...
		    	popWindow('<%=contextPath%>/mat/multiproject/repeatMatLedger/findEscDep.srq?laborId='+ids,'1024:800');
			    }
	  ...
	  function getCheckId(){
			...
			for(var i=1;i<row.length;i++){
				if(row[i].cells[0].firstChild.checked==true){
					var id="";
					if(row[i].cells[12].innerHTML=="&nbsp;"){
						id += row[i].cells[0].firstChild.value;
					}
					else{
						id += row[i].cells[0].firstChild.value+","+row[i].cells[12].innerHTML;
						}
					ckeckId+=id;
					}
				}
  		...
  		if(datas!=null){
  			for(var i=0;i<datas.length;i++){
  				...
  				vTr.onclick = function(){
  					...
  					// 加载Tab数据
  					if(cells[12].innerHTML != '&nbsp;'){
  						loadDataDetail(cells[0].childNodes[0].value+","+cells[12].innerHTML);
  					}
  					else{
