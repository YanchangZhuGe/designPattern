			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      ...
			      <td class="bt_info_even" exp="{avg_price}">单价</td>
...
<script type="text/javascript">
	...
	function refreshData(){
		...
		if(codeId !=null && codeId != rootId){
			...
			sql +="select aa.wz_id,(aa.mat_num- case when bb.out_num is null then 0 else bb.out_num end) stock_num,round((aa.in_price-case when bb.out_price is null then 0 else bb.out_price end)/ case when (aa.mat_num - case when bb.out_num is null then 0 else bb.out_num end) is null then 1 when (aa.mat_num - case when bb.out_num is null then 0 else bb.out_num end)=0 then 1 else (aa.mat_num - case when bb.out_num is null then 0 else bb.out_num end) end,3) avg_price,i.coding_code_id,i.wz_name,i.wz_prickie,i.note,i.wz_price from ( ";
			sql +="select tid.wz_id,sum(tid.mat_num) mat_num,sum(nvl(tid.actual_price,0)*nvl(tid.mat_num,0)) in_price from gms_mat_teammat_invoices mti inner join gms_mat_teammat_info_detail tid on mti.invoices_id=tid.invoices_id and tid.bsflag='0' where mti.bsflag='0' and mti.invoices_type='1' and mti.project_info_no='<%=projectInfoNo%>' and mti.if_input='0' group by tid.wz_id ";
			sql +=")aa left join ( ";
			sql +="select tod.wz_id,sum(tod.mat_num) out_num,sum(nvl(tod.actual_price,0)*nvl(tod.mat_num,0)) out_price from gms_mat_teammat_out mto inner join gms_mat_teammat_out_detail tod on mto.teammat_out_id=tod.teammat_out_id and tod.bsflag='0' where mto.bsflag='0' and mto.wz_type = '2' and mto.project_info_no='<%=projectInfoNo%>' group by tod.wz_id ";
	...
	function loadDataDetail(shuaId){
		...
		var retObj;
		if(shuaId!= undefined){
			 retObj = jcdpCallService("MatItemSrv", "getSinglePurLedger", "laborId="+shuaId);
			 taskShow(shuaId);
			 detailRKShow(shuaId);
		}else{
		    ...
		    if(ids==''){ 
		     ...
		     retObj = jcdpCallService("MatItemSrv", "getSinglePurLedger", "laborId="+ids);
		     taskShow(ids);
		     detailRKShow(ids);
		}
		...
		document.getElementById("wz_price").value = retObj.matInfo.avgPrice;
	  ...
	  function toAdd(){ 
       ...
       //出库明细
    	 function taskShow(value){
    			...
    			var retObj = jcdpCallService("MatItemSrv", "getTeamListXH", "wz_type=2&ids="+value);
