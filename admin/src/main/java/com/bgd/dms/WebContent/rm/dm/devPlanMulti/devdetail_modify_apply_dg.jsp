	  ...
	  <% if(allappType.equals("S0000")) {  //自有%>
		...
		<div id="tag-container_3">
		 ...
		 <% } else if(allappType.equals("S9997")) { //专业化地震仪器%>
		  		 <li class="selectTag"  id="tag3_1"><a href="#" onclick="getContentTab(this,1)">地震仪器</a></li>
		 <% } else if(allappType.equals("S9998")) { //调剂设备%%>
	...
	function refreshData(){
		...
		if('<%=deviceallappid%>'!='null'){
			if('<%=seqinfo%>'=='0'){
				...
				str += "alldet.apply_num,alldet.approve_num,";
				str += "alldet.teamid,";
				str += "alldet.teamtype,";
				str += "alldet.team, ";
				...
				for(var index=0;index<retObj.length;index++){
					if(index==0){
						...
						$("#teamtype").val(retObj[0].teamtype);
					}
					...
					var approve_num = retObj[index].approve_num;
					...
					innerhtml += "<td><input id='neednum"+index+"' name='neednum"+index+"' class='input_width' value='' size='8' type='text' onkeyup='checkAppNum(this,\"approvenum"+index+"\")'/>";
					//综合物化探 项目资源配置中 审批数量
					innerhtml += "<input type='hidden' id='approvenum"+index+"' name='approvenum"+index+"'></td>";
					...
					$("#approvenum"+index).val(approve_num);
				...
				//alert("devdetail modify apply jsp 22222222");
				var proSql = "select devapp.project_info_no,project.project_name,devapp.device_allapp_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.remark, ";
				proSql += "to_char(sysdate,'yyyy-mm-dd') as appdate "; 
				proSql += "from gms_device_allapp devapp left join gp_task_project project on devapp.project_info_no=project.project_info_no "; 
				proSql += "where devapp.bsflag='0' and devapp.device_allapp_id='<%=deviceallappid%>'";
				...
				str += "alldet.unitinfo,alldet.apply_num,alldet.approve_num,alldet.teamid,alldet.team, ";
				str += "alldet.purpose,alldet.employee_id,emp.employee_name, ";
				str += "alldet.plan_start_date,alldet.plan_end_date  ";
				...
				for(var index=0;index<retObj.length;index++){
					...
					innerhtml += "<td><input name='collneednum"+index+"' id='collneednum"+index+"' class='input_width' value='' size='10' type='text' onkeyup='checkAppNum(this,\"collapprovenum"+index+"\")'/>";
					//综合物化探 项目资源配置中 审批数量
					//innerhtml += "<input type='hidden' id='collapprovenum"+index+"' name='collapprovenum"+index+"'></td>";
					...
					innerhtml += "<td><input name='collpurpose"+index+"' id='collpurpose"+index+"' class='input_width' value='' size='10' type='text'/></td>";
					innerhtml += "</tr>";
					...
					$("#collapprovenum"+index).val(approve_num);
