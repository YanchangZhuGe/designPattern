...
function save(){
	if(checkForm()){
		...
		form.action = "<%=contextPath%>/td/toSavenewStandardDoc.srq";
		form.submit();
		newClose();
	}
}
...
function page_init(){
	...
	if(fileId!='null'){
		var querySql = " select t.file_id,t.file_name,t.ucm_id,t.doc_type,t.doc_file_type,t.parent_file_id,to_char(t.upload_date,'yyyy-MM-dd') upload_date,t.creator_id,e.employee_name  from bgp_doc_gms_file t left join comm_human_employee e on t.creator_id=e.employee_id and e.bsflag='0'  where t.file_id='"+fileId+"'";
