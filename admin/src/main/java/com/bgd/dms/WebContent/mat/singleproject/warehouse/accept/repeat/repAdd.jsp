...
<%
	...
	String team_name = "";
	String sql = "select oi.org_abbreviation from gp_task_project_dynamic t left join comm_org_information oi on t.org_id=oi.org_id and oi.bsflag='0' where t.bsflag='0' and t.project_info_no='"+projectInfoNo+"'";
	Map map = BeanFactory.getQueryJdbcDAO().queryRecordBySQL(sql);
	if(map!=null){
		team_name = (String)map.get("orgAbbreviation");
	}
...
<table border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
    	<td colspan="6" align="center">可重复利用物资入库</td>
    </tr>
	<tr>
		<td class="inquire_item6">入库单号：</td>
		<td class="inquire_item6">
			<input type="hidden" name="invoices_id" id="invoices_id" class="input_width" value=""  />
			<input type="text" name="input_no" id="input_no" class="input_width" value="" readonly="readonly"/></td>
		...
		<td class="inquire_item6"><input type="text"
			name="end_date" id="end_date" class="input_width"
			value="" readonly />
			<img src="<%=contextPath%>/images/calendar.gif" id="tributton1" width="16" height="16" style="cursor: hand;" onmouseover="calDateSelector(end_date,tributton1);" /></td>
	</tr>
	<tr>
		<td class="inquire_item6">单位：</td>
		<td class="inquire_item6"><input type="text" name="team_name" id="team_name"
			class="input_width" value="<%=team_name %>" /></td>
...
<script type="text/javascript"><!--
	...
	function refreshData(){
		...
		document.getElementById("end_date").value = "<gms:msg msgTag="matInfo" key="input_date"/>";
		var invoicesId="<gms:msg msgTag="matInfo" key="invoices_id"/>";
		debugger;
		var sql ="select i.wz_id,i.wz_name,i.wz_prickie,t.teammat_info_idetail_id,t.mat_num,t.actual_price,t.receive_number,t.total_money,t.warehouse_number,t.goods_allocation,t.give_out from GMS_MAT_TEAMMAT_INFO_DETAIL t inner join gms_mat_infomation i on t.wz_id = i.wz_id and i.bsflag='0' and t.invoices_id = '"+invoicesId+"'and t.project_info_no='<%=projectInfoNo%>'and t.if_accept='1'";
		cruConfig.cdtType = 'form';
		cruConfig.queryStr = sql;
		cruConfig.currentPageUrl = "/mat/singleproject/warehouse/accept/repeat/repAdd.jsp";
		queryData(1);
	}							
	function save(){	
		//if (!checkForm()) return;
		...
		document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/warehouse/accept/repeat/saveTracLedger.srq?laborId="+ids+"&invoicesId="+invoicesId;
