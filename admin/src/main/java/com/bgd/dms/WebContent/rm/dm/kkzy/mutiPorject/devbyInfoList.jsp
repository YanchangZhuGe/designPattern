...
<link rel="stylesheet" type="text/css" media="all" href="/DMS/css/calendar-blue.css" /> 
...
.input_width2 {
 ...
 <body style="background:#cdddef" onload="searchDevData()">
      	<div id="list_table">
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>
			    ...
			    <td class="ali_cdn_input" style="width:50px;">
					<img src="/DMS/images/magnifier.gif" width="16" height="16" style="cursor:pointer;" onclick="showMatTreePage()"  />
			    </td>
			    ...
			    <!-- <td align="right">
			    <a  href="/DMS/mat/singleproject/mattemplate/download.jsp?path=/rm/dm/kkzy/bywx.xls&filename=bywx.xls">下载模板</a>
			    </td> -->
			...
			<div id="table_box">
			  <table style="width:98.5%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     ...
			     <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{bywx_id}' id='rdo_entity_id_{bywx_id}'  onclick='chooseOne(this);loadDataDetail();'/>" >选择</td>
					...
					<td class="bt_info_odd" exp="{maintenance_level}" func="getOpValue,projectStatus1">保养级别</td>
			...
			<div id="tag-container_3">
			  <ul id="tags" class="tags">
			   ...
			   <li id="tag3_2" ><a href="#" onclick="getTab3(2);loaddata('',2)">故障信息</a></li>
			   <li id="tag3_3" ><a href="#" onclick="getTab3(3);loaddata('',3)">保养内容</a></li>
			  </ul>
			</div>
			...
			<div id="tab_box_content1" class="tab_box_content" style="display:none;">
						...
						<td class="bt_info_even">系列号</td><!-- 520系列号 -->
					 </tr>	
		...
		<div id="tab_box_content2" class="tab_box_content" style="display:none;">
					    ...
					    <tr>   
					    <td class="bt_info_odd">序号</td>
						<td class="bt_info_even">故障大类</td>
						<td class="bt_info_odd">故障小类</td>
						<td class="bt_info_even">故障现象</td>
						<td class="bt_info_odd">是否已解决</td>
						<td class="bt_info_even">故障解决方式</td>
						<td class="bt_info_odd">故障原因及解决方法</td>
					 </tr>	
					<tbody id="assign_body"></tbody>
				</table>
			</div>
...
<script type="text/javascript">
    ...
    function refreshData(v_self_num,s_wz_id){
		var str = "";
		str+="";
		str+=" select    (case when l.bywx_type='0' then '更换' when l.bywx_type='1' then '维修' when l.bywx_type='2' then '保养' end) as bywx_type, (case when d.coding_name is null then '无' when d.coding_name is not null then d.coding_name end)  as zcj_type, l.bywx_id,l.bywx_date,l.work_hours,l.falut_desc,l.falut_case,l.maintenance_level,l.maintenance_desc, (case  when l.performance_desc = '0' then    '良好'    when l.performance_desc = '1' then '待修'  when l.performance_desc = '2' then '待查' end) as performance_desc,l.falut_reason, l.legacy,l.repair_unit,l.repair_men,l.bak, dui.self_num, dui.dev_acc_id,dui.dev_sign  from gms_device_zy_bywx l  left join gms_device_account dui on dui.dev_acc_id = l.dev_acc_id  left join comm_coding_sort_detail d on d.coding_code_id=l.zcj_type      where l.bywx_type='2' and l.project_info_id is null and l.bsflag='0' and l.ORG_SUBJECTION_ID like 'C105006003%'";
		...
		str+="   order  by l.bywx_date desc, dui.self_num asc";
	...
	//打开新增界面
	 function toAdd(){   
	 	popWindow("<%=contextPath%>/rm/dm/kkzy/mutiPorject/acceptLedgerbyAdd.jsp",'1150:680'); 
	 }
	 ...
	 function toModifyXgll(){
			...
			popWindow('<%=contextPath%>/rm/dm/kkzy/mutiPorject/acceptLedgerbyAdd.jsp?wx_ids='+wz_ids,'1350:680');
    ...
    function loadDataDetail(shuaId){       
    	var retObj;
		if(shuaId!=null){		     
			 retObj = jcdpCallService("DevInsSrv", "getDevBywxInfos", "wx_id="+shuaId);		
			 yzjl(shuaId);
			 gzxx(shuaId);
			 bynr(shuaId);
		}else{
		    ...
		    if(ids==''){ 
		     ...
		     retObj = jcdpCallService("DevInsSrv", "getDevBywxInfos", "wx_id="+ids);
		     yzjl(ids);
		     gzxx(ids);
		     bynr(ids);
		}
	...
	*/
	function yzjl(shuaId){
        ...
        var querySql = "select info.wz_sequence,mat.wz_id,info.actual_price,info.stock_num,i.wz_name,i.wz_prickie,mat.use_num,d.coding_name,mat.wz_sequences from gms_device_zy_wxbymat mat   left join gms_device_zy_bywx l on  l.usemat_id=mat.usemat_id ";
            querySql +=" left join gms_mat_recyclemat_info info on info.wz_id=mat.wz_id and info.bsflag='0'  and info.project_info_id is null ";
		...
		if (retObj != undefined) {
			for (var i = 0; i < retObj.length; i++) {
				...
				newTr.insertCell().innerText=retObj[i].wz_sequences;//520系列号
			}
		}
		...
		$("#assign_body>tr:even>td:even",'#tab_box_content1').addClass("even_even");
		 ...
		 /**
			 *故障信息************************************************************************************************************************
			 */
		function gzxx(shuaId){
			if (shuaId != null) {
				var baseData;
				baseData = jcdpCallService("DevInsSrv", "getGzInfoByBywxid", "bywx_id="+shuaId);
				...
				if (baseData.datas!=null) {
					var retObj = baseData.datas;debugger;
					for (var i = 0; i < retObj.length; i++) {
						...
						if(usingdatas != null){
							for (var j = 0; j< usingdatas.length; j++) {
								if(baseData.datas[i].parent_falut_id==usingdatas[j].falut_id){
									newTr.insertCell().innerText = usingdatas[j].falut_name;
								}
							}
						}
						newTr.insertCell().innerText = retObj[i].falut_name;
						newTr.insertCell().innerText = retObj[i].deal_name;
						if(baseData.datas[i].falut_deal_flag=='0'){//0未解决，1已解决
							newTr.insertCell().innerText = "否";
						}else if(baseData.datas[i].falut_deal_flag=='1'){
							newTr.insertCell().innerText = "是";
						}
						if(baseData.datas[i].falut_case=='0'){
							newTr.insertCell().innerText = "现场解决";
						}else if(baseData.datas[i].falut_case=='1'){
							newTr.insertCell().innerText = "维修解决";
						}
 ...
 /**
			 *保养内容************************************************************************************************************************
			 */
		function bynr(shuaId){
			if (shuaId != null) {
				var baseData;
				baseData = jcdpCallService("DevInsSrv", "getByItemDetailByBywxid", "bywx_id="+shuaId);
				//通过查询结果动态填充使用情况select;
				var size = $("#assign_body", "#tab_box_content3").children("tr").size();
				if (size > 0) {
					$("#assign_body", "#tab_box_content3").children("tr").remove();
				}
				var jc_body2 = $("#assign_body", "#tab_box_content3")[0];
				if (baseData.byItemList!=null) {
					var retObj = baseData.byItemList;
					for (var i = 0; i < retObj.length; i++) {
					 
						var newTr = jc_body2.insertRow();
						newTr.id = retObj[i].devwork_id;					 
						newTr.insertCell().innerText = i+1; 
						newTr.insertCell().innerText = retObj[i].itemcode;
						newTr.insertCell().innerText = retObj[i].itemname;
						newTr.insertCell().innerText = retObj[i].ycbz;
					}
				}
			}
