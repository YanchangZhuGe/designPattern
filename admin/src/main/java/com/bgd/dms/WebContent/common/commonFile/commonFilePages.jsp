...
function refreshData(v_project_name, v_filename){
	var str="select f.FILE_ID,f.FILE_NAME,f.UCM_ID,f.MODIFI_DATE,f.CREATE_DATE,f.PARENT_FILE_ID,u.USER_NAME,p.PROJECT_NAME,p.PROJECT_INFO_NO, "
		+" case wf.proc_status when '1' then '待审批' when '3' then '审批通过' when '4' then '审批不通过' else '未提交' end as proc_status "
		...
		+" where f.IS_FILE='1' and f.BSFLAG='0' and f3.FILE_ABBR='<%=folder_id%>' and wf.proc_status in ('1','3','4') ";
		...
		str += "order by  f.create_date desc, wf.proc_status desc ";
...
function loadDataDetail(file_id){
	...
	processAppendInfo={ 	
		projectInfoNo:file_id.split(":")[2],
		targetCostPlanid:file_id.split(":")[0]
	};
