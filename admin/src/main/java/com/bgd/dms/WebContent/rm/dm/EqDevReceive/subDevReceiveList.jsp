...
<script type="text/javascript" src="<%=contextPath%>/js/dialog_open1.js"></script>
  ...
  <title>单项目-设备接收-设备接收(装备调配单台)-接收明细</title> 
 </head> 
			...
			<div id="table_box">
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr id='device_oif_detid_{device_oif_detid}' name='device_oif_detid'>
			     	<td class="bt_info_even" exp="<input type='checkbox' name='rdo_entity_id' value='{device_oif_detid}' id='selectedbox_{device_oif_detid}' />" ><input type='checkbox' name='rdo_entity_id' id='rdo_entity_id' onclick='check()'/></td>
...
<script type="text/javascript">
	...
	function refreshData(v_dev_name,v_dev_model){
		var str = "select dt.*,acc.dev_name,acc.dev_model,acc.dev_unit,acc.usage_org_name,acc.dev_position,"+
			...
			"where sub.device_outinfo_id='"+mixId+"'";
...
//     }   
    ...
    function loadDataDetail(shuaId){
    	var retObj;
    	var mainsql ="select dt.*,acc.dev_name,acc.dev_model,acc.dev_unit,acc.usage_org_name,"+
    	"case dt.state when '0' then '未接收' when '9' then '已接收'  else '未接收' end as state_desc, "+
		"teamid.coding_name as team_name "+
		"from gms_device_equ_outdetail dt "+
		"left join gms_device_account acc on acc.dev_acc_id=dt.dev_acc_id "+
...
// 				td.innerHTML = '<input type="checkbox" name="task_id" value="'+datas[i].id+'">';
    ...
    function deleteTableTr(tableID){
	 ...
	 function toAdd(){ 
		...
		editUrl2 = "<%=contextPath%>/rm/dm/EqDevReceive/devRecieveSubmit.jsp?id={id}&mixId="+mixId+"&mixTypeId="+mixTypeId;
	 ...
	 function toModify(){ 
		...
		editUrl2 = "<%=contextPath%>/rm/dm/EqDevReceive/devRecieveReSubmit.jsp?id={id}&mixId="+mixId;
		editUrl2 = editUrl2.replace('{id}',selId); 
		popWindow(editUrl2,'900:680'); 
	 }
	 function toBack(){
		...
		function plAdd(){ 
			...
			$("input[type='checkbox'][name='rdo_entity_id']").each(function(i){
				if(this.checked){
					...
					if(columnsObj(10).innerText=='已接收'){
						alert(columnsObj(3).innerText+"-"+columnsObj(4).innerText+"已经接收!");
						flag=1;
					}
				}
			});
		    ...
		    if(flag)return;
		 	...
		 	editUrl = "<%=contextPath%>/rm/dm/EqDevReceive/plSubmit.jsp?ids="+ids+"&mixId="+mixId+"&mixTypeId="+mixTypeId;
