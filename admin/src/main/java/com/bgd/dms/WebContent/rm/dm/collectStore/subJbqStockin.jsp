...
<script type="text/javascript">
	...
	function refreshData(v_dev_ci_name,v_dev_ci_model){
		...
		var str = "select t.device_coll_backdet_id,t.back_num as mix_num,account.dev_name,account.dev_model, case t.is_leaving when '0' then '未验收' when '1' then '已验收' end as isleaving, account.dev_unit,account.total_num,account.unuse_num,account.use_num,account.actual_in_time,unitsd.coding_name as dev_unit_name,f.alter_date from gms_device_coll_back_detail t  left join gms_device_coll_account_dui account on t.dev_acc_id2 = account.dev_acc_id left join comm_coding_sort_detail unitsd on account.dev_unit = unitsd.coding_code_id left join gms_device_coll_dym f on t.device_coll_mixinfo_id = f.device_appmix_id where t.device_coll_mixinfo_id ='<%=devicebackappid%>' ";
...
<script type="text/javascript">	
	...
	function toSumbitDevApp(){
	  ...
	  editUrl = "<%=contextPath%>/rm/dm/collectStore/sjbqSubmit.jsp?id="+selId;  
	  popWindow(editUrl);
