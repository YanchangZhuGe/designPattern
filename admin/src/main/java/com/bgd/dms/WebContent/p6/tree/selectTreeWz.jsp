...
//		e.stopEvent();//阻止默认行为，如：阻止脚本运行或者阻止事件处理函数的执行
	...
	function getSel(){
	    ...
	    var checkEndDates = "";
	    var checkOther1s = "";
	    var checkPlannedDurations = "";
	    
		for(var i=0;i<check.length;i++){
			checkTaskIds += "," + check[i].data.TaskId;
			checkNames += "," +check[i].data.Name;
			checkStartDates += "," +check[i].data.plannedStartDate;
			checkEndDates += "," +check[i].data.plannedEndDate;
			checkOther1s += "," +check[i].data.other1;
			checkPlannedDurations += "," +check[i].data.PlannedDuration;
		}
		...
		obj.PlannedDurations = checkPlannedDurations =="" ? "" : checkPlannedDurations.substr(1);
		window.close();
	}
</script>
</head>
...
<tr><td>
			<div id="inq_tool_box"><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <tr>			 
			    <td> <input type="button" value="保存" class="tj" onclick="getSel()"/></td>			    
			  </tr>
			</table>
			</td>
