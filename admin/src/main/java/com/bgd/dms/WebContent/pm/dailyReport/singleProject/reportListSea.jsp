      	...
      	<div id="list_table">
			...
			<div id="table_box">
			  ...
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">
			    <tr>
			      <td class="bt_info_odd" exp="<input type='checkbox' name='rdo_entity_id' value='{project_info_no}_{daily_no}_{produce_date}' id='rdo_entity_id_{daily_no}' onclick=doCheck(this)/>" >选择</td>
			      ...
			      <td class="bt_info_even" exp="{jp_num}" >日完成井炮</td>
			      <td class="bt_info_odd" exp="{qq_num}" >日完成气枪</td>			
			      <td class="bt_info_even" exp="{sp_num}" >日完成震源</td>
			      <%
			      if(exploration_method.equals("0300100012000000002")){
			    	  %>
			    	    <td class="bt_info_odd" exp="{workload} km"  align="center" >日完成工作量(km)</td>
			   	  <%
			      }else{
			    	  %>
			    	    <td class="bt_info_odd" exp="{workload} km²"   align="center" >日完成工作量(km²)</td>
...
<script type="text/javascript">
	...
	cruConfig.queryStr = "select daily_no,t.submit_status,t.audit_status,to_char(t.produce_date,'yyyy-MM-dd') produce_date,t.daily_acquire_sp_num sp_num,t.daily_jp_acquire_shot_num jp_num,"+
	" t.daily_qq_acquire_shot_num qq_num,(t.daily_acquire_workload-(-t.daily_jp_acquire_workload)-(-t.daily_qq_acquire_workload)) workload,t.project_info_no"+
	" from gp_ops_daily_report t   where t.bsflag ='0' and t.project_info_no ='<%=project_info_no%>'  order by t.produce_date desc";<%-- and t.exploration_method ='<%=exploration_method%>' --%>
	...
	function toDelete(){
	    ...
	    var i=1;
	    for(var i in params){
	    	dailyNos = dailyNos+","+params[i].split("_")[1];
	    	var audit_status = document.getElementById("audit_status_"+params[i].split("_")[1]).value;
		    if(audit_status == "3"){
		    	alert("有已经审批通过的，不能删除！");
		    	return;
		...
		if(confirm('确定要删除吗?')){
			...
			var dailyNo = dailyNos.substr(1);
			dailyNo = dailyNo.replace(/\,/g,"','");
			var sql ="update bgp_pm_daily_information t set t.bsflag ='1' where t.daily_no in('"+dailyNo+"')";
			retObj = jcdpCallService("QualitySrv", "saveQualityBySql", "sql="+sql);
	...
	function dbclickRow(ids){
	    ...
	    var ids1 = idss[1];
	    var ids2 = idss[2];
		popWindow('<%=contextPath%>/pm/dailyReport/singleProject/viewDailyReportSea.jsp?daily_no='+ids1+'&project_info_no='+ids+'&produce_date='+ids2,'1280:720');
	}
	...
	function toSubmit(){
	    ...
	    if(row_data_str==''){ alert("请先选中一条记录!");
	    	...
	    	var row_data = row_data_str.split("_") ;
