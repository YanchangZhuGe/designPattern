...
//增加
function toAdd(){
 	 	popWindow("<%=contextPath%>/dmsManager/device/notice/noticeEdit.jsp?org_id="+org_id+"&org_sub_id="+org_sub_id,'1024:800');
 }
//修改
function toEdit(){
	var notice_id ="";
	...
	if(isSelectedData){
		popWindow("<%=contextPath%>/dmsManager/device/notice/noticeEdit.jsp?notice_id="+notice_id,'1024:800');
	}else{
		 alert("请先选中一条记录!");
  		return;
	}
...
//</div>
	...
	function getContentTab(obj,index) { 
		...
		if(index == 0){
			...
			var ids = getSelIds('rdo_entity_name');
			if(ids!=""&&ids!=undefined){
			$("#attachement").attr("src","<%=contextPath%>/dmsManager/device/notice/common_doc_list.jsp?relationId="+ids);
			}
		}
		$(filternotobj).hide();
		$(filterobj).show();
