...
<body style="background:#fff" onload="refreshData()">
      	<div id="list_table" >
			<div id="inq_tool_box" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
			  ...
			  <%if(proc_status){ %>
			   <auth:ListButton functionId="OP_TARGET_TR_EDIT" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_TR_EDIT" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
			   <auth:ListButton functionId="OP_TARGET_TR_EDIT" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>
			   <%} %>
			  </tr>
			</table>
			</td>
			  ...
			  <table id="editionList" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" style="margin-top:2px;" >
			    	<tr class="bt_info">
			            ...
			            <td class="bt_info_even">吨位</td>		
			             ...
			             <td class="bt_info_even" onclick="getSum()">运费(元)</td>			
			        </tr>            
			   </table>
			</div>
			<div id="fenye_box">
...
var rowsCount=0;
var queryListAction = "/tcg/ajaxServiceProxyAction.srq?JCDP_SRV_NAME=OPCostSrv&JCDP_OP_NAME=getTransportShareOtherInfo&projectInfoNo=<%=projectInfoNo%>";
...
function refreshData(ids){
	...
	if(datas != null){
		...
		for (var i = 0; i< queryRet.datas.length; i++) {
    		...
    		'<INPUT id="fy'+i+'checkbox" name="rdo_entity_id"  value='+datas[i].costTransportId+' type=checkbox>';
    		
    		var td = tr.insertCell(1);
			td.innerHTML = datas[i].rownum;
			
			var td = tr.insertCell(2);
			td.innerHTML = datas[i].transportName;
			...
			td.innerHTML = datas[i].transportCount;
			
			var td = tr.insertCell(7);
			td.innerHTML = datas[i].startMeter;
			
			var td = tr.insertCell(8);
			td.innerHTML = datas[i].backLoc;
			
			var td = tr.insertCell(9);
			td.innerHTML = datas[i].backMeter;
			
			var td = tr.insertCell(10);
			td.innerHTML = datas[i].transportUnit;
...
function toAdd(){
	popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareTransportOtherEdit.upmd?pagerAction=edit2Add&projectInfoNo=<%=projectInfoNo%>");
}
function toModify(){
	...
	if (ids == '') {
		...
		popWindow(cruConfig.contextPath+"/op/costTargetShare/costTargetShareTransportOtherEdit.upmd?pagerAction=edit2Edit&id="+ids+"&projectInfoNo=<%=projectInfoNo%>");
}
function toDelete(){
	ids = getSelIds('rdo_entity_id');
	if (ids == '') {
	...
	ids = ids.replace(/\,/g,"','");
	var sql = "update bgp_op_cost_tartet_transport t set t.bsflag='1' where t.cost_transport_id in('"+ids+"')";
 ...
 function toShare(){
  ...
  function importDatas(){
	popWindow(cruConfig.contextPath+"/common/ExcelImportFile.jsp?modelName=OpTargetDeviceTransport&redirectUrl=/common/close_page.jsp");
}
  
  function getSum(){
		var project_info_no = '<%=projectInfoNo%>';
		var querySql = "select sum(nvl(to_char(t.tonnage*t.transport_count*t.transport_unit*(nvl(t.start_meter,0)+nvl(t.back_meter,0)),'9999999999999999.00'),0)) sum_value"+
			" from bgp_op_cost_tartet_transport t where t.project_info_no='<%=projectInfoNo%>'"+
			" and t.if_change ='0' and t.bsflag='0' and transport_type = '1'";
