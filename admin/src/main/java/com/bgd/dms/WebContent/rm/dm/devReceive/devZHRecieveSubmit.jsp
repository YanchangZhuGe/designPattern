...
<title>单项目-设备接收-设备接收(自有单台)-设备接收明细--综合物化探专用</title>
</head>
	      ...
	      <tr style="display:none" id="inCountry">
			...
			<input id="dev_position" name="dev_position" class="input_width" type="hidden" style="width: 60%; float: none;margin-bottom: 7px;height:20px;line-height:20px;"/>
	      	<tr>
				...
				<!-- <td class="inquire_item4">保养周期</td>
				<td class="inquire_form4">
	...
	function submitInfo(){
	   ...
	   // var maintenance_cycle_value = $("option:selected","#maintenance_cycle").text();
		...
		if(confirm("确认提交？")){
			...
			document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toZHSubmitReceive.srq?id="+id+"&devaccId="+devaccId+"&projectInfoNo="+projectInfoNo;
