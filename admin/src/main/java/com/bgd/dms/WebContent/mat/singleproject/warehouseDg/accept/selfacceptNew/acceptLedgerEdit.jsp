...
<table border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<td class="inquire_form6">
			<select id="wz_from" name="wz_from" class="select_width" disabled="disabled">
 ...
 -->	 	    <option value="1">大港分中心</option>
	...
	<div id="table_box" >
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="right" id="queryRetTable">
			<tr>
				<td class="bt_info_even" exp="<input name = 'rdo_entity_id'  type='checkbox' checked='true' value='{wz_id},{plan_flat_id}' onclick='loadDataDetail()'/>" ><input type='checkbox' name='rdo_entity_id' value='' onclick='check()'/></td>
...
<script type="text/javascript"><!--
	...
	function refreshData(){
		document.getElementById("wz_from").value = "<gms:msg msgTag="matInfo" key="wz_from"/>";
		cruConfig.submitStr ="ids="+"<gms:msg msgTag="matInfo" key="invoices_id"/>"+"&wz_from="+"<gms:msg msgTag="matInfo" key="wz_from"/>";
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getAcceptDatasDg";
		queryData(1);
	...
	function refreshData2(){
		cruConfig.queryService = "MatItemSrv";
		cruConfig.queryOp = "getAcceptDatas2Dg";
		queryData(1);
	}	
	...
	function save(){	
		...
		if(loadDataDetail()){
			openMask();
			document.getElementById("form1").action="<%=contextPath%>/mat/singleproject/warehouse/accept/selfaccept/EditAcceptLedgerDg.srq?id="+ids;
	...
	function openMatHave(){
		...
		cruConfig.submitStr ="wz_ids="+wz_id+"&ids="+invoices_id+"&wz_from="+wz_from;
	...
	function deleteMatHave(){
	   ...
	   // for(var i=0;i<tab.rows.length;i++) {
		...
		if(wz_ids!=null&&wz_ids!=""){
			cruConfig.submitStr ="wz_ids="+wz_ids+"&ids="+invoices_id+"&wz_from="+wz_from;
			refreshData2();
		}
	}
