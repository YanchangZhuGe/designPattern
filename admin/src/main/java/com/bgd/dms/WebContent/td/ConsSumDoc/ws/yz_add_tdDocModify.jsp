...
function save(){
	...
	form.action = "<%=contextPath%>/td/ws/yljcToSaveConsSumDoc.srq";
	form.submit();
	newClose();
}
...
function checkForm(){
	...
	if(fileId!='null'){ 
		if (!isTextPropertyNotNull("pk_0110000061110000013_0", "压裂监测施工总结报告")) return false;	
	}else{
		if (!isTextPropertyNotNull("0110000061110000013_0", "压裂监测施工总结报告")) return false;	
		...
		if(datas!=null && datas.length>0){
			...
			for(var i=0;i<datas.length;i++){
				if(""!=datas[i].ucm_id){
					...
					document.getElementById("pk_0110000061110000013_"+i).value =ucmId;
					var str=datas[i].file_name==""?"":datas[i].file_name.substr(0,10)+'...';
					$("#down_0110000061110000013_"+i).append("&nbsp;<a style='color:blue;'  href=<%=contextPath%>/doc/downloadDocByUcmId.srq?docId="+ucmId+"&emflag=0>"+str+"</a>");
				}
			}
		}
	}else{
...
function addRow(){
	...
	var tr=document.all.tableDoc.insertRow();
  	tr.insertCell(0).innerHTML="<input type='hidden' id='pk_0110000061110000013_"+num+"' name='pk_0110000061110000013_"+num+"' value=''/>";
  	
	var td = tr.insertCell(1);
	td.setAttribute("colspan",3);
	td.innerHTML="<input type='file' name='0110000061110000013_"+num+"' id='0110000061110000013_"+num+"' class='input_width'/><div id='down_0110000061110000013_"+num+"'></div>";
...
function delRow(){
	//获得当前点击标签的附件ucm_id值 
    var oldUcmId=$("#pk_0110000061110000013_"+event.srcElement.name).val();
    //如果存在旧文件，则先删除
					...
					<div id="tab_box" class="tab_box">
						<div id="tab_box_content0" class="tab_box_content">
							<table id="tableDoc" width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
								...
								<tr>
									<td class="inquire_item6"><span class="red_star">*</span>压裂监测施工总结报告：</td>
									<td colspan="3">
										<input type="hidden" id="pk_0110000061110000013_0" name="pk_0110000061110000013_0" value=""/>
										<input type="file" name="0110000061110000013_0" id="0110000061110000013_0" class="input_width"/>
										<div id="down_0110000061110000013_0"></div>
									</td>
									<td></td>
