...
<%
	...
	String templates = "0110000061000000073,0110000061000000070,0110000061000000053";
	...
	if(null != request.getParameter("parentFileId")){
		parentFileId = request.getParameter("parentFileId");
	}else if (resultMsg != null && null != resultMsg.getValue("parentFileId")){
			...
			<div id="table_box" >
			  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_info" id="queryRetTable">		
			     <tr>
			      ...
			      <td class="bt_info_even" exp="<a href=<%=contextPath%>/doc/downloadDocByUcmId.srq?docId={ucm_id}&emflag=0>{file_name}</a>">文档名称</td>
...
<script type="text/javascript">
	...
	function refreshData(){
		cruConfig.queryStr = "select t.file_id,t.file_name,t.ucm_id,to_char(t.upload_date,'yyyy-MM-dd') upload_date,e.employee_name,t.project_info_no,p.project_name "+
		"from bgp_doc_gms_file t left join comm_human_employee e on t.creator_id=e.employee_id and e.bsflag='0' "+
		...
		"where t.bsflag='0' and t.doc_type='"+codingCodeId+"'  and dy.org_subjection_id like '"+orgCode+"%'  order by t.upload_date desc ";
 ...
 //选择项目
   function selectTeam(){
    ...
    function toAdd(){
    	popWindow('<%=contextPath%>/td/projectDoc/add_tdDocModify.jsp?codingCodeId='+codingCodeId+'&parentFileId='+parentFileId,'800:600');
    }
	...
	function toEdit() {
		...
		popWindow('<%=contextPath%>/td/projectDoc/add_tdDocModify.jsp?id='+ids,'800:600');
	}
	function toDelete(){
	...
	function simpleSearch(){
		var s_project_info_id = document.getElementById("s_project_info_id").value;
		var file_name = document.getElementById("s_file_name").value;
		
		var str = " 1=1 ";
		if(s_project_info_id!=''){
			str += " and project_info_no = '"+s_project_info_id+"' ";
		}
		if(file_name!=''){
