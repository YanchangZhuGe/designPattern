...
<%
	...
	String mdetail_id = request.getParameter("mdetail_id");
...
<div id="list_table" > 
	   ...
	   <table width="100%" height="630px" border="1" cellpadding="0" cellspacing="0" class="tab_info" id="queryRetTable">
	   <tr class="bt_info">
	       <td colspan="5" >
	...
	function page_init(){		
		if(org_type=='2'){
			...
			if(queryRet.returnCode=='0'){
				...
				var querySql = "  select t.mdetail_id,t.pmain_id, t.ucm_id, t.creator_id,t.notes, e.employee_name,oi.org_abbreviation  as org_name from BGP_HSE_PICTURE_MDETAIL t left join comm_human_employee e on t.creator_id = e.employee_id  and e.bsflag = '0'  left  join comm_org_subjection os   on os.org_subjection_id = t.org_id  and os.bsflag = '0'  left  join comm_org_information oi    on os.org_id = oi.org_id   and oi.bsflag = '0'   where  t.bsflag='0' and t.mdetail_id='<%=mdetail_id%>'";
				var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+encodeURI(encodeURI(querySql)));
				var datas = queryRet.datas;		
				  if(queryRet.returnCode=='0'){
						if(datas != null && datas != ''  ){	
