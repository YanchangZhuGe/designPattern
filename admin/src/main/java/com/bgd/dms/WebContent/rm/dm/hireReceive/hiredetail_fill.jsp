		  ...
		  <div style="height:210px;overflow:auto">
			  <table width="97%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
		       <tr>
					...
					<td class="bt_info_even" width="16%">所在单位</td>
				</tr>
...
<script type="text/javascript"> 
	function submitInfo(){
		...
		for(var index=0;index<count;index++){
			...
			if(detdev_ci_code != "" && realstartval !="" && devsignval !=""){
		...
		if(wronglineinfos>0){
			if(confirm("确认提交？")){
				...
				document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveHireFillDetailInfo.srq?count="+count+"&checknum="+wronglineinfos;
				document.getElementById("form1").submit();
			}
		}else{
			alert("请选择设备规格型号，填写实际进场日期及实物标识号!");
		}
	...
	function refreshData(){		
		...
		for(var index=0;index<colnum;index++){
			...
			innerhtml += "<td><input name='usageorgname"+index+"' id='usageorgname"+index+"' value='"+app_org_name+"' type='text' />";
