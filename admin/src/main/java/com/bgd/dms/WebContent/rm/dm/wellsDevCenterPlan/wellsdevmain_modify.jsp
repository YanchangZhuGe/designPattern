...
<title>多项目-井中设备分中心-设备申请-修改子页面</title>
</head>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab_line_height">
        <tr>
          ...
          <td class="inquire_form4" colspan="3">
          	...
          	<input name="device_allapp_id" id="device_allapp_id" class="input_width" type="hidden" value="<%=deviceallappid%>" />
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td class="inquire_form4">
          	<input name="org_id" id="org_id" class="input_width" type="hidden" value="" readonly/>
          	<input name="org_name" id="org_name" class="input_width" type="text" value="" readonly/>
          </td>
          <td class="inquire_item4">申请人:</td>
          <td class="inquire_form4">
          	<input name="employee_id" id="employee_id" class="input_width" type="hidden" value="" readonly/>
...
<script type="text/javascript">
	function saveInfo(){
		//调配申请保存
		document.getElementById("form1").action = "<%=contextPath%>/rm/dm/toSaveDevAllAppBaseInfo.srq?state=0&wellsflag=Y&allapp_type=S10002";
	...
	function refreshData(){
		...
		var basedatas;
			//查询基本信息
			var querySql = "select devapp.org_id,devapp.employee_id,devapp.device_allapp_no,devapp.device_allapp_name,devapp.appdate,";
			querySql += "to_char(sysdate,'yyyy-mm-dd') as currentdate,org.org_name,emp.employee_name ";
			querySql += "from gms_device_allapp devapp ";
			...
			querySql += "where devapp.bsflag = '0' and devapp.allapp_type = 'S10002' and devapp.device_allapp_id='<%=deviceallappid%>'";
			var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'querySql='+querySql);
			basedatas = queryRet.datas;
			//回填基本信息
			$("#device_allapp_name").val(basedatas[0].device_allapp_name);
			$("#appdate").val(basedatas[0].appdate);
			$("#org_id").val(basedatas[0].org_id);
			$("#org_name").val(basedatas[0].org_name);
