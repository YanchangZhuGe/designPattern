...
<%
	...
	//action=="view" 审批页面
	//String shenText = request.getParameter("shenText")==null?"":request.getParameter("shenText"); //升请理由
...
<script type="text/javascript">
	...
	function refreshData(){
		cruConfig.queryStr = "select d.plan_detail_id,d.task_id,d.apply_team,s1.coding_name apply_team_name,d.post,s2.coding_name post_name,nvl(d.people_number,0) people_number,nvl(d.profess_number,0) profess_number,d.plan_start_date,d.plan_end_date,(d.plan_end_date-d.plan_start_date) nums,d.notes "+
			...
			"where d.spare1='"+planId+"' "+
			"and d.project_info_no='"+projectInfoNo+"' and d.bsflag='0' and d.resourceflag='0'";
		queryData(1);
	}

	function toAdd(){
		popWindow("<%=contextPath%>/rm/em/planning/add_planning.jsp?planId="+planId+"&mid=<%=mid%>&projectInfoNo=<%=projectInfoNo%>&projectType=<%=projectType%>&resourceFlag=0","900:750");
	}
	...
	function toEdit() {
		...
		popWindow("<%=contextPath%>/rm/em/planning/add_planning.jsp?planId="+planId+"&projectInfoNo=<%=projectInfoNo%>&projectType=<%=projectType%>&resourceFlag=0&id="+ids,"900:750");
	}
