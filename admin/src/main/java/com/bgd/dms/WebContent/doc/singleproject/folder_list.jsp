...
<%
	...
	//String rootMenuId = "8ad889f1375356d20137546f25410004";
...
var querySql = "Select b.file_id,b.file_name,b.file_abbr FROM bgp_doc_gms_file b WHERE b.project_info_no = '"+project_info_no+"' and b.bsflag='0' and b.is_file='0' and b.parent_file_id is null and b.ucm_id is null";
...
var rootMenuAbbr = queryOrgRet.datas[0].file_abbr;
...
function moveMenu(tree,node,oldParent,newParent,index){
	  ...
	  var retuObj = jcdpCallService("ucmSrv","moveTreeNodePosition","pkValue="+node.id+"&index="+index+"&oldParentId="+oldParent.id+"&newParentId="+newParent.id);
}
...
function getSubNodes(parentNode) {
	...
	Ext.Ajax.request({
		...
		params : {
			currentPage:"1",
			pageSize:"100",
			querySql:"Select b.file_id,b.file_name,b.file_abbr FROM BGP_DOC_GMS_FILE b join bgp_doc_gms_file b1 on b.parent_file_id = b1.file_abbr where b1.file_id = '"+parentNode.id+"' and b.bsflag='0' and b.is_file='0' and b.project_info_no = '"+project_info_no+"' and b.is_template is null ORDER BY b.order_num"
		},
		method : 'Post',
...
function clickNode(selectedNode){
	...
	if(selectedNode.id != undefined && selectedNode.id != ""){
		var querySql = "Select b.file_abbr,n.file_number_name,b.order_num FROM bgp_doc_gms_file b left outer join bgp_doc_file_number n on b.file_number_format = n.bgp_doc_file_number_id  WHERE b.file_id = '"+selectedNode.id+"' and b.bsflag='0' and b.is_file='0' and b.ucm_id is null";
		var queryOrgRet = syncRequest('Post',cruConfig.contextPath+appConfig.queryListAction,'querySql='+querySql);
		if(queryOrgRet.datas.length != 0){
			fileAbbr = queryOrgRet.datas[0].file_abbr; 
			numberFormat = queryOrgRet.datas[0].file_number_name; 
			orderNum = queryOrgRet.datas[0].order_num; 
		}
	}
...
function toAdd(){
	...
	if(selectedNodeId == undefined){
		selectedFolderAbbr = rootMenuAbbr;
	}
	var addParentText = needReload.text;
	var addParentTextEncode = encodeURI(encodeURI(addParentText));
	//新增刷新当前目录即可  	
	popWindow('<%=contextPath %>/doc/singleproject/add_folder.jsp?pagerAction=edit2Add&id='+selectedFolderAbbr+'&parentfoldertext='+addParentTextEncode); 
...
function toModify(){
	...
	var modifyParentText;
 	if(selectedNodeId == rootMenuId){
 		modifyParentText = rootMenuName;
 	}else{
 		modifyParentText = needReload.parentNode.text;
 	}
 	var modifyParentTextEncode = encodeURI(encodeURI(modifyParentText));
	popWindow('<%=contextPath %>/doc/singleproject/edit_folder.jsp?pagerAction=edit2Edit&id='+selectedNodeId+'&modifyfoldertext='+modifyParentTextEncode); 
}
/**
	删除目录
*/
function toDelete(){
	...
	//如果目录中有文档则提示不能删除
	var querySql = "select t.file_id,t.file_name from bgp_doc_gms_file t where t.is_file = '0' and t.bsflag = '0' and t.project_info_no is null start with t.parent_file_id = '"+selectedNodeId+"' connect by prior t.file_id = t.parent_file_id";
...
function importTemplate(){
	//检查是否已经导入了
	var querySql = "select t.file_id as projectcount from bgp_doc_gms_file t where t.project_info_no = '"+project_info_no+"' and t.bsflag = '0' and t.is_file = '0' and t.parent_file_id is null";
	var queryRet = syncRequest('Post','<%=contextPath%>'+appConfig.queryListAction,'pageSize=10000&querySql='+querySql);
	if(queryRet.datas != ""){
		alert("已经导入目录，不能再导入!");
		return;
			  ...
			  <tr>
			    ...
			    <td background="<%=contextPath%>/images/list_15.png">
			    	<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr>
					  	<td width="90%">&nbsp;</td>					    
						    <auth:ListButton functionId="F_DOC_004" css="zj" event="onclick='toAdd()'" title="JCDP_btn_add"></auth:ListButton>
						    <auth:ListButton functionId="F_DOC_005" css="xg" event="onclick='toModify()'" title="JCDP_btn_edit"></auth:ListButton>
						    <auth:ListButton functionId="F_DOC_006" css="sc" event="onclick='toDelete()'" title="JCDP_btn_delete"></auth:ListButton>		
						    <auth:ListButton functionId="F_DOC_007" css="dc" event="onclick='importTemplate()'" title="JCDP_btn_importTemplate"></auth:ListButton>			    
					  </tr>
					</table>
				</td>
			...
			<div id="tab_box" class="tab_box">
				...
				<div id="tab_box_content0" class="tab_box_content">
					<table width="100%" border="0" cellspacing="0" cellpadding="0" id="folderInfoTable" class="tab_line_height">
					  <tr>
					    <td class="inquire_item8">目录名称：</td>
					    <td class="inquire_form8" id="item0_1"><input type="text" id="folder_name" name="folder_name" class="input_width" readonly="readonly"/></td>			
					    <td class="inquire_item8">目录缩写：</td>
					    <td class="inquire_form8" id="item0_0"><input type="text" id="folder_abbr" name="folder_abbr" class="input_width" readonly="readonly"/></td>		  
					    <td class="inquire_item8">排序编号：</td>
					    <td class="inquire_form8" id="item0_0"><input type="text" id="order_num" name="order_num" class="input_width" readonly="readonly"/></td>		  	
					    <td class="inquire_item8">编号规则：</td>
					    <td class="inquire_form8" id="item0_0">
