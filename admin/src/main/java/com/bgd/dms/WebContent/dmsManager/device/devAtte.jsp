...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
		...
		function getFusionChart(){
			//设备出勤率图表
			getDevAtteFusionChart();
		}
		//获取完好率
		function getDevAtteFusionChart(){
			var orgSubId = $('#org_sub_id_1 option:selected') .val();
			var chart1 = new FusionCharts( "${applicationScope.fusionChartsURL}/Charts/Column3D.swf", "chart1", "95%", "95%", "0", "0" );    
			chart1.setXMLUrl("<%=contextPath%>/dms/device/getAtteChartData.srq?orgSubId="+orgSubId);
		...
		function toQuery1(){
			...
			chartReference1.setXMLUrl("<%=contextPath%>/dms/device/getAtteChartData.srq?orgSubId="+orgSubId+"&startDate="+startDate+"&endDate="+endDate);
		}
		//清除
		function toClear(){
			$('#org_sub_id_1') .val('');
			...
			var chartReference1 = FusionCharts("chart1"); 
			chartReference1.setXMLUrl("<%=contextPath%>/dms/device/getAtteChartData.srq");
		}
		//钻取设备类型
		function popProDevAtte(orgSubId,deviceType,startDate,endDate){
			popWindow('<%=contextPath%>/dmsManager/device/proDevAtte.jsp?orgSubId='+orgSubId+'&deviceType='+deviceType+'&startDate='+startDate+'&endDate='+endDate,'800:572');
		}
	</script>
</html>
