...
<div style="overflow-y: hidden; overflow-x: hidden;">
		...
		if(retObj!=null && retObj.returnCode==''){
			if(retObj.datas != null && retObj.datas.length>0){
				for(var i =0 ;i<retObj.datas.length ;i++){
					...
					p = p +"<font color='blue'>"+ eps_name + "</font>(一级品率≥<span >"+first_percent+"</span>%;&nbsp;累计完成炮数:<span >"+total+"</span>;&nbsp;累计完成一级品炮数:<span >"+first+"</span>);";
					if(i!=0 && i%2 ==1){
						p = p+"</p><p class='small' >";
					}
				}
	...
	function projectFirst(org_subjection_id,org_name){
		...
		window.open('<%=contextPath%>/qua/dashboard/first_project.jsp?org_subjection_id='+org_subjection_id+'&org_name='+encodeURI(org_name));
	}
	</script>
    </body>
</html>