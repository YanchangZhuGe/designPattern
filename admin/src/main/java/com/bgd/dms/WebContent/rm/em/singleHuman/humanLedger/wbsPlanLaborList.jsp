...
<script type="text/javascript">
	...
	function refreshData(taskId,projectInfoNo,taskObjectId){
		...
		cruConfig.queryStr = "select  distinct t.labor_deploy_id,        t.labor_id,        l.employee_name,        l.employee_id_code_no,        t.project_info_no,        t.start_date,        t.end_date,  d2.deploy_detail_id,        d2.apply_team,        d2.post,        d3.coding_name apply_team_name,        d4.coding_name post_name,        round( case when nvl(t.end_date, sysdate) - t.start_date > 0 then nvl(t.end_date, sysdate) - t.start_date  -(-1) else 0 end ) days   from bgp_comm_human_labor_deploy t   left join bgp_comm_human_deploy_detail d2 on t.labor_deploy_id = d2.labor_deploy_id and d2.bsflag='0'  left join bgp_comm_human_labor l on t.labor_id = l.labor_id   left join comm_coding_sort_detail d3 on d2.apply_team = d3.coding_code_id   left join comm_coding_sort_detail d4 on d2.post = d4.coding_code_id  left join bgp_comm_human_receive_labor lr     on lr.labor_id=l.labor_id   where t.bsflag = '0' and  lr.task_id in (select d.id || '' from (select c.* from (select a.object_id as id, a.id as task_id, a.name, a.wbs_object_id as parent_id, 'true' as is_activity from bgp_p6_activity a union all select b.object_id as id, b.code as task_id, b.name, b.parent_object_id as parent_id, 'false' as is_activity from bgp_p6_project_wbs b) c start with c.id = '"+taskObjectId+"' connect by prior c.id = c.parent_id ) d where d.is_activity='true' )  and t.project_info_no =  '"+projectInfoNos+"'  and l.if_engineer='<%=laborCategory%>' and t.start_date is not null and  t.end_date is null ";
		cruConfig.currentPageUrl = "/rm/em/singleHuman/humanLedger/wbsPlanLaborList.jsp";
		queryData(1); 
	}
