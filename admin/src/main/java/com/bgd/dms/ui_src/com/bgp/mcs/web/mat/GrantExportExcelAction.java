...
public class GrantExportExcelAction extends WSAction {
    ...
    throws Exception{
    	...
    	if(datas!=null){
    		String[] titles = {"物资编码","物资说明","计量单位","数量","单价","金额","发料库","货位","收料库","需求计划单号","计划备注"};
    		String[] names={"wz_id","wz_name","wz_prickie","mat_num","actual_price","total_money","mat_sourth","goods_allocation","warehouse_number","plan_no","note"};
...
//    		borderStyle.setBorderTop(HSSFCellStyle.BORDER_THIN);// 上边框   
    		...
    		cell0.setCellValue(""+headMap.get("org_abbreviation")+"出库单");
    		cell0.setCellStyle(style);
    		
    		HSSFRow row1 = sheet.createRow((short)1);//建立新行
    		sheet.addMergedRegion(new Region(1, (short) 0, 1,(short) 10));
    		HSSFCell cell1 = row1.createCell(0);
    		String cellData1="     库存项目："+headMap.get("project_name")+"                                          发料时间："+headMap.get("outmat_date")+"                                 出库单号："+headMap.get("procure_no")+"";
    		...
    		cell.setCellValue("开票员："+headMap.get("drawer")+"                                                                          保管员: "+headMap.get("storage")+"                                                提料人:"+headMap.get("pickupgoods")+"          ");
//    		
//    		// 设置列宽，自动 
    		for(int j=0;j<titles.length;j++){
...
//			response.setHeader("Content-Disposition","attachment;  filename=" + "wzOut" + ".xls");
			response.setHeader("Content-Disposition","attachment;  filename=" +URLEncoder.encode("出库单", "utf-8") + ".xls");
