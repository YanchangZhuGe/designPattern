...
public class Configurations {
	...
	//默认文件上传路径
	private static final String REPOSITORY = WebPathUtil.getWebProjectPath() + File.separator+ WebPathUtil.file_path;
//			System.getProperty(
...
//					+ "upload-repository");
	...
	public static String getFileRepository(UserToken userToken) {
		String val = getConfig("STREAM_FILE_REPOSITORY");
		if (val == null || val.isEmpty())
			val = REPOSITORY + File.separator + userToken.getEmpId();
		else
			 val =  val + File.separator + userToken.getEmpId();
		return val;
	}
