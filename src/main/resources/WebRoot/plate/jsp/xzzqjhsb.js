 ...
 */
if (is_zxzq == '1') {
    sbpc_store.proxy.extraParams['BOND_TYPE'] = '02';
} else if (is_zxzq == '2') {
    sbpc_store.proxy.extraParams['BOND_TYPE'] = '01';
} else {
    sbpc_store.proxy.extraParams['BOND_TYPE'] = '01';
}
sbpc_store.proxy.extraParams['BATCH_YEAR'] = SET_YEAR;
sbpc_store.proxy.extraParams['AD_CODE'] = AD_CODE;
sbpc_store.proxy.extraParams['is_fxjh'] = is_fxjh;
sbpc_store.proxy.extraParams['TYPE'] = 'jhsb';
sbpc_store.load();
        ...
        '001': [
            ...
            },
            {
                ...
                handler: function (btn) {
                    ...
                    // 获取选中数据
                    var records;
                    if (is_fxjh == '1' || is_fxjh == '3' || is_fxjh == '4' || is_fxjh == '5') {//限额库上报
        ...
        '002': [
            ...
            },
            {
                ...
                handler: function (btn) {
                    ...
                    var ids = [];
                    for (var i in records) {
                        if (is_fxjh == '1' || is_fxjh == '3' || is_fxjh == '4' || is_fxjh == '5') {//限额库
                            ids.push(records[i].get("ID"));//取明细id
                        } else {
                            ids.push(records[i].get("HZ_ID"));
                        }
    ...
    items_content_rightPanel_dockedItems: [
        {
            ...
            items: [
                ...
                },
                {
                    ...
                    displayField: 'name',
                    valueField: 'id',
                    store: DebtEleStoreDB('DEBT_YEAR', {condition: " and code >= '2015'"}),//20210827liyue下级项目上报页面批次，应与发行库遴选时申报批次一致
                    listeners: {
                        change: function (self, newValue) {
                            sbpc_store.proxy.extraParams['BATCH_YEAR'] = newValue;
                            sbpc_store.proxy.extraParams['AD_CODE'] = AD_CODE;
                            sbpc_store.proxy.extraParams['is_fxjh'] = is_fxjh;
                            sbpc_store.proxy.extraParams['TYPE'] = 'jhsb';
                            sbpc_store.load();
                ...
                },
                {
                    ...
                    /*  store: DebtEleTreeStoreDB('DEBT_FXPC',{condition:" and (EXTEND2 IS NULL OR EXTEND2 = '"+((is_fxjh=='1'||is_fxjh=='3'||is_fxjh=='4')?"1":"0")+"')" +
                              "and (EXTEND1 like '01%' or EXTEND1 !='')"}),//20201103_wangyl_追加再融资批次过滤条件*/
                    store: sbpc_store,
                    editable: false,
                    labelWidth: 70,
