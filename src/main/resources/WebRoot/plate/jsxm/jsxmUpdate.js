...
var tab_items_load = {'jbqk': {}, 'tzjh': {}, 'szys': {},/*'clzw':{},'xmzc':{},*/'ztb': {}, 'jsjd': {}, 'sjsy': {}};
...
function initContentTabGrid() {
    return Ext.create('Ext.tab.Panel', {
        ...
        items: [
            {title: '基础库', opstatus: 1, items: [initContentGrid("contentGrid")]},
            {title: '储备库', opstatus: 2, items: [initContentGrid("contentGrid2")]},
            {title: '发行库', opstatus: 3, items: [initContentGrid("contentGrid3")]},
            {title: '存续期', opstatus: 4, items: [initContentGrid("contentGrid4")]},
        ],
        listeners: {
...
function submitInfo() {
    ...
    if (isOld_szysGrid == '1') {
        ...
        for (var i = 0; i <= yyqx_count; i++) {
            var SR = "SRAMT_Y" + i;
            var ZC = "ZCAMT_Y" + i;
            zxzqhb_amt += record_06.get(ZC);
            zxzq_amt += record_0202.get(SR);
            schrzhb_amt += record_07.get(ZC);
            schrz_amt += record_0203.get(SR);
            ...
            if (parseFloat(cwzyzx + cwzysc).toFixed(6) - parseFloat(jsxmzc).toFixed(6) > 0.000001) {//20210420liyue添加建设项目支出与其中金额校验
                Ext.Msg.alert('提示', '其中：专项债券与市场化融资付息和和不能大于项目建设支出！');
                return false;
            }
        }
        if (is_tdcb == false) {
            if (Math.abs(((zxzqhb_amt).toFixed(6) - (zxzq_amt).toFixed(6)).toFixed(6)) > 0.000001) {
        ...
        szysGrid.getStore().each(function (record) {
            for (var j = 0; j < yyqx_count; j++) {
                var SR = "SRAMT_Y" + j;
                var ZC = "ZCAMT_Y" + j;
                var curry_year_sramt = record.get(SR); // 当前年度收入金额
                var curry_year_zcamt = record.get(ZC); // 当前年度支出金额

                /* if ( curry_year_sramt < 0) {
                     message_error = '明细收入或者支出金额不允许存在负数';
                     is_negative = true;
                     break;
                 }

                 if (curry_year_zcamt < 0) {
                     message_error = '明细收入或者支出金额不允许存在负数';
                     is_negative = true;
                     break;
                 }*/
