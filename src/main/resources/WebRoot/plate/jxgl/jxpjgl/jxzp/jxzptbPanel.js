...
function jxzp(xm_id, button_text, view_id) {
    var HEADERJSON = [
        ...
        {
            field: 'SJ_TARGET_VALUE',
            width: '16%',
            title: '<span class="required">✶</span>实际完成值',
            style: 'background-color: #F2F2F2;',
            templet: function (d) {
                if (d.LEVEL_NO == '03' && d.TV_TYPE == 1) {
                    var sjTargetValue = parseFloat(d.SJ_TARGET_VALUE).toFixed(2);
    ...
    var edit = {
        'edit(jxzp_table)': function (obj) {
            ...
            if (obj.field == 'SCORE_VALUE') {
                ...
                // 当前只有分值，没有权重逻辑
                $(this).parent().next().find('p').text(parseFloat(obj.value).toFixed(2));
                json_dfz[obj.data.DTL_ID] = parseFloat(obj.value).toFixed(2);
