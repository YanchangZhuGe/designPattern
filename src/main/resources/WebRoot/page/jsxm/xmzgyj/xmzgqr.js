                ...
                handler: function (btn) {
                    ...
                    for(var i=0;i<records.length;i++){
                        if(isNull(records[i].get("OP_CONTENT"))){
                            Ext.MessageBox.alert('提示', '项目名称：'+records[i].get("XM_NAME")+'  还未录入确认意见，请录入确认意见后再次下发！');
                            btn.setDisabled(false);
                            return;
                        }
                    }
        ...
        tbar: [
            {
                ...
                fieldLabel: '状态',
                name: 'WF_STATUS',
                store: DebtEleStore(json_debt_ztqr),
                width: 150,
                labelWidth: 30,
                labelAlign: 'right',
                displayField: "name",
...
function next(btn) {
    ...
    $.post("/xmzg/doWorkFlowAction.action", {
        ...
        if (data.success) {
            ...
            //操作日志
            saveLog('项目整改确认', 'BUTTON', '项目整改' + msg_success, '0');
        } else {
            btn.setDisabled(false);
...
function saveInfo(btn, b_btn, XM_ID, ZGXX_ID) {
    ...
    if(getStringLength(OP_CONTENT)>500){
        result = result + '确认意见过长，最多只能输入250个汉字！';
    }
    if (result == '') {
        ...
        form.submit({
            ...
            failure: function (form, action) {
                ...
                Ext.Msg.alert('提示', "确认失败！" + result ? result.message : '无返回响应');
                DSYGrid.getGrid("contentGrid").getStore().loadPage(1);
                DSYGrid.getGrid("contentGrid_detail").getStore().removeAll();
                Ext.ComponentQuery.query('window[name="xmzgWin"]')[0].close();
            }
