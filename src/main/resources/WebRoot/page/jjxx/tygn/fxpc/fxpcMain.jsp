...
<script>
        ...
        var tbar = [
            {
                ...
                handler: function (btn) {
                    flag_edit = true;
                    refreshButtonStatus();
                    store_xepc = Ext.create('Ext.data.ArrayStore', {
                        fields: ['NAME', 'GUID', 'CODE'],
                        proxy: {// ajax获取后端数据
                            type: "ajax",
                            method: "POST",
                            url: '/tygn/getTygnXepcGrid.action',
                            reader: {
                                type: "json",
                                root: "list"
                            },
            ...
            },
            {
                ...
                handler: function (btn) {
                    ...
                    $.post('/tygn/saveTygnFxpcGrid.action', {
    ...
    function initContentGrid() {
        var headerJson = [
            ...
            {
                dataIndex: "XEPC_NAME", itemId: "XEPC_NAME", type: "string", text: "限额批次",
             ...
             if (store_xepc != null) {
             var xepcRecord = store_xepc.findRecord('GUID', value, 0, true, true, true);
             return xepcRecord == null ? value : xepcRecord.get('NAME');
             } else {
             return value;
             }
             }*/
            },
			{dataIndex: "XEPC_CODE", itemId: "XEPC_CODE", type: "string", text: "限额批次CODE",hidden:true},
			{dataIndex: "XEPC_ID", itemId: "XEPC_ID", type: "string", text: "限额批次ID",hidden:true}
        ];
        return DSYGrid.createGrid({
            ...
            height: '100%',
            flex: 1,
            dataUrl: '/tygn/getTygnFxpcGrid.action',
            pageConfig: {
                enablePage: false
            },
            plugins: [
                {
                    ...
                    listeners: {
                        edit: function (editor, context) {
                            ...
                            //选择限额批次后存储限额其他字段
                            if (context.field == 'XEPC_NAME') {
                                var xepcRecord = store_xepc.findRecord('GUID', context.value, 0, true, true, true);
                                if (xepcRecord == null) {
                                    context.record.set('XEPC_ID', '');
                                    context.record.set('XEPC_CODE', '');
                                    context.record.set('XEPC_NAME', '');
                                } else {
                                    context.record.set('XEPC_ID', context.value);
                                    context.record.set('XEPC_CODE', xepcRecord.get('CODE'));
                                    context.record.set('XEPC_NAME', xepcRecord.get('NAME'));
                                }
                            }
                        },
                        validateedit: function (editor, context) {
                            ...
                            /*if (context.field == 'XEPC_NAME' && context.value != null && context.value != '') {
                                var codes = context.grid.getStore().queryRecords('XEPC_ID', context.value);
                                if (codes != null && codes.length > 0) {
                                    if (codes.length > 1 || codes[0].id != context.record.id) {
                                        Ext.Msg.alert('提示', context.record.data.NAME + '批次关联限额批次编码重复！');
                                        return false;
                                    }
                                }
                            }*/
    ...
    function updateFxpcStatus(btn) {
        ...
        //发送ajax请求，启用数据
        $.post("/tygn/updateTygnFxpcStatus.action", {
         ids: ids,
         STATUS: btn.status
         }, function (data) {
         if (data.success) {
