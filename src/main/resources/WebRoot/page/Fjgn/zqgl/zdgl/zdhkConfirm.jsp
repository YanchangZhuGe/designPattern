   ...
   /* var WF_STATUS = getQueryParam("WF_STATUS");//当前状态
    ...
    function initContentGrid() {
        ...
        return DSYGrid.createGrid({
            ...
            dataUrl: 'getZdHkConfirmMainGridData.action',
            checkBox: true,
            border: false,
            autoLoad: true,
            /*dockedItems: [
    ...
    function initWin_input_form() {
        return Ext.create('Ext.form.Panel', {
            ...
            items: [
                ...
                {
                    fieldLabel: "<span class=\"required\">✶</span>还款单号", name: "HK_NO", width: 150, fieldCls: null, readOnly: false
                },
                {
                    fieldLabel: '<span class="required">✶</span>实际到账日期',
                    name: "HK_DZ_DATE",
                    ...
                    readOnly: true,
                    // format: 'Y-m-d',
                    hidden: WF_STATUS=='002'&&IS_VIEW?false:true
                },
                {
                    fieldLabel: '备注',
                    name: "REMARK",
    ...
    function doWorkFlow_DZDATE(btn,dz_date) {
        ...
        for (var i in records) {
            ...
            zdhkInfoArray.push(array);
        }
        button_name = btn.text;
        //弹出对话框
        Ext.Msg.confirm("提示", "是否" + button_name + "?", function (op) {
            if (op == 'yes') {
                $.post("/confirmZdhkxxNode.action", {
                    workflow_direction: btn.name,
                    button_name: button_name,
                    ids: ids,
                    wf_id: wf_id,
                    node_code: node_code,
                    zdhkInfoArray: Ext.util.JSON.encode(zdhkInfoArray),
