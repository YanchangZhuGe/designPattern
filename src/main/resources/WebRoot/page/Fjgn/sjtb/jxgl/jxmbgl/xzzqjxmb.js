 ...
 * @param BILL_ID
 */
function initWindow_contentForm_tab_jxmb_lnTbCk(XM_ID, MB_ID) {
    Ext.create('Ext.window.Window', {
        ...
        closeAction: 'destroy',
        items: initWindow_zqxxtb_contentForm_tab_jxmb("lnjxmbForm", 'lnTreeGrid_Table', XM_ID, MB_ID, true, true, ''),
        buttons: [
            {
                text: '取消',
 ...
 */
function initWindow_contentForm_tab_jxmb(XM_ID, MB_ID, mbYear, xmlx_id) {
    return initWindow_zqxxtb_contentForm_tab_jxmb('jxmbForm', 'treeGrid_Table', XM_ID, MB_ID, false, false, mbYear, xmlx_id);
}
 ...
 * @returns {Ext.form.Panel}
 */
function initWindow_contentForm_tab_jxmb_yhs(XM_ID, MB_ID) {
    ...
    return initWindow_zqxxtb_contentForm_tab_jxmb(itemId, treeGrid_Table, XM_ID, MB_ID, true, false, '');
}
 ...
 * @returns {Ext.form.Panel}
 */
function initWindow_zqxxtb_contentForm_tab_jxmb(itemId, treeGrid_Table, XM_ID, MB_ID, isNoEditable, isLnTbCk, mbYear, xmlx_id) {
                ...
                items: [
                    {
                        ...
                        listeners: {
                            afterrender: function () {
                                jxmb(XM_ID, MB_ID, isNoEditable, treeGrid_Table, mbYear, xmlx_id);
                            }
                        }
                    }
                ]
 ...
 * @param mbYear 总体目标年度
 */
function jxmb(XM_ID, MB_ID, isNoEditable, treeGrid_Table, mbYear, xmlx_id) {
    var HEADERJSON = [
        {
        ...
        {field: 'WEIGHT_VALUE', width: 100, title: '权重'},
        {
            ...
            templet: function (d) {
                if (d.TV_TYPE == 1 && d.TARGET_VALUE) {
                    return new BigNumber(d.TARGET_VALUE).toFixed(2);
                }
        ...
        {field: 'DLDW_ID', width: 400, title: '度量单位ID', style: 'background-color: #F2F2F2;'},
        {
            ...
            templet: function (d) {
                if (d.DLDW_NAME) {
                    return d.DLDW_NAME;
                } else if (d.IS_COMMON_IND == 2 && d.LEVEL_NO == '03' && !d.DLDW_NAME) {
                    return "请选择";
                } else {
                    return '';
                }
        ...
        selectData: dldwDatas.data, // 下拉框数据源
            ...
            if (!isNoEditable) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    let row = data[i];
                    // 修改时，新增指标和指标库挑选指标可修改  1:新增指标  2:已同步指标  3:指标库挑选指标
                    if (row['IS_NEW'] == 1 || row['IS_NEW'] == 3 || row['IS_NEW'] == 2) {
                    ...
                    if (row['LEVEL_NO'] == '03') {
                        if (row['TV_TYPE'] == '1') {
                            // 数字
                            $(".layui-table").find('tr[data-index=' + i + ']').find('td[data-field=' + 'TARGET_VALUE' + ']').prepend("<div class='tdpd'>请输入数字</div>")
                        } else {
                            // 字符串
                            $(".layui-table").find('tr[data-index=' + i + ']').find('td[data-field=' + 'TARGET_VALUE' + ']').prepend("<div class='tdpd'>请输入字符</div>")
                        }
                        if (isNull(row['TARGET_VALUE'])) {
    ...
    var edit = {
        'edit(treeGrid_Table)': function (obj) {
            if (obj.field == 'IND_NAME' && obj.value.toString().length > 60) {
                Ext.toast({
                    html: "指标名称输入内容过长！",
                    closable: false,
                    align: 't',
            ...
            if (obj.field == 'TARGET_VALUE') {
                if (obj.data.TV_TYPE == 1) {
                    if (isNaN(obj.value) || isNull(obj.value)) {
                        Ext.toast({
                            html: "绩效目标名称为" + obj.data.IND_NAME + "的目标值必须为数值！",
                            closable: false,
                            align: 't',
                            slideInDuration: 400,
                            minWidth: 400,
                        ...
                        Ext.toast({
                            html: "绩效目标名称为" + obj.data.IND_NAME + "的目标值必须大于0！",
                            closable: false,
                            align: 't',
                            slideInDuration: 400,
                            minWidth: 400,
...
function add(obj) {
    ...
    tableStyle(index);
    // 字符串
    $(".layui-table").find('tr[data-index=' + index + ']').find('td[data-field=' + 'TARGET_VALUE' + ']').prepend("<div class='tdpd'>请输入字符</div>")
}
    ...
    var headerJson = [
        ...
        },
        {
            ...
            renderer: function (data, cell, record) {
                ...
                var mb_id = record.data.MB_ID;
                var result = '<a href="#" onclick="initWindow_contentForm_tab_jxmb_lnTbCk(\'' + xmId + '\',\'' + mb_id + '\');" style="color:#3329ff;">' + data + '</a>';
                return result;
            }
        }
    ];
...
function addzbk(Obj, record) {
    ...
    // 添加行表格样式
    tableStyle(index);
    if (record.data.TV_TYPE == '1') {
        // 数字
        $(".layui-table").find('tr[data-index=' + index + ']').find('td[data-field=' + 'TARGET_VALUE' + ']').prepend("<div class='tdpd'>请输入数字</div>")
    } else {
        // 字符串
        $(".layui-table").find('tr[data-index=' + index + ']').find('td[data-field=' + 'TARGET_VALUE' + ']').prepend("<div class='tdpd'>请输入字符</div>")
    }
...
function getJxmbtbAllDatas() {
    ...
    for (var i = 0; i < jxmbtb_treetable.length; i++) {
        ...
        if (!isNull(zb_name) && zb_name.length > 60) {
            return false;
        }
        ...
        if (!isNull(zb_targetValue) && zb_tvType == 1) {
            if (isNaN(zb_targetValue)) {
                return false;
            } else if (Math.round(parseFloat(zb_targetValue) * 100) / 100 <= 0) {
                return false;
            }
        }
        ...
        if (!isNull(zb_targetValue) && zb_targetValue.length > 2000) {
            Ext.toast({
                html: "绩效目标名称为" + zb_name + "的绩效目标值输入内容过长!",
                closable: false,
                align: 't',
                slideInDuration: 400,
                minWidth: 400
        ...
        if (!isNull(zb_pfbz) && zb_pfbz.length > 2000) {
            Ext.toast({
                html: "绩效目标名称为" + zb_name + "的指标解释输入内容过长!",
                closable: false,
                align: 't',
                slideInDuration: 400,
                minWidth: 400
        ...
        if (!isNull(zb_zbsm) && zb_zbsm.length > 2000) {
            Ext.toast({
                html: "绩效目标名称为" + zb_name + "的指标说明输入内容过长!",
                closable: false,
                align: 't',
                slideInDuration: 400,
                minWidth: 400
    ...
    var jxmbObj = {};
    jxmbObj.NDZTMB = NDZTMB;
    jxmbObj.jxmbGrid = jxmbtb_treetable;
    return jxmbObj;
}
