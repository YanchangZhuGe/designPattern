...
function initZbtxTree() {
    ...
    var xmlxStore = Ext.create('Ext.data.TreeStore', {
        ...
        proxy: {
            ...
            extraParams: {"year": CURRENT_YEAR, "IND_TYPE": IND_TYPE, "XMLX_ID": XMLX_ID_SEL, "isXd": 0},
            reader: {
                type: 'json'
            }
        },
...
function appoint() {
    ...
    var msgTitleUnSel = '将发布当前年度未发布的绩效' + (IND_TYPE == 1 ? '评价' : '目标') + '指标，请确认是否发布！';
...
function cancelAppoint() {
    ...
    var msgTitleUnSel = '将撤销发布当前年度已发布的绩效' + (IND_TYPE == 1 ? '评价' : '目标') + '指标，请确认是否撤销！';
